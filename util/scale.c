...
static int32_t scale(CSOUND *csound, int32_t argc, char **argv) {
  ...
  do {
    ...
    if (*s++ == '-') /* read all flags:  */
      ...
      usage(csound, Str("too many arguments"));
  } while (--argc);
...
retry:
  ...
  if (factor != 0.0 || factorfile != NULL) { /* perform scaling */
    ...
    if (strcmp(O.outfilename, "stdout") == 0 ||
      ...
      if (outfile != NULL) {
        if (UNLIKELY((fd = csound->CreateFileHandle(
                          csound, &outfile, CSFILE_SND_W, "stdout")) == NULL)) {
          sflib_close(outfile);
      ...
      fd = csound->FileOpen2(
          csound, &outfile, CSFILE_SND_W, O.outfilename, &sfinfo, "SFDIR",
    ...
    csound->Message(csound, Str("writing %d-byte blks of %s to %s %s\n"),
                    (int32_t)outbufsiz, csound->getstrformat(O.outformat),
...
static void InitScaleTable(CSOUND *csound, SCALE *thissc, double factor,
                           char *factorfile) {
  if (factor != 0.0)
    thissc->ff = factor;
  else {
    ...
    double x, y;
    if (UNLIKELY(csound->FileOpen2(csound, &f, CSFILE_STD, factorfile, "r",
...
static SNDFILE *SCsndgetset(CSOUND *csound, SCALE *thissc, char *inputfile) {
  SNDFILE *infile;
  double dur;
  SOUNDIN *p;
  ...
  csound->Message(csound, "%s %" PRId64 " %s (%3.1f secs)\n", Str("scaling"),
                  p->getframes, Str("sample frame"), dur);
  return (infile);
}

#define BUFFER_LEN (1024)

static void ScaleSound(CSOUND *csound, SCALE *thissc, SNDFILE *infile,
  ...
  minpos = 0;
  mintimes = 0;
  while ((read_in = csound->getsndin(csound, infile, buffer, bufferLenSamples,
                                     thissc->p)) > 0) {
    for (i = 0; i < read_in; i++) {
...
int32_t scale_init_(CSOUND *csound) {
  ...
  if (retval)
    return retval;
  return csound->SetUtilityDescription(
      csound, "scale", Str("Reports and/or adjusts maximum gain"));
}
