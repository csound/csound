...
static int mixer_main(CSOUND *csound, int argc, char **argv) {
  ...
  SFLIB_INFO sfinfo;
  MIXER_GLOBALS *pp =
      (MIXER_GLOBALS *)csound->Calloc(csound, sizeof(MIXER_GLOBALS));
  ...
  mixin[n].non_clear = 0;
  mixin[n].fulltable = NULL;
  mixin[n].use_table = 0;
  for (i = 1; i < 5; i++)
    mixin[n].channels[i] = 0;
  if (UNLIKELY(!(--argc)))
  ...
  for (i = 0; i < n; i++) {
    if (UNLIKELY(!MXsndgetset(csound, &mixin[i]))) {
      csound->ErrorMsg(csound, Str("%s: error while opening %s"), argv[0],
                       inputfile);
      return -1;
    }
  ...
  if (strcmp(O.outfilename, "stdout") == 0) {
    ...
    if (outfd != NULL) {
      if (UNLIKELY(csound->CreateFileHandle(csound, &outfd, CSFILE_SND_W,
                                            "stdout") == NULL)) {
        sflib_close(outfd);
        return -1;
      }
    }
  } else if (csound->FileOpen2(
                 csound, &outfd, CSFILE_SND_W, O.outfilename, &sfinfo, "SFDIR",
                 csound->type2csfiletype(O.filetyp, O.outformat), 0) == NULL)
    outfd = NULL;
  ...
  csound->Message(csound, Str("writing %d-byte blks of %s to %s (%s)\n"),
                  pp->outbufsiz, csound->getstrformat(O.outformat),
                  O.outfilename, csound->type2string(O.filetyp));
...
static void InitScaleTable(MIXER_GLOBALS *pp, int32_t i) {
  ...
  if (UNLIKELY(csound->FileOpen2(csound, &f, CSFILE_STD, mixin[i].fname, "r",
                                 NULL, CSFTYPE_FLOATS_TEXT, 0) == NULL)) {
    csound->Die(csound, Str("Cannot open scale table file %s"), mixin[i].fname);
    return; /* not reached */
  }
...
static MYFLT gain(MIXER_GLOBALS *pp, int32_t n, int32_t i) {
  ...
  if (!mixin[n].use_table)
    return mixin[n].factor;
  if (i < mixin[n].table->x0)
    mixin[n].table = mixin[n].fulltable;
  while (i < mixin[n].table->x0 ||
    ...
    if (UNLIKELY(pp->debug))
      csound->Message(csound, "Table %d: %d (%d %f) -> %d %f [%f]\n", n, i,
                      mixin[n].table->x0, mixin[n].table->y0,
...
static SNDFILE *MXsndgetset(CSOUND *csound, inputs *ddd) {
  ...
  csound->Message(csound, "%s %" PRId64 " %s (%3.1f secs)\n", Str("mixing"),
                  p->getframes, Str("sample frames"), dur);
  ddd->fd = infd;
  return infd;
}
...
static void MixSound(MIXER_GLOBALS *pp, int32_t n, SNDFILE *outfd, OPARMS *O) {
  ...
  inputs *mixin = &(pp->mixin[0]);
  MYFLT *buffer =
      (MYFLT *)csound->Calloc(csound, sizeof(MYFLT) * 6 * NUMBER_OF_SAMPLES);
  MYFLT *ibuffer =
      (MYFLT *)csound->Calloc(csound, sizeof(MYFLT) * 6 * NUMBER_OF_SAMPLES);
  long read_in;
  MYFLT tpersample;
  ...
  while (more_to_read) {
    ...
    for (i = 0; i < n; i++) {
      if (sample >= mixin[i].start) {
        ...
        if (mixin[i].non_clear) {
          ...
          for (k = 1; k <= mixin[i].p->nchanls; k++) {
            for (j = 0; j < read_in; j++) {
              buffer[j * outputs + k - 1] += ibuffer[j * outputs + k - 1] *
                                             gain(pp, i, sample + j + k - 1);
            }
          }
          mixin[i].fulltable = mixin[i].table;
        }
    ...
    for (j = 0; j < this_block * outputs; j++) {
      ...
      if (buffer[j] == min)
        mintimes++;
      if (buffer[j] > max)
        max = buffer[j], lmaxpos = sample + j, maxtimes = 1;
      if (buffer[j] < min)
        min = buffer[j], lminpos = sample + j, mintimes = 1;
    }
  ...
  csound->Message(csound,
                  Str("Max val %d at index %ld (time %.4f, chan %d) "
                      "%d times\n"),
                  (int32_t)max, lmaxpos, tpersample * (lmaxpos / outputs),
                  (int32_t)lmaxpos % outputs, maxtimes);
  csound->Message(csound,
                  Str("Min val %d at index %ld (time %.4f, chan %d) "
                      "%d times\n"),
