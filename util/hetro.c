...
static int32_t hetro(CSOUND *csound, int32_t argc, char **argv) {
  ...
  if (UNLIKELY((t->smpsin =
                    csound->getsndin(csound, infd, t->auxp, nsamps, p)) <= 0)) {
    char errmsg[256];
...
#endif
      if (UNLIKELY(t->num_pts > 32767 || t->num_pts >= nsamps - t->windsiz))
    return quit(csound, Str("number of output points is too great"));
  t->delta_t = FL(1.0) / t->sr;
  t->t = FL(1.0) / t->fund_est;
  t->outdelta_t = (MYFLT)t->num_pts / (t->smpsin - t->windsiz);
  ...
  t->c_p = (double *)dsp;
  dsp += smpspc; /* space for the    */
  t->s_p = (double *)dsp;
  dsp += smpspc; /* quadrature terms */
  begbufs = (double *)dsp;
  t->cos_mul = (double *)dsp;
  dsp += bufspc; /* bufs that will be */
  t->sin_mul = (double *)dsp;
  dsp += bufspc; /* refilled each hno */
  t->a_term = (double *)dsp;
  dsp += bufspc;
  t->b_term = (double *)dsp;
  dsp += bufspc;
  ...
  t->ph_av3 = (double *)dsp;
  dsp += bufspc;
  t->r_phase = (double *)dsp;
  dsp += bufspc;
  t->amp_av1 = (double *)dsp;
  dsp += bufspc;
  t->amp_av2 = (double *)dsp;
  dsp += bufspc;
  t->amp_av3 = (double *)dsp;
  dsp += bufspc;
  t->a_avg = (double *)dsp;
  dsp += bufspc;
  endbufs = (double *)dsp;
  ...
  t->MAGS = (MYFLT **)csound->Malloc(csound, t->hmax * sizeof(MYFLT *));
  t->FREQS = (MYFLT **)csound->Malloc(csound, t->hmax * sizeof(MYFLT *));
  for (i = 0; i < t->hmax; i++) {
    t->MAGS[i] = (MYFLT *)dsp;
    dsp += mgfrspc;
    t->FREQS[i] = (MYFLT *)dsp;
...
static double GETVAL(HET *t, double *inb,
                     int32 smpl) { /* get value at position smpl in array inb */
  if (smpl < 0)
    return 0.0;
  return inb[(smpl + t->midbuf) & t->bufmask];
}

static void PUTVAL(HET *t, double *outb, int32 smpl,
                   double value) { /* put value in array outb at postn smpl */
  outb[(smpl + t->midbuf) & t->bufmask] = value;
}

static int32_t hetdyn(CSOUND *csound, HET *t,
                      int32_t hno) /* HETERODYNE FILTER */
{
  int32 smplno;
...
{ /* 3rd order butterworth LPF coefficients calculated using */
  ...
  t->x2 =
      (MYFLT)(omega_c * t->delta_t *
              (exp(-omega_c * t->delta_t) - exp(-3 * omega_c * t->delta_t / 2) *
                                                (costerm + sinterm / SQRTOF3)));
...
{
  PUTVAL(t, out, smpl,
         (t->x1 * GETVAL(t, in, smpl - 1) + t->x2 * GETVAL(t, in, smpl - 2) -
          t->yA * GETVAL(t, out, smpl - 1) - t->y2 * GETVAL(t, out, smpl - 2) -
...
{                   /* is taken to represent the freq. change.   */
  ...
  // printf("phase: %f %f\n", GETVAL(t, t->a_term,smpl),
  // GETVAL(t,t->b_term,smpl));
  if ((temp_a = GETVAL(t, t->a_term, smpl)) == 0)
    t->new_ph = (-PI / FL(2.0)) * sgn(GETVAL(t, t->b_term, smpl));
  else
    t->new_ph = -atan(GETVAL(t, t->b_term, smpl) / temp_a) - PI * u(-temp_a);
  ...
  delt_temp = ((GETVAL(t, t->r_phase, smpl) - GETVAL(t, t->r_phase, smpl - 1)) /
               (TWOPI * t->delta_t));
  if ((t->freq_c <= 1) || (smpl < 3)) {
    PUTVAL(
        t, t->amp_av1, smpl,
        (MYFLT)hypot(GETVAL(t, t->a_term, smpl), GETVAL(t, t->b_term, smpl)));
...
{ /* current amp and new freq in arrays */
  ...
  if (pnt < t->num_pts) {
    ...
    t->FREQS[hno][pnt] = new_freq = (MYFLT)(delt_freq + t->cur_est);
    t->MAGS[hno][pnt] = new_amp = (MYFLT)GETVAL(t, t->r_ampl, smpl);
...
static int32_t filedump(HET *t, CSOUND *csound) {
  ...
  for (h = 0; h < t->hmax; h++) {
    mags[h] = (int16 *)csound->Malloc(csound, (t->num_pts + 2) * sizeof(int16));
    freqs[h] =
        (int16 *)csound->Malloc(csound, (t->num_pts + 2) * sizeof(int16));
  }
  ...
  if (t->newformat) {
    if (UNLIKELY(csound->FileOpen2(csound, &ff, CSFILE_STD, t->outfilnam, "w",
                                   "", CSFTYPE_HETROT, 0) == NULL))
      return quit(csound, Str("cannot create output file\n"));
  } else if (UNLIKELY(csound->FileOpen2(csound, &ofd, CSFILE_FD_W, t->outfilnam,
  ...
  magout =
      (int16 *)csound->Malloc(csound, (t->num_pts + 1) * 2 * sizeof(int16));
  frqout =
      (int16 *)csound->Malloc(csound, (t->num_pts + 1) * 2 * sizeof(int16));
...
static int32_t writesdif(CSOUND *csound, HET *t) {
  ...
  if (UNLIKELY((r = SDIF_OpenWrite(t->outfilnam, &sdiffile)) !=
               ESDIF_SUCCESS)) {
  ...
  for (i = 0; i < t->num_pts; i++) {
    ...
    if (UNLIKELY((r = SDIF_WriteFrameHeader(&head, sdiffile)) !=
                 ESDIF_SUCCESS)) {
    ...
    if (UNLIKELY((r = SDIF_WriteMatrixHeader(&mh, sdiffile)) !=
                 ESDIF_SUCCESS)) {
  ...
  csound->Message(csound, Str("wrote %d 1TRC frames to %s\n"), t->num_pts,
                  t->outfilnam);
...
static int32_t is_sdiffile(char *name) {
  ...
  if (dot == NULL)
    return 0;
  if (strcmp(dot, ".sdif") == 0 || strcmp(dot, ".SDIF") == 0)
    return 1;
  return 0;
}
#endif
