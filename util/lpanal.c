...
static void alpol(CSOUND *, LPC *, MYFLT *, double *, double *, double *,
                  double *);
...
static void synthetize(int32_t poleCount, double *poleReal, double *poleImag,
...
static void DumpPoles(CSOUND *csound, int32_t poleCount, double *part1,
                      double *part2, int32_t isMagn, char *where) {
  int32_t i;
...
static int32_t lpanal(CSOUND *csound, int32_t argc, char **argv) {
  ...
  *tp = '\0';
  ...
  do {
    ...
    if (*s++ == '-')
      ...
      case 'p':
        ...
        if (lpc.poleCount <= 0) {
          csound->Message(csound, "%s", Str("Invalid pole count; set to 1\n"));
          lpc.poleCount = 1;
        }
        break;
      case 'h':
        ...
        break;
      case 'C':
        FIND(Str("no comment string"))
        // MKG 2014 Jan 29: No linkage for strlcat with MinGW here.
        // but wrong; corrected
        // strlcat(tp,s,(LPBUFSIZ - sizeof(LPHEADER) + 4));
        strncat(tp, s, (LPBUFSIZ - sizeof(LPHEADER) + 3 - strlen(tp)));
        tp[LPBUFSIZ - 1] = '\0';
        tp += strlen(tp);
        break;
      case 'P':
        FIND(Str("no low frequency"))
#if defined(USE_DOUBLE)
        ...
        break;
      case 'Q':
        FIND(Str("no high frequency"))
#if defined(USE_DOUBLE)
        csound->sscanf(s, "%lf", &pchhigh);
        break;
#else
        csound->sscanf(s, "%f", &pchhigh);
        break;
#endif
      case 'v':
        FIND(Str("no verbose level"))
  ...
  /* Allocate space now */
  coef = (MYFLT *)csound->Malloc(csound,
                                 (NDATA + lpc.poleCount * 2) * sizeof(MYFLT));
  /* Space allocated */
  ...
  if (UNLIKELY(lpc.verbose)) {
    ...
    if (pchlow > 0.0)
      ...
      csound->Message(csound, "%s", Str("pitch tracking inhibited\n"));
  }
  if (UNLIKELY((input_dur < 0) || (beg_time < 0)))
    quit(csound, Str("input and begin times cannot be less than zero"));
  ...
  if (storePoles)
    ...
    csound->Message(csound, "%s",
                    Str("Using filter coefficient storage method\n"));
  ...
  if (new_format) {
    if (UNLIKELY(csound->FileOpen2(csound, &oFd, CSFILE_STD, outfilnam, "w", "",
                                   CSFTYPE_LPC, 0) == NULL))
  ...
  if (new_format) { /* ****This is not accurate wrt doubles**** */
    fprintf(oFd, "LPANAL\n%d %d %d %d\n%a %a %a\n", lph->headersize,
            lph->lpmagic, lph->npoles, lph->nvals, (double)lph->framrate,
            (double)lph->srate, (double)lph->duration);
...
#endif
  /* Space for a array */
  lpc.a = (double(*)[MAXPOLES])csound->Malloc(csound, MAXPOLES * MAXPOLES *
                                                          sizeof(double));
  ...
  do {
...
#ifdef TRACE_POLES
    csound->Message(csound, "%s", Str("Starting new frame...\n"));
#endif
    counter++;
    ...
    if (lpc.doPitch)
      ...
      coef[3] = FL(0.0);
    if (lpc.debug)
      csound->Message(csound, "%d\t%9.4f\t%9.4f\t%9.4f\t%9.4f\n", counter,
                      coef[0], coef[1], coef[2], coef[3]);
#ifdef TRACE
    if (lpc.debug)
      fprintf(trace, "%d\t%9.4f\t%9.4f\t%9.4f\t%9.4f\n", counter, coef[0],
    ...
    if (storePoles) {
      ...
      polyzero(lpc.poleCount, filterCoef, polePart1, polePart2, &poleFound,
               2000, &indic, workArray1);

      if (UNLIKELY(poleFound < lpc.poleCount)) {
        csound->Message(csound, Str("Found only %d poles...sorry\n"),
                        poleFound);
        csound->Message(csound, Str("wanted %d poles\n"), lpc.poleCount);
        return -1;
      }
      InvertPoles(lpc.poleCount, polePart1, polePart2);

#ifdef TRACE_POLES
      DumpPoles(csound, lpc.poleCount, polePart1, polePart2, 0,
                "Extracted Poles");
#endif
      ...
      for (i = 0; i < lpc.poleCount; i++) {
        ...
        if (UNLIKELY(filterCoef[i] - polyReal[lpc.poleCount - i] > 1e-10))
          csound->Message(csound, Str("Error in coef %d : %f <> %f\n"), i,
                          filterCoef[i], polyReal[lpc.poleCount - i]);
      }
      csound->Message(csound, ".");
      InvertPoles(lpc.poleCount, polePart1, polePart2);
#endif
    ...
    if (new_format) {
      ...
      for (i = 0, j = 0; i < osiz; i += sizeof(MYFLT), j++)
        fprintf(oFd, "%a\n", (double)coef[j]);
    } else if (UNLIKELY((nb = write(ofd, (char *)coef, osiz)) != osiz))
...
/* b filled here */
{
  if (!thislp->newmethod) {
    ...
    for (j = 0; j < thislp->poleCount; j++) {
      sum += thislp->x[thislp->poleCount - (j + 1)] *
                 thislp->x[thislp->poleCount - (j + 1)] -
             thislp->x[thislp->WINDIN - (j + 1)] *
                 thislp->x[thislp->WINDIN - (j + 1)];
      thislp->a[j][j] = sum;
    ...
    *errn = sumy / sumx;
    ...
    for (i = 1; i < thislp->poleCount + 1; i++)
      b[i - 1] = c[i];
 ...
 *
 */
static void gauss(LPC *thislp, double (*a /*old*/)[MAXPOLES], double *bold,
                  double b[]) {
...
static void trigpo(MYFLT, phi_typ, psi_typ, MYFLT *, MYFLT *, int32_t,
                   LPANAL_GLOBALS *);
static MYFLT lowpass(MYFLT, LPANAL_GLOBALS *);
static MYFLT search(MYFLT *fm, MYFLT qsum, MYFLT g[], MYFLT h[],
                    LPANAL_GLOBALS *);

static void trigpo(MYFLT omega, phi_typ phi, psi_typ psi, MYFLT *gamphi,
                   MYFLT *gampsi, int32_t n, LPANAL_GLOBALS *lpg)
...
static MYFLT search(MYFLT *fm, MYFLT qsum, MYFLT g[], MYFLT h[],
  ...
  *fm = a * (x0 - x1) * (x0 - x2) + b * (x0 - x1) * (x0 - x3) +
        c * (x0 - x2) * (x0 - x3);
  return (x0);
}
...
static MYFLT getpch(CSOUND *csound, MYFLT *sigbuf, LPANAL_GLOBALS *lpg) {
  ...
  if (lpg->firstcall) { /* on first call, alloc dbl dbuf  */
    lpg->Dwind_dbuf = (MYFLT *)csound->Calloc(csound, (int64_t)lpg->Dwind * 2 *
                                                          sizeof(MYFLT));
...
static void ptable(CSOUND *csound, MYFLT fmin, MYFLT fmax, MYFLT sr,
                   int32_t windsiz, LPANAL_GLOBALS *lpg) {
  int32_t i, n;
  ...
  for (i = 0; i < FREQS; ++i) {  /*   as if MAX dimension of Hwind */
    ...
    lpg->tgamps[i] =
        (MYFLT *)csound->Calloc(csound, (int64_t)NP * sizeof(MYFLT));
    lpg->freq[i] = fmin + (MYFLT)i * fstep;
    n = (int32_t)(lpg->NYQ10 / lpg->freq[i]);
    if (n > NN)
      n = NN;
    omega = lpg->freq[i] * tpidsrd10;
    trigpo(omega, (phi_typ)lpg->tphi[i], (psi_typ)lpg->tpsi[i], lpg->tgamph[i],
           lpg->tgamps[i], n, lpg);
  }
}
...
int32_t lpanal_init_(CSOUND *csound) {
  ...
  if (!retval) {
    retval = csound->SetUtilityDescription(
        csound, "lpanal", Str("Linear predictive analysis for lpread"));
  }
  return retval;
}
