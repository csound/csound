...
static int32_t envext(CSOUND *csound, int32_t argc, char **argv) {
  ...
  do {
    ...
    if (*s++ == '-') /* read all flags:  */
      ...
      envext_usage(csound, Str("too many arguments"));
  } while (--argc);
...
static SNDFILE *SCsndgetset(CSOUND *csound, SOUNDIN **pp, char *inputfile) {
  SNDFILE *infd;
  double dur;
  SOUNDIN *p;
  ...
  *pp = p = (SOUNDIN *)csound->Calloc(csound, sizeof(SOUNDIN));
  ...
  csound->Message(csound,
                  Str("enveloping %" PRId64 " sample frames (%3.1f secs)\n"),
                  (int64_t)p->getframes, dur);
  return (infd);
}

static void FindEnvelope(CSOUND *csound, SNDFILE *infd, SOUNDIN *p,
...
int32_t envext_init_(CSOUND *csound) {
  ...
  if (!retval) {
    retval = csound->SetUtilityDescription(
        csound, "envext", Str("Create a text file of envelope"));
  }
  return retval;
}
