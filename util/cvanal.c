...
static int32_t takeFFT(CSOUND *csound, SOUNDIN *inputSound, CVSTRUCT *outputCVH,
                       int64_t Hlenpadded, SNDFILE *infd, FILE *ofd,
                       int32_t nf);
static int32_t quit(CSOUND *, char *msg);
static int32_t CVAlloc(CSOUND *, CVSTRUCT **, int64_t, int32_t, MYFLT, int32_t,
...
static int32_t cvanal(CSOUND *csound, int32_t argc, char **argv) {
  ...
  if (UNLIKELY((err = CVAlloc(csound, &cvh, Estdatasiz, CVMYFLT, sr, p->nchanls,
                              channel, Hlen, CVRECT, 4)))) {
  ...
  if (new_format) {
    ...
    if (UNLIKELY((int64_t)fwrite(cvh, 1, cvh->headBsize, ofd) <
                 cvh->headBsize)) {
...
static int32_t takeFFT(CSOUND *csound, SOUNDIN *p, CVSTRUCT *cvh,
  ...
  fp1 = inbuf;
  outbuf = fp2 =
      (MYFLT *)csound->Malloc(csound, sizeof(MYFLT) * (Hlenpadded + 2));
  ...
  for (i = 0; i < nchanls; i++) {
    ...
    } else if (UNLIKELY(1 != fwrite(outbuf, cvh->dataBsize / nchanls, 1, ofd)))
...
static int32_t
CVAlloc(CSOUND *csound, CVSTRUCT **pphdr, /* returns address of new block */
...
int32_t cvanal_init_(CSOUND *csound) {
  ...
  if (!retval) {
    retval = csound->SetUtilityDescription(
        csound, "cvanal", Str("Soundfile analysis for convolve"));
  }
  return retval;
}
