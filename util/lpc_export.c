...
void lpc_export_usage(CSOUND *csound) {
  csound->Message(csound, "%s",
                  Str("usage: lpc_export lpc_file cstext-file\n"));
}
...
static int32_t lpc_export(CSOUND *csound, int32_t argc, char **argv) {
  ...
  fprintf(outf, "%d,%d,%d,%d,%f,%f,%f", hdr.headersize, hdr.lpmagic, hdr.npoles,
          hdr.nvals, hdr.framrate, hdr.srate, hdr.duration);
  ...
  if (UNLIKELY(fread(str, sizeof(char), hdr.headersize - sizeof(LPHEADER) + 4,
                     inf) != hdr.headersize - sizeof(LPHEADER) + 4))
    csound->Message(csound, "%s", Str("Read failure\n"));
  for (i = 0; i < hdr.headersize - sizeof(LPHEADER) + 4; i++)
    putc(str[i], outf);
  putc('\n', outf);
  coef =
      (MYFLT *)csound->Malloc(csound, (hdr.npoles + hdr.nvals) * sizeof(MYFLT));
...
int32_t lpc_export_init_(CSOUND *csound) {
  ...
  if (!retval) {
    retval = csound->SetUtilityDescription(csound, "lpc_export",
