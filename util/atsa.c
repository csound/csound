...
// #define  AMP_DB(amp)  ((amp) != 0.0 ? (float) log10((amp) * 20.0) :
// -32767.0f) #define  DB_AMP(db)   ((float) pow(10.0, (db) / 20.0))
 ...
 */
static ATS_PEAK *push_peak(CSOUND *csound, ATS_PEAK *new_peak, ATS_PEAK *peaks,
                           int *peaks_size);
...
static ATS_PEAK *peak_detection(CSOUND *csound, ATS_FFT *ats_fft,
                                ...
                                float lowest_mag, double norm, int *peaks_size);
...
static ATS_FRAME *peak_tracking(CSOUND *csound, ATS_PEAK *tracks,
                                ...
                                int *peaks_size, float frq_dev, float SMR_cont,
                                int *n_partials);
 ...
 */
static void compute_residual(CSOUND *csound, mus_sample_t **fil, int fil_len,
                             char *output_file, ATS_SOUND *sound,
                             int *win_samps, int file_sampling_rate);
...
static int main_anal(CSOUND *csound, char *soundfile, char *ats_outfile,
  ...
  /* open output file */
  fd = csound->FileOpen2(csound, &outfile, CSFILE_STD, ats_outfile, "wb", NULL,
                         CSFTYPE_ATS, 0);
  if (UNLIKELY(fd == NULL)) {
...
static CS_NOINLINE CS_NORETURN void usage(CSOUND *csound) {
  ...
  csound->Message(csound, "%s",
                  Str("\t\t(Options: 0=BLACKMAN, 1=BLACKMAN_H, "
                      "2=HAMMING, 3=VONHANN)\n"));
...
static int atsa_main(CSOUND *csound, int argc, char **argv) {
  ...
  if (cur_opt != '\0' || soundfile == NULL || soundfile[0] == '\0' ||
      ...
      for (j = 0; j < peaks_size; j++)
        if (i != j) {
          ...
          mask_term = (frq_masker < frq_maskee)
                          ? (amp_masker + delta_dB +
                             (slope_r * (frq_maskee - frq_masker)))
                          : (amp_masker + delta_dB +
                             (slope_l * (frq_masker - frq_maskee)));
...
static float *make_window(CSOUND *csound, int win_type, int win_size) {
  ...
  for (i = 0; i < win_size; i++) {
    ...
    case BLACKMAN: /* Blackman (3 term) */
      buffer[i] =
          (float)(0.42 - 0.5 * cos(arg * i) + 0.08 * cos(arg * (i + i)));
      break;
    case BLACKMAN_H: /* Blackman-Harris (4 term) */
      buffer[i] = (float)(0.35875 - 0.48829 * cos(arg * i) +
                          0.14128 * cos(arg * (i + i)) -
...
static ATS_PEAK *push_peak(CSOUND *csound, ATS_PEAK *new_peak,
  ...
  peaks_list = (ATS_PEAK *)csound->ReAlloc(csound, peaks_list,
...
static ATS_PEAK *peak_detection(CSOUND *csound, ATS_FFT *ats_fft,
  ...
  for (k = first_bin; k < N; k++) {
    ...
    if ((central_bin > (double)lowest_mag) && (central_bin > right_bin) &&
      ...
      ats_peak.pha =
          (offset < 0.0)
              ? phase_interp(fftphase[k - 2], fftphase[k - 1], fabs(offset))
              : phase_interp(fftphase[k - 1], fftphase[k], offset);
      ats_peak.track = -1;
...
static void parabolic_interp(double alpha, double beta, double gamma,
                             double *offset, double *height) {
  double dbAlpha = amp2db(alpha), dbBeta = amp2db(beta),
         dbGamma = amp2db(gamma);
...
static ATS_FRAME *peak_tracking(CSOUND *csound, ATS_PEAK *tracks,
  ...
  for (k = 0; k < *peaks_size; k++) {
    ...
    track_candidates[k].cands = find_candidates(
        csound, tracks, *tracks_size, lo, hi, &track_candidates[k].size);
...
static void sort_candidates(ATS_CANDS *cands, ATS_PEAK peak, float SMR_cont) {
  ...
  for (i = 0; i < cands->size; i++)
    cands->cands[i].amp = (fabs(cands->cands[i].frq - peak.frq) +
                           (SMR_cont * fabs(cands->cands[i].smr - peak.smr))) /
                          (SMR_cont + 1);
...
static ATS_PEAK *update_tracks(CSOUND *csound, ATS_PEAK *tracks,
  ...
  if (tracks != NULL) {
    ...
    for (g = 0; g < *tracks_size; g++) {
      ...
      if (f)
        tracks[g].frq = (last_peak_cont * last_frq) +
                        ((1 - last_peak_cont) * (frq_acc / f));
      if (a)
        tracks[g].amp = (last_peak_cont * last_amp) +
                        ((1 - last_peak_cont) * (amp_acc / a));
      if (s)
        tracks[g].smr = (last_peak_cont * last_smr) +
                        ((1 - last_peak_cont) * (smr_acc / s));
    }
  } else
    for (g = 0; g < ana_frames[frame_n - 1].n_peaks; g++)
      tracks = push_peak(csound, &ana_frames[frame_n - 1].peaks[g], tracks,
                         tracks_size);
...
static void residual_get_bands(double fft_mag, double *true_bands, int *limits,
                               int bands);
...
static double residual_get_band_energy(int lo, int hi, ATS_FFT *fft_struct,
                                       double norm);
static void residual_compute_band_energy(ATS_FFT *fft_struct, int *band_limits,
...
static void residual_compute_band_energy(ATS_FFT *fft, int *band_limits,
  ...
  for (b = 0; b < bands - 1; b++)
    band_energy[b] =
        residual_get_band_energy(band_limits[b], band_limits[b + 1], fft, norm);
}
...
static void residual_analysis(CSOUND *csound, char *file, ATS_SOUND *sound) {
  ...
  fd = csound->FileOpen2(csound, &sf, CSFILE_SND_R, file, &sfinfo,
...
static double interp_phase(double pha_1, double frq_1, double alpha,
                           double beta, int i);
static void read_frame(mus_sample_t **fil, int fil_len, int samp_1, int samp_2,
                       double *in_buffer);
static void synth_buffer(double a1, double a2, double f1, double f2, double p1,
                         double p2, double *buffer, int frame_samps);
...
static int compute_m(double pha_1, double frq_1, double pha, double frq,
  ...
  /* int val = (int) ((((pha_1 + (frq_1 * (double) buffer_size) - pha)
                      ...
                      + 0.5); */
  return ((int)((((pha_1 + (frq_1 * (double)buffer_size) - pha) +
                  ((frq - frq_1) * 0.5 * (double)buffer_size)) /
                 TWOPI) +
                0.5));
}
...
static double compute_aux(double pha_1, double pha, double frq_1,
  ...
  return ((double)((pha + (TWOPI * (double)M)) -
                   (pha_1 + (frq_1 * (double)buffer_size))));
}
...
static double compute_alpha(double aux, double frq_1, double frq,
  ...
  return ((double)(((3.0 / (double)(buffer_size * buffer_size)) * aux) -
                   ((frq - frq_1) / (double)buffer_size)));
}
...
static double compute_beta(double aux, double frq_1, double frq,
  ...
  return ((double)(((-2.0 / (double)(buffer_size * buffer_size * buffer_size)) *
                    aux) +
                   ((frq - frq_1) / (double)(buffer_size * buffer_size))));
}
...
static double interp_phase(double pha_1, double frq_1, double alpha,
  ...
  return ((double)((beta * (double)(i * i * i)) + (alpha * (double)(i * i)) +
                   (frq_1 * (double)i) + pha_1));
}
...
static void compute_residual(CSOUND *csound, mus_sample_t **fil, int fil_len,
  ...
  fd = csound->FileOpen2(csound, &sf, CSFILE_SND_W, output_file, &sfinfo, NULL,
                         CSFTYPE_WAVE, 0);
  if (UNLIKELY(fd == NULL)) {
  ...
  obuf[0] =
      (mus_sample_t *)csound->Calloc(csound, frm_samps * sizeof(mus_sample_t));
  obuf[1] =
      (mus_sample_t *)csound->Calloc(csound, frm_samps * sizeof(mus_sample_t));
...
static ATS_SOUND *tracker(CSOUND *csound, ANARGS *anargs, char *soundfile,
  ...
  if (UNLIKELY(sfinfo.channels != 1)) {
    csound->ErrorMsg(csound, Str("atsa: file has %d channels, must be mono !"),
                     (int)sfinfo.channels);
    return NULL;
  }
  ...
  if (UNLIKELY(!(anargs->lowest_freq > 0.0 &&
  ...
  if (UNLIKELY(!(anargs->highest_freq > anargs->lowest_freq &&
  ...
  if (UNLIKELY(!(anargs->win_cycles >= 1 && anargs->win_cycles <= 8))) {
    csound->Warning(csound,
                    Str("windows cycles %d out of bounds, "
  ...
  if (UNLIKELY(!(anargs->win_type >= 0 && anargs->win_type <= 3))) {
    csound->Warning(csound,
                    Str("window type %d out of bounds, "
  ...
  if (UNLIKELY(!(anargs->lowest_mag <= 0.0))) {
    csound->Warning(csound,
                    Str("lowest magnitude %f out of bounds, "
                        "should be >= 0.0 and <= 1.0, "
                        "forced to default: %f"),
                    anargs->lowest_mag, ATSA_LMAG);
    anargs->lowest_mag = ATSA_LMAG;
  ...
  /* window size */
  anargs->win_size =
      (anargs->cycle_smp % 2 == 0) ? anargs->cycle_smp + 1 : anargs->cycle_smp;
  ...
  /* track length */
  if (UNLIKELY(
          !(anargs->track_len >= 1 && anargs->track_len < anargs->frames))) {
    i_tmp = (ATSA_TRKLEN < anargs->frames) ? ATSA_TRKLEN : anargs->frames - 1;
    csound->Warning(csound, Str("track length %d out of bounds, forced to: %d"),
  ...
  /* SMR threshold */
  if (UNLIKELY(
          !(anargs->SMR_thres >= 0.0 && anargs->SMR_thres < ATSA_MAX_DB_SPL))) {
    csound->Warning(csound,
                    Str("SMR threshold %f out of bounds, "
  ...
  /* min. seg. SMR */
  if (UNLIKELY(!(anargs->min_seg_SMR >= anargs->SMR_thres &&
                 anargs->min_seg_SMR < ATSA_MAX_DB_SPL))) {
    csound->Warning(csound,
                    Str("min. seg. SMR %f out of bounds, "
                        "should be >= %f and < %f dB SPL, "
                        "forced to default: %f"),
                    anargs->min_seg_SMR, anargs->SMR_thres, ATSA_MAX_DB_SPL,
                    ATSA_MSEGSMR);
    anargs->min_seg_SMR = ATSA_MSEGSMR;
  }
  /* last peak contribution */
  if (UNLIKELY(
          !(anargs->last_peak_cont >= 0.0 && anargs->last_peak_cont <= 1.0))) {
    csound->Warning(csound,
                    Str("last peak contribution %f out of bounds, "
                        "should be >= 0.0 and <= 1.0, "
                        "forced to default: %f"),
                    anargs->last_peak_cont, ATSA_LPKCONT);
    anargs->last_peak_cont = ATSA_LPKCONT;
  ...
  if (UNLIKELY(!(anargs->SMR_cont >= 0.0 && anargs->SMR_cont <= 1.0))) {
    csound->Warning(csound,
                    Str("SMR contribution %f out of bounds, "
                        "should be >= 0.0 and <= 1.0, "
                        "forced to default: %f"),
                    anargs->SMR_cont, ATSA_SMRCONT);
    anargs->SMR_cont = ATSA_SMRCONT;
  ...
  ana_frames =
      (ATS_FRAME *)csound->Malloc(csound, anargs->frames * sizeof(ATS_FRAME));
  ...
  M_2 = (anargs->win_size - 1) /
        2; /* Was (int)floor((anargs->win_size - 1) / 2) */
  ...
  fft.data =
      (MYFLT *)csound->Malloc(csound, (anargs->fft_size + 2) * sizeof(MYFLT));
  ...
  for (frame_n = 0; frame_n < anargs->frames; frame_n++) {
    ...
    if (peaks != NULL) {
      ...
      if (frame_n) {
        ...
        if ((tracks = update_tracks(csound, tracks, &tracks_size,
                                    anargs->track_len, frame_n, ana_frames,
                                    anargs->last_peak_cont)) != NULL) {
          /* do peak matching */
          unmatched_peaks = peak_tracking(csound, tracks, &tracks_size, peaks,
                                          &peaks_size, anargs->freq_dev,
                                          2.0 * anargs->SMR_cont, &n_partials);
          ...
          if (unmatched_peaks[1].peaks != NULL) {
            for (k = 0; k < unmatched_peaks[1].n_peaks; k++) {
              tracks = push_peak(csound, &unmatched_peaks[1].peaks[k], tracks,
                                 &tracks_size);
      ...
      ana_frames[frame_n].time =
          (double)(win_samps[frame_n] - anargs->first_smp) /
          (double)anargs->srate;
      /* free memory */
...
static int compute_frames(ANARGS *anargs) {
  ...
  while ((n_frames++ * anargs->hop_smp - anargs->hop_smp + anargs->first_smp) <
         (anargs->first_smp + anargs->total_samps))
    ;
  return (n_frames);
}
...
static void init_sound(CSOUND *csound, ATS_SOUND *sound, int sampling_rate,
  ...
  sound->av = (ATS_PEAK *)csound->Malloc(csound, partials * sizeof(ATS_PEAK));
  sound->optimized = NIL;
  ...
  for (i = 0; i < partials; i++) {
    sound->time[i] = (double *)csound->Malloc(csound, frames * sizeof(double));
    sound->amp[i] = (double *)csound->Calloc(csound, frames * sizeof(double));
    sound->frq[i] = (double *)csound->Calloc(csound, frames * sizeof(double));
    sound->pha[i] = (double *)csound->Calloc(csound, frames * sizeof(double));
    sound->smr[i] = (double *)csound->Calloc(csound, frames * sizeof(double));
    sound->res[i] = (double *)csound->Calloc(csound, frames * sizeof(double));
  }
  ...
  /*   } */
  if (use_noise) {
    sound->band_energy = (double **)csound->Malloc(
        csound, ATSA_CRITICAL_BANDS * sizeof(double *));
    for (i = 0; i < ATSA_CRITICAL_BANDS; i++)
      sound->band_energy[i] =
...
int atsa_init_(CSOUND *csound) {
  ...
  if (!retval) {
    retval = csound->SetUtilityDescription(
        csound, "atsa", Str("Soundfile analysis for ATS opcodes"));
  }
  return retval;
}
