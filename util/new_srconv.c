...
int main(int argc, char **argv) {
  ...
  if (tvflg) {
    if (UNLIKELY((tvfp = fopen(bfile, "r")) == NULL)) {
      strncpy(err_msg, Str("srconv: cannot open time-vary function file"), 299);
      goto err_rtn_msg1;
    }
    ...
    for (i = 0; i < tvlen; i++) {
      if (UNLIKELY(fscanf(tvfp, "%lf %lf", &warp[i].time, &warp[i].ratio) !=
                   2)) {
        strncpy(err_msg,
                Str("srconv: too few x-y pairs "
    ...
    warp[tvlen].frame = flen;
    warp[tvlen].time = flen / Rin;
    tvlen++;
    if (UNLIKELY(warp[0].frame != 0.0)) {
      strncpy(err_msg,
              Str("srconv: first frame value "
  ...
  if (UNLIKELY(outf == NULL)) {
    snprintf(err_msg, 299, Str("cannot open %s (%s)."), outfilename,
             sf_strerror(outf));
    goto err_rtn_msg1;
  }
  ...
  if (tvflg) {
    ...
    if (C == 0)
      C = 1;                         /* avoid silly value for buffer sizes */
    state = src_new(Q, Chans, &err); /* initialise */
    if (UNLIKELY(state == NULL)) {
      fprintf(stderr, "Error: failed to initialise SRC -- %s\n",
              src_strerror(err));
      sf_close(inf);
      sf_close(outf);
      usage();
    ...
    for (;;) {
      ...
      if (UNLIKELY(data.end_of_input && data.output_frames_gen == 0))
        break;
...
static const char *usage_txt[] = {
    ...
    Str_noop("-u\tulaw sound samples"), Str_noop("-s\tshort_int sound samples"),
    Str_noop("-l\tlong_int sound samples"), Str_noop("-f\tfloat sound samples"),
