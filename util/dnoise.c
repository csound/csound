...
static int32_t writebuffer(CSOUND *, SNDFILE *, MYFLT *, int32_t, int32_t *,
                           OPARMS *);
...
static int32_t dnoise(CSOUND *csound, int32_t argc, char **argv) {
  ...
  *ibuf1,     /* pointer to start of input buffer */
  ...
  int32_t N = 0, /* number of phase vocoder channels (bands) */
      Np2,       /* N+2 */
  ...
  int64_t oCnt = 0L, /* number of samples written to output */
      ...
      if (*s++ == '-') { /* read all flags:  */
        while ((c = *s++) != '\0') {
          ...
          case 'N':
            ...
            break;
          case 'M':
            FIND(Str("no M argument"));
            sscanf(s, "%d", &M);
            while (*++s)
              ;
            break;
          case 'L':
            FIND(Str("no L argument"));
            sscanf(s, "%d", &L);
            while (*++s)
              ;
            break;
          case 'w':
            FIND(Str("no w argument"));
            sscanf(s, "%d", &W);
            while (*++s)
              ;
            break;
          case 'D':
            FIND(Str("no D argument"));
            sscanf(s, "%d", &D);
            while (*++s)
              ;
            break;
    ...
    O.outfilename = "test";
  {
    ...
    if (strcmp(O.outfilename, "stdout") != 0) {
      ...
      if (outfd != NULL)
        csound->NotifyFileOpened(
            csound, name, csound->type2csfiletype(O.filetyp, O.outformat), 1,
            0);
    ...
    (void)csound->CreateFileHandle(csound, &outfd, CSFILE_SND_W, O.outfilename);
  ...
  if (UNLIKELY(Chans > 2)) {
    csound->Message(csound, "%s",
                    Str("dnoise: input MUST be mono or stereo\n"));
    return -1;
  }
  ...
  if (end >= FL(0.0))
    End = (int64_t)(end * R);
  else if (End == 99999999)
    End = (int64_t)(input_ndur * R);
  ...
  if (W != -1) {
    if (UNLIKELY(M != 0))
      ...
      csound->Message(csound, "%s",
                      Str("dnoise: warning - invalid W ignored\n"));
  }
...
#endif
  csound->Message(csound, Str("writing %u-byte blks of %s to %s"), outbufsiz,
  ...
  if (UNLIKELY((aWin = (MYFLT *)csound->Calloc(
                    csound, (M + Meven) * sizeof(MYFLT))) == NULL)) {
  ...
  if (M > N) {
    ...
    for (i = 1; i <= aLen; i++)
      aWin[i] *=
          (MYFLT)(N * sin(PI * ((double)i + 0.5 * (double)Meven) / (double)N) /
  ...
  if (UNLIKELY((sWin = (MYFLT *)csound->Calloc(
                    csound, (L + Leven) * sizeof(MYFLT))) == NULL)) {
    ERR(Str("dnoise: insufficient memory\n"));
  }
  ...
  if (M <= N) {
    ...
    for (i = 1; i <= sLen; i++)
      sWin[i] *=
          (MYFLT)(I * sin(PI * ((double)i + 0.5 * (double)Leven) / (double)I) /
  ...
  if (UNLIKELY((ibuf1 = (MYFLT *)csound->Calloc(
                    csound, ibuflen * sizeof(MYFLT))) == NULL)) {
    ERR("dnoise: insufficient memory\n");
  }
  if (UNLIKELY((ibuf2 = (MYFLT *)csound->Calloc(
                    csound, ibuflen * sizeof(MYFLT))) == NULL)) {
    ERR(Str("dnoise: insufficient memory\n"));
  }
  ...
  if (UNLIKELY((obuf1 = (MYFLT *)csound->Calloc(
                    csound, obuflen * sizeof(MYFLT))) == NULL)) {
    ERR(Str("dnoise: insufficient memory\n"));
  }
  if (UNLIKELY((obuf2 = (MYFLT *)csound->Calloc(
                    csound, obuflen * sizeof(MYFLT))) == NULL)) {
    ERR(Str("dnoise: insufficient memory\n"));
  }
  ...
  if (UNLIKELY((fbuf = (MYFLT *)csound->Calloc(csound, Np2 * sizeof(MYFLT))) ==
               NULL)) {
    ERR(Str("dnoise: insufficient memory\n"));
  }
  ...
  if (UNLIKELY((nref = (MYFLT *)csound->Calloc(
                    csound, (N2 + 1) * sizeof(MYFLT))) == NULL)) {
    ERR(Str("dnoise: insufficient memory\n"));
  }

  if (UNLIKELY((mbuf = (MYFLT *)csound->Calloc(
                    csound, (m * Np2) * sizeof(MYFLT))) == NULL)) {
    ERR(Str("dnoise: insufficient memory\n"));
  }
  if (UNLIKELY((nbuf = (MYFLT *)csound->Calloc(
                    csound, (m * Np2) * sizeof(MYFLT))) == NULL)) {
    ERR(Str("dnoise: insufficient memory\n"));
  }
  if (UNLIKELY((rsum = (MYFLT *)csound->Calloc(
                    csound, (N2 + 1) * sizeof(MYFLT))) == NULL)) {
    ERR(Str("dnoise: insufficient memory\n"));
  }
  if (UNLIKELY((ssum = (MYFLT *)csound->Calloc(
                    csound, (N2 + 1) * sizeof(MYFLT))) == NULL)) {
    ERR(Str("dnoise: insufficient memory\n"));
  }
  ...
  while (nI < (nMax + aLen)) {
    ...
    if (Verbose) {
      ...
      if (nImodR > (int64_t)R) {
        nImodR -= (int64_t)R;
        csound->Message(csound, Str("%5.1f seconds of input complete\n"),
                        (time + D * invR));
      }
    }
  }
...
static void sndwrterr(CSOUND *csound, int32_t nret, int32_t nput) {
  csound->Message(csound,
                  Str("soundfile write returned sample count of %d, "
                      "not %d\n"),
                  nret, nput);
  csound->Message(csound, "%s",
...
static int32_t writebuffer(CSOUND *csound, SNDFILE *outfd, MYFLT *outbuf,
                           int32_t nsmps, int32_t *nrecs, OPARMS *O) {
  int32_t n;
  ...
  case 3:
    ...
    while (n--)
      csound->MessageS(csound, CSOUNDMSG_REALTIME, "\b");
    break;
  case 4:
...
int32_t dnoise_init_(CSOUND *csound) {
  ...
  if (!retval) {
    retval = csound->SetUtilityDescription(
        csound, "dnoise", Str("Removes noise from a sound file"));
  }
  return retval;
}
