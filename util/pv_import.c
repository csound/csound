...
static int32_t pv_import(CSOUND *csound, int32_t argc, char **argv) {
  ...
  if (UNLIKELY(UNLIKELY(
          EOF == fscanf(inf, "FormatTag,Channels,SamplesPerSec,AvgBytesPerSec,"
  ...
  }
  {
    ...
    if (UNLIKELY(7 != fscanf(inf, "%d,%d,%d,%d,%u,%u,%d\n", &fmt1, &fmt2,
                             &fmt.nSamplesPerSec, &fmt.nAvgBytesPerSec, &fmt3,
                             &fmt4, &fmt5))) {
  ...
  if (UNLIKELY(EOF == fscanf(inf,
                             "WordFormat,AnalFormat,SourceFormat,WindowType,"
  ...
  }
  {
    ...
    if (UNLIKELY(10 != fscanf(inf, "%d,%d,%d,%d,%d,%d,%d,%d,%g,%g\n", &data1,
                              &data2, &data3, &data4, &data.nAnalysisBins,
                              &data.dwWinlen, &data.dwOverlap,
                              &data.dwFrameAlign, &data.fAnalysisRate,
                              &data.fWindowParam))) {
...

  {
    pv_stype stype = (fmt.wBitsPerSample == 16   ? STYPE_16
                      ...
                      : fmt.wBitsPerSample == 32 ? STYPE_32
                                                 : STYPE_IEEE_FLOAT);
    outf = csound->PVOC_CreateFile(
        csound, argv[2], (data.nAnalysisBins - 1) * 2, data.dwOverlap,
        fmt.nChannels, data.wAnalFormat, fmt.nSamplesPerSec, stype,
        data.wWindowType, data.fWindowParam, NULL, data.dwWinlen);
  }
  if (UNLIKELY(outf < 0)) {
...
int32_t pv_import_init_(CSOUND *csound) {
  ...
  if (!retval) {
    retval = csound->SetUtilityDescription(csound, "pv_import",
