...
static int32_t sndinfo(CSOUND *csound, int32_t argc, char **argv) {
  ...
  while (--argc) {
    ...
    if (UNLIKELY(hndl == NULL)) {
      ...
      csound->NotifyFileOpened(
          csound, fname, csound->sftype2csfiletype(sflib_info.format), 0, 0);
          ...
          for (k = 0; k < inst.loop_count; k++)
            csound->Message(
                ...
                (inst.loops[k].mode == SF_LOOP_NONE          ? "none"
                 ...
                 : inst.loops[k].mode == SF_LOOP_ALTERNATING ? "alt "
                                                             : ""),
...
int32_t sndinfo_init_(CSOUND *csound) {
  ...
  if (!retval) {
    retval = csound->SetUtilityDescription(
        csound, "sndinfo", Str("Prints information about sound files"));
  }
  return retval;
}
