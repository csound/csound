...
static int32_t pvxanal(CSOUND *csound, SOUNDIN *p, SNDFILE *fd,
                       const char *fname, int64_t srate, int64_t chans,
                       int64_t fftsize, int64_t overlap, int64_t winsize,
                       pv_wtype wintype, double beta, int32_t displays);
...
static int32_t init(CSOUND *csound, PVX **pvx, int64_t srate, int64_t fftsize,
                    int64_t winsize, int64_t overlap, pv_wtype wintype,
                    double beta);
...
static int32_t pvanal(CSOUND *csound, int32_t argc, char **argv) {
  ...
  do {
    ...
    if (*s++ == '-')
      ...
      case 'G':
        ...
        break;
      case 'V':
        FIND(Str("no output file for trace"));
        {
          void *dummy = csound->FileOpen2(csound, &trfil, CSFILE_STD, s, "w",
                                          NULL, CSFTYPE_OTHER_TEXT, 0);
  ...
  if (UNLIKELY(pvxanal(csound, p, infd, outfilnam, p->sr,
                       ...
                       frameSize, frameIncr, frameSize * 2, WindowType, beta,
                       displays) != 0)) {
...
static void PVDisplay_Init(CSOUND *csound, PVDISPLAY *p, int32_t fftSize,
                           int32_t dispCntMax) {
  int32_t i;
...
static void PVDisplay_Display(PVDISPLAY *p, int32_t frame) {
  ...
  if (p->dispFrame >= DISPFRAMES || p->dispCnt < p->dispCntMax)
    return;
  for (i = 0; i < p->npts; i++)
    p->dispBufs[p->dispFrame][i] =
        (MYFLT)sqrt((double)(p->dispBufs[p->dispFrame][i] / (MYFLT)p->dispCnt));
...
static int32_t pvxanal(CSOUND *csound, SOUNDIN *p, SNDFILE *fd,
  ...
  for (i = 0; i < chans; i++)
    rc +=
        init(csound, &pvx[i], srate, fftsize, winsize, overlap, wintype, beta);
  ...
  pvfile = csound->PVOC_CreateFile(csound, fname, fftsize, overlap, chans,
                                   PVOC_AMP_FREQ, srate, stype, wintype, 0.0f,
                                   NULL, winsize);
  if (UNLIKELY(pvfile < 0)) {
    csound->Message(csound, Str("pvxanal: unable to create analysis file: %s"),
  ...
  if (displays)
    PVDisplay_Init(
        csound, &disp, (int32_t)fftsize,
        (int32_t)(((int64_t)p->getframes * chans / overlap) / DISPFRAMES));

  while ((sampsread = csound->getsndin(csound, fd, inbuf, buflen_samps, p)) >
         0) {
    ...
    for (i = 0; i < sampsread / chans; i += overlap) {
      for (k = 0; k < chans; k++) {
        ...
        generate_frame(csound, pvx[k], chanbuf + i, frame, overlap,
                       PVOC_AMP_FREQ);
  ...
  csound->Message(csound, Str("\n%" PRId64 " %d-chan blocks written to %s\n"),
                  (int64_t)blocks_written / (int64_t)chans, (int32_t)chans,
                  fname);
...
static int32_t init(CSOUND *csound, PVX **pvx, int64_t srate, int64_t fftsize,
  ...
  if (M > N) {
    ...
    for (i = 1; i <= thispvx->analWinLen; i++)
      *(thispvx->analWindow + i) *= /* D.T. 2000*/
          (MYFLT)((double)N * sin((double)(PI * (i + 0.5 * Mf) / N)) /
                  (PI * (i + 0.5 * Mf)));
    for (i = 1; i <= thispvx->analWinLen; i++)
      *(thispvx->analWindow - i) = *(thispvx->analWindow + i - Mf);
  }
  ...
  thispvx->anal = (MYFLT *)csound->Malloc(csound, (N + 2) * sizeof(MYFLT));
  thispvx->oldInPhase =
...
static int64_t generate_frame(CSOUND *csound, PVX *pvx, MYFLT *fbuf,
                              float *outanal, int64_t samps,
                              int32_t frametype) {
  ...
  j = (pvx->nI - pvx->analWinLen - 1 + pvx->ibuflen) %
      pvx->ibuflen; /*input pntr*/
  ...
  if (frametype == PVOC_AMP_FREQ) {
    for (i = 0, i0 = anal, i1 = anal + 1, oi = pvx->oldInPhase; i <= pvx->N2;
         i++, i0 += 2, i1 += 2, oi++) {
      real = *i0;
      imag = *i1;
...
static void kaiser(MYFLT *win, int32_t len, double Beta) {
  ...
  for (i = -(double)len / 2.0 + 0.1; i < (double)len / 2.0; i++) {
    ...
    *ft++ = (MYFLT)(xarg * besseli(Beta * sqrt(1.0 - z * z)) / besseli(Beta));
  }
