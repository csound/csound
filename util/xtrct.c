...
static void usage(CSOUND *csound, char *mesg, ...) {
  ...
  csound->Message(csound, "%s",
                  Str("-N\t\tnotify (ring the bell) when done\n"));
  csound->Message(csound, "%s",
                  Str("-S integer\tsample number at which"
                      " to start file\n"));
  csound->Message(csound, "%s",
                  Str("-Z integer\tsample number at which"
                      " to end file\n"));
  csound->Message(csound, "%s", Str("-Q integer\tnumber of samples to read\n"));

  csound->Message(csound, "%s",
                  Str("-T fpnum\ttime in secs at which"
                      " to start file\n"));
  csound->Message(csound, "%s",
                  Str("-E fpnum\ttime in secs at which"
                      " to end file\n"));
...
static int32_t xtrct(CSOUND *csound, int32_t argc, char **argv) {
  ...
  do {
    ...
    if (*s++ == '-') /* read all flags:  */
      ...
      if (UNLIKELY(inputfile != NULL))
        usage(csound, Str("Too many inputs"));
      inputfile = --s;
    }
  } while (--argc);
  ...
  /* Read sound file */
  if (UNLIKELY(inputfile == NULL))
    usage(csound, Str("No input"));

  if (UNLIKELY(!(infd = EXsndgetset(csound, &xtrc, inputfile)))) {
    csound->Message(csound, Str("%s: error while opening %s"), argv[0],
                    inputfile);
    return 1;
  }
  ...
  if (debug) {
    csound->Message(csound, "Times %f %f %f\nNums %ld %ld %ld\n", xtrc.stime,
                    xtrc.endtime, xtrc.dur, xtrc.sample, xtrc.stop,
                    xtrc.numsamps);
  }
  if (xtrc.stime >= FL(0.0))
    xtrc.sample = (long)(xtrc.stime * xtrc.p->sr);
  if (xtrc.endtime >= FL(0.0))
    xtrc.numsamps = (long)(xtrc.endtime * xtrc.p->sr - xtrc.sample);
  else if (xtrc.dur >= FL(0.0))
    xtrc.numsamps = (long)(xtrc.dur * xtrc.p->sr);
  else if (xtrc.stop >= 0)
    xtrc.numsamps = xtrc.stop - xtrc.sample;
  else if (xtrc.numsamps < 0)
    xtrc.numsamps = xtrc.p->getframes - xtrc.sample;

  if (xtrc.sample < 0)
    xtrc.sample = 0;
  csound->Message(
      csound, Str("Extracting from sample %ld for %ld samples (%.5f secs)\n"),
  ...
  /* open file for write */
  fd = NULL;
  if (strcmp(O.outfilename, "stdout") == 0 || strcmp(O.outfilename, "-") == 0) {
    ...
    fd = csound->FileOpen2(csound, &outfd, CSFILE_SND_W, O.outfilename, &sfinfo,
                           "SFDIR",
                           csound->type2csfiletype(O.filetyp, O.outformat), 0);
  if (UNLIKELY(fd == NULL))
    csound->Die(csound, Str("Failed to open output file %s: %s"), O.outfilename,
...
static SNDFILE *EXsndgetset(CSOUND *csound, XTRC *x, char *name) {
  SNDFILE *infd;
  MYFLT dur;
  ...
  if ((infd = csound->sndgetset(csound, x->p)) ==
      0) /*open sndfil, do skiptime*/
    return (0);
  x->p->getframes = x->p->framesrem;
  dur = (MYFLT)x->p->getframes / x->p->sr;
  csound->Message(csound,
                  Str("extracting from %ld sample frames (%3.1f secs)\n"),
                  (long)x->p->getframes, dur);
  return (infd);
}

static void ExtractSound(CSOUND *csound, XTRC *x, SNDFILE *infd, SNDFILE *outfd,
                         OPARMS *oparms) {
