...
#ifdef HAVE_SOCKETS
extern "C" {
void *csound_test_sockrecv_create(CSOUND *, INSDS *, MYFLT *, MYFLT *, int32_t);
void csound_test_sockrecv_packet(void *, const MYFLT *, int32_t);
...
protected:
    ...
    void expect(const MYFLT *output, std::initializer_list<MYFLT> values) {
      EXPECT_EQ(std::vector<MYFLT>(output, output + 6),
...
TEST_F(SockrecvTests, MonoAndControlUnderrunsStaySilentAndResume)
{
    ...
    const MYFLT data[] = {FL(0.25), FL(0.5)};
