<CsTest>
description = "outrg returns subinstrument audio to its caller without leaking into the main output"
...
instr CheckSilence
  ...
  while kSample < ksmps do
    ...
    if kLeft != 0 || kRight != 0 then
      printks "outrg leaked subinstrument audio into the main mix\n", 0
      exitnowk -1
    endif
    kSample += 1
  od
...
instr CheckResults
  if i(gkChecks) != 2 then
    prints "outrg subinstrument checks did not complete\n"
    exitnow -1
  endif
endin
</CsInstruments>
