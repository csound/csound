...
; position wrapped by more than one loop length. Scalar and array outputs and
; the crossfade and hard-wrap readers are all checked.
...
instr 3
  ...
  if iXf > 0.001 || iArr > 0.001 || iHard > 0.001 || iFastXf > 0.001 || iFastHd > 0.001 then
    prints "diskin2 short loop read outside loop: xf=%g arr=%g hard=%g fastxf=%g fasthd=%g\n", iXf, iArr, iHard, iFastXf, iFastHd
    exitnow(1)
  endif
