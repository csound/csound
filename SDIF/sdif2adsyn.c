...
/* if anyone wants it, there is enough information to select a frequency range to output.*/
...
void main(int argc, char **argv)
{
	...
	float thistime;
	float maxfreq = 0.0f, minfreq = 100000.0f,maxamp = 0.0f, frame_maxamp = 0.0;	
	float scalefac = 1.0;	
	int doscale = 0;	 
	...
	/* STRATEGY: a very straight conversion from a frame structure to a breakpoint structure.
	 * for each new partial in the SDIF file, we start a new partial in the array, at the given index.
	 *  when a partial dies, the amplitude is (presumed to be) zero, but the list is not terminated.
	 * if an SDIF frame with that index starts a new partial, the partial with that index
	 * continues accordingly.
	 * 
	 *  Though in most cases, track indices will small and sequential, 
	 *  this cannot be presumed. If most or all indices are above MAXPARTIALS, we are b******d.
	 * Of course, the SDIF file will not tell us what the index range is. That would be too easy.
	 */
	...
	while((result = SDIF_ReadFrameHeader(&fh, infile))==ESDIF_SUCCESS){
		...
		/* Csound adsyn counts time in msecs, in shorts, so can only have 32.767 seconds! */
		if(thistime > 32.76f){
