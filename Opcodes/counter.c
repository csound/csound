...
static int32_t setcnt(CSOUND *csound, CNTSET *p) {
  COUNT *y;
  CNT_GLOBALS *q =
      (CNT_GLOBALS *)csound->QueryGlobalVariable(csound, "counterGlobals_");
  int m = 0;
  if (q == NULL) {
  ...
  if (q->free) {
    ...
    if (q->max_num >= q->used) {
      COUNT **tt = q->cnts;
      tt = (COUNT **)csound->ReAlloc(csound, tt,
                                     (q->max_num + 10) * sizeof(COUNT *));
      if (tt == NULL)
        return csound->InitError(csound, "%s",
...
static int32_t count_init_perf(CSOUND *csound, COUNTER *p) {
  if (count_init(csound, p) == OK)
    return count_perf(csound, p);
  return NOTOK;
}
...
static OENTRY counter_localops[] = {
    ...
    {"cntCycles", S(COUNTER), SK, 3, "k", "o", (SUBR)count_init,
     (SUBR)count_cycles},
    {"cntRead", S(COUNTER), SK, 3, "k", "o", (SUBR)count_init,
     (SUBR)count_read},
    {"cntReset", S(COUNTER), SK, 3, "", "o", (SUBR)count_init0,
     (SUBR)count_reset},
    {"cntState", S(CNTSTATE), SK, 3, "kkk", "o", (SUBR)count_init3,
     (SUBR)count_state},
