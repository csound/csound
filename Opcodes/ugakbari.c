...
static int32_t gainslider_perf(CSOUND *csound, gainslider *p) {
  IGN(csound);
  if (*p->kindex <= FL(0.0)) {
    *p->koutsig = FL(0.0);
  } else {
...
static OENTRY ugakbari_localops[] = {
    {"scale", sizeof(scale), 0, 2, "k", "kkkPO", NULL, (SUBR)scale_process,
     NULL},
    {"scale2", sizeof(SCALE2), 0, 3, "k", "kkkOPo", (SUBR)scale2_init,
     (SUBR)scale2_process, NULL},
    {"expcurve", sizeof(expcurve), 0, 2, "k", "kk", NULL, (SUBR)expcurve_perf,
     NULL},
    {"logcurve", sizeof(logcurve), 0, 2, "k", "kk", NULL, (SUBR)logcurve_perf,
     NULL},
