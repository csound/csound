  ...
  for (j = 0; j < n; j++) {
    ...
    if (ngain != FL(0.0) || ogain != FL(0.0))
      if (ngain != ogain) {
        ...
        p->curr_gains[j] = ogain + (MYFLT)(i)*invfloatn * gainsubstr;
      } else {
        for (i = offset; i < nsmps; ++i)
...
int32_t vbap_zak_control(CSOUND *csound, VBAP_ZAK *p) {
  ...
  calc_vbap_gns(p->ls_set_am, p->dim, p->ls_sets, p->updated_gains, n,
                p->cart_dir);
  ...
  if (*p->spread > FL(0.0)) {
    if (p->dim == 3) {
      ...
      new_spread_dir(&spreaddir[0], p->cart_dir, p->spread_base, *p->azi,
                     *p->spread);
      ...
      for (i = 1; i < spreaddirnum; i++) {
        new_spread_dir(&spreaddir[i], p->cart_dir, spreadbase[i], *p->azi,
                       *p->spread);
        calc_vbap_gns(p->ls_set_am, p->dim, p->ls_sets, tmp_gains, n,
                      spreaddir[i]);
        for (j = 0; j < n; j++) {
...
int32_t vbap_zak_init(CSOUND *csound,
                      VBAP_ZAK *p) { /* Initializations before run time */
  int32_t i, j, indx;
  ...
  if ((int32_t)*p->layout == 0)
    strcpy(name, "vbap_ls_table");
  else
    snprintf(name, 24, "vbap_ls_table_%d", (int32_t)*p->layout == 0);
...
int32_t vbap_zak_moving(CSOUND *csound,
                        VBAP_ZAK_MOVING *p) { /* during note performance: */
  MYFLT *outptr, *inptr;
  MYFLT ogain, ngain, gainsubstr;
  MYFLT invfloatn;
  int32_t j;
...
int32_t vbap_zak_moving_control(CSOUND *csound, VBAP_ZAK_MOVING *p) {
  ...
  if (*p->field_am >= FL(0.0)) { /* point-to-point */
    ...
    if (p->dim == 2) {
      p->ang_dir.azi = p->ang_dir.azi + (*p->fld[p->next_fld] * CS_ONEDKR);
      scale_angles(&(p->ang_dir));
    } else { /* 3D angular */
      p->ang_dir.azi = p->ang_dir.azi + (*p->fld[p->next_fld] * CS_ONEDKR);
      p->ang_dir.ele =
          p->ang_dir.ele + p->ele_vel * (*p->fld[p->next_fld + 1] * CS_ONEDKR);
...
int32_t vbap_zak_moving_init(CSOUND *csound, VBAP_ZAK_MOVING *p) {
  ...
  if (UNLIKELY(fabs(*p->field_am) < (2 + (p->dim - 2) * 2))) {
    return csound->InitError(
        csound, Str("Have to have at least %d directions in vbapzmove"),
        2 + (p->dim - 2) * 2);
  }
  if (p->dim == 2)
