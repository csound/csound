...
static int32_t psynth_init(CSOUND *csound, _PSYN *p) {
  ...
  if (UNLIKELY(p->func == NULL)) {
    return csound->InitError(csound, "%s",
                             Str("psynth: function table not found\n"));
  }
  ...
  if (*p->thresh == -1)
    p->min = 0.00002 * csound->Get0dBFS(csound);
  else
    p->min = *p->thresh * csound->Get0dBFS(csound);

  if (p->amps.auxp == NULL || (uint32_t)p->amps.size < sizeof(double) * numbins)
      ...
      while (i < maxtracks * 4) {
        ...
        if ((id = (int32_t)fin[i + 3]) != -1) {
          ...
          if (amp > min) {
            ...
            /* interpolation coefs */
            a2 = 3.0 / facsqr *
                 (phasediff - factor / 3.0 * (2.0 * freq + freqnext));
...
static OENTRY localops[] = {{"tradsyn", sizeof(_PSYN), 0, 3, "a", "fkkkij",
                             (SUBR)psynth_init, (SUBR)psynth_process},
                            {"sinsyn", sizeof(_PSYN2), TR, 3, "a", "fkkij",
                             (SUBR)psynth2_init, (SUBR)psynth2_process},
                            {"resyn", sizeof(_PSYN), TR, 3, "a", "fkkkij",
                             (SUBR)psynth_init, (SUBR)psynth3_process},
                            {"trscale", sizeof(_PTRANS), 0, 3, "f", "fz",
                             (SUBR)trans_init, (SUBR)trscale_process},
                            {"trshift", sizeof(_PTRANS), 0, 3, "f", "fz",
                             (SUBR)trans_init, (SUBR)trshift_process},
                            {"trsplit", sizeof(_PSPLIT), 0, 3, "ff", "fz",
                             (SUBR)trsplit_init, (SUBR)trsplit_process},
                            {"trmix", sizeof(_PSMIX), 0, 3, "f", "ff",
                             (SUBR)trmix_init, (SUBR)trmix_process},
                            {"trlowest", sizeof(_PLOW), 0, 3, "fkk", "fk",
                             (SUBR)trlowest_init, (SUBR)trlowest_process},
                            {"trhighest", sizeof(_PLOW), 0, 3, "fkk", "fk",
                             (SUBR)trlowest_init, (SUBR)trhighest_process},
                            {"trfilter", sizeof(_PSFIL), 0, 3, "f", "fki",
                             (SUBR)trfil_init, (SUBR)trfil_process},
                            {"trcross", sizeof(_PSCROSS), 0, 3, "f", "ffkz",
                             (SUBR)trcross_init, (SUBR)trcross_process},
                            {"binit", sizeof(_PSBIN), 0, 3, "f", "fi",
                             (SUBR)binit_init, (SUBR)binit_process}};
