...
int
psynth2_init(ENVIRON *csound, _PSYN2 *p){
	int numbins = p->fin->N/2 + 1;
	if(p->fin->format != PVS_TRACKS){
		csound->InitError(csound, "psynth: first input not in TRACKS format \n");
		return NOTOK;
		...
		if(pos == hopsize) {
			...
			while(i < maxtracks*4){
				...
				if((id = (int) fin[i+3]) != -1){
					...
					/* interpolation coefs */
					a2 = 3./facsqr * (phasediff - factor/3.*(2.*freq+freqnext));
...
static OENTRY localops[] = {
	...
	{"sinsyn", sizeof(_PSYN2), 5, "a", "fkki", (SUBR)psynth2_init, NULL, (SUBR)psynth2_process },
};
