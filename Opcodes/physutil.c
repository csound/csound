...
void DLineL_setDelay(DLineL *p, MYFLT lag) {
  MYFLT outputPointer =
      p->inPoint - lag; /* read chases write, +1 for interp. */
...
MYFLT Envelope_tick(Envelope *e) {
  //    printf("(Envelope_tick: %p state=%d target=%f, rate=%f, value=%f => ",
  //    e,
...
void OnePole_print(CSOUND *csound, OnePole *p) {
  csound->Message(csound, "OnePole: gain=%f outputs=%f poleCoeff=%f sgain=%f\n",
