...
static HANDLE get_port(CSOUND *csound, int32_t port) {
  HANDLE hport;
  SERIAL_GLOBALS *q;
  q = (SERIAL_GLOBALS *)csound->QueryGlobalVariable(csound, "serialGlobals_");
  if (q == NULL) {
...
int32_t serialport_init(CSOUND *csound, const char *serialport, int32_t baud) {
  ...
  hSerial = CreateFileA(serialport, GENERIC_READ | GENERIC_WRITE, 0, NULL,
...
#endif
#ifndef WIN32
  if (UNLIKELY(write((int32_t)*p->port, ((STRINGDAT *)p->toWrite)->data,
                     ((STRINGDAT *)p->toWrite)->size)) !=
      ((STRINGDAT *)p->toWrite)
          ->size) /* Does Windows write behave correctly? */
    return NOTOK;
#else
  int32_t nbytes;
  WriteFile(port, p->toWrite, strlen((char *)p->toWrite), (PDWORD)&nbytes,
            NULL);
#endif
  return OK;
}
...
/* Basic design:  when arduinoStart is called it opens serial line like
   ...
   Issue: it assumes that the arduino is already running the correct sketch
   type. Issue:  Can we load the sketch from csound? Issue: Need to stop the
   listen thread. Issue: Windows version incomplete
*/
...
#define SYN (0xf8)
  ...
  while (1) {
    ...
    c = (hi >> 3) & 0x1f;
    if (DEBUG)
      printf("In bits: va1=%.2x va2= %.2x; c1=%.2x\n", (hi & 0x7) << 7,
             low & 0x7f, (hi >> 3) & 0x1f);
    if (DEBUG)
...
int32_t arduino_deinit(CSOUND *csound, ARD_START *p) { /* NOT FINISHED */
  p->q->stop = 1;
...
int32_t arduinoStart(CSOUND *csound, ARD_START *p) {
  ARDUINO_GLOBALS *q;
  int n;
  MYFLT xx = (MYFLT)serialport_init(csound, (const char *)p->portName->data,
                                    *p->baudRate);
  // printf("xx=%g\n", xx);
  if (xx < 0)
  ...
  q = (ARDUINO_GLOBALS *)csound->QueryGlobalVariable(csound, "arduinoGlobals_");
  if (q == NULL)
    return csound->InitError(csound, "&%s", Str("Failed to allocate\n"));
  p->q = q;
  q->csound = csound;
...
int32_t arduinoReadSetup(CSOUND *csound, ARD_READ *p) {
  p->q =
      (ARDUINO_GLOBALS *)csound->QueryGlobalVariable(csound, "arduinoGlobals_");
  if (p->q == NULL)
...
int32_t arduinoReadF(CSOUND *csound, ARD_READF *p) {
  ...
  int ind3 = *p->index3;
  int c1, c2, c3;
  if (ind1 < 0 || ind1 > MAXSENSORS || ind2 < 0 || ind2 > MAXSENSORS ||
...
static OENTRY serial_localops[] = {
    ...
    {"arduinoRead", S(ARD_READ), 0, 3, "k", "iio", (SUBR)arduinoReadSetup,
     (SUBR)arduinoRead},
    {"arduinoReadF", S(ARD_READF), 0, 3, "k", "iiii", (SUBR)arduinoReadFSetup,
     (SUBR)arduinoReadF},
    {"arduinoStop", S(ARD_START), 0, 1, "", "i", (SUBR)arduinoStop, NULL},
    /* { (char *)"serialAvailable", S(SERIALAVAIL), 0, 2, (char *)"k", (char
     *)"i", */
    /*   (SUBR)NULL, (SUBR)serialAvailable, (SUBR)NULL   }, */
    /* { (char *)"serialPeekByte", S(SERIALPEEK),0,  2, (char *)"k", (char
     *)"i", */
