...
static int32_t make_Modal4(CSOUND *csound, Modal4 *m, MYFLT *ifn, MYFLT vgain,
                           MYFLT vrate) {
  FUNC *ftp;
...
void Modal4_setRatioAndReson(CSOUND *csound, Modal4 *m, int32_t whichOne,
  ...
  if (ratio * m->baseFreq < m->sr * FL(0.5)) {
    ...
    temp = ratio;
    while (temp * m->baseFreq > FL(0.5) * m->sr)
      temp *= FL(0.5);
...
int32_t marimbaset(CSOUND *csound, MARIMBA *p) {
  ...
  if (UNLIKELY(make_Modal4(csound, m, p->ivfn, *p->vibAmt, *p->vibFreq) ==
               NOTOK))
    return NOTOK;
  p->m4.w_phaseOffset = FL(0.0);
  /*     p->m4.w_rate = 0.5; */
  Modal4_setRatioAndReson(csound, m, 0, FL(1.00),
                          FL(0.9996)); /* Set all 132.0 */
  Modal4_setRatioAndReson(csound, m, 1, FL(3.99),
                          FL(0.9994)); /* of our  523.0 */
  Modal4_setRatioAndReson(csound, m, 2, FL(10.65),
                          FL(0.9994)); /* default 1405.0 */
  Modal4_setRatioAndReson(csound, m, 3, -FL(18.50),
                          FL(0.999));  /* resonances 2443 */
  ...
  temp = SIN(temp2);
  BiQuad_setGain(p->m4.filters[0],
                 FL(0.12) * temp); /* 1st mode function of pos.*/
  ...
  BiQuad_setGain(p->m4.filters[2],
                 FL(0.11) * temp); /* 3rd mode function of pos.*/
                                   /* Strike */
  {
    int32_t triples = (*p->triples <= FL(0.0) ? 20 : (int32_t)*p->triples);
    int32_t doubles =
        (*p->doubles <= FL(0.0) ? 40 : triples + (int32_t)*p->doubles);
    OPARMS parm;
...
int32_t vibraphnset(CSOUND *csound, VIBRAPHN *p) {
  ...
  Modal4_setRatioAndReson(csound, m, 3, FL(14.37),
                          FL(0.99990)); /*  values here */
  ...
  BiQuad_setGain(p->m4.filters[1],
                 FL(0.015) * SIN(FL(0.1) + (FL(2.01) * temp)));
...
int32_t agogobelset(CSOUND *csound, VIBRAPHN *p) {
  ...
  Modal4_setRatioAndReson(csound, m, 3, -FL(3725.0),
                          FL(0.999)); /* values here */
