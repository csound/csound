...
static const float nonlinitd[5] = {1.570024f, 1.378733f, 1.155164f, 1.101230f,
                                   1.0f};
static const float nonlinitd48k[5] = {1.549748f, 1.305457f, 1.124501f,
                                      1.112852f, 1.0f};
static const float nonlinitd96k[5] = {1.550297f, 1.305671f, 1.124456f,
                                      1.112818f, 1.0f};
...
public:
  ...
  virtual int32_t hrtfstat_init(CSOUND *csound, MYFLT elev, MYFLT angle,
                                MYFLT radius, STRINGDAT *filel,
                                STRINGDAT *filer, MYFLT srxl) = 0;
  virtual int32_t hrtfstat_process(CSOUND *csound, MYFLT *in, MYFLT *outsigl,
                                   MYFLT *outsigr, uint32_t offset,
                                   uint32_t early, uint32_t nsmps) = 0;
};
...
private:
  ...
  AUXCH lowl1_p, lowr1_p, lowl2_p, lowr2_p, highl1_p, highr1_p, highl2_p,
      highr2_p;
...
public:
  ...
  virtual int32_t hrtfstat_init(CSOUND *csound, MYFLT elev, MYFLT angle,
                                MYFLT r, STRINGDAT *ifilel, STRINGDAT *ifiler,
                                MYFLT sr) {
    ...
    if (!complexinsig_p.auxp ||
        complexinsig_p.size < irlengthpad * sizeof(MYFLT))
    ...
    magllow =
        FABS(lowl1[0]) + (FABS(lowl2[0]) - FABS(lowl1[0])) * angleindex2per;
    maglhigh =
        FABS(highl1[0]) + (FABS(highl2[0]) - FABS(highl1[0])) * angleindex4per;
    hrtflfloat[0] = magllow + (maglhigh - magllow) * elevindexhighper;

    magllow =
        FABS(lowl1[1]) + (FABS(lowl2[1]) - FABS(lowl1[1])) * angleindex2per;
    maglhigh =
        FABS(highl1[1]) + (FABS(highl2[1]) - FABS(highl1[1])) * angleindex4per;
    hrtflfloat[1] = magllow + (maglhigh - magllow) * elevindexhighper;

    magrlow =
        FABS(lowr1[0]) + (FABS(lowr2[0]) - FABS(lowr1[0])) * angleindex2per;
    magrhigh =
        FABS(highr1[0]) + (FABS(highr2[0]) - FABS(highr1[0])) * angleindex4per;
    hrtfrfloat[0] = magrlow + (magrhigh - magrlow) * elevindexhighper;

    magrlow =
        FABS(lowr1[1]) + (FABS(lowr2[1]) - FABS(lowr1[1])) * angleindex2per;
    magrhigh =
        FABS(highr1[1]) + (FABS(highr2[1]) - FABS(highr1[1])) * angleindex4per;
...
typedef struct {
  ...
  double *currPos[MAX_INPUTS];
  /* delay-line current position pointer */ /* >>Was float<< */
...
static int32_t ihoambdec(CSOUND *csound, HOAMBDEC *p) {
  ...
  case 36:
    ...
    break;
  default:
    return csound->InitError(csound, "%s", Str("illegal number of inputs"));
  }

  if ((isetup == 1) & (p->order >= 2)) {
    return csound->InitError(
        csound, "%s", Str("Stereo configuration only works with first order"));
  }
  if ((isetup == 2) & (p->order >= 2)) {
    return csound->InitError(
        csound, "%s", Str("Quad configuration only works with first order"));
  }
  ...
  if ((isetup == 4) & (p->order >= 4)) {
    return csound->InitError(csound, "%s",
                             Str("Octagon configuration only works with first, "
                                 "second and third order"));
  }

  if ((isetup == 5) & (p->order >= 2)) {
    return csound->InitError(
        csound, "%s", Str("Cube configuration only works with first order"));
  }
  ...
  if ((isetup == 21) & (p->order >= 4)) {
    return csound->InitError(csound, "%s",
                             Str("2D Binaural configuration only works with "
                                 "first, second and third order"));
  }

  if ((isetup == 31) & (p->order >= 4)) {
    return csound->InitError(csound, "%s",
                             Str("3D Binaural configuration only works with "
                                 "first, second and third order"));
  }

  p->horizontal = ((isetup == 1) | (isetup == 2) | (isetup == 3) |
                   (isetup == 4) | (isetup == 6) | (isetup == 21));
  ...
  if (isetup == 3) { // 5.0 order 1
    ...
    if (p->order == 2) {
      ...
      // ( -0.6902095308,   2.032716615,  0.2831185829,  -1.544664776,
      // 0.4138916792); ( -0.6902095308,   2.032716615, -0.2831185829,
      // -1.544664776, -0.4138916792); (   1.603030056,  -2.914433535,
      // -0,   2.780920112,            -0); (  0.5958012839, -0.5754998474,
      // 0.3814446348,  0.1542047194, -0.2202271626); (  0.5958012839,
      // -0.5754998474, -0.3814446348,  0.1542047194,  0.2202271626);
  ...
  if (isetup == 4 || isetup == 21) { // Octagon or binaural 2D
    ...
    // Decoders matrix
    // 0.1767766953,  0.2309698831,  0.0956708581,  0.1767766953,  0.1767766953,
    // 0.0956708581,  0.2309698831
    //  0.1767766953,  0.0956708581,  0.2309698831, -0.1767766953, 0.1767766953,
    //  -0.2309698831, -0.0956708581
    // 0.1767766953, -0.0956708581,  0.2309698831, -0.1767766953, -0.1767766953,
    // 0.2309698831, -0.0956708581 0.1767766953, -0.2309698831,  0.0956708581,
    // 0.1767766953, -0.1767766953, -0.0956708581,  0.2309698831
    // 0.1767766953, -0.2309698831, -0.0956708581,  0.1767766953,  0.1767766953,
    // -0.0956708581, -0.2309698831 0.1767766953, -0.0956708581, -0.2309698831,
    // -0.1767766953,  0.1767766953,  0.2309698831,  0.0956708581
    // 0.1767766953,  0.0956708581, -0.2309698831, -0.1767766953, -0.1767766953,
    // -0.2309698831,  0.0956708581 0.1767766953,  0.2309698831, -0.0956708581,
    // 0.1767766953, -0.1767766953,  0.0956708581, -0.2309698831
    const1 = 0.1767766953;
  ...
  if (isetup == 5) { // Cube order 1
    ...
    double M_lf[8][4] = {
        {const1, const2, const2, 0.0},   {const1, const2, const2, 0.0},
        {const1, -const2, const2, 0.0},  {const1, -const2, const2, 0.0},
        {const1, -const2, -const2, 0.0}, {const1, -const2, -const2, 0.0},
        {const1, const2, -const2, 0.0},  {const1, const2, -const2, 0.0}};
  ...
  if (isetup == 6) { // hexagon
    ...
    // Decoders matrix
    //(  0.2357022604,  0.2886751346,  0.1666666667,  0.1666666667,
    // 0.2886751346); (  0.2357022604,             0,  0.3333333333,
    //-0.3333333333,            -0); (  0.2357022604, -0.2886751346,
    // 0.1666666667,  0.1666666667, -0.2886751346); (  0.2357022604,
    //-0.2886751346, -0.1666666667,  0.1666666667,  0.2886751346); (
  ...
  if (isetup == 31) { // binaural 3D (dodecahedron)
    ...
    // 0.0707106781,  0.0866025404,  0.0866025404, 0.0866025404,  -0, 0.125,
    // 0.125,-0, 0.125, -0.1948557159,  -0.024376941,  0.225623059,
    // 0.1397542486,0.125, -0.2125578696,  0.0310117752);
    //  0.0707106781,  0.0866025404, -0.0866025404,  0.0866025404,  0, 0.125,
    //  -0.125, 0,-0.125, -0.1948557159,  -0.024376941,  -0.225623059,
    //  0.1397542486, -0.125, -0.2125578696, -0.0310117752); 0.0707106781,
    //  -0.0866025404, -0.0866025404,  0.0866025404, -0,-0.125, -0.125,-0,
    //  0.125, -0.1948557159,   0.024376941,-0.225623059,  0.1397542486,0.125,
    //  0.2125578696, -0.0310117752
    // 0.0707106781, -0.0866025404,  0.0866025404,  0.0866025404, -0,-0.125,
    // 0.125,0, -0.125, -0.1948557159,   0.024376941,  0.225623059,
    // 0.1397542486, -0.125,  0.2125578696,  0.0310117752
    //  0.0707106781, 0.0866025404, 0.0866025404, -0.0866025404,0,
    //  -0.125,-0.125,0, 0.125,  0.1948557159,  -0.024376941, 0.225623059,
    //  -0.1397542486,-0.125, -0.2125578696,  0.0310117752 0.0707106781,
    //  0.0866025404, -0.0866025404, -0.0866025404,0,-0.125,0.125,-0, -0.125,
    //  0.1948557159,  -0.024376941, -0.225623059, -0.1397542486,0.125,
    //  -0.2125578696, -0.0310117752 0.0707106781, -0.0866025404, -0.0866025404,
    //  -0.0866025404, -0, 0.125, 0.125,  -0,  0.125,  0.1948557159,
    //  0.024376941,  -0.225623059, -0.1397542486,-0.125,  0.2125578696,
    //  -0.0310117752 0.0707106781, -0.0866025404,  0.0866025404,
    //  -0.0866025404,-0,  0.125,-0.125,-0,-0.125,  0.1948557159,   0.024376941,
    //  0.225623059, -0.1397542486,0.125,  0.2125578696,  0.0310117752
    // 0.0707106781,  0.1401258538,  0.0535233135,-0, -0.125, 0, -0,
    // 0.1397542486, 0.125, 0, -0.0658359214, -0.3341640786, 0, -0,
    // 0.1623797632,  0.1623797632 0.0707106781,  0.1401258538, -0.0535233135,
    // -0, -0.125, 0, 0,  0.1397542486, -0.125, -0, -0.0658359214,
    // 0.3341640786,-0, -0,  0.1623797632, -0.1623797632 0.0707106781,
    // -0.1401258538, -0.0535233135,  0, -0.125, 0, -0,  0.1397542486, 0.125,
    // -0,  0.0658359214,  0.3341640786, -0, 0, -0.1623797632, -0.1623797632
    // 0.0707106781, -0.1401258538,  0.0535233135,  0, -0.125, -0, 0,
    // 0.1397542486, -0.125, -0,  0.0658359214, -0.3341640786, -0, 0,
    // -0.1623797632,  0.1623797632
    //  0.0707106781,  -0,  0.1401258538,  0.0535233135, -0.0772542486, 0,0.125,
    //  -0.1636271243,  0,  0.0802849702, 0,  -0.151246118, -0.3272542486, 0,-0,
    //  -0.1003562127 0.0707106781, 0,  0.1401258538, -0.0535233135,
    //  -0.0772542486,  -0, -0.125, -0.1636271243,  0, -0.0802849702, 0,
    //  -0.151246118,  0.3272542486, 0,  -0, -0.1003562127
    // 0.0707106781, -0, -0.1401258538, -0.0535233135, -0.0772542486,  -0,
    // 0.125, -0.1636271243,  -0, -0.0802849702,  -0,   0.151246118,
    // 0.3272542486,   0,  0,  0.1003562127
    //  0.0707106781,  0, -0.1401258538,  0.0535233135, -0.0772542486, -0,
    //  -0.125, -0.1636271243, 0,  0.0802849702,-0,   0.151246118,
    //  -0.3272542486,  -0,-0,  0.1003562127 0.0707106781,  0.0535233135, -0,
    //  0.1401258538,  0.2022542486, 0.125, 0,  0.0238728757,  0,  0.2101887808,
    //  0.251246118, -0, -0.0477457514, -0,   0.262735976,  -0 0.0707106781,
    //  0.0535233135,-0, -0.1401258538,  0.2022542486,  -0.125, -0,
    //  0.0238728757,  -0, -0.2101887808,   0.251246118,  -0,  0.0477457514, 0,
    //  0.262735976, -0
    // 0.0707106781, -0.0535233135,  0, -0.1401258538,  0.2022542486,  0.125, 0,
    // 0.0238728757, -0, -0.2101887808,  -0.251246118, 0,  0.0477457514,  0,
    // -0.262735976, 0
    //  0.0707106781, -0.0535233135, 0,  0.1401258538,  0.2022542486,  -0.125,
    //  -0,  0.0238728757,  -0,  0.2101887808,  -0.251246118, 0, -0.0477457514,
    //  -0,  -0.262735976,0

    double M_lf[20][16] = {
        {const1, const2, const2, const2, 0.0, const3, const3, 0.0, const3,
         -const4, -const5, const6, const7, const3, -const8, const9},
        {const1, const2, -const2, const2, 0.0, const3, -const3, 0.0, -const3,
         -const4, -const5, -const6, const7, -const3, -const8, -const9},
        {const1, -const2, -const2, const2, 0.0, -const3, -const3, 0.0, const3,
         -const4, const5, -const6, const7, const3, const8, -const9},
        ...
        {const1, -const2, const2, -const2, 0.0, const3, -const3, 0.0, -const3,
         const4, const5, const6, -const7, const3, const8, const9},
        {const1, const10, const11, 0.0, -const3, 0.0, 0.0, const7, const3, 0.0,
         -const12, -const13, 0.0, 0.0, const14, const14},
        {const1, const10, -const11, 0.0, -const3, 0.0, 0.0, const7, -const3,
         0.0, -const12, const13, 0.0, 0.0, const14, -const14},
        {const1, -const10, -const11, 0.0, -const3, 0.0, 0.0, const7, const3,
         0.0, const12, const13, 0.0, 0.0, -const14, -const14},
        {const1, -const10, const11, 0.0, -const3, 0.0, 0.0, const7, -const3,
         0.0, const12, -const13, 0.0, 0.0, -const14, const14},
        {const1, 0.0, const10, const11, -const15, 0.0, const3, -const16, 0.0,
         const17, 0.0, -const18, -const19, 0.0, 0.0, const20},
        {const1, 0.0, const10, -const11, -const15, 0.0, -const3, -const16, 0.0,
         -const17, 0.0, -const18, const19, 0.0, 0.0, -const20},
        {const1, 0.0, -const10, -const11, -const15, 0.0, const3, -const16, 0.0,
         -const17, 0.0, const18, const19, 0.0, 0.0, const20},
        {const1, 0.0, -const10, const11, -const15, 0.0, -const3, -const16, 0.0,
         const17, 0.0, const18, -const19, 0.0, 0.0, const20},
        {const1, const11, 0.0, const10, const21, const3, 0.0, const22, 0.0,
         const23, const24, 0.0, -const25, 0.0, const26, 0.0},
        {const1, const11, 0.0, -const10, const21, -const3, 0.0, const22, 0.0,
         -const23, const24, 0.0, const25, 0.0, const26, 0.0},
        {const1, -const11, 0.0, -const10, const21, const3, 0.0, const22, 0.0,
         -const23, const24, 0.0, const25, 0.0, -const26, 0.0},
  ...
  if (isetup == 21) { // binaural 2D
    int elev = 0;
    float angle[8] = {-22.5f, -67.5f, -112.5f, -157.5f,
                      157.5f, 112.5f, 67.5f,   22.5f};
    int r = 1;
    ...
    for (int j = 0; j < 8; j++) {
      ...
      p->binaural[j]->hrtfstat_init(csound, elev, angle[j], r, p->ifilel,
                                    p->ifiler, CS_ESR);
    }
  }

  if (isetup == 31) { // binaural 3D
    float angle[20] = {-45.0f,      45.0f,      135.0f,      -135.0f,
                       -45.0f,      45.0f,      135.0f,      -135.0f,
                       -20.905157f, 20.905157f, 159.094843f, -159.094843f,
    ...
    for (int j = 0; j < 20; j++) {
      if (p->binaural_mem[j].auxp == NULL)
        csound->AuxAlloc(csound, sizeof(hrtf_c), &p->binaural_mem[j]);
      p->binaural[j] = new (p->binaural_mem[j].auxp) hrtf_c;
      p->binaural[j]->hrtfstat_init(csound, elev[j], angle[j], r, p->ifilel,
                                    p->ifiler, CS_ESR);
    }
  }
...
static int32_t ahoambdec(CSOUND *csound, HOAMBDEC *p) {
  ...
  // double
  // poleSamp_nfc[p->n_signals],inSamp_nfc[p->n_signals],zeroSamp_nfc[p->n_signals];

  double y_lf, y_hf;

  // dict to convert 3d ambisonic signals in 2d. MAX for order 5
  int dict_3dto2d[11] = {0, 1, 2, 7, 8, 14, 15, 23, 24, 34, 35};
  int order_signals3d[36] = {0, 1, 1, 1, 2, 2, 2, 2, 2, 3, 3, 3,
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    if (isetup == 1) { // Stereo configuration. Nor band splitting nor near
                       // field compensation.
      /* Left: */
      out[n] = in[n] * SQRT(FL(0.5)) +
               in[2 * ksmps + n] * FL(0.5); // w*sqrt(0.5) + y*0.5;
      /* Right: */
      out[ksmps + n] = in[n] * SQRT(FL(0.5)) -
                       in[2 * ksmps + n] * FL(0.5); // w*sqrt(0.5) - y*0.5;
      continue;
    }
    ...
    for (j = 0; j < p->n_signals; j++) { // ambisonics signals
      ...
      for (int o = 0; o < n_outs_A; o++) {
        if (isetup == 21 || isetup == 31) { // binaural
          switch ((int)*(p->band)) {
          case 0:
            out_A[o * ksmps + n] += (MYFLT)((p->M_lf[o][in_ix]) * y_lf -
                                            (p->M_hf[o][in_ix]) * y_hf);
            break;
          case 1:
            out_A[o * ksmps + n] += (MYFLT)y_lf;
            break;
          case 2:
            out_A[o * ksmps + n] += (MYFLT)y_hf;
            break;
          default:;
          }
        } else {
          switch ((int)*(p->band)) {
          case 0:
            out[o * ksmps + n] += (MYFLT)((p->M_lf[o][in_ix]) * y_lf -
                                          (p->M_hf[o][in_ix]) * y_hf);
            break;
          case 1:
            out[o * ksmps + n] += (MYFLT)y_lf;
            break;
          case 2:
            out[o * ksmps + n] += (MYFLT)y_hf;
            break;
          default:;
          }
        }
  ...
  if (isetup == 21 || isetup == 31) { // binaural 2D or 3D
    for (int j = 0; j < n_outs_A; j++) {
      p->binaural[j]->hrtfstat_process(csound, &out_A[j * ksmps], out_binaural0,
                                       out_binaural1, offset, early, nsmps);
...
static void insertFilter(HOAMBDEC *p, double val, int j) {
  ...
  *p->currPos[j] = val;
  ...
  if (((double *)(++p->currPos[j])) >
      ((double *)p->delay[j].auxp + (delay - 1)))
    p->currPos[j] -= delay;
}
 ...
 *
 */
static void process_nfc(CSOUND *csound, HOAMBDEC *p, int signal_order, int n,
                        int j, int in_ix, int sr, int ksmps) {
  // char buffer[50];
...
static OENTRY bformdec2_localops[] = {
    {(char *)"bformdec2.A", S(HOAMBDEC), 0, 3, (char *)"a[]",
     (char *)"ia[]ooooNN", (SUBR)ihoambdec, (SUBR)ahoambdec},
};
