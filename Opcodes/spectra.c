...
/* db or mag2) of log freq within each octave */
{
  ...
  if (nocts != dwnp->nocts ||
    ...
    if (csound->GetTypeForArg(p->signal) !=
        csound->AsigType(csound)) { /* for sr sampling:           */
      oct =
          ((int32_t)(oct * 12.0 + 0.5)) / 12.0; /*     semitone round to A440 */
    ...
    p->linbufp = fltp;
    fltp += windsiz; /* linbuf must take nsamps */
    p->sinp = sinp = fltp;
    fltp += sumk;
    ...
    for (n = nocts, octp = dwnp->octdata + (nocts - 1); n--; octp--) {
      bufsiz = majr + minr;
      octp->begp = fltp;
      fltp += bufsiz; /*        (lo oct first) */
      octp->endp = fltp;
      minr *= 2;
    }
    csound->Warning(csound,
                    Str("\t%d oct analysis window "
    ...
    if (p->disprd) {                      /* if display requested, */
      ...
      csound->AuxAlloc(csound, (size_t)totsize,
                       &p->auxch2); /*  linear output window */
      csound->dispset(csound, &p->octwindow, (MYFLT *)p->auxch2.auxp,
                      (int32_t)totsamps, Str("octdown buffers:"), 0,
                      "spectrum");
    }
...
int32_t spectrum(CSOUND *csound, SPECTRUM *p) {
  ...
  do {
    ...
    if (offset--)
      SIG = FL(0.0);       /* for sample accuracy         */
    ...
    do { /*   then for each oct:        */
      ...
      *curp++ = SIG; /*  write samp to cur buf  */
      ...
      octp->curp = curp;
      if (!(--nocts))
        break; /*  if lastoct, break      */
      coefp = bicoefs;
      ytp = octp->feedback;
      for (nfilt = 3; nfilt--;) { /*  apply triple biquad:   */
        yt2 = *ytp++;
        yt1 = *ytp--;            /* get prev feedback */
        SIG -= (*coefp++ * yt1); /* apply recurs filt */
        SIG -= (*coefp++ * yt2);
        *ytp++ = yt1;
        *ytp++ = SIG; /* stor nxt feedback */
        SIG *= *coefp++;
  ...
  if ((--p->scountdown))
    return OK;                 /* if not yet time for new spec, return */
...
/*       return csound->InitError(csound,
                                  "%s", Str("noctdft: unknown dbout code of
   %d"), */
/*                                        p->dbout); */
/*     } */
/*     nocts = downp->nocts; */
  ...
  if (!(p->dwindow.windid)) {
    ...
    if (downp->lofrq > FL(5.0)) {
      snprintf(strmsg, 256,
               ...
               outstring[specp->dbout], downp->nocts, (int32_t)downp->lofrq,
               (int32_t)downp->hifrq);
    } else { /* more detail if low frequency  */
      snprintf(strmsg, 256,
               Str("instr %d %s, dft (%s), %d octaves (%3.1f - %3.1f Hz):"),
               (int32_t)p->h.insdshead->p1.value, p->h.optext->t.inlist->arg[0],
               outstring[specp->dbout], downp->nocts, downp->lofrq,
               downp->hifrq);
    }
...
int32_t sptrkset(CSOUND *csound, SPECPTRK *p) {
  ...
  if ((npts = inspecp->npts) != p->winpts) {   /* if size has changed */
    SPECset(csound, &p->wfund, (int32_t)npts); /*   realloc for wfund */
  ...
  case 0:
    ...
    break;
  case 1:
    p->threshon = dbthresh; /* db  */
    p->threshoff = dbthresh - FL(6.0);
    break;
  case 2:
    p->threshon = ampthresh * ampthresh; /* mag sqrd */
    p->threshoff = p->threshon / FL(4.0);
    break;
  case 3:
    p->threshon = (MYFLT)sqrt(ampthresh); /* root mag */
  ...
  csound->Warning(csound,
                  ...
                  dbthresh, outstring[inspecp->dbout], p->threshon,
                  p->threshoff);
...
int32_t specptrk(CSOUND *csound, SPECPTRK *p) {
  ...
  if (inspecp->ktimstamp == CS_KCNT) { /* if inspectrum is new: */
    ...
    lobin = (int32_t)((kval - kvar) *
                      inspecp->nfreqs); /* set lim of frq interest */
...
{
  SPECDAT *specp = p->wsig;
  if (UNLIKELY(specp->auxch.auxp == NULL))
    goto err1;                       /* RWD fix */
...
int32_t spadmset(CSOUND *csound, SPECADDM *p) {
  ...
  if (npts != p->waddm->npts) {               /* if out does not match ins */
    SPECset(csound, p->waddm, (int32_t)npts); /*       reinit the out spec */
...
int32_t spdifset(CSOUND *csound, SPECDIFF *p) {
  ...
  if ((npts = inspecp->npts) != p->specsave.npts) { /* if inspec not matched  */
    SPECset(csound, &p->specsave, (int32_t)npts);   /*   reinit the save spec */
    SPECset(csound, p->wdiff, (int32_t)npts);       /*   & the out diff spec  */
...
int32_t spsclset(CSOUND *csound, SPECSCAL *p) {
  ...
  if ((npts = inspecp->npts) != outspecp->npts) { /* if size has changed,   */
    SPECset(csound, outspecp, (int32_t)npts);     /*    realloc             */
...
int32_t specscal(CSOUND *csound, SPECSCAL *p) {
  ...
  if ((inspecp->auxch.auxp == NULL) /* RWD fix */ ||
      (p->wscaled->auxch.auxp == NULL) || (p->fscale == NULL))
    goto err1;
  if (inspecp->ktimstamp == CS_KCNT) { /* if inspectrum is new: */
    SPECDAT *outspecp = p->wscaled;
    MYFLT *inp = (MYFLT *)inspecp->auxch.auxp;
    ...
    if (p->thresh) { /* if thresh requested,  */
      ...
      for (n = 0; n < npts; n++) {
        if ((val = inp[n] - threshp[n]) > FL(0.0)) /* for vals above thresh */
          ...
          outp[n] = FL(0.0); /*   else output is 0.   */
      }
    } else {
      for (n = 0; n < npts; n++) {
...
int32_t sphstset(CSOUND *csound, SPECHIST *p) {
  ...
  if ((npts = inspecp->npts) != p->accumer.npts) { /* if inspec not matched   */
    SPECset(csound, &p->accumer, (int32_t)npts);   /*   reinit the accum spec */
    SPECset(csound, p->wacout, (int32_t)npts);     /*    & the output spec    */
...
int32_t spfilset(CSOUND *csound, SPECFILT *p) {
  ...
  if ((npts = inspecp->npts) != outspecp->npts) { /* if inspec not matched */
    SPECset(csound, outspecp, (int32_t)npts);     /*   reinit the out spec */
    csound->AuxAlloc(csound, (size_t)npts * 2 * sizeof(MYFLT),
...
int32_t specfilt(CSOUND *csound, SPECFILT *p) {
  if (p->wsig->ktimstamp == CS_KCNT) { /* if input spec is new,  */
    ...
    if (UNLIKELY(newp == NULL || outp == NULL || coefp == NULL ||
                 persp == NULL)) /* RWD */
      goto err1;
...
static OENTRY spectra_localops[] = {
    {"spectrum", S(SPECTRUM), _QQ, 3, "w", "kiiiqoooo", (SUBR)spectset,
     (SUBR)spectrum},
    {"spectrum", S(SPECTRUM), _QQ, 3, "w", "aiiiqoooo", (SUBR)spectset,
     (SUBR)spectrum},
    {"specaddm", S(SPECADDM), _QQ, 3, "w", "wwp", (SUBR)spadmset,
     (SUBR)specaddm},
    {"specdiff", S(SPECDIFF), _QQ, 3, "w", "w", (SUBR)spdifset, (SUBR)specdiff},
    {"specscal", S(SPECSCAL), _QQ, 3, "w", "wii", (SUBR)spsclset,
     (SUBR)specscal},
    {"spechist", S(SPECHIST), _QQ, 3, "w", "w", (SUBR)sphstset, (SUBR)spechist},
    {"specfilt", S(SPECFILT), _QQ, 3, "w", "wi", (SUBR)spfilset,
     (SUBR)specfilt},
    {"specptrk", S(SPECPTRK), _QQ, 3, "kk", "wkiiiiiioqooo", (SUBR)sptrkset,
     (SUBR)specptrk},
    {"specsum", S(SPECSUM), _QQ, 3, "k", "wo", (SUBR)spsumset, (SUBR)specsum},
    {"specdisp", S(SPECDISP), _QQ, 3, "", "wio", (SUBR)spdspset,
     (SUBR)specdisp},
    {"pitch", S(PITCH), 0, 3, "kk", "aiiiiqooooojo", (SUBR)pitchset,
     (SUBR)pitch},
    ...
    {"readscratch", S(SCRATCHPAD), 0, 1, "i", "o", (SUBR)scratchread, NULL,
     NULL},
    {"writescratch", S(SCRATCHPAD), 0, 1, "", "io", (SUBR)scratchwrite, NULL,
     NULL},
    {"pitchamdf", S(PITCHAMDF), 0, 3, "kk", "aiioppoo", (SUBR)pitchamdfset,
     (SUBR)pitchamdf},
    {"hsboscil", S(HSBOSC), TR, 3, "a", "kkkiiioo",

     (SUBR)hsboscset, (SUBR)hsboscil},
    {"phasorbnk", S(PHSORBNK), 0, 3, "a", "xkio", (SUBR)phsbnkset,
     (SUBR)phsorbnk},
    {"phasorbnk.k", S(PHSORBNK), 0, 3, "k", "xkio", (SUBR)phsbnkset,
     (SUBR)kphsorbnk, NULL},
    ...
    {"transeg", S(TRANSEG), 0, 3, "k", "iiim", (SUBR)trnset, (SUBR)ktrnseg,
     NULL},
    {"transeg.a", S(TRANSEG), 0, 3, "a", "iiim", (SUBR)trnset, (SUBR)trnseg},
    {"transegb", S(TRANSEG), 0, 3, "k", "iiim", (SUBR)trnset_bkpt,
     (SUBR)ktrnseg, (SUBR)NULL},
    {"transegb.a", S(TRANSEG), 0, 3, "a", "iiim", (SUBR)trnset_bkpt,
     (SUBR)trnseg},
    {"transegr", S(TRANSEG), 0, 3, "k", "iiim", (SUBR)trnsetr, (SUBR)ktrnsegr,
     (SUBR)NULL},
    {"transegr.a", S(TRANSEG), 0, 3, "a", "iiim", (SUBR)trnsetr, (SUBR)trnsegr},
    {"clip", S(CLIP), 0, 3, "a", "aiiv", (SUBR)clip_set, (SUBR)clip},
    {"median", S(MEDFILT), 0, 3, "a", "akio", (SUBR)medfiltset, (SUBR)medfilt},
    {"mediank", S(MEDFILT), 0, 3, "k", "kkio", (SUBR)medfiltset,
     (SUBR)kmedfilt},
};
