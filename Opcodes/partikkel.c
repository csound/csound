...
#define PERFERROR(x)                                                           \
  csound->PerfError(csound, &(p->h), "%s", Str("partikkel: " x))
...
static int32_t setup_globals(CSOUND *csound, PARTIKKEL *p) {
  ...
  if (pg == NULL) {
    ...
    pg->zzhhhhz_tab->ftable =
        (MYFLT *)csound->Calloc(csound, 8 * sizeof(MYFLT));
    for (i = 2; i <= 5; ++i)
...
static inline MYFLT dsf(FUNC *tab, GRAIN *grain, double beta, MYFLT zscale,
  ...
  if (denominator < FL(1e-6) && denominator > FL(-1e-6)) {
    ...
    numerator =
        FL(1.0) - a * cos_beta - lastharmonic +
...
static int32_t partikkel_init(CSOUND *csound, PARTIKKEL *p) {
  ...
  p->disttab = *p->dist >= FL(0.0) ? csound->FTFind(csound, p->dist)
                                   : p->globals->zzz_tab;
  ...
  p->env_decay_tab = *p->env_decay >= FL(0.0)
                         ...
                         : p->globals->ooo_tab;
  p->env2_tab = *p->env2 >= FL(0.0) ? csound->FTFind(csound, p->env2)
                                    : p->globals->ooo_tab;
  ...
  p->fmamptab = *p->fm_indices >= FL(0.0)
                    ...
                    : p->globals->zzo_tab;
  p->wavgaintab = *p->waveamps >= FL(0.0) ? csound->FTFind(csound, p->waveamps)
...
static int32_t schedule_grain(CSOUND *csound, PARTIKKEL *p, NODE *node, int32 n,
  ...
  const double phase_corr =
      offset == 0.0 && p->graininc > 0.0032 ? p->grainphase / p->graininc : 0.0;
  ...
  for (i = 0; i < 5; ++i) {
    ...
    MYFLT freqmult = i != WAV_TRAINLET ? *(*(&p->wavekey1 + i)) * (*p->wavfreq)
                                       : *p->trainletfreq;
    ...
    if (i == WAV_TRAINLET) {
      ...
      if (FABS(grain->falloff) > FL(0.9999) &&
        ...
        normalize =
            (1.0 - fabs(grain->falloff)) / (1.0 - fabs(grain->falloff_pow_N));
    ...
    if (curwav->delta == enddelta || *p->freqsweepshape == FL(0.5)) {
      ...
      if (*p->freqsweepshape < FL(0.001)) {
        ...
        start_offset =
            (enddelta - curwav->delta * total_decay) / (1.0 - total_decay);
...
static int32_t schedule_grains(CSOUND *csound, PARTIKKEL *p) {
  ...
  p->fmenvtab = *p->fm_env >= FL(0.0) ? csound->FTFind(csound, p->fm_env)
                                      : p->globals->ooo_tab;
...
static inline void render_wave(PARTIKKEL *p, GRAIN *grain, WAVEDATA *wav,
  ...
  for (n = grain->start; n < stop; ++n) {
    ...
    buf[n] += lrp(wav->table->ftable[x0], wav->table->ftable[x0 + 1], frac) *
              wav->gain;

    fmenv =
        grain->fmenvtab
            ->ftable[(size_t)(fmenvphase * FMAXLEN) >> grain->fmenvtab->lobits];
...
static inline void render_trainlet(PARTIKKEL *p, GRAIN *grain, WAVEDATA *wav,
  ...
  for (n = grain->start; n < stop; ++n) {
    ...
    buf[n] += wav->gain *
              dsf(p->costab, grain, wav->phase, p->zscale, p->cosineshift);
...
static inline void render_grain(CSOUND *csound, PARTIKKEL *p, GRAIN *grain) {
  ...
  uint32_t stop = grain->stop > CS_KSMPS ? CS_KSMPS : grain->stop;
  ...
  for (n = grain->start; n < stop; ++n) {
    ...
    if (grain->envphase < grain->envattacklen) {
      ...
      envphase = (grain->envphase - grain->envdecaystart) /
                 (1.0 - grain->envdecaystart);
    } else {
      /* clamp envelope phase because of round-off errors */
      envtable =
          grain->envdecaystart < 1.0 ? p->env_decay_tab : p->env_attack_tab;
      envphase = grain->envphase = 1.0;
    }

    /* fetch envelope values */
    env = envtable->ftable[(size_t)(envphase * FMAXLEN) >> envtable->lobits];
...
static int32_t get_global_entry(CSOUND *csound, PARTIKKEL_GLOBALS_ENTRY **entry,
  ...
  pg = csound->QueryGlobalVariable(csound, "partikkel");
  if (UNLIKELY(pg == NULL))
    return csound->InitError(csound, Str("%s: partikkel not initialized"),
                             prefix);
  /* try to find entry corresponding to our opcodeid */
  pe = pg->rootentry;
  ...
  if (UNLIKELY(pe == NULL))
    return csound->InitError(csound, Str("%s: could not find opcode id"),
                             prefix);
  *entry = pe;
  return OK;
}
...
static OENTRY partikkel_localops[] = {
    {"partikkel", sizeof(PARTIKKEL), TR, 3, "ammmmmmm",
     "xkiakiiikkkkikkiiaikikkkikkkkkiaaaakkkkioj", (SUBR)partikkel_init,
     (SUBR)partikkel},
    {"partikkelsync", sizeof(PARTIKKEL_SYNC), TR, 3, "am", "i",
     (SUBR)partikkelsync_init, (SUBR)partikkelsync},
    {"partikkelget", sizeof(PARTIKKEL_GET), TR, 3, "k", "ki",
     (SUBR)partikkelget_init, (SUBR)partikkelget, (SUBR)NULL},
    {"partikkelset", sizeof(PARTIKKEL_SET), TR, 3, "", "kki",
     (SUBR)partikkelset_init, (SUBR)partikkelset, (SUBR)NULL}};
