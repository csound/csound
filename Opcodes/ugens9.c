...
static int32_t cvset_(CSOUND *csound, CONVOLVE *p, int32_t stringname) {
  ...
  if (stringname == 0) {
    if (csound->ISSTRCOD(*p->ifilno))
      ...
      csound->strarg2name(csound, cvfilnam, p->ifilno, "convolve.", 0);
  } else
    strncpy(cvfilnam, ((STRINGDAT *)p->ifilno)->data, MAXNAME - 1);

  if ((mfp = p->mfp) == NULL || strcmp(mfp->filename, cvfilnam) != 0) {
    /* if file not already readin */
    if (UNLIKELY((mfp = csound->ldmemfile2withCB(
                      csound, cvfilnam, CSFTYPE_CVANAL, NULL)) == NULL)) {
      return csound->InitError(csound, Str("CONVOLVE cannot load %s"),
                               cvfilnam);
    }
  }
  ...
  if (p->auxch.auxp == NULL || p->auxch.size < siz * sizeof(MYFLT)) {
    ...
    p->fftbuf = fltp;
    fltp += (Hlenpadded + 2); /* and insert addresses */
    p->olap = fltp;
    fltp += p->nchanls * (Hlen - 1);
    p->outbuf = fltp;
    fltp += p->nchanls * obufsiz;
    p->X = fltp;
  } else {
    fltp = (MYFLT *)p->auxch.auxp;
    ...
    if (incount == Hlen) {
      ...
      for (chn = nchm1; chn >= 0; chn--) {
        ...
        if ((nsmpso > 0) && (outcnt == 0)) {
          /*    csound->Message(csound, "Outputting to audio buffer
           * proper\n");*/
...
static int32_t pconvset_(CSOUND *csound, PCONVOLVE *p, int32_t stringname) {
  ...
  if (stringname == 0) {
    if (csound->ISSTRCOD(*p->ifilno))
      ...
      csound->strarg2name(csound, IRfile.sfname, p->ifilno, "soundin.", 0);
  } else
    strncpy(IRfile.sfname, ((STRINGDAT *)p->ifilno)->data, 511);
  ...
  if (UNLIKELY(p->nchanls != (int32_t)p->OUTOCOUNT)) {
    return csound->InitError(csound, "%s",
                             Str("PCONVOLVE: number of output channels "
  ...
  /* set up FFT tables */
  inbuf = (MYFLT *)csound->Malloc(csound, p->Hlen * p->nchanls * sizeof(MYFLT));
  csound->AuxAlloc(csound,
                   p->numPartitions * (p->Hlenpadded + 2) * sizeof(MYFLT) *
                       p->nchanls,
                   &p->H);
  ...
  csound->AuxAlloc(csound,
                   (p->Hlenpadded + 2) * p->numPartitions * p->nchanls *
                       sizeof(MYFLT),
                   &p->convBuf);
  p->curPart = 0;
...
static int32_t pconvolve(CSOUND *csound, PCONVOLVE *p) {
  ...
  for (nn = 0; nn < nsmps; nn++) {
    ...
    input[count++] = *workWrite++ =
        (nn < offset || nn > early ? FL(0.0) : ai[nn]);
    ...
    if (count == p->Hlen) {
      ...
      *workWrite = FL(0.0); /* zero out nyquist bin from last fft result
      ...
      buf =
          (MYFLT *)p->convBuf.auxp + p->curPart * p->nchanls * hlenpaddedplus2;
      for (i = 0; i < p->nchanls; i++) {
        MYFLT *bufp;
...
static OENTRY localops[] = {{"convolve", sizeof(CONVOLVE), 0, 3, "mmmm", "aSo",
                             (SUBR)cvset_S, (SUBR)convolve},
                            {"convle", sizeof(CONVOLVE), 0, 3, "mmmm", "aSo",
                             (SUBR)cvset_S, (SUBR)convolve},
                            {"pconvolve", sizeof(PCONVOLVE), 0, 3, "mmmm",
                             "aSoo", (SUBR)pconvset_S, (SUBR)pconvolve},
                            {"convolve.i", sizeof(CONVOLVE), 0, 3, "mmmm",
                             "aio", (SUBR)cvset, (SUBR)convolve},
                            {"convle.i", sizeof(CONVOLVE), 0, 3, "mmmm", "aio",
                             (SUBR)cvset, (SUBR)convolve},
                            {"pconvolve.i", sizeof(PCONVOLVE), 0, 3, "mmmm",
                             "aioo", (SUBR)pconvset, (SUBR)pconvolve}};
