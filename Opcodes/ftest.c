...
static int32_t tanhtable(FGDATA *ff, FUNC *ftp) {
  ...
  MYFLT resc = ff->e.p[7];
  if (ftp->flen <= 0)
    return csound->ftError(ff, "%s", Str("Illegal zero table size"));
...
static int32_t wavetable(FGDATA *ff, FUNC *ftp) {
  ...
  *pInp = FL(1.0);
  ...
  /* DEC to BIN */
  for (i = 0; i < steps; i++)
    pOrder[i] = ((int32_t)order >> i) & 0x1;
  /* main loop */
  for (i = 0; i < steps; i++)
    deconvolve(pInp, pwaveS, pnewLen, pBuf, pOrder++);
  for (i = 0; i < *pnewLen; i++)
    fp[i] = pInp[i];
  csound->Free(csound, pBuf);
  csound->Free(csound, pInp);
  csound->Free(csound, xfree);
  csound->Free(csound, mirr);
  if (resc != FL(0.0))
    ff->e.p[4] *= -1;
  /*else ff->e.p[4] = 1;*/
  return OK;
...
static NGFENS ftest_fgens[] = {{"tanh", tanhtable},
                               {"exp", exptable},
                               {"sone", sonetable},
                               {"wave", wavetable},
                               {NULL, NULL}};
