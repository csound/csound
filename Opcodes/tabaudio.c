...
static uintptr_t write_tab(void *pp) {
  ...
  if (csound->SndfileWrite(csound, ff, t, size) != size) {
    ...
    csound->PerfError(csound, h, Str("tabaudio: failed to write data %d"),
                      size);
    *ans = -FL(1.0);
  } else
    *ans = FL(1.0);
  ...
  if (*p->trig) {
    ...
    *p->kans = FL(0.0);
    ...
    if (format >= 51)
      ...
      sfinfo.format = format_table[format];
    ...
    ff = csound->FileOpen2(
        csound, &ff, CSFILE_SND_W, p->file->data, &sfinfo, NULL,
        csound->type2csfiletype(parm.filetyp, parm.outformat), 0);
    if (ff == NULL)
    ...
    if (*p->sync == FL(0.0)) { /* write in perf thread */
      if ((n = csound->SndfileWrite(csound, ff, t, size)) != size) {
        ...
        csound->FileClose(csound, ff);
        return csound->PerfError(csound, &(p->h),
                                 Str("tabaudio: failed to write data %d %d"), n,
                                 size);
      }
      csound->FileClose(csound, ff);
    } else { /* Use a helper thread */
      SAVE_THREAD *q =
          (SAVE_THREAD *)csound->Malloc(csound, sizeof(SAVE_THREAD));
      q->t = t;
      q->size = size;
      q->ff = ff;
      q->ans = p->kans;
      ...
      if ((q->thread = csound->CreateThread(write_tab, (void *)q)) == NULL) {
        OPDS *i = q->h;
        free(q);
        return csound->PerfError(csound, i, "%s", Str("Error creating thread"));
      }
