...
static int32_t moogvcfset(CSOUND *csound, MOOGVCF *p) {
  ...
  if ((p->maxint = *p->max) == FL(0.0))
    p->maxint = csound->Get0dBFS(csound);
...
static int32_t rezzy(CSOUND *csound, REZZY *p) {
  ...
  double fqcadj, a = 0.0, /* Initialisations fake */
      csq = 0.0, invb = 0.0,
                 tval = 0.0; /* Temporary variables for the filter */
  double xnm1 = p->xnm1, xnm2 = p->xnm2, ynm1 = p->ynm1, ynm2 = p->ynm2;
      ...
      { // POLES
        ...
        if (disc < 0.0) {
          ...
          if (p0 * p0 + pi * pi >= 1.0) {
            ...
            if (warn)
              csound->Warning(csound, "%s", Str("rezzy instability corrected"));
          ...
          if (p0 * p0 >= 1.0 || p1 * p1 >= 1) {
            if (warn)
              csound->Warning(csound, "%s", Str("rezzy instability corrected"));
            if (p0 * p0 >= 1)
              p0 = NEARONE * (p0 > 0.0 ? 1 : (-1));
            if (p1 * p1 >= 1)
              p1 = NEARONE * (p1 > 0.0 ? 1 : (-1));
            b1 = -(p0 + p1);
            b2 = p0 * p1;
            warn = 0;
          }
...
static int32_t distort(CSOUND *csound, DISTORT *p) {
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    out[n] = /* IV - Dec 28 2002: optimised */
        ((EXP(sig * shape1) - EXP(sig * shape2)) / COSH(sig * pregain)) *
        postgain;
  }
  return OK;
}
...
static int32_t vcoset(CSOUND *csound, VCO *p) {
  ...
  if (UNLIKELY(ndel == 0))
    ndel = 1; /* fix due to Troxler */
  if (p->aux.auxp == NULL || (uint32_t)(ndel * sizeof(MYFLT)) > p->aux.size)
...
static int32_t vco(CSOUND *csound, VCO *p) {
  ...
  /*-----------------------------------------------------*/
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      p->ynm1 = sqr;
      p->ynm2 = tri;
      ar[n] = tri * amp * fqc / (CS_ESR * FL(0.42) * (FL(0.05) + pw - pw * pw));
    }
  }
  /*-----------------------------------------------------*/
  /* Sawtooth Wave                                       */
...
static int32_t nestedapset(CSOUND *csound, NESTEDAP *p) {
  ...
  if (UNLIKELY(((int32)(*p->del1 * CS_ESR)) <=
               ((int32)(*p->del2 * CS_ESR) + (int32)(*p->del3 * CS_ESR)))) {
...
static OENTRY localops[] = {
    {"biquad", S(BIQUAD), 0, 3, "a", "akkkkkko", (SUBR)biquadset, (SUBR)biquad},
    {"biquada", S(BIQUAD), 0, 3, "a", "aaaaaaao", (SUBR)biquadset,
     (SUBR)biquada},
    {"moogvcf", S(MOOGVCF), 0, 3, "a", "axxpo", (SUBR)moogvcfset,
     (SUBR)moogvcf},
    {"moogvcf2", S(MOOGVCF), 0, 3, "a", "axxoo", (SUBR)moogvcfset,
     (SUBR)moogvcf},
    ...
    {"tbvcf", S(TBVCF), 0, 3, "a", "axxkkp", (SUBR)tbvcfset, (SUBR)tbvcf},
    {"planet", S(PLANET), 0, 3, "aaa", "kkkiiiiiiioo", (SUBR)planetset,
     (SUBR)planet},
    {"pareq", S(PAREQ), 0, 3, "a", "akkkoo", (SUBR)pareqset, (SUBR)pareq},
    {"nestedap", S(NESTEDAP), 0, 3, "a", "aiiiiooooo", (SUBR)nestedapset,
     (SUBR)nestedap},
    {"lorenz", S(LORENZ), 0, 3, "aaa", "kkkkiiiio", (SUBR)lorenzset,
     (SUBR)lorenz},
    {"mode", S(MODE), 0, 3, "a", "axxo", (SUBR)modeset, (SUBR)mode},
    {"mvmfilter", S(MVMFILT), 0, 3, "a", "axxo", (SUBR)mvmfilterset,
     (SUBR)mvmfilter},
};
