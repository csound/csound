...
static CS_NOINLINE int32_t fout_open_file(CSOUND *csound, FOUT_FILE *p,
                                          void *fp, int32_t fileType,
                                          MYFLT *iFile, int32_t isString,
  ...
  /* get file name, */
  if (isString)
    name = csound->Strdup(csound, ((STRINGDAT *)iFile)->data);
  ...
  if (idx > pp->file_num) {
    if (idx >= pp->file_max) {
      ...
      pp->file_max = (idx | 3) + 1;
      tmp = (struct fileinTag *)csound->ReAlloc(
          csound, pp->file_opened, sizeof(struct fileinTag) * pp->file_max);
      pp->file_opened = tmp;
  ...
  if (fileType == CSFILE_STD) {
    ...
    if (fileType == CSFILE_SND_W) {
      ...
      if (pp->oparms.realtime == 0 || forceSync == 1) {
        fd = csound->FileOpen2(csound, &sf, fileType, name, fileParams, "SFDIR",
                               csFileType, 0);
        p->async = 0;
      } else {
        p->fd = fd = csound->FileOpenAsync(
            csound, &sf, fileType, name, fileParams, "SFDIR;SSDIR",
            CSFTYPE_UNKNOWN_AUDIO, p->bufsize, 0);
        p->async = 1;
      }
  ...
  if (p->f.sf == NULL) {
    ...
    if (p->buf_pos >= p->guard_pos) {
      ...
      if (p->f.async == 1)
        ...
        csound->SndfileWrite(csound, p->f.sf, buf,
                             p->buf_pos / nargs); // in frames
      p->buf_pos = 0;
    }
  }
  return OK;
  ...
  if (p->f.sf == NULL) {
    ...
    if (p->buf_pos >= p->guard_pos) {
      ...
      if (p->f.async == 1)
        ...
        csound->SndfileWrite(csound, p->f.sf, (MYFLT *)buf,
                             p->buf_pos / nargs); // in frames
      p->buf_pos = 0;
    }
  }
...
static const int32_t fout_format_table[51] = {
    /* 0 - 9 */
    (AE_FLOAT | TYP2SF(TYP_RAW)), (AE_SHORT | TYP2SF(TYP_RAW)), AE_SHORT,
    AE_ULAW, AE_SHORT, AE_LONG, AE_FLOAT, AE_UNCH, AE_24INT, AE_DOUBLE,
    /* 10 - 19 */
    TYP2SF(TYP_WAV), (AE_CHAR | TYP2SF(TYP_WAV)), (AE_ALAW | TYP2SF(TYP_WAV)),
    (AE_ULAW | TYP2SF(TYP_WAV)), (AE_SHORT | TYP2SF(TYP_WAV)),
    (AE_LONG | TYP2SF(TYP_WAV)), (AE_FLOAT | TYP2SF(TYP_WAV)),
    (AE_UNCH | TYP2SF(TYP_WAV)), (AE_24INT | TYP2SF(TYP_WAV)),
    (AE_DOUBLE | TYP2SF(TYP_WAV)),
    /* 20 - 29 */
    ...
    /* 30 - 39 */
    TYP2SF(TYP_RAW), (AE_CHAR | TYP2SF(TYP_RAW)), (AE_ALAW | TYP2SF(TYP_RAW)),
    (AE_ULAW | TYP2SF(TYP_RAW)), (AE_SHORT | TYP2SF(TYP_RAW)),
    (AE_LONG | TYP2SF(TYP_RAW)), (AE_FLOAT | TYP2SF(TYP_RAW)),
    (AE_UNCH | TYP2SF(TYP_RAW)), (AE_24INT | TYP2SF(TYP_RAW)),
    (AE_DOUBLE | TYP2SF(TYP_RAW)),
    /* 40 - 49 */
...
static int32_t fouta_flush_callback(CSOUND *csound, void *p_) {
  ...
  if (p->f.sf != NULL && p->buf_pos > 0) {
    ...
    if (p->f.async == 1)
      ...
      // sflib_write_MYFLT(p->f.sf, (MYFLT *) p->buf.auxp, p->buf_pos);
      csound->SndfileWrite(csound, p->f.sf, (MYFLT *)p->buf.auxp,
                           p->buf_pos / p->tabin->sizes[0]); // in frames
  }
  return OK;
}
...
static int32_t outfile_set_S(CSOUND *csound, OUTFILE *p /*, int32_t istring*/) {
  ...
  n = fout_open_file(csound, &(p->f), NULL, CSFILE_SND_W, p->fname, istring,
                     &sfinfo, 0);
  if (UNLIKELY(n < 0))
    return NOTOK;
...
static int32_t outfile_set_A(CSOUND *csound, OUTFILEA *p) {
  ...
  p->f.bufsize = p->buf.size;
  sfinfo.channels = len;
  n = fout_open_file(csound, &(p->f), NULL, CSFILE_SND_W, p->fname, 1, &sfinfo,
                     0);
  if (UNLIKELY(n < 0))
    return NOTOK;
...
static int32_t koutfile_set_(CSOUND *csound, KOUTFILE *p, int32_t istring) {
  ...
  if (((STDOPCOD_GLOBALS *)csound->QueryGlobalVariable(csound,
                                                       "STDOPC_GLOBALS"))
          ->file_opened[n]
          .do_scale)
    p->scaleFac = (FL(1.) / csound->Get0dBFS(csound));
...
static int32_t fiopen_(CSOUND *csound, FIOPEN *p, int32_t istring) {
  ...
  if (idx < 0 || idx > 3)
    idx = 0;
  n = fout_open_file(csound, (FOUT_FILE *)NULL, &rfp, CSFILE_STD, p->fname,
                     istring, omodes[idx], 1);
  if (UNLIKELY(n < 0))
    return NOTOK;
  if (idx > 1)
    setbuf(rfp, NULL);
...
static int32_t ficlose_opcode_(CSOUND *csound, FICLOSE *p, int32_t istring) {
  ...
  if (istring || csound->ISSTRCOD(*(p->iFile))) {
    char *fname = NULL;
    if (istring)
      fname = csound->Strdup(csound, ((STRINGDAT *)p->iFile)->data);
    ...
    if (UNLIKELY(idx < 0 || idx > pp->file_num ||
                 pp->file_opened[idx].fd == NULL)) {
      csound->Warning(csound, Str("cannot close file #%d: not a valid handle"),
                      idx);
      return OK;
    }
  }
  if (pp->file_opened[idx].refCount) {
    if (UNLIKELY(!(pp->file_opened[idx].refCount & 0x80000000U))) {
      ...
      csound->Warning(csound,
                      Str("file #%d (%s) is in use, will be closed "
                          "when released"),
...
static int32_t ioutfile_set(CSOUND *csound, IOUTFILE *p) {
  ...
  if (*p->iascii == 0) { /* ascii format */
    ...
    case 2: /* with prefix (start at 0 time) */
      ...
      {
        int32_t p1 = (int32_t)p->h.insdshead->p1.value;
        double p2 = (double)(CS_KCNT - pp->fout_kreset) * CS_ONEDKR;
        double p3 = p->h.insdshead->p3.value;
        if (p3 > FL(0.0))
          fprintf(rfil, "i %i %f %f ", p1, p2, p3);
        else
    ...
    for (j = 0; j < p->INOCOUNT - 3; j++) {
      if (UNLIKELY(1 != fwrite(args[j], sizeof(MYFLT), 1, rfil)))
        return NOTOK;
    }
  }
  return OK;
...
static int32_t ioutfile_r(CSOUND *csound, IOUTFILE_R *p) {
  ...
  if (*p->iascii == 0) { /* ascii format */
    ...
    case 1: { /* whith prefix (i-statement, p1, p2 and p3) */
      ...
      double p3 = (double)(CS_KCNT - p->counter) * CS_ONEDKR;
      fprintf(rfil, "i %i %f %f ", p1, p2, p3);
    } break;
    case 2: /* with prefix (start at 0 time) */
    {
...
static int32_t infile_set_(CSOUND *csound, INFILE *p, int32_t istring) {
  ...
  p->f.bufsize = p->buf.size;
  n = fout_open_file(csound, &(p->f), NULL, CSFILE_SND_R, p->fname, istring,
                     &sfinfo, 0);
  if (UNLIKELY(n < 0))
    return NOTOK;
...
static int32_t infile_set_A(CSOUND *csound, INFILEA *p) {
  ...
  p->f.bufsize = p->buf.size;
  n = fout_open_file(csound, &(p->f), NULL, CSFILE_SND_R, p->fname, 1, &sfinfo,
                     0);
  if (UNLIKELY(n < 0))
    return NOTOK;
  ...
  if (p->flag) {
    if (p->buf_pos >= p->guard_pos) {
      if (UNLIKELY(p->f.async == 0)) {
        csound->SndfileSeek(csound, p->f.sf, p->currpos * p->f.nchnls,
                            SEEK_SET);
...
static int32_t i_infile_(CSOUND *csound, I_INFILE *p, int32_t istring) {
  ...
  if (UNLIKELY(idx < 0 || idx > 2))
    idx = 0;
  n = fout_open_file(csound, (FOUT_FILE *)NULL, &fp, CSFILE_STD, p->fname,
                     istring, omodes[idx], 0);
  if (UNLIKELY(n < 0))
    return NOTOK;
  ...
  case 2: /* binary floats without loop */
    if (fseek(fp, p->currpos * sizeof(float) * nargs, SEEK_SET) < 0)
      return NOTOK;
    p->currpos++;
    for (j = 0; j < nargs; j++) {
...
static int32_t fprintf_set_(CSOUND *csound, FPRINTF *p, int32_t istring) {
  ...
  if (p->h.opadr != (SUBR)NULL) /* fprintks */
    n = fout_open_file(csound, &(p->f), NULL, CSFILE_STD, p->fname, istring,
                       "w", 1);
  else /* fprints */
...
static OENTRY localops[] = {
    ...
    {"fprints.i", S(FPRINTF), 0, 1, "", "iSN", (SUBR)fprintf_i, (SUBR)NULL,
     (SUBR)NULL, NULL},
    {
        "fprintks",
        S(FPRINTF),
    ...
    {"fprintks.i", S(FPRINTF), WR, 3, "", "iSN", (SUBR)fprintf_set,
     (SUBR)fprintf_k, (SUBR)NULL, NULL},
    {"vincr", S(INCR), WI, 2, "", "aa", (SUBR)NULL, (SUBR)incr, NULL},
    {"clear", S(CLEARS), WI, 2, "", "y", (SUBR)NULL, (SUBR)clear, NULL},
    {"fout", S(OUTFILE), 0, 3, "", "Siy", (SUBR)outfile_set_S, (SUBR)outfile,
     NULL},
    {"fout.A", S(OUTFILEA), 0, 3, "", "Sia[]", (SUBR)outfile_set_A,
     (SUBR)outfile_array, NULL},
    /* { "fout.i",       S(OUTFILE),     0, 3,  "",     "iiy", */
    /*     (SUBR) outfile_set,     (SUBR) outfile, NULL}, */
    {"foutk", S(KOUTFILE), 0, 3, "", "Siz", (SUBR)koutfile_set_S,
     (SUBR)koutfile, (SUBR)NULL, NULL},
    {"foutk.i", S(KOUTFILE), 0, 3, "", "iiz", (SUBR)koutfile_set,
     (SUBR)koutfile, (SUBR)NULL, NULL},
    {"fouti", S(IOUTFILE), 0, 1, "", "iiim", (SUBR)ioutfile_set, (SUBR)NULL,
     (SUBR)NULL, NULL},
    {"foutir", S(IOUTFILE_R), 0, 3, "", "iiim", (SUBR)ioutfile_set_r,
     (SUBR)ioutfile_r, (SUBR)NULL, NULL},
    {"fiopen", S(FIOPEN), 0, 1, "i", "Si", (SUBR)fiopen_S, (SUBR)NULL,
     (SUBR)NULL, NULL},
    {"fiopen.i", S(FIOPEN), 0, 1, "i", "ii", (SUBR)fiopen, (SUBR)NULL,
     (SUBR)NULL, NULL},
    {"ficlose", S(FICLOSE), 0, 1, "", "S", (SUBR)ficlose_opcode_S, (SUBR)NULL,
     (SUBR)NULL, NULL},
    {"ficlose.S", S(FICLOSE), 0, 1, "", "i", (SUBR)ficlose_opcode, (SUBR)NULL,
     (SUBR)NULL, NULL},
    {"fin.a", S(INFILE), WI | _QQ, 3, "", "Siiy", (SUBR)infile_set_S,
     (SUBR)infile_act, NULL},
    {"fin.A", S(INFILEA), WI, 3, "", "Siia[]", (SUBR)infile_set_A,
     (SUBR)infile_arr, NULL},
    {"fin.i", S(INFILE), WI | _QQ, 3, "", "iiiy", (SUBR)infile_set,
     (SUBR)infile_act, NULL},
    {"fink", S(KINFILE), WI, 3, "", "Siiz", (SUBR)kinfile_set_S, (SUBR)kinfile,
     (SUBR)NULL, NULL},
    {"fink.i", S(KINFILE), WI, 3, "", "iiiz", (SUBR)kinfile_set, (SUBR)kinfile,
     (SUBR)NULL, NULL},
    {"fini", S(I_INFILE), WI, 1, "", "Siim", (SUBR)i_infile_S, (SUBR)NULL,
     (SUBR)NULL, NULL},
    {"fini.i", S(I_INFILE), WI, 1, "", "iiim", (SUBR)i_infile, (SUBR)NULL,
     (SUBR)NULL, NULL}};
