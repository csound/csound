...
static inline int32_t buf_bytes_alloc(int32_t nChannels, int32_t partSize,
                                      int32_t nPartitions) {
  int32_t nSmps;
...
static void set_buf_pointers(FTCONV *p, int32_t nChannels, int32_t partSize,
                             int32_t nPartitions) {
  MYFLT *ptr;
  int32_t i;
...
static int32_t ftconv_init(CSOUND *csound, FTCONV *p) {
  ...
  if (UNLIKELY(p->partSize < 4 || (p->partSize & (p->partSize - 1)) != 0)) {
    return csound->InitError(csound, "%s",
                             Str("ftconv: invalid impulse response "
                                 "partition length"));
  }
...
static int32_t ftconv_perf(CSOUND *csound, FTCONV *p) {
  ...
  for (nn = offset; nn < nsmps; nn++) {
    ...
    for (n = 0; n < p->nChannels; n++) {
      ...
      multiply_fft_buffers(p->tmpBuf, p->ringBuf, p->IR_Data[n], nSamples,
...
int32_t ftconv_init_(CSOUND *csound) {
  return csound->AppendOpcode(csound, "ftconv", (int32_t)sizeof(FTCONV), TR, 3,
                              "mmmmmmmm", "aiiooo",
                              (int32_t(*)(CSOUND *, void *))ftconv_init,
                              (int32_t(*)(CSOUND *, void *))ftconv_perf, NULL);
}
