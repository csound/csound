...
static MYDBL kontrolconvert(CSOUND *csound, MYDBL in1, MYDBL in2);
...
{
    MYDBL c3, r3, rf, max_res, a, f=p->f, a1, a2, b1, b2, b3, b4;
    MYDBL Dmas, yx, yo, yd, tanh_xo, Dx, Do;
    MYFLT *x, *out1;
    //MYFLT *out2, *out3;
    ...
    if (*p->ksw6 != FL(0.0)) {
      MYDBL txo2 = tanh_xo*tanh_xo;
      MYDBL knt = *p->knt;
      for (n=offset; n<nsmps; n++) {
        rf = kontrolconvert(csound, (MYDBL)p->aenv[n], knt);
...
int32_t vactrol_init(CSOUND *csound, VACTROL* p)
{
    ...
    p->t_down = *p->down<FL(0.0) ? 3.0e3 : (MYDBL)*p->down;
    p->t_up   = *p->up<FL(0.0) ? 20.0 : (MYDBL)*p->up;
    return OK;
}
...
{
    MYDBL s1 = p->s1;
    MYDBL a_base = p->a_base;
    MYDBL T_DOWN = p->t_down; // Fall time
    MYDBL T_UP   = p->t_up; // Rise time
    uint32_t offset = p->h.insdshead->ksmps_offset;
    uint32_t early  = p->h.insdshead->ksmps_no_end;
    uint32_t n, nsmps = CS_KSMPS;
    MYFLT *in = p->inp;
    MYFLT *out = p->out;
    MYDBL e0db = csound->Get0dBFS(csound);
    ...
    for (n=offset; n<nsmps; n++) {
      MYDBL t_down = 10.0 + T_DOWN*(1-0.9*s1);
      MYDBL a_down = a_base /t_down;
      MYDBL dsl = (MYDBL)in[n]/e0db - s1;
      MYDBL x,y;
      MYDBL t_up = 1.0 + T_UP*(1.0-0.999*s1);
      MYDBL a_up = a_base /t_up;
...
{
    MYDBL R1, R2;
    MYDBL offset = 0.9999*in2 + 0.0001;
    MYDBL zerodb = csound->Get0dBFS(csound);
    MYDBL V3, Ia, If, Ifbound1, Ifbound2, Ifbound3;
    MYDBL Vb = in1/zerodb;
    MYDBL ans;
