...
int32_t poly_LPG_init(CSOUND *csound, BUCHLA *p) {
  ...
  if (warn == 0)
    csound->Message(csound, "**** Experimental code ****\n");
  warn++;
#define C1 (1e-09)
#define C2 (2.2e-10)
  p->f = 0.5 / CS_ESR;
...
int32_t poly_LPG_perf(CSOUND *csound, BUCHLA *p) {
  ...
  if (*p->ksw6 != FL(0.0)) {
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      Dx = 1.0 / (1.0 - b2 * f);
      Do = 1.0 / (1.0 - a2 * f);
      Dmas = 1.0 / (1.0 - Dx * (f * f * b3 * Do * a1 +
                                b4 * f * a * (1.0 - txo2) * Do * a1 - b4));
      yx =
          (p->sx + f * b1 * x[n] / e0dbfs + f * b3 * Do * p->so +
           f * b4 * (p->sd + (1.0 / f) * a * (tanh_xo - p->xo * (1.0 - txo2))) +
           b4 * a * (1.0 - txo2) * Do * p->so) *
          Dx * Dmas;
      yo = (p->so + f * a2 * yx) * Do;
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      Do = 1.0 / (1.0 - a2 * f);
      Dmas =
          1.0 / (1.0 - Dx * (f * f * b3 * Do * a1 + b4 * f * a * Do * a1 - b4));
      yx = (p->sx + f * b1 * x[n] + f * b3 * Do * p->so + f * b4 * p->sd +
            b4 * a * Do * p->so) *
           Dx * Dmas;
      yo = (p->so + f * a1 * yx) * Do;
...
#define scale                                                                  \
  (0.48) // This value is tuned for appropriate input range.
         // Constants
#define A (3.4645912)
...
#define bound1 (600 * alpha * n * VT / (G * (R6 + R7 - 1 / (alpha * beta))))
  ...
  Ifbound2 = VB / (R6 + R7);
  Ifbound3 = (gamma * G * VB + alpha * R9 * (VB * beta + Ifmax)) /
             (gamma * G * (R6 + R7) + R9);
...
#ifdef JPFF
    {"buchla", S(BUCHLA), 0, 3, "a", "aakkaPP", (SUBR)poly_LPG_init,
     (SUBR)poly_LPG_perf},
#endif
    {"vactrol", S(VACTROL), 0, 3, "a", "ajj", (SUBR)vactrol_init,
     (SUBR)vactrol_perf}};
