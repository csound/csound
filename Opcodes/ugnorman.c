...
static int32_t load_atsfile(CSOUND *csound, void *p, MEMFIL **mfp, char *fname,
  ...
  if (istring)
    strncpy(fname, ((STRINGDAT *)name_arg)->data, MAXNAME - 1);
  else {
    if (csound->ISSTRCOD(*((MYFLT *)name_arg)))
      strncpy(fname, csound->GetString(csound, *((MYFLT *)name_arg)),
              MAXNAME - 1);
    else
      csound->strarg2name(csound, fname, name_arg, "ats.", 0);
  }
  /* load memfile */
  if (UNLIKELY((*mfp = csound->ldmemfile2withCB(csound, fname, CSFTYPE_ATS,
                                                NULL)) == NULL)) {
  ...
  if (UNLIKELY((int32_t)bswap(&(atsh->magic)) != 123)) {
    (void)csound->InitError(csound,
                            Str("%s: either %s is not an ATS file "
                                "or the byte endianness is wrong"),
                            opname, fname);
    return NOTOK;
  }
  pp =
      (STDOPCOD_GLOBALS *)csound->QueryGlobalVariable(csound, "STDOPC_GLOBALS");
...
static int32_t atsinfo_S(CSOUND *csound, ATSINFO *p) {
  ...
  case 2:
    ...
    break;
  case 3:
    ret_data = &(atsh->npartials);
    break;
  case 4:
    ret_data = &(atsh->nfrms);
    break;
  case 5:
    ret_data = &(atsh->ampmax);
    break;
  case 6:
    ret_data = &(atsh->freqmax);
    break;
  case 7:
    ret_data = &(atsh->dur);
    break;
...
static int32_t atsreadset(CSOUND *csound, ATSREAD *p) {
  ...
  /* load memfile */
  p->swapped =
      load_atsfile(csound, p, &(p->atsmemfile), atsfilname, p->ifileno, 0);
  ...
  if (UNLIKELY((int32_t)(*p->ipartial) > n_partials ||
    ...
    return csound->InitError(csound,
                             Str("ATSREAD: partial %i out of range, "
                                 "max allowed is %i"),
...
static int32_t atsreadset_S(CSOUND *csound, ATSREAD *p) {
  ...
  /* load memfile */
  p->swapped =
      load_atsfile(csound, p, &(p->atsmemfile), atsfilname, p->ifileno, 1);
  if (UNLIKELY(p->swapped < 0))
    return NOTOK;
  atsh = (ATSSTRUCT *)p->atsmemfile->beginp;
...
static int32_t atsreadnzset(CSOUND *csound, ATSREADNZ *p) {
  ...
  if (UNLIKELY((int32_t)(*p->inzbin) > 25 || (int32_t)(*p->inzbin) <= 0)) {
    return csound->InitError(csound,
                             Str("ATSREADNZ: band %i out of range, "
...
static int32_t atsaddset(CSOUND *csound, ATSADD *p) {
  ...
  if (UNLIKELY((int32_t)(*p->iptloffset + *p->iptls * *p->iptlincr) >
    ...
    return csound->InitError(csound,
                             Str("ATSADD: Partial(s) out of range, "
...
static int32_t atsadd(CSOUND *csound, ATSADD *p) {
  ...
  if (UNLIKELY(p->auxch.auxp == NULL || (ftp = p->ftp) == NULL))
    goto err1;
...
static void
AtsAmpGate(                   /* adaption of PvAmpGate by Richard Karpen */
...
static const double freqs[25] = {
    100.0, 100.0,  100.0,  100.0,  110.0,  120.0,  140.0, 150.0, 160.0,
    190.0, 210.0,  240.0,  280.0,  320.0,  380.0,  450.0, 550.0, 700.0,
    900.0, 1100.0, 1300.0, 1800.0, 2500.0, 3500.0, 4500.0};
...
static int32_t atsaddnz(CSOUND *csound, ATSADDNZ *p) {
  ...
  for (i = 0; i < 25; i++) {
    ...
    if (i == synthme && nsynthed < p->bands) { /* synthesize cosine */
      ...
      for (n = offset; n < nsmps; n++) {
        ar[n] +=
            (COS(p->oscphase[i]) * amp * randiats(csound, &(p->randinoise[i])));
...
static void band_energy_to_res(CSOUND *csound, ATSSINNOI *p) {
  ...
  partialband = (double **)csound->Malloc(
      csound, sizeof(double *) * (int32_t)p->atshead->npartials);
  bandnum = (int32_t *)csound->Malloc(
      csound, sizeof(int32_t) * (int32_t)p->atshead->npartials);
...
static int32_t atssinnoiset(CSOUND *csound, ATSSINNOI *p) {
  ...
  memsize = (int32_t)(*p->iptls) *
            (sizeof(ATS_DATA_LOC) + 2 * sizeof(double) + sizeof(RANDIATS));
  /* allocate space if we need it */
  /* need room for a buffer and an array of oscillator phase increments */
  ...
  default:
    return csound->InitError(csound, "%s",
                             Str("ATSSINNOI: Type not implemented"));
  }
...
static int32_t atssinnoi(CSOUND *csound, ATSSINNOI *p) {
  ...
  FetchADDNZbands(/*25*/ *p->iptls, p->firstband, p->datastart, p->frmInc,
                  p->maxFr, p->swapped, p->nzbuf, frIndx);
...
static void fetchSINNOIpartials(ATSSINNOI *p, MYFLT position) {
  ...
  if (UNLIKELY(frame == p->maxFr)) {
    if (p->firstband == -1) { /* there is no noise data */
      if (p->swapped == 1) {
        for (i = (int32_t)*p->iptloffset;
             i < (int32_t)*p->iptls + *p->iptloffset;
             i += (int32_t)*p->iptlincr) {
          oscbuf->amp = bswap(frm_0 + 1 + i * (int32_t)p->partialinc); /* amp */
          oscbuf->freq =
              bswap(frm_0 + 2 + i * (int32_t)p->partialinc); /* freq */
          oscbuf++;
        }
      } else {
        for (i = (int32_t)*p->iptloffset;
...
static int32_t atsbufreadset(CSOUND *csound, ATSBUFREAD *p) {
  ...
  if (UNLIKELY((int32_t)(*p->iptloffset + *p->iptls * *p->iptlincr) >
               ...
               (int32_t)(*p->iptloffset) < 0)) {
    return csound->InitError(csound,
                             Str("ATSBUFREAD: Partial out of range, "
                                 "max partial is %i"),
                             n_partials);
  }
  ...
  p->table[(int32_t)*p->iptls + 1].freq =
      p->utable[(int32_t)*p->iptls + 1].freq = 20000;
  p->table[(int32_t)*p->iptls + 1].amp = p->utable[(int32_t)*p->iptls + 1].amp =
      0;
...
static void FetchBUFPartials(ATSBUFREAD *p, ATS_DATA_LOC *buf,
                             ATS_DATA_LOC *buf2, MYFLT position) {
  MYFLT frac; /* the distance in time we are between frames */
  double *frm_0, *frm_1;
  double frm0amp, frm0freq, frm1amp, frm1freq;
  int32_t frame;
...
static int32_t atsinterpread(CSOUND *csound, ATSINTERPREAD *p) {
  ...
  frac =
      (*p->kfreq - (atsbufreadaddr->table[i]).freq) /
...
static int32_t atscrossset(CSOUND *csound, ATSCROSS *p) {
  ...
  if ((int32_t)(*p->iptloffset + *p->iptls * *p->iptlincr) > n_partials ||
      (int32_t)(*p->iptloffset) < 0) {
    return csound->InitError(csound,
                             Str("ATSCROSS: Partial(s) out of range, "
                                 "max partial allowed is %i"),
                             n_partials);
  }
  /* get a pointer to the beginning of the data */
...
static void
ScalePartials(CSOUND *csound, ATS_DATA_LOC *cbuf, /* the current buffer */
  ...
  for (i = 0; i < cbufnp; i++) {
    ...
    if (LIKELY(j < tbufnp && j > 0)) {
      ...
      frac =
          (cbuf[i].freq - tbuf[j - 1].freq) / (tbuf[j].freq - tbuf[j - 1].freq);
...
static int32_t atscross(CSOUND *csound, ATSCROSS *p) {
  ...
  ScalePartials(
      ...
      *p->katsbufamp, *p->kthresh);        /* the amp of the table buffer */
...
static OENTRY localops[] = {
    {"ATSread", S(ATSREAD), 0, 3, "kk", "kSi", (SUBR)atsreadset_S,
     (SUBR)atsread, (SUBR)NULL},
    {"ATSread.i", S(ATSREAD), 0, 3, "kk", "kii", (SUBR)atsreadset,
     (SUBR)atsread, (SUBR)NULL},
    {"ATSreadnz", S(ATSREADNZ), 0, 3, "k", "kSi", (SUBR)atsreadnzset_S,
     (SUBR)atsreadnz, (SUBR)NULL},
    {"ATSreadnz.i", S(ATSREADNZ), 0, 3, "k", "kii", (SUBR)atsreadnzset,
     (SUBR)atsreadnz, (SUBR)NULL},
    {"ATSadd", S(ATSADD), TR, 3, "a", "kkSiiopo", (SUBR)atsaddset_S,
     (SUBR)atsadd},
    {"ATSadd.i", S(ATSADD), TR, 3, "a", "kkiiiopo", (SUBR)atsaddset,
     (SUBR)atsadd},
    {"ATSaddnz", S(ATSADDNZ), 0, 3, "a", "kSiop", (SUBR)atsaddnzset_S,
     (SUBR)atsaddnz},
    {"ATSaddnz.i", S(ATSADDNZ), 0, 3, "a", "kiiop", (SUBR)atsaddnzset,
     (SUBR)atsaddnz},
    {"ATSsinnoi", S(ATSSINNOI), 0, 3, "a", "kkkkSiop", (SUBR)atssinnoiset_S,
     (SUBR)atssinnoi},
    {"ATSsinnoi.i", S(ATSSINNOI), 0, 3, "a", "kkkkiiop", (SUBR)atssinnoiset,
     (SUBR)atssinnoi},
    {"ATSbufread", S(ATSBUFREAD), TW, 3, "", "kkSiop", (SUBR)atsbufreadset_S,
     (SUBR)atsbufread, (SUBR)NULL},
    {"ATSbufread.i", S(ATSBUFREAD), TW, 3, "", "kkiiop", (SUBR)atsbufreadset,
     (SUBR)atsbufread, (SUBR)NULL},
    {"ATSpartialtap", S(ATSPARTIALTAP), 0, 3, "kk", "i", (SUBR)atspartialtapset,
     (SUBR)atspartialtap, (SUBR)NULL},
    {"ATSinterpread", S(ATSINTERPREAD), 0, 3, "k", "k", (SUBR)atsinterpreadset,
     (SUBR)atsinterpread, (SUBR)NULL},
    {"ATScross", S(ATSCROSS), TR, 3, "a", "kkSikkiopoo", (SUBR)atscrossset_S,
     (SUBR)atscross},
    {"ATSinfo", S(ATSINFO), 0, 1, "i", "Si", (SUBR)atsinfo_S, (SUBR)NULL,
     (SUBR)NULL},
    {"ATScross.i", S(ATSCROSS), TR, 3, "a", "kkiikkiopoo", (SUBR)atscrossset,
     (SUBR)atscross},
    {"ATSinfo.i", S(ATSINFO), 0, 1, "i", "ii", (SUBR)atsinfo, (SUBR)NULL,
     (SUBR)NULL}};
