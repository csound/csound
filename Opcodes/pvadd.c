...
void FetchInForAdd(float *inp, MYFLT *buf, int32 fsize, MYFLT pos,
                   int32_t binoffset, int32_t maxbin, int32_t binincr) {
  int32 j;
  float *frame0, *frame1;
  int32 base;
  MYFLT frac;
...
int32_t pvaddset_(CSOUND *csound, PVADD *p, int32_t stringname) {
  ...
  if (*p->igatefun > FL(0.0))
    if (UNLIKELY((AmpGateFunc = csound->FTnp2Finde(csound, p->igatefun)) ==
                 NULL))
      return NOTOK;
  ...
  if (*p->imode == 1 || *p->imode == 2) {
    SpectralExtract(p->frPtr, p->pvcopy, size, p->maxFr, (int32_t)*p->imode,
                    *p->ifreqlim);
...
int32_t pvadd(CSOUND *csound, PVADD *p) {
  ...
  FetchInForAdd(p->frPtr, p->buf, size, frIndx, (int32_t)*p->ibinoffset,
                p->maxbin, binincr);
