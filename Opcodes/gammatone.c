...
static int32_t gammatone_perf(CSOUND *csound, GAMMA *p) {
  ...
  if (UNLIKELY(offset))
    memset(p->ans, '\0', offset * sizeof(MYFLT));
  if (UNLIKELY(early)) {
    nsmps -= early;
  ...
  for (i = offset; i < nsmps; i++) {
    MYFLT real =
        p->expmbt * (cc * yrm1 - ss * yim1) + (1.0 - p->expmbt) * (p->x[i]);
...
static OENTRY gamma_localops[] = {{"gtf", S(GAMMA), 0, 3, "a", "akioo",
                                   (SUBR)gammatone_init, (SUBR)gammatone_perf}};
