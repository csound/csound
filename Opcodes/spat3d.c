...
int    spat3d_init_window (SPAT3D *p)
{
    int     i, j, o;
    double  d, w;
...
spat3d_init_wall(
                 ...
                 int    wallno,         /* wall number                          */
                 int    dep,            /* recursion depth                      */
                 long   *wmax,          /* wall structure number                */
                 MYFLT  X, MYFLT Y, MYFLT Z) /* coordinates (spat3di and spat3dt)*/
{
    int             i;
    SPAT3D_WALL     *ws;
    ...
    for (i = 1; i <= 6; i++)
      if ((p->ftable[(i << 3) - 2] > FL(0.5))
        ...
        ws->nextRefl[i - 1] = spat3d_init_wall (p, i, dep,
                                                wmax, X, Y, Z);
...
int spat3d_init_delay (SPAT3D *p)
{
    long    i, j;
...
void spat3d_count_refl (long *cnt, int *md, int d, int mdep, int w, int wm)
{
    int     j;
...
int spat3d_set_opcode_params (SPAT3D *p)
{
    ...
    if (ximode >= 0)                                /* output mode */
      p->zout = (int) SPAT3D_ROUND (*(p->args[ximode]));
    if (xidist >= 0)                                /* unit circle dist. */
      p->mdist = *(p->args[xidist]);
    if (xift >= 0) {                                /* ftable */
      if ((int) *(p->args[xift]) <= 0) {
        p->ftable = NULL;
      }
      else {
    ...
    if (p->ftable != NULL) {
      if (p->o_num == 2) {                    /* min, max depth    */
        p->mindep = (int) SPAT3D_ROUND (p->ftable[0]) + 1;
        p->maxdep = (int) SPAT3D_ROUND (p->ftable[1]);
      }
      else {
        p->mindep = 0;
        p->maxdep = (int) SPAT3D_ROUND (p->ftable[0]);
      }
      ...
      if (p->rseed < 0L) p->rseed = (long) time (NULL) & 0xFFFFL;
...
int    spat3d (SPAT3D *p)
{
    ...
    if ((p->ws.auxp == NULL) || (p->y.auxp == NULL)) {
      perferror (Str(X_1558,"spat3d: not initialised"));
      return NOTOK;
    }
...
void    spat3di_wall_perf (
                           SPAT3D       *p,     /* opcode struct                */
                           MYFLT        *xn,    /* input signal                 */
                           SPAT3D_WALL  *ws)    /* wall params                  */
{
    MYFLT       *yn, *Wb, *Xb, *Yb, *Zb, w, x, y, z;
    long        xpos0, xpos1, nn, bs;

    yn = ws->yn;
    bs = p->mdel_s;
    xpos0 = (long) ws->D0 + p->del_p; while (xpos0 >= bs) xpos0 -= bs;
    xpos1 = xpos0; Wb = Xb = Yb = Zb = NULL;
    w = ws->W0; x = ws->X0; y = ws->Y0; z = ws->Z0;
    switch (p->zout) {
    case 4:     xpos1 = (long) ws->D1 + p->del_p;
    ...
    for (nn = 0; nn < 6; nn++)
      if (ws->nextRefl[nn] != NULL)
        spat3di_wall_perf (p, ws->yn,
                           (SPAT3D_WALL *) ws->nextRefl[nn]);
}

/* spat3di routine */

int    spat3di (SPAT3D *p)
{
    long        nn;
    ...
    if ((p->ws.auxp == NULL) || (p->y.auxp == NULL)) {
      perferror (Str(X_1559,"spat3di: not initialised"));
      return NOTOK;
    }
                           ...
                           SPAT3D_WALL  *ws)    /* wall params                  */
{
    ...
    Wb = endp + ((long) ws->D0 << 2);
    Yb = (p->zout < 4 ? Wb : endp + ((long) ws->D1 << 2)) + 2;
    ...
    nn = p->bs; while (nn--) {  /* EQ */
      ...
      *yn -= ws->a2 * ws->ynm2; *yn -= ws->a1 * (ws->ynm2 = ws->ynm1);
      ...
      if (p->zout < 4) {
        if (Wb >= endp) continue;       /* end of table */
        *(Wb++) += yw * w; *(Wb++) += yw * x;   /* W, X */
        *(Wb++) += yw * y; *(Wb++) += yw * z;   /* Y, Z */
      } else {
        if (Wb < endp) {                /* Ll, Lh */
    ...
    for (nn = 0; nn < 6; nn++)
      if (ws->nextRefl[nn] != NULL)
        spat3dt_wall_perf (p, ws->yn,
                           (SPAT3D_WALL *) ws->nextRefl[nn]);
}

/* spat3dt opcode (i-time only) */

int    spat3dt (SPAT3D *p)
{
    long    wmax;
    MYFLT   *ir;
    ...
    if (p->maxdep < 0) return OK;           /* nothing to render */
    wmax = 0L; p->mdel = FL(0.0);           /* init. wall structures */
    spat3d_init_wall (p, 0, 0, &wmax, *(p->args[1]), *(p->args[2]),
                      *(p->args[3]));
...
{ "spat3d", S(SPAT3D), 5, "aaaa", "akkkiiiiio", (SUBR)spat3dset,NULL,(SUBR)spat3d},
{ "spat3di",S(SPAT3D), 5, "aaaa", "aiiiiiio", (SUBR)spat3diset,NULL,(SUBR)spat3di},
{ "spat3dt",S(SPAT3D), 1, "", "iiiiiiiio", (SUBR)spat3dt, NULL, NULL           },
};
