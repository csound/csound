...
static int32_t spat3d_init_eq(SPAT3D *p, SPAT3D_WALL *wstruct, MYFLT *ftable) {
  ...
  k = tan((eqmode > 1 ? (PI - omega) : omega) * 0.5);
  kk = k * k;
  vk = (double)ftable[1] * k;
  vkk = (double)ftable[1] * kk;
  ...
  a0 = 1.0 / a0;
  wstruct->a1 = (MYFLT)(a0 * a1);
  wstruct->a2 = (MYFLT)(a0 * a2);
  wstruct->b0 = (MYFLT)(a0 * b0);
  wstruct->b1 = (MYFLT)(a0 * b1);
  wstruct->b2 = (MYFLT)(a0 * b2);
  return OK;
}
...
static int32_t spat3d_init_delay(CSOUND *csound, SPAT3D *p) {
  ...
  if (p->o_num == 1) {
    ...
    /* FALLTHRU */ case 4:
    /* FALLTHRU */ case 3:
      (p->Zb)++; /* spat3d requires 1    */
    /* FALLTHRU */ case 2:
      (p->Xb)++; /* extra sample before  */
    /* FALLTHRU */ case 1:
      (p->Yb)++; /* and 3 samples after  */
    /* FALLTHRU */ case 0:
      (p->Wb)++; /* the delay buffer     */
    }
  }
  return OK;
}
...
static void spat3d_count_refl(int32_t *cnt, int32_t *md, int32_t d,
                              int32_t mdep, int32_t w, int32_t wm) {
  int32_t j;

  (*cnt)++; /* update count    */
  if (++d > *md)
    *md = d; /* md is the max. depth reached + 1 */
  if (d > mdep)
    return;                /* mdep is the max. depth allowed   */
...
static int32_t spat3d_set_opcode_params(CSOUND *csound, SPAT3D *p) {
  ...
  if (xioutft >= 0) { /* output table */
    int32_t fLen;
    fLen =
        csound->GetTable(csound, &(p->outft), (int32_t) * (p->args[xioutft]));
    if (fLen < 1) {
      p->outft = NULL;
      p->outftlnth = 0;
    } else
  ...
  if (p->o_num == 2)
    p->bs = p->irlen; /* block size        */

  /* allocate space */

  if (p->maxdep >= 0) {
    i = d = 0;
    spat3d_count_refl(&i, &d, 0, p->maxdep, 0, wmask);
...
static int32_t spat3diset(CSOUND *csound, SPAT3D *p) {
  ...
  if (p->maxdep < 0)
    return OK; /* nothing to render     */
  wmax = 0L;
  p->mdel = FL(0.0); /* init. wall structures */
  spat3d_init_wall(p, 0, 0, &wmax, *(p->args[5]), *(p->args[6]), *(p->args[7]));
  spat3d_init_delay(csound, p); /* alloc delay buffers   */
  return OK;
}
                             ...
                             MYFLT Z)         /* sound source Z coordinate */
{
  ...
  while (nn--) { /* EQ */
    ...
    *yn = ws->b2 * ws->xnm2;
    *yn += ws->b1 * (ws->xnm2 = ws->xnm1);
    *yn += ws->b0 * (ws->xnm1 = *(xn++));
    *yn -= ws->a2 * ws->ynm2;
    *yn -= ws->a1 * (ws->ynm2 = ws->ynm1);
    ws->ynm1 = *yn;
  ...
  for (nn = 0; nn < 6; nn++)
    if (ws->nextRefl[nn] != NULL)
      spat3d_wall_perf(csound, p, ws->yn, (SPAT3D_WALL *)ws->nextRefl[nn], X, Y,
                       Z);
}
...
static int32_t spat3d(CSOUND *csound, SPAT3D *p) {
  ...
  if (p->maxdep < 0)
    return OK; /* depth < 0 : nothing to render */

  if (UNLIKELY((p->ws.auxp == NULL) || (p->y.auxp == NULL)))
    goto err1;
  ...
  /* FALLTHRU */
  case 3:
    *(p->Zb - 1) = p->Zb[j] = p->Zb[j + 1] = p->Zb[j + 2] = FL(0.0);
  /* FALLTHRU */
  case 2:
    *(p->Xb - 1) = p->Xb[j] = p->Xb[j + 1] = p->Xb[j + 2] = FL(0.0);
  /* FALLTHRU */
  case 1:
    *(p->Yb - 1) = p->Yb[j] = p->Yb[j + 1] = p->Yb[j + 2] = FL(0.0);
  /* FALLTHRU */
  case 0:
    *(p->Wb - 1) = p->Wb[j] = p->Wb[j + 1] = p->Wb[j + 2] = FL(0.0);
  }
...
static int32_t spat3di(CSOUND *csound, SPAT3D *p) {
  ...
  do { /* read from delay line */
    ...
    /* FALLTHRU */ case 4:
    /* FALLTHRU */ case 3:
      *(a_outZ++) = p->Zb[p->del_p]; /* Z */
      p->Zb[p->del_p] = FL(0.0);
      /* FALLTHRU */
    /* FALLTHRU */ case 2:
      *(a_outX++) = p->Xb[p->del_p]; /* X */
      p->Xb[p->del_p] = FL(0.0);
      /* FALLTHRU */
    /* FALLTHRU */ case 1:
      *(a_outY++) = p->Yb[p->del_p]; /* Y */
      p->Yb[p->del_p] = FL(0.0);
      /* FALLTHRU */
    /* FALLTHRU */ case 0:
      *(a_outW++) = p->Wb[p->del_p]; /* W */
      p->Wb[p->del_p] = FL(0.0);
    }
    /* next sample */
    if (++(p->del_p) >= p->mdel_s)
...
static int32_t spat3dt(CSOUND *csound, SPAT3D *p) {
  ...
  wmax = 0L;
  p->mdel = FL(0.0); /* init. wall structures */
  spat3d_init_wall(p, 0, 0, &wmax, *(p->args[1]), *(p->args[2]), *(p->args[3]));
...
static OENTRY localops[] = {
    {"spat3d", S(SPAT3D), 0, 3, "aaaa", "akkkiiiiio", (SUBR)spat3dset,
     (SUBR)spat3d},
    {"spat3di", S(SPAT3D), 0, 3, "aaaa", "aiiiiiio", (SUBR)spat3diset,
     (SUBR)spat3di},
    {"spat3dt", S(SPAT3D), 0, 1, "", "iiiiiiiio", (SUBR)spat3dt, NULL, NULL}};
