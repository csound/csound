...
static int32_t osc_send2_init(CSOUND *csound, OSCSEND2 *p) {
  ...
  if (UNLIKELY(p->INOCOUNT > 4 &&
               p->INOCOUNT < (uint32_t)strlen(p->type->data) + 4))
    return csound->InitError(csound, "%s",
    ...
    for (i = 0; i < p->type->size - 1; i++) {
      ...
      case 't':
        if (UNLIKELY(p->INOCOUNT < (uint32_t)p->type->size + 5))
          return csound->InitError(csound,
                                   "extra argument needed for type t\n");
        bsize += 8;
        iarg += 2;
        break;
      case 'f':
    ...
    if (p->INCOUNT > 4) {
      ...
      */
      ...
      for (i = 0; i < p->iargs; i++) {
        if (p->type->data[i] == 'b') {
          if (*p->arg[i] == FL(0.0))
            ((char *)p->types.auxp)[i] = 'F';
          else
            ((char *)p->types.auxp)[i] = 'T';
        } else if (p->type->data[i] == 'D' || p->type->data[i] == 'A' ||
                   p->type->data[i] == 'G' || p->type->data[i] == 'a')
      ...
      for (i = 0; i < p->iargs; i++) {
        ...
        case 'a':
          ...
          memcpy(out + buffersize + sizeof(MYFLT), p->arg[i],
    ...
    if (UNLIKELY(sendto(p->sock, (void *)out, buffersize, 0, to,
      ...
      if (p->err_state == 0) {
        csound->Warning(csound,
                        Str("OSCsend failed to send "
                            "message with destination %s to %s:%d\n"),
                        p->dest->data, p->ipaddress->data, (int)*p->port);
      }
      p->err_state = 1;
      return OK;
    }
  ...
  */
  ...
  if (p->type->dimensions > 1 || p->dest->dimensions > 1)
    return csound->InitError(
        csound, "%s", Str("type and dest arrays need to be unidimensional\n"));
  if ((p->type->sizes[0] != p->dest->sizes[0]))
    return csound->InitError(
        csound, "%s", Str("type and dest arrays need to have the same size\n"));
...
static int oscbundle_perf(CSOUND *csound, OSCBUNDLE *p) {
  if (*p->kwhen != p->last) {
    int32_t i, n, size = 0, tstrs, dstrs, msize, buffsize = 0, tmp;
    STRINGDAT *dest, *type;
    float fdata;
    int32_t idata, cols;
    char tstr[64], *dstr;
...
static OENTRY socksend_localops[] = {
    ...
    {"OSCsend", S(OSCSEND2), 0, 3, "", "kSkSN", (SUBR)osc_send2_init,
     (SUBR)osc_send2},
    {"OSCbundle", S(OSCBUNDLE), 0, 3, "", "kSkS[]S[]k[][]o",
     (SUBR)oscbundle_init, (SUBR)oscbundle_perf},
};
