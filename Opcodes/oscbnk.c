...
static int32_t grain2(CSOUND *csound, GRAIN2 *p)
{
  ...
  wf            = csound->onedsr / *(p->kgdur);    /* window frequency    */
...
static int32_t osckkikt(CSOUND *csound, OSCKT *p)
{
  ...
  if(floatph) {
   phsf = p->phsf;
   xcps = *(p->xcps) * csound->onedsr;
  }
  else {
  v = *(p->xcps) * csound->onedsr;  
