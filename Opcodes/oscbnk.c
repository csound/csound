...
static MYFLT oscbnk_rnd_bipolar(int32_t *seed, MYFLT rpow, int32_t rmode) {
  ...
  *seed = oscbnk_rand31(*seed);
  ...
  if (!(rmode))
    return ((MYFLT)x); /* uniform distribution */
...
static void oscbnk_flen_setup(int32 flen, uint32 *mask, uint32 *lobits,
                              MYFLT *pfrac);
...
static MYFLT oscbnk_rand(OSCBNK *p) {
  ...
  default:
    if (y > FL(1.0))
      y = FL(1.0); /* limit frequency */
  }

  /* store in output table */

  if (p->tabl_cnt < p->outft_len)
    p->outft[p->tabl_cnt] = y;
...
static int32_t oscbnk(CSOUND *csound, OSCBNK *p) {
  ...
  MYFLT k, a_d = FL(0.0), a1_d = FL(0.0), a2_d = FL(0.0), b0_d = FL(0.0),
           b1_d = FL(0.0), b2_d = FL(0.0);
  ...
  } else if (UNLIKELY((p->seed == 0L) || (p->osc == NULL)))
    goto err1;
...
static int32_t grain2set(CSOUND *csound, GRAIN2 *p) {
  ...
  if (UNLIKELY((ftp == NULL) || ((p->wft = ftp->ftable) == NULL)))
    return NOTOK;
...
static void grain2_init_grain_phase(GRAIN2_OSC *o, uint32 frq, uint32 w_frq,
                                    MYFLT frq_scl, int32_t f_nolock) {
  double d;
  MYFLT f;
  ...
  if (!(w_frq))
    return;
  if (f_nolock) {
    d = (double)o->grain_frq_flt * (double)frq_scl * (double)OSCBNK_PHSMAX +
        (double)frq;
  } else {
...
static int32_t grain2(CSOUND *csound, GRAIN2 *p) {
  ...
  for (nn = offset; nn < nsmps; nn++) {
    ...
    do {
      ...
      /* window waveform */
      n = o->window_phs >> w_lobits;
      a = w_ft[n++];
      if (w_interp)
        a += (w_ft[n] - a) * (MYFLT)((int32)(o->window_phs & w_mask)) * w_pfrac;
      o->window_phs += w_frq;
      /* mix to output */
      aout[nn] += a * k;
...
static void grain3_init_grain(GRAIN3 *p, GRAIN2_OSC *o, uint32 w_ph,
                              uint32 g_ph) {
  MYFLT f;
...
static int32_t grain3(CSOUND *csound, GRAIN3 *p) {
  ...
  if (UNLIKELY((p->seed == 0L) || (p->osc == NULL)))
    goto err1;
  ...
  if ((p->init_k) && (!(p->mode & 0x10))) {
    ...
    if (g_ph < (OSCBNK_PHSMAX >> 16))
      g_ph += g_frq;
    g_ph = OSCBNK_PHSMAX - g_ph;
    while (g_ph) {
      grain3_init_grain(p, p->osc_end, g_ph, *phs);
      if (!(p->mode & 0x40)) /* init. grain phase    */
        grain2_init_grain_phase(p->osc_end, frq, w_frq, frq_scl, f_nolock);
      if (++(p->osc_end) > p->osc_max)
        p->osc_end = p->osc;
      if (UNLIKELY(p->osc_end == p->osc_start))
        goto err2;
      g_ph -= g_frq;
    }
  }
  ...
  while (nn > offset) {
    ...
    while (i--) {
      ...
      a = w_ft[n++];
      if (w_interp)
        a += (w_ft[n] - a) * w_pfrac * (MYFLT)((int32)(w_ph & w_mask));
      /* grain waveform */
      n = g_ph >> lobits;
      k = ft[n++];
      if (g_interp)
        k += (ft[n] - k) * pfrac * (MYFLT)((int32)(g_ph & mask));
      /* mix to output */
      *(aout++) += a * k;
      /* update phase */
      g_ph = (g_ph + g_frq) & OSCBNK_PHSMSK;
...
static int32_t kosclikt(CSOUND *csound, OSCKT *p) {
  ...
  if (*(p->kfn) != p->oldfn || p->ft == NULL) {
    ...
    if (UNLIKELY((ftp == NULL) || ((p->ft = ftp->ftable) == NULL)))
      return NOTOK;
  ...
  n = phs >> p->lobits;
  v = (MYFLT)((int32)(phs & p->mask)) * p->pfrac;
...
static int32_t osckkikt(CSOUND *csound, OSCKT *p) {
  ...
  for (nn = offset; nn < nsmps; nn++) {
    n = phs >> lobits;
    v = ft[n++];
    v += (ft[n] - v) * (MYFLT)((int32)(phs & mask)) * pfrac;
...
static int32_t osckts(CSOUND *csound, OSCKTS *p) {
  ...
  /* copy object data to local variables */
  ft = p->ft;
  a_amp = (IS_ASIG_ARG(p->xamp) ? 1 : 0);
  a_cps = (IS_ASIG_ARG(p->xcps) ? 1 : 0);
  phs = p->phs;
  ar = p->ar;
  xcps = p->xcps;
  xamp = p->xamp;
...
static void vco2_calculate_table(CSOUND *csound, VCO2_TABLE *table,
  ...
  if (tp->waveform >= 0) { /* no DC offset for   */
    minh = 1;
    fftbuf[0] = fftbuf[1] = FL(0.0); /* built-in waveforms */
  } else
    minh = 0;
  ...
  switch (tp->waveform) {
  case 0:
    scaleFac *= (FL(-2.0) / PI_F);
    break;
  case 1:
    scaleFac *= (FL(-4.0) / (PI_F * PI_F));
    break;
  case 3:
    scaleFac *= (FL(-4.0) / PI_F);
    break;
  case 4:
    scaleFac *= (FL(8.0) / (PI_F * PI_F));
    break;
  }
  ...
  for (i = minh; i <= (table->size >> 1); i++) {
    ...
    case 4: /* triangle */
      fftbuf[(i << 1) + 1] =
          (i & 1 ? ((i & 2 ? scaleFac : (-scaleFac)) / ((MYFLT)i * (MYFLT)i))
                 : FL(0.0));
      break;
...
static int32_t vco2_tables_create(CSOUND *csound, int32_t waveform,
                                  int32_t base_ftable, VCO2_TABLE_PARAMS *tp) {
  STDOPCOD_GLOBALS *pp = get_oscbnk_globals(csound);
  ...
  if (waveform >= pp->vco2_nr_table_arrays) {
    ...
    pp->vco2_tables = (VCO2_TABLE_ARRAY **)csound->ReAlloc(
        csound, pp->vco2_tables, sizeof(VCO2_TABLE_ARRAY *) * ntables);
  ...
  if (pp->vco2_tables[waveform] != NULL) {
    ...
    csound->Warning(csound, Str("redefined table array for waveform %d\n"),
                    (waveform > 4 ? 4 - waveform : waveform));
  }
  /* calculate number of tables */
  i = tp->max_size >> 1;
  if (i > VCO2_MAX_NPART)
    i = VCO2_MAX_NPART; /* max number of partials */
  npart_f = 0.0;
  ntables = 0;
  do {
    ntables++;
  ...
  /* ... and all tables */
#ifdef VCO2FT_USE_TABLE
  tables->nparts_tabl = (VCO2_TABLE **)csound->Malloc(
      csound, sizeof(VCO2_TABLE *) * (VCO2_MAX_NPART + 1));
#else
  tables->nparts =
  ...
  do {
    ...
    oscbnk_flen_setup((int32)tables->tables[i].size, &(tables->tables[i].mask),
                      &(tables->tables[i].lobits), &(tables->tables[i].pfrac));
    ...
    if (base_ftable > 0) {
      ...
      tables->tables[i].ftable = /* standard Csound ftable) */
          (MYFLT *)csound->Malloc(csound,
                                  sizeof(MYFLT) * (tables->tables[i].size + 1));
  ...
  do {
    ...
    if (i < (ntables - 1) && npart >= tables->tables[i + 1].npart)
      i++;
...
static int32_t vco2init(CSOUND *csound, VCO2INIT *p) {
  ...
  if (UNLIKELY(waveforms < -1000000 || waveforms > 31)) {
    return csound->InitError(
        csound, Str("vco2init: invalid waveform number: %f"), *(p->iwaveforms));
  }
  ...
  *(p->ift) = (MYFLT)ftnum;
  ...
  do {
    ...
    if (*(p->imaxsiz) > FL(0.0)) {
      ...
      if (UNLIKELY(i < 16 || i > 16777216 || (i & (i - 1)) ||
                   i < tp.min_size)) {
        return csound->InitError(csound, "%s",
      ...
      if (tp.max_size > 16384)
        tp.max_size = 16384;
      if (tp.max_size < tp.min_size)
        tp.max_size = tp.min_size;
    }
...
static int32_t vco2ftset(CSOUND *csound, VCO2FT *p) {
  ...
  if (w < 0)
    w = 4 - w;
  if (UNLIKELY(w >= *(p->vco2_nr_table_arrays) ||
               (*(p->vco2_tables))[w] == NULL ||
  ...
  p->nparts = (*(p->vco2_tables))[w]->nparts + (*(p->vco2_tables))[w]->ntabl;
...
static int32_t vco2set(CSOUND *csound, VCO2 *p) {
  ...
  //      return csound->InitError(csound, "%s", Str("vco2: invalid argument
  //      type"));
  //    }
  ...
  if (tnum >= *(p->vco2_nr_table_arrays) || (*(p->vco2_tables))[tnum] == NULL) {
    if (LIKELY(tnum < 5))
  ...
  /* address of number of partials list (with offset for padding) */
  p->nparts =
      (*(p->vco2_tables))[tnum]->nparts + (*(p->vco2_tables))[tnum]->ntabl;
...
static int32_t vco2(CSOUND *csound, VCO2 *p) {
  ...
  if (p->pm_enabled) {
    f1 = (MYFLT)((double)*(p->kphs) - (double)p->kphs_old) / (nsmps - offset);
    p->kphs_old = *(p->kphs);
...
#ifndef USE_DOUBLE
#define DENORM_RND                                                             \
  ((MYFLT)((*seed = (*seed * 15625 + 1) & 0xFFFF) - 0x8000) * FL(1.0e-24))
#else
#define DENORM_RND                                                             \
  ((MYFLT)((*seed = (*seed * 15625 + 1) & 0xFFFF) - 0x8000) * FL(1.0e-60))
#endif
...
static int32_t delaykset(CSOUND *csound, DELAYK *p) {
  ...
  if (UNLIKELY(npts < 1))
    return csound->InitError(csound, "%s",
                             Str("delayk: invalid delay time "
                                 "(must be >= 0)"));
  p->readp = 0;
  p->npts = npts;
  /* allocate space for delay buffer */
  if (p->aux.auxp == NULL || (uint32_t)(npts * sizeof(MYFLT)) > p->aux.size) {
...
static int32_t vdelaykset(CSOUND *csound, VDELAYK *p) {
  ...
  if (UNLIKELY(npts < 1))
    return csound->InitError(csound, "%s",
                             Str("vdel_k: invalid max delay time "
                                 "(must be >= 0)"));
  p->wrtp = 0;
  p->npts = npts;
  /* allocate space for delay buffer */
...
static const OENTRY localops[] = {
    ...
    {"grain2", sizeof(GRAIN2), TR, 3, "a", "kkkikiooo", (SUBR)grain2set,
     (SUBR)grain2},
    {"grain3", sizeof(GRAIN3), TR, 3, "a", "kkkkkkikikkoo", (SUBR)grain3set,
     (SUBR)grain3},
    {"rnd31", 0xFFFF, 0, 0, NULL, NULL, (SUBR)NULL, (SUBR)NULL, (SUBR)NULL},
    {"rnd31.i", sizeof(RND31), 0, 1, "i", "iio", (SUBR)rnd31i, (SUBR)NULL,
     (SUBR)NULL},
    {"rnd31.k", sizeof(RND31), 0, 3, "k", "kko", (SUBR)rnd31set, (SUBR)rnd31k,
     (SUBR)NULL},
    {"rnd31.a", sizeof(RND31), 0, 3, "a", "kko", (SUBR)rnd31set, (SUBR)rnd31a},
    {"oscilikt", 0xFFFE, TR},
    {"oscilikt.a", sizeof(OSCKT), 0, 3, "a", "kkkoo", (SUBR)oscktset,
     (SUBR)osckkikt},
    {"oscilikt.kk", sizeof(OSCKT), 0, 3, "k", "kkkoo", (SUBR)oscktset,
     (SUBR)kosclikt, NULL},
    {"oscilikt.ka", sizeof(OSCKT), 0, 3, "a", "kakoo", (SUBR)oscktset,
     (SUBR)osckaikt},
    {"oscilikt.ak", sizeof(OSCKT), 0, 3, "a", "akkoo", (SUBR)oscktset,
     (SUBR)oscakikt},
    {"oscilikt.aa", sizeof(OSCKT), 0, 3, "a", "aakoo", (SUBR)oscktset,
     (SUBR)oscaaikt},
    {"osciliktp", sizeof(OSCKTP), TR, 3, "a", "kkko", (SUBR)oscktpset,
     (SUBR)oscktp},
    {"oscilikts", sizeof(OSCKTS), TR, 3, "a", "xxkako", (SUBR)oscktsset,
     (SUBR)osckts},
    {"vco2init", sizeof(VCO2INIT), TW, 1, "i", "ijjjjj", (SUBR)vco2init,
     (SUBR)NULL, (SUBR)NULL},
    {"vco2ift", sizeof(VCO2FT), TW, 1, "i", "iov", (SUBR)vco2ftset, (SUBR)NULL,
     (SUBR)NULL},
    {"vco2ft", sizeof(VCO2FT), TW, 3, "k", "kov", (SUBR)vco2ftset, (SUBR)vco2ft,
     (SUBR)NULL},
    //    { "vco2",       sizeof(VCO2),       TR, 3,      "a",    "kkoM",
    {"vco2", sizeof(VCO2), TR, 3, "a", "kkoOOo", (SUBR)vco2set, (SUBR)vco2},
    {"denorm", sizeof(DENORMS), WI, 2, "", "y", (SUBR)NULL, (SUBR)denorms},
    {"delayk", sizeof(DELAYK), 0, 3, "k", "kio", (SUBR)delaykset, (SUBR)delayk,
     (SUBR)NULL},
    {"vdel_k", sizeof(VDELAYK), 0, 3, "k", "kkio", (SUBR)vdelaykset,
     (SUBR)vdelayk, (SUBR)NULL},
    {"rbjeq", sizeof(RBJEQ), 0, 3, "a", "akkkko", (SUBR)rbjeqset, (SUBR)rbjeq}};
