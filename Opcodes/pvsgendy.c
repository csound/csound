...
static int32_t pvsgendy(CSOUND *csound, PVSGENDY *p) {
  ...
  if (p->fin->sliding) {
    ...
    for (n = 0; n < offset; n += 2)
      foutf[n] = foutf[n + 1] = FL(0.0);
    for (n = nsmps - early; n < nsmps; n += 2)
      foutf[n] = foutf[n + 1] = FL(0.0);
    nsmps -= early;
    for (n = offset; n < nsmps; n++) {
      ...
      for (i = 0; i < NB - 1; i++) {
        ...
        fout[i].im = fin[i].im + frate * (MYFLT)(rand() - RAND_MAX / 2) /
                                     (MYFLT)RAND_MAX / (MYFLT)(i + 1);
      }
    }
    return OK;
  }
  if (p->lastframe < p->fin->framecount) {
    for (i = 0; i < N; i += 2) {
      MYFLT x = frate * (MYFLT)(rand() - RAND_MAX / 2) / (MYFLT)RAND_MAX /
                (MYFLT)(i + 1);
      foutf[i + 1] = finf[i + 1] + x;
      foutf[i] =
          finf[i]; //+ mrate * (MYFLT)(rand()-RAND_MAX/2)/(MYFLT)RAND_MAX;
    }
...
static OENTRY pvsgendy_localops[] = {{"pvsgendy", sizeof(PVSGENDY), 0, 3, "f",
                                      "fkk", (SUBR)pvsgendyinit, (SUBR)pvsgendy,
                                      (SUBR)NULL}};

int32_t pvsgendy_localops_init_(CSOUND *csound) {
  return csound->AppendOpcodes(
      csound, &(pvsgendy_localops[0]),
