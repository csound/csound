...
static PSCSNU *listget(CSOUND *csound, int32_t id) {
  ...
  if (UNLIKELY(i == NULL)) {
    csound->ErrorMsg(csound, "%s",
                     Str("scans: No scan synthesis net specified"));
    return NULL;
  }
  while (1) {
    if (i->id == id)
...
static int32_t scsnu_init(CSOUND *csound, PSCSNU *p) {
  ...
  /* Spring stiffness */
  {
    ...
    if (UNLIKELY((f = csound->FTnp2Find(csound, p->i_f)) == NULL)) {
      return csound->InitError(csound, "%s %p",
...
/* Make buffers to hold data */
  ...
  if (MODF(*p->i_init, &temp)) {
    ...
    int32_t res;
    if (*p->i_id <= FL(0.0))
      scsnu_hammer(csound, p, FL(0.5), FL(1.0));
    else if ((res = scsnu_initw(csound, p)) != OK)
      return res;
  }
  if (*p->i_disp)
...
#if PHASE_INTERP == 3
#define pinterp(ii, x)                                                         \
  (pp->x1[p->t[(int32_t)ii]] +                                                 \
   x * (-pp->x3[p->t[(int32_t)ii]] * FL(0.5) +                                 \
        x * (pp->x3[p->t[(int32_t)ii]] * FL(0.5) - pp->x1[p->t[(int32_t)ii]] + \
             pp->x2[p->t[(int32_t)ii]] * FL(0.5)) +                            \
        pp->x2[p->t[(int32_t)ii]] * FL(0.5)))
#else
#define pinterp(ii, x)                                                         \
  (pp->x2[p->t[(int32_t)ii]] +                                                 \
   (pp->x1[p->t[(int32_t)ii]] - pp->x2[p->t[(int32_t)ii]]) * x)
#endif
...
static int32_t scsns_play(CSOUND *csound, PSCSNS *p) {
  ...
  case 4:
    for (i = offset; i < nsmps; i++) {
      ...
      out[i] = *p->k_amp * (y2 + x * (-y1 / FL(3.0) - y2 * FL(0.5) + y3 +
                                      x * (y1 * FL(0.5) - y2 + y3 * FL(0.5) +
                                           x * (-y1 / FL(6.0) + y2 * FL(0.5) -
                                                y3 * FL(0.5) + y4 / FL(6.0))) -
                                      y4 / FL(6.0)));
      /* Update oscillator phase and wrap around if needed */
      phs += inc;
...
static OENTRY localops[] = {
    {"scanu", S(PSCSNU), TR, 3, "", "iiiiiiikkkkiikkaii", (SUBR)scsnu_init1,
     (SUBR)scsnu_play},
    {"scanu2", S(PSCSNU), TR, 3, "", "iiiiiiikkkkiikkaii", (SUBR)scsnu_init2,
     (SUBR)scsnu_play},
    {"scans", S(PSCSNS), TR, 3, "a", "kkiio", (SUBR)scsns_init,
     (SUBR)scsns_play},
