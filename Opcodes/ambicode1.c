...
static int32_t abformenc(CSOUND *csound, AMBIC *p) {
  ...
  for (channelIndex = 0; channelIndex < channelCount; channelIndex++) {
    ...
    if (UNLIKELY(early))
      memset(&output[sampleCount], '\0', early * sizeof(MYFLT));
...
static int32_t abformenc_a(CSOUND *csound, AMBICA *p) {
  IGN(csound);
  uint32_t offset = p->h.insdshead->ksmps_offset;
  uint32_t early = p->h.insdshead->ksmps_no_end;
...
static int32_t ibformdec(CSOUND *csound, AMBID *p) {
  /* All we do in here is police our parameters. */
  if (UNLIKELY(p->INOCOUNT != 1 + 4 && p->INOCOUNT != 1 + 9 &&
               p->INOCOUNT != 1 + 16)) {
    return csound->InitError(
        csound, "%s", Str("The number of input arguments is not valid."));
  } else if (UNLIKELY(*(p->isetup) < 1 || *(p->isetup) > 5)) {
    return csound->InitError(
        csound, "%s", Str("The isetup value should be between 1 and 5."));
  } else {
...
static int32_t abformdec(CSOUND *csound, AMBID *p) {
  ...
  case 3: /* 5.0 */
    ...
    if (p->INOCOUNT == 1 + 4) {
      ...
      for (sampleIndex = offset; sampleIndex < sampleCount; sampleIndex++) {
        ...
        /* Left: */
        p->aouts[0][sampleIndex] = w * FL(0.405) + x * FL(0.32) + y * FL(0.31);
        /* Right: */
        p->aouts[1][sampleIndex] = w * FL(0.405) + x * FL(0.32) - y * FL(0.31);
        /* Centre: */
        p->aouts[2][sampleIndex] = w * FL(0.085) + x * FL(0.04);
        /* Surround Left: */
        p->aouts[3][sampleIndex] = w * FL(0.635) - x * FL(0.335) + y * FL(0.28);
        /* Surround Right: */
        p->aouts[4][sampleIndex] = w * FL(0.635) - x * FL(0.335) - y * FL(0.28);
      }
    } else {
      ...
      for (sampleIndex = offset; sampleIndex < sampleCount; sampleIndex++) {
        ...
        p->aouts[0][sampleIndex] =
            (w * FL(0.405) + x * FL(0.32) + y * FL(0.31) + u * FL(0.085) +
             v * FL(0.125));
        /* Right: */
        ...
        p->aouts[2][sampleIndex] =
            (w * FL(0.085) + x * FL(0.04) + u * FL(0.045));
        /* Surround Left: */
        p->aouts[3][sampleIndex] = (w * FL(0.635) - x * FL(0.335) +
                                    y * FL(0.28) - u * FL(0.08) + v * FL(0.08));
        /* Surround Right: */
        p->aouts[4][sampleIndex] = (w * FL(0.635) - x * FL(0.335) -
                                    y * FL(0.28) - u * FL(0.08) - v * FL(0.08));
      }
    }
    break;
  case 4: /* Octagon: */
    ...
    if (p->INOCOUNT == 1 + 4) {
      ...
      if (UNLIKELY(early)) {
        sampleCount -= early;
        for (sampleIndex = 0; sampleIndex < 8; sampleIndex++)
          memset(&p->aouts[sampleIndex][sampleCount], '\0',
                 early * sizeof(MYFLT));
      }
      for (sampleIndex = offset; sampleIndex < sampleCount; sampleIndex++) {
      ...
      for (sampleIndex = offset; sampleIndex < sampleCount; sampleIndex++) {
        ...
        /* About 11 o'clock: */
        p->aouts[0][sampleIndex] =
            (w + x * FL(0.173227) + y * FL(0.071753) + u + v +
             p0 * FL(0.004784) + q * FL(0.011548));
        /* About 10 o'clock: */
        p->aouts[1][sampleIndex] =
            (w + x * FL(0.071753) + y * FL(0.173227) - u + v -
...
static int32_t ibformdec_a(CSOUND *csound, AMBIDA *p) {
  int32_t dim;
  ...
  dim = p->tabin->sizes[0];
  /* All we do in here is police our parameters. */
  if (UNLIKELY(dim != 4 && dim != 9 && dim != 16)) {
    return csound->InitError(
        csound, "%s", Str("The number of input arguments is not valid."));
  } else if (UNLIKELY(*(p->isetup) < 1 || *(p->isetup) > 5)) {
    return csound->InitError(
...
static int32_t abformdec_a(CSOUND *csound, AMBIDA *p) {
  ...
  case 3: /* 5.0 */
    ...
    if (dim == 4) {
      ...
      for (sampleIndex = offset; sampleIndex < sampleCount; sampleIndex++) {
        ...
        /* Left: */
        tabout[sampleIndex] = w * FL(0.405) + x * FL(0.32) + y * FL(0.31);
        /* Right: */
        tabout[ksmps + sampleIndex] =
            w * FL(0.405) + x * FL(0.32) - y * FL(0.31);
        /* Centre: */
        tabout[2 * ksmps + sampleIndex] = w * FL(0.085) + x * FL(0.04);
        /* Surround Left: */
      ...
      for (sampleIndex = offset; sampleIndex < sampleCount; sampleIndex++) {
        ...
        /* Surround Left: */
        tabout[3 * ksmps + sampleIndex] =
            (w * FL(0.635) - x * FL(0.335) + y * FL(0.28) - u * FL(0.08) +
             v * FL(0.08));
        /* Surround Right: */
        tabout[4 * ksmps + sampleIndex] =
            (w * FL(0.635) - x * FL(0.335) - y * FL(0.28) - u * FL(0.08) -
  ...
  case 4: /* Octagon: */
    if (p->tabin->sizes[0] == 4) {
      ...
      for (sampleIndex = offset; sampleIndex < sampleCount; sampleIndex++) {
        ...
        /* About 11 o'clock: */
        tabout[sampleIndex] = w + x * FL(0.15906) + y * FL(0.06588) + u + v;
        /* About 10 o'clock: */
        tabout[ksmps + sampleIndex] =
            w + x * FL(0.06588) + y * FL(0.15906) - u + v;
        /* About 8 o'clock: */
      ...
      for (sampleIndex = offset; sampleIndex < sampleCount; sampleIndex++) {
        ...
        /* About 11 o'clock: */
        tabout[0 + sampleIndex] = (w + x * FL(0.173227) + y * FL(0.071753) + u +
                                   v + p0 * FL(0.004784) + q * FL(0.011548));
        /* About 10 o'clock: */
        tabout[ksmps + sampleIndex] =
            (w + x * FL(0.071753) + y * FL(0.173227) - u + v -
             p0 * FL(0.011548) - q * FL(0.004784));
...
static OENTRY ambicode1_localops[] = {
    {"bformenc1.a", S(AMBIC), 0, 3, "mmmmmmmmmmmmmmmm", "akk", (SUBR)ibformenc,
     (SUBR)abformenc},
    {"bformenc1.A", S(AMBIC), 0, 3, "a[]", "akk", (SUBR)ibformenc_a,
     (SUBR)abformenc_a},
    {"bformdec1.a", S(AMBID), 0, 3, "mmmmmmmm", "iy", (SUBR)ibformdec,
     (SUBR)abformdec},
    {"bformdec1.A", S(AMBIDA), 0, 3, "a[]", "ia[]", (SUBR)ibformdec_a,
     (SUBR)abformdec_a},
};
