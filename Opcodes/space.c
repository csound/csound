...
static int32_t spaceset(CSOUND *csound, SPACE *p) {
  ...
  if (p->auxch.auxp == NULL || p->auxch.size < sizeof(MYFLT) * (CS_KSMPS * 4)) {
    MYFLT *fltp;
    csound->AuxAlloc(csound, (size_t)(CS_KSMPS * 4) * sizeof(MYFLT), &p->auxch);
...
static int32_t space(CSOUND *csound, SPACE *p) {
  ...
  ch3 = SIN(HALFPI_F * (FL(1.0) - xndx)) * SIN(HALFPI_F * (FL(1.0) - yndx)) *
        sqrt2;
