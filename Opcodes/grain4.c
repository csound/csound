...
static int32_t grainsetv4(CSOUND *csound, GRAINV4 *p) {
  ...
  if (*p->ipshift != FL(0.0)) {
    if (UNLIKELY(*p->ipitch1 < FL(0.0))) {
      return csound->InitError(
          csound, "%s", Str("granule_set: ipitch1 must be greater then zero"));
    }
    if (UNLIKELY(*p->ipitch2 < FL(0.0))) {
      return csound->InitError(
          csound, "%s", Str("granule_set: ipitch2 must be greater then zero"));
    }
    if (UNLIKELY(*p->ipitch3 < FL(0.0))) {
      return csound->InitError(
          csound, "%s", Str("granule_set: ipitch3 must be greater then zero"));
    }
    if (UNLIKELY(*p->ipitch4 < FL(0.0))) {
      return csound->InitError(
          csound, "%s", Str("granule_set: ipitch4 must be greater then zero"));
    }
  }
  ...
  if (UNLIKELY((*p->iatt < FL(0.0)) || (*p->idec < 0.0) ||
    ...
    return csound->InitError(
        csound, "%s", Str("granule_set: Illegal value of iatt and/or idec"));
  } /* end if */
  ...
  if (*p->ipshift == 0) {
    for (nvoice = 0; nvoice < *p->ivoice; nvoice++) {
      tmpfloat1 = grand(p);
      p->pshift[nvoice] = (tmpfloat1 < FL(0.0))
                              ? (tmpfloat1 * FL(0.5)) + FL(1.0)
                              : tmpfloat1 + 1.0f;
    }
  }
    ...
    for (nvoice = 0; nvoice < *p->ivoice; nvoice++) {
      ...
      if (*cnt >= *stretch) {
        ...
        *gskip += (int32)((*gsize / *pshift) * iratio);
        ...
        if (*p->ipshift == 0) {
          tmpfloat1 = grand(p);
          *pshift = (tmpfloat1 < FL(0.0)) ? (tmpfloat1 * FL(0.5)) + FL(1.0)
                                          : tmpfloat1 + FL(1.0);
        }
