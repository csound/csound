  ...
  if (LIKELY((p->ftp1 = csound->FTFind(csound, p->ifna)) != NULL &&
    ...
    if (!skip) {               /* legato: skip all memory management */
      ...
      if (*p->iphs == FL(0.0)) /* if fundphs zero,  */
        ...
        p->fundphs = (int32)(*p->iphs * FMAXLEN) & PHMASK;
    ...
    /* p->speedcod  = (p->XINCODE & 0x8) ? 1 : 0; */ /*out for phs version of
                                                        fog*/
...
static int32_t fog(CSOUND *csound, FOGS *p) {
  ...
  for (n = offset; n < nsmps; n++) {
    if (p->fundphs & MAXLEN) { /* if phs has wrapped */
      p->fundphs &= PHMASK;
      if (UNLIKELY((ovp = p->basovrlap.nxtfree) == NULL))
        goto err1;
    ...
    while (ovp->nxtact != NULL) { /* perform cur actlist:  */
      ...
      /*  result = *(ftp1->ftable + (ovp->formphs >> ftp1->lobits) ); FOF
       * version*/
      if (p->fmtmod)
    ...
    if (p->xincod) {
      ...
      if (p->fundcod)
        fund_inc = (int32)(*++fund * CS_SICVT);
      if (p->formcod)
        form_inc = (int32)(*++ptch * fogcvt);
      /*form_inc = *++form * CS_SICVT;*/
      /*      if (p->speedcod)  speed_inc = *++speed * fogcvt; */ /*JMC for
                                                                     FOG*/
    }
    p->durtogo--;
  }
...
static int32_t newpulse(CSOUND *csound, FOGS *p, OVERLAP *ovp, MYFLT *amp,
   ...
   */
    ...
    ovp->formphs = (int32)(p->fundphs * form / *fund) & PHMASK;
  ...
  if (*p->kris >= CS_ONEDSR && form != 0.0) { /* init fnb ris */
    ovp->risphs =
        (uint32)(ovp->formphs / (fabs(form)) / *p->kris); /* JPff fix */
