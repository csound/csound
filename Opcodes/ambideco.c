...
typedef struct
{
  ...
  double w[8], x[8], y[8], z[8], r[8], s[8], t[8], u[8],
    v[8], k[8], l[8], m[8], n[8], o[8], p[8], q[8];  /* private dataspace */
} AMBID;
#include <math.h>

void ambideco_set_coefficients(AMBID *p, MYFLT alpha, MYFLT beta, int index)
{
    ...
    /* 2nd order */
    (p -> r)[index] = FL(0.5) * (FL(3.0) * (p -> z)[index] *
                                 (p -> z)[index] - FL(1.0));
    (p -> s)[index] = FL(2.0) * (p -> x)[index] * (p -> z)[index];
    (p -> t)[index] = FL(2.0) * (p -> y)[index] * (p -> z)[index];
    (p -> u)[index] = (p -> x)[index] * (p -> x)[index] - (p -> y)[index] *
      (p -> y)[index];
    ...
    (p -> m)[index] = FL(8.0) / FL(11.0) * (p -> x)[index] *
      (FL(5.0) * (p -> z)[index] * (p -> z)[index] - FL(1.0));
    (p -> n)[index] = FL(2.0) * (p -> x)[index] *
      (p -> y)[index] * (p -> z)[index];
    (p -> o)[index] = (p -> z)[index] *
      ((p -> x)[index] * (p -> x)[index] - (p -> y)[index] * (p -> y)[index]);
    (p -> p)[index] = FL(3.0) * (p -> y)[index] *
      (FL(3.0) * (p -> x)[index] * (p -> x)[index] -
       (p -> y)[index] * (p -> y)[index]);
    (p -> q)[index] = FL(3.0) * (p -> x)[index] *
      ((p -> x)[index] * (p -> x)[index] - FL(3.0) * (p -> y)[index] *
       (p -> y)[index]);
}

int iambideco(CSOUND *csound, AMBID *p)
{
    ...
    switch ((int)*p -> isetup)
          ...
          if (p -> OUTOCOUNT != 2)
            {
              return csound->InitError(csound,
                                       "Wrong number of output cells! There "
                                       "must be 2 output cells.\n");
            }
          else
            {
      ...
      case 3: {
        if (p -> OUTOCOUNT != 5) {
          ...
          ambideco_set_coefficients(p, FL(250.0), FL(0.0), 3);  /* surround left */
          ambideco_set_coefficients(p, FL(110.0), FL(0.0), 4);  /* surround right */
        }
        break;
      }
...
int aambideco(CSOUND *csound, AMBID *p)
{
