...
static int32_t harmon234(CSOUND *csound, HARM234 *p) {
  ...
  if (koct >= p->minoct) { /* PERIODIC: find the pulse */
    ...
    if ((val0 = *x++) == FL(0.0))
      while (*x == FL(0.0) && ++x < plim)
        ; /* if no signal in this range   */
    else if (val0 > FL(0.0))
      while (*x > 0. && ++x < plim)
        ; /* or unipolar with no z-crossing */
    else
      while (*x < FL(0.0) && ++x < plim)
        ;
    if (x >= plim)
      goto nonprd; /*      then non-periodic       */
    ...
    if (p->polarity > 0) {
      ...
      if (posp == NULL)
        goto nonprd;
      for (x = posp; x >= buf0 && *x > FL(0.0); x--)
        ; /* & its preceding z-crossing */
      xdist = posp - x;
...
static OENTRY harmon_localops[] = {
    {"harmon2", S(HARM234), 0, 3, "a", "akkkiip", (SUBR)harm2set,
     (SUBR)harmon234},
    {"harmon3", S(HARM234), 0, 3, "a", "akkkkiip", (SUBR)harm3set,
     (SUBR)harmon234},
    {"harmon4", S(HARM234), 0, 3, "a", "akkkkkiip", (SUBR)harm4set,
     (SUBR)harmon234},
};
