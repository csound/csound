...
static int32_t pvsgainset(CSOUND *csound, PVSGAIN *p) {
  ...
  if (p->fa->sliding) {
    ...
    p->fout->NB = p->fa->NB;
    p->fout->sliding = 1;
  } else if (p->fout->frame.auxp == NULL ||
...
static int32_t pvsfwriteset_(CSOUND *csound, PVSFWRITE *p, int32_t stringname) {
  ...
  if (stringname == 0) {
    if (csound->ISSTRCOD(*p->file))
      ...
      csound->strarg2name(csound, fname, p->file, "pvoc.", 0);
  } else
    strncpy(fname, ((STRINGDAT *)p->file)->data, MAXNAME);
  ...
  p->pvfile = -1;
  N = p->N = p->fin->N;
  if ((p->pvfile = csound->PVOC_CreateFile(
           csound, fname, p->fin->N, p->fin->overlap, 1, p->fin->format, CS_ESR,
           STYPE_16, p->fin->wintype, 0.0f, NULL, p->fin->winsize)) == -1)

    return csound->InitError(csound, Str("pvsfwrite: could not open file %s\n"),
                             fname);
#ifndef __EMSCRIPTEN__
  OPARMS parm;
  ...
  if (parm.realtime) {
    ...
    p->cb = csound->CreateCircularBuffer(
        csound, (N + 2) * sizeof(float) * bufframes, sizeof(MYFLT));
    // PTHREAD: change
...
static int32_t pvsdiskinset_(CSOUND *csound, pvsdiskin *p, int32_t stringname) {
  ...
  if (UNLIKELY(p->fout->sliding))
    return csound->InitError(csound, "%s",
                             Str("SDFT Not implemented in this case yet"));
  if ((p->pvfile = csound->PVOC_OpenFile(csound, fname, &pvdata, &fmt)) < 0)
    return csound->InitError(csound, Str("pvsdiskin: could not open file %s\n"),
                             fname);
  ...
  if (p->buffer.auxp == NULL ||
    ...
    csound->AuxAlloc(csound, (N + 2) * sizeof(float) * FSIGBUFRAMES * p->chans,
                     &p->buffer);
...
static int32_t pvsoscset(CSOUND *csound, PVSOSC *p) {
  ...
  if (p->fout->overlap < (int32_t)CS_KSMPS || p->fout->overlap <= 10) {
    ...
    if (p->fout->frame.auxp == NULL ||
      ...
      csound->AuxAlloc(csound, (N + 2) * CS_KSMPS * sizeof(MYFLT),
                       &p->fout->frame);
    else
      memset(p->fout->frame.auxp, '\0', (N + 2) * CS_KSMPS * sizeof(MYFLT));
...
static int32_t pvsoscprocess(CSOUND *csound, PVSOSC *p) {
  ...
  if (p->fout->sliding) {
    ...
    if (type == 1)
      famp *= FL(1.456) / POWER((MYFLT)harm, FL(1.0) / FL(2.4));
    else if (type == 2)
  ...
  if (p->lastframe > p->fout->framecount) {
    ...
    if (type == 1)
      ...
      famp *= FL(1.456) / POWER(harm, FL(0.25));
    else if (type == 3)
      famp *= FL(1.456) / POWER(harm, FL(1.0) / FL(160.0));
    else {
      harm = 1;
      famp *= FL(1.456);
    }
...
static int32_t pvsmoothset(CSOUND *csound, PVSMOOTH *p) {
  ...
  if (p->fin->sliding) {
    if (p->fout->frame.auxp == NULL ||
      ...
      csound->AuxAlloc(csound, (N + 2) * sizeof(MYFLT) * CS_KSMPS,
                       &p->fout->frame);
    if (p->del.auxp == NULL || p->del.size < sizeof(MYFLT) * CS_KSMPS * (N + 2))
      csound->AuxAlloc(csound, (N + 2) * sizeof(MYFLT) * CS_KSMPS, &p->del);
  } else {
    if (p->fout->frame.auxp == NULL ||
        p->fout->frame.size < sizeof(float) * (N + 2))
      csound->AuxAlloc(csound, (N + 2) * sizeof(float), &p->fout->frame);
...
static int32_t pvsmixset(CSOUND *csound, PVSMIX *p) {
  ...
  /* if (UNLIKELY(p->fa == p->fout || p->fb == p->fout))
     csound->Warning(csound, "%s", Str("Unsafe to have same fsig as in and
     out"));*/
  p->fout->sliding = 0;
  if (p->fa->sliding) {
    if (p->fout->frame.auxp == NULL ||
...
static int32_t pvsmix(CSOUND *csound, PVSMIX *p) {
  ...
  if (UNLIKELY(!fsigs_equal(p->fa, p->fb)))
    goto err1;
  if (p->fa->sliding) {
    CMPLX *fout, *fa, *fb;
    uint32_t offset = p->h.insdshead->ksmps_offset;
...
static int32_t pvsfilter(CSOUND *csound, PVSFILTER *p) {
  ...
  if (UNLIKELY(fout == NULL))
    goto err1;
  if (UNLIKELY(!fsigs_equal(p->fin, p->fil)))
    goto err2;
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (IS_ASIG_ARG(p->kdepth)) {
        kdepth = p->kdepth[n] >= FL(0.0)
                     ? (p->kdepth[n] <= FL(1.0) ? p->kdepth[n] * g : g)
                     : FL(0.0);
        dirgain = (FL(1.0) - kdepth) * g;
      }
      for (i = 0; i < NB; i++) {
...
static int32_t pvsscaleset(CSOUND *csound, PVSSCALE *p) {
  ...
  if (p->ftmp.auxp == NULL || p->ftmp.size < sizeof(float) * (N + 4))
  ...
  p->lastframe = 0;
  tmp = N + N % 2;
  if (p->ceps.auxp == NULL || p->ceps.size < sizeof(MYFLT) * (tmp + 2))
  ...
  if (p->fout->sliding) {
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      for (i = 1; i < NB - 1; i++) {
        ...
        /* Remove aliases */
        if (fout[i].im >= CS_ESR * 0.5 || fout[i].im <= -CS_ESR * 0.5)
          fout[i].re = 0.0;
      }
...
static int32_t pvsblurset(CSOUND *csound, PVSBLUR *p) {
  ...
  if (p->fin->sliding) {
    ...
    if (p->delframes.auxp == NULL ||
      ...
      csound->AuxAlloc(csound, (N + 2) * sizeof(MYFLT) * CS_KSMPS * delayframes,
                       &p->delframes);
  } else {
...
static int32_t fsigs_equal(const PVSDAT *f1, const PVSDAT *f2) {
  if ((f1->sliding == f2->sliding) && (f1->overlap == f2->overlap) &&
...
static int32_t pvsenvw(CSOUND *csound, PVSENVW *p) {
  ...
  if (ft == NULL) {
    csound->PerfError(csound, &(p->h), Str("could not find table number %d\n"),
                      (int32_t)*p->ftab);
    return NOTOK;
  }
  size = ft->flen;
...
static OENTRY localops[] = {
    ...
    {"pvsfilter", sizeof(PVSFILTER), 0, 3, "f", "fffp", (SUBR)pvsfilterset,
     (SUBR)pvsfilter},
    {"pvscale", sizeof(PVSSCALE), 0, 3, "f", "fkOPO", (SUBR)pvsscaleset,
     (SUBR)pvsscale},
    {"pvshift", sizeof(PVSSHIFT), 0, 3, "f", "fkkOPO", (SUBR)pvsshiftset,
     (SUBR)pvsshift},
    {"pvsmix", sizeof(PVSMIX), 0, 3, "f", "ff", (SUBR)pvsmixset, (SUBR)pvsmix,
     NULL},
    {"pvsfilter", sizeof(PVSFILTER), 0, 3, "f", "ffxp", (SUBR)pvsfilterset,
     (SUBR)pvsfilter},
    {"pvsblur", sizeof(PVSBLUR), 0, 3, "f", "fki", (SUBR)pvsblurset,
     (SUBR)pvsblur, NULL},
    ...
    {"pvsdiskin.i", sizeof(pvsdiskin), 0, 3, "f", "ikkopP", (SUBR)pvsdiskinset,
     (SUBR)pvsdiskinproc, NULL},
    {"pvstanal", sizeof(PVST1), 0, 3, "f", "kkkkPPoooP", (SUBR)pvstanalset1,
     (SUBR)pvstanal1, NULL},
    {"pvstanal", sizeof(PVST), 0, 3, "FFFFFFFFFFFFFFFF", "kkkkPPoooP",
     (SUBR)pvstanalset, (SUBR)pvstanal, NULL},
    {"pvswarp", sizeof(PVSWARP), 0, 3, "f", "fkkOPPO", (SUBR)pvswarpset,
     (SUBR)pvswarp},
    {"pvsenvftw", sizeof(PVSENVW), 0, 3, "k", "fkPPO", (SUBR)pvsenvwset,
     (SUBR)pvsenvw},
    {"pvsgain", sizeof(PVSGAIN), 0, 3, "f", "fk", (SUBR)pvsgainset,
     (SUBR)pvsgain, NULL},
    {"pvs2tab", sizeof(PVS2TAB_T), 0, 3, "k", "k[]f", (SUBR)pvs2tab_init,
     (SUBR)pvs2tab, NULL},
    ...
    {"pvs2array", sizeof(PVS2TAB_T), 0, 3, "k", "k[]f", (SUBR)pvs2tab_init,
     (SUBR)pvs2tab, NULL},
    {"pvs2array", sizeof(PVS2TABSPLIT_T), 0, 3, "k", "k[]k[]f",
     (SUBR)pvs2tabsplit_init, (SUBR)pvs2tabsplit, NULL},
    {"pvsfromarray", sizeof(TAB2PVS_T), 0, 3, "f", "k[]oop", (SUBR)tab2pvs_init,
     (SUBR)tab2pvs, NULL},
