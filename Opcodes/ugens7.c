...
#define kgliss ifmode
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), "%s", Str("fof: not initialised"));
err2:
  return csound->PerfError(csound, &(p->h), "%s",
                           Str("FOF needs more overlaps"));
}

static int32_t newpulse(CSOUND *csound, FOFS *p, OVRLAP *ovp, MYFLT *amp,
                        MYFLT *fund, MYFLT *form) {
...
static int32_t harmset(CSOUND *csound, HARMON *p) {
  ...
  printf("ekr = %f iptrk = %f, autocnt = %d; autotim = %d\n", CS_EKR,
         *p->iptrkprd, p->autokcnt, p->autoktim);
