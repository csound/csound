      ...
      while (j++ < n) {                                                        \
        *slnum = (unsigned char)*sld->ictlno;                                  \
        if (UNLIKELY(*slnum > 127)) {                                          \
          snprintf(sbuf, 120, Str("illegal control number at position n.%d"),  \
                   j);                                                         \
  ...
  {                                                                            \
    ...
    {                                                                          \
      ...
      while (j++ < 8) {                                                        \
        ...
        *yt1++ = FL(0.0);                                                      \
        b = (MYFLT)(2.0 -                                                      \
                    cos((double)(*(sld++)->ihp * CS_TPIDSR * CS_KSMPS)));      \
...
#define SLIDERF(p, n)                                                          \
  {                                                                            \
    ...
    while (j++ < n) {                                                          \
      ...
      min++;                                                                   \
      **result++ = *yt1 =                                                      \
          *c1++ * value + *c2++ * *yt1; /* filters the output */               \
      yt1++;                                                                   \
    }                                                                          \
    return OK;                                                                 \
  }
      ...
      while (j++ < n) {                                                        \
        *slnum_msb = (unsigned char)*sld->ictlno_msb;                          \
        if (UNLIKELY(*slnum_msb > 127)) {                                      \
          snprintf(sbuf, 120,                                                  \
                   Str("illegal msb control number at position n.%d"), j);     \
          return csound->InitError(csound, "%s", sbuf);                        \
        }                                                                      \
        *slnum_lsb = (unsigned char)*sld->ictlno_lsb;                          \
        if (UNLIKELY(*slnum_lsb > 127)) {                                      \
          snprintf(sbuf, 120,                                                  \
                   Str("illegal lsb control number at position n.%d"), j);     \
          return csound->InitError(csound, "%s", sbuf);                        \
        }                                                                      \
        if (UNLIKELY((value = *sld->initvalue) < (*min = *sld->imin) ||        \
                     value > (*max = *sld->imax))) {                           \
        ...
        if (*sld->ifn > 0)                                                     \
          ...
          *ftp++ = NULL;                                                       \
        intvalue =                                                             \
            (int32_t)(((*(sld++)->initvalue - *min) / (*max++ - *min)) *       \
                          f14bit +                                             \
                      FL(0.5));                                                \
        min++;                                                                 \
...
#define SLIDER14(p, n)                                                         \
  {                                                                            \
    ...
    while (j++ < n) {                                                          \
      value =                                                                  \
          (MYFLT)((chanblock[*slnum_msb++] * 128 + chanblock[*slnum_lsb++]) *  \
                  oneTOf14bit);                                                \
      if (*(++ftp)) { /* if valid ftable,use value as index   */               \
      ...
      while (j++ < n) {                                                        \
        ...
        if (UNLIKELY(slnum_lsb > 127)) {                                       \
          ...
          return csound->InitError(csound, "%s", sbuf);                        \
                                                                               ...
                                                                               \
        value = (MYFLT)((chanblock[slnum_msb] * 128 + chanblock[slnum_lsb]) *  \
                        oneTOf14bit);                                          \
...
static OENTRY localops[] = {
    ...
    {"slider8.k", S(SLIDER8), 0, 3, "kkkkkkkk",
     "iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii"
     "iiiiiiii",
     (SUBR)slider_i8, (SUBR)slider8, NULL},
    {"slider8f", S(SLIDER8f), 0, 3, "kkkkkkkk",
     "iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii"
     "iiiiiiiiiiii",
     (SUBR)slider_i8f, (SUBR)slider8f, NULL},
    {"slider8.i", S(SLIDER8), 0, 1, "iiiiiiii",
     "iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii", (SUBR)islider8, NULL, NULL},
