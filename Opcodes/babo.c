 ...
 * [2] D. Rocchesso and J.O. Smith "Circulant and Elliptic Feedback Delay
 *Networks for Artificial Reverberation", IEEE Trans. on Speech and Audio
 *Processing, vol. 5, n. 1, pp. 51-63, jan. 1997.
 *
...
static BaboMemory *BaboMemory_create(CSOUND *csound, BaboMemory *this,
...
static inline size_t BaboMemory_samples(const BaboMemory *this) {
  return this->samples;
}

static inline MYFLT *BaboMemory_start(const BaboMemory *this) {
  return (MYFLT *)this->memptr.auxp;
}

static inline MYFLT *BaboMemory_end(const BaboMemory *this) {
...
static void _Babo_common_delay_create(CSOUND *csound, BaboDelay *this,
                                      MYFLT max_time) {
  size_t num_floats =
...
static BaboDelay *BaboDelay_create(CSOUND *csound, BaboDelay *this,
                                   MYFLT max_time) {
...
static MYFLT BaboDelay_input(BaboDelay *this, MYFLT input) {
  *this->input++ = input;
...
static MYFLT BaboDelay_output(const BaboDelay *this) {
...
static BaboTapline *BaboTapline_create(CSOUND *csound, BaboTapline *this,
...
static inline MYFLT BaboTapline_maxtime(CSOUND *csound, BaboDelay *this) {
  return (((MYFLT)BaboMemory_samples(&this->core)) * (1. / this->sr));
}

static inline MYFLT BaboTapline_input(BaboTapline *this, MYFLT input) {
...
/* k-rate function */
static BaboTaplineParameters *BaboTapline_precalculate_parameters(
    CSOUND *csound, BaboTaplineParameters *results, MYFLT r_x, MYFLT r_y,
    MYFLT r_z,                       /* receiver position (i-rate) */
...
/* a-rate function */
static MYFLT BaboTapline_output(CSOUND *csound, const BaboTapline *this,
...
static MYFLT BaboTapline_output2(CSOUND *csound, const BaboTapline *this,
...
static BaboLowPass *BaboLowPass_create(BaboLowPass *this, MYFLT decay,
                                       MYFLT hidecay, MYFLT norm) {
...
static inline MYFLT BaboLowPass_input(BaboLowPass *this, MYFLT input) {
  this->z2 = this->z1;
  this->z1 = this->input;
  this->input = input;
  return input;
}
static inline MYFLT BaboLowPass_output(const BaboLowPass *this) {
  return (this->a0 * this->input) + (this->a1 * this->z1) +
         (this->a0 * this->z2);
}
/*
...
static BaboNode *BaboNode_create(CSOUND *csound, BaboNode *this, MYFLT time,
                                 MYFLT min_time, MYFLT decay, MYFLT hidecay) {
...
static inline MYFLT BaboNode_input(BaboNode *this, MYFLT input) {
  return BaboDelay_input(&this->delay, input);
}

static inline void BaboNode_feed_filter(BaboNode *this) {
  BaboLowPass_input(&this->filter, BaboDelay_output(&this->delay));
}

static inline MYFLT BaboNode_output(const BaboNode *this) {
...
static void BaboMatrix_create_FDN(BaboMatrix *this, MYFLT diffusion) {
  int32_t i, j;
  ...
  const MYFLT max_diffusion_eigenvalues[BABO_NODES] = {
      FL(3.142592), -FL(1.7370), -FL(2.1559), -FL(1.2566), -FL(2.9936),
      FL(1.0472),   -FL(2.5133), -FL(1.6140), FL(1.6140),  FL(2.5133),
      -FL(1.0472),  FL(2.9936),  FL(1.2566),  FL(2.1559),  FL(1.7370)};
  ...
  MYFLT real_X[BABO_NODES] = {FL(0.0)}, imaginary_X[BABO_NODES] = {FL(0.0)},
        arg_X[BABO_NODES] = {FL(0.0)}, real_x[BABO_NODES] = {FL(0.0)};
  ...
  for (i = 0; i < BABO_NODES; ++i)
    for (j = 0; j < BABO_NODES; ++j)
      real_x[j] += (real_X[j] * COS(_2PI_NODES * i * j) -
                    imaginary_X[j] * SIN(_2PI_NODES * i * j)) /
                   BABO_NODES;
...
static MYFLT BaboMatrix_calculate_delays(MYFLT delay_time[], MYFLT x, MYFLT y,
                                         MYFLT z) {
  int32_t i = 0;
  MYFLT min = FL(0.0);
  ...
  for (i = 0; i < BABO_NODES; ++i) {
    ...
    delay_time[i] =
        FL(2.0) / (sound_speed * SQRT(((dbdp->x / x) * (dbdp->x / x)) +
                                      ((dbdp->y / y) * (dbdp->y / y)) +
                                      ((dbdp->z / z) * (dbdp->z / z))));
...
static BaboMatrix *BaboMatrix_create(CSOUND *csound, BaboMatrix *this,
                                     MYFLT diffusion, MYFLT x, MYFLT y, MYFLT z,
                                     MYFLT decay, MYFLT hidecay,
                                     MYFLT early_diffusion) {
  int32_t i = 0;
  ...
  for (i = 0; i < BABO_NODES; ++i)
    BaboNode_create(csound, &this->node[i], delays[i], min_delay, decay,
                    hidecay);

  return this;
}

static inline MYFLT BaboMatrix_coefficient(const BaboMatrix *this, int32_t x,
                                           int32_t y) {
  return this->fdn[x][y];
}

/* a-rate function */
static void BaboMatrix_output(BaboMatrix *this, MYFLT outputs[], MYFLT input,
                              MYFLT diffusion_coeff) {
  MYFLT filter_tmpout[BABO_NODES] = {FL(0.0)}, tmp2[BABO_NODES] = {FL(0.0)};
  ...
  outputs[0] +=
      (diffusion_coeff *
       ((BaboNode_output(&this->node[7]) + BaboNode_output(&this->node[12]))));

  outputs[1] +=
      (diffusion_coeff *
       ((BaboNode_output(&this->node[9]) + BaboNode_output(&this->node[13]))));
...
static void set_defaults(CSOUND *csound, BABO *p) {
  resolve_defaults(p);

  p->diffusion_coeff = p->diffusion_coeff < 0 ? BABO_DEFAULT_DIFFUSION_COEFF
                                              : p->diffusion_coeff;
...
static inline MYFLT load_value_or_default(const FUNC *table, int32_t idx,
                                          MYFLT dEfault) {
...
static void set_expert_values(CSOUND *csound, BABO *p) {
  ...
  p->inter_receiver_distance =
      load_value_or_default(ftp, n++, BABO_DEFAULT_INTER_RECV_DISTANCE);
  p->direct = load_value_or_default(ftp, n++, BABO_DEFAULT_DIRECT);
  p->early_diffuse = load_value_or_default(ftp, n++, BABO_DEFAULT_DIFFUSE);
}

static void verify_coherence(CSOUND *csound, BABO *p) {
  if (UNLIKELY(*(p->lx) <= FL(0.0) || *(p->ly) <= FL(0.0) ||
               *(p->lz) <= FL(0.0))) {
    csound->Warning(csound,
                    Str("Babo: resonator dimensions are incorrect "
                        "(%.1f, %.1f, %.1f)"),
                    *(p->lx), *(p->ly), *(p->lz));
  }
}
...
static int32_t baboset(CSOUND *csound, void *entry) {
  ...
  BaboMatrix_create(csound, &p->matrix, p->diffusion_coeff, *(p->lx), *(p->ly),
                    *(p->lz), p->decay, p->hidecay, p->early_diffuse);
  return OK;
}

static int32_t babo(CSOUND *csound, void *entry) {
  BABO *p = (BABO *)entry;
  uint32_t offset = p->h.insdshead->ksmps_offset;
  uint32_t early = p->h.insdshead->ksmps_no_end;
  uint32_t n, nsmps = CS_KSMPS;
  MYFLT *outleft = p->outleft, *outright = p->outright, *input = p->input;

  BaboTaplineParameters left = {{FL(0.0)}, {{FL(0.0)}}},
                        right = {{FL(0.0)}, {{FL(0.0)}}};

  BaboTapline_precalculate_parameters(
      csound, &left, p->receiver_x - p->inter_receiver_distance, p->receiver_y,
      p->receiver_z, *(p->ksource_x), *(p->ksource_y), *(p->ksource_z),
      *(p->lx), *(p->ly), *(p->lz));

  BaboTapline_precalculate_parameters(
      csound, &right, p->receiver_x + p->inter_receiver_distance, p->receiver_y,
      p->receiver_z, *(p->ksource_x), *(p->ksource_y), *(p->ksource_z),
      *(p->lx), *(p->ly), *(p->lz));
  ...
  for (n = offset; n < nsmps; n++) { /* k-time cycle                */
    MYFLT left_tapline_out = FL(0.0), right_tapline_out = FL(0.0),
    ...
    left_tapline_out =
        BaboTapline_output(csound, &p->tapline, &left) * p->early_diffuse;

    right_tapline_out =
        BaboTapline_output(csound, &p->tapline, &right) * p->early_diffuse;
...
static int32_t babo2(CSOUND *csound, void *entry) {
  BABO *p = (BABO *)entry;
  uint32_t offset = p->h.insdshead->ksmps_offset;
  uint32_t early = p->h.insdshead->ksmps_no_end;
  uint32_t n, nsmps = CS_KSMPS;
  ...
  for (n = offset; n < nsmps; n++) { /* k-time cycle                */
    ...
    left_tapline_out =
        BaboTapline_output2(csound, &p->tapline, &left, p->direct) *
        p->early_diffuse;

    right_tapline_out =
        BaboTapline_output2(csound, &p->tapline, &right, p->direct) *
        p->early_diffuse;
