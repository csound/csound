...
int32_t pvspitch_process(CSOUND *csound, PVSPITCH *p) {
  ...
  if (p->lastframe < p->fin->framecount) {
    ...
    for (i = 1; i < (numBins - 1) && numPeaks < numBins / 2; i++) {
      ...
      if (Frame[2 * i] > Threshold && Frame[2 * i] > Frame[2 * (i - 1)] &&
    ...
    if (UNLIKELY(numPeaks == 0)) {
      ...
      for (i = 0; i < maxPartial && i < numBins / 2; i++) {
        ...
        if (i == 0 || (i > 0 && inharmonic[i] < inharmonic[Partial - 1]) ||
            (i > 0 && PrevNotAdj && Adj)) {
...
static OENTRY localops[] = {{"pvscent", sizeof(PVSCENT), 0, 3, "k", "f",
                            ...
                            {"centroid", sizeof(CENT), 0, 3, "k", "aki",
                             (SUBR)cent_i, (SUBR)cent_k, NULL},
                            {"pvspitch", sizeof(PVSPITCH), 0, 3, "kk", "fk",
                             (SUBR)pvspitch_init, (SUBR)pvspitch_process,
                             NULL}};
