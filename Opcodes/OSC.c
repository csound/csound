...
static int32_t osc_send_set(CSOUND *csound, OSCSEND *p) {
  ...
  /*     return csound->InitError(csound,"%s", Str("No a-rate arguments
   * allowed")); */
  /*   } */
  /* } */
...
#elif defined(MSVC)
        setsockopt((SOCKET)p->addr, IPPROTO_IP, IP_MULTICAST_TTL, &ttl,
                   sizeof(ttl));
#else
        setsockopt((uintptr_t)p->addr, IPPROTO_IP, IP_MULTICAST_TTL, &ttl,
                   sizeof(ttl));
#endif
      }
      csound->Free(csound, p->lhost);
    ...
    for (i = 0; type[i] != '\0'; i++) {
      ...
      case 'G': /* fGen Table/blob */
      {
        ...
        if (LIKELY((ftp = csound->FTnp2Find(csound, arg[i])) != NULL)) {
          ...
          data = csound->Malloc(csound, olen = /*sizeof(FUNC)-sizeof(MYFLT*)+*/
                                        sizeof(MYFLT) * len);
          // memcpy(data, ftp, sizeof(FUNC)-sizeof(MYFLT*));
          memcpy(data /*+sizeof(FUNC)-sizeof(MYFLT*)*/, ftp->ftable,
                 sizeof(MYFLT) * len);
        } else {
      ...
      case 'A': /* Array/blob */
      {
        ...
        ARRAYDAT *ss;
        /* make sure fn exists */
        if (LIKELY((ss = (ARRAYDAT *)arg[i]) != NULL && ss->data != NULL)) {
          int32_t j, d;
        ...
        // two parts needed
        {
          void *dd = csound->Malloc(csound, len + sizeof(int32_t) *
                                                      (1 + ss->dimensions));
          memcpy(dd, &ss->dimensions, sizeof(int32_t));
          memcpy((char *)dd + sizeof(int32_t), ss->sizes,
                 sizeof(int32_t) * ss->dimensions);
          memcpy((char *)dd + sizeof(int32_t) * (1 + ss->dimensions), ss->data,
                 len);
          /* printf("dd length = %d dimensions = %d, %d %d %.8x %.8x %.8x
           * %.8x\n", */
      ...
      case 'S':
        csound->Warning(csound, "S unimplemented");
        break;
        // #endif
      default:
...
static int32_t OSC_handler(const char *path, const char *types, lo_arg **argv,
                           int32_t argc, lo_message data, void *p) {
  IGN(argc);
  IGN(data);
    ...
    if (strcmp(o->saved_path, path) == 0 &&
      ...
      if (m != NULL) {
        ...
        for (i = 0; o->saved_types[i] != '\0'; i++) {
          ...
          case 'i':
            m->args[i].number = (MYFLT)argv[i]->i;
            break;
          case 'h':
            m->args[i].number = (MYFLT)argv[i]->i64;
            break;
          case 'c':
            m->args[i].number = (MYFLT)argv[i]->c;
            break;
          case 'f':
            m->args[i].number = (MYFLT)argv[i]->f;
            break;
          case 'd':
            m->args[i].number = (MYFLT)argv[i]->d;
            break;
          case 's': { // ***NO CHECK THAT m->args[i] IS A STRING
            char *src = (char *)&(argv[i]->s), *dst = m->args[i].string.data;
            if (m->args[i].string.size <= strlen(src)) {
              if (dst != NULL)
                csound->Free(csound, dst);
          ...
          case 'b': {
            int32_t len = lo_blobsize((lo_blob)argv[i]);
            m->args[i].blob = csound->Malloc(csound, len);
...
static int32_t osc_listener_initMulti(CSOUND *csound, OSCINITM *p) {
  ...
  snprintf(buff, 32, "%d", (int32_t) * (p->port));
  ports[n].thread =
      lo_server_thread_new_multicast(p->group->data, buff, OSC_error);
  if (UNLIKELY(ports[n].thread == NULL))
    return csound->InitError(
        csound, Str("cannot start OSC listener on port %s\n"), buff);
  /// if (lo_server_thread_start(ports[n].thread)<0)
  ...
  pp->nPorts = n + 1;
  csound->Warning(
      csound, Str("OSC multicast listener #%d started on port %s\n"), n, buff);
  *(p->ihandle) = (MYFLT)n;
  csound->RegisterDeinitCallback(csound, p,
                                 (int32_t(*)(CSOUND *, void *))OSC_deinit);
  return OK;
...
static int32_t OSC_list_init(CSOUND *csound, OSCLISTEN *p) {
  ...
  p->port = &(pp->ports[n]);
  p->c.saved_path =
      (char *)csound->Malloc(csound, strlen((char *)p->dest->data) + 1);
  ...
  p->c.method = lo_server_thread_add_method(
      p->port->thread, p->c.saved_path, p->c.saved_types, OSC_handler, p->port);
  csound->RegisterDeinitCallback(csound, p,
    ...
    for (i = 0; p->c.saved_types[i] != '\0'; i++) {
      ...
      if (p->c.saved_types[i] == 's') {
        ...
        if (c == 'D') {
          ...
          len /= sizeof(MYFLT);
          if (asize < len) {
            arr->data = (MYFLT *)csound->ReAlloc(csound, arr->data,
                                                 len * sizeof(MYFLT));
            asize = len;
          ...
          foo->sizes =
              (int32_t *)csound->Malloc(csound, sizeof(int32_t) * idata[0]);
#ifdef OSC_DEBUG
          printf("dimension=%d\n", idata[0]);
#endif
          for (j = 0; j < idata[0]; j++) {
...
#endif
          {
            ...
            printf("copy data: from %p to %p length %d %d\n", src, dst,
                   len - sizeof(FUNC) + sizeof(MYFLT *), data->flen);
            printf("was %f %f %f ...\n", dst[0], dst[1], dst[2]);
          ...
          return csound->PerfError(csound, &(p->h), "Oh dear");
...
static int32_t OSC_ahandler(const char *path, const char *types, lo_arg **argv,
                            int32_t argc, lo_message data, void *p) {
  IGN(argc);
  IGN(data);
  OSC_PORT *pp = (OSC_PORT *)p;
...
static int32_t OSC_alist_init(CSOUND *csound, OSCLISTENA *p) {
  ...
  csound->UnlockMutex(p->port->mutex_);
  p->c.method =
      lo_server_thread_add_method(p->port->thread, p->c.saved_path,
                                  p->c.saved_types, OSC_ahandler, p->port);
  csound->RegisterDeinitCallback(csound, p,
...
static OENTRY localops[] = {{"OSCsend_lo", S(OSCSEND), 0, 3, "", "kSkSN",
