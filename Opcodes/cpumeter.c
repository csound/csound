...
int32_t cpupercent_init(CSOUND *csound, CPUMETER *p) {
  ...
  num = sscanf(buf, "cpu %Lu %Lu %Lu %Lu %Lu %Lu %Lu %Lu", &u, &n, &s, &i, &w,
               &x, &y, &z);
  for (k = 0;; k++) {
    if (!fgets(buf, SMLBUFSIZ, p->fp))
      return csound->InitError(csound, Str("failed /proc/stat read"));
    num = sscanf(buf, "cpu%llu %Lu %Lu %Lu %Lu %Lu %Lu %Lu %Lu", &id, &u, &n,
                 &s, &i, &w, &x, &y, &z);
    if (num < 4)
      break;
  }
...
#define TRIMz(x) ((tz = (SIC_t)(x)) < 0 ? 0 : tz)
  ...
  k = p->cpu_max;
  if (!fgets(buf, SMLBUFSIZ, p->fp))
    return csound->PerfError(csound, &(p->h), Str("failed /proc/stat read"));
  /*num = */ sscanf(buf, "cpu %Lu %Lu %Lu %Lu %Lu %Lu %Lu %Lu", &cpu[k].u,
                    &cpu[k].n, &cpu[k].s, &cpu[k].i, &cpu[k].w, &cpu[k].x,
                    &cpu[k].y, &cpu[k].z);
  ...
  tot_frme =
      u_frme + s_frme + n_frme + i_frme + w_frme + x_frme + y_frme + z_frme;
  if (tot_frme < 1)
    tot_frme = 1;
  ...
  *p->k0 = 100.0 - (double)i_frme * scale;
  if (TEST)
    printf("**%5.2f %7.2f %7.2f %7.2f %7.2f %7.2f %7.2f %7.2f\n",
           (double)u_frme * scale, (double)s_frme * scale,
           (double)n_frme * scale, (double)i_frme * scale,
           (double)w_frme * scale, (double)x_frme * scale,
           (double)y_frme * scale, (double)z_frme * scale);
...
static int32_t systime(CSOUND *csound, SYST *p) {
  IGN(csound);
...
static OENTRY cpumeter_localops[] = {
    {"cpumeter", S(CPUMETER), 0, 3, "kzzzzzzzz", "i", (SUBR)cpupercent_init,
     (SUBR)cpupercent},
