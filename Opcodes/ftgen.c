...
static int32_t register_ftable_delete(CSOUND *csound, void *p,
                                      int32_t tableNum) {
...
static int32_t ftgen_(CSOUND *csound, FTGEN *p, int32_t istring1,
                      int32_t istring2) {
  MYFLT *fp;
  FUNC *ftp;
  EVTBLK *ftevt;
...
static int32_t ftload_(CSOUND *csound, FTLOAD *p, int32_t istring) {
  ...
  if (!istring) {
    if (csound->ISSTRCOD(*p->ifilno))
      ...
      strncpy(filename, csound->GetString(csound, *p->ifilno), MAXNAME);
  } else {
...
static OENTRY localops[] = {
    ...
    {"ftgentmp.iS", S(FTGEN), TW, 1, "i", "iiiiSm", (SUBR)ftgentmp_S, NULL,
     NULL},
    {"ftgentmp.Si", S(FTGEN), TW, 1, "i", "iiiSim", (SUBR)ftgentmp_Si, NULL,
     NULL},
    {"ftgentmp.SS", S(FTGEN), TW, 1, "i", "iiiSSm", (SUBR)ftgentmp_SS, NULL,
     NULL},
    ...
    {"ftloadk.S", S(FTLOAD_K), TW, 3, "", "Skim", (SUBR)ftsave_k_set,
     (SUBR)ftload_kS, NULL},
    {"getftargs", sizeof(FTARGS), 0, 3, "S", "ik", (SUBR)getftargs_init,
     (SUBR)getftargs_process}
