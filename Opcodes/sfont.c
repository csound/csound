...
static int SoundFontLoad(CSOUND *csound, char *fname) {
  ...
  fd = csound->FileOpen2(csound, &fil, CSFILE_STD, fname, "rb", "SFDIR;SSDIR",
                         CSFTYPE_SOUNDFONT, 0);
  if (UNLIKELY(fd == NULL)) {
#ifndef __wasi__
    csound->ErrorMsg(
        csound, Str("sfload: cannot open SoundFont file \"%s\" (error %s)"),
        fname, strerror(errno));
#else
    csound->ErrorMsg(csound, Str("sfload: cannot open SoundFont file \"%s\""),
                     fname);
#endif
    return -1;
  }
...
{ /* the handle is simply a stack index */
  ...
  if (istring)
    fname = csound->Strdup(csound, ((STRINGDAT *)p->fname)->data);
  else {
    if (csound->ISSTRCOD(*p->fname))
      fname = csound->Strdup(csound, csound->GetString(csound, *p->fname));
    else
      fname = csound->strarg2name(csound, NULL, p->fname, "sfont.", 0);
  }
  ...
  if (hand < 0) {
    *p->ihandle = (MYFLT)globals->currSFndx;
    ...
    if (UNLIKELY(++globals->currSFndx >= globals->maxSFndx)) {
      globals->maxSFndx += 5;
      globals->sfArray = (SFBANK *)csound->ReAlloc(
          csound, globals->sfArray,
...
static int32_t Sfplist(CSOUND *csound, SFPLIST *p) {
  ...
  for (j = 0; j < sf->presets_num; j++) {
    ...
    csound->Message(csound, Str("%3d) %-20s\tprog:%-3d bank:%d\n"), j,
                    filter_string(prs->name, temp_string), prs->prog,
                    prs->bank);
  }
...
static int32_t SfPreset(CSOUND *csound, SFPRESET *p) {
  ...
  if (UNLIKELY(presetHandle >= MAX_SFPRESET || presetHandle < 0)) {
    return csound->InitError(
        csound, Str("sfpreset: preset handle too big (%d), max: %d"),
    ...
    if (notnum >= layer->minNoteRange && notnum <= layer->maxNoteRange &&
      ...
      for (k = 0; k < splitsNum; k++) {
        ...
        if (notnum >= split->minNoteRange && notnum <= split->maxNoteRange &&
          ...
          if (orgkey == -1)
            orgkey = sample->byOriginalKey;
          orgfreq = globals->pitches[orgkey];
          if (flag) {
            freq = orgfreq * pow(2.0, ONETWELTH * tuneCorrection);
            p->si[spltNum] =
                (freq / (orgfreq * orgfreq)) * sample->dwSampleRate * CS_ONEDSR;
          } else {
            freq = orgfreq * pow(2.0, ONETWELTH * tuneCorrection) *
                   pow(2.0,
                       ONETWELTH * (split->scaleTuning * 0.01) * (nm - orgkey));
            p->si[spltNum] =
                (freq / orgfreq) * sample->dwSampleRate * CS_ONEDSR;
          };
          attenuation =
              (MYFLT)(layer->initialAttenuation + split->initialAttenuation);
          ...
          if (*p->ienv > 1) {
            ...
            p->attr[spltNum] = 1.0 / (CS_EKR * split->attack);
            p->decr[spltNum] = (split->sustain - 1.0) / (CS_EKR * split->decay);
            if (split->attack != 0.0)
              p->env[spltNum] = 0.0;
            else
              p->env[spltNum] = 1.0;
          ...
          p->ti[spltNum] = 0;
          /*csound->Message(csound, "play: split %d, samplebase:%p freq: %f
             orig: %f"
                                   "\n\t atten:%f pan:%f mode:%d \n",
                          k, p->base[spltNum],
                          freq, orgfreq,  attenuation, pan,
             split->sampleModes);*/
          spltNum++;
        }
      }
      ...
      for (k = 0; k < splitsNum; k++) {
        ...
        if (notnum >= split->minNoteRange && notnum <= split->maxNoteRange &&
          ...
          if (flag) {
            ...
            freq = orgfreq * pow(2.0, ONETWELTH * tuneCorrection) *
                   pow(2.0,
                       ONETWELTH * (split->scaleTuning * 0.01) * (nn - orgkey));
            p->si[spltNum] =
                (freq / orgfreq) * sample->dwSampleRate * CS_ONEDSR;
          }
          p->attenuation[spltNum] =
              POWER(FL(2.0),
                    (-FL(1.0) / FL(60.0)) * (layer->initialAttenuation +
    ...
    for (k = 0; k < splitsNum; k++) {
      ...
      if (notnum >= split->minNoteRange && notnum <= split->maxNoteRange &&
        ...
        if (flag) {
          ...
          freq =
              orgfreq * pow(2.0, ONETWELTH * tuneCorrection) *
  ...
  if (UNLIKELY(*p->instrNum > sf->instrs_num)) {
    ...
    for (k = 0; k < splitsNum; k++) {
      ...
      if (notnum >= split->minNoteRange && notnum <= split->maxNoteRange &&
        ...
        p->attenuation[spltNum] =
            (MYFLT)pow(2.0, (-1.0 / 60.0) * split->initialAttenuation) *
            GLOBAL_ATTENUATION;
        p->base[spltNum] = sBase + start;
        p->phs[spltNum] = (double)split->startOffset + *p->ioffset;
        p->end[spltNum] = sample->dwEnd + split->endOffset - start;
 ...
 *  fmt: ((b|w|d)[0-9]*)+
 *  b:byte (no conversion), w:word, d:double word, digits(optional):repeat n
 * times
 */
...
#define UNUSE 0x7fffffff
          ...
          int32_t Gpan = UNUSE, GinitialAttenuation = UNUSE,
                  GscaleTuning = UNUSE;
          ...
          layer->split = (splitType *)csound->Malloc(
              csound, split_num * sizeof(splitType));
          ...
          for (l = 0, ll = 0; l < ibag_num; l++) {
            ...
            if (sglobal_zone) {
              ...
              split->attack = split->decay = split->sustain = split->release =
                  FL(0.0);
              ...
              for (m = mStart; m < mEnd; m++) {
                ...
                case coarseTune:
                  split->coarseTune =
                      (/*char*/ SBYTE)igen[m].genAmount.shAmount;
                  break;
                case fineTune:
                ...
                case attackVolEnv: /*attack */
                  split->attack =
                      POWER(FL(2.0), igen[m].genAmount.shAmount / FL(1200.0));
                ...
                case sustainVolEnv: /*sustain */
                  split->sustain =
                      POWER(FL(10.0), -igen[m].genAmount.shAmount / FL(20.0));
...
static void fill_SfPointers(CSOUND *csound) {
  ...
  CHUNK *main_chunk;
  CHUNK *smplChunk = NULL, *phdrChunk = NULL, *pbagChunk = NULL,
        *pmodChunk = NULL;
  CHUNK *pgenChunk = NULL, *instChunk = NULL, *ibagChunk = NULL,
        *imodChunk = NULL;
      ...
      if (chkid == s2d("INFO")) {
        ...
        do {
          ...
          if (chkid == s2d("phdr")) {
            ...
            /* #ifdef BETA */
            /*               csound->Message(csound, "Unknown sfont
             * %.4s(%.8x)\n", */
            /*                                       (char*) &chkid, (uint32_t)
             * chkid); */
            /* #endif */
      ...
      for (k = 0; k < splitsNum; k++) {
        ...
        if (notnum >= split->minNoteRange && notnum <= split->maxNoteRange &&
          ...
          if (*p->iflag) {
            ...
            freq = orgfreq * pow(2.0, ONETWELTH * tuneCorrection) *
                   pow(2.0, ONETWELTH * (split->scaleTuning * 0.01) *
                                (notnum - orgkey));
            p->freq[spltNum] =
                (freq / orgfreq) * sample->dwSampleRate * CS_ONEDSR;
          }
  ...
  for (k = 0; k < spltNum; k++) {
    ...
    if (firsttime[k]) {
      ...
      if (loop_start[k] > send) {
        csound->Warning(csound,
                        "loop start %f beyond sample end %f, clamping.\n",
                        (loop_start[k] - sstart) / sr, (send - sstart) / sr);
        loop_start[k] = send;
      }
      if (loop_end[k] > send) {
        csound->Warning(csound, "loop end %f beyond sample end %f, clamping.\n",
                        (loop_end[k] - sstart) / sr, (send - sstart) / sr);
        loop_end[k] = send;
      }
    ...
    for (i = offset; i < nsmps; i++) {
      if (mode == 1) { /* backwards */
        ...
        if (ndx[0] > crossfade + loop_start[k])
          ...
          out =
              amp *
              (fadeout * (tab[tndx0] + frac0 * (tab[tndx0 + 1] - tab[tndx0])) +
        ...
        if (ndx[0] <= loop_start[k]) {
          ...
          if (loop_start[k] > send) {
            csound->Warning(
                csound, "loop start %f beyond sample end %f, clamping.\n",
                (loop_start[k] - sstart) / sr, (send - sstart) / sr);
            loop_start[k] = send;
          }
          if (loop_end[k] > send) {
            csound->Warning(csound,
                            "loop end %f beyond sample end %f, clamping.\n",
                            (loop_end[k] - sstart) / sr, (send - sstart) / sr);
            loop_end[k] = send;
          }
          loopsize = loop_end[k] - loop_start[k];
        ...
        if (init && ndx[0] < loop_start[k] + crossfade) {
          ...
          frac0 = ndx[0] - tndx0;
          out += amp * fadein *
                 (tab[tndx0] + frac0 * (tab[tndx0 + 1] - tab[tndx0]));
          ndx[0] += pitch;
          count += pitch;
          ...
          if (etab == NULL)
            ...
            fadeout = etab[(int32_t)(elen * (FL(1.0) - count / crossfade))];
          tndx0 = (int32_t)ndx[0];
          frac0 = ndx[0] - tndx0;
          out += amp * fadeout *
                 (tab[tndx0] + frac0 * (tab[tndx0 + 1] - tab[tndx0]));
        ...
        if (ndx[1] > loop_end[k] - crossfade) {
          ...
          frac1 = ndx[1] - tndx1;
          out += amp * fadein *
                 (tab[tndx1] + frac1 * (tab[tndx1 + 1] - tab[tndx1]));
          ndx[1] -= pitch;
          ...
          if (ndx[1] <= loop_start[k]) {
            ...
            if (loop_end[k] > send) {
              csound->Warning(
                  csound, "loop end %f beyond sample end %f, clamping.\n",
                  (loop_end[k] - sstart) / sr, (send - sstart) / sr);
              loop_end[k] = send;
            }
        ...
        if (ndx[0] >= loop_end[k]) {
          ...
          crossfade = (int32_t)(*p->crossfade * sr);
          p->cfade = crossfade =
              crossfade > loopsize ? loopsize - 1 : crossfade;
          ndx[0] = ndx[1];
          ndx[1] = (double)loop_start[k];
          count = 0;
        }
...
static OENTRY localops[] = {
    ...
    {"sfplay", S(SFPLAY), 0, 3, "aa", "iixxioooo", (SUBR)SfPlay_set,
     (SUBR)SfPlay},
    {"sfplaym", S(SFPLAYMONO), 0, 3, "a", "iixxioooo", (SUBR)SfPlayMono_set,
     (SUBR)SfPlayMono},
    ...
    {"sfinstr", S(SFIPLAY), 0, 3, "aa", "iixxiioooo", (SUBR)SfInstrPlay_set,
     (SUBR)SfInstrPlay},
    {"sfplay3", S(SFPLAY), 0, 3, "aa", "iixxioooo", (SUBR)SfPlay_set,
     (SUBR)SfPlay3},
    {"sfplay3m", S(SFPLAYMONO), 0, 3, "a", "iixxioooo", (SUBR)SfPlayMono_set,
     (SUBR)SfPlayMono3},
    {"sfinstr3", S(SFIPLAY), 0, 3, "aa", "iixxiioooo", (SUBR)SfInstrPlay_set,
     (SUBR)SfInstrPlay3},
    {"sfinstr3m", S(SFIPLAYMONO), 0, 3, "a", "iixxiioooo",
     (SUBR)SfInstrPlayMono_set, (SUBR)SfInstrPlayMono3},
    {"sflooper", S(sflooper), 0, 3, "aa", "iikkikkkooooo", (SUBR)sflooper_init,
     (SUBR)sflooper_process},
    {NULL, 0, 0, 0, NULL, NULL, (SUBR)NULL, (SUBR)NULL, (SUBR)NULL}};

int32_t sfont_ModuleCreate(CSOUND *csound) {
  int32_t j;
  sfontg *globals;
  csound->CreateGlobalVariable(csound, "::sfontg", sizeof(sfontg));
  globals = (sfontg *)(csound->QueryGlobalVariable(csound, "::sfontg"));
  if (globals == NULL)
    return csound->InitError(csound, "%s",
  ...
  globals->sfArray =
      (SFBANK *)csound->Calloc(csound, MAX_SFONT * sizeof(SFBANK));
  globals->presetp = (presetType **)csound->Calloc(
      csound, MAX_SFPRESET * sizeof(presetType *));
  globals->sampleBase =
  ...
  for (j = 0; j < 128; j++) {
    globals->pitches[j] =
        (MYFLT)(csound->GetA4(csound) * pow(2.0, (double)(j - 69) / 12.0));
  }
