...
int32_t tblesegset(CSOUND *csound, TABLESEG *p) {
  ...
  p->outfunc = (FUNC *)csound->Calloc(csound, sizeof(FUNC));
  p->outfunc->ftable =
  ...
  // memset(p->outfunc->ftable, 0, sizeof(MYFLT)*(flength+1)); not needed --
  // calloc
  if (**argp <= 0.0)
...
int32_t vpvset_(CSOUND *csound, VPVOC *p, int32_t stringname) {
  ...
  for (i = 0; i < OPWLEN / 2 + 1; ++i) /* time window is OPWLEN long */
    p->window[i] =
        (FL(0.5) - FL(0.5) * COS(TWOPI_F * (MYFLT)i / (MYFLT)OPWLEN));
  /* NB: HANNING */
