...
int lowpr_setx(LOWPRX *p)
{
    int j;
    if ((p->loop = (int) (*p->ord + FL(0.5))) < 1) p->loop = 4; /*default value*/
    else if (p->loop > 10) {
      return initerror(Str(X_878,"illegal order num. (min 1, max 10)"));
    }
    if (*p->istor!= 0)
...
int lowpr_w_sep_set(LOWPR_SEP *p)
{
    int j;
