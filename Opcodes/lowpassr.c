...
static int32_t lowpr(CSOUND *csound, LOWPR *p) {
  ...
  if (p->okf != kfco || p->okr != kres) { /* Only if changed */
    ...
    p->okf = kfco;
    p->okr = kres; /* remember to save recalculation */
  }
  ar = p->ar;
  asig = p->asig;
  ynm1 = p->ynm1;
...
static int32_t lowprak(CSOUND *csound, LOWPR *p) {
  ...
  if (okf != fco[0] || okr != kres) { /* Only if changed */
    ...
    p->coef2 = coef2 = 1.0 / (1.0 + b + k);
    okf = fco[0];
    okr = kres; /* remember to save recalculation */
  }
  ar = p->ar;
  asig = p->asig;
  ynm1 = p->ynm1;
...
static int32_t lowprka(CSOUND *csound, LOWPR *p) {
  ...
  if (okf != fco || okr != res[0]) { /* Only if changed */
    ...
    p->coef2 = coef2 = 1.0 / (1.0 + b + k);
    okf = fco;
    okr = res[0]; /* remember to save recalculation */
  }
  ar = p->ar;
  asig = p->asig;
  ynm1 = p->ynm1;
  ...
  for (j = 0; j < p->loop; j++) {
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (p->okf != fco || p->okr != res) { /* Only if changed */
        ...
        p->okf = fco;
        p->okr = res; /* remember to save recalculation */
      }
...
static OENTRY localops[] = {
    ...
    {"lowresx.kk", S(LOWPRX), 0, 3, "a", "akkoo", (SUBR)lowpr_setx,
     (SUBR)lowprx},
    {"lowresx.ak", S(LOWPRX), 0, 3, "a", "aakoo", (SUBR)lowpr_setx,
     (SUBR)lowprx},
    {"lowresx.ka", S(LOWPRX), 0, 3, "a", "akaoo", (SUBR)lowpr_setx,
     (SUBR)lowprx},
    {"lowresx.aa", S(LOWPRX), 0, 3, "a", "aaaoo", (SUBR)lowpr_setx,
     (SUBR)lowprx},

    {"vlowres", S(LOWPR_SEP), 0, 3, "a", "akkik", (SUBR)lowpr_w_sep_set,
     (SUBR)lowpr_w_sep}};
