...
static int32_t agsset(CSOUND *csound, PGRA *p) /*      Granular U.G. set-up */
{
  ...
  bufsize =
      sizeof(MYFLT) * (2L * (size_t)(CS_ESR * *p->imkglen) + (3L * CS_KSMPS));

  if (p->aux.auxp == NULL || (uint32_t)bufsize > p->aux.size)
    csound->AuxAlloc(csound, bufsize, &p->aux);
  else
    memset(p->aux.auxp, '\0', bufsize); /* Clear any old data */
...
static int32_t ags(CSOUND *csound,
                   PGRA *p) /*  Granular U.G. a-rate main routine */
{
  FUNC *gtp, *etp;
  ...
  for (i = offset; i < nsmps; i++) {
    if (gcount >= FL(1.0)) { /* I wonder..... */
      ...
      if (pow2tab) {
        ...
        do {
          *temp++ += amp * *(gtbl + (isc >> lb)) * *(etbl + (isc2 >> lb2));
          isc = (isc + inc) & PHMASK;
