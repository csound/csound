...
/* Scanned Synthesis Opcodes:
   An extended system from an algorithm by Bill Verplank, Max Mathews and Rob
   Shaw
...
static int32_t scsnux_init_(CSOUND *csound, PSCSNUX *p, int32_t istring) {
  ...
  if (!istring) {
    ...
    csound->AuxAlloc(csound, 1L + (len * len * sizeof(int32)) / BITS_PER_UNIT,
                     &p->aux_f);
    ...
    char filnam[256];
    MEMFIL *mfp;
    strncpy(filnam, ((STRINGDAT *)p->i_f)->data, 255);
    filnam[255] = '\0';
    /* readfile if reqd */
    if (UNLIKELY((mfp = csound->ldmemfile2withCB(
                      csound, filnam, CSFTYPE_XSCANU_MATRIX, NULL)) == NULL)) {
...
#else
      csound->AuxAlloc(
          csound, BITS_PER_UNIT + (len * len * sizeof(int32)) / BITS_PER_UNIT,
          &p->aux_f);
      p->f = (uint32 *)p->aux_f.auxp;
#endif
      while (pp < mfp->endp) {
        if (strncmp(pp, NMATRIXLF, NMATLENLF) == 0)
          break;
        if (strncmp(pp, NMATRIXCRLF, NMATLENCRLF) == 0)
          break;
        if (1 != sscanf(pp, "%d", &i))
          break;
        if (1 != sscanf(pp, "%d", &j))
...
/* Make buffers to hold data */
  ...
  p->id = (int32_t)*p->i_id;
  if (p->id < 0) {
    if (UNLIKELY(csound->GetTable(csound, &(p->out), -(p->id)) <
                 (int32_t)len)) {
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    if (idx >= rate) {
      ...
      for (i = 0; i != len; i++) {
        ...
        a += -x1[i] * p->c[i] * *p->k_c - (x2[i] - x1[i]) * p->d[i] * *p->k_d;
        a /= p->m[i] * *p->k_m;
...
#define PINTERP(ii, x)                                                         \
  (pp->x1[p->t[ii]] +                                                          \
   x * ((pp->x2[p->t[ii]] - pp->x3[p->t[ii]]) * FL(0.5) +                      \
        x * ((pp->x3[p->t[ii]] + pp->x2[p->t[ii]]) * FL(0.5) -                 \
             pp->x1[p->t[ii]])))
#else
#define PINTERP(ii, x)                                                         \
...
static int32_t scsnsx_init(CSOUND *csound, PSCSNSX *p) {
  ...
  /* Get trajectory matrix */
  {
    ...
    if (oscil_interp < 1 || oscil_interp > 4)
      oscil_interp = 4;
...
static OENTRY localops[] = {
    {"xscanu", S(PSCSNUX), _QQ | TR, 3, "", "iiiiSiikkkkiikkaii",
     (SUBR)scsnux_init_S, (SUBR)scsnux},
    {"xscanu", S(PSCSNUX), _QQ | TR, 3, "", "iiiiiiikkkkiikkaii",
     (SUBR)scsnux_init, (SUBR)scsnux},
