...
static int32_t pvsbufferset(CSOUND *csound, PVSBUFFER *p) {
  ...
  while ((phandle = (FSIG_HANDLE **)csound->QueryGlobalVariable(
              csound, varname)) != NULL)
    if (p->handle == *phandle)
      break;
    else
      snprintf(varname, 32, "::buffer%d", ++i);
...
static int32_t pvsbufreadproc(CSOUND *csound, PVSBUFFERREAD *p) {
  ...
  fout = (float *)p->fout->frame.auxp, buffer = handle->data;
  N = p->fout->N;
...
static OENTRY pvsbuffer_localops[] = {
    {"pvsbuffer", S(PVSBUFFER), 0, 3, "ik", "fi", (SUBR)pvsbufferset,
     (SUBR)pvsbufferproc, NULL},
    {"pvsbufread", S(PVSBUFFERREAD), 0, 3, "f", "kkOOo", (SUBR)pvsbufreadset,
     (SUBR)pvsbufreadproc, NULL},
    {"pvsbufread2", S(PVSBUFFERREAD), 0, 3, "f", "kkkk", (SUBR)pvsbufreadset,
     (SUBR)pvsbufreadproc2, NULL}};

int32_t pvsbuffer_localops_init_(CSOUND *csound) {
  return csound->AppendOpcodes(
      csound, &(pvsbuffer_localops[0]),
