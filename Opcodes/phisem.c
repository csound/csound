...
my_random(CSOUND *csound,
          int32_t max) { /* Return Random Int Between 0 and max */
  return (csound->Rand31(csound->RandSeed1(csound)) % (max + 1));
}

static MYFLT noise_tick(
    CSOUND *csound) { /* Return random MYFLT float between -1.0 and 1.0 */
  MYFLT rnd =
      (MYFLT)csound->Rand31(csound->RandSeed1(csound)) - FL(1073741823.5);
...
static int32_t cabasaset(CSOUND *csound, CABASA *p) {
  ...
  if (p->shakeEnergy > MAX_SHAKE)
    p->shakeEnergy = MAX_SHAKE;
  p->last_num = FL(0.0);
  return OK;
}
    ...
    if ((int32)(*p->num_beads + FL(0.5)) != p->num_objects) {
      ...
      if (p->num_objects >= 1) {
        gain = p->gain = LOG((MYFLT)p->num_objects) /
                         FL(1.38629436111989061883) /* (MYFLT)log(4.0)*/ *
                         FL(40.0) / (MYFLT)p->num_objects;
      }
    }
  }
...
static int32_t sekereset(CSOUND *csound, SEKERE *p) {
  ...
  p->coeffs0 = -SEKE_RESON * FL(2.0) * COS(SEKE_CENTER_FREQ * CS_TPIDSR);
  /* Note On */
  p->shakeEnergy = *p->amp * MAX_SHAKE * FL(0.1);
  if (p->shakeEnergy > MAX_SHAKE)
    p->shakeEnergy = MAX_SHAKE;
...
static int32_t sandset(CSOUND *csound, SEKERE *p) {
  ...
  p->gain = LOG(SANDPAPR_NUM_GRAINS) * SANDPAPR_GAIN / SANDPAPR_NUM_GRAINS;
  p->resons = SANDPAPR_RESON;
  p->coeffs1 = SANDPAPR_RESON * SANDPAPR_RESON;
  p->coeffs0 =
      -SANDPAPR_RESON * FL(2.0) * COS(SANDPAPR_CENTER_FREQ * CS_TPIDSR);
  /* Note On */
  p->shakeEnergy =
      *p->amp * (FL(1.) / csound->Get0dBFS(csound)) * MAX_SHAKE * FL(0.1);
  if (p->shakeEnergy > MAX_SHAKE)
    p->shakeEnergy = MAX_SHAKE;
  p->last_num = FL(128.0);
  return OK;
...
static int32_t stixset(CSOUND *csound, SEKERE *p) {
  ...
  p->coeffs0 = -STIX1_RESON * FL(2.0) * COS(STIX1_CENTER_FREQ * CS_TPIDSR);
  /* Note On */
  p->shakeEnergy =
      *p->amp * (FL(1.) / csound->Get0dBFS(csound)) * MAX_SHAKE * FL(0.1);
  if (p->shakeEnergy > MAX_SHAKE)
...
static int32_t crunchset(CSOUND *csound, CABASA *p) {
  ...
  p->gain =
      LOG((MYFLT)CRUNCH1_NUM_BEADS) * CRUNCH1_GAIN / (MYFLT)CRUNCH1_NUM_BEADS;
  p->resons = CRUNCH1_RESON;
  p->coeffs1 = CRUNCH1_RESON * CRUNCH1_RESON;
  p->coeffs0 = -CRUNCH1_RESON * FL(2.0) * COS(CRUNCH1_CENTER_FREQ * CS_TPIDSR);
  /* Note On */
  p->shakeEnergy =
      *p->amp * (FL(1.) / csound->Get0dBFS(csound)) * MAX_SHAKE * FL(0.1);
  if (p->shakeEnergy > MAX_SHAKE)
...
static int32_t guiroset(CSOUND *csound, GUIRO *p) {
  ...
  temp = LOG((MYFLT)GUIR_NUM_PARTS) * GUIR_GAIN / (MYFLT)GUIR_NUM_PARTS;
  p->gains0 = temp;
  p->gains1 = temp;

  p->coeffs01 = GUIR_GOURD_RESON * GUIR_GOURD_RESON;
  p->coeffs00 = -GUIR_GOURD_RESON * FL(2.0) * COS(GUIR_GOURD_FREQ * CS_TPIDSR);

  p->coeffs11 = GUIR_GOURD_RESON2 * GUIR_GOURD_RESON2;
  p->coeffs10 =
      -GUIR_GOURD_RESON2 * FL(2.0) * COS(GUIR_GOURD_FREQ2 * CS_TPIDSR);
...
static int32_t guiro(CSOUND *csound, GUIRO *p) {
  ...
  if (*p->num_teeth != FL(0.0) &&
    ...
    if (p->num_objects < FL(1.0))
      p->num_objects = FL(1.0);
    p->gains0 = p->gains1 =
        LOG((MYFLT)p->num_objects) * GUIR_GAIN / (MYFLT)p->num_objects;
  }
  ...
  if (*p->freq != FL(0.0) && *p->freq != p->res_freqSave) {
    ...
    p->coeffs00 =
        -GUIR_GOURD_RESON * FL(2.0) * COS(p->res_freqSave * CS_TPIDSR);
  }
  if (*p->freq2 != p->res_freq2) {
    p->res_freq2 = *p->freq2;
    p->coeffs10 = -GUIR_GOURD_RESON2 * FL(2.0) * COS(p->res_freq2 * CS_TPIDSR);
  }
  if (p->kloop > 0 && p->h.insdshead->relesing)
    p->kloop = 1;
  if ((--p->kloop) == 0) {
...
static int32_t tambourset(CSOUND *csound, TAMBOURINE *p) {
  ...
  temp = LOG((MYFLT)TAMB_NUM_TIMBRELS) * TAMB_GAIN / (MYFLT)TAMB_NUM_TIMBRELS;
  p->gains0 = temp * TAMB_SHELL_GAIN;
  p->gains1 = temp * FL(0.8);
  p->gains2 = temp;
  p->coeffs01 = TAMB_SHELL_RESON * TAMB_SHELL_RESON;
  p->coeffs00 = -TAMB_SHELL_RESON * FL(2.0) * COS(TAMB_SHELL_FREQ * CS_TPIDSR);
  p->coeffs11 = TAMB_CYMB_RESON * TAMB_CYMB_RESON;
  p->coeffs10 = -TAMB_CYMB_RESON * FL(2.0) * COS(TAMB_CYMB_FREQ1 * CS_TPIDSR);
  p->coeffs21 = TAMB_CYMB_RESON * TAMB_CYMB_RESON;
  p->coeffs20 = -TAMB_CYMB_RESON * FL(2.0) * COS(TAMB_CYMB_FREQ2 * CS_TPIDSR);
  /* Note On */
  p->shakeEnergy =
      *p->amp * (FL(1.) / csound->Get0dBFS(csound)) * MAX_SHAKE * FL(0.1);
  p->shake_damp = FL(0.0);
...
static int32_t tambourine(CSOUND *csound, TAMBOURINE *p) {
  ...
  if (*p->freq != FL(0.0) && *p->freq != p->res_freq) {
    p->res_freq = *p->freq;
    p->coeffs00 = -TAMB_SHELL_RESON * FL(2.0) * COS(p->res_freq * CS_TPIDSR);
  }
  if (*p->damp != FL(0.0) && *p->damp != p->shake_damp) {
    p->shake_damp = *p->damp;
    p->systemDecay = TAMB_SYSTEM_DECAY + (p->shake_damp * FL(0.002));
  ...
  if (*p->freq1 != FL(0.0) && *p->freq1 != p->res_freq1) {
    p->res_freq1 = *p->freq1;
    p->coeffs10 = -TAMB_CYMB_RESON * FL(2.0) * COS(p->res_freq1 * CS_TPIDSR);
  }
  if (*p->freq2 != FL(0.0) && *p->freq2 != p->res_freq2) {
    p->res_freq2 = *p->freq2;
    p->coeffs20 = -TAMB_CYMB_RESON * FL(2.0) * COS(p->res_freq2 * CS_TPIDSR);
  }
  if (p->kloop > 0 && p->h.insdshead->relesing)
    p->kloop = 1;
  if ((--p->kloop) == 0) {
...

  {
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (my_random(csound, 1024) < p->num_objects) {
        ...
        p->coeffs10 = -TAMB_CYMB_RESON * FL(2.0) * COS(temp_rand * CS_TPIDSR);
        temp_rand = p->res_freq2 * (FL(1.0) + (FL(0.05) * noise_tick(csound)));
        p->coeffs20 = -TAMB_CYMB_RESON * FL(2.0) * COS(temp_rand * CS_TPIDSR);
      }
...
static int32_t bambooset(CSOUND *csound, BAMBOO *p) {
  ...
  temp = LOG((MYFLT)BAMB_NUM_TUBES) * BAMB_GAIN / (MYFLT)BAMB_NUM_TUBES;
  p->gain = temp;
  p->coeffs01 = BAMB_RESON * BAMB_RESON;
  p->coeffs00 = -BAMB_RESON * FL(2.0) * COS(BAMB_CENTER_FREQ0 * CS_TPIDSR);
  p->coeffs11 = BAMB_RESON * BAMB_RESON;
  p->coeffs10 = -BAMB_RESON * FL(2.0) * COS(BAMB_CENTER_FREQ1 * CS_TPIDSR);
  p->coeffs21 = BAMB_RESON * BAMB_RESON;
  p->coeffs20 = -BAMB_RESON * FL(2.0) * COS(BAMB_CENTER_FREQ2 * CS_TPIDSR);
  /* Note On */
  p->shakeEnergy =
      *p->amp * (FL(1.) / csound->Get0dBFS(csound)) * MAX_SHAKE * FL(0.1);
  p->shake_damp = FL(0.0);
...
static int32_t bamboo(CSOUND *csound, BAMBOO *p) {
  ...
  if (*p->freq != FL(0.0) && *p->freq != p->res_freq0) {
    p->res_freq0 = *p->freq;
    p->coeffs00 = -BAMB_RESON * FL(2.0) * COS(p->res_freq0 * CS_TPIDSR);
  }
  if (*p->damp != FL(0.0) && *p->damp != p->shake_damp) {
    p->shake_damp = *p->damp;
  ...
  if (*p->freq1 != FL(0.0) && *p->freq1 != p->res_freq1) {
    p->res_freq1 = *p->freq1;
    p->coeffs10 = -BAMB_RESON * FL(2.0) * COS(p->res_freq1 * CS_TPIDSR);
  }
  if (*p->freq2 != FL(0.0) && *p->freq2 != p->res_freq2) {
    p->res_freq2 = *p->freq2;
    p->coeffs20 = -BAMB_RESON * FL(2.0) * COS(p->res_freq2 * CS_TPIDSR);
  }
  if (p->kloop > 0 && p->h.insdshead->relesing)
    p->kloop = 1;
  if ((--p->kloop) == 0) {
...

  {
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (my_random(csound, 1024) < p->num_objects) {
        ...
        p->coeffs00 = -BAMB_RESON * FL(2.0) * COS(temp_rand * CS_TPIDSR);
        temp_rand = p->res_freq1 * (FL(1.0) + (FL(0.2) * noise_tick(csound)));
        p->coeffs10 = -BAMB_RESON * FL(2.0) * COS(temp_rand * CS_TPIDSR);
        temp_rand = p->res_freq2 * (FL(1.0) + (FL(0.2) * noise_tick(csound)));
        p->coeffs20 = -BAMB_RESON * FL(2.0) * COS(temp_rand * CS_TPIDSR);
      }
      inputs0 = sndLevel * noise_tick(csound); /* Actual Sound is Random */
      inputs1 = inputs0;
      inputs2 = inputs0;
...
static int32_t wuterset(CSOUND *csound, WUTER *p) {
  ...
  p->coeffs00 = -WUTR_RESON * FL(2.0) * COS(WUTR_CENTER_FREQ0 * CS_TPIDSR);
  p->coeffs11 = WUTR_RESON * WUTR_RESON;
  p->coeffs10 = -WUTR_RESON * FL(2.0) * COS(WUTR_CENTER_FREQ1 * CS_TPIDSR);
  p->coeffs21 = WUTR_RESON * WUTR_RESON;
  p->coeffs20 = -WUTR_RESON * FL(2.0) * COS(WUTR_CENTER_FREQ2 * CS_TPIDSR);
  /* Note On */
  p->shakeEnergy =
      *p->amp * (FL(1.) / csound->Get0dBFS(csound)) * MAX_SHAKE * FL(0.1);
  p->shake_damp = FL(0.0);
...
static int32_t wuter(CSOUND *csound, WUTER *p) {
  ...
  if (*p->freq != FL(0.0) && *p->freq != p->res_freq0) {
    p->res_freq0 = *p->freq;
    p->coeffs00 = -WUTR_RESON * FL(2.0) * COS(p->res_freq0 * CS_TPIDSR);
  }
  if (*p->damp != FL(0.0) && *p->damp != p->shake_damp) {
    p->shake_damp = *p->damp;
  ...
  if (*p->freq1 != FL(0.0) && *p->freq1 != p->res_freq1) {
    p->res_freq1 = *p->freq1;
    p->coeffs10 = -WUTR_RESON * FL(2.0) * COS(p->res_freq1 * CS_TPIDSR);
  }
  if (*p->freq2 != FL(0.0) && *p->freq2 != p->res_freq2) {
    p->res_freq2 = *p->freq2;
    p->coeffs20 = -WUTR_RESON * FL(2.0) * COS(p->res_freq2 * CS_TPIDSR);
  }
...

  {
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (my_random(csound, 32767) < num_objects) {
        ...
        if (j == 0) {
          p->center_freqs0 =
              p->res_freq1 * (FL(0.75) + (FL(0.25) * noise_tick(csound)));
          p->gains0 = FABS(noise_tick(csound));
        } else if (j == 1) {
          p->center_freqs1 =
              p->res_freq1 * (FL(1.0) + (FL(0.25) * noise_tick(csound)));
          p->gains1 = FABS(noise_tick(csound));
        } else {
          p->center_freqs2 =
              p->res_freq1 * (FL(1.25) + (FL(0.25) * noise_tick(csound)));
      ...
      if (p->gains0 > FL(0.001)) {
        ...
        p->coeffs00 = -WUTR_RESON * FL(2.0) * COS(p->center_freqs0 * CS_TPIDSR);
      }
      p->gains1 *= WUTR_RESON;
      if (p->gains1 > FL(0.001)) {
        p->center_freqs1 *= WUTR_FREQ_SWEEP;
        p->coeffs10 = -WUTR_RESON * FL(2.0) * COS(p->center_freqs1 * CS_TPIDSR);
      }
      p->gains2 *= WUTR_RESON;
      if (p->gains2 > FL(0.001)) {
        p->center_freqs2 *= WUTR_FREQ_SWEEP;
        p->coeffs20 = -WUTR_RESON * FL(2.0) * COS(p->center_freqs2 * CS_TPIDSR);
      }
...
static int32_t sleighset(CSOUND *csound, SLEIGHBELLS *p) {
  ...
  p->coeffs00 = -SLEI_CYMB_RESON * FL(2.0) * COS(SLEI_CYMB_FREQ0 * CS_TPIDSR);
  p->coeffs11 = SLEI_CYMB_RESON * SLEI_CYMB_RESON;
  p->coeffs10 = -SLEI_CYMB_RESON * FL(2.0) * COS(SLEI_CYMB_FREQ1 * CS_TPIDSR);
  p->coeffs21 = SLEI_CYMB_RESON * SLEI_CYMB_RESON;
  p->coeffs20 = -SLEI_CYMB_RESON * FL(2.0) * COS(SLEI_CYMB_FREQ2 * CS_TPIDSR);
  p->coeffs31 = SLEI_CYMB_RESON * SLEI_CYMB_RESON;
  p->coeffs30 = -SLEI_CYMB_RESON * FL(2.0) * COS(SLEI_CYMB_FREQ3 * CS_TPIDSR);
  p->coeffs41 = SLEI_CYMB_RESON * SLEI_CYMB_RESON;
  p->coeffs40 = -SLEI_CYMB_RESON * FL(2.0) * COS(SLEI_CYMB_FREQ4 * CS_TPIDSR);
  /* Note On */
  p->shakeEnergy =
      *p->amp * (FL(1.) / csound->Get0dBFS(csound)) * MAX_SHAKE * FL(0.1);
  p->shake_damp = FL(0.0);
...
static int32_t sleighbells(CSOUND *csound, SLEIGHBELLS *p) {
  ...
  if (*p->freq != FL(0.0) && *p->freq != p->res_freq0) {
    p->res_freq0 = *p->freq;
    p->coeffs00 = -SLEI_CYMB_RESON * FL(2.0) * COS(p->res_freq0 * CS_TPIDSR);
  }
  if (*p->damp != FL(0.0) && *p->damp != p->shake_damp) {
    p->shake_damp = *p->damp;
  ...
  if (*p->freq1 != FL(0.0) && *p->freq1 != p->res_freq1) {
    p->res_freq1 = *p->freq1;
    p->coeffs10 = -SLEI_CYMB_RESON * FL(2.0) * COS(p->res_freq1 * CS_TPIDSR);
  }
  if (*p->freq2 != FL(0.0) && *p->freq2 != p->res_freq2) {
    p->res_freq2 = *p->freq2;
    p->coeffs20 = -SLEI_CYMB_RESON * FL(2.0) * COS(p->res_freq2 * CS_TPIDSR);
  }
  if (p->kloop > 0 && p->h.insdshead->relesing)
    p->kloop = 1;
  if ((--p->kloop) == 0) {
...

  {
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (my_random(csound, 1024) < p->num_objects) {
        ...
        p->coeffs00 = -SLEI_CYMB_RESON * FL(2.0) * COS(temp_rand * CS_TPIDSR);
        temp_rand = p->res_freq1 * (FL(1.0) + (FL(0.03) * noise_tick(csound)));
        p->coeffs10 = -SLEI_CYMB_RESON * FL(2.0) * COS(temp_rand * CS_TPIDSR);
        temp_rand = p->res_freq2 * (FL(1.0) + (FL(0.03) * noise_tick(csound)));
        p->coeffs20 = -SLEI_CYMB_RESON * FL(2.0) * COS(temp_rand * CS_TPIDSR);
        temp_rand = p->res_freq3 * (FL(1.0) + (FL(0.03) * noise_tick(csound)));
        p->coeffs30 = -SLEI_CYMB_RESON * FL(2.0) * COS(temp_rand * CS_TPIDSR);
        temp_rand = p->res_freq4 * (FL(1.0) + (FL(0.03) * noise_tick(csound)));
        p->coeffs40 = -SLEI_CYMB_RESON * FL(2.0) * COS(temp_rand * CS_TPIDSR);
      }
      inputs0 = sndLevel * noise_tick(csound); /* Actual Sound is Random */
      inputs1 = inputs0;
      inputs2 = inputs0;
...
static OENTRY phisem_localops[] = {
    ...
    {"tambourine", S(TAMBOURINE), 0, 3, "a", "kioooooo", (SUBR)tambourset,
     (SUBR)tambourine},
    {"bamboo", S(BAMBOO), 0, 3, "a", "kioooooo", (SUBR)bambooset, (SUBR)bamboo},
    {"dripwater", S(WUTER), 0, 3, "a", "kioooooo", (SUBR)wuterset, (SUBR)wuter},
    {"sleighbells", S(SLEIGHBELLS), 0, 3, "a", "kioooooo", (SUBR)sleighset,
     (SUBR)sleighbells}};
