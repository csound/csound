  ...
  if (*p->kinterp) {
    ...
    for (j = 0; j < nargs; j++) {
      if (UNLIKELY((indx + 1) * nargs + j >= ftp->flen + 1)) {
        return csound->InitError(csound, "%s",
                                 Str("vtablei: reading past end of table"));
      }
...
static int32_t mtablew_set(CSOUND *csound,
                           MTABLEW *p) /* mtabw by G.Maldonado */
{
  FUNC *ftp;
  if (UNLIKELY((ftp = csound->FTnp2Find(csound, p->xfn)) == NULL)) {
    return csound->InitError(csound, "%s",
  ...
  /* p->dstoffset = (int64_t) *p->kdstoffset;
   if (UNLIKELY((elements | (int64_t)*p->kdstoffset) < 0L ||
     ...
     return csound->InitError(csound,
                              "%s", Str("vectorop: Destination table length
   exceeded"));
   } */
  return OK;
}
...
static int32_t vmap_i(CSOUND *csound, VECTORSOPI *p) {
  ...
  if (UNLIKELY(ftp1 == NULL)) {
    return csound->InitError(csound, Str("vmap: ifn1 invalid table number %i"),
                             (int32_t)*p->ifn1);
  } else if (UNLIKELY(ftp2 == NULL)) {
    return csound->InitError(csound, Str("vmap: ifn2 invalid table number %i"),
                             (int32_t)*p->ifn2);
  }
  /*     if (*p->ifn1 == *p->ifn2)
         csound->Warning(csound, Str("vcopy_i: ifn1 = ifn2."));*/
...
static int32_t vport_set(CSOUND *csound, VPORT *p) {
  ...
  if (LIKELY((ftp = csound->FTnp2Find(csound, p->ifn)) != NULL)) {
    ...
    return csound->InitError(csound, "%s", Str("vport: invalid table"));
  if (LIKELY(*p->ifnInit)) {
    if (LIKELY((ftp = csound->FTnp2Find(csound, p->ifnInit)) != NULL)) {
      ...
      return csound->InitError(csound, "%s", Str("vport: invalid init table"));
  }
...
static int32_t vrandh_set(CSOUND *csound, VRANDH *p) {
  ...
  if (*p->iseed >= FL(0.0)) {  /* new seed:*/
    if (*p->iseed > FL(1.0)) { /* Seed from current time */
      ...
      csound->Message(
          csound, Str("vrandh: Seeding from current time %" PRIu32 "\n"), seed);
    } else {
      if (*p->isize == 0)
    ...
    if ((ftp = csound->FTnp2Find(csound, p->ifn)) != NULL) {
      ...
      return csound->InitError(csound, "%s", Str("vrandh: Invalid table."));
...
static int32_t vrandi_set(CSOUND *csound, VRANDI *p) {
  ...
  if (*p->iseed >= FL(0.0)) {  /* new seed:*/
    if (*p->iseed > FL(1.0)) { /* Seed from current time */
      ...
      csound->Message(
          csound, Str("vrandi: Seeding from current time %" PRIu32 "\n"), seed);
    } else {
      if (*p->isize == 0)
        p->rand = 0xffff & (short)(*p->iseed * 32768L); /* init rand integ    */
      else
    ...
    if (LIKELY((ftp = csound->FTnp2Find(csound, p->ifn)) != NULL)) {
      ...
      return csound->InitError(csound, "%s", Str("vrandi: Invalid table."));
...
static int32_t vrandi(CSOUND *csound, VRANDI *p) {
  ...
  do {
    *vector++ =
        (((MYFLT)*num1++ + ((MYFLT)p->phs * *dfdmax++)) * value) + *p->ioffset;
  } while (--elements);
...
static int32_t vecdly_set(CSOUND *csound, VECDEL *p) {
  ...
  if (LIKELY((ftp = csound->FTnp2Find(csound, p->ifnOut)) != NULL)) {
    ...
    return csound->InitError(csound, "%s", Str("vecdly: invalid output table"));
  if (LIKELY((ftp = csound->FTnp2Find(csound, p->ifnIn)) != NULL)) {
    ...
    return csound->InitError(csound, "%s", Str("vecdly: invalid input table"));
  if (LIKELY((ftp = csound->FTnp2Find(csound, p->ifnDel)) != NULL)) {
    ...
    return csound->InitError(csound, "%s", Str("vecdly: invalid delay table"));
  ...
  if (!*p->istod) {
    if (p->aux.auxp == NULL ||
        (uint32_t)(elements * sizeof(MYFLT *) + n * elements * sizeof(MYFLT) +
                   elements * sizeof(int32)) > p->aux.size) {
      csound->AuxAlloc(csound,
                       elements * sizeof(MYFLT *) +
...
static int32_t ca_set(CSOUND *csound, CELLA *p) {
  ...
  if (LIKELY((ftp = csound->FTnp2Find(csound, p->ioutFunc)) != NULL)) {
    ...
    return csound->InitError(csound, "%s", Str("cella: invalid output table"));
  ...
  if (LIKELY((ftp = csound->FTnp2Find(csound, p->iRuleFunc)) != NULL)) {
    ...
    return csound->InitError(csound, "%s", Str("cella: invalid rule table"));
  ...
  if (*p->ktrig) {
    ...
    if (*p->iradius == 1) {
      ...
      for (j = 0; j < elements; j++) {
        ...
        outVec[j] = previous[j];
        actual[j] =
            ruleVec[(int32_t)(previous[jm2] + previous[jm1] + previous[j] +
                              previous[(j + 1) % elements] +
...
OENTRY vectorial_localops[] = {
    ...
    {"vtablek", S(MTABLE), TR, 3, "", "kkkiz", (SUBR)mtable_set, (SUBR)mtable_k,
     NULL},
    {"vtablea", S(MTABLE), TR, 3, "", "akkiy", (SUBR)mtable_set,
     (SUBR)mtable_a},
    {"vtablewi", S(MTABLEIW), TB, 1, "", "iiim", (SUBR)mtablew_i, NULL},
    {"vtablewk", S(MTABLEW), TB, 3, "", "kkiz", (SUBR)mtablew_set,
     (SUBR)mtablew_k, NULL},
    {"vtablewa", S(MTABLEW), TB, 3, "", "akiy", (SUBR)mtablew_set,
     (SUBR)mtablew_a},
    {"vtabi", S(MTABI), TR, 1, "", "iim", (SUBR)mtab_i, NULL},
    {"vtabk", S(MTAB), TR, 3, "", "kiz", (SUBR)mtab_set, (SUBR)mtab_k, NULL},
    {"vtaba", S(MTAB), TR, 3, "", "aiy", (SUBR)mtab_set, (SUBR)mtab_a},
    {"vtabwi", S(MTABIW), TB, 1, "", "iim", (SUBR)mtabw_i, NULL},
    {"vtabwk", S(MTABW), TB, 3, "", "kiz", (SUBR)mtabw_set, (SUBR)mtabw_k,
     NULL},
    {"vtabwa", S(MTABW), TB, 3, "", "aiy", (SUBR)mtabw_set, (SUBR)mtabw_a},

    {"vadd", S(VECTOROP), TB, 3, "", "ikkOO", (SUBR)vectorOp_set, (SUBR)vaddk},
    {"vadd_i", S(VECTOROPI), TB, 1, "", "iiio", (SUBR)vadd_i, NULL, NULL},
    {"vmult", S(VECTOROP), TB, 3, "", "ikkOO", (SUBR)vectorOp_set,
     (SUBR)vmultk},
    ...
    {"vaddv", S(VECTORSOP), TB, 3, "", "iikOOO", (SUBR)vectorsOp_set,
     (SUBR)vaddvk},
    {"vaddv_i", S(VECTORSOPI), TB, 1, "", "iiioo", (SUBR)vaddv_i, NULL, NULL},
    {"vsubv", S(VECTORSOP), TB, 3, "", "iikOOO", (SUBR)vectorsOp_set,
     (SUBR)vsubvk},
    {"vsubv_i", S(VECTORSOPI), TB, 1, "", "iiioo", (SUBR)vsubv_i, NULL, NULL},
    {"vmultv", S(VECTORSOP), TB, 3, "", "iikOOO", (SUBR)vectorsOp_set,
     (SUBR)vmultvk},
    {"vmultv_i", S(VECTORSOPI), TB, 1, "", "iiioo", (SUBR)vmultv_i, NULL, NULL},
    {"vdivv", S(VECTORSOP), TB, 3, "", "iikOOO", (SUBR)vectorsOp_set,
     (SUBR)vdivvk},
    {"vdivv_i", S(VECTORSOPI), TB, 1, "", "iiioo", (SUBR)vdivv_i, NULL, NULL},
    {"vpowv", S(VECTORSOP), TB, 3, "", "iikOOO", (SUBR)vectorsOp_set,
     (SUBR)vpowvk},
    {"vpowv_i", S(VECTORSOPI), TB, 1, "", "iiioo", (SUBR)vpowv_i, NULL, NULL},
    {"vexpv", S(VECTORSOP), TB, 3, "", "iikOOO", (SUBR)vectorsOp_set,
     (SUBR)vexpvk},
    {"vexpv_i", S(VECTORSOPI), TB, 1, "", "iiioo", (SUBR)vexpv_i, NULL, NULL},
    {"vcopy", S(VECTORSOP), TB, 3, "", "iikOOO", (SUBR)vectorsOp_set,
     (SUBR)vcopy},
    ...
    {"vrandh", S(VRANDH), TB, 3, "", "ikkiovoo", (SUBR)vrandh_set,
     (SUBR)vrandh},
    {"vrandi", S(VRANDI), TB, 3, "", "ikkiovoo", (SUBR)vrandi_set,
     (SUBR)vrandi},
    {"vport", S(VPORT), TB, 3, "", "ikio", (SUBR)vport_set, (SUBR)vport},
    {"vecdelay", S(VECDEL), TB, 3, "", "iiiiio", (SUBR)vecdly_set,
     (SUBR)vecdly},
