...
#include "csoundCore.h"  /* instead of "csd.h" in order to access csound->midiGlobals */

// These opcodes have not been implemented because csound->midiGlobals is not there yet.
...
static int rbatonXYZ(CSOUND *csound, RB_XYZ *p)
{
	...
	if 	(p->local_buf_index < mg->MIDIINbufIndex) {
		MIDIMESSAGE temp = mg->MIDIINbuffer2[(p->local_buf_index)++ % MIDIINBUFMAX];
			...
			*/
    if  (p->local_buf_index != mg->MIDIINbufIndex) {
		unsigned char *temp;temp = &(mg->MIDIINbuffer2[p->local_buf_index++].bData[0]);
	...
	if (status == 0xA0) {
		switch (data1) {
			case 8:  *p->x1 = data2/127.f; break;
			case 9:  *p->y1 = data2/127.f; break;
			case 10: *p->z1 = data2/127.f; break;
			case 11: *p->x2 = data2/127.f; break;
			case 12: *p->y2 = data2/127.f; break;
			case 13: *p->z2 = data2/127.f; break;
		}
	}
	return OK;
}
...
static int rbatonPot (CSOUND *csound, RB_POT *p)
{
	...
	if (status == 0xA0) {
		switch (data1) {
			case 4: *p->pot1 = data2/127.f; break;
			case 5: *p->pot2 = data2/127.f; break;
			case 6: *p->pot3 = data2/127.f; break;
			case 7: *p->pot4 = data2/127.f; break;
			case 3:
				switch (data2) {
					case 1: *p->button = (*p->button == 0) ? 1.f : 0.f;	break;
...
typedef struct {
    OPDS   h;
    MYFLT  *kDur, *kBaton1instr, *kBaton2Instr, *kFootSw1dn, *kFootSw1up, *kFootSw2dn, *kFootSw2up, *kButInstr;
	MYFLT baton, whack, x, y, p0, p2, p3, p3neg;
	MYFLT fs1dn, fs1up, fs2dn, fs2up;
	MYFLT frac1, frac2;
	MYFLT pot1, pot2, pot3; /* pontentiometer 4 is reservet to stick sensitivity */
	int local_buf_index;
} RB_PERCPAD;
...
static OENTRY localops[] = {
	{ "rbatonPercPad", S(RB_PERCPAD),3,  "",	    "kkkkkkkk", (SUBR)rbatonPercPad_set, (SUBR)rbatonPercPad },
	{ "rbatonXYZ",     S(RB_XYZ),    3,  "kkkkkk",	"",         (SUBR)rbatonXYZ_set, (SUBR)rbatonXYZ },
	{ "rbatonPot",     S(RB_POT),    3,  "kkkkkkk",	"",         (SUBR)rbatonPot_set, (SUBR)rbatonPot  }
};
