...
static int32_t kresonx(CSOUND *csound,
                       KRESONX *p) /* Gabriel Maldonado, modified */
{
  int32_t flag = 0, j;
  MYFLT *ar, *asig;
...
#define TAB_MACRO(W, X, Y, Z)                                                  \
  ...
  static int32_t Y(CSOUND *csound, FASTB *p) { return tab_k_tmp(csound, p, Z); }
...
static int32_t adsynt2_set(CSOUND *csound, ADSYNT2 *p) {
  ...
  if (iphs > 1) {
    ...
    for (c = 0; c < count; c++) {
      lphs[c] =
          ((int32)((MYFLT)((double)(csound->Rand31(csound->RandSeed1(csound)) -
                                    1) /
                           2147483645.0) *
                   FMAXLEN)) &
          PHMASK;
...
OENTRY gab_localops[] = {
    {"resonxk", S(KRESONX), 0, 3, "k", "kkkooo", (SUBR)krsnsetx, (SUBR)kresonx,
     NULL},
    {"tab_i", S(FASTAB), TR, 1, "i", "iio", (SUBR)fastabi, NULL, NULL},
    {"tab", S(FASTAB), TR, 3, "a", "xio", (SUBR)fastab_set, (SUBR)fastab},
    {"tab.k", S(FASTAB), TR, 3, "k", "kio", (SUBR)fastab_set, (SUBR)fastabk,
     NULL},
    {"tabw_i", S(FASTAB), TW, 1, "", "iiio", (SUBR)fastabiw, NULL, NULL},
    {"tabw", S(FASTAB), TW, 3, "", "kkio", (SUBR)fastab_set, (SUBR)fastabkw},
    {"tabw", S(FASTAB), TW, 3, "", "aaio", (SUBR)fastab_set, (SUBR)fastabw},
    ...
    {"nlalp", S(NLALP), 0, 3, "a", "akkoo", (SUBR)nlalp_set, (SUBR)nlalp},
    {"adsynt2", S(ADSYNT2), TR, 3, "a", "kkiiiio", (SUBR)adsynt2_set,
     (SUBR)adsynt2},
     ...
     */
    {"tabrec", S(TABREC), TW, 3, "", "kkkkz", (SUBR)tabrec_set, (SUBR)tabrec_k,
     NULL},
    {"tabplay", S(TABPLAY), TR, 3, "", "kkkz", (SUBR)tabplay_set,
     (SUBR)tabplay_k, NULL},
    {"changed.k", S(ISCHANGED), 0, 3, "k", "z", (SUBR)isChanged_set,
     (SUBR)isChanged, NULL},
    {"changed2.k", S(ISCHANGED), 0, 3, "k", "z", (SUBR)isChanged2_set,
     (SUBR)isChanged, NULL},
    {"changed2.A", S(ISACHANGED), 0, 3, "k", ".[]", (SUBR)isAChanged_set,
     (SUBR)isAChanged, NULL},
    {"max_k", S(P_MAXIMUM), 0, 3, "k", "aki", (SUBR)partial_maximum_set,
     (SUBR)partial_maximum},
    {"mandel", S(MANDEL), 0, 3, "kk", "kkkk", (SUBR)mandel_set, (SUBR)mandel,
     NULL}};

int32_t gab_gab_init_(CSOUND *csound) {
  return csound->AppendOpcodes(
      csound, &(gab_localops[0]),
