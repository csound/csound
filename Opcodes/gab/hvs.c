...
typedef struct {
  OPDS h;
  MYFLT *kx, *inumParms, *inumPointsX, *iOutTab, *iPositionsTab, *iSnapTab,
      *iConfigTab;
...
typedef struct {
  OPDS h;
  MYFLT *kx, *ky, *inumParms, *inumlinesX, *inumlinesY, *iOutTab,
...
typedef struct {
  OPDS h;
  MYFLT *kx, *ky, *kz, *inumParms, *inumlinesX, *inumlinesY, *inumlinesZ,
...
static int32_t hvs3_set(CSOUND *csound, HVS3 *p) {
  ...
  if (UNLIKELY(*p->inumlinesX < 2 || *p->inumlinesY < 2))
    return csound->InitError(csound, "%s",
                             Str("hvs3: a square area must be "
                                 "delimited by 2 lines at least"));
...
static int32_t hvs3(CSOUND *csound, HVS3 *p) {
  ...
  int32_t ndx2 =
      (int32_t)p->posTable[posX + 1 + posY * linesX + posZ * linesXY];
  int32_t ndx3 =
      (int32_t)p->posTable[posX + (posY + 1) * linesX + posZ * linesXY];
  int32_t ndx4 =
      (int32_t)p->posTable[posX + 1 + (posY + 1) * linesX + posZ * linesXY];

  int32_t ndx5 =
      (int32_t)p->posTable[posX + posY * linesX + (posZ + 1) * linesXY];
  int32_t ndx6 =
      (int32_t)p->posTable[posX + 1 + posY * linesX + (posZ + 1) * linesXY];
  int32_t ndx7 =
      (int32_t)p->posTable[posX + (posY + 1) * linesX + (posZ + 1) * linesXY];
  int32_t ndx8 =
      (int32_t)
          p->posTable[posX + 1 + (posY + 1) * linesX + (posZ + 1) * linesXY];
...
static int32_t vphaseseg_set(CSOUND *csound, VPSEG *p) {
  ...
  if (LIKELY((ftp = csound->FTnp2Find(csound, p->ioutfunc)) != NULL)) {
    ...
    return csound->InitError(csound, "%s", Str("Failed to find ftable"));
...
OENTRY hvs_localops[] = {{"hvs1", S(HVS1), TB, 3, "", "kiiiiio", (SUBR)hvs1_set,
                          (SUBR)hvs1, (SUBR)NULL},
