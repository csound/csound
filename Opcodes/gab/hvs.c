...
The iPositionsTab is made up of the positions of the snapshots (contained in the iSnapTab)
in the two-dimensional grid. Each subsequent element is actually a pointer representing
the position in the iSnapTab. For example, in an HVS grid such as the following:

  5 2 1 3
  7 9 6 2
  0 4 1 3

This represents the snapshot position within the grid (in this case a 4x3 grid). So, the first
element 5, has index zero and represents the sixth (element zero is the first) element of the 
iSnapTab, the second element 2 represents the third element of iSnapTab and so on.
...
typedef struct {
	OPDS	h;
	MYFLT	*kx,  *inumParms, *inumPointsX, *iOutTab, *iPositionsTab, *iSnapTab, *iConfigTab;
...
static int hvs1_set(CSOUND *csound, HVS1 *p)
{
	...
	if (*p->inumPointsX < 2 ) 
		return csound->InitError(csound, "hvs1: a line segment must be delimited by 2 points at least");
...
typedef struct {
	OPDS	h;
	MYFLT	*kx, *ky, *inumParms, *inumlinesX, *inumlinesY, *iOutTab, *iPositionsTab, *iSnapTab, *iConfigTab;
	MYFLT   *outTable, *posTable, *snapTable, *confTable; 
	int iconfFlag;
} HVS2;
...
static int hvs2_set(CSOUND *csound, HVS2 *p)
{
	...
	if (*p->inumlinesX < 2 || *p->inumlinesY < 2) 
		return csound->InitError(csound, "hvs2: a square area must be delimited by 2 lines at least");
...
typedef struct {
	OPDS	h;
	MYFLT	*kx, *ky, *kz, *inumParms, *inumlinesX, *inumlinesY, *inumlinesZ, *iOutTab, *iPositionsTab, *iSnapTab, *iConfigTab;
	MYFLT   *outTable, *posTable, *snapTable, *confTable; 
	int iconfFlag;
} HVS3;
...
static int hvs3_set(CSOUND *csound, HVS3 *p)
{
    ...
    if ((ftp = csound->FTFind(csound, p->iSnapTab)) != NULL) 
      p->snapTable = ftp->ftable;
	if (*p->inumlinesX < 2 || *p->inumlinesY < 2) 
		return csound->InitError(csound, "hvs3: a square area must be delimited by 2 lines at least");
...
static int vphaseseg_set(CSOUND *csound, VPSEG *p)
{
    ...
    if ((nxtfunc = csound->FTnp2Find(csound, *argp++)) == NULL)
        return csound->InitError(csound, "vphaseseg: the first function is invalid or missing");
    ...
    do { 
		...
		dur = **argp++;
		if ((nxtfunc = csound->FTnp2Find(csound, *argp++)) == NULL) 
			return csound->InitError(csound, "vphaseseg: function invalid or missing");
		if (dur > 0.0f) {
			durtot+=dur;
			segp->d = dur; //* ekr;
