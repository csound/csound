...
static int32_t lposca(CSOUND *csound, LPOSC *p) {
  ...
  if ((loop = (int64_t)*p->kloop) < 0)
    loop = 0; /* gab */
  else if (loop > p->tablen - 3)
    loop = p->tablen - 3;
  if ((end = (int64_t)*p->kend) > p->tablen - 1)
    end = p->tablen - 1;
  else if (end <= 2)
    end = 2;
  if (end < loop + 2)
...
static int32_t lposc_stereo_set(CSOUND *csound, LPOSC_ST *p) {
  ...
  if (UNLIKELY(!(fsr = ftp->gen01args.sample_rate))) {
    csound->Message(csound, "%s",
                    Str("lposcil: no sample rate stored in function;"
                        " assuming=sr\n"));
    p->fsr = CS_ESR;
  }
...
static int32_t lposca_stereo(CSOUND *csound, LPOSC_ST *p) /* stereo lposcinta */
{
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    out1[n] = amp[n] *
              (MYFLT)(*curr_samp1 + (*(curr_samp1 + 2) - *curr_samp1) * fract);
    out2[n] = amp[n] *
              (MYFLT)(*curr_samp2 + (*(curr_samp2 + 2) - *curr_samp2) * fract);
    *phs += si;
    while (*phs >= end)
      *phs -= looplength;
    while (*phs < loop)
...
static int32_t
lposca_stereo_no_trasp(CSOUND *csound,
                       LPOSC_ST *p) { /* transposition is allowed only */
...
static int32_t trRangeRand(CSOUND *csound, TRANGERAND *p) { /* gab d5*/
  if (*p->ktrig)
...
static OENTRY localops[] = {
    {"vtable1k", S(MTABLE1), TR, 3, "", "kz", (SUBR)mtable1_set,
     (SUBR)mtable1_k, (SUBR)NULL},
    {"trandom", S(TRANGERAND), 0, 2, "k", "kkk", NULL, (SUBR)trRangeRand},
    {"lposcila", S(LPOSC), TR, 3, "a", "akkkio", (SUBR)lposc_set, (SUBR)lposca},
    {"lposcilsa", S(LPOSC_ST), TR, 3, "aa", "akkkio", (SUBR)lposc_stereo_set,
     (SUBR)lposca_stereo},
    {"lposcilsa2", S(LPOSC_ST), TR, 3, "aa", "akkkio", (SUBR)lposc_stereo_set,
     (SUBR)lposca_stereo_no_trasp}};
