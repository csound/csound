        ...
        *max = *sld->imax;                                                     \
        ...
        case -1: /* EXPONENTIAL */                                             \
          if (UNLIKELY(*min == 0 || *max == 0)) {                              \
            return csound->InitError(csound, Str("sliderXtable: zero is "      \
  ...
  {                                                                            \
    ...
    {                                                                          \
      ...
      while (j < n) {                                                          \
        ...
        *yt1++ = FL(0.0);                                                      \
        b = (MYFLT)(2.0 - cos((double)(*(sld)->ihp * CS_TPIDSR * CS_KSMPS)));  \
...
static int32_t sliderKawai_i(CSOUND *csound, SLIDERKAWAI *p) {
  ...
  do {
    ...
    value = (*(sld++)->initvalue - *min) / (*max++ - *min);
    min++;
    csound->m_chnbp[j]->ctl_val[7] =
        (MYFLT)((int32_t)(value * f7bit + FL(0.5)));
  } while (++j < n);
  return OK;
}
...
/*--------------------------------------------------------*/
#define TOOSMALL                                                               \
  0.0000000000000000000000001f /* to avoid denormalization bug                 \
                                */
...
OENTRY sliderTable_localops[] = {
    ...
    {"slider64table", S(SLIDER8t), 0, 3, "k",
     "iii"
     "iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii"
     "iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii"
     "iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii"
    ...
    {"slider64tablef", S(SLIDER64tf), 0, 3, "k",
     "iii"
     "iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii"
     "iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii"
     "iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii"
    ...
    {"ctrl7.a", S(CTRL7a), 0, 3, "a", "iikkoo", (SUBR)ctrl7a_set, (SUBR)ctrl7a},
};

int32_t slidertable_init_(CSOUND *csound) {
  return csound->AppendOpcodes(
      csound, &(sliderTable_localops[0]),
