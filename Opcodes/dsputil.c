...
void FetchIn(float *inp,  /* pointer to input data */
    ...
    int32 circBufSize)      /* Size of circ buf i.e. dst[0..circBufSize-1] */
{
  ...
  breakPoint =
      circBufSize - dstStart; /* i.e. if we start at (dIndx = lim -2) */
...
void UDSample(PVOC_GLOBALS *p, MYFLT *inSnd, MYFLT stindex, MYFLT *outSnd,
              int32 inLen, int32 outLen, MYFLT fex)
 ...
 */
{
  ...
  for (i = 0; i < outLen; ++i) {                 /* output sample loop      */
    ...
    nrst = (int32_t)negPhase;
    frac = negPhase - (MYFLT)nrst;
    a = (p->dsputil_sncTab[nrst] +
         frac * (p->dsputil_sncTab[nrst + 1] - p->dsputil_sncTab[nrst])) *
    ...
    for (j = 1L; j < in2out; ++j) { /* inner FIR convolution loop */
      ...
      if ((x = nrstInStep + j) < inLen) { /* Brackets inserted here */
        nrst = (int32_t)posPhase;
        frac = posPhase - (MYFLT)nrst;
      }
      a += (p->dsputil_sncTab[nrst] +
            frac * (p->dsputil_sncTab[nrst + 1] - p->dsputil_sncTab[nrst])) *
           (MYFLT)inSnd[x];
...
void MakeSinc(PVOC_GLOBALS *p) /* initialise our static sinc table */
{
  ...
  MYFLT dtheta =
      (MYFLT)(SBW * PI) / (MYFLT)SPTS; /* SBW lowers cutoff to redcali */
...
void PreWarpSpec(
  ...
  for (i = 1; i < someof(size); i++) { /*  step thru spectrum */
    ...
    if ((mag >= lastmag) && (mag > nextmag) && (slope > eps)) {
      ...
      pkcnt--;
      for (j = 1; j <= pkcnt; j++) {
        dsputil_env[i - pkcnt + j - 1] = pkOld * (FL(1.0) + slope * j);
      }
      pkOld = mag;
      pkcnt = 1;
    } else
