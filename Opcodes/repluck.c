...
/*       printf("Pluck point %f invalid, using 0.5\n", plk); */
    ...
    if (p->upper.auxp == NULL) {/* get newspace    */
      auxalloc(csound, sizeof(DelayLine),&p->upper);
    }
    upper_rail = (DelayLine*)p->upper.auxp;
    upper_rail->length = rail_len;
    if (rail_len > 0) {
      auxalloc(csound, rail_len*sizeof(MYFLT),&p->up_data);
    ...
    if (p->lower.auxp == NULL) {/* get newspace    */
      auxalloc(csound, sizeof(DelayLine),&p->lower);
    }
    lower_rail = (DelayLine*)p->lower.auxp;
    lower_rail->length = rail_len;
    if (rail_len > 0) {
      auxalloc(csound, rail_len*sizeof(MYFLT),&p->down_data);
    ...
    if (plk != 0.0) {
      ...
      mfree(csound, (char *)initial_shape);
    }
    else {
      for (i = 0; i < rail_len; i++)
...
int stresonset(ENVIRON *csound, STRES *p)
{
    ...
    auxalloc(csound, p->size*sizeof(MYFLT), &p->aux);
