...
static int32_t sequencer_init(CSOUND *csound, SEQ *p) {
  ...
  if (p->max_length != p->instr->sizes[0] ||
    ...
    return csound->InitError(csound, "%s",
                             Str("sequ: arrays have differing sizes"));
  }
  p->time = 0;
  p->next = 0;
  p->cnt = 1;
  ...
  }
  {
    ...
    if (inst != 0) {
      ...
      if (p->data->dimensions == 2) {
        int j;
        snprintf(buff, 99, "i %0.2f 0 %g ", inst,
...
static int sequStateInit(CSOUND *csound, SEQSTATE *p) {
  ...
  if (p->riff->sizes[0] != p->q->max_length)
    csound->InitError(csound, "%s",
                      Str("sequstate: Wrong sizeof output array"));
  sequState(csound, p);
  return OK;
}
...
static OENTRY sequencer_localops[] = {
    {"sequ", sizeof(SEQ), 0, 3, "k", "i[]i[]i[]kkOOOoo", (SUBR)sequencer_init,
     (SUBR)sequencer},
    {"sequstate.i", sizeof(SEQSTATE), 0, 1, "ii[]", "o", (SUBR)sequStateInit},
    {"sequstate.k", sizeof(SEQSTATE), 0, 3, "kk[]", "o", (SUBR)sequStateInit,
     (SUBR)sequState}};
