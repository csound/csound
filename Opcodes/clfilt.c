...
static int32_t clfiltset(CSOUND *csound, CLFILT *p) {
  ...
  if (UNLIKELY((p->ikind < 0) || (p->ikind > 3))) {
    return csound->InitError(
        csound, Str("filter kind, %d, out of range in clfilt"), p->ikind);
  }
  if (UNLIKELY((*p->npol < FL(1.0)) || (*p->npol > 2 * CL_LIM))) {
    return csound->InitError(csound,
                             Str("number of poles, %f, out of range "
                                 "in clfilt"),
                             *p->npol);
    /*       p->nsec = nsec = 1; */
    ...
    csound->Warning(csound,
                    Str("odd number of poles chosen in clfilt,"
                        " rounded to %d"),
                    2 * nsec);
  } else
    p->nsec = nsec = (int32_t)((*p->npol) * FL(0.5));
  switch (p->ilohi) {
  case 0: /* Lowpass filters */
    ...
    case 1: /* Lowpass Chebyshev type I */
      if (UNLIKELY(pbr < FL(0.0))) {
        pbr = -pbr;
        csound->Warning(csound,
                        Str("passband ripple must be positive "
                            "in clfilt. Set to %f"),
                        pbr);
      } else if (UNLIKELY(pbr == FL(0.0))) {
        pbr = FL(1.0);
        csound->Warning(csound,
                        Str("passband ripple must be non-zero in "
                            "clfilt. Set to %f"),
                        pbr);
      }
      ...
      for (m = 0; m <= nsec - 1; m++) {
        ...
        if (m == 0) { /* H0 and pole magnitudes */
          p->b0[m] =
              ((p->alpha[m]) * (p->alpha[m]) + (p->beta[m]) * (p->beta[m])) *
              FL(1.0) / (MYFLT)sqrt(1.0 + eps * eps);
          p->b1[m] =
              ((p->alpha[m]) * (p->alpha[m]) + (p->beta[m]) * (p->beta[m])) *
              FL(2.0) / (MYFLT)sqrt(1.0 + eps * eps);
          p->b2[m] =
              ((p->alpha[m]) * (p->alpha[m]) + (p->beta[m]) * (p->beta[m])) *
              FL(1.0) / (MYFLT)sqrt(1.0 + eps * eps);
    ...
    case 2: /* Lowpass Chebyshev type II */
      if (sbr == FL(1.0)) {
        ...
        sbr = -sbr;
        csound->Warning(csound,
                        Str("stopband attenuation must be negative "
                            "in clfilt. Set to %f"),
                        sbr);
      } else if (UNLIKELY(sbr == FL(0.0))) {
        sbr = FL(-60.0);
        csound->Warning(csound,
                        Str("stopband attenuation must be non-zero "
                            "in clfilt. Set to %f"),
                        sbr);
      }
    ...
    case 3: /* Lowpass Elliptical */
      return csound->InitError(
          csound, "%s", Str("Lowpass Elliptical not implemented yet. Sorry!"));
      break;
  ...
  case 1: /* Highpass filters */
    ...
    case 1: /* Highpass Chebyshev type I */
      if (UNLIKELY(pbr < FL(0.0))) {
        ...
        pbr = FL(1.0);
        csound->Warning(csound,
                        Str("passband ripple must be non-zero "
                            "in clfilt. Set to %f"),
                        pbr);
      }
      eps = sqrt((pow(10.0, (pbr / 10.0)) - 1.0));
      ...
      case 2: /* Lowpass Chebyshev type II */
        for (m = 0; m <= nsec - 1; m++) {
          p->a0[m] = a0[m] = (p->alpha[m]) * (p->alpha[m]) +
                             ...
                             tanfpi * (tanfpi - FL(2.0) * (p->alpha[m]));
          p->a1[m] = a1[m] =
              FL(2.0) * (tanfpi2 - ((p->alpha[m]) * (p->alpha[m]) +
                                    (p->beta[m]) * (p->beta[m])));
          p->a2[m] = a2[m] = (p->alpha[m]) * (p->alpha[m]) +
                             (p->beta[m]) * (p->beta[m]) +
                             tanfpi * (tanfpi + FL(2.0) * (p->alpha[m]));
      ...
      case 2: /* Highpass Chebyshev type II */
        for (m = 0; m <= nsec - 1; m++) {
          p->a0[m] = a0[m] = (p->alpha[m]) * (p->alpha[m]) +
                             ...
                             cotfpi * (cotfpi - FL(2.0) * (p->alpha[m]));
          p->a1[m] = a1[m] =
              FL(2.0) * (-cotfpi2 + ((p->alpha[m]) * (p->alpha[m]) +
                                     (p->beta[m]) * (p->beta[m])));
          p->a2[m] = a2[m] = (p->alpha[m]) * (p->alpha[m]) +
                             (p->beta[m]) * (p->beta[m]) +
                             cotfpi * (cotfpi + FL(2.0) * (p->alpha[m]));
