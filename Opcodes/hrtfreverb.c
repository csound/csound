...
{ 
	...
	/* mean free path and order are optional, meanfp defaults to medium room, opcode
	   can be used as stand alone binaural reverb, or spatially accurate taking meanfp and order from earlies opcode */
	...
	AUXCH del1tf, del2tf, del3tf, del4tf, del5tf, del6tf, del7tf, del8tf, del9tf, del10tf, del11tf, del12tf;
	...
	/* output of matrix cycle, with IIRs in combs and FIR tone, then l and r o/p processed with u and v coherence filters */
...
{ 
	...
	/* meanfp: defaults to doom size 10 * 10 * 4 (max of 1: v. large room, min according to min room dimensions in early: 2 * 2 * 2) */
	...
	/* which no. of delay lines implies ave delay nearest to mfp(which is an appropriate ave)? */
	...
	/* usually, just go through all files; in this case, just doubled due to symmetry (with exceptions, as below) */
	for(i = 0; i < 368; i ++)
	{
		/* if at a measurement where no doubling for symmetry necessary... */
		if(i == 0 || i == 28 || i == 29 || i == 59 || i == 60 || i == 96  || i == 97 || i == 133 
			|| i == 134 || i == 170 || i == 171 || i == 207 || i == 208 || i == 244  || i == 245
			|| i == 275 || i == 276 || i == 304  || i == 305 || i == 328 || i == 346 || i == 347 
			|| i == 359 || i == 360 || i == 366 || i == 367)
			skipdouble = 1;
		else
			skipdouble = 0;
		...
		/* deal with 0 hz and nyq: may be a negative real val, no need for fabs() as squaring anyway! */
		/* skipdouble: l = r */
		if(skipdouble)
		{
	...
	{	
		...
		= (a c + b d) + i(- a d + b c) */
		/* doing l * conj r and r * conj l here, as dataset symmetrical...for non symmetrical, just go through all and do l * conj r */
		for(j = 2; j < irlength; j += 2)
		{
			...
			else
			{
				...
				nump[j + 1] = nump[j + 1] + ((rel * -imr) + (iml * rer)) + ((rer * -iml) + (imr * rel));
			}		
		}	
		skip += irlength;	
	}
	...
	/* 0 & nyq = fabs() for mag... */
	nump[0] = FL(fabs(nump[0]));
	nump[1] = FL(fabs(nump[1]));
	...
	do
	{
		...
		aconst = FL((log(10.0) / 4.0) * (1.0 - (1.0 / alphsq)));
		for(i = 0; i < M; i++)
		{
...
{ 
	...
	MYFLT *del1tfp, *del2tfp, *del3tfp, *del4tfp, *del5tfp, *del6tfp, *del7tfp, *del8tfp, *del9tfp, *del10tfp, *del11tfp, *del12tfp;
	int *delaysp;
	...
	{ 
		...
		if(M==12 || M==24)
			tonall += (del1tp[ut] - del2tp[vt] + del3tp[wt] - del4tp[xt] + del5tp[yt] - del6tp[zt]);	
		if(M==24)
			tonall += (del1tfp[utf1] - del2tfp[vtf1] + del3tfp[wtf1] - del4tfp[xtf1] + del5tfp[ytf1] - del6tfp[ztf1] + del7tfp[utf2] - del8tfp[vtf2] + del9tfp[wtf2] - del10tfp[xtf2] + del11tfp[ytf2] - del12tfp[ztf2]);			
		...
		if(M==12 || M==24)
			tonalr += (del1tp[ut] + del2tp[vt] + del3tp[wt] + del4tp[xt] + del5tp[yt] + del6tp[zt]);
		if(M==24)
			tonalr += (del1tfp[utf1] - del2tfp[vtf1] + del3tfp[wtf1] - del4tfp[xtf1] + del5tfp[ytf1] - del6tfp[ztf1] + del7tfp[utf2] - del8tfp[vtf2] + del9tfp[wtf2] - del10tfp[xtf2] + del11tfp[ytf2] - del12tfp[ztf2]);			
...
in[i] = in[i] * (FL(32767.0) / FL(csound->e0dbfs));
...
outl[i] = hrtflp[counter] * (FL(csound->e0dbfs) / FL(32767.0));
outr[i] = hrtfrp[counter] * (FL(csound->e0dbfs) / FL(32767.0));
			...
			if(counter == irlength)
			{
				...
				csound->RealFFTMult(csound, matrixlup, matrixlup, filtupadp, irlengthpad, (MYFLT)1.0);  
				csound->RealFFTMult(csound, matrixrvp, matrixrvp, filtvpadp, irlengthpad, (MYFLT)1.0);  
