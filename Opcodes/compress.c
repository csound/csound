...
typedef struct {
	OPDS	h;
	MYFLT	*ar, *aasig, *acsig, *kthresh, *kloknee, *khiknee, *kratio, *katt, *krls, *ilook;
...
int compset(CSOUND *csound, CMPRS *p)
{
    long  delsmps;
    ...
    if ((delsmps = (long)(*p->ilook * csound->esr)) <= 0);
    delsmps = 1;			/* alloc 2 delay bufs	*/
...
int compress(CSOUND *csound, CMPRS *p)
{
    MYFLT	*ar, *ainp, *cinp;
    ...
    if (*p->kloknee != p->loknee
      ...
      else p->kneemul = FL(1.0);
    }
    if (*p->katt != p->curatt) {
      if ((p->curatt = *p->katt) < csound->onedsr)
        p->c2 = 0.0;
      else p->c2 = pow(0.5, csound->onedsr / p->curatt);
      p->c1 = 1.0 - p->c2;
    }
    if (*p->krls != p->currls) {
      if ((p->currls = *p->krls) < csound->onedsr)
        p->d2 = 0.0;
      else p->d2 = pow(0.5, csound->onedsr / p->currls);
      p->d1 = 1.0 - p->d2;
    }
    ar = p->ar;
    ainp = p->aasig;
...
int distset(CSOUND *csound, DIST *p)
{
    double  b;
    FUNC  *ftp;

    if (csound->FTFind(csound, p->ifn) == NULL) return NOTOK;
    p->ftp = ftp;
...
int distort(CSOUND *csound, DIST *p)
{
    MYFLT   *ar, *asig;
...
{ "compress", S(CMPRS), 5, "a", "aakkkkkki", (SUBR)compset, NULL, (SUBR)compress},
{ "distort", S(DIST), 5, "a", "akiqo", (SUBR)distset, NULL, (SUBR)distort}
};

int compress_init_(CSOUND *csound)
{
