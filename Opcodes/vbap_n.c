...
int32_t vbap_moving_control(CSOUND *, VBAP_MOVE_DATA *, OPDS *, MYFLT, MYFLT *,
                            MYFLT *, MYFLT **);
  ...
  for (j = 0; j < cnt; j++) {
    ...
    if (ngain != FL(0.0) || ogain != FL(0.0)) {
      if (ngain != ogain) {
        ...
        p->q.curr_gains[j] = ogain + (MYFLT)(i)*invfloatn * gainsubstr;
      } else {
        for (i = offset; i < nsmps; ++i) {
...
int32_t vbap_control(CSOUND *csound, VBAP_DATA *p, MYFLT *azi, MYFLT *ele,
                     MYFLT *spread) {
  ...
  calc_vbap_gns(p->ls_set_am, p->dim, p->ls_sets, p->updated_gains, cnt,
                p->cart_dir);
  ...
  if (*spread > FL(0.0)) {
    if (p->dim == 3) {
      ...
      new_spread_dir(&spreaddir[0], p->cart_dir, p->spread_base, *azi, *spread);
      ...
      for (i = 1; i < spreaddirnum; i++) {
        new_spread_dir(&spreaddir[i], p->cart_dir, spreadbase[i], *azi,
                       *spread);
        calc_vbap_gns(p->ls_set_am, p->dim, p->ls_sets, tmp_gains, cnt,
                      spreaddir[i]);
        for (j = 0; j < cnt; j++) {
...
int32_t vbap_init(CSOUND *csound,
                  VBAP *p) { /* Initializations before run time*/
  int32_t i, j;
  MYFLT *ls_table, *ptr;
  LS_SET *ls_set_ptr;
  ...
  snprintf(name, 24, "vbap_ls_table_%d",
           (p->layout == NULL ? 0 : (int32_t)*p->layout));
...
int32_t vbap_init_a(CSOUND *csound,
                    VBAPA *p) { /* Initializations before run time*/
  int32_t i, j;
  MYFLT *ls_table, *ptr;
  LS_SET *ls_set_ptr;
  int32_t cnt;
...
int32_t vbap_moving(CSOUND *csound,
                    VBAP_MOVING *p) { /* during note performance:   */
  MYFLT *outptr, *inptr;
  MYFLT ogain, ngain, gainsubstr;
  MYFLT invfloatn;
  int32_t j;
  ...
  vbap_moving_control(csound, &p->q, &(p->h), CS_ONEDKR, p->spread, p->field_am,
                      p->fld);
...
int32_t vbap_moving_control(CSOUND *csound, VBAP_MOVE_DATA *p, OPDS *h,
                            MYFLT ONEDKR, MYFLT *spread, MYFLT *field_am,
                            MYFLT *fld[]) {
  CART_VEC spreaddir[16];
  CART_VEC spreadbase[16];
  ANG_VEC atmp;
  ...
  if (p->point_change_counter++ >= p->point_change_interval) {
    ...
    if (*field_am >= FL(0.0) && p->dim == 2) /* point-to-point */
      if (UNLIKELY(fabs(fabs(*fld[p->next_fld] - *fld[p->curr_fld]) - 180.0) <
                   1.0))
        csound->Warning(csound, "%s",
  ...
  if (*field_am >= FL(0.0)) { /* point-to-point */
    if (p->dim == 3) {        /* 3-D*/
      ...
      coeff =
          ((MYFLT)p->point_change_counter) / ((MYFLT)p->point_change_interval);
      ...
      coeff = (MYFLT)sqrt(
          (double)(tmp3.x * tmp3.x + tmp3.y * tmp3.y + tmp3.z * tmp3.z));
      tmp3.x /= coeff;
      tmp3.y /= coeff;
      tmp3.z /= coeff;
    ...
    if (p->dim == 2) {
      p->ang_dir.azi = p->ang_dir.azi + (*fld[p->next_fld] * ONEDKR);
      scale_angles(&(p->ang_dir));
    } else { /* 3D angular*/
      p->ang_dir.azi = p->ang_dir.azi + (*fld[p->next_fld] * ONEDKR);
      p->ang_dir.ele =
          p->ang_dir.ele + p->ele_vel * (*fld[p->next_fld + 1] * ONEDKR);
  ...
  if (*spread > FL(0.0)) {
    if (p->dim == 3) {
      spreaddirnum = 16;
      /* four orthogonal dirs*/
      new_spread_dir(&spreaddir[0], p->cart_dir, p->spread_base, p->ang_dir.azi,
                     *spread);
      ...
      for (i = 1; i < spreaddirnum; i++) {
        new_spread_dir(&spreaddir[i], p->cart_dir, spreadbase[i],
                       p->ang_dir.azi, *spread);
        calc_vbap_gns(p->ls_set_am, p->dim, p->ls_sets, tmp_gains, cnt,
                      spreaddir[i]);
        for (j = 0; j < cnt; j++) {
          p->updated_gains[j] += tmp_gains[j];
...
int32_t vbap_moving_init(CSOUND *csound, VBAP_MOVING *p) {
  ...
  if (UNLIKELY(fabs(*p->field_am) < (2 + (p->q.dim - 2) * 2))) {
    return csound->InitError(
        csound, Str("Have to have at least %d directions in vbapmove"),
        2 + (p->q.dim - 2) * 2);
  }
  if (p->q.dim == 2)
...
int32_t vbap_moving_a(CSOUND *csound,
                      VBAPA_MOVING *p) { /* during note performance:   */
  MYFLT *outptr, *inptr;
  MYFLT ogain, ngain, gainsubstr;
  MYFLT invfloatn;
  int32_t j;
