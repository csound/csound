 ...
 *     If table is symmetric, kform and -kform should give bit-identical
 * outputs.
 *
...
typedef struct {
  OPDS h;
  MYFLT *ar, *amp, *kfund, *kform, *kdamp, *knofpulse, *kpulsemul, *iftab,
      *iskip;
  FUNC *ftable;
...
void vosim_pulse(CSOUND *csound, VOSIM *p) {
  ...
  pulselen =
      (p->pulseinc != FL(0.0)) ? (int32)FABS(FMAXLEN / p->pulseinc) : INT_MAX;
