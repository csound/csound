...
static int32_t ifd_init(CSOUND *csound, IFD *p) {
  ...
  if (p->win.auxp == NULL || fftsize * sizeof(MYFLT) > (uint32_t)p->win.size)
  ...
  if (p->cnt >= hopsize) {
    ...
    if (UNLIKELY(ft == NULL)) {
      return csound->PerfError(
          csound, &(p->h), "could not find table number %d\n", (int32_t)*p->p7);
    }
    MYFLT *tab = ft->ftable;
...
static OENTRY localops[] = {{"pvsifd", sizeof(IFD), 0, 3, "ff", "aiiip",
