...
int32_t stdopc_ModuleInit(CSOUND *csound) {
  ...
  p = (STDOPCOD_GLOBALS *)csound->QueryGlobalVariable(csound, "STDOPC_GLOBALS");

  if (p == NULL) {
    if (UNLIKELY(csound->CreateGlobalVariable(csound, "STDOPC_GLOBALS",
                                              sizeof(STDOPCOD_GLOBALS)) !=
                 CSOUND_SUCCESS)) {
    ...
    return CSOUND_SUCCESS; // already initialised
