...
static int32_t dconvset(CSOUND *csound, DCONV *p) {
  ...
  if (LIKELY((ftp = csound->FTnp2Find(csound, p->ifn)) !=
             NULL)) { /* find table */
    p->ftp = ftp;
...
static int32_t not_k(CSOUND *csound,
                     AOP *p) /* Added for completeness by JPff */
{
  IGN(csound);
#ifndef USE_DOUBLE
  int32_t input1 = MYFLT2LRND(*p->a);
...
static int32_t vcomb(CSOUND *csound, VCOMB *p) {
  ...
  if (p->lpta) { /* if xlpt is a-rate */
    ...
    xlpt = (uint32)((*p->insmps != 0) ? *p->xlpt : *p->xlpt * CS_ESR);
    if (xlpt > maxlpt)
      xlpt = maxlpt;
    if ((rp = wp - xlpt) < startp)
      rp += maxlpt;
...
static int32_t ftmorfset(CSOUND *csound, FTMORF *p) {
  ...
  do { /* make sure tables in ftfn exist and are right size*/
    if (LIKELY((ftp = csound->FTnp2Find(csound, p->ftfn->ftable + j)) !=
               NULL)) {
...
static int32_t ftmorf(CSOUND *csound, FTMORF *p) {
  ...
  if (*p->kftndx >= p->ftfn->flen)
    *p->kftndx = (MYFLT)(p->ftfn->flen - 1);
  i = (int32_t)*p->kftndx;
  f = *p->kftndx - i;
  if (p->ftndx != *p->kftndx) {
    p->ftndx = *p->kftndx;
    ...
    do {
      *(p->resfn->ftable + j) =
          (*(ftp1->ftable + j) * (1 - f)) + (*(ftp2->ftable + j) * f);
    } while (++j < p->len);
  }
  return OK;
}
