                                    ...
                                    void *messageBuffer, struct lws *websocket)
{
    ...
    if (inputArgument->bytesWritten + writeBufferBytesCount <
        ...
        writeFlags |=
          inputArgument->bytesWritten == 0 ? LWS_WRITE_BINARY
                                  ...
                                  void *messageBuffer, struct lws *websocket)
{
    if (inputArgument->bytesCount <= writeBufferBytesCount) {
        ...
        WebSocketOpcode_writeFragments(self, inputArgument, messageBuffer,
                                       websocket);
                                          ...
                                          CSOUND *csound, void *messageBuffer)
{
    ...
    // If it is an output argument just return
                                   ...
                                   void *inputData)
{
    ...
    if (argument->argumentType == STRING_VAR
      ...
      csound->Message(csound, Str("websocket: received string message from %s "
                                  "is too large, message dumped"), protocol->name);
    ...
    if (writtenItems == 0) {
      ...
      csound->Message(csound, Str("websocket: received message from %s dumped, "
                              ...
                              void *user, void *inputData, size_t inputDataSize)
{
    ...
    if (reason != LWS_CALLBACK_ESTABLISHED && reason !=
        LWS_CALLBACK_SERVER_WRITEABLE && reason != LWS_CALLBACK_RECEIVE) {
        return OK;
    }
                                            ...
                                            CSOUND *csound, bool isInputArgument)
{
    ...
    argumentArrayItem->circularBuffer =
      csoundCreateCircularBuffer(csound, argumentArrayItem->itemsCount *
                                              ...
                                              bool areInputArguments)
{
    ...
    for (i = 0; i < argumentsCount; ++i) {
      ...
      case STRING_VAR: {
        ...
        WebSocketOpcode_allocateStringArgument(arguments[i], argumentArrayItem,
...
void WebSocketOpcode_sendInputArgumentData(CSOUND *csound, WebSocketOpcode *self)
{
    ...
    for (i = 0; i < self->inputArgumentCount; ++i) {
      ...
      if (itemsWritten != currentArgument->itemsCount) {
        ...
        csound->Message(csound, Str("websocket: variable %s data not sent, "
