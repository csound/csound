...
static inline void init_load(rbload_t *buffer, int32_t size) {
...
static inline load_t *next_load(const rbload_t *buffer, load_t *const now) {
  load_t *temp = now + 1;
...
static inline load_t *previous_load(const rbload_t *buffer, load_t *const now) {
...
**    outBuf - the output of the operation (called with tmpBuf), single channel
*only
...
static inline int32_t buf_bytes_alloc(int32_t partSize, int32_t nPartitions) {
  ...
  nSmps +=
      (nPartitions + 1) * (int32_t)sizeof(load_t); /* Load/unload structure */
  /* One load/unload pr. partitions and an extra for buffering is sufficient */

  return nSmps;
}

static void set_buf_pointers(liveconv_t *p, int32_t partSize,
                             int32_t nPartitions) {
  MYFLT *ptr;
...
static int32_t liveconv_perf(CSOUND *csound, liveconv_t *p) {
  ...
  */
  ...
  for (nn = offset; nn < nsmps; nn++) {
    ...
    multiply_fft_buffers(p->tmpBuf, p->ringBuf, p->IR_Data, nSamples,
                         p->nPartitions, rBufPos);
