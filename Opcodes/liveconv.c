...
/* The implementation is indepted to the ftconv opcode by Istvan Varga 2005 */
...
inline void init_load(rbload_t *buffer, int size)
{
...
inline load_t* next_load(const rbload_t *buffer, load_t* const now)
{
	load_t *temp = now + 1;
	if (temp == buffer->end) {
		temp = buffer->begin;
...
inline load_t* previous_load(const rbload_t *buffer, load_t* const now)
{
