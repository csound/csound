...
static int vstaudiog(CSOUND *csound, void *data) {
  ...
  for (j = 0; j < p->pluginInChannels && j < p->opcodeInChannels; j++)
    for (i = offset; i < p->framesPerBlock; i++)
      plugin->inputs_[j][i] = (float)(p->ains[j][i] / csound->Get0dBFS(csound));
  for (; j < p->pluginInChannels; j++)
    for (i = 0; i < p->framesPerBlock; i++) {
      plugin->inputs_[j][i] = 0.0f;
    }
...
static int vstmidiout(CSOUND *csound, void *data) {
  ...
  plugin->Debug("vstmidiout. kstatus = %i kdata1 = %i kdata2 = %i"
                "--- mididata = %i\n",
                (int)*(p->kstatus), (int)*(p->kdata1), (int)*(p->kdata2),
                midiData);
...
static int vstbankload(CSOUND *csound, void *data) {
  ...
  plugin->Dispatch(effBeginLoadBank, 0, 0,
                   (VstPatchChunkInfo *)fxBank.GetChunk(), 0);
  if (plugin->Dispatch(effBeginLoadBank, 0, 0,
                       (VstPatchChunkInfo *)fxBank.GetChunk(), 0)) {
  ...
  if (fxBank.IsLoaded()) {
    ...
    if (fxBank.IsChunk()) {
      ...
      plugin->Dispatch(effSetChunk, 0, fxBank.GetChunkSize(), fxBank.GetChunk(),
                       0); // isPreset = 0
    ...
    return csound->InitError(
        csound, Str("Problem loading bank.")); /* check if error loading */
  }
...
static int vstprogset(CSOUND *csound, void *data) {
  ...
  if (program <= 0) {
    csound->Message(csound, Str("VSTprogset: Program %d treated as 1\n"),
                    program);
    program = 1;
  }
...
int vstbanksave(CSOUND *csound, void *data) {
  ...
  if (plugin->aeffect->flags & effFlagsProgramChunks) {
    ...
    b.SetSize(plugin->aeffect->numPrograms, plugin->aeffect->numParams);
    if (b.IsLoaded()) {
      int i, j;
...
static OENTRY localops[] = {
    ...
    {"vstaudio", sizeof(VSTAUDIO), 0, 3, "mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm",
     "iy", &vstaudio_init, &vstaudio},
    {"vstaudiog", sizeof(VSTAUDIO), 0, 3, "mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm",
     "iy", &vstaudio_init, &vstaudiog},
    ...
    {
        "vsttempo", sizeof(VSTTEMPO), 0, 2, "", "ki", 0, &vstSetTempo,
        0 /*, &vstedit_deinit*/
    },
...
// It is necessary to store global references to plugins and editors from Seib's
// code
