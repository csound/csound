...
MYFLT filter(MYFLT* sig, MYFLT highcoeff, MYFLT lowcoeff, MYFLT *del, int vecsize, MYFLT sr)
{
	MYFLT costh, coef;
	int i;
...
MYFLT band(MYFLT* sig, MYFLT cfreq, MYFLT bw, MYFLT g, MYFLT *del, int vecsize, MYFLT sr)
{
	MYFLT T = FL(1.0) / sr; 
...
typedef struct 
{
	...
	/* input, source, listener, hrtf files, default room, [fadelength, sr, order, threed, headrot, roomsize, wall high and low absorb coeffs, gain for 3 band pass, same for floor and ceiling] */ 
	MYFLT *in, *srcx, *srcy, *srcz, *lstnrx, *lstnry, *lstnrz, *ifilel, *ifiler, 
		*idefroom, *ofade, *osr, *porder, *othreed, *Oheadrot, 
		*ormx, *ormy, *ormz, *owlh, *owll, *owlg1, *owlg2, *owlg3, *oflh, *ofll, 
		*oflg1, *oflg2, *oflg3,*oclh, *ocll, *oclg1, *oclg2, *oclg3;
...
static int early_init(CSOUND *csound, early *p)
{
    ...
    if (UNLIKELY((fpl = csound->ldmemfile2withCB(csound, filel, CSFTYPE_FLOATS_BINARY, swap4bytes)) == NULL)) 
		return
			csound->InitError(csound, Str("\n\n\nCannot load left data file, exiting\n\n"));

    if (UNLIKELY((fpr = csound->ldmemfile2withCB(csound, filer, CSFTYPE_FLOATS_BINARY, swap4bytes)) == NULL))
		return
	...
	/* setup wall coeffs: walls: plasterboard, ceiling: painted plaster, floor: carpet 
	...
	/* read values if they exist, use medium if not valid (must be at least a 2*2*2 room! */ 
	else
	{
	...
	if(threed)
	{
		for(i = 1; i <= order; i++)
		{
			impulses += (4 * i);		
			if(i <= (order - 1))
				/* sources = 2d impulses for order, plus 2 * each preceding no of impulses 
	...
	if (!p->hrtflpadspec.auxp || p->hrtflpadspec.size < irlengthpad * impulses * sizeof(MYFLT))
		csound->AuxAlloc(csound, irlengthpad * impulses * sizeof(MYFLT), &p->hrtflpadspec);
	if (!p->hrtfrpadspec.auxp || p->hrtfrpadspec.size < irlengthpad * impulses * sizeof(MYFLT))
	...
	if (!p->hrtflpadspecold.auxp || p->hrtflpadspecold.size < irlengthpad * impulses * sizeof(MYFLT))
		csound->AuxAlloc(csound, irlengthpad * impulses * sizeof(MYFLT), &p->hrtflpadspecold);
	if (!p->hrtfrpadspecold.auxp || p->hrtfrpadspecold.size < irlengthpad * impulses * sizeof(MYFLT))
		csound->AuxAlloc(csound, irlengthpad * impulses * sizeof(MYFLT), &p->hrtfrpadspecold);
	...
	if (!p->currentphasel.auxp || p->currentphasel.size < irlength * impulses * sizeof(MYFLT))
		csound->AuxAlloc(csound, irlength * impulses * sizeof(MYFLT), &p->currentphasel);
	if (!p->currentphaser.auxp || p->currentphaser.size < irlength * impulses * sizeof(MYFLT))
	...
	/* wall filter quality factor (4 octaves for required spread!: .2666667) 
	   (2 octaves = .6667 implies 125 - 500, cf 250, 500 - 2k, cf 1k, 2000 - 8000, cf 4k) 
	   (4 octaves = = .2666667 implies 62.5 - 1000, cf 250, 250 - 4000, cf 1k, 1000 - 16k, cf 4k */
	p->q = FL(.2666667);
...
static int early_process(CSOUND *csound, early *p)
{
	...
	/* only update if relative source updates! improves speed in static sources by a factor of 2-3! */
	if(srcx != p->srcxk || srcy != p->srcyk || srcz != p->srczk || lstnrx != p->lstnrxk || lstnry != p->lstnryk || lstnrz != p->lstnrzk)
	{
		p->srcxk = srcx;
		p->srcyk = srcy;
		p->srczk = srcz;
		...
		for(xc = -order; xc <= order; xc++)	
		{
			for(yc = abs(xc) - order; yc <= order - abs(xc); yc++)
			{
				...
				for(zc = lowz; zc <= highz; zc++)
				{
					...
					dist[M] = FL(sqrt(SQUARE(tempsrcx[M] - lstnrx) + SQUARE(tempsrcy[M] - lstnry) + SQUARE(tempsrcz[M] - lstnrz)));
					...
					/* high amp value may cause clipping on early reflections...reduce overall amp if so...*/
	...
	for(j=0;j<n;j++) 
	{
		...
		for(M = 0; M < impulses; M++)
		{
			...
			outltot += dell[pos] + frac*(dell[(pos + 1 < (maxdelsamps + skipdel[M]) ? pos + 1 : skipdel[M])] - dell[pos]);
			outrtot += delr[pos] + frac*(delr[(pos + 1 < (maxdelsamps + skipdel[M]) ? pos + 1 : skipdel[M])] - delr[pos]);
			...
			for(xc = -order; xc <= order; xc++)	
			{
				for(yc = abs(xc) - order; yc <= order - abs(xc); yc++)
				{
					...
					for(zc = lowz; zc <= highz; zc++)
					{
						...
						if(srcx != p->srcxv || srcy != p->srcyv || srcz != p->srczv || lstnrx != p->lstnrxv || lstnry != p->lstnryv || lstnrz != p->lstnrzv || rotate != p->rotatev)
						{
						/* if first process complete (128 samps in) and source is too close to listener: warning, do not process */
						/* duda and martens range dependence jasa 98: 5 times radius: near field...hrtf changes! */
						...
						else
						{
							...
							/* xy point will be same as source, z same as listener: a workable triangle */ 
							newpntx = tempsrcx[M];
							newpnty = tempsrcy[M];
							newpntz = lstnrz;

							/* ab: source to listener, ac: source to new point under/over source, bc listener to new point */
							/* a = source, b = listener, c = new point */
							ab = FL(sqrt(SQUARE(tempsrcx[M] - lstnrx) + SQUARE(tempsrcy[M] - lstnry) + SQUARE(tempsrcz[M] - lstnrz)));
							ac = FL(sqrt(SQUARE(tempsrcx[M] - newpntx) + SQUARE(tempsrcy[M] - newpnty) + SQUARE(tempsrcz[M] - newpntz)));
							bc = FL(sqrt(SQUARE(lstnrx - newpntx) + SQUARE(lstnry - newpnty) + SQUARE(lstnrz - newpntz)));

							/* elev: when bc == 0 -> source + listener at same x,y point 
							   (may happen in first run, checked after that)
							   angle = 0, elev = 0 if at same point,
							...
							{
								/* cosine rule */
								coselev = FL((SQUARE(bc) + SQUARE(ab) - SQUARE(ac)) / (2.0 * ab * bc));
							...
							/* crossfade happens if index changes:nearest measurement changes, 1st step: store old values */ 
							if (oldelevindex[M] != elevindex || oldangleindex[M] != angleindex)
							{
								if(initialfade > irlength)
								{
									...
									{
										hrtflpadspecold[irlengthpad * M + i] = hrtflpadspec[irlengthpad * M + i];				
										hrtfrpadspecold[irlengthpad * M + i] = hrtfrpadspec[irlengthpad * M + i];
									}
								}
							...
							/* 0hz and Nyq */ 
							magllow = FL(fabs(lowl1[0])) + (FL(fabs(lowl2[0]) - fabs(lowl1[0]))) * angleindex2per;
							maglhigh = FL(fabs(highl1[0])) + (FL(fabs(highl2[0]) - fabs(highl1[0]))) * angleindex4per;
							magrlow = FL(fabs(lowr1[0])) + (FL(fabs(lowr2[0]) - fabs(lowr1[0]))) * angleindex2per;
							magrhigh = FL(fabs(highr1[0])) + (FL(fabs(highr2[0]) - fabs(highr1[0]))) * angleindex4per;
							...
							magllow = FL(fabs(lowl1[1])) + (FL(fabs(lowl2[1]) - fabs(lowl1[1]))) * angleindex2per;
							maglhigh = FL(fabs(highl1[1])) + (FL(fabs(highl2[1]) - fabs(highl1[1]))) * angleindex4per;
							magrlow = FL(fabs(lowr1[1])) + (FL(fabs(lowr2[1]) - fabs(lowr1[1]))) * angleindex2per;
							magrhigh = FL(fabs(highr1[1])) + (FL(fabs(highr2[1]) - fabs(highr1[1]))) * angleindex4per;
							magl = magllow + (maglhigh - magllow) * elevindexhighper;
							magr = magrlow + (magrhigh - magrlow) * elevindexhighper;
							...
							/* x axis, wall1 (left) */ 
							wallreflections = (int)abs((int)(xc * .5 - .25 + (.25 * pow(-1.0, xc))));
							/* wall2, x (right) */
							wallreflections += (int)abs((int)(xc * .5 + .25 - (.25 * pow(-1.0, xc))));
							/* yaxis, wall3 (bottom) */ 
							wallreflections += (int)abs((int)(yc * .5 - .25 + (.25 * pow(-1.0, yc))));
							/* yaxis, wall 4 (top) */
							wallreflections += (int)abs((int)(yc * .5 + .25 - (.25 * pow(-1.0, yc))));
							if(threed)
							{
								/* floor (negative z) */
								floorreflections = (int)abs((int)(zc * .5 - .25 + (.25 * pow(-1.0, zc))));
								/* ceiling (positive z) */
								ceilingreflections = (int)abs((int)(zc * .5 + .25 - (.25 * pow(-1.0, zc))));
							}

							/* fixed parameters on bands etc (to limit no of inputs), but these could trivially be variable */
							...
							/* if processing was continuous, would need separate mem for each filter, store for next run etc...*/
							for(i = 0; i < wallreflections; i++)
							{
								delsinglel = delsingler = FL(0.0);
								filter(hrtflinterp, p->wallcoefhigh, p->wallcoeflow, &delsinglel, irlength, sr);
								filter(hrtfrinterp, p->wallcoefhigh, p->wallcoeflow, &delsingler, irlength, sr);
						...
						csound->RealFFTMult(csound, outlspec, hrtflpad, inbufpad, irlengthpad, FL(1.0));
						csound->RealFFTMult(csound, outrspec, hrtfrpad, inbufpad, irlengthpad, FL(1.0));
						...
						{	
							...
							/* convolution */
							csound->RealFFTMult(csound, outlspecold, hrtflpadold, inbufpad, irlengthpad, FL(1.0));
							csound->RealFFTMult(csound, outrspecold, hrtfrpadold, inbufpad, irlengthpad, FL(1.0));
						...
						if(crossout)		
							for(i = 0; i < irlength; i++)
							{
								predell[i + M * irlength] = (outlspecold[i] + (i < overlapsize ? overlaplold[i] : FL(0.0))) * FL(1. - (FL(l[M]) / fadebuffer)) +
								(outlspec[i] + (i < overlapsize ? overlapl[i] : FL(0.0))) * (FL(l[M]) / fadebuffer);
								predelr[i + M * irlength] = (outrspecold[i] + (i < overlapsize ? overlaprold[i] : FL(0.0))) * FL(1. - (FL(l[M]) / fadebuffer)) +
								(outrspec[i] + (i < overlapsize ? overlapr[i] : FL(0.0))) * (FL(l[M]) / fadebuffer);
								l[M]++;
							}
						else
							for(i = 0; i < irlength; i++)
							{					
								predell[i + M * irlength] = outlspec[i] + (i < overlapsize ? overlapl[i] : FL(0.0));
								predelr[i + M * irlength] = outrspec[i] + (i < overlapsize ? overlapr[i] : FL(0.0));
							}
		...
		counter = 0;
		/* need to store these values here, as storing them after check would not allow each impulse to be processed! */		
		p->srcxv = srcx;
		p->srcyv = srcy;
		p->srczv = srcz;
		p->lstnrxv = lstnrx;
