...
MYFLT FormSwep_tick(OPDS *pp, FormSwep *p,
                    MYFLT sample) /* Perform Filter Operation */
{
  MYFLT temp;
...
static MYFLT Samp_tick(Wave *p) {
  ...
  temp1 = temp + 1;
  if (UNLIKELY(temp1 == (int32_t)p->wave->flen))
    temp1 = 0; /* Wrap!! */
...
#ifdef DEBUG
      csound->Message(csound, "Attack: (%d, %d), alpha=%f\t", itemp, itemp + 1,
                      alpha);
#endif
