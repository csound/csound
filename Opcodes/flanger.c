...
static int32_t flanger(CSOUND *csound, FLANGER *p) {
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    while (fv1 < 0)
      fv1 += maxdelay;
    while (fv1 >= maxdelay)
      fv1 -= maxdelay; /* Is this necessary? JPff */
    v1 = (int32)fv1;
...
static int32_t wguide2set(CSOUND *csound, WGUIDE2 *p) {
  ...
  if (UNLIKELY(p->xdel1cod != p->xdel2cod))
    return csound->InitError(csound, "%s",
                             Str("wguide2 xfreq1 and xfreq2 arguments must"
...
static int32_t wguide2(CSOUND *csound, WGUIDE2 *p) {
  ...
  if (p->xdel1cod) { /* delays change at audio-rate */
    for (n = offset; n < nsmps; n++) {
      MYFLT fd1 = *freq_del1++;
      MYFLT fd2 = *freq_del2++;
      buf1[indx1] = buf2[indx2] = in[n] + old_out * (feedback1 + feedback2);
      ...
      while (fv1_2 < 0)
        fv1_2 += p->maxd;
      fv2_1 = (fv1_1 < maxdM1) ? fv1_1 + 1
                               : 0; /*Find next sample for interpolation */
      fv2_2 = (fv1_2 < maxdM1) ? fv1_2 + 1
                               : 0; /*Find next sample for interpolation */
      v1_1 = (int32)fv1_1;
      v1_2 = (int32)fv1_2;
      out_delay1 =
          buf1[v1_1] + (fv1_1 - v1_1) * (buf1[(int32)fv2_1] - buf1[v1_1]);
      out_delay2 =
          buf2[v1_2] + (fv1_2 - v1_2) * (buf2[(int32)fv2_2] - buf2[v1_2]);
      if (UNLIKELY(++indx1 == p->maxd))
        indx1 = 0; /* Advance current pointer */
      if (UNLIKELY(++indx2 == p->maxd))
...
static OENTRY localops[] = {
    {"flanger", S(FLANGER), 0, 3, "a", "aakvo", (SUBR)flanger_set,
     (SUBR)flanger},
    {"wguide1", S(WGUIDE1), 0, 3, "a", "axkk", (SUBR)wguide1set, (SUBR)wguide1},
    {"wguide2", S(WGUIDE2), 0, 3, "a", "axxkkkk", (SUBR)wguide2set,
     (SUBR)wguide2}};
