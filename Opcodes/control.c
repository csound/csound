...
static CS_NOINLINE CONTROL_GLOBALS *get_globals_(CSOUND *csound) {
  CONTROL_GLOBALS *p;
  p = (CONTROL_GLOBALS *)csound->QueryGlobalVariable(csound, "controlGlobals_");
  if (p != NULL)
    return p;
...
static inline CONTROL_GLOBALS *get_globals(CSOUND *csound,
                                           CONTROL_GLOBALS **p) {
  if (*p == NULL)
...
static int32_t kill_wish(CSOUND *csound, CONTROL_GLOBALS *p) {
  ...
  kill(p->wish_pid, 9);
  if (p->values != NULL)
    csound->Free(csound, p->values);
  if (p->minvals != NULL)
    csound->Free(csound, p->minvals);
  if (p->maxvals != NULL)
    csound->Free(csound, p->maxvals);
  if (p->buttons != NULL)
    csound->Free(csound, p->buttons);
  if (p->checks != NULL)
    csound->Free(csound, p->checks);
  fclose(p->wish_cmd);
  fclose(p->wish_res);
  return OK;
}
...
static void ensure_slider(CONTROL_GLOBALS *p, int32_t n) {
  ...
  if (n > p->max_sliders) {
    int32_t i, nn = n + 1;
    p->values = (int32_t *)p->csound->ReAlloc(p->csound, p->values,
                                              nn * sizeof(int32_t));
    p->minvals = (int32_t *)p->csound->ReAlloc(p->csound, p->minvals,
                                               nn * sizeof(int32_t));
    p->maxvals = (int32_t *)p->csound->ReAlloc(p->csound, p->maxvals,
                                               nn * sizeof(int32_t));
...
static int32_t ocontrol_(CSOUND *csound, SCNTRL *p, int32_t istring) {
  ...
  case 4: {
    char buffer[100];
    if (istring) {
      csound->strarg2name(csound, buffer, ((STRINGDAT *)p->val)->data,
                          "Control ", istring);
    } else
...
static int32_t textflash_(CSOUND *csound, TXTWIN *p, int32_t istring) {
  ...
  if (istring) {
    ...
    csound->strarg2name(csound, buffer, csound->GetString(csound, *p->val), "",
                        1);
  } else {
...
static int32_t textflash_S(CSOUND *csound, TXTWIN *p) {
