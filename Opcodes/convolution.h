...
static inline int32_t conv_array_channels(MYFLT channels)
{
    double n = (double) channels;
    return n >= 1 && n <= INT32_MAX && n == floor(n) ? (int32_t)n : 0;
}
                                         ...
                                         const CONV_OUTPUT *out)
{
    ...
    if (a != NULL &&
        (a->data == NULL || a->dimensions != 1 || a->sizes == NULL ||
         ...
         a->arrayMemberSize < h->insdshead->ksmps * sizeof(MYFLT) ||
         a->arrayMemberSize % sizeof(MYFLT) != 0 ||
...
static inline MYFLT *conv_output_channel(const CONV_OUTPUT *out, int32_t ch)
{
    return out->array != NULL
        ? out->array->data + (size_t)ch *
                            (out->array->arrayMemberSize / sizeof(MYFLT))
        : out->scalar[ch];
}
