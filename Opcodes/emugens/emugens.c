...
static int32_t linlin1_perf(CSOUND *csound, LINLIN1 *p) {
  MYFLT x0 = *p->kx0;
  MYFLT y0 = *p->ky0;
  MYFLT x = *p->kx;
  MYFLT x1 = *p->kx1;
...
static int32_t linlinarr1_perf(CSOUND *csound, LINLINARR1 *p) {
  MYFLT x0 = *p->kx0;
  MYFLT y0 = *p->ky0;
  MYFLT x1 = *p->kx1;
  MYFLT y1 = *p->ky1;
...
static int32_t linlinarr1_i(CSOUND *csound, LINLINARR1 *p) {
...
static int32_t blendarray_init(CSOUND *csound, BLENDARRAY *p) {
...
static int32_t blendarray_perf(CSOUND *csound, BLENDARRAY *p) {
  MYFLT x0 = *p->kx0;
  MYFLT x1 = *p->kx1;
  MYFLT x = *p->kx;
...
static int32_t blendarray_i(CSOUND *csound, BLENDARRAY *p) {
...
/*
    ...
    given x between x0 and x1, find y between y0 and y1 with cosine
   interpolation

    The order of ky0 and ky1 are given first because it is often used in
   contexts where kx is defined between 0 and 1

*/

static int32_t lincos_perf(CSOUND *csound, LINLIN1 *p) {
  MYFLT x0 = *p->kx0;
  MYFLT y0 = *p->ky0;
  MYFLT x = *p->kx;
  MYFLT x1 = *p->kx1;
...
static inline MYFLT mtof_func(MYFLT midi, MYFLT a4) {
...
static int32_t ftom_arr(CSOUND *csound, PITCHCONV_ARR *p) {
...
static int32_t ftom_arr_init(CSOUND *csound, PITCHCONV_ARR *p) {
  p->freqA4 = csound->GetA4(csound);
  p->rnd = (int)*p->irnd;
...
static int32_t mtof_arr(CSOUND *csound, PITCHCONV_ARR *p) {
  MYFLT x, *indata, *outdata;
  int32_t i;
  if (p->skip) {
    p->skip = 0;
...
static int32_t mtof_arr_init(CSOUND *csound, PITCHCONV_ARR *p) {
  p->freqA4 = csound->GetA4(csound);
  tabinit(csound, p->outarr, p->inarr->sizes[0]);
  p->skip = 0;
  mtof_arr(csound, p);
...
/*
static inline int32_t bpf_find_multidim(MYFLT x, MYFLT *xs, int32_t xslen,
int32_t step, int32_t lastidx) {
    // xslen: size of xs (number of frames, not size of array)
    // step: normally 1, can be more for the case where xs and ys (and possibly
zs, etc)
...
{ return lastidx;
    ...
    // check next idx
    if(lastidx < xslen - 2 && xs[(lastidx+1)*step] <= x && x <
xs[(lastidx+2)*step]) { return lastidx+1;
    }
...
static inline int32_t bpfx_find(MYFLT **data, MYFLT x, int32_t datalen,
                                int32_t lastidx) {
  ...
  if (x >= *data[datalen - 2])
    return -2;
  if (lastidx >= 0 && lastidx < datalen - 4 && *data[lastidx] <= x &&
      x < *data[lastidx + 2])
    return lastidx;
  // bin search
...
static inline int64_t bpfarr_find(MYFLT x, MYFLT *xs, int64_t xslen,
                                  int64_t lastidx) {
  ...
  if (lastidx >= 0 && lastidx < xslen - 2 && xs[lastidx] <= x &&
      x < xs[lastidx + 1]) {
    return lastidx;
  }
...
static int32_t bpf_a_am_init(CSOUND *csound, BPF_a_am *p) {
  ...
  if (datalen % 2)
    return INITERRF(
        Str("bpf: data length should be even (pairs of x, y), got %d"),
        datalen);
  if (datalen < 4)
    return INITERRF(Str("At least two pairs are needed, got %d"), datalen % 2);
  if (datalen >= BPF_MAXPOINTS)
...
static MYFLT ntomfunc(CSOUND *csound, char *note) {
  ...
  if (pcidx < 0 || pcidx >= 7) {
    csound->Message(
        csound, Str("expecting a char between A and G, but got %c\n"), n[1]);
    return FAIL;
  }
  ...
  if (rest > 0) {
    ...
    if (rest == 1) {
      ...
      csound->Message(csound,
                      Str("format not understood, note: "
                          "%s, notelen: %d\n"),
                      n, notelen);
      return FAIL;
    }
...
static int32_t cmparray1_init(CSOUND *csound, Cmp_array1 *p) {
  int32_t N = p->in->sizes[0];
  tabinit(csound, p->out, N);
  int32_t mode = op2mode(p->op->data, p->op->size - 1);
  if (mode == -1) {
...
static int32_t cmparray2_init(CSOUND *csound, Cmp_array2 *p) {
...
static int32_t cmp2array1_init(CSOUND *csound, Cmp2_array1 *p) {
  int32_t N = p->in->sizes[0];
  tabinit(csound, p->out, N);
...
static int32_t cmparray1_k(CSOUND *csound, Cmp_array1 *p) {
...
static int32_t cmparray1_i(CSOUND *csound, Cmp_array1 *p) {
  cmparray1_init(csound, p);
  return cmparray1_k(csound, p);
}

static int32_t cmp2array1_k(CSOUND *csound, Cmp2_array1 *p) {
  int32_t L = p->in->sizes[0];
  ARRAY_ENSURESIZE_PERF(csound, p->out, L);
...
static int32_t cmp2array1_i(CSOUND *csound, Cmp2_array1 *p) {
  cmp2array1_init(csound, p);
  return cmp2array1_k(csound, p);
}

static int32_t cmparray2_k(CSOUND *csound, Cmp_array2 *p) {
...
static int32_t cmparray2_i(CSOUND *csound, Cmp_array2 *p) {
...
static int32_t tabslice_init(CSOUND *csound, TABSLICE *p) {
  FUNC *ftpsrc, *ftpdst;
...
static int32_t tabslice_k(CSOUND *csound, TABSLICE *p) {
  IGN(csound);
...
static int32_t tabslice_allk(CSOUND *csound, TABSLICE *p) {
...
static int32_t tabslice_i(CSOUND *csound, TABSLICE *p) {
...
static int32_t ftset_init(CSOUND *csound, FTSET *p) {
  IGN(csound);
  p->lastTabnum = -1;
  return OK;
}

static int32_t ftset_common(CSOUND *csound, FTSET *p) {
  IGN(csound);
  FUNC *tab = p->tab;
...
static int32_t ftset_k(CSOUND *csound, FTSET *p) {
...
static int32_t ftset_i(CSOUND *csound, FTSET *p) {
...
static int32_t arrayreshape(CSOUND *csound, ARRAYRESHAPE *p) {
  ARRAYDAT *a = p->in;
  ...
  if (numrows < 0 || numcols < 0) {
    return INITERR(
        Str("reshapearray: neither numcols nor numrows can be negative"));
  }

  if (dims > 2) {
    return INITERR(
        Str("Arrays of more than 2 dimensions are not supported yet"));
  }
...
static int32_t arrayprint_init(CSOUND *csound, ARRAYPRINTK *p) {
  ...
  p->printfmt = (p->Sfmt == NULL || strlen(p->Sfmt->data) < 2) ? default_fmt
                                                               : p->Sfmt->data;

  if (strstr(p->printfmt, "%d") != NULL) {
    str_replace(p->fmtdata, p->printfmt, "%d", "%.0f");
    fflush(stdout);
...
static int32_t arrayprint_init_notrig(CSOUND *csound, ARRAYPRINT *p) {
  if (p->in->arrayType->varTypeName[0] == 'S' && p->in->dimensions > 1)
    return INITERR(Str("cannot print multidimensional string arrays"));
  if (p->in->dimensions > 2)
    return INITERRF(Str("only 1-D and 2-D arrays supported, got %d dimensions"),
...
// print a string arry
static int32_t arrprint_str(CSOUND *csound, ARRAYDAT *arr, const char *fmt,
                            const char *label) {
  int32_t i;
...
static int32_t arrprint(CSOUND *csound, ARRAYDAT *arr, const char *fmt,
                        const char *label) {
  MYFLT *in = arr->data;
...
static inline int32_t arrprint_(CSOUND *csound, ARRAYDAT *arr, const char *fmt,
                                const char *label) {
...
static int32_t arrayprint_perf(CSOUND *csound, ARRAYPRINTK *p) {
...
static int32_t arrayprint_perf_notrig(CSOUND *csound, ARRAYPRINT *p) {
  return arrprint_(csound, p->in, p->printfmt, p->label);
}

static int32_t arrayprint_i(CSOUND *csound, ARRAYPRINT *p) {
  const char *label = p->Slabel != NULL ? p->Slabel->data : NULL;
  return arrprint(csound, p->in, default_printfmt, label);
}

static int32_t arrayprintf_i(CSOUND *csound, ARRAYPRINT *p) {
  char tmpfmt[256];
  const char *fmt;
...
static int32_t ftprint_perf(CSOUND *csound, FTPRINT *p) {
  ...
  char currline[ARRPRINT_MAXLINE];
  uint32_t i, elemsprinted = 0, charswritten = 0, startidx = start;
...
static int32_t array_binop_init(CSOUND *csound, BINOP_AAA *p) {
  int32_t numitems = 1;
  int32_t i;
...
static int32_t array_or(CSOUND *csound, BINOP_AAA *p) {
...
static int32_t array_and(CSOUND *csound, BINOP_AAA *p) {
  int32_t numitems = p->numitems;
  ARRAY_ENSURESIZE_PERF(csound, p->out, numitems);
...
static int32_t ftexists_init(CSOUND *csound, FTEXISTS *p) {
  int ifn = (int)*p->ifn;
  if (ifn == 0) {
...
static int32_t lastcycle_init(CSOUND *csound, LASTCYCLE *p) {
  MYFLT p3 = p->h.insdshead->p3.value;
  p->numcycles = p3 < 0 ? 0 : (int)(p->h.insdshead->offtim * CS_EKR + 0.5);
  p->extracycles = p->h.insdshead->xtratim;
  if (p->extracycles == 0) {
    p->h.insdshead->xtratim = 1;
    p->extracycles = 1;
    // MSG(Str("lastcycle: adding an extra cycle to the duration of the
    // event\n"));
  }
...
static int32_t lastcycle(CSOUND *csound, LASTCYCLE *p) {
  IGN(csound);
  if (p->fired == 1) {
...
static int32_t stripl(CSOUND *csound, STR1_1 *p) {
  char *str = p->in->data;
  size_t idx0;
...
static int32_t stripr(CSOUND *csound, STR1_1 *p) {
  // Trim trailing space
  char *str = p->in->data;
...
static int32_t stripside(CSOUND *csound, STR1_1 *p) {
  if (p->which->size < 2)
...
static int32_t strstrip(CSOUND *csound, STR1_1 *p) {
  int startidx;
...
int32_t printsk_init(CSOUND *csound, PRINTLN *p) {
  ...
  if (p->buf.size < bufsize || p->strseg.size < maxSegmentSize) {
    ...
    p->allocatedBuf = 1;
    csound->RegisterResetCallback(
        csound, p, (int32_t(*)(CSOUND *, void *))(println_reset));
  } else {
    p->allocatedBuf = 0;
  }
  p->newline = 0;
...
static int32_t
sprintf_opcode_(CSOUND *csound, PRINTLN *p, /* opcode data structure pointer */
  ...
  while (1) {
    ...
    if (segwaiting != NULL) {
      ...
      case 's':
        ...
        if ((((STRINGDAT *)parm)->size + strlen(strseg)) >=
            (uint32_t)maxChars) {
...
int32_t println_perf(CSOUND *csound, PRINTLN *p) {
  int32_t err =
      sprintf_opcode_(csound, p, &p->buf, (char *)p->sfmt->data, p->fmtlen,
...
static OENTRY emugens_localops[] = {
    ...
    {"linlin", S(BLENDARRAY), 0, 3, "k[]", "kk[]k[]OP", (SUBR)blendarray_init,
     (SUBR)blendarray_perf},
    ...
    {"mtof", S(PITCHCONV_ARR), 0, 3, "k[]", "k[]", (SUBR)mtof_arr_init,
     (SUBR)mtof_arr},
    ...
    {"ftom", S(PITCHCONV_ARR), 0, 3, "k[]", "k[]o", (SUBR)ftom_arr_init,
     (SUBR)ftom_arr},
    {"ftom", S(PITCHCONV_ARR), 0, 1, "i[]", "i[]o", (SUBR)ftom_arr_init,
     (SUBR)ftom_arr},
    ...
    {"bpf.K_KM", S(BPF_K_Km), 0, 3, "k[]", "k[]M", (SUBR)bpf_K_Km_init,
     (SUBR)bpf_K_Km_kr},
    {"bpfcos.K_KM", S(BPF_K_Km), 0, 3, "k[]", "k[]M", (SUBR)bpf_K_Km_init,
     (SUBR)bpfcos_K_Km_kr},

    {"bpf.a_aM", S(BPF_a_am), 0, 3, "a", "aM", (SUBR)bpf_a_am_init,
     (SUBR)bpf_a_am_kr},
    {"bpfcos.a_aM", S(BPF_a_am), 0, 3, "a", "aM", (SUBR)bpf_a_am_init,
     (SUBR)bpfcos_a_am_kr},

    {"bpf.k_kKK", S(BPF_k_kKK), 0, 3, "k", "kk[]k[]", (SUBR)bpf_k_kKK_init,
     (SUBR)bpf_k_kKK_kr},
    {"bpfcos.k_kKK", S(BPF_k_kKK), 0, 3, "k", "kk[]k[]", (SUBR)bpf_k_kKK_init,
     (SUBR)bpfcos_k_kKK_kr},

    {"bpf.k_kII", S(BPF_k_kKK), 0, 3, "k", "ki[]i[]", (SUBR)bpf_k_kKK_init,
     (SUBR)bpf_k_kKK_kr},
    {"bpfcos.k_kII", S(BPF_k_kKK), 0, 3, "k", "ki[]i[]", (SUBR)bpf_k_kKK_init,
     (SUBR)bpfcos_k_kKK_kr},

    {"bpf.a_aKK", S(BPF_k_kKK), 0, 3, "a", "ak[]k[]", (SUBR)bpf_k_kKK_init,
     (SUBR)bpf_a_aKK_kr},
    {"bpfcos.a_aKK", S(BPF_k_kKK), 0, 3, "a", "ak[]k[]", (SUBR)bpf_k_kKK_init,
     (SUBR)bpfcos_a_aKK_kr},

    {"bpf.a_aII", S(BPF_k_kKK), 0, 3, "a", "ai[]i[]", (SUBR)bpf_k_kKK_init,
     (SUBR)bpf_a_aKK_kr},
    {"bpfcos.a_aII", S(BPF_k_kKK), 0, 3, "a", "ai[]i[]", (SUBR)bpf_k_kKK_init,
     (SUBR)bpfcos_a_aKK_kr},

    {"bpf.i_iII", S(BPF_k_kKK), 0, 1, "i", "ii[]i[]", (SUBR)bpf_k_kKK_ir},
    {"bpfcos.i_iII", S(BPF_k_kKK), 0, 1, "i", "ii[]i[]", (SUBR)bpfcos_k_kKK_ir},

    {"bpf.kk_kKKK", S(BPF_kk_kKKK), 0, 3, "kk", "kk[]k[]k[]",
     (SUBR)bpf_kk_kKKK_init, (SUBR)bpf_kk_kKKK_kr},
    // TODO

    {"bpf.kk_kIII", S(BPF_kk_kKKK), 0, 3, "kk", "ki[]i[]i[]",
     (SUBR)bpf_kk_kKKK_init, (SUBR)bpf_kk_kKKK_kr},
    // TODO

    {"bpf.ii_iIII", S(BPF_kk_kKKK), 0, 1, "ii", "ii[]i[]i[]",
     (SUBR)bpf_kk_kKKK_ir},
    // TODO
    ...
    {"cmp", S(Cmp_array1), 0, 3, "k[]", "k[]Sk", (SUBR)cmparray1_init,
     (SUBR)cmparray1_k},
    {"cmp", S(Cmp_array1), 0, 1, "i[]", "i[]Si", (SUBR)cmparray1_i},

    {"cmp", S(Cmp_array2), 0, 3, "k[]", "k[]Sk[]", (SUBR)cmparray2_init,
     (SUBR)cmparray2_k},
    {"cmp", S(Cmp_array2), 0, 1, "i[]", "i[]Si[]", (SUBR)cmparray2_i},
    {"cmp", S(Cmp2_array1), 0, 3, "k[]", "kSk[]Sk", (SUBR)cmp2array1_init,
     (SUBR)cmp2array1_k},
    {"cmp", S(Cmp2_array1), 0, 1, "i[]", "iSi[]Si", (SUBR)cmp2array1_i},

    {"##or", S(BINOP_AAA), 0, 3, "k[]", "k[]k[]", (SUBR)array_binop_init,
     (SUBR)array_or},
    {"##and", S(BINOP_AAA), 0, 3, "k[]", "k[]k[]", (SUBR)array_binop_init,
     (SUBR)array_and},
    {"reshapearray", S(ARRAYRESHAPE), 0, 1, "", ".[]io", (SUBR)arrayreshape},

    {"ftslicei", S(TABSLICE), TB, 1, "", "iioop", (SUBR)tabslice_i},

    {"ftslice.perf", S(TABSLICE), TB, 3, "", "iiOOP", (SUBR)tabslice_init,
     (SUBR)tabslice_k},

    {"ftslice.onlyperf", S(TABSLICE), TB, 2, "", "kkOOP", NULL,
     (SUBR)tabslice_allk},

    {"ftset.i", S(FTSET), TW, 1, "", "iioop", (SUBR)ftset_i},
    {"ftset.k", S(FTSET), TW, 3, "", "kkOOP", (SUBR)ftset_init, (SUBR)ftset_k},

    {"tab2array", S(TAB2ARRAY), TR, 3, "k[]", "iOOP", (SUBR)tab2array_init,
     (SUBR)tab2array_k},
    {"tab2array", S(TAB2ARRAY), TR, 1, "i[]", "ioop", (SUBR)tab2array_i},

    {"printarray", S(ARRAYPRINTK), 0, 3, "", "k[]J", (SUBR)arrayprint_init,
     (SUBR)arrayprint_perf},
    {"printarray", S(ARRAYPRINTK), 0, 3, "", "k[]kS", (SUBR)arrayprint_init,
     (SUBR)arrayprint_perf},
    {"printarray.k_notrig", S(ARRAYPRINT), 0, 3, "", "k[]S",
     (SUBR)arrayprint_init_notrig, (SUBR)arrayprint_perf_notrig},

    {"printarray", S(ARRAYPRINTK), 0, 3, "", "k[]kSS", (SUBR)arrayprint_init,
     (SUBR)arrayprint_perf},
    ...
    {"printarray", S(ARRAYPRINTK), 0, 3, "", "S[]J", (SUBR)arrayprint_init,
     (SUBR)arrayprint_perf},
    {"printarray", S(ARRAYPRINTK), 0, 3, "", "S[]kS", (SUBR)arrayprint_init,
     (SUBR)arrayprint_perf},
    ...
    {"ftprint", S(FTPRINT), TR, 3, "", "iPOOPo", (SUBR)ftprint_init,
     (SUBR)ftprint_perf},

    {"ftexists", S(FTEXISTS), TR, 1, "i", "i", (SUBR)ftexists_init},
    {"ftexists", S(FTEXISTS), TR, 3, "k", "k", (SUBR)ftexists_init,
     (SUBR)ftexists_init},
    {"lastcycle", S(LASTCYCLE), 0, 3, "k", "", (SUBR)lastcycle_init,
     (SUBR)lastcycle},
    {"strstrip.i_side", S(STR1_1), 0, 1, "S", "SS", (SUBR)stripside},
    {"strstrip.i", S(STR1_1), 0, 1, "S", "S", (SUBR)strstrip},
    {"println", S(PRINTLN), 0, 3, "", "SN", (SUBR)println_init,
     (SUBR)println_perf},
    {"printsk", S(PRINTLN), 0, 3, "", "SN", (SUBR)printsk_init,
     (SUBR)printsk_perf}};
