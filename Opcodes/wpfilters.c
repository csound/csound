...
static int32_t diode_ladder_init(CSOUND *csound, DIODE_LADDER *p) {
  IGN(csound);
  if (*p->skip == 0) {
    int32_t i;
    p->a[0] = 1.0;
...
static int32_t diode_ladder_perf(CSOUND *csound, DIODE_LADDER *p) {
...
static OENTRY wpfilters_localops[] = {
    {"zdf_1pole", sizeof(ZDF_1POLE), 0, 3, "a", "axOo", (SUBR)zdf_1pole_init,
     (SUBR)zdf_1pole_perf},
    {"zdf_1pole_mode", sizeof(ZDF_1POLE_MODE), 0, 3, "aa", "axo",
     (SUBR)zdf_1pole_mode_init, (SUBR)zdf_1pole_mode_perf},
    {"zdf_2pole", sizeof(ZDF_2POLE), 0, 3, "a", "axxOo", (SUBR)zdf_2pole_init,
     (SUBR)zdf_2pole_perf},
    {"zdf_2pole_mode", sizeof(ZDF_2POLE_MODE), 0, 3, "aaa", "axxo",
     (SUBR)zdf_2pole_mode_init, (SUBR)zdf_2pole_mode_perf},
    {"zdf_ladder", sizeof(ZDF_LADDER), 0, 3, "a", "axxo", (SUBR)zdf_ladder_init,
     (SUBR)zdf_ladder_perf},
    {"diode_ladder", sizeof(DIODE_LADDER), 0, 3, "a", "axxOPo",
     (SUBR)diode_ladder_init, (SUBR)diode_ladder_perf},
    {"K35_lpf", sizeof(K35_LPF), 0, 3, "a", "axxOPo", (SUBR)k35_lpf_init,
     (SUBR)k35_lpf_perf},
    {"K35_hpf", sizeof(K35_LPF), 0, 3, "a", "axxOPo", (SUBR)k35_hpf_init,
     (SUBR)k35_hpf_perf},
};
