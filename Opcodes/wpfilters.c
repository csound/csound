...
static int32_t zdf_1pole_mode_perf(CSOUND* csound, ZDF_1POLE_MODE* p) {
    ...
    cs_double T = CS_ONEDSR;
    cs_double Tdiv2 = T / 2.0;
    cs_double two_div_T = 2.0 / T;

    int32_t cutoff_arate = IS_ASIG_ARG(p->cutoff);

    cs_float cutoff = cutoff_arate ? 0.0 : *p->cutoff;

    if (UNLIKELY(offset)) {
      memset(p->outlp, '\0', offset*sizeof(cs_float));
      memset(p->outhp, '\0', offset*sizeof(cs_float));
    }
    if (UNLIKELY(early)) {
      nsmps -= early;
      memset(&p->outlp[nsmps], '\0', early*sizeof(cs_float));
      memset(&p->outhp[nsmps], '\0', early*sizeof(cs_float));
    }
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (cutoff != last_cut) {
        ...
        cs_double wd = TWOPI * cutoff;
        cs_double wa = two_div_T * tan(wd * Tdiv2);
        cs_double g = wa * Tdiv2;
        G = g / (1.0 + g);
      }
...
static int32_t zdf_1pole_perf(CSOUND* csound, ZDF_1POLE* p) {
    ...
    cs_double Tdiv2 = T / 2.0;
    cs_double two_div_T = 2.0 / T;
    int32_t mode = CS_FLOAT2LONG(*p->mode);
...
static int32_t zdf_2pole_mode_perf(CSOUND* csound, ZDF_2POLE_MODE* p) {
    ...
    cs_float cutoff = *p->cutoff;
    cs_float q = *p->q;

    if (UNLIKELY(offset)) {
      memset(p->outlp, '\0', offset*sizeof(cs_float));
      memset(p->outhp, '\0', offset*sizeof(cs_float));
      memset(p->outbp, '\0', offset*sizeof(cs_float));
    }
    if (UNLIKELY(early)) {
      nsmps -= early;
      memset(&p->outlp[nsmps], '\0', early*sizeof(cs_float));
      memset(&p->outhp[nsmps], '\0', early*sizeof(cs_float));
      memset(&p->outbp[nsmps], '\0', early*sizeof(cs_float));
    }
    for (n = offset; n < nsmps; n++) {
      ...
      cs_double in = p->in[n];
      cs_double hp = (in - (2.0 * R + g) * z1 - z2) / (1.0 + (2.0 * R * g) + g2);
      cs_double bp = g * hp + z1;
      cs_double lp = g * bp + z2;
...
static int32_t zdf_ladder_perf(CSOUND* csound, ZDF_LADDER* p) {
    ...
    cs_double last_cut = p->last_cut;
    cs_double last_q = p->last_q;
    cs_double k = p->last_k;
    cs_double g = p->last_g;
    cs_double G = p->last_G;
    cs_double G2 = p->last_G2;
    cs_double G3 = p->last_G3;
    cs_double GAMMA = p->last_GAMMA;
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      cs_double g_plus_1 = g + 1.0;

      cs_double S1 = z1 / g_plus_1;
      cs_double S2 = z2 / g_plus_1;
      cs_double S3 = z3 / g_plus_1;
      cs_double S4 = z4 / g_plus_1;

      cs_double S = (G3 * S1) + (G2 * S2) + (G * S3) + S4;
      cs_double u = (p->in[n] - k *  S) / (1 + k * GAMMA);
...
#endif
static inline cs_double diode_ladder_normalize(cs_double saturation, cs_double input,
                                            cs_double normalization)
{
...
static int32_t diode_ladder_perf(CSOUND* csound,
    ...
    cs_double G3 = p->G[2];
    cs_double G4 = p->G[3];
    cs_double beta1 = p->beta[0];
    cs_double beta2 = p->beta[1];
    cs_double beta3 = p->beta[2];
    cs_double beta4 = p->beta[3];
    cs_double delta1 = p->delta[0];
    cs_double delta2 = p->delta[1];
    cs_double delta3 = p->delta[2];
    cs_double epsilon1 = p->epsilon[0];
    cs_double epsilon2 = p->epsilon[1];
    cs_double epsilon3 = p->epsilon[2];
    cs_double gamma1 = p->gamma[0];
    cs_double gamma2 = p->gamma[1];
    cs_double gamma3 = p->gamma[2];
    cs_double SG1 = p->SG[0];
    cs_double SG2 = p->SG[1];
    cs_double SG3 = p->SG[2];
    cs_double SG4 = p->SG[3];
    cs_double GAMMA = p->GAMMA;
    cs_double SIGMA = p->SIGMA;
    cs_double alpha = p->last_alpha;
    cs_double last_cut = p->last_cut;
    ...
    cs_float cutoff = cutoff_arate ? 0.0 : *p->cutoff;
    cs_float k = k_arate ? 0.0 : *p->kval;
    cs_float nlp = *p->nlp;
    cs_double saturation = *p->saturation;
    cs_double normalization = 1.0;
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      //feedback inputs
      cs_double fb4 = beta4 * z4;
      cs_double fb3 = beta3 * (z3 + fb4 * delta3);
      cs_double fb2 = beta2 * (z2 + fb3 * delta2);

      //feedback process
      cs_double fbo1 = (beta1 * (z1 + fb2 * delta1));
      cs_double fbo2 = (beta2 * (z2 + fb3 * delta2));
      cs_double fbo3 = (beta3 * (z3 + fb4 * delta3));
      ...
      // form input to loop
      cs_double un = (in - k * SIGMA) / (1.0 + k * GAMMA);

      // 1st stage
      cs_double xin = un * gamma1 + fb2 + epsilon1 * fbo1;
      cs_double v = (a1 * xin - z1) * alpha;
      cs_double lp = v + z1;
      z1 = lp + v;
...
static int32_t k35_lpf_perf(CSOUND* csound, K35_LPF* p) {
    ...
    cs_double K = p->K;
    cs_double S35 = p->S35;
    cs_double alpha = p->alpha;
    cs_double lpf2_beta = p->lpf2_beta;
    cs_double hpf1_beta = p->hpf1_beta;
    ...
    int32_t nonlinear = CS_FLOAT2LONG(*p->nonlinear);
    cs_double saturation = *p->saturation;
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      // LPF1
      cs_double v1 = (in - z1) * G;
      cs_double lp1 = v1 + z1;
      z1 = lp1 + v1;

      cs_double u = alpha * (lp1 + S35);
...
static int32_t k35_hpf_perf(CSOUND* csound, K35_HPF* p) {
    ...
    cs_double S35 = p->S35;
    cs_double alpha = p->alpha;
    cs_double hpf2_beta = p->hpf2_beta;
    cs_double lpf1_beta = p->lpf1_beta;
    ...
    int32_t
      nonlinear = CS_FLOAT2LONG(*p->nonlinear);
    cs_double saturation = *p->saturation;
