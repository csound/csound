...
int squinewave_gen(CSOUND* csound, SQUINEWAVE *p)
{
    ...
    if (p->init_phase) {
        const double freq = max(freq_sig[0], 0.0);
        ...
        if (warped_phase < 1.0) {
            const double sweep_length = max(clip * midpoint, min_sweep);
            if (warped_phase < 0.5) {
            ...
            const double sweep_length = max(clip * (2.0 - midpoint), min_sweep);
            if (warped_phase < 1.5) {
        ...
        if (warped_phase >= 2.0 && phase >= 2.0)
        {
            if (hardsync_phase) {
                ...
                if (freq < Max_Warp_Freq) {
                    ...
                    const double midpoint = Clamp(skew, min_sweep, 2.0 - min_sweep);
                    const double next_sweep_length = max(clip * midpoint, min_sweep);
                    warped_phase = min(phase / next_sweep_length, Max_Warp);
                }
                else
                    warped_phase = phase;
            }
...
static OENTRY squinewave_localops[] = {
    { "squinewave", sizeof(SQUINEWAVE), 0, 5, "am", "aaaxoj", (SUBR)squinewave_init, NULL, (SUBR)squinewave_gen },
};
