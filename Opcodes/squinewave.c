...
typedef struct {
    OPDS h;
    cs_float *aout, *async_out, *acps, *aclip, *askew, *async_in, *iminsweep, *iphase;

    // phase and sweep_phase range 0-2.
    // This makes clip/skew into simple proportions
    cs_double phase;
    cs_double sweep_phase;
    cs_double hardsync_phase;
    cs_double hardsync_inc;
    int32_t neg_freq;

    // Const inited from environment
    cs_double Min_Sweep;
    cs_double Maxphase_By_sr;
    cs_double Max_Sweep_Freq;
    cs_double Max_Sync_Freq;
    cs_double Sync_Phase_Inc;

    cs_float *sync_sig;        // holds async_in if a-rate
    int32_t init_phase;
} SQUINEWAVE;

/* ================================================================== */

static inline int32_t find_sync(const cs_float* sync_sig, const uint32_t first,
                                const uint32_t last)
{
    uint32_t i;
    if (sync_sig == 0)
...
static void hardsync_init(SQUINEWAVE *p, const cs_double freq,
                          const cs_double sweep_phase)
{
...
static inline cs_float Clamp(const cs_float x, const cs_float minval, const cs_float maxval) {
...
int32_t squinewave_init(CSOUND* csound, SQUINEWAVE *p)
{
    ...
    if (p->Min_Sweep < 4.0 || p->Min_Sweep > sr * 0.01) {
      const cs_float minsweep_default = Clamp(sr / 3000.0, 8.0, sr * 0.01);
      if (p->Min_Sweep != 0.0) {
...
int32_t squinewave_gen(CSOUND* csound, SQUINEWAVE *p)
{
    ...
    if (UNLIKELY(ksmps_offset)) memset(p->aout, 0, ksmps_offset * sizeof(cs_float));
    if (UNLIKELY(ksmps_end < nsmps)) {
      memset(&p->aout[ksmps_end], 0, p->h.insdshead->ksmps_no_end * sizeof(cs_float));
    }

    const cs_double Min_Sweep = p->Min_Sweep;
    const cs_double Maxphase_By_sr = p->Maxphase_By_sr;
    const cs_double Max_Sweep_Freq = p->Max_Sweep_Freq;
    const cs_double Max_Sweep_Inc = 1.0 / Min_Sweep;
    const cs_double Max_Sync_Freq = p->Max_Sync_Freq;

    cs_float *aout = &p->aout[ksmps_offset];
    const cs_float * const freq_sig = p->acps;
    const cs_float * const clip_sig = p->aclip;
    const cs_float * const skew_sig = p->askew;

    cs_double phase = p->phase;
    cs_double sweep_phase = p->sweep_phase;
    int32_t neg_freq = p->neg_freq;

    cs_double hardsync_phase = p->hardsync_phase;
    cs_double hardsync_inc = p->hardsync_inc;
    ...
    if (p->init_phase && ksmps_offset < ksmps_end) {
      const cs_double freq = fabs(freq_sig[ksmps_offset]);
      const cs_double phase_inc = Maxphase_By_sr * freq;
      const cs_double min_sweep = phase_inc * Min_Sweep;
      const cs_double skew = 1.0 - Clamp(skew_sig[ksmps_offset], -1.0, 1.0);
      const cs_double clip = 1.0 - Clamp(clip_sig[ksmps_offset], 0.0, 1.0);
      const cs_double midpoint = Clamp(skew, min_sweep, 2.0 - min_sweep);
      ...
      if (sweep_phase < 1.0) {
        const cs_double sweep_length = fmax(clip * midpoint, min_sweep);
        if (sweep_phase < 0.5) {
          phase = sweep_length * (sweep_phase * 2.0);
          sweep_phase *= 2.0;
        }
        else {
          const cs_double flat_length = midpoint - sweep_length;
          phase = sweep_length + flat_length * ((sweep_phase - 0.5) * 2.0);
          sweep_phase = 1.0;
        }
      }
      else {
        const cs_double sweep_length = fmax(clip * (2.0 - midpoint), min_sweep);
        if (sweep_phase < 1.5) {
          phase = midpoint + sweep_length * ((sweep_phase - 1.0) * 2.0);
          sweep_phase = 1.0 + (sweep_phase - 1.0) * 2.0;
        }
        else {
          const cs_double flat_length = 2.0 - (midpoint + sweep_length);
    ...
    if (p->async_out)
      memset(p->async_out, 0, nsmps * sizeof(cs_float));


    for (n = ksmps_offset; n < ksmps_end; ++n)
    {
      const cs_double raw_freq = freq_sig[n];
      cs_double freq = fabs(raw_freq);
      ...
      if (hardsync_phase) {
        const cs_double syncsweep = 0.5 * (1.0 - cos(hardsync_phase));
      ...
      {
        const cs_double min_sweep = phase_inc * Min_Sweep;
        const cs_double clip = 1.0 - Clamp(clip_sig[n], 0.0, 1.0);
        // If neg_freq, invert symmetry for backward waveform
        const cs_double skew =
        ...
        if (sweep_phase < 1.0)
        {
          ...
          *aout++ = cos(PI * sweep_phase);
          ...
          if (sweep_phase > 1.0) {
            ...
            const cs_double phase_overshoot = (sweep_phase - 1.0) * sweep_length;
            ...
            if (flat_length >= phase_overshoot) {
              ...
              const cs_double next_sweep_length =
        ...
        else if (sweep_phase < 2.0)
        {
          ...
          *aout++ = cos(PI * sweep_phase);
          ...
          if (sweep_phase > 2.0) {
            const cs_double flat_length = 2.0 - (midpoint + sweep_length);
            const cs_double phase_overshoot = (sweep_phase - 2.0) * sweep_length;
            ...
            if (hardsync_phase) {
              ...
              if (freq < Max_Sweep_Freq) {
                ...
                const cs_double skew =
                  1.0 - Clamp( neg_freq? -skew_sig[n] : skew_sig[n], -1.0, 1.0);
                const cs_double midpoint = Clamp(skew, min_sweep, 2.0 - min_sweep);
                const cs_double next_sweep_length = fmax(clip * midpoint, min_sweep);
