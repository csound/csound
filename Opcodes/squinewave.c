...
typedef struct {
  OPDS h;
  MYFLT *aout, *async_out, *acps, *aclip, *askew, *async_in, *iminsweep,
      *iphase;
...
static inline MYFLT Clamp(const MYFLT x, const MYFLT minval,
                          const MYFLT maxval) {
...
int32_t squinewave_init(CSOUND *csound, SQUINEWAVE *p) {
  ...
  if (p->Min_Sweep < 4.0 || p->Min_Sweep > sr * 0.01) {
    const int32_t minsweep_default =
        (int32_t)Clamp(sr / 3000.0, 8.0, sr * 0.01);
    if (p->Min_Sweep != 0.0) {
...
int32_t squinewave_gen(CSOUND *csound, SQUINEWAVE *p) {
  ...
  if (UNLIKELY(ksmps_offset))
    memset(p->aout, 0, ksmps_offset * sizeof(MYFLT));
  if (UNLIKELY(ksmps_end < nsmps)) {
    memset(&p->aout[ksmps_end], 0,
           p->h.insdshead->ksmps_no_end * sizeof(MYFLT));
  }
      ...
      // flat +1 until phase >= 2
        ...
        if (warped_phase < 2.0) {
          ...
          if (warped_phase == 1.0) {
            ...
            warped_phase =
                1.0 + fmin(fmin(phase - midpoint, phase_inc) / sweep_length,
                           Max_Warp);
          }
          *aout++ = cos(PI * warped_phase);
          warped_phase += fmin(phase_inc / sweep_length, Max_Warp);
...
/* ar[, aSyncOut] squinewave   aFreq, aClip, aSkew [, aSyncIn, aMinSweep,
 * iphase] */
