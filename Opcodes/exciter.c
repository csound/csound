...
static int32_t exciter_init(CSOUND *csound, EXCITER *p) {
  ...
  p->rdrive = p->rbdr = p->kpa = p->kpb = p->kna = p->knb = p->ap = p->an =
      p->imr = p->kc = p->srct = p->sq = p->pwrq = p->prev_med = p->prev_out =
          0.0;
...
static inline double distort(EXCITER *p, double in) {
  ...
  int32_t i;
  double ap = p->ap, an = p->an, kpa = p->kpa, kna = p->kna, kpb = p->kpb,
         knb = p->knb, pwrq = p->pwrq;
  // printf("in: %f\n", in);
...
static OENTRY exciter_localops[] = {
    {"exciter", S(EXCITER), 0, 3, "a", "akkkk", (SUBR)exciter_init,
     (SUBR)exciter_perf},
};
