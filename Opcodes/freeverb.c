...
static int32_t freeverb_init(CSOUND *csound, FREEVERB *p) {
  ...
  for (i = 0; i < (NR_COMB << 1); i++) {
    combp =
        (freeVerbComb *)((unsigned char *)p->auxData.auxp + (int32_t)nbytes);
  ...
  for (i = 0; i < (NR_ALLPASS << 1); i++) {
    allpassp =
        (freeVerbAllPass *)((unsigned char *)p->auxData.auxp + (int32_t)nbytes);
...
static int32_t freeverb_perf(CSOUND *csound, FREEVERB *p) {
  ...
  if (UNLIKELY(p->auxData.size <= 0L || p->auxData.auxp == NULL))
    goto err1;
...
int32_t freeverb_init_(CSOUND *csound) {
  return csound->AppendOpcode(
      csound, "freeverb", (int32_t)sizeof(FREEVERB), 0, 3, "aa", "aakkjo",
      (int32_t(*)(CSOUND *, void *))freeverb_init,
      (int32_t(*)(CSOUND *, void *))freeverb_perf, NULL);
}
