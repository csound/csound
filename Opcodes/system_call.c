...
static int32_t call_system(CSOUND *csound, SYSTEM *p) {
  ...
  if ((int32_t)*p->nowait != 0) {
    if ((*p->res = fork()))
      ...
      if (UNLIKELY(system((char *)p->commandLine->data) < 0))
        exit(1);
      exit(0);
    }
  } else {
...
int32_t call_system_k(CSOUND *csound, SYSTEM *p) {
...
int32_t call_system_k(CSOUND *csound, void *p) {
  IGN(csound);
  IGN(p);
  return OK;
}
...
static OENTRY system_localops[] = {
    {"system", S(SYSTEM), 0, 3, "k", "kSO", (SUBR)call_system_set,
     (SUBR)call_system_k},
