...
static void add_ldsp_triplet(CSOUND *csound, int32_t i, int32_t j, int32_t k,
                             ls_triplet_chain **ls_triplets, ls *lss);
...
static void choose_ls_tuplets(CSOUND *csound, ls lss[],
                              ls_triplet_chain **ls_triplets, int32_t ls_amount,
                              int32_t ind);
static void sort_2D_lss(ls lss[], int32_t sorted_lss[], int32_t ls_amount);
...
static MYFLT *create_ls_table(CSOUND *csound, size_t cnt, int32_t ind) {
  ...
  if (UNLIKELY(csound->CreateGlobalVariable(csound, name,
    ...
    csound->ErrorMsg(csound, "%s",
                     Str("vbap: error allocating loudspeaker table"));
    return NULL;
  }
  return (MYFLT *)(csound->QueryGlobalVariableNoCheck(csound, name));
}

void calc_vbap_gns(int32_t ls_set_am, int32_t dim, LS_SET *sets, MYFLT *gains,
                   int32_t ls_amount, CART_VEC cart_dir)
   ...
   Calculates gain factors in that base. */
{
  ...
  if (sets[j].set_gains[0] <= FL(0.0) && sets[j].set_gains[1] <= FL(0.0) &&
   ...
   which can be used in panning.*/
{
  ...
  if (UNLIKELY(ls_amount == 0)) {
    csound->ErrorMsg(csound, "%s",
                     Str("Number of loudspeakers is zero\nExiting"));
    return;
  }

  connections = csound->Calloc(csound, ls_amount * ls_amount * sizeof(int32_t));
  distance_table = csound->Calloc(csound, ((ls_amount * (ls_amount - 1)) / 2) *
                                              sizeof(MYFLT));
  distance_table_i = csound->Calloc(
      csound, ((ls_amount * (ls_amount - 1)) / 2) * sizeof(int32_t));
  distance_table_j = csound->Calloc(
      csound, ((ls_amount * (ls_amount - 1)) / 2) * sizeof(int32_t));
...
static void add_ldsp_triplet(CSOUND *csound, int32_t i, int32_t j, int32_t k,
                             struct ls_triplet_chain **ls_triplets, ls lss[]) {
  IGN(lss);
  ...
  ls_ptr = (struct ls_triplet_chain *)csound->Malloc(
      csound, sizeof(struct ls_triplet_chain));
  if (prev == NULL)
    *ls_triplets = ls_ptr;
  else
    prev->next = ls_ptr;
...
static inline int32_t vbap_ls_init_sr(CSOUND *csound, int32_t dim,
                                      int32_t count, MYFLT **f, int32_t layout)
   ...
   stored in transposed form to ease calculation at run time.*/
{
  ...
  if (UNLIKELY(!((dim == 2) || (dim == 3)))) {
    free(lss);
    csound->ErrorMsg(
        csound, Str("Error in loudspeaker dimension. %d not permitted"), dim);
    return NOTOK;
  }
...
int32_t vbap_ls_init(CSOUND *csound, VBAP_LS_INIT *p) {
  ...
  return vbap_ls_init_sr(csound, dim, (int32_t)*p->ls_amount, p->f,
                         round(layout));
}
   ...
   matrices and stores the data to a global array */
{
  ...
  for (i = 0; i < (ls_amount - 1); i++) {
    csound->Message(csound, "***%d %d %f %f\n", sorted_lss[i],
                    sorted_lss[i + 1], lss[sorted_lss[i]].angles.azi,
    ...
    if (LIKELY((lss[sorted_lss[i + 1]].angles.azi -
      ...
      csound->Warning(csound, Str("Pair of speakers at %f and %f ignored\n"),
  ...
  if (LIKELY(((TWOPI_F - lss[sorted_lss[ls_amount - 1]].angles.azi) +
    ...
    csound->Warning(csound, Str("Pair of speakers at %f and %f ignored\n"),
                    lss[sorted_lss[ls_amount - 1]].angles.azi * FL(180.0) /
                        PI_F,
...
void new_spread_dir(CART_VEC *spreaddir, CART_VEC vscartdir,
  ...
  MYFLT power;
  ANG_VEC tmp;
  gamma = ACOS(vscartdir.x * spread_base.x + vscartdir.y * spread_base.y +
               vscartdir.z * spread_base.z) /
          PI_F * FL(180.0);
  ...
  beta = FL(180.0) - gamma;
  b = SIN(spread * PI_F / FL(180.0)) / SIN(beta * PI_F / FL(180.0));
  ...
  power = SQRT(spreaddir->x * spreaddir->x + spreaddir->y * spreaddir->y +
...
void new_spread_base(CART_VEC spreaddir, CART_VEC vscartdir, MYFLT spread,
  ...
  power =
      SQRT(spread_base->x * spread_base->x + spread_base->y * spread_base->y +
...
static OENTRY vbap_localops[] = {
    ...
    {"vbap.A", S(VBAPA), TR, 3, "a[]", "akOOo", (SUBR)vbap_init_a,
     (SUBR)vbap_a},
    {"vbap4", S(VBAP), TR | _QQ, 3,
     "aaaammmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm"
     "mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm",
     "akOOo", (SUBR)vbap_init, (SUBR)vbap},
    {"vbap8", S(VBAP), TR | _QQ, 3,
     "aaaaaaaammmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm"
     "mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm",
     "akOOo", (SUBR)vbap_init, (SUBR)vbap},
    {"vbap16", S(VBAP), TR | _QQ, 3,
     "aaaaaaaaaaaaaaaammmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm"
     "mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm",
     "akOOo", (SUBR)vbap_init, (SUBR)vbap},
    {"vbapg.a", S(VBAP1), TR, 3,
     "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz"
     "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz",
     "kOOo", (SUBR)vbap1_init, (SUBR)vbap1},
    {"vbapg.A", S(VBAPA1), TR, 3, "k[]", "kOOo", (SUBR)vbap1_init_a,
     (SUBR)vbap1a},
    {"vbapz", S(VBAP_ZAK), ZW | TR, 3, "", "iiakOOo", (SUBR)vbap_zak_init,
     (SUBR)vbap_zak},
    ...
    {"vbaplsinit", S(VBAP_LS_INIT), TR, 1, "", "iii[]", (SUBR)vbap_ls_inita,
     (SUBR)NULL, (SUBR)NULL, (SUBR)NULL},
    {"vbapmove.a", S(VBAP_MOVING), TR, 3,
     "mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm"
     "mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm",
    ...
    {"vbapgmove.a", S(VBAP1_MOVING), TR, 3,
     "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz"
     "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz",
     "iiim", (SUBR)vbap1_moving_init, (SUBR)vbap1_moving},
    {"vbapmove.A", S(VBAPA_MOVING), TR, 3, "a[]", "aiiim",
     (SUBR)vbap_moving_init_a, (SUBR)vbap_moving_a},
    {"vbapgmove.A", S(VBAPA1_MOVING), TR, 3, "k[]", "iiim",
    ...
    {"vbap4move", S(VBAP_MOVING), TR | _QQ, 3, "aaaa", "aiiim",
     (SUBR)vbap_moving_init, (SUBR)vbap_moving},
    {"vbap8move", S(VBAP_MOVING), TR | _QQ, 3, "aaaaaaaa", "aiiim",
     (SUBR)vbap_moving_init, (SUBR)vbap_moving}
