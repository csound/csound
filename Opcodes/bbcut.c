...
static int32_t BBCutMono(CSOUND *csound, BBCUTMONO *p) {
  ...
  for (i = offset; i < nsmps; i++) {
    ...
    if (p->repeatsdone >= p->repeats) {
      ...
      if (UNLIKELY(
              (*p->stutterchance > myfltrandom(csound, FL(0.0), FL(1.0))) &&
    ...
    if (p->repeatsdone == 0) {
      ...
      if (p->repeatsampdone < p->envsize) {
        ...
        /* envmult=
         * sin(PI*0.5*(((MYFLT)(p->repeatsampdone))/(MYFLT)p->envsize)); */
      ...
      if (p->repeatsampdone >= (p->repeatlengthsamp - p->envsize)) {
        ...
        /* envmult = sin(PI*0.5*
           ...
           (MYFLT)p->envsize)); */
        envmult = (EXP(((p->repeatlengthsamp - p->repeatsampdone)) / (xx)) -
                   FL(1.0)) /
                  FL(1.7182818284590);
      }
...
static int32_t BBCutStereoInit(CSOUND *csound, BBCUTSTEREO *p) {
  ...
  /* = samp per beat * beats per bar /subdiv */
  /* =(samp per sec / beats per sec)* (beats per bar/subdiv)  */
  p->samplesperunit = roundoffint(((MYFLT)CS_ESR / (*p->bps)) *
                                  (*p->barlength / (MYFLT)p->Subdiv));
...
static int32_t BBCutStereo(CSOUND *csound, BBCUTSTEREO *p) {
  ...
  for (i = offset; i < nsmps; i++) {
    ...
    if (p->repeatsdone == 0) {
      ...
      if (p->repeatsampdone >= (p->repeatlengthsamp - p->envsize)) {
        /* envmult =
           sin(PI*0.5*(((MYFLT)(p->repeatlengthsamp-p->repeatsampdone))/
           (MYFLT)p->envsize)); */
        MYFLT xx = p->envsize;
        if (xx == FL(0.0))
          xx = 0.001; /* JPff patch 2019 Apr 28 */
        envmult = (EXP(((p->repeatlengthsamp - p->repeatsampdone)) / (xx)) -
                   FL(1.0)) /
                  FL(1.7182818284590);
      }
      ...
      p->aout2[i] =
          ((MYFLT *)(p->repeatbuffer.auxp))[2 * p->repeatsampdone + 1];
    }
...
static OENTRY localops[] = {{"bbcutm", S(BBCUTMONO), 0, 3, "a", "aiiiiipop",
