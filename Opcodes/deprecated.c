...
static CS_NOINLINE void fsg_assign(CSOUND *csound, PVSDAT *fdst,
                                   const PVSDAT *fsrc) {
  ...
  if (fdst->frame.auxp == NULL ||
    ...
    csound->AuxAlloc(csound, (fdst->N + 2L) * (int64_t)sizeof(float),
                     &(fdst->frame));
...
static CS_NOINLINE CsoundArgStack_t *
csoundStack_AllocGlobals(CSOUND *csound, int32_t stackSize) {
  CsoundArgStack_t *pp;
  int32_t nBytes;
  ...
  nBytes += stackSize;
  if (UNLIKELY(csound->CreateGlobalVariable(csound, "csArgStack",
                                            (size_t)nBytes) != 0)) {
  ...
  pp->dataSpace = (void *)((char *)pp + (int32_t)csoundStack_Align(
                                            (int32_t)sizeof(CsoundArgStack_t)));
...
static CS_NOINLINE int32_t csoundStack_CreateArgMap(PUSH_OPCODE *p,
                                                    int32_t *argMap,
                                                    int32_t isOutput) {
  CSOUND *csound;
...
static int32_t push_opcode_init(CSOUND *csound, PUSH_OPCODE *p) {
  if (!p->initDone) {
    ...
    if (UNLIKELY(csoundStack_CreateArgMap(p, (int32_t *)&(p->argMap[0]), 0) !=
                 OK))
      return NOTOK;
...
static int32_t pop_opcode_init(CSOUND *csound, POP_OPCODE *p) {
  if (!p->initDone) {
    p->pp = csoundStack_GetGlobals(csound);
    if (UNLIKELY(csoundStack_CreateArgMap((PUSH_OPCODE *)p, &(p->argMap[0]),
                                          1) != OK))
      return NOTOK;
      ...
      if (!(p->argMap[0] & (1 << i))) {
        ...
        case CS_STACK_I:
          *(p->args[i]) = *(
          ...
          break;
        case CS_STACK_S: {
          STRINGDAT **ans = ((STRINGDAT **)(char *)bp +
                             (int32_t)(curOffs & (int32_t)0x00FFFFFF));
          STRINGDAT *str = *ans;
...
static OENTRY localops[] = {
    ...
    {"push", sizeof(PUSH_OPCODE), SK | _QQ, 3, "", "N", (SUBR)push_opcode_init,
     (SUBR)notinit_opcode_stub_perf, (SUBR)NULL},
    ...
    {"pop_f", sizeof(POP_OPCODE), SK | _QQ, 3, "f", "", (SUBR)pop_f_opcode_init,
     (SUBR)notinit_opcode_stub_perf, (SUBR)NULL}};
