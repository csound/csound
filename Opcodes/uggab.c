...
static int posckk(CSOUND *csound, POSC *p)
{
    ...
    if (UNLIKELY(ftp==NULL))
      return csound->PerfError(csound, p->h.insdshead, Str("poscil: not initialised"));
    ft = p->ftp->ftable;
...
static int posc3kk(CSOUND *csound, POSC *p)
{
    ...
    if (UNLIKELY(ftp==NULL))
      return csound->PerfError(csound, p->h.insdshead, Str("poscil3: not initialised"));
    ftab = p->ftp->ftable;
    if (UNLIKELY(offset)) memset(out, '\0', offset*sizeof(MYFLT));
    if (UNLIKELY(early)) {
      nsmps -= early;
...
static int vibrato(CSOUND *csound, VIBRATO *p)
{
    ...
    *p->out = (v1 + (*ftab - v1) * fract) *
    ...
    return OK;
 err1:
    return csound->PerfError(csound, p->h.insdshead, Str("vibrato(krate): not initialised"));
}
...
{ /* gab d5*/
    ...
    *p->out = p->ftp->ftable[(int32)(randGab * MYFLT2LONG(p->ftp->flen))];
    return OK;
 err1:
    return csound->PerfError(csound, p->h.insdshead, Str("Invalid ftable no. %f"),
                             *p->tableNum);
}
...
{ "rspline",  S(RANDOM3), 0,7, "s", "xxkk",
                               (SUBR)random3_set, (SUBR)random3, (SUBR)random3a },
{ "randomi",  S(RANDOMI), 0,7, "s", "kkxoo",
                               (SUBR)randomi_set, (SUBR)krandomi, (SUBR)randomi },
{ "randomh",  S(RANDOMH), 0,7, "s", "kkxoo",
...
{ "duserrnd.a", S(DURAND),0,4, "a", "k",
                                (SUBR)Cuserrnd_set,NULL,(SUBR)aDiscreteUserRand },
{ "poscil", 0xfffe, TR                                                          },
{ "poscil.kk", S(POSC), 0,7, "s", "kkjo", (SUBR)posc_set,(SUBR)kposc,(SUBR)posckk },
