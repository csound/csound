...
static int32_t trig(CSOUND *csound, TRIG *p) {
  ...
  case 2: /* both */
    ...
    break;
  default:
    return csound->PerfError(csound, &(p->h), "%s", Str(" bad imode value"));
  }
  p->old_sig = sig;
  return OK;
}
...
static int32_t posc_set(CSOUND *csound, POSC *p) {
  ...
  if (UNLIKELY((ftp = csound->FTnp2Find(csound, p->ift)) == NULL))
    return csound->InitError(csound, "%s", Str("table not found in poscil"));
  p->ftp = ftp;
  p->tablen = ftp->flen;
...
static int32_t poscaa(CSOUND *csound, POSC *p) {
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    curr_samp = ft + (int32)phs;
    fract = (MYFLT)(phs - (int32)phs);
    out[n] = amp[n] * (*curr_samp +
                       (*(curr_samp + 1) - *curr_samp) * fract); /* gab c3 */
...
static int32_t lposc_set(CSOUND *csound, LPOSC *p) {
  ...
  if (UNLIKELY(!(p->fsr = ftp->gen01args.sample_rate))) {
    csound->Warning(csound, "%s",
                    Str("losc: no sample rate stored in function "
                        "assuming=sr\n"));
    p->fsr = CS_ESR;
  }
  p->ftp = ftp;
...
static int32_t sum_(CSOUND *csound, SUM *p) {
  ...
  if (UNLIKELY(offset))
    memset(accum, '\0', offset * sizeof(MYFLT));
  if (UNLIKELY(early)) {
    nsmps -= early;
    ...
    return csound->InitError(csound, "%s", Str("loop cannot be zero"));
  {
    ...
    for (j = 0; j < loop; j++) {
      if (flag) /* linear separation in hertz */
        cosf =
            (MYFLT)cos((cf = (double)(*p->kcf * sep * j)) * (double)CS_TPIDSR);
...
static int32_t looptseg(CSOUND *csound, LOOPTSEG *p) {
  ...
  for (j = 0; j < nsegs; j++) {
    ...
    if (beg_seg <= phs && end_seg > phs) {
      ...
      MYFLT v2 =
          (j != nsegs - 1) ? *(p->argums[j + 1].start) : *(p->argums[0].start);
      if (alpha == FL(0.0))
        *p->out = v1 + (v2 - v1) * fract;
      else
        *p->out = v1 + (v2 - v1) * (FL(1.0) - EXP(alpha * fract)) /
                           (FL(1.0) - EXP(alpha));
      break;
    }
  }
...
static int32_t tlineto(CSOUND *csound, LINETO2 *p) {
  ...
  if (*p->ktrig) {
    ...
    /* p->val_incremented = p->current_val; */
    p->current_time = FL(0.0);
    p->incr = (*p->ksig - p->current_val) / ((int32)(CS_EKR * p->old_time) + 1);
    p->current_val = *p->ksig;
  } else if (p->current_time < p->old_time) {
...
static int32_t vibrato_set(CSOUND *csound, VIBRATO *p) {
  ...
  if (LIKELY((ftp = csound->FTnp2Finde(csound, p->ifn)) != NULL)) {
    ...
    return NOTOK;
  p->xcpsAmpRate =
      randGab(csound) * (*p->cpsMaxRate - *p->cpsMinRate) + *p->cpsMinRate;
  p->xcpsFreqRate =
      randGab(csound) * (*p->ampMaxRate - *p->ampMinRate) + *p->ampMinRate;
  p->tablen = ftp->flen;
...
static int32_t vibrato(CSOUND *csound, VIBRATO *p) {
  ...
  RandAmountAmp =
      (p->num1amp + (MYFLT)p->phsAmpRate * p->dfdmaxAmp) * *p->randAmountAmp;
...
#define ampMaxRate                                                             \
  FL(3.0) /* by posting a message to                                           \
...
static int32_t jitter2_set(CSOUND *csound, JITTER2 *p) {
  if (*p->cps1 == FL(0.0) && *p->cps2 == FL(0.0) && /* accept default values */
      *p->cps2 == FL(0.0) && *p->amp1 == FL(0.0) && *p->amp2 == FL(0.0) &&
      *p->amp3 == FL(0.0))
    p->flag = 1;
  else
    p->flag = 0;
...
static int32_t jitters(CSOUND *csound, JITTERS *p) {
  ...
  next:
    p->si =
        (randGab(csound) * (*p->cpsMax - *p->cpsMin) + *p->cpsMin) * CS_ONEDKR;
    if (p->si == 0)
      p->si = 1; /* Is this necessary? */
    while (p->phs > 1.0)
      p->phs -= 1.0;
    f0 = p->num0 = p->num1;
    f1 = p->num1 = p->num2;
...
static int32_t kDiscreteUserRand(CSOUND *csound, DURAND *p) { /* gab d5*/
...
static int32_t aDiscreteUserRand(CSOUND *csound, DURAND *p) { /* gab d5*/
...
static int32_t kContinuousUserRand(CSOUND *csound, CURAND *p) { /* gab d5*/
  int32 indx;
...
static int32_t aContinuousUserRand(CSOUND *csound, CURAND *p) { /* gab d5*/
...
static int32_t ikRangeRand(CSOUND *csound, RANGERAND *p) { /* gab d5*/
  *p->out = randGab(csound) * (*p->max - *p->min) + *p->min;
  return OK;
}

static int32_t aRangeRand(CSOUND *csound, RANGERAND *p) { /* gab d5*/
...
static int32_t randomi_set(CSOUND *csound, RANDOMI *p) {
  ...
  case 2: /* immediate interpolation between ifirstval and 1st random number */
    p->num1 = (*p->max - *p->min) ? (*p->fstval - *p->min) / (*p->max - *p->min)
                                  : FL(0.0);
    p->num2 = randGab(csound);
    p->dfdmax = (p->num2 - p->num1) / FMAXLEN;
    break;
...
static int32_t krandomi(CSOUND *csound, RANDOMI *p) {
  *p->ar =
      (p->num1 + (MYFLT)p->phs * p->dfdmax) * (*p->max - *p->min) + *p->min;
  p->phs += (int32)(*p->xcps * CS_KICVT);
  if (p->phs >= MAXLEN) {
    p->phs &= PHMASK;
    p->num1 = p->num2;
...
static int32_t random3(CSOUND *csound, RANDOM3 *p) {
  ...
  x = (MYFLT)p->phs;
  *p->ar =
      (((c3 * x + c2) * x + df0) * x + f0) * (*p->rangeMax - *p->rangeMin) +
      *p->rangeMin;
  return OK;
}
...
static int32_t random3a(CSOUND *csound, RANDOM3 *p) {
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    x = (MYFLT)phs;
    ar[n] = (((c3 * x + c2) * x + df0) * x + f0) * (*rangeMax - *rangeMin) +
            *rangeMin;
    if (rangeMin_cod)
      rangeMin++;
    if (rangeMax_cod)
...
static OENTRY localops[] = {
    ...
    {"ntrpol.k", S(INTERPOL), 0, 3, "k", "kkkop", (SUBR)nterpol_init,
     (SUBR)knterpol},
    {"ntrpol.a", S(INTERPOL), 0, 3, "a", "aakop", (SUBR)nterpol_init,
     (SUBR)anterpol},
    {"fold", S(FOLD), 0, 3, "a", "ak", (SUBR)fold_set, (SUBR)fold},
    {"lineto", S(LINETO), 0, 3, "k", "kk", (SUBR)lineto_set, (SUBR)lineto,
     NULL},
    {"tlineto", S(LINETO2), 0, 3, "k", "kkk", (SUBR)tlineto_set, (SUBR)tlineto,
     NULL},
    {"vibrato", S(VIBRATO), TR, 3, "k", "kkkkkkkkio", (SUBR)vibrato_set,
     (SUBR)vibrato, NULL},
    {"vibr", S(VIBRATO), TR, 3, "k", "kki", (SUBR)vibr_set, (SUBR)vibr, NULL},
    {"jitter2", S(JITTER2), 0, 3, "k", "kkkkkkko", (SUBR)jitter2_set,
     (SUBR)jitter2},
    {"jitter", S(JITTER), 0, 3, "k", "kkk", (SUBR)jitter_set, (SUBR)jitter,
     NULL},
    {"jspline", S(JITTERS), 0, 3, "k", "xkk", (SUBR)jitters_set, (SUBR)jitters,
     NULL},
    {"jspline.a", S(JITTERS), 0, 3, "a", "xkk", (SUBR)jitters_set,
     (SUBR)jittersa},
    {"loopseg", S(LOOPSEG), 0, 3, "k", "kkiz", (SUBR)loopseg_set, (SUBR)loopseg,
     NULL},
    {"loopxseg", S(LOOPSEG), 0, 3, "k", "kkiz", (SUBR)loopseg_set,
     (SUBR)loopxseg, NULL},
    {"looptseg", S(LOOPSEG), 0, 3, "k", "kkiz", (SUBR)looptseg_set,
     (SUBR)looptseg, NULL},
    {"lpshold", S(LOOPSEG), 0, 3, "k", "kkiz", (SUBR)loopseg_set, (SUBR)lpshold,
     NULL},
    {"loopsegp", S(LOOPSEGP), 0, 3, "k", "kz", (SUBR)loopsegp_set,
     (SUBR)loopsegp, NULL},
    {"lpsholdp", S(LOOPSEGP), 0, 3, "k", "kz", (SUBR)loopsegp_set,
     (SUBR)lpsholdp, NULL},
    {"cuserrnd.i", S(CURAND), 0, 1, "i", "iii", (SUBR)iContinuousUserRand, NULL,
     NULL},
    {"cuserrnd.k", S(CURAND), 0, 2, "k", "kkk", (SUBR)Cuserrnd_set,
     (SUBR)kContinuousUserRand, NULL},
    {"cuserrnd.a", S(CURAND), 0, 2, "a", "kkk", (SUBR)Cuserrnd_set,
     (SUBR)aContinuousUserRand},
    ...
    {"rspline", S(RANDOM3), 0, 3, "k", "xxkk", (SUBR)random3_set, (SUBR)random3,
     NULL},
    {"rspline.a", S(RANDOM3), 0, 3, "a", "xxkk", (SUBR)random3_set,
     (SUBR)random3a},
    {"randomi", S(RANDOMI), 0, 3, "a", "kkxoo", (SUBR)randomi_set,
     (SUBR)randomi},
    {"randomi.k", S(RANDOMI), 0, 3, "k", "kkkoo", (SUBR)randomi_set,
     (SUBR)krandomi, NULL},
    {"randomh", S(RANDOMH), 0, 3, "a", "kkxoo", (SUBR)randomh_set,
     (SUBR)randomh},
    {"randomh.k", S(RANDOMH), 0, 3, "k", "kkkoo", (SUBR)randomh_set,
     (SUBR)krandomh, NULL},
    {"urd.i", S(DURAND), 0, 1, "i", "i", (SUBR)iDiscreteUserRand, NULL, NULL},
    {"urd.k", S(DURAND), 0, 2, "k", "k", (SUBR)Cuserrnd_set,
     (SUBR)kDiscreteUserRand},
    {"urd.a", S(DURAND), 0, 2, "a", "k", (SUBR)Cuserrnd_set,
     (SUBR)aDiscreteUserRand},
    {"duserrnd.i", S(DURAND), 0, 1, "i", "i", (SUBR)iDiscreteUserRand, NULL,
     NULL},
    {"duserrnd.k", S(DURAND), 0, 2, "k", "k", (SUBR)Cuserrnd_set,
     (SUBR)kDiscreteUserRand, NULL},
    {"duserrnd.a", S(DURAND), 0, 2, "a", "k", (SUBR)Cuserrnd_set,
     (SUBR)aDiscreteUserRand},
    //{ "poscil", 0xfffe, TR },
    {"poscil.a", S(POSC), TR, 3, "a", "kkjo", (SUBR)posc_set, (SUBR)posckk},
    {"poscil.kk", S(POSC), TR, 3, "k", "kkjo", (SUBR)posc_set, (SUBR)kposc,
     NULL},
    ...
    //{ "poscil3", 0xfffe, TR },
    {"poscil3.a", S(POSC), TR, 3, "a", "kkjo", (SUBR)posc_set, (SUBR)posc3kk},
    {"poscil3.kk", S(POSC), TR, 3, "k", "kkjo", (SUBR)posc_set, (SUBR)kposc3,
     NULL},
