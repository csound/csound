...
static int32_t bar_init(CSOUND *csound, BAR *p) {
  if (*p->iK >= FL(0.0) || p->w_aux.auxp == NULL) {
    ...
    /* %%%%%%%%%%%%%%%%%%% scheme coefficients */
    p->s0 = (2.0 - 6.0 * K * K * dt * dt / (dx * dx * dx * dx) -
             2.0 * b * dt / (dx * dx)) /
            (1.0 + sig * dt * 0.5);
    p->s1 = (4.0 * K * K * dt * dt / (dx * dx * dx * dx) + b * dt / (dx * dx)) /
            (1.0 + sig * dt * 0.5);
    p->s2 = -K * K * dt * dt / ((dx * dx * dx * dx) * (1.0 + sig * dt * 0.5));
    p->t0 = (-1.0 + 2.0 * b * dt / (dx * dx) + sig * dt * 0.5) /
            (1.0 + sig * dt * 0.5);
    ...
    csound->AuxAlloc(csound, (size_t)3 * ((N + 5) * sizeof(double)),
                     &(p->w_aux));
...
static int32_t bar_run(CSOUND *csound, BAR *p) {
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    for (rr = 0; rr < N + 1; rr++) { /* Is N+1 correct here?? */
      w[rr + 2] = s0 * w1[rr + 2] + s1 * (w1[rr + 3] + w1[rr + 1]) +
    ...
    ar[n] = (csound->Get0dBFS(csound)) *
            ((1.0 - xofrac) * w[xoint] + xofrac * w[xoint + 1]);
    step++;
    {
      double *ww = w2;
...
int32_t init_pp(CSOUND *csound, CSPP *p) {
  if (*p->K >= FL(0.0)) {
    ...
    if (*p->rattle_tab == FL(0.0) ||
        (ftp = csound->FTnp2Finde(csound, p->rattle_tab)) == NULL)
      p->rattle_num = 0;
    else {
    ...
    if (*p->rubber_tab == FL(0.0) ||
        (ftp = csound->FTnp2Finde(csound, p->rubber_tab)) == NULL)
      p->rubber_num = 0;
    else {
    ...
    csound->AuxAlloc(csound,
                     3 * ((1 + (N + 5)) * NS + p->rattle_num + p->rubber_num) *
                         sizeof(MYFLT),
                     &p->auxch);
    ...
    for (n = 0; n < NS; n++) {
      p->s0[n] = (2.0 - 6.0 * K * K * dt * dt * N * N * N * N -
                  2.0 * b * dt * N * N - 2.0 * c[n] * c[n] * dt * dt * N * N) /
                 (1.0 + sig * dt * 0.5);
    ...
    p->t0 =
        (-1.0 + 2.0 * b * dt * N * N + sig * dt * 0.5) / (1.0 + sig * dt * 0.5);
...
int32_t play_pp(CSOUND *csound, CSPP *p) {
  ...
  double dt = CS_ONEDSR;
  MYFLT *w = p->w, *w1 = p->w1, *w2 = p->w2, *rub = p->rub, *rub1 = p->rub1,
        *rub2 = p->rub2, *rat = p->rat, *rat1 = p->rat1, *rat2 = p->rat2;
  ...
  if (p->init) {
    ...
    p->hammer_index =
        2 + (int32_t)(*p->ipos * N); /* find location of hammerstrike */
  ...
  for (t = offset; t < nsmps; t++) {
    ...
    if (p->rattle_num)
      ...
      for (qq = 0; qq < p->rattle_num; qq++) {
        ...
        for (n = 0; n < NS; n++) {
          ...
          if (force != 0.0) {
            w[rattle_index * NS + n] +=
                -dt * dt * (TWOPI * p->rattle[qq].freq) *
                (TWOPI * p->rattle[qq].freq) * p->rattle[qq].massden * force;
          }
          rat[qq] = 2 * rat1[qq] - rat2[qq] +
                    (TWOPI * p->rattle[qq].freq) *
                        (TWOPI * p->rattle[qq].freq) * dt * dt * force -
                    dt * dt * 9.8;
          rat2[qq] = rat1[qq];
          rat1[qq] = rat[qq];
        }
      ...
      for (qq = 0; qq < p->rubber_num; qq++) {
        ...
        for (n = 0; n < NS; n++) {
          w[rubber_index * NS + n] += -dt * dt * (TWOPI * p->rubber[qq].freq) *
                                      (TWOPI * p->rubber[qq].freq) *
                                      p->rubber[qq].massden * force;
          rub[qq] = 2 * rub1[qq] / (1 + p->rubber[qq].loss * dt / 2) -
                    (1 - p->rubber[qq].loss * dt / 2) * rub2[qq] /
                        (1 + p->rubber[qq].loss * dt * 0.5) +
                    (TWOPI * p->rubber[qq].freq) *
                        (TWOPI * p->rubber[qq].freq) * dt * dt *
                        (-rub1[qq] + force) /
                        (1 + p->rubber[qq].loss * dt * 0.5);
          rub2[qq] = rub1[qq];
          rub1[qq] = rub[qq];
    ...
    if (p->hammer_on) {
      ...
      p->ham = 2.0 * p->ham1 - p->ham2 -
               dt * dt * hammer_force_sum * (TWOPI * (*p->ham_freq)) *
                   (TWOPI * (*p->ham_freq));
      p->ham2 = p->ham1;
      p->ham1 = p->ham;
      for (qq = 0; qq < NS; qq++)
        w[p->hammer_index * NS + qq] +=
            dt * dt * (*p->ham_massden) * (TWOPI * (*p->ham_freq)) *
            (TWOPI * (*p->ham_freq)) * p->hammer_force[qq];
    }
    {
...
#define xoctr FL(0.3333)
      ...
      for (qq = 0; qq < NS; qq++) {
        out += (1 - xofrac) * w[xoint * NS + qq] +
               xofrac * w[(xoint + 1) * NS + qq];
      }
...
static OENTRY bilbar_localops[] = {
    {"barmodel", S(BAR), 0, 3, "a", "kkiikiiii", (SUBR)bar_init, (SUBR)bar_run},
    {"prepiano", S(CSPP), 0, 3, "mm", "iiiiiikkiiiiiiioo", (SUBR)init_pp,
     (SUBR)play_pp},
};
