...
static int32_t datestringset(CSOUND *csound, DATESTRING *p) {
  ...
  /* if (q) *q='\0'; */
  if (p->Stime_->data != NULL)
    csound->Free(csound, p->Stime_->data);
...
static int32_t readf_init_(CSOUND *csound, READF *p, int32_t isstring) {
  ...
  if (isstring) {
    ...
    csound->strarg2name(csound, name, p->Sfile, "input.", 0);
  p->fd = fopen(name, "r");
  p->lineno = 0;
  if (p->Sline->size < MAXLINE) {
    if (p->Sline->data != NULL)
      csound->Free(csound, p->Sline->data);
...
static int32_t readf(CSOUND *csound, READF *p) {
  p->Sline->data[0] = '\0';
  if (UNLIKELY(p->fd &&
               (fgets(p->Sline->data, p->Sline->size - 1, p->fd) == NULL))) {
...
static OENTRY date_localops[] = {
    ...
    {"readf", sizeof(READF), 0, 3, "Sk", "i", (SUBR)readf_init, (SUBR)readf},
    {"readf.S", sizeof(READF), 0, 3, "Sk", "S", (SUBR)readf_init_S, (SUBR)readf}
