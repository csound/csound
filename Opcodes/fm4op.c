...
{ /*  TwoZero is a two zero filter (duh!)  */
  ...
  MYFLT lastOutput =
      p->zeroCoeffs[0] * p->inputs[0] + p->zeroCoeffs[1] * p->inputs[1];
...
MYFLT Wave_tick(MYFLT *vTime, int32_t len, MYFLT *data, MYFLT rate,
                MYFLT phase) { /* Tick on vibrato table */
  int32 temp;
  MYFLT alpha;
  MYFLT lastOutput;
  MYFLT vvTime = *vTime;
...
void build_FM(void) { /* The following tables are pre-built */
  MYFLT temp = FL(1.0);
  int32_t i;
...
static int32_t FM4Op_loadWaves(CSOUND *csound, FM4OP *p) {
  ...
  if (UNLIKELY((ftp = csound->FTnp2Finde(csound, p->ifn0)) == NULL))
    goto err1;
  p->waves[0] = ftp;
  if (UNLIKELY((ftp = csound->FTnp2Finde(csound, p->ifn1)) == NULL))
    goto err1;
  p->waves[1] = ftp;
  if (UNLIKELY((ftp = csound->FTnp2Finde(csound, p->ifn2)) == NULL))
    goto err1;
  p->waves[2] = ftp;
  if (UNLIKELY((ftp = csound->FTnp2Finde(csound, p->ifn3)) == NULL))
    goto err1;
  p->waves[3] = ftp;
...
MYFLT FM4Alg5_tick(FM4OP *p, MYFLT c1, MYFLT c2) {
  ...
  temp = p->gains[1] * ADSR_tick(&p->adsr[1]) *
         Wave_tick(&p->w_time[1], (int32_t)p->waves[1]->flen,
                   p->waves[1]->ftable, p->w_rate[1], p->w_phase[1]);
  temp = temp * c1;
  ...
  temp = p->gains[3] * ADSR_tick(&p->adsr[3]) *
         Wave_tick(&p->w_time[3], (int32_t)p->waves[3]->flen,
                   p->waves[3]->ftable, p->w_rate[3], p->w_phase[3]);
  TwoZero_tick(&p->twozero, temp);
  p->w_phase[2] = p->waves[2]->flen * temp; /* addPhaseOffset */
  temp = (FL(1.0) - (c2 * FL(0.5))) * p->gains[0] * ADSR_tick(&p->adsr[0]) *
         Wave_tick(&p->w_time[0], (int32_t)p->waves[0]->flen,
                   p->waves[0]->ftable, p->w_rate[0], p->w_phase[0]);
  temp += c2 * FL(0.5) * p->gains[2] * ADSR_tick(&p->adsr[2]) *
          Wave_tick(&p->w_time[2], (int32_t)p->waves[2]->flen,
                    p->waves[2]->ftable, p->w_rate[2], p->w_phase[2]);
...
int32_t tubebellset(CSOUND *csound, FM4OP *p) {
  ...
  if (UNLIKELY(make_FM4Op(csound, p)))
    return NOTOK;
  ...
  ADSR_setAllTimes(csound, &p->adsr[2], FL(0.001), FL(0.5) * opt, FL(0.0),
                   FL(0.04));
...
int32_t rhodeset(CSOUND *csound, FM4OP *p) {
  ...
  if (UNLIKELY(make_FM4Op(csound, p)))
    return NOTOK;
  if (UNLIKELY(FM4Op_loadWaves(csound, p)))
    return NOTOK; /* 3 times "sinewave.raw";
           1 x fwavblnk.raw */
...
int32_t heavymetset(CSOUND *csound, FM4OP *p) {
  ...
  ADSR_setAllTimes(csound, &p->adsr[0], FL(0.001), FL(0.001), FL(1.0),
                   FL(0.01));
  ADSR_setAllTimes(csound, &p->adsr[1], FL(0.001), FL(0.010), FL(1.0),
                   FL(0.50));
  ADSR_setAllTimes(csound, &p->adsr[2], FL(0.010), FL(0.005), FL(1.0),
                   FL(0.20));
  ADSR_setAllTimes(csound, &p->adsr[3], FL(0.030), FL(0.010), FL(0.2),
                   FL(0.20));
...
int32_t b3set(CSOUND *csound, FM4OP *p) {
  ...
  ADSR_setAllTimes(csound, &p->adsr[0], FL(0.005), FL(0.003), FL(1.0),
                   FL(0.01));
  ADSR_setAllTimes(csound, &p->adsr[1], FL(0.005), FL(0.003), FL(1.0),
                   FL(0.01));
  ADSR_setAllTimes(csound, &p->adsr[2], FL(0.005), FL(0.003), FL(1.0),
                   FL(0.01));
  ADSR_setAllTimes(csound, &p->adsr[3], FL(0.005), FL(0.001), FL(0.4),
                   FL(0.03));
...
MYFLT FM4Alg4_tick(CSOUND *csound, FM4OP *p, MYFLT c1, MYFLT c2) {
  ...
  temp = Wave_tick(&p->v_time, (int32_t)p->vibWave->flen, p->vibWave->ftable,
                   p->v_rate, FL(0.0)) *
         *p->modDepth * FL(0.2);
...
int32_t percfluteset(CSOUND *csound, FM4OP *p) {
  ...
  ADSR_setAllTimes(csound, &p->adsr[0], FL(0.05), FL(0.05), FM4Op_susLevels[14],
                   FL(0.05));
  ADSR_setAllTimes(csound, &p->adsr[1], FL(0.02), FL(0.50), FM4Op_susLevels[13],
                   FL(0.5));
  ADSR_setAllTimes(csound, &p->adsr[2], FL(0.02), FL(0.30), FM4Op_susLevels[11],
                   FL(0.05));
  ADSR_setAllTimes(csound, &p->adsr[3], FL(0.02), FL(0.05), FM4Op_susLevels[13],
                   FL(0.01));
  p->twozero.gain = FL(0.0);
  /*     modDepth = FL(0.005); */
  ADSR_keyOn(&p->adsr[0]);
