...
public:
  ...
  int kontrol(CSOUND *csound) {
    IGN(csound);
    *kamplitude = dbfs * std::pow((*kvelocity * im) + ib, MYFLT(2.0)) * onedrms;
    return OK;
  }
};
...
public:
  ...
  int init(CSOUND *csound) {
    ...
    *iamplitude = dbfs * std::pow((*ivelocity * im) + ib, MYFLT(2.0)) * onedrms;
    return OK;
  }
  int noteoff(CSOUND *) {
    return OK;
...
public:
  ...
  int init(CSOUND *csound) {
    *k_gain =
        *k_dynamic_range * std::pow(*k_midi_velocity / FL(127.), *k_exponent) +
        FL(1.) - *k_dynamic_range;
    return OK;
  }
  int kontrol(CSOUND *csound) {
    *k_gain =
...
PUBLIC int32_t csoundModuleInit_ampmidid(CSOUND *csound) {
  int status = csound->AppendOpcode(
      ...
      (char *)"kio", (int32_t(*)(CSOUND *, void *))KAMPMIDID::init_,
  ...
  status |= csound->AppendOpcode(
      ...
      (char *)"iio", (int32_t(*)(CSOUND *, void *))IAMPMIDID::init_,
  ...
  status = csound->AppendOpcode(
      ...
      (char *)"kkk", (int32_t(*)(CSOUND *, void *))AMPMIDICURVE::init_,
      (int32_t(*)(CSOUND *, void *))AMPMIDICURVE::kontrol_,
      (int32_t(*)(CSOUND *, void *))0);
  status |= csound->AppendOpcode(
      csound, (char *)"ampmidicurve.i", sizeof(AMPMIDICURVE), 0, 1, (char *)"i",
      (char *)"iii", (int32_t(*)(CSOUND *, void *))AMPMIDICURVE::init_,
      (int32_t(*)(CSOUND *, void *))0, (int32_t(*)(CSOUND *, void *))0);
  status |= csound->AppendOpcode(csound, (char *)"ampmidicurve", 0xffff, 0, 0,
                                 0, 0, 0, 0, 0);
  return status;
}
