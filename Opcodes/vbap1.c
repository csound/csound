...
static int32_t vbap1_control(CSOUND *csound, VBAP1_DATA *p, MYFLT *azi,
                             MYFLT *ele, MYFLT *spread) {
  ...
  calc_vbap_gns(p->ls_set_am, p->dim, p->ls_sets, p->gains, cnt, p->cart_dir);
  ...
  if (*spread > FL(70.0))
    for (i = 0; i < cnt; i++) {
      p->gains[i] += (*spread - FL(70.0)) / FL(30.0) * (*spread - FL(70.0)) /
                     FL(30.0) * FL(20.0);
    }
...
int32_t vbap1_init(CSOUND *csound,
                   VBAP1 *p) { /* Initializations before run time*/
  int32_t i, j;
  MYFLT *ls_table, *ptr;
  LS_SET *ls_set_ptr;
  char name[24];
...
int32_t vbap1_init_a(CSOUND *csound,
                     VBAPA1 *p) { /* Initializations before run time*/
  int32_t i, j;
  MYFLT *ls_table, *ptr;
  LS_SET *ls_set_ptr;
  char name[24];
...
int32_t vbap1_moving(CSOUND *csound,
                     VBAP1_MOVING *p) { /* during note performance:   */
  int32_t j;
  int32_t cnt = p->q.number;

  vbap1_moving_control(csound, &p->q, &(p->h), CS_ONEDKR, *p->spread,
                       *p->field_am, p->fld);
...
int32_t vbap1_moving_a(CSOUND *csound,
                       VBAPA1_MOVING *p) { /* during note performance:   */
  //    int32_t j;
  int32_t cnt = p->q.number;
...
static int32_t vbap1_moving_control(CSOUND *csound, VBAP1_MOVE_DATA *p, OPDS *h,
