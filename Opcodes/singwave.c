...
static MYFLT Modulatr_tick(CSOUND *csound, Modulatr *p) {
  MYFLT lastOutput;
  lastOutput =
      Wave_tick(&p->v_time, p->wave->flen, p->wave->ftable, p->v_rate, FL(0.0));
...
static int32_t make_SingWave(CSOUND *csound, SingWave *p, MYFLT *ifn,
                             MYFLT *ivfn) {
  FUNC *ftp;
  ...
  if (UNLIKELY(make_Modulatr(csound, &p->modulator, ivfn)))
    return NOTOK;
...
char phonemes[32][4] = {"eee", "ihh", "ehh", "aaa", "ahh", "aww", "ohh", "uhh",
...
static void VoicForm_setPhoneme(CSOUND *csound, VOICF *p, int32_t i, MYFLT sc) {
  ...
  csound->Message(csound, Str("Found Formant: %s (number %i)\n"), phonemes[i],
                  i);
}
...
int32_t voicform(CSOUND *csound, VOICF *p) {
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    temp = OnePole_tick(
        &p->onepole,
        OneZero_tick(&p->onezero, SingWave_tick(csound, &p->voiced)));
