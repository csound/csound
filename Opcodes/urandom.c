...
static int32_t urand_run(CSOUND *csound, URANDOM *p) {
  ...
  int64_t x;
  if (UNLIKELY(read(ur, &x, sizeof(int64_t)) != sizeof(int64_t)))
    return NOTOK;

  /* x.ieee.exponent = x.ieee.exponent& 0x377; */
  /* printf("Debug: %s(%d): %g %d %03x %05x %08x\n", __FILE__, __LINE__, x.d, */
  /*        x.ieee.negative, x.ieee.exponent, x.ieee.mantissa0,
   * x.ieee.mantissa1); */
...
static int32_t urand_irate(CSOUND *csound, URANDOM *p) {
  if (LIKELY(urand_init(csound, p) == OK))
    return urand_run(csound, p);
  else
    return NOTOK;
}
...
static OENTRY urandom_localops[] = {
    ...
    {"urandom.k", S(URANDOM), 0, 3, "k", "jp", (SUBR)urand_init,
     (SUBR)urand_run},
    {"urandom.a", S(URANDOM), 0, 3, "a", "jp", (SUBR)urand_init,
     (SUBR)urand_arun}};
