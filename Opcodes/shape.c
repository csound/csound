	...
	if	(amt == FL(0.0))	{					/* treat zero-power with care */
		...
		do	{
			...
			if	(cur < FL(0.0))				/* treat negatives with care */
				/* make output negative to avoid DC offsets and preserve continuity with even powers */
				*out++ = - (MYFLT)pow(-cur,amt) * maxampl;
			else	*out++ = (MYFLT)pow(cur,amt) * maxampl;
		}
		while	(--nsmps);
	}
	return OK;	
...
static int Polynomial(CSOUND* csound, POLYNOMIAL* data)
{
	int     i;
	int     nsmps = csound->ksmps;
	int     ncoeff = csound->GetInputArgCnt(data) - 2;	/* index of the last coefficient */
	MYFLT   *out = data->aout;
	MYFLT   *in = data->ain;
...
static int Phasine(CSOUND* csound, PHASINE* data)
{
	...
	do	{
		cur = *in++;
		phase = (MYFLT)asin(cur/maxampl) / (MYFLT)PI;	/* current "phase" value *\/
		if	(last < cur)	{
...
static OENTRY localops[] = {
  /* { "phasine", S(PHASINE), 5, "a", "akp", (SUBR)PhasineInit, NULL, (SUBR)Phasine }, */
  { "powershape", S(POWER_SHAPE), 5, "a", "akp", (SUBR)PowerShapeInit, NULL, (SUBR)PowerShape },
  { "polynomial", S(POLYNOMIAL), 4, "a", "az", NULL, NULL, (SUBR)Polynomial },
  { "chebyshevpoly", S(CHEBPOLY), 5, "a", "az", (SUBR)ChebyshevPolyInit, NULL, (SUBR)ChebyshevPolynomial },
  ...
  { "syncphasor", S(SYNCPHASOR), 5, "aa", "xao", (SUBR)SyncPhasorInit, NULL, (SUBR)SyncPhasor },
};
