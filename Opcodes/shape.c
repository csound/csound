...
static int32_t PowerShapeInit(CSOUND *csound, POWER_SHAPE *p) {
  ...
  if (UNLIKELY(p->maxamplitude <= 0.0))
    return csound->InitError(
        csound, "%s", Str("powershape: ifullscale must be strictly positive"));
...
static int32_t PDClip(CSOUND *csound, PD_CLIP *p) {
  ...
  if (bipolarMode) {
    ...
    if (*(p->kcenter) < -width)
      center = -width * maxampl;
    else if (*(p->kcenter) > width)
      center = width * maxampl;
    else
      center = *(p->kcenter) * maxampl;
  } else {
    ...
    if (center < (FL(0.5) - width))
      center = (FL(0.5) - width) * maxampl;
    else if (center > (FL(0.5) + width))
      center = (FL(0.5) + width) * maxampl;
    else
      center = center * maxampl;
  }
...
static int32_t PDHalfX(CSOUND *csound, PD_HALF *p) {
  ...
  if (*(p->ibipolar) != FL(0.0)) { /* bipolar mode */
    ...
    for (n = offset; n < nsmps; n++) {
      cur = in[n];
      if (cur < midpoint)
        out[n] = leftslope * (cur - midpoint);
      else
        out[n] = rightslope * (cur - midpoint);
    }
  } else { /* unipolar mode */
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (cur < midpoint)
        ...
        out[n] = rightslope * (cur - midpoint) + halfmaxampl;
    }
  }
  ...
  if (*(p->ibipolar) != FL(0.0)) { /* bipolar mode */
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (cur < halfmaxampl)
        ...
        out[n] = rightslope * (cur - halfmaxampl) + midpoint;
    }
  }
  return OK;
}
...
static OENTRY shape_localops[] = {
    ...
    {"powershape", S(POWER_SHAPE), 0, 3, "a", "akp", (SUBR)PowerShapeInit,
     (SUBR)PowerShape},
    {"polynomial", S(POLYNOMIAL), 0, 2, "a", "az", NULL, (SUBR)Polynomial},
    {"chebyshevpoly", S(CHEBPOLY), 0, 3, "a", "az", (SUBR)ChebyshevPolyInit,
     (SUBR)ChebyshevPolynomial},
    ...
    {"syncphasor", S(SYNCPHASOR), 0, 3, "aa", "xao", (SUBR)SyncPhasorInit,
     (SUBR)SyncPhasor},
};
