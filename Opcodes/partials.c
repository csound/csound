...
static int32_t partials_init(CSOUND *csound, _PARTS *p) {
  ...
  if (p->lastpk.auxp == NULL || p->lastpk.size < sizeof(uint32) * maxtracks * 2)
  ...
  if (UNLIKELY(p->fin1->format != PVS_AMP_FREQ)) {
    return csound->InitError(
        csound, "%s", Str("partials: first input not in AMP_FREQ format\n"));
  }
...
static void Analysis(CSOUND *csound, _PARTS *p) {
  ...
  int32_t accum = p->accum,
          minpoints = (int32_t)(*p->pts > 1 ? *p->pts : 1) - 1;
  ...
  if (*p->kthresh >= 0) {
    ...
    absthresh = (float)(-*p->kthresh * csound->Get0dBFS(csound));
  ...
  for (i = 0; i < n; i++) {
    ...
    y1 = lmags[rmax] - (dtmp = (rmax ? lmags[rmax - 1] : lmags[rmax + 1])) +
         0.000001;
  ...
  for (j = 0; j < maxtracks && oldbins[prev + j] != 0.f; j++) {
    ...
    if (n > 0) { /* check for peaks; n will be > 0 */
      ...
      if (ftmp2 < 1.0f) {
        ...
        if (adthresh[prev + j] <
          ...
          ftmp = dbstep * 1.5f;
          ftmp2 =
              adthresh[prev + j] - (adthresh[prev + j] - 1.5f) * 0.048770575f;
    ...
    if (foundcont == 0) {
      if ((mags[(int32_t)(oldbins[prev + j] + 0.5f)]) <
              (0.2 * pmags[prev + j]) ||
...
static OENTRY localops[] = {{"partials", sizeof(_PARTS), 0, 3, "f", "ffkkki",
