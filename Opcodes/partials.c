...
int partials_init(ENVIRON *csound, _PARTS *p){
	...
	if(p->tstart.auxp==NULL && p->tstart.size < sizeof(unsigned long) * maxtracks * 2)
		csound->AuxAlloc(csound, sizeof(unsigned long) * maxtracks * 2,&p->tstart);
	if(p->lastpk.auxp==NULL && p->lastpk.size < sizeof(unsigned long) * maxtracks * 2)
		csound->AuxAlloc(csound, sizeof(unsigned long) * maxtracks * 2, &p->lastpk);
	...
	if(p->oldbins.auxp==NULL && p->oldbins.size < sizeof(MYFLT) * maxtracks * 2)
	...
	if(p->adthresh.auxp==NULL && p->adthresh.size < sizeof(MYFLT) * maxtracks * 2)
		csound->AuxAlloc(csound, sizeof(MYFLT) * maxtracks * 2, &p->adthresh);
    
	if(p->fout->frame.auxp==NULL && p->fout->frame.size < sizeof(float) * numbins * 4)
	...
	memset(p->cflag.auxp, 0, sizeof(int)  * maxtracks);
	 ...
	 for(i=0; i < maxtracks;i++) trkid[p->cur+i] = trkid[p->prev+i] = trndx[i] = -1;
	
	p->mtracks = maxtracks;
	
	if(p->fin1->format != PVS_AMP_FREQ){ csound->InitError(csound, 
...
void Analysis(ENVIRON *csound, _PARTS *p){
	...
	MYFLT  *diffs = (MYFLT *) p->diffs.auxp;
	...
	absthresh = *p->kthresh*max;
	logthresh = log(absthresh/5.f);
	...
	for(i=0;i < numbins-1; i++) {
		...
		if(i) test1 = (lmags[i] > lmags[i-1] ? 1 : 0);
		else test1 = 0;
		test2 = (lmags[i] >= lmags[i+1] ? 1 : 0); 
		
		if((lmags[i] > logthresh) && 
		   (test1 && test2)){
			index[n] = i;
			n++;
		}
	...
	for(i =0; i < n; i++){
		...
		y1 = lmags[rmax] - (dtmp = (rmax ? lmags[rmax-1] : lmags[rmax+1])) + 0.000001;
		y2 = (rmax < numbins-1 ? lmags[rmax+1] : lmags[rmax]) - dtmp + 0.000001;
		
		a = (y2 - 2*y1)/2.f;
		b = 1.f - y1/a;
		
		binex[i] = (MYFLT) (rmax - 1. + b/2.);
		magex[i] = (MYFLT) exp(dtmp - a*b*b/4.);
	}
	/* Track allocation */
	...
	for(j=0; oldbins[prev+j] != 0.f && j < maxtracks; j++){
		...
		if(n > 0){ /* check for peaks; n will be > 0 */
			...
			ndx = 0;  /* best index */
			for(i=0; i < numbins; i++) 
				if(diffs[i] < diffs[ndx]) ndx = i;
			
			/* if difference smaller than 1 bin */
			ftmp2  =  ftmp - binex[ndx];
			ftmp2  = (ftmp2 < 0 ? -ftmp2 : ftmp2);
			if(ftmp2 < 1.){
				
				/* if amp jump is too great */
				if(adthresh[prev+j] < 
				   (dbstep = 20*log10(magex[ndx]/pmags[prev+j]))){
		...
		if(foundcont==0) {
		 if((mags[(int)(oldbins[prev+j]+0.5f)]) < (0.2*pmags[prev+j])
			   || (timecount - lastpk[prev+j]) > maxgap) cflag[j] = 0;
			else {
				cflag[j] = 1;
				count++;
			}
		...
		for(i=0, n=0; i < maxtracks; i++){
			...
			for(i = count; i < maxtracks; i++){
				/* zero the right-hand size of the current arrays */
					pmags[cur+i] = oldbins[cur+i] = adthresh[cur+i] = 0.f;
					trkid[cur+i] = -1;
...
int 
partials_process(ENVIRON *csound, _PARTS *p){
	...
	MYFLT *mags = p->mags.auxp;
	MYFLT *bins = p->bins.auxp;
	...
	if(p->lastframe < p->fin1->framecount) {
		...
		for(i=k=0; i < fftsize+2; i+=2, k++ ) mags[k] = fin1[i];
	       Analysis(csound, p);
		   ...
		   for(i=k=0; i < end; i+=4, k++){
			   if(k < tracks){
				   ...
				   if(!nophase)
					   ...
					   fout[i+2] = 0.f;
				   fout[i+3] = (MYFLT) trndx[k]; /* trk IDs */
			   }
			   else{ /* empty tracks */
				   fout[i+3] = trndx[k];
			   }
