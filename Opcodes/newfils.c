...
static inline MYFLT nlf(MYFLT *t, double x, MYFLT mx, int siz) {
  ...
  int32_t n = (int32_t)p;
  return n > 0 ? (n < siz ? t[n] + (p - n) * (t[n + 1] - t[n]) : t[siz - 1])
               : t[0];
}
...
static int32_t moogladder_process(CSOUND *csound, moogladder *p) {
  ...
  double vt =
      1. / (1.22070315 *
            csound->Get0dBFS(
                csound)); /* (1.0 / 40000.0) transistor thermal voltage  */
  int32_t j;
  ...
  for (i = offset; i < nsmps; i++) {
    ...
    for (j = 0; j < 2; j++) {
      ...
      input = delay[2] = stg[2];
      stg[3] = delay[3] +
               tune * ((tanhstg[2] = tanh(input * vt)) - tanh(delay[3] * vt));
      delay[3] = stg[3];
#else
      {
        int32_t k;
        for (k = 1; k < 4; k++) {
          input = stg[k - 1];
          stg[k] =
              delay[k] + tune * ((tanhstg[k - 1] = tanh(input * vt)) -
...
int32_t mvclpf24_init(CSOUND *csound, mvclpf24 *p) {
  IGN(csound);
  if (!*p->skipinit) {
    p->c1 = p->c2 = p->c3 = p->c4 = p->c5 = FL(0.0);
    p->fr = FL(0.0);
  }
  return OK;
}
...
int32_t mvclpf24_perf1(CSOUND *csound, mvclpf24 *p) {
  MYFLT *out = p->out;
  MYFLT *in = p->in, res;
  double c1 = p->c1 + 1e-6, c2 = p->c2, c3 = p->c3, c4 = p->c4, c5 = p->c5, w,
         x, t;
  uint32_t offset = p->h.insdshead->ksmps_offset;
  uint32_t early = p->h.insdshead->ksmps_no_end;
  uint32_t i, nsmps = CS_KSMPS;
...
int32_t mvclpf24_perf1_ak(CSOUND *csound, mvclpf24 *p) {
  ...
  for (i = offset; i < nsmps; i++) {
    ...
    wi = (int32_t)(floor(t));
    t -= wi;
    t = ldexp(1 + t * (0.6930 + t * (0.2416 + t * (0.0517 + t * 0.0137))), wi);
    w = t / CS_ESR;
    if (w < 0.8)
      w *= 1 - 0.4 * w - 0.125 * w * w;
    else {
...
int32_t mvclpf24_perf1_ka(CSOUND *csound, mvclpf24 *p) {
  ...
  for (i = offset; i < nsmps; i++) {
    ...
    x = -4.2 * c5 * res[i] + in[i] / scal + 1e-10;
    t = c1 / (1 + fabs(c1));
    c1 += w * (x - t);
    x = c1 / (1 + fabs(c1));
    c2 += w * (x - c2);
...
int32_t mvclpf24_perf2_ka(CSOUND *csound, mvclpf24 *p) {
  ...
  for (i = offset; i < nsmps; i++) {
    x = -4.5 * c5 * res[i] + in[i] / scal + 1e-10;
    x /= sqrt(1 + x * x);
    c1 += w * (x - c1) / (1 + c1 * c1);
    c2 += w * (c1 - c2) / (1 + c2 * c2);
    c3 += w * (c2 - c3) / (1 + c3 * c3);
...
int32_t mvclpf24_perf3_ka(CSOUND *csound, mvclpf24 *p) {
  ...
  for (i = offset; i < nsmps; i++) {
    x = in[i] / scal -
        (4.3 - 0.2 * w) *
            (res[i] > FL(0.0) ? (res[i] < FL(1.0) ? res[i] : FL(1.0))
                              : FL(0.0)) *
            c5 +
        1e-10;
    x /= sqrt(1 + x * x);
    ...
    x = in[i] / scal - (4.3 - 0.2 * w) * res[i] * c5 + 1e-10;
    x /= sqrt(1 + x * x);
    d = w * (x - c1) / (1 + c1 * c1);
    x = c1 + 0.77 * d;
    c1 = x + 0.23 * d;
...
int32_t mvclpf24_perf4(CSOUND *csound, mvclpf24_4 *p) {
  MYFLT *out0 = p->out0, *out1 = p->out1, *out2 = p->out2, *out3 = p->out3;
  MYFLT *in = p->in, res;
  double c1 = p->c1 + 1e-6, c2 = p->c2, c3 = p->c3, c4 = p->c4, c5 = p->c5, w,
         x, d;
  uint32_t offset = p->h.insdshead->ksmps_offset;
  ...
  res = *p->res > FL(0.0) ? (*p->res < FL(1.0) ? *p->res : FL(1.0)) : FL(0.0);
...
int32_t mvchpf24_init(CSOUND *csound, mvchpf24 *p) {
  IGN(csound);
  if (!*p->skipinit) {
    p->c1 = p->c2 = p->c3 = p->c4 = p->c5 = p->x = FL(0.0);
    p->fr = FL(0.0);
  }
  return OK;
}
...
static int32_t calc_derivatives(CSOUND *csound, BOB *p, double *dstate,
  ...
  dstate[0] = k * (sat * tanh((in - res * state[3]) * satinv) - satstate0);
...
static int32_t bob_process(CSOUND *csound, BOB *p) {
  ...
  int32_t asgfr = IS_ASIG_ARG(p->freq), asgrs = IS_ASIG_ARG(p->res),
          asgsa = IS_ASIG_ARG(p->sat);
  double deriv1[DIM], deriv2[DIM], deriv3[DIM], deriv4[DIM], tempstate[DIM];
    ...
    for (j = 0; j < ostimes; j++) {
      ...
      for (l = 0; l < DIM; l++)
        p->state[l] += (1. / 6.) * stepsize *
                       (deriv1[l] + 2 * deriv2[l] + 2 * deriv3[l] + deriv4[l]);
    }
    out[i] = p->state[0];
  }
  return OK;
...
int svn_perfkk(CSOUND *csound, SVN *p) {
  ...
  if (kn > 0.) {
    ...
    for (i = offset; i < nsmps; i++) {
      ...
      u = w * nlf(tab, yh[i] * kn, max, size) *
          (tn ? tn[(int)(sz * kn / mx)] : kno1);
      yb[i] = u + s[0];
      s[0] = yb[i] + u;
      u = w * nlf(tab, yb[i] * kn, max, size) *
          (tn ? tn[(int)(sz * kn / mx)] : kno1);
      yl[i] = u + s[1];
      s[1] = yl[i] + u;
      yr[i] = (yh[i] + yl[i]) * scal;
...
static OENTRY localops[] = {
    {"mvchpf", sizeof(mvchpf24), 0, 3, "a", "ako", (SUBR)mvchpf24_init,
     (SUBR)mvchpf24_perf},
    {"mvchpf", sizeof(mvchpf24), 0, 3, "a", "aao", (SUBR)mvchpf24_init,
     (SUBR)mvchpf24_perf_a},
    {"mvclpf1", sizeof(mvclpf24), 0, 3, "a", "akko", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf1},
    {"mvclpf1", sizeof(mvclpf24), 0, 3, "a", "aako", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf1_ak},
    {"mvclpf1", sizeof(mvclpf24), 0, 3, "a", "akao", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf1_ka},
    {"mvclpf1", sizeof(mvclpf24), 0, 3, "a", "aaao", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf1_aa},
    {"mvclpf2", sizeof(mvclpf24), 0, 3, "a", "akko", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf2},
    {"mvclpf2", sizeof(mvclpf24), 0, 3, "a", "aako", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf2_ak},
    {"mvclpf2", sizeof(mvclpf24), 0, 3, "a", "akao", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf2_ka},
    {"mvclpf2", sizeof(mvclpf24), 0, 3, "a", "aaao", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf2_aa},
    {"mvclpf3", sizeof(mvclpf24), 0, 3, "a", "akko", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf3},
    {"mvclpf3", sizeof(mvclpf24), 0, 3, "a", "aako", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf3_ak},
    {"mvclpf3", sizeof(mvclpf24), 0, 3, "a", "akao", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf3_ka},
    {"mvclpf3", sizeof(mvclpf24), 0, 3, "a", "aaao", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf3_aa},
    {"mvclpf4", sizeof(mvclpf24_4), 0, 3, "aaaa", "akko", (SUBR)mvclpf24_4_init,
     (SUBR)mvclpf24_perf4},
    {"mvclpf4", sizeof(mvclpf24), 0, 3, "aaaa", "aako", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf4_ak},
    {"mvclpf4", sizeof(mvclpf24), 0, 3, "aaaa", "akao", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf4_ka},
    {"mvclpf4", sizeof(mvclpf24), 0, 3, "aaaa", "aaao", (SUBR)mvclpf24_init,
     (SUBR)mvclpf24_perf4_aa},
    ...
    {"statevar", sizeof(statevar), 0, 3, "aaaa", "axxoo", (SUBR)statevar_init,
     (SUBR)statevar_process},
    {"fofilter", sizeof(fofilter), 0, 3, "a", "axxxo", (SUBR)fofilter_init,
     (SUBR)fofilter_process},
    {"bob", sizeof(BOB), 0, 3, "a", "axxxoo", (SUBR)bob_init,
     (SUBR)bob_process},
    {"vps", sizeof(VPS), 0, 2, "a", "akk", (SUBR)NULL, (SUBR)vps_process},
    {"vclpf", sizeof(VCF), 0, 3, "a", "akko", (SUBR)vcf_init, (SUBR)vcf_perfk},
    {"vclpf", sizeof(VCF), 0, 3, "a", "aako", (SUBR)vcf_init, (SUBR)vcf_perfak},
    {"vclpf", sizeof(VCF), 0, 3, "a", "akao", (SUBR)vcf_init, (SUBR)vcf_perfka},
    {"vclpf", sizeof(VCF), 0, 3, "a", "aaao", (SUBR)vcf_init, (SUBR)vcf_perfaa},
    {"spf", sizeof(SPF), 0, 3, "a", "aaakko", (SUBR)spf_init, (SUBR)spf_perfkk},
    {"spf", sizeof(SPF), 0, 3, "a", "aaaako", (SUBR)spf_init, (SUBR)spf_perfak},
    {"spf", sizeof(SPF), 0, 3, "a", "aaaaao", (SUBR)spf_init, (SUBR)spf_perfaa},
    {"spf", sizeof(SPF), 0, 3, "a", "aaakao", (SUBR)spf_init, (SUBR)spf_perfka},
    {"skf", sizeof(SKF), 0, 3, "a", "akkoo", (SUBR)skf_init, (SUBR)skf_perfkk},
    {"skf", sizeof(SKF), 0, 3, "a", "aakoo", (SUBR)skf_init, (SUBR)skf_perfak},
    {"skf", sizeof(SKF), 0, 3, "a", "aaaoo", (SUBR)skf_init, (SUBR)skf_perfaa},
    {"skf", sizeof(SKF), 0, 3, "a", "akaoo", (SUBR)skf_init, (SUBR)skf_perfka},
    {"svn", sizeof(SVN), 0, 3, "aaaa", "akkkoopo", (SUBR)svn_init,
     (SUBR)svn_perfkk},
    {"svn", sizeof(SVN), 0, 3, "aaaa", "aakkoopo", (SUBR)svn_init,
     (SUBR)svn_perfak},
    {"svn", sizeof(SVN), 0, 3, "aaaa", "akakoopo", (SUBR)svn_init,
     (SUBR)svn_perfka},
    {"svn", sizeof(SVN), 0, 3, "aaaa", "aaakoopo", (SUBR)svn_init,
     (SUBR)svn_perfaa},
    {"st2ms", sizeof(MIDSID), 0, 2, "aa", "aa", (SUBR)NULL, (SUBR)ms_encod},
    {"ms2st", sizeof(MIDSID), 0, 2, "aa", "aak", (SUBR)NULL, (SUBR)ms_decod},
    {"otafilter", sizeof(VCFNL), 0, 3, "aa", "akkko", (SUBR)vcfnl_init,
     (SUBR)vcfnl_perfk},
    {"otafilter", sizeof(VCFNL), 0, 3, "aa", "aakko", (SUBR)vcfnl_init,
     (SUBR)vcfnl_perfak},
    {"otafilter", sizeof(VCFNL), 0, 3, "aa", "akako", (SUBR)vcfnl_init,
     (SUBR)vcfnl_perfka},
    {"otafilter", sizeof(VCFNL), 0, 3, "aa", "aaako", (SUBR)vcfnl_init,
     (SUBR)vcfnl_perfaa},
};
