...
static int32_t calc_derivatives(CSOUND *csound, bob *p, double *dstate, double *state, MYFLT in)
{
  	MYFLT  freq = *p->freq;
  	MYFLT  res = *p->res;
  	double k = TWOPI * freq;
...
static int32_t bob_process(CSOUND *csound,bob *p)
{
    ...
    int32_t      asgfr = IS_ASIG_ARG(p->freq), asgrs = IS_ASIG_ARG(p->res), asgsa = IS_ASIG_ARG(p->sat);
