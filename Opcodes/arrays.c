...
static int32_t tabfill(CSOUND *csound, TABFILL *p)
{
    ...
    size_t memMyfltSize;
    MYFLT  **valp = p->iargs;
    int n = tabinit(csound, p->ans, nargs);
    if (n) return n;
...
static int32_t tabadd(CSOUND *csound, TABARITH *p)
{
    ...
    if (tabcheck(csound, ans, size, &(p->h))) return NOTOK;
    for (i=0; i<size; i++)
...
static int32_t tabiadd(CSOUND *csound, ARRAYDAT *ans, ARRAYDAT *l,
                       MYFLT r, void *p)
{
    int32_t size;
    int32_t i;
    ...
    size = l->sizes[0];
    if (tabcheck(csound, ans, size, &((TABARITH)p->h))) return NOTOK;
    for (i=0; i<size; i++)
...
static int32_t tabaadd(CSOUND *csound, TABARITH *p)
{
    ...
    size = l->sizes[0];
    if (r->sizes[0]<size) size = r->sizes[0];
    if (tabcheck(csound, ans, size, &((TABARITH)-h)) return NOTOK;
    for (i=0; i<size; i++) {
      MYFLT *a,*b,*aa;
      int32_t j = i*span;
...
static int32_t tabamul(CSOUND *csound, TABARITH *p)
{
    ...
    size = l->sizes[0];
    if (r->sizes[0]<size) size = r->sizes[0];
    if (tabcheck(csound, ans, size, &((TABARITH)p->)) return NOTOK;
    for (i=0; i<size; i++) {
      MYFLT *a,*b,*aa;
      int32_t j = i*span;
      a = (MYFLT*)&(l->data[j]); b = (MYFLT*)&(r->data[j]);
...
static int32_t ftab2tabi(CSOUND *csound, TABCOPY *p)
{
    ...
    fsize = ftp->flen;
    if (UNLIKELY(p->tab->data==NULL)) {
      int n = tabinit(csound, p->tab, fsize);
      if (n) return n;
      p->tab->sizes[0] = fsize;
    }
    tlen = p->tab->sizes[0];
...
{
    int size = (int)(*p->size);
    int n = tabinit(csound, p->tab, size);
    if (n) return n;
    p->tab->sizes[0] = size;
    return OK;
}
...
int32_t init_rfft(CSOUND *csound, FFT *p) {
  ...
  if (isPowerOfTwo(N)) {
    int n = tabinit(csound, p->out,N);
    if (n) return n;
    p->setup = csound->RealFFT2Setup(csound, N, FFT_FWD);
  }
  else
    return tabinit(csound, p->out, N+2);
  return OK;
}
...
int32_t init_fft(CSOUND *csound, FFT *p) {
  ...
  return tabinit(csound,p->out,N2);
  //return OK;
}
...
int32_t init_mags(CSOUND *csound, FFT *p) {
    int32_t   N = p->in->sizes[0];
    return tabinit(csound, p->out, N/2+1);
    //return OK;
}
...
int32_t init_logarray(CSOUND *csound, FFT *p) {
    int n = tabinit(csound, p->out, p->in->sizes[0]);
    if (n) return n;
...
int32_t init_ceps(CSOUND *csound, FFT *p) {
    ...
    if (LIKELY(isPowerOfTwo(N))) {
      p->setup = csound->RealFFT2Setup(csound, N, FFT_FWD);
      return tabinit(csound, p->out, N+1);
    }
    else
      return csound->InitError(csound, "%s",
                               Str("non-pow-of-two case not implemented yet\n"));
...
int32_t rows_init(CSOUND *csound, FFT *p) {
    if (p->in->dimensions == 2) {
      ...
      return tabinit(csound, p->out, siz);
      //return OK;
    }
    else
      return csound->InitError(csound, "%s",
...
int32_t shiftin_init(CSOUND *csound, FFT *p) {
    ...
    p->n = 0;
    return tabinit(csound, p->out, sizs);
    //return OK;
}
...
int32_t interleave_i (CSOUND *csound, INTERL *p) {
  if(p->b->dimensions == 1 &&
    ...
    int n = tabinit(csound, p->a, len*2);
    if (n) return n;
...
int32_t deinterleave_i (CSOUND *csound, INTERL *p) {
  if(p->c->dimensions == 1) {
    ...
    int n;
    n = tabinit(csound, p->a, len);
    if (n) return n;
    n = tabinit(csound, p->b, len);
    if (n) return n;
    for(i = 0, j = 0; i < len; i++,j+=2) {
...
static OENTRY arrayvars_localops[] =
  {
    ...
    { "slicearray_i", sizeof(TABSLICE), 0, 1, ".[]", "i[]iip",
      (SUBR) tabslice, NULL },
    { "trim.i", sizeof(TRIM), WI, 1, "", "i[]i", (SUBR)trim, NULL },
    { "trim.k", sizeof(TRIM), WI, 2, "", ".[]k", NULL, (SUBR)trim },
    { "trim_i", sizeof(TRIM), WI, 1, "", ".[]i", (SUBR)trim, NULL },
