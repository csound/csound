...
static int32_t array_init(CSOUND *csound, ARRAYINIT *p) {
  ...
  if (UNLIKELY(inArgCount == 0))
    return csound->InitError(
        csound, "%s", Str("Error: no sizes set for array initialization"));
  ...
  {
    CS_VARIABLE *var =
        arrayDat->arrayType->createVariable(csound, arrayDat->arrayType);
    char *mem;
    arrayDat->arrayMemberSize = var->memBlockSize;
    arrayDat->data =
        csound->Calloc(csound, arrayDat->allocated = var->memBlockSize * size);
...
static int32_t tabfill(CSOUND *csound, TABFILL *p) {
  ...
  /*     } */
  size = p->ans->sizes[0];
  for (i = 1; i < p->ans->dimensions; i++)
    size *= p->ans->sizes[i];
  if (size < nargs)
    nargs = size;
  memMyfltSize = p->ans->arrayMemberSize / sizeof(MYFLT);
  for (i = 0; i < nargs; i++) {
    p->ans->arrayType->copyValue(csound, p->ans->arrayType,
                                 p->ans->data + (i * memMyfltSize), valp[i]);
  }
  return OK;
}
...
static int32_t array_set(CSOUND *csound, ARRAY_SET *p) {
  ...
  //        (int)(*p->indexes[0]), (int)(*p->indexes[1]),
  //        (int)(*p->indexes[2]));
...
static int32_t array_get(CSOUND *csound, ARRAY_GET *p) {
  ...
  dat->arrayType->copyValue(csound, dat->arrayType, (void *)p->out,
                            (void *)mem);
  return OK;
}
...
static int32_t tabarithset(CSOUND *csound, TABARITH *p) {
  if (LIKELY(p->left->data && p->right->data)) {
    ...
    if (UNLIKELY(p->left->dimensions != p->right->dimensions))
      return csound->InitError(
          csound, "%s", Str("Dimensions do not match in array arithmetic"));
...
static int32_t tabiadd(CSOUND *csound, ARRAYDAT *ans, ARRAYDAT *l, MYFLT r,
                       void *p);
  ...
  for (i = 0; i < sizel; i++)
    if (LIKELY(r->data[i] != 0))
      ...
      return csound->PerfError(
          csound, &(p->h), Str("division by zero in array-var at index %d"), i);
  return OK;
}
...
static int32_t tabpow(CSOUND *csound, TABARITH *p) {
  ...
  for (i = 0; i < sizel; i++)
    if (LIKELY(l->data[i] >= 0) || MODF(r->data[i], &tmp) == FL(0.0))
      ...
      return csound->PerfError(
          csound, &(p->h), Str("undefined power in array-var at index %d"), i);
  return OK;
}

#define IIARRAY(opcode, fn)                                                    \
  static int32_t opcode(CSOUND *csound, TABARITH *p) {                         \
    if (!tabarithset(csound, p))                                               \
      return fn(csound, p);                                                    \
    else                                                                       \
      return NOTOK;                                                            \
  }
...
static int32_t tabiadd(CSOUND *csound, ARRAYDAT *ans, ARRAYDAT *l, MYFLT r,
  ...
  if (UNLIKELY(ans->data == NULL || l->data == NULL))
    return csound->PerfError(csound, &(((TABARITH *)p)->h), "%s",
                             Str("array-variable not initialised"));
...
static int32_t tabaaddin(CSOUND *csound, TABARITHIN *p) {
  ...
  for (i = 0; i < sizel; i++) {
    ...
    aa = (MYFLT *)&(ans->data[j]);
    if (UNLIKELY(offset))
      memset(aa, '\0', offset * sizeof(MYFLT));
    if (UNLIKELY(early)) {
...
static int32_t tabimult(CSOUND *csound, ARRAYDAT *ans, ARRAYDAT *l, MYFLT r,
                        void *p) {
  int32_t sizel = l->sizes[0];
  int32_t i;

  if (UNLIKELY(ans->data == NULL || l->data == NULL))
    return csound->PerfError(csound, &(((TABARITH1 *)p)->h), "%s",
                             Str("array-variable not initialised"));
...
#define IiARRAY(opcode, fn)                                                    \
  static int32_t opcode(CSOUND *csound, TABARITH1 *p) {                        \
    if (!tabarithset1(csound, p))                                              \
      return fn(csound, p);                                                    \
    else                                                                       \
      return NOTOK;                                                            \
  }

IiARRAY(tabaiaddi, tabaiadd) IiARRAY(tabaisubi, tabaisub)
    IiARRAY(tabaimulti, tabaimult) IiARRAY(tabaidivi, tabaidiv)
        IiARRAY(tabairemi, tabairem) IiARRAY(tabaipowi, tabaipow)

#define iIARRAY(opcode, fn)                                                    \
  static int32_t opcode(CSOUND *csound, TABARITH2 *p) {                        \
    if (!tabarithset2(csound, p))                                              \
      return fn(csound, p);                                                    \
    else                                                                       \
      return NOTOK;                                                            \
  }

            iIARRAY(tabiaaddi, tabiaadd) iIARRAY(tabiasubi, tabiasub)
                iIARRAY(tabiamulti, tabiamult) iIARRAY(tabiadivi, tabiadiv)
                    iIARRAY(tabiaremi, tabiarem) iIARRAY(tabiapowi, tabiapow)
...
static int32_t tabmax1(CSOUND *csound, TABQUERY *p) {
  if (tabqset(csound, p) == OK)
    return tabmax(csound, p);
  else
    return NOTOK;
}
...
static int32_t tabmin1(CSOUND *csound, TABQUERY *p) {
  if (LIKELY(tabqset(csound, p) == OK))
    return tabmin(csound, p);
  else
    return NOTOK;
}
...
static int32_t tabsuma1(CSOUND *csound, TABQUERY1 *p) {
  if (LIKELY(tabqset1(csound, p) == OK))
    return tabsuma(csound, p);
  else
    return NOTOK;
}
...
static int32_t tabsum1(CSOUND *csound, TABQUERY1 *p) {
  if (LIKELY(tabqset1(csound, p) == OK))
    return tabsum(csound, p);
  else
    return NOTOK;
}

static int32_t tabscaleset(CSOUND *csound, TABSCALE *p) {
  if (LIKELY(p->tab->data && p->tab->dimensions == 1))
    return OK;
  return csound->InitError(csound, "%s", Str("array-variable not initialised"));
}
...
static int32_t tabscale(CSOUND *csound, TABSCALE *p) {
  ...
  else if (end > t->sizes[0])
    end = t->sizes[0];
  if (strt < 0)
    strt = 0;
  else if (strt > t->sizes[0])
    strt = t->sizes[0];
  if (end < strt) {
    int32_t x = end;
    end = strt;
...
static int32_t tabscale1(CSOUND *csound, TABSCALE *p) {
  if (LIKELY(tabscaleset(csound, p) == OK))
    return tabscale(csound, p);
  else
    return NOTOK;
}
...
static int32_t tabcopy(CSOUND *csound, TABCPY *p) {
  ...
  if (arrayTotalSize != get_array_total_size(p->dst)) {
    ...
    p->dst->sizes =
        csound->Malloc(csound, sizeof(int32_t) * p->src->dimensions);
    memcpy(p->dst->sizes, p->src->sizes, sizeof(int32_t) * p->src->dimensions);

    if (p->dst->data == NULL) {
      p->dst->data =
          csound->Calloc(csound, p->src->arrayMemberSize * arrayTotalSize);
...
static int32_t tabcopy2(CSOUND *csound,
                        TABCPY *p) { /* Like tabcopy but sample-accurate */
...
static int32_t tabgen(CSOUND *csound, TABGEN *p) {
  ...
  if (UNLIKELY(size < 0))
    return csound->InitError(
        csound, "%s", Str("inconsistent start, end and increment parameters"));
...
static int32_t ftab2tab(CSOUND *csound, TABCOPY *p) {
  ...
  fsize = ftp->flen;
  if (UNLIKELY(p->tab->data == NULL)) {
    if (tabcheck(csound, p->tab, fsize, &(p->h)))
      return NOTOK;
    p->tab->sizes[0] = fsize;
  }
  tlen = p->tab->sizes[0];
...
static int32_t tabmap_set(CSOUND *csound, TABMAP *p) {
  ...
  if (UNLIKELY(p->tab->data == NULL)) {
    ...
    size = size < p->tab->sizes[0] ? size : p->tab->sizes[0];
  data = p->tab->data;
...
static int32_t tabmap_perf(CSOUND *csound, TABMAP *p) {
  ...
  if (UNLIKELY(opc == NULL))
    return csound->PerfError(csound, &(p->h), "%s",
                             Str("map fn not found at k rate"));
  for (n = 0; n < size; n++) {
    eval.a = &tabin[n];
    eval.r = &data[n];
...
int32_t tablength(CSOUND *csound, TABQUERY1 *p) {
  ...
  if (UNLIKELY(p->tab == NULL || opt > p->tab->dimensions))
    *p->ans = -FL(1.0);
  else if (UNLIKELY(opt <= 0))
    *p->ans = p->tab->dimensions;
  else
    *p->ans = p->tab->sizes[opt - 1];
  return OK;
}
...
static int32_t ina_set(CSOUND *csound, OUTA *p) {
  ...
  aa->dimensions = 1;
  if (aa->sizes)
    csound->Free(csound, aa->sizes);
  if (aa->data)
    csound->Free(csound, aa->data);
  aa->sizes = (int32_t *)csound->Malloc(csound, sizeof(int32_t));
  aa->sizes[0] = p->len = csound->GetNchnls_i(csound);
  aa->data = (MYFLT *)csound->Malloc(csound, CS_KSMPS * sizeof(MYFLT) * p->len);
...
int32_t init_ifft(CSOUND *csound, FFT *p) {
  ...
  if (UNLIKELY(p->in->dimensions > 1))
    return csound->InitError(
        csound, "%s", Str("fftinv: only one-dimensional arrays allowed"));
  tabinit(csound, p->out, N2);
  return OK;
}
...
int32_t rows_init(CSOUND *csound, FFT *p) {
  if (p->in->dimensions == 2) {
    ...
    return csound->InitError(csound, "%s", Str("in array not 2-dimensional\n"));
}
...
int32_t rows_perf_S(CSOUND *csound, FFT *p) {
  ...
  if (LIKELY(index < p->in->sizes[0])) {
    ...
    for (i = 0; i < p->in->sizes[1]; i++) {
      dat->arrayType->copyValue(csound, dat->arrayType, (void *)dest,
                                (void *)mem);
...
int32_t set_rows_perf_S(CSOUND *csound, FFT *p) {
  ...
  if (LIKELY(index < p->in->sizes[0])) {
    ...
    // printf("*** mem = %p dst = %p\n", mem, dest);
    for (i = 0; i < p->in->sizes[1]; i++) {
      dat->arrayType->copyValue(csound, dat->arrayType, (void *)mem,
                                (void *)dest);
      // printf("*** copies i=%d: %s -> %s\n",
      // i,(char*)(mem->data),(char*)(dest->data));
      dest += 1;
...
static inline void tabensure2D(CSOUND *csound, ARRAYDAT *p, int32_t rows,
                               int32_t columns) {
...
int32_t init_dct(CSOUND *csound, FFT *p) {
  ...
  if (LIKELY(isPowerOfTwo(N))) {
    ...
    return csound->InitError(
        csound, "%s", Str("dct: non-pow-of-two sizes not yet implemented"));
}
...
int32_t init_dctinv(CSOUND *csound, FFT *p) {
  ...
  if (LIKELY(isPowerOfTwo(N))) {
    if (UNLIKELY(p->in->dimensions > 1))
      return csound->InitError(
          csound, "%s", Str("dctinv: only one-dimensional arrays allowed"));
    tabinit(csound, p->out, N);
    p->setup = csound->DCTSetup(csound, N, FFT_INV);
    return OK;
  } else
    return csound->InitError(
        csound, "%s", Str("dctinv: non-pow-of-two sizes not yet implemented"));
}
...
int32_t mfb_init(CSOUND *csound, MFB *p) {
  ...
  if (LIKELY(L < N)) {
    ...
    return csound->InitError(
        csound, "%s", Str("mfb: filter bank size exceeds input array length"));
...
int32_t interleave_i(CSOUND *csound, INTERL *p) {
  if (p->b->dimensions == 1 && p->c->dimensions == 1 &&
...
static OENTRY arrayvars_localops[] = {
    ...
    {"##array_set.k", sizeof(ARRAY_SET), 0, 2, "", "k[]km", NULL,
     (SUBR)array_set},
    {"##array_set.a", sizeof(ARRAY_SET), 0, 2, "", "a[]am", NULL,
     (SUBR)array_set},
    // VL: 11.2.22 I think array set S needs to be added running at thread 3 for
    // parser3
    {"##array_set.S", sizeof(ARRAY_SET), 0, 3, "", "S[].m", (SUBR)array_set,
     (SUBR)array_set},
    {"##array_set.i", sizeof(ARRAY_SET), 0, 1, "", ".[].m", (SUBR)array_set},
    {"##array_set.e", sizeof(ARRAY_SET), 0, 1, "", "i[].z", (SUBR)array_err},
    {"##array_set.x", sizeof(ARRAY_SET), 0, 2, "", ".[].z", NULL,
     (SUBR)array_set},
    {"##array_get.k", sizeof(ARRAY_GET), 0, 2, "k", "k[]m", NULL,
     (SUBR)array_get},
    {"##array_get.a", sizeof(ARRAY_GET), 0, 2, "a", "a[]m", NULL,
     (SUBR)array_get},
    {"##array_get.x", sizeof(ARRAY_GET), 0, 1, ".", ".[]m", (SUBR)array_get},
    {"##array_get.K", sizeof(ARRAY_GET), 0, 2, ".", ".[]z", NULL,
     (SUBR)array_get},
    {"i.Ai", sizeof(ARRAY_GET), 0, 1, "i", "k[]m", (SUBR)array_get},
    {"i.Ak", sizeof(ARRAY_GET), 0, 1, "i", "k[]z", (SUBR)array_get},
    /* ******************************************** */
    {"##add.[s]", sizeof(TABARITH), 0, 3, "a[]", "a[]a[]", (SUBR)tabarithset,
     (SUBR)tabaadd},
    {"##add.[]", sizeof(TABARITH), 0, 3, "k[]", "k[]k[]", (SUBR)tabarithset,
     (SUBR)tabadd},
    {"##add.[i]", sizeof(TABARITH), 0, 1, "i[]", "i[]i[]", (SUBR)tabaddi},
    /* ******************************************** */
    {"##sub.[a]", sizeof(TABARITH), 0, 3, "a[]", "a[]a[]", (SUBR)tabarithset,
     (SUBR)tabasub},
    {"##sub.[]", sizeof(TABARITH), 0, 3, "k[]", "k[]k[]", (SUBR)tabarithset,
     (SUBR)tabsub},
    {"##sub.[i]", sizeof(TABARITH), 0, 1, "i[]", "i[]i[]", (SUBR)tabsubi},
    //    {"##neg.[]",  sizeof(TABARITH), 0, 3, "k[]", "k[]",
    //                                         (SUBR)tabarithset1,
    //                                         (SUBR)tabneg},
    {"##mul.[a]", sizeof(TABARITH), 0, 3, "a[]", "a[]a[]", (SUBR)tabarithset,
     (SUBR)tabamul},
    {"##mul.[]", sizeof(TABARITH), 0, 3, "k[]", "k[]k[]", (SUBR)tabarithset,
     (SUBR)tabmult},
    {"##mul.[i]", sizeof(TABARITH), 0, 1, "i[]", "i[]i[]", (SUBR)tabmulti},
    {"##div.[a]", sizeof(TABARITH), 0, 3, "a[]", "a[]a[]", (SUBR)tabarithset,
     (SUBR)tabadiv},
    {"##div.[]", sizeof(TABARITH), 0, 3, "k[]", "k[]k[]", (SUBR)tabarithset,
     (SUBR)tabdiv},
    {"##div.[i]", sizeof(TABARITH), 0, 1, "i[]", "i[]i[]", (SUBR)tabdivi},
    {"##rem.[]", sizeof(TABARITH), 0, 3, "k[]", "k[]k[]", (SUBR)tabarithset,
     (SUBR)tabrem},
    {"##rem.[i]", sizeof(TABARITH), 0, 1, "i[]", "i[]i[]", (SUBR)tabremi},
    {"##add.[i", sizeof(TABARITH1), 0, 3, "k[]", "k[]i", (SUBR)tabarithset1,
     (SUBR)tabaiadd},
    {"##add.i[", sizeof(TABARITH2), 0, 3, "k[]", "ik[]", (SUBR)tabarithset2,
     (SUBR)tabiaadd},
    {"##add.[p", sizeof(TABARITH1), 0, 1, "i[]", "i[]i", (SUBR)tabaiaddi},
    {"##add.p[", sizeof(TABARITH2), 0, 1, "i[]", "ii[]", (SUBR)tabiaaddi},
    {"##add.k[a[", sizeof(TABARITH), 0, 3, "a[]", "k[]a[]", (SUBR)tabarithset,
     (SUBR)tabkrardd},
    {"##add.a[k[", sizeof(TABARITH), 0, 3, "a[]", "a[]k[]", (SUBR)tabarithset,
     (SUBR)tabarkrdd},
    {"##addin.[i", sizeof(TABARITHIN1), 0, 1, "i[]", "i", (SUBR)addinAA, NULL},
    {"##addin.[k", sizeof(TABARITHIN1), 0, 2, "k[]", "i", NULL, (SUBR)addinAA},
    {"##addin.[", sizeof(TABARITHIN), 0, 1, "i[]", "i[]", (SUBR)tabaddinkk,
     NULL},
    {"##addin.[K", sizeof(TABARITHIN), 0, 2, "k[]", "k[]", NULL,
     (SUBR)tabaddinkk},
    {"##addin.[a", sizeof(TABARITHIN), 0, 2, "a[]", "a[]", NULL,
     (SUBR)tabaaddin},
    {"##addin.[a", sizeof(TABARITHIN), 0, 2, "a[]", "a[]", NULL,
     (SUBR)tabaaddin},
    {"##addin.[ak", sizeof(TABARITHIN), 0, 2, "a[]", "k[]", NULL,
     (SUBR)tabarkrddin},
    {"##addin.[aks", sizeof(TABARITHIN), 0, 2, "a[]", "k", NULL,
     (SUBR)tabakaddin},
    {"##sub.[i", sizeof(TABARITH1), 0, 3, "k[]", "k[]i", (SUBR)tabarithset1,
     (SUBR)tabaisub},
    {"##sub.i[", sizeof(TABARITH2), 0, 3, "k[]", "ik[]", (SUBR)tabarithset2,
     (SUBR)tabiasub},
    {"##subin.[i", sizeof(TABARITHIN1), 0, 1, "i[]", "i", (SUBR)subinAA, NULL},
    {"##subin.[", sizeof(TABARITHIN), 0, 1, "i[]", "i[]", (SUBR)tabsubinkk,
     NULL},
    {"##subin.[K", sizeof(TABARITHIN), 0, 2, "k[]", "k[]", NULL,
     (SUBR)tabsubinkk},
    {"##subin.[k", sizeof(TABARITHIN1), 0, 2, "k[]", "k", NULL, (SUBR)subinAA},
    {"##subin.[a", sizeof(TABARITHIN), 0, 2, "a[]", "a[]", NULL,
     (SUBR)tabaasubin},
    {"##subin.[ak", sizeof(TABARITHIN), 0, 2, "a[]", "k[]", NULL,
     (SUBR)tabarkrsbin},
    {"##subdin.[aks", sizeof(TABARITHIN), 0, 2, "a[]", "k", NULL,
     (SUBR)tabaksubin},
    {"##sub.[p", sizeof(TABARITH1), 0, 1, "i[]", "i[]i", (SUBR)tabaisubi},
    {"##sub.p[", sizeof(TABARITH2), 0, 1, "i[]", "ii[]", (SUBR)tabiasubi},
    {"##sub.k[a[", sizeof(TABARITH), 0, 3, "a[]", "k[]a[]", (SUBR)tabarithset,
     (SUBR)tabkrarsb},
    {"##sub.a[k[", sizeof(TABARITH), 0, 3, "a[]", "a[]k[]", (SUBR)tabarithset,
     (SUBR)tabarkrsb},
    {"##mul.[i", sizeof(TABARITH1), 0, 3, "k[]", "k[]i", (SUBR)tabarithset1,
     (SUBR)tabaimult},
    {"##mul.i[", sizeof(TABARITH2), 0, 3, "k[]", "ik[]", (SUBR)tabarithset2,
     (SUBR)tabiamult},
    {"##mul.[p", sizeof(TABARITH1), 0, 1, "i[]", "i[]i", (SUBR)tabaimulti},
    {"##mul.p[", sizeof(TABARITH2), 0, 1, "i[]", "ii[]", (SUBR)tabiamulti},
    {"##mul.k[a[", sizeof(TABARITH), 0, 3, "a[]", "k[]a[]", (SUBR)tabarithset,
     (SUBR)tabkrarml},
    {"##mul.a[k[", sizeof(TABARITH), 0, 3, "a[]", "a[]k[]", (SUBR)tabarithset,
     (SUBR)tabarkrml},
    {"##div.[i", sizeof(TABARITH1), 0, 3, "k[]", "k[]i", (SUBR)tabarithset1,
     (SUBR)tabaidiv},
    {"##div.i[", sizeof(TABARITH2), 0, 3, "k[]", "ik[]", (SUBR)tabarithset2,
     (SUBR)tabiadiv},
    {"##div.[p", sizeof(TABARITH1), 0, 1, "i[]", "i[]i", (SUBR)tabaidivi},
    {"##div.p[", sizeof(TABARITH2), 0, 1, "i[]", "ii[]", (SUBR)tabiadivi},
    {"##div.k[a[", sizeof(TABARITH), 0, 3, "a[]", "k[]a[]", (SUBR)tabarithset,
     (SUBR)tabkrardv},
    {"##div.a[k[", sizeof(TABARITH), 0, 3, "a[]", "a[]k[]", (SUBR)tabarithset,
     (SUBR)tabarkrdv},
    {"##rem.[i", sizeof(TABARITH1), 0, 3, "k[]", "k[]i", (SUBR)tabarithset1,
     (SUBR)tabairem},
    {"##rem.i[", sizeof(TABARITH2), 0, 3, "k[]", "ik[]", (SUBR)tabarithset2,
     (SUBR)tabiarem},
    {"##rem.[p", sizeof(TABARITH1), 0, 1, "i[]", "i[]i", (SUBR)tabairemi},
    {"##rem.p[", sizeof(TABARITH2), 0, 1, "i[]", "ii[]", (SUBR)tabiaremi},
    {"##rem.a[k[", sizeof(TABARITH), 0, 3, "a[]", "a[]k[]", (SUBR)tabarithset,
     (SUBR)tabarkrmd},
    {"##re.k[a[", sizeof(TABARITH), 0, 3, "a[]", "k[]a[]", (SUBR)tabarithset,
     (SUBR)tabkrarmd},
    {"##add.[k", sizeof(TABARITH1), 0, 3, "k[]", "i[]k", (SUBR)tabarithset1,
     (SUBR)tabaiadd},
    {"##add.[k", sizeof(TABARITH1), 0, 3, "k[]", "k[]k", (SUBR)tabarithset1,
     (SUBR)tabaiadd},
    {"##add.k[", sizeof(TABARITH2), 0, 3, "k[]", "ik[]", (SUBR)tabarithset2,
     (SUBR)tabiaadd},
    {"##add.k[", sizeof(TABARITH2), 0, 3, "k[]", "kk[]", (SUBR)tabarithset2,
     (SUBR)tabiaadd},
    {"##add.[ak", sizeof(TABARITH1), 0, 3, "a[]", "a[]k", (SUBR)tabarithset1,
     (SUBR)tabakadd},
    {"##add.k[a", sizeof(TABARITH2), 0, 3, "a[]", "ka[]", (SUBR)tabarithset2,
     (SUBR)tabkaadd},
    {"##add.aa[", sizeof(TABARITH1), 0, 3, "a[]", "aa[]", (SUBR)tabarithset1,
     (SUBR)tabaardd},
    {"##add.a[a", sizeof(TABARITH2), 0, 3, "a[]", "a[]a", (SUBR)tabarithset2,
     (SUBR)tabaradd},
    {"##sub.[k", sizeof(TABARITH1), 0, 3, "k[]", "i[]k", (SUBR)tabarithset1,
     (SUBR)tabaisub},
    {"##sub.[k", sizeof(TABARITH1), 0, 3, "k[]", "k[]k", (SUBR)tabarithset1,
     (SUBR)tabaisub},
    {"##sub.k[", sizeof(TABARITH2), 0, 3, "k[]", "kk[]", (SUBR)tabarithset2,
     (SUBR)tabiasub},
    {"##sub.k[", sizeof(TABARITH2), 0, 3, "k[]", "ki[]", (SUBR)tabarithset2,
     (SUBR)tabiasub},
    {"##sub.[ak", sizeof(TABARITH1), 0, 3, "a[]", "a[]k", (SUBR)tabarithset1,
     (SUBR)tabaksub},
    {"##sub.k[a", sizeof(TABARITH2), 0, 3, "a[]", "ka[]", (SUBR)tabarithset2,
     (SUBR)tabkasub},
    {"##sub.aa[", sizeof(TABARITH2), 0, 3, "a[]", "a[]a", (SUBR)tabarithset2,
     (SUBR)tabaarsb},
    {"##sub.a[a", sizeof(TABARITH1), 0, 3, "a[]", "aa[]", (SUBR)tabarithset1,
     (SUBR)tabarasb},
    {"##mul.[k", sizeof(TABARITH1), 0, 3, "k[]", "k[]k", (SUBR)tabarithset1,
     (SUBR)tabaimult},
    {"##mul.[k", sizeof(TABARITH1), 0, 3, "k[]", "i[]k", (SUBR)tabarithset1,
     (SUBR)tabaimult},
    {"##mul.k[", sizeof(TABARITH2), 0, 3, "k[]", "kk[]", (SUBR)tabarithset2,
     (SUBR)tabiamult},
    {"##mul.k[", sizeof(TABARITH2), 0, 3, "k[]", "ki[]", (SUBR)tabarithset2,
     (SUBR)tabiamult},
    {"##mul.[ak", sizeof(TABARITH1), 0, 3, "a[]", "a[]k", (SUBR)tabarithset1,
     (SUBR)tabakmult},
    {"##mul.k[a", sizeof(TABARITH2), 0, 3, "a[]", "ka[]", (SUBR)tabarithset2,
     (SUBR)tabkamult},
    {"##mul.a[a", sizeof(TABARITH1), 0, 3, "a[]", "a[]a", (SUBR)tabarithset1,
     (SUBR)tabaraml},
    {"##mul.aa[", sizeof(TABARITH2), 0, 3, "a[]", "aa[]", (SUBR)tabarithset2,
     (SUBR)tabaarml},
    {"##div.[k", sizeof(TABARITH1), 0, 3, "k[]", "k[]k", (SUBR)tabarithset1,
     (SUBR)tabaidiv},
    {"##div.k[", sizeof(TABARITH2), 0, 3, "k[]", "ki[]", (SUBR)tabarithset2,
     (SUBR)tabiadiv},
    {"##div.[k", sizeof(TABARITH1), 0, 3, "k[]", "i[]k", (SUBR)tabarithset1,
     (SUBR)tabaidiv},
    {"##div.k[", sizeof(TABARITH2), 0, 3, "k[]", "kk[]", (SUBR)tabarithset2,
     (SUBR)tabiadiv},
    {"##div.[ak", sizeof(TABARITH1), 0, 3, "a[]", "a[]k", (SUBR)tabarithset1,
     (SUBR)tabakdiv},
    {"##div.k[a", sizeof(TABARITH2), 0, 3, "a[]", "ka[]", (SUBR)tabarithset2,
     (SUBR)tabkadiv},
    {"##div.a[a", sizeof(TABARITH1), 0, 3, "a[]", "a[]a", (SUBR)tabarithset1,
     (SUBR)tabaardv},
    {"##div.a[a", sizeof(TABARITH1), 0, 3, "a[]", "a[]a", (SUBR)tabarithset1,
     (SUBR)tabaradv},
    {"##rem.[k", sizeof(TABARITH1), 0, 3, "k[]", "k[]k", (SUBR)tabarithset1,
     (SUBR)tabairem},
    {"##rem.[ak", sizeof(TABARITH1), 0, 3, "a[]", "a[]k", (SUBR)tabarithset1,
     (SUBR)tabarkrem},
    {"##rem.k[", sizeof(TABARITH2), 0, 3, "k[]", "kk[]", (SUBR)tabarithset2,
     (SUBR)tabiarem},
    {"##pow.[]", sizeof(TABARITH), 0, 3, "k[]", "k[]k[]", (SUBR)tabarithset,
     (SUBR)tabpow},
    {"##pow.[i]", sizeof(TABARITH), 0, 1, "i[]", "i[]i[]", (SUBR)tabpowi},
    {"##pow.[i", sizeof(TABARITH1), 0, 3, "k[]", "k[]i", (SUBR)tabarithset1,
     (SUBR)tabaipow},
    {"##pow.i[", sizeof(TABARITH2), 0, 3, "k[]", "ik[]", (SUBR)tabarithset2,
     (SUBR)tabiapow},
    {"##pow.[p", sizeof(TABARITH1), 0, 1, "i[]", "i[]i", (SUBR)tabaipowi},
    {"##pow.p[", sizeof(TABARITH2), 0, 1, "i[]", "ii[]", (SUBR)tabiapowi},
    {"##pow.[k", sizeof(TABARITH1), 0, 3, "k[]", "k[]k", (SUBR)tabarithset1,
     (SUBR)tabaipow},
    {"##pow.k[", sizeof(TABARITH2), 0, 3, "k[]", "kk[]", (SUBR)tabarithset2,
     (SUBR)tabiapow},
    {"##pow.ki[", sizeof(TABARITH2), 0, 3, "k[]", "ki[]", (SUBR)tabarithset2,
     (SUBR)tabiapow},

    {"##pow.[ak", sizeof(TABARITH1), 0, 3, "a[]", "a[]k", (SUBR)tabarithset1,
     (SUBR)tabarkpow},
    {"##pow.a[k[", sizeof(TABARITH), 0, 3, "a[]", "a[]k[]", (SUBR)tabarithset,
     (SUBR)tabarkrpw},
    {"maxtab.k", sizeof(TABQUERY), _QQ, 3, "kz", "k[]", (SUBR)tabqset,
     (SUBR)tabmax},
    {"maxarray.k", sizeof(TABQUERY), 0, 3, "kz", "k[]", (SUBR)tabqset,
     (SUBR)tabmax},
    {"maxarray.i", sizeof(TABQUERY), 0, 1, "iI", "i[]", (SUBR)tabmax1, NULL},
    {"mintab.k", sizeof(TABQUERY), _QQ, 3, "kz", "k[]", (SUBR)tabqset,
     (SUBR)tabmin},
    {"minarray.k", sizeof(TABQUERY), 0, 3, "kz", "k[]", (SUBR)tabqset,
     (SUBR)tabmin},
    {"minarray.i", sizeof(TABQUERY), 0, 1, "iI", "i[]", (SUBR)tabmin1},
    {"sumtab", sizeof(TABQUERY1), _QQ, 3, "k", "k[]", (SUBR)tabqset1,
     (SUBR)tabsum},
    {"sumarray.k", sizeof(TABQUERY1), 0, 3, "k", "k[]", (SUBR)tabqset1,
     (SUBR)tabsum},
    {"sumarray.a", sizeof(TABQUERY1), 0, 3, "a", "a[]", (SUBR)tabqset1,
     (SUBR)tabsuma1},
    {"sumarray.i", sizeof(TABQUERY1), 0, 1, "i", "i[]", (SUBR)tabsum1},
    {"scalet", sizeof(TABSCALE), _QQ | WI, 3, "", "k[]kkOJ", (SUBR)tabscaleset,
     (SUBR)tabscale},
    {"scalearray.k", sizeof(TABSCALE), WI, 3, "", "k[]kkOJ", (SUBR)tabscaleset,
     (SUBR)tabscale},
    ...
    {"=.IK", sizeof(TABCPY), 0, 3, "k[]", "i[]", (SUBR)tabcopy1,
     (SUBR)tabcopy1},
    {"=.K", sizeof(TABCPY), 0, 3, "k[]", "k[]", (SUBR)tabcopyk_init,
     (SUBR)tabcopyk},
    ...
    {"genarray_i", sizeof(TABGEN), 0, 1, "k[]", "iip", (SUBR)tabgen, NULL,
     NULL},
    ...
    /*                                               (SUBR) tabmap_perf     },
     */
    {"tabslice", sizeof(TABSLICE), _QQ, 2, "k[]", "k[]iip", NULL,
     (SUBR)tabslice, NULL},
    {"slicearray.i", sizeof(TABSLICE), 0, 1, "i[]", "i[]iip", (SUBR)tabslice,
     NULL, NULL},
    {"slicearray.k", sizeof(TABSLICE), 0, 2, "k[]", "k[]iip", (SUBR)tabslice,
     (SUBR)tabslice, NULL},
    {"slicearray.a", sizeof(TABSLICE), 0, 2, "a[]", "a[]iip", (SUBR)tabslice,
     (SUBR)tabslice, NULL},
    {"slicearray.S", sizeof(TABSLICE), 0, 2, "S[]", "S[]iip", (SUBR)tabslice,
     (SUBR)tabslice, NULL},
    {"slicearray_i.i", sizeof(TABSLICE), 0, 1, "i[]", "i[]iip", (SUBR)tabslice,
     NULL},
    {"slicearray_i.k", sizeof(TABSLICE), 0, 1, "k[]", "k[]iip", (SUBR)tabslice,
     NULL},
    {"slicearray_i.S", sizeof(TABSLICE), 0, 1, "S[]", "S[]iip", (SUBR)tabslice,
     NULL},
    ...
    {"copy2ftab", sizeof(TABCOPY), TW | _QQ, 2, "", "k[]k", NULL,
     (SUBR)tab2ftab},
    {"copy2ttab", sizeof(TABCOPY), TR | _QQ, 2, "", "k[]k", NULL,
     (SUBR)ftab2tab},
    {"copya2ftab.k", sizeof(TABCOPY), TW, 3, "", "k[]k", (SUBR)tab2ftabi,
     (SUBR)tab2ftab},

    {"copyf2array.k", sizeof(TABCOPY), TR, 3, "", "k[]k", (SUBR)ftab2tabi,
     (SUBR)ftab2tab},
    {"copyf2array.kk", sizeof(TABCOPY), TR, 3, "", "k[]kk", (SUBR)ftab2tabi,
     (SUBR)ftab2tab},
    // { "copya2ftab.i", sizeof(TABCOPY), TW, 1, "", "i[]i", (SUBR) tab2ftabi },
    {"copya2ftab.ii", sizeof(TABCOPY2), TW, 1, "", "i[]io",
     (SUBR)tab2ftab_offset_i},
    {"copya2ftab.kk", sizeof(TABCOPY2), TW, 2, "", "k[]kO", NULL,
    ...
    {"lenarray.kx", sizeof(TABQUERY1), 0, 2, "k", ".[]p", NULL,
     (SUBR)tablength},
    {"in.A", sizeof(OUTA), 0, 3, "a[]", "", (SUBR)ina_set, (SUBR)ina},
    {"monitor.A", sizeof(OUTA), IB, 3, "a[]", "", (SUBR)monitora_init,
     (SUBR)monitora_perf},
    {"rfft", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_rfft, (SUBR)perf_rfft,
     NULL},
    {"rfft", sizeof(FFT), 0, 1, "i[]", "i[]", (SUBR)rfft_i, NULL, NULL},
    {"rifft", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_rifft,
     (SUBR)perf_rifft, NULL},
    {"rifft", sizeof(FFT), 0, 1, "i[]", "i[]", (SUBR)rifft_i, NULL, NULL},
    {"cmplxprod", sizeof(FFT), 0, 3, "k[]", "k[]k[]", (SUBR)init_rfftmult,
     (SUBR)perf_rfftmult, NULL},
    {"fft", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)initialise_fft,
     (SUBR)perf_fft, NULL},
    {"fft", sizeof(FFT), 0, 1, "i[]", "i[]", (SUBR)fft_i, NULL, NULL},
    {"fftinv", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_ifft,
     (SUBR)perf_ifft, NULL},
    {"fftinv", sizeof(FFT), 0, 1, "i[]", "i[]", (SUBR)ifft_i, NULL, NULL},
    {"rect2pol", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_recttopol,
     (SUBR)perf_recttopol, NULL},
    {"pol2rect", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_recttopol,
     (SUBR)perf_poltorect, NULL},
    {"pol2rect.AA", sizeof(FFT), 0, 3, "k[]", "k[]k[]", (SUBR)init_poltorect2,
     (SUBR)perf_poltorect2, NULL},
    {"mags", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_mags, (SUBR)perf_mags,
     NULL},
    {"pows", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_mags, (SUBR)perf_pows,
     NULL},
    {"phs", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_mags, (SUBR)perf_phs,
     NULL},
    {"log", sizeof(FFT), 0, 3, "k[]", "k[]i", (SUBR)init_logarray,
     (SUBR)perf_logarray, NULL},
    {"r2c", sizeof(FFT), 0, 1, "i[]", "i[]", (SUBR)rtoc_i, NULL, NULL},
    {"r2c", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_rtoc, (SUBR)perf_rtoc,
     NULL},
    {"c2r", sizeof(FFT), 0, 1, "i[]", "i[]", (SUBR)ctor_i, NULL, NULL},
    {"c2r", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_ctor, (SUBR)perf_ctor,
     NULL},
    {"window", sizeof(FFT), 0, 3, "k[]", "k[]Op", (SUBR)init_window,
     (SUBR)perf_window, NULL},
    {"pvsceps", sizeof(PVSCEPS), 0, 3, "k[]", "fo", (SUBR)pvsceps_init,
     (SUBR)pvsceps_perf, NULL},
    {"cepsinv", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_iceps,
     (SUBR)perf_iceps, NULL},
    {"ceps", sizeof(FFT), 0, 3, "k[]", "k[]k", (SUBR)init_ceps, (SUBR)perf_ceps,
     NULL},
    {"getrow", sizeof(FFT), 0, 1, "i[]", "i[]i", (SUBR)rows_i, NULL, NULL},
    {"getrow.S", sizeof(FFT), 0, 3, "S[]", "S[]k", (SUBR)rows_init_S,
     (SUBR)rows_perf_S, NULL},
    {"getrow", sizeof(FFT), 0, 3, "k[]", "k[]k", (SUBR)rows_init,
     (SUBR)rows_perf, NULL},
    {"getcol", sizeof(FFT), 0, 1, "i[]", "i[]i", (SUBR)cols_i, NULL, NULL},
    {"getcol", sizeof(FFT), 0, 3, "k[]", "k[]k", (SUBR)cols_init,
     (SUBR)cols_perf, NULL},
    {"getcol", sizeof(FFT), 0, 3, "S[]", "S[]k", (SUBR)cols_init_S,
     (SUBR)cols_perf_S, NULL},
    {"setrow", sizeof(FFT), 0, 1, "i[]", "i[]i", (SUBR)set_rows_i, NULL, NULL},
    {"setrow", sizeof(FFT), 0, 3, "k[]", "k[]k", (SUBR)set_rows_init,
     (SUBR)set_rows_perf, NULL},
    {"setrow.S", sizeof(FFT), 0, 3, "S[]", "S[]k", (SUBR)set_rows_init_S,
     (SUBR)set_rows_perf_S, NULL},
    {"setcol", sizeof(FFT), 0, 1, "i[]", "i[]i", (SUBR)set_cols_i, NULL, NULL},
    {"setcol", sizeof(FFT), 0, 3, "k[]", "k[]k", (SUBR)set_cols_init,
     (SUBR)set_cols_perf, NULL},
    {"setcol", sizeof(FFT), 0, 3, "S[]", "S[]k", (SUBR)set_cols_init_S,
     (SUBR)set_cols_perf_S, NULL},
    {"shiftin", sizeof(FFT), 0, 3, "k[]", "a", (SUBR)shiftin_init,
     (SUBR)shiftin_perf},
    {"shiftout", sizeof(FFT), 0, 3, "a", "k[]o", (SUBR)shiftout_init,
     (SUBR)shiftout_perf},
    {"unwrap", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_recttopol,
     (SUBR)unwrap},
    {"=.X", sizeof(TABCOPY), 0, 3, "k[]", "k", (SUBR)scalarset,
     (SUBR)scalarset},
    {"=.Z", sizeof(TABCOPY), 0, 2, "a[]", "a", NULL, (SUBR)arrayass},
    {"dct", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_dct, (SUBR)kdct, NULL},
    {"dct", sizeof(FFT), 0, 1, "i[]", "i[]", (SUBR)dct, NULL, NULL},
    {"dctinv", sizeof(FFT), 0, 3, "k[]", "k[]", (SUBR)init_dctinv, (SUBR)kdct,
     NULL},
    {"dctinv", sizeof(FFT), 0, 1, "i[]", "i[]", (SUBR)dctinv, NULL, NULL},
    {"mfb", sizeof(MFB), 0, 3, "k[]", "k[]kki", (SUBR)mfb_init, (SUBR)mfb,
     NULL},
    {"mfb", sizeof(MFB), 0, 1, "i[]", "i[]iii", (SUBR)mfbi, NULL, NULL},
    {"centroid", sizeof(CENTR), 0, 1, "i", "i[]", (SUBR)array_centroid, NULL,
     NULL},
    {"centroid", sizeof(CENTR), 0, 2, "k", "k[]", NULL, (SUBR)array_centroid,
     NULL},
    {"interleave", sizeof(INTERL), 0, 1, "i[]", "i[]i[]", (SUBR)interleave_i},
    {"interleave", sizeof(INTERL), 0, 1, "k[]", "k[]k[]", (SUBR)interleave_i,
     (SUBR)interleave_perf},
    ...
    {"taninv2.Ak", sizeof(TABARITH), 0, 2, "k[]", "k[]k[]", (SUBR)tabarithset,
     (SUBR)taninv2_A},
    {"taninv2.Aa", sizeof(TABARITH), 0, 2, "a[]", "a[]a[]", (SUBR)tabarithset,
     (SUBR)taninv2_Aa},
    {"autocorr", sizeof(AUTOCORR), 0, 3, "k[]", "k[]", (SUBR)init_autocorr,
     (SUBR)perf_autocorr},
    {"clear", sizeof(TABCLEAR), 0, 3, "", "a[]", (SUBR)tabclearset,
     (SUBR)tabclear},
    {"clear", sizeof(TABCLEAR), 0, 3, "", "k[]", (SUBR)tabclearset,
     (SUBR)tabcleark}};
