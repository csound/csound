...
int32_t clarin(CSOUND *csound, CLARIN *p) {
  ...
  if ((--p->kloop) == 0) {
    ...
    csound->Message(csound, "Set off phase time = %f Breath v,r = %f, %f\n",
                    (MYFLT)CS_KCNT * CS_ONEDKR, p->envelope.value,
                    p->envelope.rate);
#endif
  }
  if (UNLIKELY(offset))
...
int32_t flute(CSOUND *csound, FLUTE *p) {
  ...
  if (p->kloop > FL(0.0) && p->h.insdshead->relesing)
    p->kloop = FL(1.0);
  if ((--p->kloop) == 0) {
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    randPress =
        noisegain * Noise_tick(csound, &p->noise); /* Random Deviation */
                                                   /* Tick on vibrato table */
...
MYFLT BowTabl_lookup(CSOUND *csound, BowTabl *b,
                     MYFLT sample) { /*  sample is differential  */
  ...
  /* if (lastOutput < FL(0.0) ) lastOutput = FL(0.0); */ /* minimum frict is 0.0
                                                          */
  if (lastOutput > FL(1.0))
    lastOutput = FL(1.0); /*  maximum friction is 1.0 */
  return lastOutput;
}
...
int32_t bowed(CSOUND *csound, BOWED *p) {
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    if (*p->vibAmt > FL(0.0)) {
      ...
      DLineL_setDelay(&p->neckDelay, (p->baseDelay * (FL(1.0) - p->lastbeta)));
...
int32_t DLineA_setDelay(CSOUND *csound, DLineA *p, MYFLT lag) {
  ...
  p->alpha =
      FL(1.0) + p->outPoint - outputPointer; /* fractional part of delay */
  if (p->alpha < FL(0.1)) {
  ...
  p->coeff =
      (FL(1.0) - p->alpha) / (FL(1.0) + p->alpha); /* coefficient for all pass*/
  return 0;
err1:
...
/*   Perform "Table Lookup" By Polynomial Calculation */
{
  ...
  if (temp > FL(1.0))
    temp = FL(1.0);            /* Saturation at + 1.0          */
  output = temp * mouthSample; /* Assume mouth input = area    */
  output +=
      (FL(1.0) - temp) * boreSample; /* and Bore reflection is compliment */
  return output;
}
...
int32_t brassset(CSOUND *csound, BRASS *p) {
  ...
  if (*p->lowestFreq >= FL(0.0)) {
    ...
    make_DCBlock(&p->dcBlock);
    make_ADSR(&p->adsr, CS_ESR);
    ADSR_setAllTimes(csound, &p->adsr, FL(0.005), FL(0.001), FL(1.0),
                     FL(0.010));
...
int32_t brass(CSOUND *csound, BRASS *p) {
  ...
  if (p->frq != *p->frequency) { /* Set frequency if changed */
    ...
    /* fudge correction for filter delays */
    /*  we'll play a harmonic */
    if (DLineA_setDelay(csound, &p->delayLine, p->slideTarget))
      return OK;
    p->lipTarget = p->frq;
...
static OENTRY physmod_localops[] = {
    {"wgclar", S(CLARIN), TR, 3, "a", "kkkiikkkjo", (SUBR)clarinset,
     (SUBR)clarin},
    {"wgflute", S(FLUTE), TR, 3, "a", "kkkiikkkjovv", (SUBR)fluteset,
     (SUBR)flute},
    {"wgbow", S(BOWED), TR, 3, "a", "kkkkkkjo", (SUBR)bowedset, (SUBR)bowed},
    {"wgbrass", S(BRASS), TR, 3, "a", "kkkikkjo", (SUBR)brassset, (SUBR)brass},
    {"mandol", S(MANDOL), TR, 3, "a", "kkkkkkio", (SUBR)mandolinset,
     (SUBR)mandolin},
    {"voice", S(VOICF), TR, 3, "a", "kkkkkkii", (SUBR)voicformset,
     (SUBR)voicform},
    {"fmbell", S(FM4OP), TR, 3, "a", "kkkkkkjjjjjo", (SUBR)tubebellset,
     (SUBR)tubebell},
    {"fmrhode", S(FM4OP), TR, 3, "a", "kkkkkkiiiii", (SUBR)rhodeset,
     (SUBR)tubebell},
    {"fmwurlie", S(FM4OP), TR, 3, "a", "kkkkkkiiiii", (SUBR)wurleyset,
     (SUBR)wurley},
    {"fmmetal", S(FM4OP), TR, 3, "a", "kkkkkkiiiii", (SUBR)heavymetset,
     (SUBR)heavymet},
    {"fmb3", S(FM4OP), TR, 3, "a", "kkkkkkjjjjj", (SUBR)b3set, (SUBR)hammondB3},
    {"fmvoice", S(FM4OPV), TR, 3, "a", "kkkkkkjjjjj", (SUBR)FMVoiceset,
     (SUBR)FMVoice},
    {"fmpercfl", S(FM4OP), TR, 3, "a", "kkkkkkjjjjj", (SUBR)percfluteset,
     (SUBR)percflute},
    {"moog", S(MOOG1), TR, 3, "a", "kkkkkkiii", (SUBR)Moog1set, (SUBR)Moog1},
    {"shaker", S(SHAKER), 0, 3, "a", "kkkkko", (SUBR)shakerset, (SUBR)shaker},
    {"wgbowedbar", S(BOWEDBAR), 0, 3, "a", "kkkkkoooo", (SUBR)bowedbarset,
     (SUBR)bowedbar},
    {"marimba", S(MARIMBA), TR, 3, "a", "kkiiikkiijj", (SUBR)marimbaset,
     (SUBR)marimba},
    {"vibes", S(VIBRAPHN), TR, 3, "a", "kkiiikkii", (SUBR)vibraphnset,
     (SUBR)vibraphn},
    {"gogobel", S(VIBRAPHN), TR, 3, "a", "kkiiikki", (SUBR)agogobelset,
     (SUBR)agogobel},
};
