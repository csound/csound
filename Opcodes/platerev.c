...
typedef struct {
    ...
    MYFLT       *ain[40];
 // Internals
    MYDBL       s00, s10, s01, s11, s20, s02, t00, t01, t10;
    uint32_t     nin, nout, Nx, Ny;
    MYDBL       *u, *u1, *u2;
    AUXCH        auxch;
    MYDBL       L, dy, dt;
    MYFLT        *in_param, *out_param;
    MYDBL       ci[40], si[40], co[40], so[40];
 } PLATE;
...
static int32_t platerev_init(CSOUND *csound, PLATE *p)
{
    FUNC *inp, *outp;
    MYDBL a = *p->asp;
    MYDBL dt = (p->dt = 1.0/CS_ESR); /* time step */
    MYDBL sig = (CS_ESR+CS_ESR)*
                 (POWER(10.0, FL(3.0)*dt/(*p->decay))-FL(1.0)); /* loss constant */
    MYDBL b2 = *p->loss;
    MYDBL dxmin = 2.0*sqrt(dt*(b2+hypot(*p->loss, *p->stiff)));
    uint32_t Nx = (p->Nx = (uint32_t)floor(1.0/dxmin));
    uint32_t Nx5 = Nx+5;
    MYDBL dx = 1.0/(MYDBL)Nx;
    uint32_t Ny = (p->Ny = (uint32_t)floor(a*Nx));
    MYDBL dy = (p->dy = *p->asp/Ny);
    MYDBL alf = dx/dy;
    MYDBL mu = dt*(*p->stiff)*Nx*Nx;
    MYDBL mu2 = mu*mu;
    MYDBL eta = 1.0/(1.0+sig*dt);
    MYDBL V = 2.0*b2*dt*Nx*Nx;
    uint32_t qq;
    ...
    p->L = (a<1.0 ? a : 1.0);
    csound->AuxAlloc(csound, 3*Nx5*(Ny+5)*sizeof(MYDBL), &p->auxch);
    p->u = (MYDBL*)p->auxch.auxp;
    p->u1 = p->u+Nx5*(Ny+5);
    p->u2 = p->u1+Nx5*(Ny+5);
...
static int32_t platerev(CSOUND *csound, PLATE *p)
{
    ...
    MYDBL *u = p->u, *u1 = p->u1, *u2 = p->u2;
    MYDBL s00 = p->s00, s10 = p->s10, s01 = p->s01,
           s11 = p->s11, s20 = p->s20, s02 = p->s02,
           t00 = p->t00, t10 = p->t10, t01 = p->t01;
    MYDBL dt = p->dt, dy = p->dy;
    uint32_t n, qq;
    MYDBL wi[40], wo[40], sdi[40], cdi[40], sdo[40], cdo[40];
    ...
    for (n=offset; n<nsmps; n++) {
      ...
      for (qq=0; qq<p->nin; qq++) {
        MYDBL w = wi[qq];
        MYDBL cv = p->ci[qq]*cdi[qq] - p->si[qq]*sdi[qq];
        MYDBL sv = p->ci[qq]*sdi[qq] + p->si[qq]*cdi[qq];
        MYDBL xid = (0.5+w*cv)*Nx;
        MYDBL yid = ((*p->asp)*0.5+w*sv)/dy;
        int32_t xi = (int32_t)(floor(xid))+2;
        int32_t yi = (int32_t)(floor(yid))+2;
        MYDBL xf = xid-(MYDBL)(xi-2);
        MYDBL yf = yid-(MYDBL)(yi-2);
        MYDBL xyf = xf*yf;
        MYDBL uin=(p->ain[qq])[n];
      ...
      for (qq=0; qq<p->nout; qq++) {
        MYDBL w = wo[qq];
        MYDBL cv = p->co[qq]*cdo[qq] - p->so[qq]*sdo[qq];
        MYDBL sv = p->co[qq]*sdo[qq] + p->so[qq]*cdo[qq];
        MYDBL xod = (0.5+w*cv)*Nx;
        MYDBL yod = (*p->asp*0.5+w*sv)/dy;
        int32_t xo = (int32_t)(floor(xod))+2;
        int32_t yo = (int32_t)(floor(yod))+2;
        MYDBL xf = xod-(MYDBL)(xo-2);
        MYDBL yf = yod-(MYDBL)(yo-2);
        MYDBL xyf = xf*yf;
