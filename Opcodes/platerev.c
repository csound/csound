...
static int32_t platerev_init(CSOUND *csound, PLATE *p) {
  ...
  double sig = (CS_ESR + CS_ESR) * (POWER(10.0, FL(3.0) * dt / (*p->decay)) -
                                    FL(1.0)); /* loss constant */
  double b2 = *p->loss;
  ...
  p->nin = (int32_t)(p->INOCOUNT) - 7;
  p->nout = (int32_t)(p->OUTOCOUNT);
  ...
  p->u2 = p->u1 + Nx5 * (Ny + 5);
  p->s00 = 2.0 * eta *
           (1.0 - mu2 * (3.0 + 4.0 * alf * alf + 3.0 * alf * alf * alf * alf) -
            V * (1.0 + alf * alf));
...
static int32_t platerev(CSOUND *csound, PLATE *p) {
  ...
  double s00 = p->s00, s10 = p->s10, s01 = p->s01, s11 = p->s11, s20 = p->s20,
         s02 = p->s02, t00 = p->t00, t10 = p->t10, t01 = p->t01;
  ...
  for (qq = 0; qq < (uint32_t)p->nin; qq++) {
    ...
    if (UNLIKELY(offset))
      memset(p->aout[qq], '\0', offset * sizeof(MYFLT));
    if (UNLIKELY(early))
      memset(&p->aout[qq][nsmps], '\0', early * sizeof(MYFLT));
  }
  for (n = offset; n < nsmps; n++) {
    ...
    for (j = 2; j < Ny + 3; j++) /* Loop from 2,3,...Nf, Nf+1, Nf+2 */
      for (i = 2; i < Nx + 3; i++) {
        int32_t ij = i + Nx5 * j;
        u[ij] = s00 * u1[ij] + s10 * (u1[ij - Nx5] + u1[ij + Nx5]) +
                s20 * (u1[ij - 2 * Nx5] + u1[ij + 2 * Nx5]) +
                s01 * (u1[ij - 1] + u1[ij + 1]) +
                s02 * (u1[ij - 2] + u1[ij + 2]) +
                s11 * (u1[ij - 1 - Nx5] + u1[ij + 1 + Nx5] + u1[ij - 1 + Nx5] +
                       u1[ij + 1 - Nx5]);
        u[ij] += t00 * u2[ij] + t10 * (u2[ij - Nx5] + u2[ij + Nx5]) +
                 t01 * (u2[ij - 1] + u2[ij + 1]);
      }
    ...
    if (bc == 1) { /* clamped*/
      ...
      for (i = 0; i < Nx + 5; i++) {
        u[i + Nx5 * 0] = u[i + Nx5 * 2] = u[i + Nx5 * (Ny + 2)] =
            u[i + Nx5 * (Ny + 4)] = 0.0;
      }
      for (i = 2; i < Nx + 3; i++) {
        u[i + Nx5 * 1] = u[i + Nx5 * 3];
      ...
      u[1 + Nx5 * 1] = u[1 + Nx5 * (Ny + 3)] = u[Nx + 3 + Nx5 * 1] =
          u[Nx + 3 + Nx5 * (Ny + 3)] = 0.0;
    ...
    for (qq = 0; qq < p->nout; qq++) {
      ...
      yo = yo * Nx5 + xo;
      (p->aout[qq])[n] =
          (MYFLT)((1.0 - xf - yf + xyf) * u[yo] + (yf - xyf) * u[Nx5 + yo] +
                  (xf - xyf) * u[1 + yo] + xyf * u[1 + Nx5 + yo]) /
          FL(25.0);
    }
    {
...
static OENTRY platerev_localops[] = {
    {"platerev", sizeof(PLATE), 0, 3,
     "mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm", "iikiiiiy",
