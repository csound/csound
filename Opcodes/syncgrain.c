  ...
  for (vecpos = offset; vecpos < vecsize; vecpos++) {
    ...
    for (j = 0; j < olaps; j++) {
      ...
      if (UNLIKELY(envindex[j] < envtablesize)) {
        ...
        sig += ((datap[(int32_t)index[j]] + (index[j] - (int32_t)index[j]) *
                                                (datap[(int32_t)index[j] + 1] -
                                                 datap[(int32_t)index[j]])) *
                (ftable[(int32_t)envindex[j]] +
                 (envindex[j] - (int32_t)envindex[j]) *
                     (ftable[(int32_t)envindex[j] + 1] -
                      ftable[(int32_t)envindex[j]])));
      }
...
static int32_t syncgrainloop_process(CSOUND *csound, syncgrainloop *p) {
  ...
  if (UNLIKELY(loop_start < 0))
    loop_start = 0;
  if (UNLIKELY(loop_start >= datasize))
    loop_start = datasize - 1;
  ...
  for (vecpos = offset; vecpos < vecsize; vecpos++) {
    ...
    /* depending on pitch transpsition a
       ...
       table.
     */
    for (i = numstreams, j = firststream; i; i--, j = (j + 1) % olaps) {
...
static int32_t filegrain_init(CSOUND *csound, filegrain *p) {
  ...
  if (p->buffer.auxp == NULL ||
    ...
    csound->AuxAlloc(csound, (p->dataframes + 1) * sizeof(MYFLT) * p->nChannels,
                     &p->buffer);
  ...
  if (UNLIKELY(fd == NULL)) {
    return csound->InitError(csound,
                             Str("diskgrain: could not open file: %s\n"),
  ...
  if (LIKELY(csound->SndfileRead(csound, p->sf, buffer, p->dataframes / 2) !=
             0)) {
    p->read1 = 1;
    p->read2 = 0;
  } else {
...
static int32_t filegrain_process(CSOUND *csound, filegrain *p) {
  MYFLT sig[DGRAIN_MAXCHAN], pitch, amp, grsize, envincr, period, fperiod,
      prate;
  MYFLT **output = p->output;
        ...
        if (trigger >= (dataframes - jump)) {
          ...
          if (!read1) {
            ...
            if (items < hdatasize) {
              ...
              csound->SndfileRead(csound, p->sf, datap + items,
                                  (hdatasize - items) / chans);
            }
            for (n = 0; n < chans; n++)
          ...
          if (!read2) {
            ...
            if (items < hdatasize) {
              ...
              csound->SndfileRead(csound, p->sf, datap + items + hdatasize,
                                  (hdatasize - items) / chans);
            }
            for (n = 0; n < chans; n++)
        ...
        if (trigger < jump) {
          ...
          if (!read1) {
            ...
            */
            ...
            csound->SndfileSeek(csound, p->sf, pos, SEEK_SET);
            items = csound->SndfileRead(csound, p->sf, datap + hdatasize,
                                        hdatasize / chans);
            if (items < hdatasize) {
              csound->SndfileSeek(csound, p->sf, items - hdatasize, SEEK_END);
              csound->SndfileRead(csound, p->sf, datap + hdatasize + items,
                                  (hdatasize - items) / chans);
            }
    ...
    for (i = numstreams, j = firststream; i; i--, j = (j + 1) % olaps) {
      ...
      for (n = 0; n < chans; n++) {
        ...
        sig[n] += ((datap[tndx + n] +
                   ...
                   (ftable[endx] +
                    (envindex[j] - endx) * (ftable[endx + 1] - ftable[endx])));
      }
