...
#define unirand(x)                                                             \
  ((MYFLT)(x->Rand31((x->RandSeed1(x))) - 1) / FL(2147483645.0))
...
static int32_t sndwarpgetset(CSOUND *csound, SNDWARP *p) {
  ...
  if ((auxp = p->auxch.auxp) == NULL || nsections != p->nsections) {
    ...
    csound->AuxAlloc(csound, (size_t)nsections * sizeof(WARPSECTION),
                     &p->auxch);
    auxp = p->auxch.auxp;
...
static OENTRY localops[] = {{"sndwarp", S(SNDWARP), TR, 3, "mm", "xxxiiiiiii",
                             (SUBR)sndwarpgetset, (SUBR)sndwarp},
                            {"sndwarpst", S(SNDWARPST), TR, 3, "mmmm",
                             "xxxiiiiiii", (SUBR)sndwarpstset,
                             (SUBR)sndwarpst}};
