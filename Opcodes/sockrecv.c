...
static int32_t init_srecv(CSOUND *csound, SOCKRECVT *p) {
  ...
  /* associate the socket with the address and port */
  err =
      bind(p->sock, (struct sockaddr *)&p->server_addr, sizeof(p->server_addr));
#if defined(WIN32) && !defined(__CYGWIN__)
...
static int32_t perf_raw_osc(CSOUND *csound, RAWOSC *p) {
  ...
  if (sout->sizes[0] < 2 || sout->dimensions > 1)
    return csound->PerfError(csound, &(p->h), "%s",
  ...
  struct sockaddr from;
  socklen_t clilen = sizeof(from);
  int32_t bytes = recvfrom(p->sock, (void *)buf, MTU - 1, 0, &from, &clilen);
  if (bytes < 0)
    bytes = 0;
...
static OENTRY sockrecv_localops[] = {
    {"sockrecv.k", S(SOCKRECV), 0, 3, "k", "ii", (SUBR)init_recv,
     (SUBR)send_recv_k, NULL},
    {"sockrecv.a", S(SOCKRECV), 0, 3, "a", "ii", (SUBR)init_recv,
     (SUBR)send_recv, NULL},
    ...
    {"strecv", S(SOCKRECVT), 0, 3, "az", "Si", (SUBR)init_srecv,
     (SUBR)send_srecv, NULL},
    {"OSCraw", S(RAWOSC), 0, 3, "S[]k", "i", (SUBR)init_raw_osc,
     (SUBR)perf_raw_osc, NULL, NULL}};
