...
void OLABuffer_writeFrame(OLABuffer *self, MYFLT *inputFrame,
                          int32_t frameIndex) {
...
void OLABuffer_readFrame(OLABuffer *self, MYFLT *outputFrame,
                         int32_t outputFrameOffset, int32_t olaBufferOffset,
                         int32_t samplesCount) {
  memcpy(&outputFrame[outputFrameOffset], &self->frames[0][olaBufferOffset],
         samplesCount * sizeof(MYFLT));
  ...
  for (i = 1; i < self->framesCount; ++i) {
    ...
    for (j = 0; j < samplesCount; ++j) {
      ...
      outputFrame[j + outputFrameOffset] +=
          self->frames[i][j + olaBufferOffset];
    }
  }
}
...
int32_t OLABuffer_process(CSOUND *csound, OLABuffer *self) {
  ...
  if (nextKPassSampleIndex == 0) {
    ...
    OLABuffer_readFrame(self, self->outputArgument, 0, self->readSampleIndex,
                        self->ksmps);
    self->frameIndex++;
    self->frameIndex %= self->framesCount;
  } else if (nextKPassSampleIndex < self->overlapSampleIndex) {

    int32_t firstHalfCount =
        self->overlapSamplesCount - self->overlapSampleIndex;
...
int32_t Framebuffer_initialise(CSOUND *csound, Framebuffer *self) {
  ...
  if (self->outputType == KRATE_ARRAY) {
    ...
    array->data =
        csound->Calloc(csound, var->memBlockSize * self->elementCount);
  }
...
void Framebuffer_checkArgumentSanity(CSOUND *csound, Framebuffer *self) {
  if (UNLIKELY((uint32_t)self->elementCount < self->h.insdshead->ksmps)) {
    ...
    csound->Die(csound, "%s",
                Str("framebuffer: Error, specified element "
