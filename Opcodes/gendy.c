...
static MYFLT gendy_distribution(CSOUND *csound, int32_t which, MYFLT a,
                                int32 rnd) {
  IGN(csound);
  MYFLT c, r;
  if (a > FL(1.0))
...
static int32_t kgendy(CSOUND *csound, GENDY *p) {
  ...
  if (p->phase >= FL(1.0)) {
    ...
    if (p->nextamp < FL(-1.0) || p->nextamp > FL(1.0)) {
      ...
      if (p->nextamp > FL(1.0)) {
        p->nextamp = (p->nextamp < FL(3.0) ? FL(2.0) - p->nextamp
                                           : p->nextamp - FL(4.0));
      }
    }
    ...
    memdur[index] = p->dur;
    p->speed = (minfreq + (maxfreq - minfreq) * p->dur) * CS_ONEDSR * knum;
  }
...
static int32_t kgendyx(CSOUND *csound, GENDYX *p) {
  ...
  *p->out =
      *p->kamp * (p->amp + POWER(p->phase, curve) * (p->nextamp - p->amp));
  p->phase += p->speed;
  return OK;
}
...
static OENTRY gendy_localops[] = {
    ...
    {"gendyx", 0xffff},
    {"gendyc", 0xffff},
    {"gendy.k", sizeof(GENDY), 0, 3, "k", "kkkkkkkkkoO", (SUBR)gendyset,
     (SUBR)kgendy, (SUBR)NULL},
    {"gendy.a", sizeof(GENDY), 0, 3, "a", "kkkkkkkkkoO", (SUBR)gendyset,
     (SUBR)agendy},
    {"gendyx.k", sizeof(GENDYX), 0, 3, "k", "kkkkkkkkkkkoO", (SUBR)gendyxset,
     (SUBR)kgendyx, (SUBR)NULL},
    {"gendyx.a", sizeof(GENDYX), 0, 3, "a", "kkkkkkkkkkkoO", (SUBR)gendyxset,
     (SUBR)agendyx},
    {"gendyc.k", sizeof(GENDYC), 0, 3, "k", "kkkkkkkkkoO", (SUBR)gendycset,
     (SUBR)kgendyc, (SUBR)NULL},
    {"gendyc.a", sizeof(GENDYC), 0, 3, "a", "kkkkkkkkkoO", (SUBR)gendycset,
     (SUBR)agendyc}};
