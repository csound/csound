...
void ptrack(CSOUND *csound, PITCHTRACK *p) {
  ...
  for (i = j = 0, k = 2 * FLTLEN; i < halfhop; i++, j += 8, k += 2) {
    ...
    re =
        COEF1 * (prev[k - 2] - prev[k + 1] + spectmp[k - 2] - prev[k + 1]) +
    ...
    im =
        COEF1 * (prev[k - 1] + prev[k] + spectmp[k - 1] + spectmp[k]) +
    ...
    k += 2;
    re =
        COEF1 * (prev[k - 2] - prev[k + 1] - spectmp[k - 2] + spectmp[k + 1]) +
  ...
  for (i = 0; i < n + 4 * FLTLEN; i++)
    prev[i] = spectmp[i];

  for (i = 0; i < MINBIN; i++)
    spec[4 * i + 2] = spec[4 * i + 3] = FL(0.0);
  ...
  if (totaldb >= p->amplo) {
    ...
    for (i = 4 * MINBIN; i < (4 * (n - 2)) && npeak < numpks; i += 4) {
      ...
      if (height < h1 || height < h2 || h1 < FL(0.00001) * totalpower ||
          h2 < FL(0.00001) * totalpower)
        continue;

      peakfr = ((spec[i - 8] - spec[i + 8]) *
                    (FL(2.0) * spec[i] - spec[i + 8] - spec[i - 8]) +
                (spec[i - 7] - spec[i + 9]) *
                    (FL(2.0) * spec[i + 1] - spec[i + 9] - spec[i - 7])) /
               (height + height);
      tmpfr1 = ((spec[i - 12] - spec[i + 4]) *
                    (FL(2.0) * spec[i - 4] - spec[i + 4] - spec[i - 12]) +
                (spec[i - 11] - spec[i + 5]) *
                    (FL(2.0) * spec[i - 3] - spec[i + 5] - spec[i - 11])) /
                   (FL(2.0) * h1) -
               1;
      tmpfr2 = ((spec[i - 4] - spec[i + 12]) *
                    (FL(2.0) * spec[i + 4] - spec[i + 12] - spec[i - 4]) +
                (spec[i - 3] - spec[i + 13]) *
                    (FL(2.0) * spec[i + 5] - spec[i + 13] - spec[i - 3])) /
                   (FL(2.0) * h2) +
               1;

      m = FL(0.333333333333) * (peakfr + tmpfr1 + tmpfr2);
      var =
          FL(0.5) * ((peakfr - m) * (peakfr - m) + (tmpfr1 - m) * (tmpfr1 - m) +
                     (tmpfr2 - m) * (tmpfr2 - m));

      totalfreq = (i >> 2) + m;
      if (var * totalpower > THRSH * height || var < FL(1.0e-30))
        continue;
    ...
    for (i = 0; i < npeak; i++) {
      ...
      MYFLT binbandwidth =
          FACTORTOBINS * peaklist[i].pwidth / peaklist[i].pfreq;
      ...
      for (j = 0; j < NPARTIALONSET; j++) {
        ...
        if (bin < maxbin) {
          MYFLT para, pphase,
              score =
                  FL(30.0) * weightamp / ((j + p->npartial) * weightbandwidth);
          ...
          for (k = 0, pphase = firstbin - bin; k <= ibw; k++, pphase += FL(1.0))
            histogram[k + firstbin] +=
                score * (FL(1.0) - para * pphase * pphase);
        }
      }
    }
    ...
    putfreq = EXP((FL(1.0) / BPEROOVERLOG2) * (histpeak.hindex + FL(96.0)));
    for (j = 0; j < npeak; j++) {
...
int32_t pitchtrackinit(CSOUND *csound, PITCHTRACK *p) {
  ...
  if (UNLIKELY(winsize != (1 << powtwo))) {
    csound->Warning(csound,
                    Str("ptrack: FFT size not a power of 2; using %d\n"),
                    winsize = (1 << powtwo));
  }
  p->hopsize = *p->size;
  ...
  if (!p->prev.auxp ||
      p->prev.size < (p->hopsize * 2 + 4 * FLTLEN) * sizeof(MYFLT)) {
    csound->AuxAlloc(csound, (p->hopsize * 2 + 4 * FLTLEN) * sizeof(MYFLT),
                     &p->prev);
  }
  ...
  if (!p->spec2.auxp ||
      p->spec2.size < (winsize * 4 + 4 * FLTLEN) * sizeof(MYFLT)) {
    csound->AuxAlloc(csound, (winsize * 4 + 4 * FLTLEN) * sizeof(MYFLT),
                     &p->spec2);
  }
  ...
  if (!p->peakarray.auxp ||
      p->peakarray.size < (p->numpks + 1) * sizeof(PEAK)) {
...
int32_t pitchafproc(CSOUND *csound, PITCHAF *p) {
  ...
  int32_t lag = p->lag, n, i, j, imax = 0, len = p->len, ksmps = CS_KSMPS;
...
void update_coefs(PLLTRACK *p, double fr, double Q, BIQUAD *biquad,
                  int32_t TYPE) {
...
static OENTRY pitchtrack_localops[] = {
    {"ptrack", S(PITCHTRACK), 0, 3, "kk", "aio", (SUBR)pitchtrackinit,
     (SUBR)pitchtrackprocess},
    {"pitchac", S(PITCHTRACK), 0, 3, "k", "akki", (SUBR)pitchafset,
     (SUBR)pitchafproc},
    {"plltrack", S(PLLTRACK), 0, 3, "aa", "akOOOOO", (SUBR)plltrack_set,
     (SUBR)plltrack_perf}
