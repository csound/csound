...
static int32_t pvsbandinit(CSOUND *csound, PVSBAND *p) {
  ...
  if (p->fin->sliding) {
    if (p->fout->frame.auxp == NULL ||
      ...
      memset(p->fout->frame.auxp, 0, CS_KSMPS * (N + 2) * sizeof(MYFLT));
  } else {
    if (p->fout->frame.auxp == NULL ||
        p->fout->frame.size < (N + 2) * sizeof(float)) /* RWD MUST be 32bit */
      csound->AuxAlloc(csound, (N + 2) * sizeof(float), &p->fout->frame);
    else
      memset(p->fout->frame.auxp, 0, (N + 2) * sizeof(float));
  }
  p->fout->N = N;
...
static int32_t pvsband(CSOUND *csound, PVSBAND *p) {
  ...
  if (p->fin->sliding) {
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (IS_ASIG_ARG(p->klowcut))
        lowcut = p->klowcut[n], change = 1;
      if (IS_ASIG_ARG(p->klowbnd))
        lowbnd = p->klowbnd[n], change = 1;
      if (IS_ASIG_ARG(p->khigbnd))
        higbnd = p->khigbnd[n], change = 1;
      if (IS_ASIG_ARG(p->khigcut))
        higcut = p->khigcut[n], change = 1;
      if (change) {
        if (lowcut < FL(0.0))
          lowcut = FL(0.0);
        if (lowbnd < lowcut)
      ...
      for (i = 0; i < NB - 1; i++) {
        ...
        if (afrq < lowcut || afrq > higcut) { /* outside band */
          ...
          if (fade != FL(0.0)) {
            fout[i].re =
                fin[i].re *
                (FL(1.0) - EXP(fade * (afrq - lowcut) / (lowbnd - lowcut))) /
                opef;
          } else
          ...
          if (fade != FL(0.0)) {
            fout[i].re =
                fin[i].re *
                (FL(1.0) - EXP(fade * (higcut - afrq) / (higcut - higbnd))) /
                opef;
          } else
  ...
  if (p->lastframe < p->fin->framecount) {
    for (i = 0; i < N; i += 2) {
      ...
      if (afrq < lowcut || afrq > higcut) {
        ...
        if (fade != FL(0.0))
          fout[i] = fin[i] *
                    (1.0f - expf(fade * (afrq - lowcut) / (lowbnd - lowcut))) /
                    opef;
        else
        ...
        if (fade != FL(0.0))
          fout[i] = fin[i] *
                    (1.0f - expf(fade * (higcut - afrq) / (higcut - higbnd))) /
                    opef;
        else
      ...
      for (i = 0; i < NB - 1; i++) {
        ...
        if (afrq < lowcut || afrq > higcut) {
          ...
          if (fade)
            fout[i].re =
                fin[i].re *
                (FL(1.0) - EXP(fade * (afrq - higbnd) / (higcut - higbnd))) /
                opef;
          else
  ...
  if (p->lastframe < p->fin->framecount) {
    for (i = 0; i < N; i += 2) {
      ...
      if (afrq < lowcut || afrq > higcut) {
        ...
        if (fade != FL(0.0))
          fout[i] = fin[i] *
                    (1.0f - expf(fade * (lowbnd - afrq) / (lowbnd - lowcut))) /
                    opef;
        else
        ...
        if (fade != FL(0.0))
          fout[i] = fin[i] *
                    (1.0f - expf(fade * (afrq - higbnd) / (higcut - higbnd))) /
                    opef;
        else
...
static OENTRY localops[] = {{"pvsbandp", sizeof(PVSBAND), 0, 3, "f", "fxxxxO",
