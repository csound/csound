...
static int32_t sc_reverb_init(CSOUND *csound, SC_REVERB *p) {
  ...
  for (i = 0; i < 8; i++) {
    p->delayLines[i] =
        (delayLine *)((unsigned char *)(p->auxData.auxp) + (int32_t)nBytes);
...
static int32_t sc_reverb_perf(CSOUND *csound, SC_REVERB *p) {
  ...
  for (i = offset; i < nsmps; i++) {
    ...
    for (n = 0; n < 8; n++) {
      ...
      lp->buf[lp->writePos] = (MYFLT)((n & 1 ? ainR : ainL) - lp->filterState);
...
int32_t reverbsc_init_(CSOUND *csound) {
  return csound->AppendOpcode(
      csound, "reverbsc", (int32_t)sizeof(SC_REVERB), 0, 3, "aa", "aakkjpo",
      (int32_t(*)(CSOUND *, void *))sc_reverb_init,
      (int32_t(*)(CSOUND *, void *))sc_reverb_perf, NULL);
}
