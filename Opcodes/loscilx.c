...
static int32_t sndload_opcode_init_(CSOUND *csound, SNDLOAD_OPCODE *p,
  ...
  case -1:
    ...
    break;
  case 0:
    sfinfo.format |= (int32_t)FORMAT2SF(parms.outformat);
    break;
  case 1:
    sfinfo.format |= (int32_t)FORMAT2SF(AE_CHAR);
    break;
  case 2:
    sfinfo.format |= (int32_t)FORMAT2SF(AE_ALAW);
    break;
  case 3:
    sfinfo.format |= (int32_t)FORMAT2SF(AE_ULAW);
    break;
  case 4:
    sfinfo.format |= (int32_t)FORMAT2SF(AE_SHORT);
    break;
  case 5:
    sfinfo.format |= (int32_t)FORMAT2SF(AE_LONG);
    break;
  case 6:
    sfinfo.format |= (int32_t)FORMAT2SF(AE_FLOAT);
    break;
  case 7:
    sfinfo.format |= (int32_t)FORMAT2SF(AE_UNCH);
    break;
  case 8:
    sfinfo.format |= (int32_t)FORMAT2SF(AE_24INT);
    break;
  case 9:
    sfinfo.format |= (int32_t)FORMAT2SF(AE_DOUBLE);
    break;
  default:
  ...
  if (loopMode >= 0) {
    if (UNLIKELY(loopMode > 3))
      return csound->InitError(csound, Str("invalid loop mode: %d"), loopMode);
...
static int32_t loscilx_opcode_init(CSOUND *csound, LOSCILX_OPCODE *p) {
  ...
  if (UNLIKELY(nChannels < 1 || nChannels > LOSCILX_MAXOUTS))
    return csound->InitError(
        csound, "%s", Str("loscilx: invalid number of output arguments"));
  p->nChannels = nChannels;
  if (csound->ISSTRCOD(*p->ifn)) {
    SNDMEMFILE *sf;

    p->usingFtable = 0;
    sf = csound->LoadSoundFile(
        csound, (char *)csound->GetString(csound, *p->ifn), (SFLIB_INFO *)NULL);
    if (UNLIKELY(sf == NULL))
    ...
    if (*(p->ibas) > FL(0.0)) {
      frqScale = sf->sampleRate / ((double)CS_ESR * (double)*(p->ibas));
    } else
  ...
  if (p->curLoopMode == 1) {
    if (loscilx_phase_int(p->curLoopStart + (int_least64_t)0x80000000U) == 0L) {
...
#endif
        {
          ar1 += ((MYFLT)((MYFLT *)p->dataPtr)[ndx + ndx] * (MYFLT)winBuf[j]);
          ar2 +=
              ((MYFLT)((MYFLT *)p->dataPtr)[ndx + ndx + 1L] * (MYFLT)winBuf[j]);
        }
      } while (++j < winSmps);
      /* scale output */
