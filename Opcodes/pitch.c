...
int32_t pitchset(CSOUND *csound, PITCH *p) /* pitch - uses spectra technology */
{
  ...
  if (UNLIKELY(*p->irolloff <= FL(0.0)))
    p->rolloff = FL(0.6);
  else
    p->rolloff = *p->irolloff;
  ...
  if (UNLIKELY((rolloff = p->rolloff) == FL(0.0) || rolloff == FL(1.0) ||
               nptls == 1)) {
    p->rolloff = FL(0.0);
  ...
  p->playing = 0;
  p->kvalsav =
      (*p->istrt >= FL(0.0) ? *p->istrt : (*p->ilo + *p->ihi) * FL(0.5));
...
int32_t pitch(CSOUND *csound, PITCH *p) {
  ...
  for (n = offset; n < nsmps; n++) {
    MYFLT as =
        asig[n] * DFLT_DBFS / csound->Get0dBFS(csound); /* Normalise.... */
    q = c1 * as * as + c2 * q;
  ...
  specp = &p->wsig;
  if (LIKELY((--p->scountdown)))
    goto nxt;                  /* if not yet time for new spec  */
...
nxt:
  /* specptrk */
  {
    ...
    lobin =
        (int32)((kval - kvar) * specp->nfreqs); /* set lims of frq interest */
...
int32_t adsyntset(CSOUND *csound, ADSYNT *p) {
  ...
  if (UNLIKELY(ftp->flen < count)) {
    ...
    return csound->InitError(
        csound, "%s",
        Str("adsynt: partial count is greater than freqtable size!"));
  }
  ...
  if (UNLIKELY(ftp->flen < count)) {
    ...
    return csound->InitError(
        csound, "%s",
        Str("adsynt: partial count is greater than amptable size!"));
  }
  ...
  if (*p->iphs > 1) {
    do {
      *lphs++ =
          ((int32)((MYFLT)((double)rand_31(csound) / 2147483645.0) * FMAXLEN)) &
          PHMASK;
    } while (--count);
  } else if (*p->iphs >= 0) {
    do {
...
MYFLT medianvalue(uint32 n,
                  MYFLT *vals) { /* vals must point to 1 below relevant data! */
  uint32 i, ir, j, l, mid;
  uint32 k = (n + 1) / 2;
  MYFLT a, temp;
      ...
      if (index == size) {
        ...
        if (medisize) {
          ...
          median[medisize * 2 + mediptr] =
              medianvalue(medisize, &median[medisize - 1]);
          peri = (int32)
              median[medisize * 2 + ((mediptr + medisize / 2 + 1) % medisize)];
...
int32_t pinkset(CSOUND *csound, PINKISH *p) {
  ...
  if (IS_ASIG_ARG(p->xin)) {
    ...
    if (UNLIKELY(*p->imethod != FL(0.0))) {
      return csound->InitError(
          csound, "%s",
          Str("pinkish: Filter method requires a-rate (noise) input"));
    }
    p->ampinc = 0;
  }
...
int32_t GardnerPink_init(CSOUND *csound, PINKISH *p) {
  ...
  if (*p->iparam1 >= 4 && *p->iparam1 <= GRD_MAX_RANDOM_ROWS)
    ...
    if (UNLIKELY(*p->iparam1 != FL(0.0)))
      csound->Warning(csound,
                      Str("pinkish: Gardner method requires 4-%d bands. "
  ...
  if (*p->iseed != FL(0.0)) {
    if (*p->iseed > -1.0 && *p->iseed < 1.0)
      ...
      p->randSeed = (uint32)*p->iseed;
  } else
    p->randSeed = (uint32)csound->GetRandomSeedFromTime();

  numRows = p->grd_NumRows;
  p->grd_Index = 0;
  if (numRows == 32)
    p->grd_IndexMask = 0xFFFFFFFF;
  else
    p->grd_IndexMask = (1 << numRows) - 1;
...
int32_t impulse(CSOUND *csound, IMPULSE *p) {
  ...
  if (UNLIKELY(next < (int32)nsmps)) { /* Impulse in this frame */
    ...
    if (frq == FL(0.0))
      sfreq = INT_MAX; /* Zero means infinite */
    else if (frq < FL(0.0))
      sfreq = -(int32_t)frq; /* Negative cnts in sample */
    else
      sfreq = (int32_t)(frq * CS_ESR); /* Normal case */
    if (UNLIKELY(offset))
      memset(ar, '\0', offset * sizeof(MYFLT));
    if (UNLIKELY(early)) {
      nsmps -= early;
...
int32_t trnset(CSOUND *csound, TRANSEG *p) {
  ...
  val = **argp++;
  if (**argp <= FL(0.0))
    return OK; /* if idur1 <= 0, skip init  */
  p->curval = val;
  p->curcnt = 0;
  ...
  *p->rslt = p->curval;                  /* put the cur value    */
  ...
  if (p->segsrem) {         /* done if no more segs */
    ...
    if (p->alpha == FL(0.0))
      ...
      p->curval = p->cursegp->val + p->curinc * (FL(1.0) - EXP(p->curx));
  }
  return OK;
}
...
int32_t lpf18db(CSOUND *csound, LPF18 *p) {
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    if (fco != lfc || flag) {
      ...
      kp = ((-FL(2.7528) * kfcn + FL(3.0429)) * kfcn + FL(1.718)) * kfcn -
           FL(0.9984);
      kp1 = kp + FL(1.0);
      kp1h = FL(0.5) * kp1;
      flag = 1;
    ...
    if (res != lrs || flag) {
      lrs = res;
      kres =
          res * (((-FL(2.7079) * kp1 + FL(10.963)) * kp1 - FL(14.934)) * kp1 +
                 FL(8.4974));
      flag = 1;
    }
    if (dist != lds || flag) {
...
int32_t medfilt(CSOUND *csound, MEDFILT *p) {
  ...
  if (UNLIKELY(kwind > maxwind)) {
    csound->Warning(
        csound, Str("median: window (%d)larger than maximum(%d); truncated"),
        kwind, maxwind);
    kwind = maxwind;
  }
  if (UNLIKELY(offset))
