...
static int32_t pluckExcite(CSOUND *csound, WGPLUCK *p) {
  ...
  pluckSetFilters(csound, p, *p->Aw0,
                  *p->AwPI); /*attenuation in dB at w0 and PI*/

  /* add the pick shape to the waveguide rails */
  if (UNLIKELY((shape = pluckShape(csound, p)) == NULL))
    return NOTOK;
...
static int32_t pluckPluck(CSOUND *csound, WGPLUCK *p) {
  ...
  waveguideWaveguide(csound, (waveguide *)&p->wg,         /* waveguide       */
...
static void pluckSetFilters(CSOUND *csound, WGPLUCK *p, MYFLT A_w0,
                            MYFLT A_PI) {
...
circularBufferCircularBuffer(CSOUND *csound, circularBuffer *cb, len_t N) {
  IGN(csound);
  MYFLT *data = cb->data;
  /* if (UNLIKELY(!data)) */
  /*   return csound->InitError(csound, */
  /*                            "%s", Str("wgpluck: Buffer memory not
   * allocated!")); */
...
#define guideRailGuideRail(csound, gr, d)                                      \
  circularBufferCircularBuffer(csound, gr, d)
 ...
 */
static void waveguideWaveguide(CSOUND *csound, waveguide *wg, MYFLT freq,
                               MYFLT *upperData, MYFLT *lowerData, MYFLT sr) {
  MYFLT size, df;
...
static OENTRY localops[] = {{"wgpluck", S(WGPLUCK), 0, 3, "a", "iikiiia",
