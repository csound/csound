...
int32_t instcount(CSOUND *csound, INSTCNT *p) {
  ...
  if (n < 0 || n > csound->engineState.maxinsno ||
    ...
    *p->cnt = ((*p->opt) ? (MYFLT)csound->engineState.instrtxtp[n]->instcnt
...
int32_t pfun(CSOUND *csound, PFUN *p) {
  ...
  if (n < 1)
    ...
    ans = csound->init_event->p[n];
  else if (csound->init_event->c.extra &&
           n < PMAX + csound->init_event->c.extra[0])
...
int32_t pfunk_init(CSOUND *csound, PFUNK *p) {
  ...
  csound->AuxAlloc(csound, (csound->init_event->pcnt + 1) * sizeof(MYFLT),
                   &p->pfield);
