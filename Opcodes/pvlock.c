...
typedef struct dats {
  ...
  AUXCH outframe[MAXOUTS], win, bwin[MAXOUTS], fwin[MAXOUTS], nwin[MAXOUTS],
      prev[MAXOUTS], framecount[MAXOUTS], fdata;
  MYFLT *indata[2];
  MYFLT *tab;
  int32_t curbuf;
  SNDFILE *sf;
...
typedef struct dats1 {
  OPDS h;
  MYFLT *out[1], *time, *kamp, *kpitch, *knum, *klock, *iN, *idecim, *konset,
      *offset, *dbthresh;
  int32_t cnt, hsize, curframe, N, decim, tscale;
  uint32_t nchans;
  double pos;
...
static int32_t sprocess1(CSOUND *csound, DATASPACE *p) {
  ...
  MYFLT pitch = *p->kpitch, *time = p->time, lock = *p->klock, *out,
        amp = *p->kamp;
  MYFLT *tab, frac;
  FUNC *ft;
  ...
  int64_t spos; // = p->pos;
  double pos;
  MYFLT *fwin, *bwin, in, *prev, *win = (MYFLT *)p->win.auxp;
  MYFLT *outframe;
    ...
    if (*p->klock) {
      for (i = 2; i < N - 4; i += 2) {
        ...
        if (p3 > p1 && p3 > p2 && p3 > p4 && p3 > p5) {
          ...
          d = 0.01 * freq;
          if (FABS(fout[i - 1] - freq) < d)
            fout[i - 1] = freq;
          if (FABS(fout[i + 1] - freq) < d)
            fout[i + 1] = freq;
          if (FABS(fout[i + 3] - freq) < d)
            fout[i + 3] = freq;
          if (FABS(fout[i + 5] - freq) < d)
            fout[i + 5] = freq;
          if (FABS(fout[i + 7] - freq) < d)
            fout[i + 7] = freq;
          // l+=1;
        }
      }
...
static OENTRY pvlock_localops[] = {
    {"mincer", sizeof(DATASPACEM), 0, 3, "a", "akkkkoo", (SUBR)sinit1m,
     (SUBR)sprocess1m},
    {"mincer", sizeof(DATASPACE), 0, 3, "mm", "akkkkoo", (SUBR)sinit1,
     (SUBR)sprocess1},
    {"temposcal", sizeof(DATASPACEM), 0, 3, "a", "kkkkkooPOP", (SUBR)sinit2m,
     (SUBR)sprocess2m},
    {"temposcal", sizeof(DATASPACE), 0, 3, "mm", "kkkkkooPOP", (SUBR)sinit2,
     (SUBR)sprocess2},
    {"filescal", sizeof(DATASPACE), 0, 3, "mm", "kkkSkooPOP", (SUBR)sinit3,
     (SUBR)sprocess3},
    {"hilbert2", sizeof(HILB), 0, 3, "aa", "aii", (SUBR)hilbert_init,
     (SUBR)hilbert_proc},
    {"fmanal", sizeof(AMFM), 0, 3, "aa", "aa", (SUBR)am_fm_init, (SUBR)am_fm},
