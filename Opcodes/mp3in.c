  ...
  if(filinit(pp->p.csound,pp) != OK) {
    if(pp->p.error > 0)
      ...
      pp->p.csound->Message(pp->p.csound,
                            "%s", Str("mp3scal_load error: could not open %s\n"),
                            pp->name->data);
  }
  return NULL;
}
  ...
  if(p->playing == 0){
    ...
    if(pthread_setschedparam((pp->p.t), policy,
      ...
      csound->Message(csound, "%s", Str("could not set priority\n"));
                                ...
                                "%s", Str("cannot load: player still active\n"));
  pp->res->data = (char *) p;
...
static int32_t check_init(CSOUND *csound, CHECK *p){
  ...
  else return csound->InitError(csound, "%s", Str("invalid handle\n"));
  return OK;
}
...
#else
  while(!p->p->N) usleep(1000);
  p->fwdsetup = csound->RealFFTSetup(csound,p->p->N,FFT_FWD);
  p->invsetup = csound->RealFFTSetup(csound,p->p->N,FFT_INV);
#endif
  p->start_flag = 1;
  return OK;
}
  ...
  for (n=offset; n < nsmps; n++) {
    ...
    if (cnt == hsize){
      ...
      for (j = 0; j < MP3_CHNS; j++) {
        ...
        fwin[N] = fw[1];
#else
        csound->RealFFT(csound, pp->fwdsetup, bwin);
        csound->RealFFT(csound, pp->fwdsetup, fwin);
#endif
        bwin[N] = bwin[1];
        bwin[N+1] = FL(0.0);
        fwin[N] = fwin[1];
        ...
        for(i=0; i < N+2; i++)
          fwin[i] = prev[i];
        fwin[1] = fwin[N];
        csound->RealFFT(csound, pp->invsetup, fwin);
#endif
        framecnt[curframe] = curframe*N;
        for (i=0;i<N;i++)
#ifdef HAVE_NEON
