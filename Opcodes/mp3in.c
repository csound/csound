...
<<<<<<< HEAD
  ...
  if(p->playing == 0){
    if(pthread_create(&(pp->p.t), NULL, loader_thread, pp) != 0)
      csound->Message(csound, "failed to start thread \n");
    struct sched_param param;
    int policy;
...
static int player_play(CSOUND *csound, PLAYER *pp)
{
  ...
  if(!p->init){
    ...
    csound->Message(csound, "not init \n");
    p->ti++;
    *pp->kstamp = 0;
    return OK;
  } else *pp->ilen = p->ilen;
...
static OENTRY mp3in_localops[] = {
  {"mp3in",  S(MP3IN),  0, 5, "mm", "Soooo",
   (SUBR) mp3ininit_S, NULL, (SUBR) mp3in},
  {"mp3in.i",  S(MP3IN),  0, 5, "mm", "ioooo",
   (SUBR) mp3ininit, NULL, (SUBR) mp3in},
