...
int32_t mp3ininit_(CSOUND *csound, MP3IN *p, int32_t stringname) {
  ...
  int32_t fd;
  mp3dec_t mpa = NULL;
  mpadec_config_t config = {MPADEC_CONFIG_FULL_QUALITY,
                            MPADEC_CONFIG_STEREO,
                            MPADEC_CONFIG_16BIT,
                            MPADEC_CONFIG_LITTLE_ENDIAN,
                            MPADEC_CONFIG_REPLAYGAIN_NONE,
                            TRUE,
                            TRUE,
                            TRUE,
                            0.0};
  ...
  int32_t skip;
  if (p->OUTOCOUNT == 1)
    config.mode = MPADEC_CONFIG_MONO;
  ...
  if (stringname == 0) {
    if (csound->ISSTRCOD(*p->iFileCode))
      ...
      csound->strarg2name(csound, name, p->iFileCode, "soundin.", 0);
  } else
    strncpy(name, ((STRINGDAT *)p->iFileCode)->data, 1023);

  if (UNLIKELY(csound->FileOpen2(csound, &fd, CSFILE_FD_R, name, "rb",
                                 "SFDIR;SSDIR", CSFTYPE_OTHER_BINARY,
                                 0) == NULL)) {
    mp3dec_uninit(mpa);
    return csound->InitError(csound, Str("mp3in: %s: failed to open file"),
                             name);
  }
  ...
  if (UNLIKELY((r = mp3dec_init_file(mpa, fd, 0, FALSE)) !=
               MP3DEC_RETCODE_OK)) {
    mp3dec_uninit(mpa);
    return csound->InitError(csound, "%s", mp3dec_error(r));
  }
  ...
  /* if (UNLIKELY(csound->oparms_.msglevel & WARNMSG)) */ {
    ...
    if (mpainfo.frequency < 16000)
      strcpy(temp, "MPEG-2.5 ");
    else if (mpainfo.frequency < 32000)
      strcpy(temp, "MPEG-2 ");
    else
      strcpy(temp, "MPEG-1 ");
    if (mpainfo.layer == 1)
      strcat(temp, "Layer I");
    else if (mpainfo.layer == 2)
      strcat(temp, "Layer II");
    else
      strcat(temp, "Layer III");
    csound->Warning(csound, "Input:  %s, %s, %d kbps, %d Hz  (%d:%02d)\n", temp,
                    ((mpainfo.channels > 1) ? "stereo" : "mono"),
  ...
  while (skip > 0) {
    ...
    if (xx > buffersize)
      xx = buffersize;
    skip -= xx;
    r = mp3dec_decode(mpa, p->buf, mpainfo.decoded_sample_size * xx,
                      &p->bufused);
    // printf("u %d\n", p->bufused);
  }
  // if(!skip)
...
int32_t mp3len_(CSOUND *csound, MP3LEN *p, int32_t stringname) {
  ...
  if (stringname == 0) {
    ...
    strncpy(name, ((STRINGDAT *)p->iFileCode)->data, 1023);
  if (UNLIKELY((file = csound->FileOpen2(csound, &fd, CSFILE_FD_R, name, "rb",
                                         "SFDIR;SSDIR", CSFTYPE_OTHER_BINARY,
                                         0)) == NULL)) {
    mp3dec_uninit(mpa);
    return csound->InitError(csound, Str("mp3in: %s: failed to open file"),
...
typedef struct dats {
  ...
  double pos;
  MYFLT accum;
  AUXCH outframe[MP3_CHNS], win, bwin[MP3_CHNS], fwin[MP3_CHNS], nwin[MP3_CHNS],
      prev[MP3_CHNS], framecount[MP3_CHNS], fdata[MP3_CHNS], buffer;
...
static int32_t sinit3_(CSOUND *csound, DATASPACE *p) {
  ...
  if (UNLIKELY(csound->FileOpen2(csound, &fd, CSFILE_FD_R, name, "rb",
                                 ...
                                 0) == NULL)) {
    mp3dec_uninit(mpa);
    return csound->InitError(csound, Str("mp3scale: %s: failed to open file"),
                             name);
  } // else
  ...
  */
  ...
  if (p->initDone == -1)
    csound->RegisterDeinitCallback(
        csound, p, (int32_t(*)(CSOUND *, void *))mp3scale_cleanup);
  p->initDone = -1;
  p->finished = 0;
  p->init = 1;
...
void fillbuf(CSOUND *csound, DATASPACE *p, int32_t nsmps) {
  ...
  if (!p->finished) {
    mp3dec_decode(p->mpa, p->buffer.auxp, MP3_CHNS * nsmps * sizeof(short),
                  &p->bufused);
    if (p->bufused == 0)
      p->finished = 1;
    else {
...
static int32_t sprocess3(CSOUND *csound, DATASPACE *p) {
  uint32_t offset = p->h.insdshead->ksmps_offset;
  uint32_t early = p->h.insdshead->ksmps_no_end;
  MYFLT pitch = *p->kpitch * p->resamp, time = *p->time * p->resamp,
        lock = *p->klock;
  MYFLT *out;
  ...
  AUXCH *mfwin = p->fwin, *mbwin = p->bwin, *mprev = p->prev,
        *moutframe = p->outframe, *mframecount = p->framecount;
  ...
  if (*p->kstamp == 0)
    csound->Message(csound, "waited %d cycles\n", p->ti);
...
void fillbuf2(CSOUND *csound, MP3SCAL2 *p, int32_t nsmps) {
  ...
  if (p->async) {
    ...
    pthread_getschedparam(p->t1, &policy, &param);
    param.sched_priority = 1;
    // policy = SCHED_OTHER;
    if (pthread_setschedparam(p->t1, policy, &param) != 0)
      csound->Message(csound, "%s", Str("could not set priority\n"));
  } else
    buffiller((void *)p);
}
...
void *loader_thread(void *p) {
  ...
  if (filinit(pp->p.csound, pp) != OK) {
    if (pp->p.error > 0)
      pp->p.csound->Message(pp->p.csound, "%s", Str("mp3scal_load error: %s\n"),
                            mp3dec_error(pp->p.error));
    else
      pp->p.csound->Message(pp->p.csound, "%s",
...
static int32_t loader_init(CSOUND *csound, LOADER *pp) {
  ...
  if (p->playing == 0) {
    if (pthread_create(&(pp->p.t), NULL, loader_thread, pp) != 0)
      csound->Message(csound, "%s", Str("failed to start thread\n"));
    struct sched_param param;
    int32_t policy;
    pthread_getschedparam((pp->p.t), &policy, &param);
    param.sched_priority = 0;
    policy = SCHED_OTHER;
    if (pthread_setschedparam((pp->p.t), policy, &param) != 0)
      csound->Message(csound, "%s", Str("could not set priority\n"));
  } else
...
static int32_t check_init(CSOUND *csound, CHECK *p) {
  if (p->pp->data != NULL && p->pp->size != sizeof(MP3SCAL2)) {
...
static int32_t player_init(CSOUND *csound, PLAYER *p) {
  ...
  *p->ilen = p->p->ilen;
  ...
  if (p->p->initDone == 0)
    csound->RegisterDeinitCallback(
        csound, p, (int32_t(*)(CSOUND *, void *))mp3dec_cleanup_player);
  p->p->initDone = 1;
...
#ifdef HAVE_NEON
#include <arm_neon.h>
static inline void cmplx_multiply_scal(MYFLT *ans_r, MYFLT *ans_i, MYFLT *in1,
                                       MYFLT *in2, MYFLT div, bool neg) {
...
static inline MYFLT invsqrt(MYFLT x) {
  int64_t i;
  float x2 = x * 0.5f;
  i = *(int64_t *)&x;
  i = 0x5f3759df - (i >> 1);
...
static inline MYFLT inv_mag_(MYFLT *a) {
  float32x2_t ans, op;
  float32_t vans[2];
  op = vld1_f32(a);
  ans = vmul_f32(op, op);
...
static inline MYFLT inv_mag(MYFLT *a) {
  return FL(1.0) / (hypot(a[0], a[1]) + 1.0e-15);
  // return invsqrt(a[0]*a[0]+a[1]*a[1]);
}
static inline MYFLT inv_mag2(MYFLT *a) { // FIXME: use HYPOT
  a[0] += 1.0e-15;
...
static int32_t player_play(CSOUND *csound, PLAYER *pp) {
  ...
  if (!p->init) {
    ...
    csound->Message(csound, "%s", Str("not init\n"));
    p->ti++;
    *pp->kstamp = 0;
    return OK;
  } else
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    if (cnt == hsize) {
      ...
      if (spos > size / 8 + hsize && p->curbuf == 0 && p->filling == 0) {
        ...
        fillbuf2(csound, p, size / 8);
        p->filling = 2;
      } else if (spos > 3 * size / 8 + hsize && p->curbuf == 2 &&
                 p->filling == 2) {
        fillbuf2(csound, p, size / 8);
        p->filling = 3;
        ...
        fillbuf2(csound, p, size / 8);
        p->filling = 4;
      } else if (spos > 5 * size / 8 + hsize && p->curbuf == 4 &&
                 p->filling == 4) {
        fillbuf2(csound, p, size / 8);
        p->filling = 5;
      } else if (spos > 3 * size / 4 + hsize && p->curbuf == 5 &&
                 p->filling == 5) {
        fillbuf2(csound, p, size / 8);
        p->filling = 6;
      } else if (spos > 7 * size / 8 + hsize && p->curbuf == 6 &&
                 p->filling == 6) {
        fillbuf2(csound, p, size / 8);
        p->filling = 7;
      ...
      for (j = 0; j < MP3_CHNS; j++) {
        ...
        for (i = 0; i < N; i += 2) {
          div = inv_mag(&prev[i]);
          cmplx_multiply_scal(&prev[i], &prev[i + 1], &bwin[i], &prev[i], div,
                              true);
        }
        ...
        if (lock) {
          ...
          memcpy(bwin, prev, sizeof(MYFLT) * (N + 2));

        for (i = 0; i < N; i += 2) {
          div = inv_mag2(&bwin[i]);
          cmplx_multiply_scal(&prev[i], &prev[i + 1], &fwin[i], &bwin[i], div,
                              false);
        }
#ifdef HAVE_NEON
        for (i = 0; i < N; i++)
...
static OENTRY mp3in_localops[] = {
    ...
    {"mp3scal", sizeof(DATASPACE), 0, 3, "aak", "SkkkoooPP", (SUBR)sinit3,
     (SUBR)sprocess3},
#ifdef ANDROID
    {"mp3scal_load", sizeof(LOADER), 0, 1, "i", "Soooo", (SUBR)loader_init,
     NULL, NULL},
    {"mp3scal_play", sizeof(PLAYER), 0, 3, "aaki", "ikkkPPo", (SUBR)player_init,
     (SUBR)player_play},
    {"mp3scal_check", sizeof(CHECK), 0, 3, "k", "i", (SUBR)check_init,
     (SUBR)check_play},
#endif
    /*
...
int gen49raw(FGDATA *ff, FUNC *ftp) {
  ...
  switch (chan) {
  case 0:
    config.mode = MPADEC_CONFIG_AUTO;
    break;
  case 1:
    config.mode = MPADEC_CONFIG_MONO;
    break;
  ...
  (void)csound->FileOpen2(csound, &fd, CSFILE_FD_R, sfname, NULL, "SFDIR;SSDIR",
...
static NGFENS mp3in_fgen[] = {{"gen49", gen49}, {NULL, NULL}};
...
int32_t mp3in_localops_init(CSOUND *csound, OENTRY **ep) {
  (void)csound;
  *ep = mp3in_localops;
