...
int32_t pvset_(CSOUND *csound, PVOC *p, int32_t stringname) {
  ...
  if (*p->igatefun > 0)
    if (UNLIKELY((AmpGateFunc = csound->FTnp2Find(csound, p->igatefun)) ==
                 NULL))
      return NOTOK;
...
int32_t pvoc(CSOUND *csound, PVOC *p) {
  ...
  if (UNLIKELY(outlen <
               (int32_t)(2 * nsmps))) /* minimum post-squeeze windowlength */
    goto err3;
  ...
  addToCircBuf(buf2 + nsmps, p->outBuf, p->opBpos, buf2Size - nsmps,
               circBufSize);
