...
static int32_t pvsdemix_init(CSOUND *csound, PVSDEMIX *p) {
  ...
  if (!((p->fout->format == PVS_AMP_FREQ) ||
    ...
    return csound->InitError(
        csound, "pvsdemix: signal format must be amp-phase or amp-freq.\n");
...
static int32_t pvsdemix_process(CSOUND *csound, PVSDEMIX *p) {
  ...
  if (UNLIKELY(!fsigs_equal(p->finleft, p->finright)))
    goto err1;
  ...
  if (p->lastframe < p->finleft->framecount) {
    ...
    for (n = 0; n < N / 2 + 1; n++) {
      ...
      */
      ...
      out[n2] = sum;
      out[n2 + 1] = (azimuth < 0 ? sigl[n2 + 1] : sigr[n2 + 1]);
...
static OENTRY localops[] = {{"pvsdemix", sizeof(PVSDEMIX), 0, 3, "f", "ffkki",
                             (SUBR)pvsdemix_init, (SUBR)pvsdemix_process,
                             (SUBR)NULL}};
