...
int32_t pvbufreadset_(CSOUND *csound, PVBUFREAD *p, int32_t stringname) {
  ...
  if (UNLIKELY(csound->PVOCEX_LoadFile(csound, pvfilnam, &pp) != 0))
    return csound->InitError(csound, Str("PVBUFREAD cannot load %s"), pvfilnam);
...
int32_t pvinterpset_(CSOUND *csound, PVINTERP *p, int32_t stringname) {
  ...
  ;
  if (UNLIKELY(csound->PVOCEX_LoadFile(csound, pvfilnam, &pp) != 0))
    return csound->InitError(csound, Str("PVINTERP cannot load %s"), pvfilnam);
  ...
  if (UNLIKELY(p->frSiz != p->pvbufread->frSiz)) {
    return csound->InitError(csound,
                             ...
                             pvfilnam, (int32_t)p->frSiz,
                             (int32_t)p->pvbufread->frSiz);
  }
  if (UNLIKELY(chans != 1)) {
    return csound->InitError(csound, Str("%d chans (not 1) in PVOC file %s"),
                             (int32_t)chans, pvfilnam);
  ...
  if (UNLIKELY((OPWLEN / 2 + 1) > PVWINLEN)) {
    return csound->InitError(csound,
                             Str("ksmps of %d needs wdw of %d, "
                                 "max is %d for pv %s"),
                             CS_KSMPS, (OPWLEN / 2 + 1), PVWINLEN, pvfilnam);
  }
...
int32_t pvinterp(CSOUND *csound, PVINTERP *p) {
  ...
  if (UNLIKELY(outlen >
               PVFFTSIZE)) /* Maximum transposition down is one octave */
  ...
  addToCircBuf(buf2 + CS_KSMPS, p->outBuf, p->opBpos, buf2Size - CS_KSMPS,
               circBufSize);
...
int32_t pvcross(CSOUND *csound, PVCROSS *p) {
  ...
  // #ifdef BETA
  //  if (specwp < 0)
  //    csound->Message(csound, "%s", Str("PVOC debug: one frame gets
  //    through\n"));
  // #endif
