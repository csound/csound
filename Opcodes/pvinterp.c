...
int32_t pvbufreadset_(CSOUND *csound, PVBUFREAD *p, int32_t stringname)
{
  ...
  p->frPrtim = (*p->ifiletime != FL(0.0) ? p->asr : CS_ESR) / (cs_float) frInc;
  p->prFlg = 1;       /* true */
...
int32_t pvinterpset_(CSOUND *csound, PVINTERP *p, int32_t stringname)
{
  ...
  /* Set up time window */
  memset(p->lastPhase,'\0', pvdasiz(p)*sizeof(cs_float));
  ...
  for (i = 0; i < OPWLEN / 2 + 1; ++i)    /* time window is OPWLEN long */
    p->window[i] = (cs_float) (0.5 - 0.5 * cos(TWOPI*(cs_double)i/(cs_double)OPWLEN));
  /* NB: HANNING */
  memset(p->outBuf, 0, pvfrsiz(p)*sizeof(cs_float));
...
int32_t pvinterp(CSOUND *csound, PVINTERP *p)
{
  ...
  if (pex != FL(1.0))
    UDSample(p->pp, buf,
             (FL(0.5) * ((cs_float) size - pex * (cs_float) buf2Size)), buf2,
             size, buf2Size, pex);
  else
...
int32_t pvcross(CSOUND *csound, PVCROSS *p)
{
  ...
  PVBUFREAD *q = p->pvbufread;
  int32   i, j;
  cs_float   ampscale1 = *p->kampscale1;
  cs_float   ampscale2 = *p->kampscale2;
  ...
  */
  if(specwp == 0 || ((p->prFlg)++ != 0 && (int32_t) specwp)) {
    ...
    memset(buf2, 0, buf2Size*sizeof(cs_float));
  }
