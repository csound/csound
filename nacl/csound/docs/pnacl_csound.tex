...
\title{Csound for Portable Native Clients}
...
Native Clients (NaCl) is a sandboxing technology developed by Google that allows C/C++ modules to extend the support provided
by HTML5. Portable Native Clients (pNaCl) is one of the toolchains in the NaCl SDK (the others are newlib and glibc). The advantage
...
compiled to a native binary by the browser. To check whether your version of Chrome supports pNaCl, use the following address:
\\
...
in the top level directory of the pNaCl Csound package, then the example will be found at the http://localhost:5103 address.

\section{Csound NaCl module reference}
...
\item {\tt csound.SetChannel(name, value)} - sends the control channel \emph{name} the value \emph{value}
\end{itemize}
