...
extern "C"
{
   ...
   *
   * When a breakpoint is reached, the debugger will call a function of this
   * type see csoundSetBreakpointCallback() */
  typedef void (*breakpoint_cb_t) (CSOUND *, debug_bkpt_info_t *,
                                   void *userdata);
   ...
   *
   */
  PUBLIC void csoundSetBreakpoint (CSOUND *csound, int line, int instr,
                                   int skip);
   ...
   * particular instances. Specifying a value greater than 1 will result in
   * that number of control blocks being skipped before this breakpoint is
   * called again. A value of 0 and 1 has the same effect.
   *
   * This call is thread safe, as the breakpoint will be put in a lock free
   * queue that is processed as soon as possible in the kperf function.
   *
   * @param csound a Csound instance
  ...
  */
   ...
   *
   * Calling this function will enter the debugger at the soonest possible
   * point as if a breakpoint had been reached.
   */
   ...
   * longer needed.
   * This function is not thread safe and should only be called while the
   * csound engine is stopped at a breakpoint.
   */
