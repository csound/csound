...
#define CS_FLOAT_ALIGN(x)                                                     \
  ((int)(x + sizeof (MYFLT) - 1) & (~(sizeof (MYFLT) - 1)))
...
#ifndef USE_DOUBLE
#define MYFLT2LONG(x)                                                         \
  (x > LONG_MIN && x < LONG_MAX ? (int32)lrintf ((float)(x)) : 0)
#define MYFLT2LRND(x)                                                         \
  (x > LONG_MIN && x < LONG_MAX ? (int32)lrintf ((float)(x)) : 0)
#else
#define MYFLT2LONG(x)                                                         \
  (x > LONG_MIN && x < LONG_MAX ? (int32)lrint ((double)(x)) : 0)
#define MYFLT2LRND(x)                                                         \
  (x > LONG_MIN && x < LONG_MAX ? (int32)lrint ((double)(x)) : 0)
#define MYFLT2LONG64(x)                                                       \
  (x > LONG_MIN && x < LONG_MAX ? (int64_t)lrintl ((double)(x)) : 0)
#define MYFLT2LRND64(x)                                                       \
  (x > LONG_MIN && x < LONG_MAX ? (int64_t)lrintl ((double)(x)) : 0)
#endif
#elif defined(MSVC)
#include <emmintrin.h>
...
static inline int
MYFLT2LONG (float const x)
{
  return _mm_cvtss_si32 (_mm_load_ss (&x));
}

static inline int
MYFLT2LRND (float const x)
{
  return _mm_cvtss_si32 (_mm_load_ss (&x));
}
...
#else
static inline int
MYFLT2LONG (double const x)
{
  return _mm_cvtsd_si32 (_mm_load_sd (&x));
}

static inline int
MYFLT2LRND (double const x)
{
  return _mm_cvtsd_si32 (_mm_load_sd (&x));
}
#endif
...
static inline int32
MYFLT2LRND (double fval)
{
  return ((int32)(fval + (fval < 0.0 ? -0.5 : 0.5)));
}

static inline int64
MYFLT2LRND64 (double fval)
{
...
static inline float
csoundUndenormalizeFloat (float x)
{
...
csoundUndenormalizeDouble (double x)
{
