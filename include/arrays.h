...
static inline void
tabinit (CSOUND *csound, ARRAYDAT *p, int size)
{
  size_t ss;
  if (p->dimensions == 0)
    {
...
static inline void
tabinit_like (CSOUND *csound, ARRAYDAT *p, ARRAYDAT *tp)
{
  uint32_t ss = 1;
  if (p->data == tp->data)
    {
  ...
  if (p->dimensions != tp->dimensions)
    {
      p->sizes = (int32_t *)csound->ReAlloc (
          csound, p->sizes, sizeof (int32_t) * tp->dimensions);
...
static inline int
tabcheck (CSOUND *csound, ARRAYDAT *p, int size, OPDS *q)
{
    ...
    { /* was arr->allocate */
      return csound->PerfError (
          ...
          Str ("Array too small (allocated %zu < needed %zu), but cannot "
               "allocate during performance pass. Allocate a bigger array at "
               "init time"),
          p->allocated, s);
      return NOTOK;
    }
