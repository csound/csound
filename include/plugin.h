...
public:
   ...
   */
  int
  perf_error (const std::string &s, OPDS *inst)
  {
  ...
  midi_channel (OPDS *p)
  {
    return GetMidiChannelNumber (p);
  }

  /** midi note number for this instrument
   */
  int
  midi_note_num (OPDS *p)
  {
    return GetMidiNoteNumber (p);
  }

  /** midi note velocity for this instrument
   */
  int
  midi_note_vel (OPDS *p)
  {
    return GetMidiVelocity (p);
  }

  /** midi aftertouch for this channel
   */
  MYFLT
  midi_chn_aftertouch (OPDS *p)
  {
   ...
   */
  MYFLT
  midi_chn_polytouch (OPDS *p, uint32_t note)
  {
   ...
   */
  MYFLT
  midi_chn_ctl (OPDS *p, uint32_t ctl)
  {
   ...
   */
  fftp
  fft_setup (uint32_t size, uint32_t direction)
  {
  ...
  */
  std::complex<MYFLT> *
  fft (fftp setup, std::complex<MYFLT> *data)
  {
   ...
   */
  int
  create_global_variable (const char *name, size_t nbytes)
  {
   ...
   */
  void *
  query_global_variable (const char *name)
  {
   ...
   */
  int
  destroy_global_variable (const char *name)
  {
...
public:
  ...
  AudioSig (OPDS *p, MYFLT *s, bool res = false)
      : early (p->insdshead->ksmps_no_end),
        offset (p->insdshead->ksmps_offset),
...
public:
  ...
  {
    return (T *)((char *)data + sizes[0] * arrayMemberSize);
  }
  ...
  elem_offset ()
  {
    return arrayMemberSize / sizeof (T);
  }
...
public:
  ...
  operator pvscmplx & ()
  {
    return (pvscmplx &)reinterpret_cast<T (&)[2]> (*this);
  }
...
public:
   ...
   */
  void
  init (Csound *csound, int32_t n, int32_t h, int32_t w, int32_t t, int32_t f,
        int32_t nb = 0, int32_t sl = 0, uint32_t nsmps = 1)
  {
    N = n;
    overlap = h;
    winsize = w;
  ...
  void
  init (Csound *csound, const Fsig &f, uint32_t nsmps = 1)
  {
...
public:
  ...
  {
    return (const T *)(frame.auxp + N / 2 + 1);
  }
...
template <std::size_t N> struct InPlug : OPDS
{
  ...
  */
  ...
  midi_channel ()
  {
    return ((CSOUND *)csound)->GetMidiChannelNumber (this);
  }
  ...
  midi_note_num ()
  {
    return ((CSOUND *)csound)->GetMidiNoteNumber (this);
  }
  ...
  midi_note_vel ()
  {
    return ((CSOUND *)csound)->GetMidiVelocity (this);
  }
   ...
   */
  MYFLT
  midi_chn_polytouch (uint32_t note)
  {
...
template <typename T>
int
plugin (Csound *csound, const char *name, const char *oargs, const char *iargs,
...
template <typename T>
int
plugin (Csound *csound, const char *name, uint32_t thr, uint32_t flags = 0)
{
  CSOUND *cs = (CSOUND *)csound;
  if (thr == thread::ia || thr == thread::a)
    {
      ...
      return cs->AppendOpcode (cs, (char *)name, sizeof (T), flags, thr,
                               ...
                               (SUBR)init<T>, (SUBR)aperf<T>, NULL);
    }
  else
    return cs->AppendOpcode (cs, (char *)name, sizeof (T), flags, thr,
                             (char *)T::otypes, (char *)T::itypes,
                             (SUBR)init<T>, (SUBR)kperf<T>, NULL);
}
