...
static char *my_fgets(CSOUND *csound, char *s, int n, FILE *stream) {
  char *a = s;
  if (UNLIKELY(n <= 1))
    return NULL; /* best of a bad deal */
  do {
    int ch = getc(stream);
    if (UNLIKELY(ch == EOF)) { /* error or EOF       */
      if (s == a)
        return NULL; /* no chars -> leave  */
      if (ferror(stream))
        a = NULL;
...
nxt:
  while (my_fgets_cf(csound, buffer, CSD_MAX_LINE_LEN, cf) != NULL) {
    ...
    if (state == 0 && (q = strstr(p, "</CsInstruments>")) &&
        all_blank(buffer, q)) {
    ...
    if (state == 0) {
      while ((c = *p++)) {
        if (c == '"') {
          ...
          corfile_puts(csound, p - 1, incore);
          goto nxt;
        } else if (c == '{' && *p == '{') {
          corfile_putc(csound, c, incore);
          corfile_putc(csound, *p++, incore);
...
static int createOrchestra(CSOUND *csound, CORFIL *cf) {
  ...
  while (my_fgets_cf(csound, buffer, CSD_MAX_LINE_LEN, cf) != NULL) {
    ...
    if (comm == 0 && strstr(p, "</CsInstruments>") == p) {
      // csound->Message(csound, "closing tag\n");
      // corfile_flush(incore);
      corfile_puts(csound, "\n#exit\n", incore);
      corfile_putc(csound, '\0', incore);
...
nxt:
  while (my_fgets_cf(csound, buffer, CSD_MAX_LINE_LEN, cf) != NULL) {
    int c;
    p = buffer;
    if (state == 0 && (q = strstr(p, "</CsScore>")) && all_blank(buffer, q)) {
      corfile_puts(csound, "\ne\n#exit\n", csound->scorestr);
    ...
    if (state == 0) {
      while ((c = *p++)) {
        if (c == '"') {
          ...
          corfile_puts(csound, p - 1, csound->scorestr);
          goto nxt;
        }
        corfile_putc(csound, c, csound->scorestr);
      }
  ...
  *q = '\0';
  ...
  while (my_fgets_cf(csound, buffer, CSD_MAX_LINE_LEN, cf) != NULL) {
    ...
    if (strstr(p, "</CsScore>") == p) {
      ...
      fd = csoundFileOpenWithType(csound, &scof, CSFILE_STD, STA(sconame), "r",
                                  NULL, CSFTYPE_SCORE, 0);
      if (UNLIKELY(fd == NULL)) {
...
static int createMIDI2(CSOUND *csound, CORFIL *cf) {
  ...
  fd = csoundFileOpenWithType(csound, &midf, CSFILE_STD, STA(midname), "wb",
                              NULL, CSFTYPE_STD_MIDI, 1);
  if (UNLIKELY(fd == NULL)) {
  ...
  while (my_fgets_cf(csound, buffer, CSD_MAX_LINE_LEN, cf) != NULL) {
    ...
    if (strstr(p, "Before") != NULL) {
      ...
      if (UNLIKELY(version >= ((major * 1000) + (minor * 10)))) {
        csoundDie(csound,
                  Str("This CSD file requires a version of "
                      "Csound before %d.%02d"),
                  major, minor);
        result = FALSE;
      }
      ...
      if (UNLIKELY(version < ((major * 1000) + (minor * 10)))) {
        csoundDie(csound,
                  Str("This CSD file requires version "
  ...
  while (my_fgets_cf(csound, buffer, CSD_MAX_LINE_LEN, cf)) {
    ...
    if (strstr(p, "<CsoundSynthesizer>") == p ||
      ...
      if (!notrunning) {
        ...
        do {
          if (UNLIKELY(my_fgets_cf(csound, buffer, CSD_MAX_LINE_LEN, cf) ==
                       NULL)) {
            csoundErrorMsg(csound, Str("Missing end tag </CsOptions>"));
            result = FALSE;
            break;
      ...
      csoundMessage(
          csound, Str("CsFile is deprecated and may not work; use CsFileB\n"));
      r = createFilea(csound, buffer, cf);
      result = r && result;
    } else if (strstr(p, "<CsVersion>") == p) {
      r = checkVersion(csound, cf);
      result = r && result;
    } else if (strstr(p, "<CsLicence>") == p || strstr(p, "<CsLicense>") == p) {
      ...
      result = r && result;
    } else if (blank_buffer(/*csound,*/ buffer))
      continue;
