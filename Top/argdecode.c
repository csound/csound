...
void msg_callback(CSOUND *csound, int attr, const char *format, va_list args) {
  (void)csound;
...
void nomsg_callback(CSOUND *csound, int attr, const char *format,
                    va_list args) {
  return;
}
...
static const char *longUsageList[] = {
    ...
    Str_noop("                          1=note amps, 2=out-of-range msg, "
             "4=warnings,"),
    Str_noop("                          0/32/64/96=note amp format "
             "(raw,dB,colors),"),
    Str_noop("                          128=print benchmark information"),
    " ",
    ...
    Str_noop("                          key number to pfield N as cycles per "
             "second"),
    ...
    Str_noop("--0dbfs=N               override 0dbfs (max positive signal "
             "amplitude)"),
    Str_noop("--sinesize              length of internal sine table"),
    Str_noop(
        "--daemon                daemon mode: do not exit if CSD/orchestra is "
        "not given, is empty or does not compile"),
    Str_noop("--port=N                listen to UDP port N for "
             "instruments/orchestra "
             "code (implies --daemon)"),
    Str_noop(
        "--vbr-quality=Ft        set quality of variable bit-rate compression"),
...
static const SOUNDFILE_TYPE_ENTRY file_type_map[] = {
    {"wav", TYP_WAV},     {"aiff", TYP_AIFF}, {"au", TYP_AU},
    {"raw", TYP_RAW},     {"paf", TYP_PAF},   {"svx", TYP_SVX},
    {"nist", TYP_NIST},   {"voc", TYP_VOC},   {"ircam", TYP_IRCAM},
    {"w64", TYP_W64},     {"mat4", TYP_MAT4}, {"mat5", TYP_MAT5},
    {"pvf", TYP_PVF},     {"xi", TYP_XI},     {"htk", TYP_HTK},
    {"sds", TYP_SDS},     {"avr", TYP_AVR},   {"wavex", TYP_WAVEX},
    {"sd2", TYP_SD2},     {"mpeg", TYP_MPEG}, {"flac", TYP_FLAC},
...
static int decode_long(CSOUND *csound, char *s, int argc, char **argv) {
  ...
  /* -Z */
    ...
    if (csound->dl_opcodes_oplibs == NULL) {
      ...
      csound->dl_opcodes_oplibs =
          (char *)csound->Malloc(csound, (size_t)nbytes);
      strcpy(csound->dl_opcodes_oplibs, s);
    } else {
      /* append to existing list */
      nbytes += ((int)strlen(csound->dl_opcodes_oplibs) + 1);
      csound->dl_opcodes_oplibs = (char *)csound->ReAlloc(
          csound, csound->dl_opcodes_oplibs, (size_t)nbytes);
  ...
  } else if (!(strcmp(s, "new-parser")) || !(strcmp(s, "old-parser"))) {
      ...
      while ((c = *s++) != '\0') {
        ...
        case '@':
          FIND(Str("No indirection file"));
          {
            FILE *ind;
            void *fd;
            fd = csound->FileOpen2(csound, &ind, CSFILE_STD, s, "r", NULL,
                                   CSFTYPE_OPTIONS, 0);
            if (UNLIKELY(fd == NULL)) {
...
PUBLIC void csoundSetParams(CSOUND *csound, CSOUND_PARAMS *p) {
  ...
  /* MIDI interop */
  if (p->midi_key > 0)
    oparms->midiKey = p->midi_key;
  else if (p->midi_key_cps > 0)
    oparms->midiKeyCps = p->midi_key_cps;
  else if (p->midi_key_pch > 0)
    oparms->midiKeyPch = p->midi_key_pch;
  else if (p->midi_key_oct > 0)
    oparms->midiKeyOct = p->midi_key_oct;

  if (p->midi_velocity > 0)
    oparms->midiVelocity = p->midi_velocity;
  else if (p->midi_velocity_amp > 0)
    oparms->midiVelocityAmp = p->midi_velocity_amp;

  /* CSD line counts */
  if (p->csd_line_counts > 0)
    oparms->useCsdLineCounts = p->csd_line_counts;
  ...
  if (p->ksmps_override > 0)
    oparms->ksmps_override = p->ksmps_override;
}
