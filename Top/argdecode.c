...
void msg_callback(CSOUND *csound, int32_t attr, const char *format,
                  va_list args) {
  (void)csound;
...
void nomsg_callback(CSOUND *csound, int32_t attr, const char *format,
                    va_list args) {
  return;
}
...
static inline void set_stdin_assign(CSOUND *csound, int32_t type,
                                    int32_t state) {
  if (state)
...
static inline void set_stdout_assign(CSOUND *csound, int32_t type,
                                     int32_t state) {
  if (state)
      ...
      while ((c = *s++) != '\0') {
        ...
        case '@':
          FIND(Str("No indirection file"));
          {
            FILE *ind;
            void *fd;
            fd = csound->FileOpen(csound, &ind, CSFILE_STD, s, "r", NULL,
                                  CSFTYPE_OPTIONS, 0);
            if (UNLIKELY(fd == NULL)) {
