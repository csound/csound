...
static SPACE *
morespace(CSOUND *csound) { /* alloc large amount of memory, keep in a */
...
PUBLIC EVLIST *cscoreListCreate(CSOUND *csound, int nslots) {
  ...
  if (nxtfree != NULL && nxtfree->size >= minfreesiz)
    ...
    newblk = getfree(csound, minfreesiz);
...
PUBLIC EVENT *cscoreDefineEvent(CSOUND *csound, char *s) {
...
#endif
  {
    ...
    if (UNLIKELY(p > q && *s != '\0')) { /* too many ? */
      p++;
      csound->Message(csound, Str("PMAX exceeded, string event truncated.\n"));
      break;
    }
  }
...
PUBLIC void cscorePutEvent(CSOUND *csound, EVENT *e) {
  ...
  if (c == 's')
    warpout = 0; /* new section:  init to non-warped */
  putc(c, csound->oscfp);
  q = &e->p[1];
  if ((pcnt = e->pcnt)) {
    if (pcnt--)
    ...
    if (pcnt--) {
      if (warpout) {
        fprintf(csound->oscfp, " %g", e->p2orig);
      }
      fprintf(csound->oscfp, " %g", *q++);
    } else
      goto termin;
    if (pcnt--) {
      if (warpout) {
        fprintf(csound->oscfp, " %g", e->p3orig);
      }
      fprintf(csound->oscfp, " %g", *q++);
    } else
      goto termin;
...
termin:
  putc((int)'\n', csound->oscfp);
  if (c == 'w')
    warpout = 1; /* was warp statement: sect now warped */
}
...
PUBLIC EVLIST *
cscoreListExtractInstruments(CSOUND *csound, EVLIST *a,
                             char *s) /* list extract by instr numbers */
{
  int x[5], xcnt;
  int xn, *xp, insno, n;
  EVENT **p, **q, *e;
...
PUBLIC EVLIST *cscoreListExtractTime(CSOUND *csound, EVLIST *a, MYFLT from,
                                     MYFLT to) /* list extract by time */
{
  EVENT **p, **q, *e;
  EVLIST *b, *c;
  MYFLT maxp3;
...
static void savinfdata(/* store input file data */
                       CSOUND *csound, FILE *fp, EVENT *next, MYFLT until,
                       int wasend, int warp, int eof) {
  INFILE *infp;
  int n;
...
PUBLIC int csoundInitializeCscore(CSOUND *csound, FILE *insco, FILE *outsco) {
  ...
  savinfdata(csound, csound->scfp, next, FL(0.0), 1, 0,
             0);                     /* curuntil 0, wasend, non-warp, not eof */
...
PUBLIC void cscoreFileSetCurrent(CSOUND *csound, FILE *fp) {
  ...
  if (csound->scfp != NULL)
    savinfdata(csound, csound->scfp, nxtevt, curuntil, wasend, csound->warped,
               atEOF);
