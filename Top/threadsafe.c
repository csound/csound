...
void killInstance(CSOUND *csound, MYFLT instr, int insno, INSDS *ip, int mode,
                  int allow_release);
...
void allocate_message_queue(CSOUND *csound) {
  if (csound->msg_queue == NULL) {
    int i;
    csound->msg_queue = (message_queue_t **)csound->Calloc(
        csound, sizeof(message_queue_t *) * API_MAX_QUEUE);
    for (i = 0; i < API_MAX_QUEUE; i++) {
      csound->msg_queue[i] =
          (message_queue_t *)csound->Calloc(csound, sizeof(message_queue_t));
    }
  }
}
...
void *message_enqueue(CSOUND *csound, int32_t message, char *args, int argsiz) {
  if (csound->msg_queue != NULL) {
    ...
    message_queue_t *msg = csound->msg_queue[atomicGet_Incr_Mod(
        &csound->msg_queue_wget, API_MAX_QUEUE)];
    msg->message = message;
    if (msg->args != NULL)
  ...
  if (csound->msg_queue != NULL) {
    ...
    while (rp < rend) {
      ...
      case SCORE_EVENT: {
        ...
        long numFields;
        type = msg->args[0];
        memcpy(&pfields, msg->args + ARG_ALIGN, sizeof(MYFLT *));
        memcpy(&numFields, msg->args + ARG_ALIGN * 2, sizeof(long));
      ...
      case SCORE_EVENT_ABS: {
        ...
        memcpy(&pfields, msg->args + ARG_ALIGN, sizeof(MYFLT *));
        memcpy(&numFields, msg->args + ARG_ALIGN * 2, sizeof(long));
        memcpy(&ofs, msg->args + ARG_ALIGN * 3, sizeof(double));
      ...
      case TABLE_COPY_OUT: {
        ...
        memcpy(&ptable, msg->args + ARG_ALIGN, sizeof(MYFLT *));
      ...
      case TABLE_SET: {
        ...
        MYFLT value;
        memcpy(&table, msg->args, sizeof(int));
        memcpy(&index, msg->args + ARG_ALIGN, sizeof(int));
        memcpy(&value, msg->args + 2 * ARG_ALIGN, sizeof(MYFLT));
      ...
      case MERGE_STATE: {
        ...
        memcpy(&t, msg->args + ARG_ALIGN, sizeof(TYPE_TABLE *));
        memcpy(&ids, msg->args + 2 * ARG_ALIGN, sizeof(OPDS *));
      ...
      case KILL_INSTANCE: {
        ...
        memcpy(&insno, msg->args + ARG_ALIGN, sizeof(int));
        memcpy(&ip, msg->args + ARG_ALIGN * 2, sizeof(INSDS *));
        memcpy(&mode, msg->args + ARG_ALIGN * 3, sizeof(int));
        memcpy(&rls, msg->args + ARG_ALIGN * 4, sizeof(int));
...
static inline void csoundInputMessage_enqueue(CSOUND *csound, const char *str) {
  message_enqueue(csound, INPUT_MESSAGE, (char *)str, strlen(str) + 1);
}

static inline int64_t *csoundReadScore_enqueue(CSOUND *csound,
                                               const char *str) {
...
static inline int64_t *csoundScoreEventAbsolute_enqueue(CSOUND *csound,
                                                        char type,
                                                        const MYFLT *pfields,
                                                        long numFields,
                                                        double time_ofs) {
...
*/
void killInstance_enqueue(CSOUND *csound, MYFLT instr, int insno, INSDS *ip,
  ...
  if (csoundGetChannelPtr(csound, &pstring, name,
    ...
    if (strlen(string) + 1 > (unsigned int)size) {
      if (stringdat->data != NULL)
        csound->Free(csound, stringdat->data);
...
PUBLIC int csoundSetPvsChannel(CSOUND *csound, const PVSDATEXT *fin,
  ...
  if (LIKELY(csoundGetChannelPtr(csound, &pp, name,
    ...
    if (f->frame == NULL) {
      ...
      f->frame =
          csound->ReAlloc(csound, f->frame, sizeof(float) * (fin->N + 2));
    }
