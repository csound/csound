...
int csoundAddUtility(CSOUND *csound, const char *name,
  ...
  /* csound->Message(csound, "csoundAddUtility: name: %s  function: 0x%p\n",
                     name, UtilFunc); */
  if (UNLIKELY(csound == NULL || name == NULL || name[0] == '\0' ||
               UtilFunc == NULL))
    return -1;
...
PUBLIC int csoundRunUtility(CSOUND *csound, const char *name, int argc,
                            char **argv) {
  csUtility_t *p;
  char **lst;
 ...
 */
PUBLIC int csoundScoreExtract(CSOUND *csound, FILE *inFile, FILE *outFile,
                              FILE *extractFile) {
  int err;
  CORFIL *inf = corfile_create_w(csound);
  int c;
