...
static uintptr_t recordThread_(void *recordData_) {
  ...
  while (recordData->running) {
    ...
    do {
      sampsread =
          csoundReadCircularBuffer(NULL, recordData->cbuf, buf, bufsize);
#ifdef USE_DOUBLE
      sflib_write_double((SNDFILE *)recordData->sfile, buf, sampsread);
#else
      sflib_write_float((SNDFILE *)recordData->sfile, buf, sampsread);
#endif
    } while (sampsread != 0);
...
public:
  CsPerfThreadMsg_Record(CsoundPerformanceThread *pt, std::string filename,
                         int samplebits = 16, int numbufs = 4)
      : CsoundPerformanceThreadMessage(pt) {
    ...
    int bufsize =
        csoundGetOutputBufferSize(csound) * csoundGetNchnls(csound) * numbufs;
    recordData->cbuf =
        csoundCreateCircularBuffer(csound, bufsize, sizeof(MYFLT));
    ...
    recordData->sfile =
        (void *)sflib_open(filename.c_str(), SFM_WRITE, &sflib_info);
    if (!recordData->sfile) {
      csoundMessage(csound, "Could not open file for recording.");
      csoundDestroyCircularBuffer(csound, recordData->cbuf);
      return;
    }
    sflib_command((SNDFILE *)recordData->sfile, SFC_SET_CLIPPING, NULL,
                  SFLIB_TRUE);
...
public:
  CsPerfThreadMsg_ScoreEvent(CsoundPerformanceThread *pt, int absp2mode,
                             char opcod, int pcnt, const MYFLT *p)
      : CsoundPerformanceThreadMessage(pt) {
    this->opcod = opcod;
  ...
  int run() {
    ...
    if (absp2mode && pcnt > 1) {
      ...
      if (p2 < 0.0) {
        if (pcnt > 2 && pp[2] >= (MYFLT)0 && (opcod == 'a' || opcod == 'i')) {
...
int CsoundPerformanceThread::Perform() {
  ...
  do {
    ...
    if (recordData.running) {
      ...
      int written =
          csoundWriteCircularBuffer(NULL, recordData.cbuf, spout, len);
      if (written != len) {
        csoundMessage(csound, "perfThread record buffer overrun.\n");
      }
    }
    csoundCondSignal(
        recordData.condvar); // Needs to be outside the if
...
void CsoundPerformanceThread::Record(std::string filename, int samplebits,
                                     int numbufs) {
  QueueMessage(new CsPerfThreadMsg_Record(this, filename, samplebits, numbufs));
}
...
void CsoundPerformanceThread::ScoreEvent(int absp2mode, char opcod, int pcnt,
                                         const MYFLT *p) {
  QueueMessage(new CsPerfThreadMsg_ScoreEvent(this, absp2mode, opcod, pcnt, p));
}
...
PUBLIK void CsoundPTsetProcessCB(Cpt pt, void (*callback)(void *),
                                 void *cbData) {
  CsoundPerformanceThread *cpt = (CsoundPerformanceThread *)pt;
...
PUBLIK void CsoundPTrecord(Cpt pt, const char *filename, int samplebits,
                           int numbufs) {
  CsoundPerformanceThread *cpt = (CsoundPerformanceThread *)pt;
...
PUBLIK void CsoundPTscoreEvent(Cpt pt, int absp2mode, char opcod, int pcnt,
                               MYFLT *p) {
  CsoundPerformanceThread *cpt = (CsoundPerformanceThread *)pt;
