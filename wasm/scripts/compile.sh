...
nix-build -E "(with import <nixpkgs> {}; pkgs.callPackage ./src/plugin_example.nix {})" -o result_plugin_c --show-trace &&
nix-build -E "(with import <nixpkgs> {}; pkgs.callPackage ./src/plugin_example_cpp.nix {})" -o result_plugin_cpp --show-trace &&
    if [ -d "./lib" ]; then
