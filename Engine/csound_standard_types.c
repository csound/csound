...
void fsig_copy_value(CSOUND *csound, CS_TYPE *cstype, void *dest, void *src) {
  ...
  if (fsigout->frame.auxp == NULL ||
    ...
    ((CSOUND *)csound)
        ->AuxAlloc(csound, (N + 2) * sizeof(float), &fsigout->frame);
...
void array_copy_value(CSOUND *csound, CS_TYPE *cstype, void *dest, void *src) {
  ...
  if (aDest->data == NULL || aSrc->arrayMemberSize != aDest->arrayMemberSize ||
  ...
  for (j = 0; j < arrayNumMembers; j++) {
    ...
    aDest->arrayType->copyValue(csound, aDest->arrayType, aDest->data + index,
                                aSrc->data + index);
  }
}
...
void array_free_var_mem(void *csnd, void *p) {
  ...
  if (dat->data != NULL) {
    ...
    if (arrayType->freeVariableMemory != NULL) {
      ...
      for (i = 0; i < size; i++) {
        arrayType->freeVariableMemory(csound, mem + (i * memMyfltSize));
      }
    }
...
const char *POLY_IN_TYPES[] = {"x", "kacpri", /* ***Deprecated*** */
                               "T", "Sicpr",  "U", "Sikcpr", "i", "cpri",
                               "k", "cprki",  "B", "Bb",     NULL};
const char *OPTIONAL_IN_TYPES[] = {"o", "icpr",  "p", "icpr",  "q", "icpr",
                                   "v", "icpr",  "j", "icpr",  "h", "icpr",
                                   "O", "kicpr", "J", "kicpr", "V", "kicpr",
                                   "P", "kicpr", NULL};
const char *VAR_ARG_IN_TYPES[] = {
    "m",       "icrp", "M",      "icrpka", "N",
    "icrpkaS", "n",    "icrp", /* this one requires odd number of args... */
    "W",       "S",    "y",      "a",      "z",
    "kicrp",   "Z",    "kaicrp", NULL /* this one needs to be ka
                                         alternatating... */
};

const char *POLY_OUT_TYPES[] = {"s", "ka", /* ***Deprecated*** */
                                "i", "pi", NULL};

const char *VAR_ARG_OUT_TYPES[] = {
    "m", "a",    "z", "k",     "I", "Sip", /* had comment of (not implemented
                                              yet) in entry1.c */
    "X", "akip", "N", "akipS", "F", "f",   NULL};
