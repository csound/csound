...
OENTRY *ugen_resolve_opcode(OENTRIES *entries, char *outargTypes,
                            char *inargTypes) {
  int i;
...
PUBLIC UGEN *ugen_new(UGEN_FACTORY *factory, char *opName, char *outargTypes,
                      char *inargTypes) {
  UGEN *ugen;
  OPDS *opds;
  OPTXT *optxt;
  ...
  // CS_VAR_TYPE's
  ugen->data = (MYFLT *)csound->Calloc(csound, ugen->outPool->poolSize +
                                                   ugen->inPool->poolSize);
