...
static int parse_opcode_args(CSOUND *csound, OENTRY *opc) {
  ...
  if (*in_args[0] != '0') {
    while (in_args[i] != NULL) {
      ...
      if (*in_arg == '[') {
        ...
        CS_TYPE *type = csoundGetTypeWithVarTypeName(csound->typePool, c);
  ...
  opc->dsblksiz =
      ((opc->dsblksiz + (uint16)15) & (~((uint16)15))); /* align (needed ?) */

  // this feature is removed from 7.0
  // opc->intypes = cs_strdup(csound, intypes);
  opc->intypes =
      cs_strdup(csound, (inm->intypes[0] == '0') ? "" : inm->intypes);
  opc->outypes =
      cs_strdup(csound, (inm->outtypes[0] == '0') ? "" : inm->outtypes);
  ...
  //        synterr(csound, Str("too many input args for opcode %s"),
  //        inm->name); csound->LongJmp(csound, 1);
  //      }
  //    }
  //      default:
  //        synterr(csound, Str("invalid output type for opcode %s"),
...
OENTRY *csound_find_internal_oentry(CSOUND *csound, OENTRY *oentry) {
  ...
  while (items != NULL) {
    ep = items->value;
    if (oentry->iopadr == ep->iopadr && oentry->kopadr == ep->kopadr &&
 ...
 */
int add_udo_definition(CSOUND *csound, char *opname, char *outtypes,
  ...
  if (UNLIKELY(opc != NULL)) {
    ...
    if (UNLIKELY(!strcmp(opname, "instr") || !strcmp(opname, "endin") ||
                 !strcmp(opname, "opcode") || !strcmp(opname, "endop") ||
                 !strcmp(opname, "$label") || !strcmp(opname, "pset") ||
                 !strcmp(opname, "xin") || !strcmp(opname, "xout") ||
                 !strcmp(opname, "subinstr"))) {
      synterr(csound, Str("cannot redefine %s"), opname);
      return -2;
    }
    csound->Message(csound, Str("WARNING: redefined opcode: %s\n"), opname);
  }
  /* IV - Oct 31 2002 */
  ...
  if (opc != NULL) {
    ...
    newopc->outypes =
        csound->Malloc(csound, strlen(outtypes) + 1 + strlen(intypes) + 2);
