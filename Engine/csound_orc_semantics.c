...
extern int add_udo_definition(CSOUND *csound, char *opname, char *outtypes,
                              char *intypes, int flags);
...
static int
isirate(/*CSOUND *csound,*/ TREE
            *t) { /* check that argument is an i-rate constant or variable */
  ...
  if (t->type == INTEGER_TOKEN) {
    ...
    // printf("array case\n");
    if (isirate(/*csound, */ t->right) == 0)
      return 0;
    t = t->next;
    while (t) {
      // printf("t=%p t->type=%d\n", t, t->type);
      if (isirate(/*csound,*/ t) == 0)
        return 0;
      t = t->next;
    }
    return 1;
...
char *get_arg_type2(CSOUND *csound, TREE *tree, TYPE_TABLE *typeTable) {
  ...
  if (is_expression_node(tree)) {
    ...
    if (tree->type == '?') {
      ...
      out = resolve_opcode_get_outarg(csound, opentries, condInTypes);
    ...
    if (tree->type == T_FUNCTION && !strcmp(tree->value->lexeme, "i")) {
      ...
      if (tree->right->type == T_ARRAY && tree->right->left->type == T_IDENT &&
          isirate(/*csound,*/ tree->right->right)) {
        synterr(csound, Str("Use of i() with array element ill formed\n"));
      } else if (UNLIKELY(is_expression_node(tree->right)))
        synterr(csound,
    ...
    if (tree->type == T_FUNCTION) {
      char *argTypeRight =
          get_arg_string_from_tree(csound, tree->right, typeTable);
      ...
      if (tree->value->optype != NULL) /* if there is type annotation */
        ...
        out = resolve_opcode_get_outarg(csound, entries, argTypeRight);

      if (UNLIKELY(out == 0)) {
        synterr(csound,
                Str("error: opcode '%s' for expression with arg "
  ...
  if (is_boolean_expression_node(tree)) {
    if (tree->type == S_UNOT) {
      ...
      if (UNLIKELY(out == NULL)) {
        synterr(csound,
                Str("error: boolean expression '%s' with arg "
                    "types %s not found, line %d\n"),
                opname, inArgTypes, tree->line);
        do_baktrace(csound, tree->locn);
        csound->Free(csound, inArgTypes);
 ...
 */
OENTRY *resolve_opcode(CSOUND *csound, OENTRIES *entries, char *outArgTypes,
                       char *inArgTypes) {
  ...
  while (current != NULL) {
    ...
    if (is_expression_node(tree) || is_boolean_expression_node(tree)) {
        ...
        if (UNLIKELY(var == NULL)) {
          ...
          if (*varName == 'g')
            var = csoundFindVariableWithName(
                csound, csound->engineState.varPool, varName);
          if (UNLIKELY(var == NULL)) {
            synterr(csound, Str("Variable '%s' used before defined\nline %d"),
                    varName, tree->line);
            do_baktrace(csound, tree->locn);
      ...
      case T_ARRAY:
        ...
        if (UNLIKELY(csoundFindVariableWithName(csound, pool, varName) ==
                     NULL)) {
          CS_VARIABLE *var = 0;
          /* VL: 13-06-13
             if it is not found, we still check the global (merged) pool */
...
void add_arg(CSOUND *csound, char *varName, char *annotation,
  ...
  if (var == NULL) {
    ...
    var =
        csoundCreateVariable(csound, csound->typePool, type, varName, typeArg);
...
int add_args(CSOUND *csound, TREE *tree, TYPE_TABLE *typeTable) {
  ...
  while (current != NULL) {
    ...
    case T_TYPED_IDENT:
      ...
      /* TODO - This needs to check if someone put in sr:k or ksmps:i or
       * something like that */
      ...
      if (*varName == 't' &&
          current->value->optype == NULL) { /* Support legacy t-vars */
 ...
 * in-args.  This function does not try to expand any statements or do any
 * semantic verification, but reshapes trees so that they can all go through the
 ...
 * because variables can (now) have names that shadow opcode names.  Lookup
 * needs to give priority to an identifier being a variable over being an
 * opcode. This maintains future proofing so that if an opcode is later
 * introduced with the same name as a variable in an older project, the older
...
TREE *convert_statement_to_opcall(CSOUND *csound, TREE *root,
  ...
  if (root->type == T_ASSIGNMENT) {
    /* Rewrite tree if line is "a1, a2 = func(arg, arg1)" to "a1, a2 func arg,
     * arg1" */
  ...
  if (root->type != T_OPCALL) {
    synterr(csound, Str("Internal Error: convert_statement_to_opcall received "
                        "a non T_OPCALL TREE\n"));
    return NULL;
  }
  ...
  if (root->value != NULL) {
    /* xout exp(0) */
    if (root->left != NULL && root->left->type == T_IDENT &&
  ...
  if (root->left->type == T_OPCALL && root->right == NULL) {
    ...
    /* this branch catches this part of opcall rule: out_arg_list '(' ')'
     * NEWLINE */
...
int verify_opcode(CSOUND *csound, TREE *root, TYPE_TABLE *typeTable) {
  ...
  if (root->value->optype == NULL)
    oentry = resolve_opcode(csound, entries, leftArgString, rightArgString);
  /* if there is type annotation, try to resolve it */
  else
    oentry =
        resolve_opcode(csound, entries, root->value->optype, rightArgString);
  ...
  if (UNLIKELY(oentry == NULL)) {
    ...
    csoundMessage(csound, Str("Found:\n  %s %s %s\n"), leftArgString,
                  root->value->lexeme, rightArgString);
    ...
    if (csound->oparms->sampleAccurate &&
        (strcmp(oentry->opname, "=.a") == 0) && (left != NULL) &&
        (left->value != NULL) &&
        (left->value->lexeme[0] ==
         'a')) { /* Deal with sample accurate assigns */
      int i = 0;
    ...
    if (current->value != NULL) {
      if (strcmp("xin", current->value->lexeme) == 0) {
        if (UNLIKELY(xinArgs != NULL)) {
          synterr(csound, Str("Multiple xin statements found. "
                              "Only one is allowed."));
          return 0;
        }
      ...
      if (strcmp("xout", current->value->lexeme) == 0) {
        if (UNLIKELY(xoutArgs != NULL)) {
          synterr(csound, Str("Multiple xout statements found. "
                              "Only one is allowed."));
          return 0;
        }
  ...
  while (current != NULL) {
    ...
    case UDO_TOKEN:
      ...
      if (top->left != NULL && top->left->type == UDO_ANS_TOKEN) {
        ...
        add_udo_definition(csound, top->value->lexeme, top->left->value->lexeme,
                           top->right->value->lexeme, 0x0000);
        ...
        add_udo_definition(csound, current->left->value->lexeme, outArgString,
                           inArgString, 0x0000);
      }
      csound->inZero = 0;
      if (UNLIKELY(PARSER_DEBUG))
    ...
    case T_DECLARE: {
      ...
      char *inArgStringDecl =
          get_in_types_from_tree(csound, current->left->right, typeTable);
      add_udo_definition(csound, current->value->lexeme, inArgStringDecl,
    ...
    case FOR_TOKEN: {
      ...
      if (*arrayArgType != '[') {
        synterr(csound,
                Str("Line: %d invalid array argument in for-of statement: "
                    "found '%s'\n"),
...
TREE *make_node(CSOUND *csound, int line, int locn, int type, TREE *left,
                TREE *right) {
  TREE *ans;
  ans = (TREE *)csound->Malloc(csound, sizeof(TREE));
  if (UNLIKELY(ans == NULL)) {
...
void handle_optional_args(CSOUND *csound, TREE *l) {
    ...
    return;
  {
    ...
    if (UNLIKELY(ep == NULL)) { /* **** FIXME **** */
      csoundErrorMsg(
          csound,
          "THIS SHOULD NOT HAPPEN -- ep NULL csound_orc-semantics(%d)\n",
          __LINE__);
    }
    if (ep->intypes != NULL) {
