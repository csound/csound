...
char *cs_hash_table_put_no_key_copy(CSOUND *csound, CS_HASH_TABLE *hashTable,
                                    char *key, void *value);
...
PUBLIC void *cs_hash_table_get(CSOUND *csound, CS_HASH_TABLE *hashTable,
                               char *key) {
  IGN(csound);
  unsigned int index;
...
PUBLIC char *cs_hash_table_get_key(CSOUND *csound, CS_HASH_TABLE *hashTable,
                                   char *key) {
  unsigned int index;
  CS_HASH_TABLE_ITEM *item;
  IGN(csound);
...
char *cs_hash_table_put_no_key_copy(CSOUND *csound, CS_HASH_TABLE *hashTable,
  ...
  CS_HASH_TABLE_ITEM *newItem =
      csound->Malloc(csound, sizeof(CS_HASH_TABLE_ITEM));
  newItem->key = key;
  newItem->value = value;
  newItem->next = NULL;
...
PUBLIC void cs_hash_table_put(CSOUND *csound, CS_HASH_TABLE *hashTable,
                              char *key, void *value) {
  cs_hash_table_put_no_key_copy(csound, hashTable, cs_strdup(csound, key),
                                value);
}

PUBLIC char *cs_hash_table_put_key(CSOUND *csound, CS_HASH_TABLE *hashTable,
                                   char *key) {
  return cs_hash_table_put_no_key_copy(csound, hashTable,
                                       cs_strdup(csound, key), NULL);
}

PUBLIC void cs_hash_table_remove(CSOUND *csound, CS_HASH_TABLE *hashTable,
                                 char *key) {
...
PUBLIC CONS_CELL *cs_hash_table_values(CSOUND *csound,
...
PUBLIC void cs_hash_table_merge(CSOUND *csound, CS_HASH_TABLE *target,
                                CS_HASH_TABLE *source) {
...
PUBLIC void cs_hash_table_mfree_complete(CSOUND *csound,
                                         CS_HASH_TABLE *hashTable) {
...
PUBLIC void cs_hash_table_free_complete(CSOUND *csound,
                                        CS_HASH_TABLE *hashTable) {
