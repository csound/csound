...
void named_instr_assign_numbers (void)
{
    ...
    while (--insno_priority > -3) {
      ...
      for (inm = *inm_first; inm; inm = inm->prv) {
        ...
        if (O.msglevel)
          printf("instr %s uses instrument number %d\n", inm2->name, num);
      }
    }
...
long strarg2opcno (MYFLT *p, char *s, int force_opcode)
{
    ...
    if (insno < 1) {
      initerror("cannot find the specified instrument or opcode");
      insno = -1;
    }
    return insno;
}
...
void opcode_list_create (void)
{
    ...
    if (opcode_list) {
      die("internal error: opcode list has already been created");
      return;
    }
