...
int check_instr_name(char *s) {
  ...
  if (UNLIKELY(!*c))
    return 0; /* empty */
  if (UNLIKELY(!isalpha(*c) && *c != '_'))
    return 0; /* chk if 1st char is valid */
  while (*++c)
...
MYFLT named_instr_find_in_engine(CSOUND *csound, char *s,
  ...
  if ((tt = strchr(s, '.')) == NULL) {
    ...
    return (inm == NULL) ? 0L
                         : (ss ? -(inm->instno + frac) : inm->instno + frac);
  }
}
...
int32 strarg2opcno(CSOUND *csound, void *p, int is_string, int force_opcode) {
  ...
  if (!insno && is_string) {             /* if no instrument was found, */
    ...
    while (inm && sCmp(inm->name, (char *)p))
      inm = inm->prv;
    if (inm)
      insno = (int32)inm->instno;
  }
  if (UNLIKELY(insno < 1)) {
...
void csoundDeleteAllGlobalVariables(CSOUND *csound) {
  if (csound == NULL || csound->namedGlobals == NULL)
    return;
