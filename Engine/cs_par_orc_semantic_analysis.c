...
static INSTR_SEMANTICS *instr_semantics_alloc(CSOUND *csound, char *name) {
  INSTR_SEMANTICS *instr = csound->Malloc(csound, sizeof(INSTR_SEMANTICS));
...
void csp_orc_sa_global_read_write_add_list(CSOUND *csound, struct set_t *write,
                                           struct set_t *read) {
  if (UNLIKELY(csound->instCurr == NULL)) {
    ...
    csound->Die(csound, Str("Invalid NULL parameter set to add to global read, "
                            "write lists\n"));
  } else {
...
void csp_orc_sa_global_read_write_add_list1(CSOUND *csound, struct set_t *write,
                                            struct set_t *read) {
  if (UNLIKELY(csound->instCurr == NULL)) {
    csound->DebugMsg(csound,
                     "Add global read, write lists without any instruments\n");
    ...
    if (write->count == 1 && read->count == 1 && new->count == 1) {
      ...
      struct set_t *new_read_write =
          csp_set_union(csound, csound->instCurr->read_write, new);
      // printf("Line: %d of cs_par_orc_semantics(%p)\n",
...
void csp_orc_sa_global_write_add_list(CSOUND *csound, struct set_t *set) {
  if (UNLIKELY(csound->instCurr == NULL)) {
    ...
    csound->Die(csound, Str("Invalid NULL parameter set to add to a "
                            "global write_list\n"));
  } else {
...
static void csp_orc_sa_interlocksf(CSOUND *csound, int code, char *name) {
  if (code & 0xfff8) {
    ...
    if (code & WR)
      csp_set_add(csound, ww, "##wri");
    if (code & IR)
      csp_set_add(csound, rr, "##int");
    if (code & IW)
      csp_set_add(csound, ww, "##int");
...
void csp_orc_sa_instr_add(CSOUND *csound, char *name) {
  name =
      cs_strdup(csound, name); // JPff:  leaks: necessary?? Think it is correct
...
INSTR_SEMANTICS *csp_orc_sa_instr_get_by_name(CSOUND *csound,
                                              char *instr_name) {
...
void csp_orc_analyze_tree(CSOUND *csound, TREE *root) {
    ...
    default:
      ...
      if (current->left != NULL) {
        csp_orc_sa_global_read_write_add_list(
            csound, csp_orc_sa_globals_find(csound, current->left),
            csp_orc_sa_globals_find(csound, current->right));

      } else {
        csp_orc_sa_global_read_add_list(
            csound, csp_orc_sa_globals_find(csound, current->right));
      }
  ...
  if (UNLIKELY(PARSER_DEBUG))
    csound->Message(csound, "[End csp analysis]\n");
}
