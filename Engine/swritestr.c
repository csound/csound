...
void swritestr(CSOUND *csound, CORFIL *sco, int first) {
  ...
  lincnt = 0;
  if ((c = bp->text[0]) != 'w' && c != 's' &&
      c != 'e') { /*   if no warp stmnt but real data,  */
    /* create warp-format indicator */
    if (first)
      corfile_puts(csound, "w 0 60\n", sco);
    lincnt++;
  }
nxtlin:
  ...
  case 'a':
    ...
    if (first)
      fltout(csound, bp->newp2, sco); /*   newp2,     */
    while ((c = *p++) != SP && c != LF)
      ;
    ...
    if (isntAfunc) {
      ...
      corfile_putc(csound, SP, sco);
      if (first)
        fltout(csound, bp->newp3, sco); /*   newp3,     */
      while ((c = *p++) != SP && c != LF)
        ;
...
static SRTBLK *nxtins(SRTBLK *bp) /* find nxt note with same p1 */
{
  ...
  p1 = bp->p1val;
  while ((bp = bp->nxtblk) != NULL && (bp->p1val != p1 || bp->text[0] != 'i'))
    ;
  return (bp);
}
...
static SRTBLK *prvins(SRTBLK *bp) /* find prv note with same p1 */
{
  ...
  p1 = bp->p1val;
  while ((bp = bp->prvblk) != NULL && (bp->p1val != p1 || bp->text[0] != 'i'))
    ;
  return (bp);
}
...
static char *nextp(CSOUND *csound, SRTBLK *bp, char *p, int lincnt, int pcnt,
  ...
  error:
    csound->Message(csound,
                    Str("swrite: output, sect%d line%d p%d makes"
...
error2:
  csound->Message(csound,
                  Str("swrite: output, sect%d line%d p%d ramp "
...
error1:
  csound->Message(csound,
                  Str("swrite: output, sect%d line%d p%d has illegal"
                      " expramp symbol\n"),
                  csound->sectcnt, lincnt, pcnt);
  goto put0;
error2:
  csound->Message(csound,
                  Str("swrite: output, sect%d line%d p%d expramp "
                      "has illegal forward or backward ref\n"),
                  csound->sectcnt, lincnt, pcnt);
put0:
  corfile_putc(csound, '0', sco);
...
static char *randramp(CSOUND *csound, SRTBLK *bp, char *p, int lincnt, int pcnt,
                      CORFIL *sco)
/* NB np's may reference a ramp but ramps must terminate in valid nums */
{
  char *q;
...
error2:
  csound->Message(csound,
                  Str("swrite: output, sect%d line%d p%d expramp has"
...
static char *
pfStr(CSOUND *csound, char *p, int lincnt, int pcnt,
      CORFIL *sco) { /* moves quoted ascii string to SCOREOUT file */
  ...
  if (UNLIKELY(*p != SP && *p != LF)) {
    csound->Message(csound,
                    Str("swrite: output, sect%d line%d p%d "
...
static char *fpnum(CSOUND *csound, char *p, int lincnt, int pcnt,
                   CORFIL *sco) /* moves ascii string */
...
/* CONSIDER USING SIMPLER CODE */
{
  ...
  if (UNLIKELY((*p != SP && *p != LF) || !dcnt)) {
    csound->Message(csound,
                    Str("swrite: output, sect%d line%d p%d has "
                        "illegal number  "),
                    csound->sectcnt, lincnt, pcnt);
    while (q < p)
      csound->Message(csound, "%c", *q++);
