  ...
  /* NB np's may reference a ramp but ramps must terminate in valid nums */
{
 ...
 forwrd:
    ...
    pval = stof(csound, p);     /* the error msgs generated by stof     */
    qval = stof(csound, q);                         /*   are misleading */
    p2span = (MYDBL)(nxtbp->newp2 - prvbp->newp2);
/*  printf("pval=%f qval=%f span = %f\n", pval, qval, p2span); */
    rval = pval * (MYFLT)pow((MYDBL)(qval/pval),
                             (MYDBL)(bp->newp2 - prvbp->newp2) / p2span);
  ...
  /* NB np's may reference a ramp but ramps must terminate in valid nums */
{
 ...
 forwrd:
    ...
    pval = stof(csound, p);     /* the error msgs generated by stof     */
    qval = stof(csound, q);                         /*   are misleading */
    rval = (MYFLT) (((MYDBL) (csound->Rand31(&(csound->randSeed1)) - 1)
                     / 2147483645.0) * ((MYDBL) qval - (MYDBL) pval)
                    + (MYDBL) pval);
    fltout(csound, rval, sco);
    return(psav);
