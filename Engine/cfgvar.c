...
static int cfg_alloc_structure(CSOUND *csound, csCfgVariable_t **ptr,
                               const char *name, void *p, int type, int flags,
                               void *min, void *max, const char *shortDesc,
                               const char *longDesc) {
...
PUBLIC int csoundCreateConfigurationVariable(CSOUND *csound, const char *name,
...
static int set_cfgvariable_value(csCfgVariable_t *pp, void *value) {
  ...
  case CSOUNDCFG_INTEGER:
    iVal = *((int *)value);
    if (UNLIKELY(iVal < pp->i.min))
      return CSOUNDCFG_TOO_LOW;
    if (UNLIKELY(iVal > pp->i.max))
      return CSOUNDCFG_TOO_HIGH;
  ...
  case CSOUNDCFG_FLOAT:
    fVal = *((float *)value);
    if (UNLIKELY(fVal < pp->f.min))
      return CSOUNDCFG_TOO_LOW;
    if (UNLIKELY(fVal > pp->f.max))
      return CSOUNDCFG_TOO_HIGH;
    *(pp->f.p) = fVal;
    break;
  case CSOUNDCFG_DOUBLE:
    dVal = *((double *)value);
    if (UNLIKELY(dVal < pp->d.min))
      return CSOUNDCFG_TOO_LOW;
    if (UNLIKELY(dVal > pp->d.max))
      return CSOUNDCFG_TOO_HIGH;
    *(pp->d.p) = dVal;
    break;
  case CSOUNDCFG_MYFLT:
    mVal = *((MYFLT *)value);
    if (UNLIKELY(mVal < pp->m.min))
      return CSOUNDCFG_TOO_LOW;
    if (UNLIKELY(mVal > pp->m.max))
      return CSOUNDCFG_TOO_HIGH;
    *(pp->m.p) = mVal;
    break;
  case CSOUNDCFG_STRING:
...
static int parse_cfg_variable(csCfgVariable_t *pp, const char *value) {
  ...
  case CSOUNDCFG_BOOLEAN:
    if (strcmp(value, "0") == 0 || strcmp(value, "no") == 0 ||
      ...
      *(pp->b.p) = 0;
    else if (strcmp(value, "1") == 0 || strcmp(value, "yes") == 0 ||
...
PUBLIC csCfgVariable_t *csoundQueryConfigurationVariable(CSOUND *csound,
  ...
  return (csCfgVariable_t *)cs_hash_table_get(csound, csound->cfgVariableDB,
                                              (char *)name);
}
...
static int remove_entry_from_db(CSOUND *csound, CS_HASH_TABLE *db,
                                const char *name) {
