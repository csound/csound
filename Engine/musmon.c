...
MYFLT csoundInitialiseIO(CSOUND *csound) {
  ...
  if (csound->enableHostImplementedAudioIO && csound->hostRequestedBufferSize) {
    ...
    if (check_rtaudio_name(O->infilename, NULL, 0) >= 0 ||
      ...
      O->oMaxLag =
          ((O->oMaxLag + O->outbufsamps - 1) / O->outbufsamps) * O->outbufsamps;
...
void print_benchmark_info(CSOUND *csound, const char *s) {
  ...
  csound->ErrorMsg(csound, Str("Elapsed time at %s: real: %.3fs, CPU: %.3fs\n"),
                   (char *)s, rt, ct);
}
...
int musmon(CSOUND *csound) {
  ...
  csound->spout_tmp =
      (MYFLT *)csound->Calloc(csound, (csound->oparms->numThreads + 1) *
                                          csound->nspout * sizeof(MYFLT));
  csound->spout =
      (MYFLT *)csound->Calloc(csound, csound->nspout * sizeof(MYFLT));
  csound->auxspin =
      (MYFLT *)csound->Calloc(csound, csound->nspin * sizeof(MYFLT));
  ...
  csound->evt_poll_maxcnt =
      (int)(250.0 /
            (double)csound->ekr); /* VL this was wrong: kr/250 originally */
  ...
  if (O->playscore != NULL)
    corfile_flush(csound, O->playscore);
  // csound->scfp
  if (UNLIKELY(O->usingcscore)) {
    ...
    if (UNLIKELY(!(csound->scfp = fopen("cscore.out", "r")))) {
      ...
      int c;
      while ((c = getc(csound->scfp)) != EOF)
        corfile_putc(csound, c, inf);
      corfile_rewind(inf);
      csound->scorestr = inf;
  ...
  if (csound->oparms->realtime && csound->event_insert_loop == 0) {
    ...
    csound->alloc_queue = (ALLOC_DATA *)csound->Calloc(
        csound, sizeof(ALLOC_DATA) * MAX_ALLOC_QUEUE);
    csound->event_insert_thread =
        csound->CreateThread(event_insert_thread, (void *)csound);
...
PUBLIC int csoundCleanup(CSOUND *csound) {
  ...
  if (csound->engineState.instrtxtp && csound->engineState.instrtxtp[0] &&
...
static void print_amp_values(CSOUND *csound, int score_evt) {
  ...
  if (UNLIKELY(STA(segamps) || (p->rngflg && STA(sormsg)))) {
    if (score_evt > 0)
      p->Message(p, "B%7.3f ..%7.3f T%7.3f TT%7.3f M:", p->prvbt - p->beatOffs,
                 p->curbt - p->beatOffs, p->curp2 - p->timeOffs, p->curp2);
    else
      p->Message(p, "  rtevent:\t   T%7.3f TT%7.3f M:", p->curp2 - p->timeOffs,
                 p->curp2);
  ...
  for (n = p->nchnls, maxp = p->maxamp - 1, smaxp = p->smaxamp - 1,
      maxps = p->maxpos - 1, smaxps = p->smaxpos - 1, rngp = p->rngcnt,
      srngp = STA(srngcnt);
       n--;) {
    ++maxps;
    ++smaxps;
...
static void section_amps(CSOUND *csound, int enable_msgs) {
  ...
  STA(srngflg) = 0;
  for (n = p->nchnls, smaxp = p->smaxamp - 1, maxp = p->omaxamp - 1,
      smaxps = p->smaxpos - 1, maxps = p->omaxpos - 1, srngp = STA(srngcnt),
      rngp = STA(orngcnt);
       n--;) {
    ++maxps;
    ++smaxps;
...
static CS_NOINLINE void printScoreError(CSOUND *p, int rtEvt, const char *fmt,
                                        ...) {
  va_list args;
  ...
  case 'q':
    if (csound->ISSTRCOD(evt->p[1]) && evt->strarg) { /* IV - Oct 31 2002 */
      ...
      if (UNLIKELY((unsigned int)(insno - 1) >=
        ...
        printScoreError(csound, rtEvt,
                        Str(" - note deleted. instr %d(%d) undefined"), insno,
    ...
    if (csound->ISSTRCOD(evt->p[1]) && evt->strarg) { /* IV - Oct 31 2002 */
      ...
      if ((rfd = getRemoteInsRfd(csound, insno))) {
        ...
        if (rfd == GLOBAL_REMOT)
          ...
          insSendevt(csound, evt,
                     rfd); /* RM: or send to single remote Csound */
          break;           /* RM: and quit */
        }
      }
      evt->p[1] = (MYFLT)insno;
...
static void process_midi_event(CSOUND *csound, MEVENT *mep, MCHNBLK *chn) {
  ...
  if (mep->type == NOTEON_TYPE && mep->dat2) { /* midi note ON: */
    if (UNLIKELY((n = MIDIinsert(csound, insno, chn, mep)))) {
      /* alloc,init,activ */
      csound->ErrorMsg(csound, Str("\t\t   T%7.3f - note deleted. "),
                       csound->curp2);
      {
    ...
    if (ip == NULL) /*  if already off, done  */
      ...
      xturnoff(csound, ip); /*  else some kind of off */
  }
}
...
int sensevents(CSOUND *csound) {
  ...
  if (UNLIKELY(csound->frstoff != NULL)) {
    ...
    if (O->Beatmode) {
      ...
      if (csound->frstoff->offbet <= tval)
        beatexpire(csound, tval);
    } else {
  ...
  if (--(csound->cyclesRemaining) <=
      0) { /* if done performing score segment: */
    if (!csound->cyclesRemaining) {
...
retest:
  ...
  while (csound->cyclesRemaining <= 0 &&
    ...
    if (!O->sampleAccurate) {
      if (O->Beatmode)
        ...
        csound->cyclesRemaining = RNDINT64(
            (csound->nxtim * csound->esr - csound->icurTime) / csound->ksmps);
        csound->nxtim =
            (csound->cyclesRemaining * csound->ksmps + csound->icurTime) /
            csound->esr;
      }
    } else {
      /* VL 30-11-2012
      ...
      */
      if (O->Beatmode)
        ...
        csound->cyclesRemaining =
            (int64_t)FLOOR((csound->nxtim * csound->esr - csound->icurTime +
                            csound->onedsr * 0.5) /
                           csound->ksmps);
        csound->nxtim =
            (csound->cyclesRemaining * csound->ksmps + csound->icurTime) /
  ...
  if (UNLIKELY(O->RTevents || getRemoteSocksIn(csound))) {
    ...
    while (csound->OrcTrigEvts != NULL &&
           csound->OrcTrigEvts->start_kcnt <= (uint32)csound->global_kcounter) {
    ...
    if ((sinp = getRemoteSocksIn(csound))) {
      while ((conn = *sinp++)) {
        while ((nrecvd = SVrecv(csound, conn, (void *)&(csound->SVrecvbuf),
                                sizeof(REMOT_BUF))) > 0) {
          int lentot = 0;
          do {
            REMOT_BUF *bp =
                (REMOT_BUF *)((char *)(&(csound->SVrecvbuf)) + lentot);
            ...
            if (bp->type == SCOR_EVT) {
              ...
              if (UNLIKELY(mep->type == 0xFF && mep->dat1 == 0x2F)) {
                ...
                m_chanmsg(csound, mep); /* or a chan msg     */
            }
            lentot += bp->len;
          } while (lentot < nrecvd);
        }
...
scode:
  ...
  if (retval == 1 || (retval == 2 && STA(sectno) > 1)) {
    ...
    csound->curp2 = csound->nxtim = csound->timeOffs =
...
int insert_score_event_at_sample(CSOUND *csound, EVTBLK *evt,
                                 int64_t time_ofs) {
  double start_time;
  EVTNODE *e, *prv;
  CSOUND *st = csound;
  ...
  if (csound->freeEvtNodes != NULL) { /* pop alloc from stack */
    ...
    e = (EVTNODE *)csound->Calloc(csound,
                                  sizeof(EVTNODE)); /* or alloc new one */
    if (UNLIKELY(e == NULL))
  ...
  if (evt->strarg != NULL) { /* copy string argument if present */
    ...
    e->evt.strarg =
        (char *)csound->Malloc(csound, (size_t)(p - evt->strarg) + 1);
  ...
  cont:
    ...
    evt->p2orig =
        (MYFLT)(((start_time - st->icurTime / st->esr) / st->ibeatTime) +
    ...
    if (UNLIKELY((unsigned int)(i - 1) >=
      ...
      if (i > INT32_MAX - 10)
        ...
        csoundErrorMsg(csound,
                       Str("insert_score_event(): invalid instrument "
                           "number or name %d\n"),
                       i);
      goto err_return;
    }
...
void musmon_rewind_score(CSOUND *csound) {
  ...
  if (csound->scstr)
    ...
    csound->Warning(csound, Str("cannot rewind score: no score in memory\n"));
}
