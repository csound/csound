...
static struct set_element_t *set_element_alloc(CSOUND *csound, char *data);
...
static int
  set_element_is_set_element(CSOUND *csound,
                             struct set_element_t *set_element);
#endif
struct set_t *csp_set_alloc(CSOUND *csound, set_element_data_eq *ele_eq_func,
...
static struct set_element_t *set_element_alloc(CSOUND *csound, char *data) {
  ...
  p = (struct set_element_t *)csound->Malloc(csound,
                                             sizeof(struct set_element_t));
  if (UNLIKELY(p == NULL)) {
...
struct set_t *csp_set_alloc_string(CSOUND *csound) {
  return csp_set_alloc(csound, csp_set_element_string_eq,
...
static void set_update_cache(CSOUND *csound, struct set_t *set) {
  ...
  if (set->count > 0) {
    ...
    int ctr = 0;
    set->cache =
        csound->Malloc(csound, sizeof(struct set_element_t *) * set->count);
    ele = set->head;
    while (ele != NULL) {
      set->cache[ctr] = ele;
      ctr++;
 ...
 */
static void set_element_get(struct set_t *set, char *data,
...
struct set_t *csp_set_union(CSOUND *csound, struct set_t *first,
  ...
  result = csp_set_alloc(csound, first->ele_eq_func, first->ele_print_func);
