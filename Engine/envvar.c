...
static const char *envVar_list[] = {
    "CSNOSTOP",   "CSOUND7RC",    "CSSTRNGS",     "CS_LANG", "HOME",  "INCDIR",
    "OPCODE7DIR", "OPCODE7DIR64", "RAWWAVE_PATH", "SADIR",   "SFDIR", "SFOUTYP",
    "SNAPDIR",    "SSDIR",        "MFDIR",        NULL};
...
int csoundSetEnv(CSOUND *csound, const char *name, const char *value) {
  ...
  cs_hash_table_put(csound, csound->envVarDB, (char *)name,
                    cs_strdup(csound, (char *)value));
...
int csoundAppendEnv(CSOUND *csound, const char *name, const char *value) {
  ...
  newval = (char *)csound->Malloc(
      csound, (size_t)strlen(oldval) + (size_t)strlen(value) + (size_t)2);
  /* append to old value */
...
int csoundInitEnv(CSOUND *csound) {
  ...
  for (i = 0; i < 16; i++) {
    if (globalEnvVarName(i)[0] != '\0') {
      retval = csoundSetEnv(csound, globalEnvVarName(i), globalEnvVarValue(i));
      if (retval != CSOUND_SUCCESS)
        return retval;
    }
  }
...
char **csoundGetSearchPathFromEnv(CSOUND *csound, const char *envList) {
  ...
  for (i = j = 0; i <= len; i++) {
    if (envList[i] == ';' || envList[i] == ':' || envList[i] == '\0') {
      if (i > j) {
        tmp = (nameChain_t *)csound->Malloc(csound,
                                            sizeof(nameChain_t) + (i - j));
        for (k = 0; j < i; j++, k++)
          tmp->s[k] = envList[j];
        tmp->s[k] = '\0';
        tmp->nxt = NULL;
  ...
  while (env_lst != NULL) {
    ...
    for (i = j = 0; i <= len; i++) {
      if (i == 0 && isalpha(s[i]) && s[i + 1] == ':')
        ...
        j = i + 1;
        if (i + 2 <= len && s[i + 2] == ':' && isalpha(s[i + 1]))
          i += 2;
      }
    }
  }
  ...
  p = (searchPathCacheEntry_t *)csound->Malloc(
      csound, sizeof(searchPathCacheEntry_t) + sizeof(char *) * pathCnt +
                  sizeof(char) * totLen);
...
again:
  if (UNLIKELY(getcwd(cwd, len) == NULL)) {
    ...
    len = len + len;
    cwd = csound->ReAlloc(csound, cwd, len);
...
void *csoundFileOpenWithType(CSOUND *csound, void *fd, int type,
  ...
  case CSFILE_SND_W: /* sound file write */
    ...
    sflib_command(p->sf, SFC_SET_VBR_ENCODING_QUALITY, &csound->oparms->quality,
                  sizeof(double));
    *((SNDFILE **)fd) = p->sf;
    break;
...
void *fopen_path(CSOUND *csound, FILE **fp, char *name, char *basename,
  ...
  fd = csound->FileOpen2(csound, fp, CSFILE_STD, name, "r", NULL, csftype, 0);
  if (fd != NULL)
    return fd;
  ...
  /* or use env argument */
  fd = csound->FileOpen2(csound, fp, CSFILE_STD, name, "r", env, csftype, 0);
  return fd;
}
