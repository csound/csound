...
static int
scanflt(CSOUND *csound,
        MYFLT *pfld) { /* read a MYFLT from scorefile; return 1 if OK, else 0 */
  int c;
    ...
    while ((c = corfile_getc(csound->scstr)) != '"') {
      ...
      *sstrp++ = c;
      ...
      if (n > csound->strsiz - 10) {
        csound->sstrbuf =
            csound->ReAlloc(csound, csound->sstrbuf, csound->strsiz += SSTRSIZ);
  ...
  if (UNLIKELY(!((c >= '0' && c <= '9') || c == '+' || c == '-' || c == '.'))) {
    ...
    csound->Message(
        csound, Str("ERROR: illegal character %c(%.2x) in scoreline: "), c, c);
    dumpline(csound);
    return (0);
  }
  corfile_ungetc(csound->scstr);
...
{                                     /*      presumes good format if warped */
  ...
  while ((c = corfile_getc(csound->scstr)) != '\0') {
    ...
    unwarped:
      ...
      while (1) {
        while ((c = corfile_getc(csound->scstr)) == ' ' || c == '\t')
          ; /* eat whitespace */
        if (c == ';') {
          flushline(csound);
          break;
                  ...
                  if (pp >= plim) {
                    ...
                    while ((corfile_getc(csound->scstr) != '\n') &&
                      ...
                      if (c + 1 >= (int)e->c.extra[0]) {
                        ...
                        /*        q, q[c-3], q[c-2], q[c-1]); */
                        csound->DebugMsg(
                            csound, "and more extra p-fields [%d](%d)%d\n", c,
                            (int)e->c.extra[0],
                            (int)sizeof(MYFLT) * ((int)e->c.extra[0] + PMAX));
                        new = (MYFLT *)csound->ReAlloc(csound, e->c.extra,
                                                       sizeof(MYFLT) * size);
                        if (new == NULL) {
