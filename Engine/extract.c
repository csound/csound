...
static const SRTBLK a0 = {NULL,    NULL,    0,       3, FL(0.0), FL(0.0),
                          FL(0.0), FL(0.0), FL(0.0), 0, SP,      "a 0 0 0\n"};

static const SRTBLK f0 = {NULL,    NULL,    0,       2, FL(0.0), FL(0.0),
                          FL(0.0), FL(0.0), FL(0.0), 0, SP,      "f 0 0\n"};

static const SRTBLK e = {NULL,    NULL,    0,       0, FL(0.0), FL(0.0),
                         FL(0.0), FL(0.0), FL(0.0), 0, SP,      "e\n"};
              ...
              FILE *xfp) /* read the extract control file */
{
  ...
  extractStatics->offsect = 999;
  extractStatics->offbeat = FL(0.0);
  while (fscanf(xfp, "%100s", s) > 0) {
    ...
    default:
      ...
      case 'i':
        sscanf(s, "%d", &i);
        // printf("i: %d\n", i);
        if (LIKELY(i >= 0 && i < INSMAX))
          extractStatics->inslst[i] = 1;
        else
          csound->Message(csound, Str("instrument number out of range"));
        all = 0;
        break;
      case 'f':
        // printf("f: %s\n", s);
#if defined(USE_DOUBLE)
        CS_SSCANF(s, "%d:%lf", &extractStatics->onsect,
                  &extractStatics->onbeat);
#else
...
#if defined(USE_DOUBLE)
        CS_SSCANF(s, "%d:%lf", &extractStatics->offsect,
                  &extractStatics->offbeat);
#else
        CS_SSCANF(s, "%d:%f", &extractStatics->offsect,
                  &extractStatics->offbeat);
#endif
      }
    }
  ...
  if (all) {
    char *ip;
    for (ip = &extractStatics->inslst[0]; ip < &extractStatics->inslst[INSMAX];
         *ip++ = 1)
      ;
  }
  extractStatics->ontime = extractStatics->a0.newp3 = extractStatics->a0.p3val =
      extractStatics->onbeat;
  ...
  if (extractStatics->sectno < extractStatics->onsect) {
    ...
    do {
      ...
      case 'i':
        ...
        /* FALLTHRU */
      case 'a':
        turnoff = bp->newp2 + bp->newp3; /* i and a: */
        if (extractStatics->sectno == extractStatics->onsect) {
...
static void include(EXTRACT_STATICS *extractStatics, SRTBLK *bp) {
  if (extractStatics->frstout == NULL) /* first one is special */
    ...
    extractStatics->prvout->nxtblk = bp; /* others just add onto list */
