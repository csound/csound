...
static void create_dag(CSOUND *csound) {
  ...
  csound->dag_task_status =
      csound->Calloc(csound, sizeof(stateWithPadding) * max);
  ...
  csound->dag_wlmm =
      (watchList *)csound->Calloc(csound, sizeof(watchList) * max);
}
...
static void recreate_dag(CSOUND *csound) {
  ...
  csound->dag_task_map = csound->ReAlloc(csound, (INSDS *)csound->dag_task_map,
                                         sizeof(INSDS *) * max);
  csound->dag_task_dep = (char **)csound->ReAlloc(csound, csound->dag_task_dep,
                                                  sizeof(char *) * max);
  csound->dag_wlmm = (watchList *)csound->ReAlloc(csound, csound->dag_wlmm,
                                                  sizeof(watchList) * max);
}

static INSTR_SEMANTICS *dag_get_info(CSOUND *csound, int insno) {
  INSTR_SEMANTICS *current_instr = csp_orc_sa_instr_get_by_num(csound, insno);
  if (current_instr == NULL) {
    current_instr = csp_orc_sa_instr_get_by_name(
        csound, csound->engineState.instrtxtp[insno]->insname);
...
taskID dag_get_task(CSOUND *csound, int index, int numThreads,
                    taskID next_task) {
  int i;
  int count_waiting = 0;
...
taskID dag_end_task(CSOUND *csound, taskID i) {
  ...
  { /* ATOMIC_SWAP */
    do {
      to_notify = ATOMIC_READ(task_watch[i]);
        ...
        !ATOMIC_CAS_PTR(&task_watch[i], to_notify, (watchList *)&DoNotRead));
  ...
  while (to_notify) { /* walk the list of watchers */
    ...
    // Try the same thing again but this time waiting on active or available
    // task
