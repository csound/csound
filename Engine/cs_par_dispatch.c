...
void csp_orc_sa_parallel_compute_spec_read(CSOUND *csound, const char *path)
{
    ...
    if (rc != 0 || rc == EOF)
      csound->Die(csound,
                  "Parallel Spec File invalid format expected "
    ...
    if (rc != 0 || rc == EOF)
      csound->Die(csound, "Parallel Spec File invalid format "
                  "expected roots_avail_min parameter");
    rc = fscanf(f, "%i\n", &(global_weight_info.roots_avail_max));
    if (rc != 0 || rc == EOF)
      csound->Die(csound, "Parallel Spec File invalid format expected"
...
void csp_dag_alloc(CSOUND *csound, DAG **dag)
{
    ...
    *dag = csound->Malloc(csound, sizeof(DAG));
    if (*dag == NULL) {
    ...
    csp_semaphore_alloc(csound, &((*dag)->consume_semaphore),
...
{
    int ctr = 0;
    if (dag == NULL || *dag == NULL)
      csound->Die(csound, "Invalid NULL Parameter dag");
...
static DAG *csp_dag_build_initial(CSOUND *csound, INSDS *chain)
{
    ...
    while (chain != NULL) {
      ...
      if (current_instr == NULL)
        csound->Die(csound,
                    "Failed to find semantic information for instrument '%i'",
                    chain->insno);
