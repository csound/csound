...
static struct global_var_lock_t *global_var_lock_find(CSOUND *csound,
                                                      char *name) {
  if (UNLIKELY(name == NULL))
    csound->Die(csound,
                Str("Invalid NULL parameter name for a global variable\n"));
...
TREE *csp_locks_insert(CSOUND *csound, TREE *root) {
  csound->Message(csound, Str("Inserting Parallelism Constructs into AST\n"));
  ...
  //            instr->read->count == 0 &&
  //            instr->write->count == 0) {
  //          csound->Message(csound, Str("Instr %d needs locks"),
  //          instr->insno);
  ...
  //        /*if (current->type == '=')*/
  //        {
  //          struct set_t *left = csp_orc_sa_globals_find(csound,
  //          current->left); struct set_t *right =
  //          csp_orc_sa_globals_find(csound, current->right); struct set_t *new
  //          = NULL; csp_set_union(csound, left, right, &new);
  ...
  //                                   T_OPCODE, lock_tok);
  //            lock_leaf->right = make_leaf(csound, current->line,
  //            current->locn,
  //                                         INTEGER_TOKEN, var_tok);
  //            unlock_leaf = make_leaf(csound, current->line, current->locn,
  //                                    T_OPCODE, unlock_tok);
  //            unlock_leaf->right = make_leaf(csound, current->line,
  //            current->locn,
  ...
  //                    Str("[End Inserting Parallelism Constructs into
  //                    AST]\n"));
