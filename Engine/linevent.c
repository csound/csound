...
#endif
#define MODE , 0
  else if (UNLIKELY((csound->Linefd =
                         open(O->Linename, O_RDONLY | O_NDELAY MODE)) < 0))
    csoundDie(csound, Str("Cannot open %s"), O->Linename);
...
void RTclose(CSOUND *csound) {
...
#endif
  {
    if (strcmp(csound->oparms->Linename, "stdin") != 0)
      close(csound->Linefd);
#if !defined(DOSGCC) && !defined(WIN32)
    else if (UNLIKELY(fcntl(csound->Linefd, F_SETFL, STA(stdmode))))
...
static CS_NOINLINE int linevent_alloc(CSOUND *csound, int reallocsize) {
  ...
  if (reallocsize > 0) {
    ...
    STA(Linebuf) =
        (char *)csound->ReAlloc(csound, (void *)STA(Linebuf), reallocsize);
...
void csoundInputMessageInternal(CSOUND *csound, const char *message) {
  ...
  if ((n = linevent_alloc(csound, 0)) != 0)
    return;
  ...
  while (1) {
    ...
    while (containsLF(Linestart, Linend)) {
      ...
      do {
        ...
        if (c == '.' && /*  if lone dot,       */
            (isblank(n = cp[1]) || n == LF)) {
          if (UNLIKELY(e.opcod != 'i' || STA(prve).opcod != 'i' ||
                       pcnt > STA(prve).pcnt)) {
...
static const char *errmsg_1 = Str_noop(
    "event: param 1 must be \"a\", \"i\", \"q\", \"f\", \"d\", or \"e\"");
static const char *errmsg_2 = Str_noop(
    "event: string name is allowed only for \"i\", \"d\", and \"q\" events");
...
int eventOpcode_(CSOUND *csound, LINEVENT *p, int insname, char p1) {
  ...
  if (evt.pcnt > 0) {
    if (insname) {
      ...
      if (csound->ISSTRCOD(*p->args[1])) {
        res = csound->strarg2insno(csound, get_arg_string(csound, *p->args[1]),
                                   1);
        if (UNLIKELY(res == NOT_AN_INSTRUMENT))
          return NOTOK;
        evt.p[1] = (MYFLT)res;
        ...
        evt.p[1] = *p->args[1];
        if (UNLIKELY((opcod == 'i' || opcod == 'd') &&
                     (insno == 0 || insno > csound->engineState.maxinsno ||
                      !csound->engineState.instrtxtp[(int)insno]))) {
          csound->Message(csound,
                          Str("WARNING: Cannot Find Instrument %d. No action."),
                          (int)insno);
          return OK;
        }
      }
  ...
  if (opcod == 'e' && (int)evt.pcnt >= 1 && evt.p[1] > 0) {
    evt.p[2] = evt.p[1];
    evt.pcnt = 2;
  }

  if (UNLIKELY(insert_score_event_at_sample(csound, &evt, csound->icurTime) !=
               0))
    return csound->PerfError(csound, &(p->h),
                             Str("event: error creating '%c' event"), opcod);
  return OK;
}
...
int eventOpcodeI_(CSOUND *csound, LINEVENT *p, int insname, char p1) {
  ...
  if (evt.pcnt > 0) {
    if (insname) {
      ...
      if (csound->ISSTRCOD(*p->args[1])) {
        ...
        if (UNLIKELY(evt.p[1] == NOT_AN_INSTRUMENT))
          return NOTOK;
        evt.p[1] = (MYFLT)res;
      } else { /* Should check for valid instr num here */
        MYFLT insno = FABS(*p->args[1]);
  ...
  if (opcod == 'f' && (int)evt.pcnt >= 2 && evt.p[2] <= FL(0.0)) {
    ...
    err = insert_score_event_at_sample(csound, &evt, csound->icurTime);
  if (UNLIKELY(err))
    csound->InitError(csound, Str("event_i: error creating '%c' event"), opcod);
...
int instanceOpcode_(CSOUND *csound, LINEVENT2 *p, int insname) {
  ...
  if (evt.pcnt > 0) {
    int res;
    if (insname) {
      res = csound->strarg2insno(csound, ((STRINGDAT *)p->args[0])->data, 1);
      ...
      if (csound->ISSTRCOD(*p->args[0])) {
        res = csound->strarg2insno(csound, get_arg_string(csound, *p->args[0]),
                                   1);
        if (UNLIKELY(evt.p[1] == NOT_AN_INSTRUMENT))
          return NOTOK;
        evt.p[1] = (MYFLT)res;
