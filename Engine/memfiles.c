...
static int Load_Het_File_(CSOUND *csound, const char *filnam, char **allocp,
                          int32 *len) {
  FILE *f;
  int length = 1024;
  int i = 0;
  ...
  for (i = sizeof(int16);; i += sizeof(int16)) {
    ...
    buffer[p] = '\0';
    /* Expand as necessary */
    if (UNLIKELY(i >= length - 4))
      all = csound->ReAlloc(csound, all, length += 1024);
    x = atoi(buffer);
...
static int Load_CV_File_(CSOUND *csound, const char *filnam, char **allocp,
                         int32 *len) {
  FILE *f;
  int length = 4096;
  unsigned int i = 0;
...
static int Load_LP_File_(CSOUND *csound, const char *filnam, char **allocp,
                         int32 *len) {
  FILE *f;
  int length = 4096;
  unsigned int i = 0;
  ...
  for (i = 0; i < 6; i++)
    fgetc(f); /* Skip LPANAL */
  if (UNLIKELY(4 != fscanf(f, "%d %d %d %d\n", &lph.headersize, &lph.lpmagic,
                           &lph.npoles, &lph.nvals))) {
    fclose(f);
...
static int Load_File_(CSOUND *csound, const char *filnam, char **allocp,
                      int32 *len, int csFileType) {
  FILE *f;
  // void *dummy = 0;
  *allocp = NULL;
  f = fopen(filnam, "rb");
 ...
 */
MEMFIL *ldmemfile2withCB(
    CSOUND *csound, const char *filnam, int csFileType,
    int (*callback)(
        CSOUND *, MEMFIL *)) { /* read an entire file into memory and log it */
...
int PVOCEX_LoadFile(CSOUND *csound, const char *fname, PVOCEX_MEMFILE *p) {
  ...
  if (UNLIKELY(pvx_id < 0)) {
    return pvx_err_msg(csound, Str("unable to open pvocex file %s: %s"), fname,
  ...
  if (UNLIKELY(i < totalframes)) {
    csound->Free(csound, pp);
    return pvx_err_msg(csound,
                       Str("error reading pvoc-ex file %s "
                           "after %d frames"),
                       fname, i);
  }
  ...
  csound->Message(csound,
                  Str("file %s (%" PRIi32 " bytes) loaded into memory\n"),
                  fname, mem_wanted);
...
SNDMEMFILE *csoundLoadSoundFile(CSOUND *csound, const char *fileName,
  ...
  p = (SNDMEMFILE *)csound->Malloc(
      csound, sizeof(SNDMEMFILE) + (size_t)sfinfo->frames * sizeof(MYFLT));
  /* set parameters */
  p->name = (char *)csound->Malloc(csound, strlen(fileName) + 1);
  strcpy(p->name, fileName);
  p->fullName =
      (char *)csound->Malloc(csound, strlen(csound->GetFileName(fd)) + 1);
    ...
    if (sflib_command(sf, SFC_GET_INSTRUMENT, &lpd, sizeof(SFLIB_INSTRUMENT)) !=
      ...
      if (lpd.loop_count > 0 && lpd.loops[0].mode != SF_LOOP_NONE) {
        ...
        p->loopMode = (lpd.loops[0].mode == SF_LOOP_FORWARD
                           ? 2
                           : (lpd.loops[0].mode == SF_LOOP_BACKWARD ? 3 : 4));
      ...
      p->baseFreq =
          pow(2.0, (double)(((int)lpd.basenote - 69) * 100 + (int)lpd.detune) /
                       1200.0) *
          csound->A4;
      p->scaleFac = pow(10.0, (double)lpd.gain * 0.05);
    }
  }
  if (UNLIKELY((size_t)sflib_readf_MYFLT(
                   sf, &(p->data[0]), (sf_count_t)p->nFrames) != p->nFrames)) {
  ...
  csound->Message(csound, "%s '%s' (sr = %d Hz, %d %s, %" PRId64 " %s) %s",
                  ...
                  Str("sample frames"), Str("loaded into memory\n"));
