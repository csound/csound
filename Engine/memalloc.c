...
static void memdie(CSOUND *csound, size_t nbytes) {
  csound->ErrorMsg(csound, Str("memory allocate failure for %zd"), nbytes);
...
void *mmalloc(CSOUND *csound, size_t size) {
  ...
  if (UNLIKELY(size == (size_t)0)) {
    csound->DebugMsg(
        csound, " *** internal error: mmalloc() called with zero nbytes\n");
    return NULL;
  }
#endif
  /* allocate memory */
...
#endif
  CSOUND_MEM_SPINLOCK((memAllocBlock_t *)p)->prv = (memAllocBlock_t *)NULL;
...
void mfree(CSOUND *csound, void *p) {
  ...
  if (UNLIKELY(pp->magic != MEMALLOC_MAGIC || pp->ptr != p)) {
    csound->Warning(csound,
                    "csound->Free() called with invalid "
                    "pointer (%p) %x %p %x",
...
void *mrealloc(CSOUND *csound, void *oldp, size_t size) {
  ...
  if (UNLIKELY(pp->magic != MEMALLOC_MAGIC || pp->ptr != oldp)) {
    csound->DebugMsg(csound,
                     " *** internal error: mrealloc() called with invalid "
                     "pointer (%p)\n",
                     oldp);
    /* exit() is ugly, but this is a fatal error that can only occur */
    /* as a result of a bug */
...
void *mreallocDebug(CSOUND *csound, void *oldp, size_t size, char *file,
                    int line) {
