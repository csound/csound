...
static intptr_t expand_nxp(CSOUND *csound) {
  ...
  if (UNLIKELY((csound->sread.nxp) >= ((csound->sread.memend) + MARGIN))) {
    csound->Die(csound, Str("sread:  text space overrun, increase MARGIN"));
    return 0; /* not reached */
  }
  /* calculate the number of bytes to allocate */
  nbytes = (size_t)((csound->sread.memend) - (csound->sread.curmem));
  nbytes = nbytes + (nbytes >> 3) + (size_t)(MEMSIZ - 1);
  nbytes &= ~((size_t)(MEMSIZ - 1));
  /* extend allocated memory */
  oldp = (csound->sread.curmem);
  (csound->sread.curmem) = (char *)csound->ReAlloc(
      csound, (csound->sread.curmem), nbytes + (size_t)MARGIN);
  (csound->sread.memend) = (char *)(csound->sread.curmem) + (int32)nbytes;
...
static void print_input_backtrace(CSOUND *csound, int needLFs,
  ...
  do {
    ...
    switch (lastsource) {
    case 0:
      m = Str("  included from line %d of macro %s%s");
      break;
    case 1:
      m = Str("  called from line %d of macro %s%s");
      break;
      // default:
    case 2:
      m = Str("  in line %d of macro %s%s");
      break;
    }
    ...
    if (lastinput && csound->oparms->useCsdLineCounts && csound->csdname) {
      ...
      msgfunc(csound, m,
              (lastsource == 0 ? csound->scoLineOffset + curr->line - 1
                               : csound->scoLineOffset + curr->line),
              csound->csdname, lf);
    }
    /* else { */
    /*   msgfunc(csound, m, (lastsource == 0 ? curr->line - 1 : curr->line), */
...
static MYFLT operate(CSOUND *csound, MYFLT a, MYFLT b, char c) {
  ...
  case '^':
    ...
    break;
  case '&':
    ans = (MYFLT)(MYFLT2LRND(a) & MYFLT2LRND(b));
    break;
  case '|':
    ans = (MYFLT)(MYFLT2LRND(a) | MYFLT2LRND(b));
    break;
...
#endif
  if (c == '\n') {
    (csound->sread.str)->line++;
    (csound->sread.linepos) = -1;
  } else
    (csound->sread.linepos)++;
  return c;
}
...
void sread_initstr(CSOUND *csound, CORFIL *sco) {
  ...
  (csound->sread.str)->line = 1;
  (csound->sread.str)->mac = NULL;
...
{                         /*  each score statement gets a sortblock   */
  int rtncod;             /* return code to calling program:      */
                          ...
                          /*   0 = end of file                    */
  /* sread_alloc_globals(csound); */
  (csound->sread.bp) = (csound->sread.prvibp) = csound->frstbp = NULL;
  ...
  again:
    ...
    case 'b': /* Set a clock base */
    {
      ...
      if (csound->sread.op == 'b')
        (csound->sread.clock_base) = stof(csound, (csound->sread.sp));
      else
        (csound->sread.clock_base) += stof(csound, (csound->sread.sp));
      ...
      *(csound->sread.nxp)++ = (csound->sread.op); /* Undo this line */
      (csound->sread.nxp)++;
      goto again;
    }
    case 'C': /* toggle carry */
    {
      char *old_nxp = (csound->sread.nxp) - 2;
      getpfld(csound, 0);
      (csound->sread.nocarry) = stof(csound, (csound->sread.sp)) == 0.0 ? 1 : 0;
      ...
      copylin(csound);
      {
        ...
        if (isdigit(q) || q == '+' || q == '-' || q == '.') {
          ...
          if (tmp != p && (*tmp == '\0' || isspace(*tmp))) {
            ...
            (csound->sread.bp)->p1val = (csound->sread.bp)->p2val =
    ...
    case 'm': /* Remember this place */
    {
      ...
      for (j = 0; j < (csound->sread.last_name); j++) {
        ...
        if (strcmp(buff, (csound->sread.names)[j].name) == 0)
          break;
      }
      ...
      (csound->sread.names)[j].posit = corfile_tell(csound->expanded_sco);
    ...
    case 'n': {
      ...
      for (i = 0; i <= (csound->sread.last_name); i++) {
        ...
        if (strcmp(buff, (csound->sread.names)[i].name) == 0)
          break;
      }
      // printf("i=%d\n", i);
      if (UNLIKELY(i > (csound->sread.last_name)))
        ...
        corfile_set(csound->expanded_sco, (csound->sread.names)[i].posit);
    ...
    {         /* Set local variability of time */
      char *old_nxp = (csound->sread.nxp) - 2;
      getpfld(csound, 0);
      (csound->sread.warp_factor) = stof(csound, (csound->sread.sp));
      if (csound->oparms->msglevel & CS_TIMEMSG)
...
static void ifa(CSOUND *csound) {
  ...
  while (getpfld(csound, 0)) { /* while there's another pfield,  */
    ...
    /*   continue; */
    /* } */
    if (*(csound->sread.sp) == '^' && (csound->sread.op) == 'i' &&
      ...
      if (UNLIKELY((csound->sread.prvp2) < 0)) {
        ...
        (csound->sread.prvp2) = (csound->sread.bp)->p2val =
            (csound->sread.warp_factor) * stof(csound, (csound->sread.sp) + 1);
        ...
        (csound->sread.prvp2) = (csound->sread.bp)->p2val =
            ...
            csound->sread.warp_factor * stof(csound, (csound->sread.sp) + 1);
               ...
               (*(csound->sread.sp) == '.' || *(csound->sread.sp) == '+')) {
      if ((csound->sread.op) == 'i' &&
          (*(csound->sread.sp) == '.' || (csound->sread.bp)->pcnt == 2) &&
          (((csound->sread.bp)->pcnt >= 2 &&
      ...
      if (UNLIKELY((csound->sread.op) != 'i')) {
        *((csound->sread.nxp) - 1) = '\0';
        getmore = 1;
        sreaderr(csound, Str("ignoring '%s' in '%c' event"), (csound->sread.sp),
                 (csound->sread.op));
      ...
      (csound->sread.bp)->pcnt--;
      if (getmore)
        continue; /* not the best, but not easy to delete event */
                  /* since ifa() doesn't return anything */
      else
        break;
    } else
      switch ((csound->sread.bp)->pcnt) { /*  watch for p1,p2,p3, */
      case 1:                             /*   & MYFLT, setinsno..*/
        if (((csound->sread.op) == 'i' || (csound->sread.op) == 'd' ||
             (csound->sread.op) == 'q') &&
            *(csound->sread.sp) == '"') {
          /* csound->DebugMsg(csound,"***Entering second dubious code
             scnt=%d\n", csound->scnt0); */
          (csound->sread.bp)->p1val = SSTRCOD; /* allow string name */
        } else {
          (csound->sread.bp)->p1val = stof(csound, (csound->sread.sp));
        }
        if ((csound->sread.op) == 'i' || (csound->sread.op) == 'd')
          setprv(csound);
        else
          (csound->sread.prvibp) = NULL;
        break;
      case 2:
        (csound->sread.prvp2) = (csound->sread.bp)->p2val =
            (csound->sread.warp_factor) * stof(csound, csound->sread.sp) +
      ...
      case 3:
        if ((csound->sread.op) == 'i')
          ...
          (csound->sread.bp)->p3val = stof(csound, (csound->sread.sp));
        break;
      default:
        break;
      }
    ...
    case 2:
      if ((csound->sread.warpin)) { /* for warpin,     */
      ...
      break;
    case 3:
      if ((csound->sread.warpin) &&
          ((csound->sread.op) == 'i' || (csound->sread.op) == 'f')) {
        getpfld(csound, 0); /* same for newp3  */
        (csound->sread.bp)->newp3 =
            (csound->sread.warp_factor) * stof(csound, (csound->sread.sp));
        (csound->sread.nxp) = (csound->sread.sp);
      } else
        (csound->sread.bp)->newp3 = (csound->sread.bp)->p3val;
      break;
    }
  }
  if ((csound->sread.nocarry) && ((csound->sread.bp)->pcnt < 3) &&
      (csound->sread.op) == 'i' &&
      ((prvbp = (csound->sread.prvibp)) != NULL ||
       (!(csound->sread.bp)->pcnt &&
        (prvbp = (csound->sread.bp)->prvblk) != NULL &&
...
{                                  /*  prvibp = prv note, same insno   */
  ...
  if (csound->ISSTRCOD((csound->sread.bp)->p1val) &&
    ...
    *c = '\0';
    ...
    n = (int16)(csound->sread.bp)->p1val; /* set current insno */
...
/*     uses *nxp++;    sp untouched         */
{
  ...
  while (n--) { /*      and copy n pflds    */
    ...
    switch (pfno) {
    case 1:
      (csound->sread.bp)->p1val = prvbp->p1val; /*  with p1-p3 vals */
      setprv(csound);
      break;
    case 2:
      if (*(p - 2) == '+') /* (interpr . of +) */
        ...
        (csound->sread.prvp2) = (csound->sread.bp)->p2val = prvbp->p2val;
      (csound->sread.bp)->newp2 = (csound->sread.bp)->p2val;
      break;
    case 3:
      (csound->sread.bp)->newp3 = (csound->sread.bp)->p3val = prvbp->p3val;
      break;
    default:
      break;
    }
...
static void
salcinit(CSOUND *csound) { /* init the sorter mem space for a new section */
...
static void
salcblk(CSOUND *csound) { /* alloc a srtblk from current mem space:   */
  ...
  (csound->sread.bp) =
      (SRTBLK *)(((uintptr_t)csound->sread.nxp + (uintptr_t)7) &
                 ~((uintptr_t)7));
  if (csound->frstbp == NULL)
...
static void
flushlin(CSOUND *csound) { /* flush input to end-of-line; inc lincnt */
  int c;
  ...
  *++op = '[';
  ...
  do {
    ...
    case '%':
      if (UNLIKELY(!type)) {
        scorerr(csound,
                Str("illegal placement of operator %c in [] "
                    "expression"),
                c);
      }
      if (*op == '*' || *op == '/' || *op == '%') {
    ...
    case ' ': /* Ignore spaces */
      ...
      continue;
    default:
      scorerr(csound, Str("illegal character %c(%.2x) in [] expression"), c, c);
    }
  } while (c != '$');
  return *pv;
}

static int getpfld(CSOUND *csound,
                   int type) /* get pfield val from SCOREIN file */
{                            /*      set sp, nxp                 */
  int c;
  char *p;
  ...
  *p++ = c;
  ...
  if (c == '"') { /* if have quoted string,  */
    ...
    if (UNLIKELY((csound->sread.bp)->pcnt < 3 &&
                 !(((csound->sread.op) == 'i' || (csound->sread.op) == 'd' ||
                    (csound->sread.op) == 'q') &&
                   !(csound->sread.bp)->pcnt))) {
...
{                                    /*      but a blank or nl at end)       */
  ...
  if (*p == 'z')
    return FL(800000000000.0); /* 25367 years */
