...
static int insert_midi(CSOUND *csound, int insno, MCHNBLK *chn, MEVENT *mep);
...
static void message_string_enqueue(CSOUND *csound, int attr, const char *str) {
...
static void no_op(CSOUND *csound, int attr, const char *format, va_list args) {
  IGN(csound);
  IGN(attr);
  IGN(format);
  IGN(args);
...
uintptr_t event_insert_thread(void *p) {
  ...
  void (*csoundMessageStringCallback)(CSOUND * csound, int attr,
                                      const char *str) = NULL;
  void (*csoundMessageCallback)(CSOUND * csound, int attr, const char *format,
                                va_list args) = csound->csoundMessageCallback_;
  if (csound->oparms_.msglevel) {
    if (csound->message_string_queue == NULL)
      csound->message_string_queue = (message_string_queue_t *)csound->Calloc(
          csound, QUEUESIZ * sizeof(message_string_queue_t));
...
static void set_xtratim(CSOUND *csound, INSDS *ip) {
  if (UNLIKELY(ip->relesing))
    return;
  ip->offtim =
      (csound->icurTime + ip->ksmps * (double)ip->xtratim) / csound->esr;
...
int insert(CSOUND *csound, int insno, EVTBLK *newevtp) {
  ...
  if (csound->oparms->realtime) {
    ...
    return insert_event(csound, insno, newevtp);
}
...
int insert_event(CSOUND *csound, int insno, EVTBLK *newevtp) {
  ...
  n = tp->pmax;
  if (UNLIKELY((tp->nocheckpcnt == 0) && n != newevtp->pcnt && !tp->psetdata)) {
    char *name = csound->engineState.instrtxtp[insno]->insname;
    if (UNLIKELY(name))
  ...
  if (newevtp->p3orig >= FL(0.0))
    ip->offbet =
        csound->beatOffs + (double)newevtp->p2orig + (double)newevtp->p3orig;
  else
    ip->offbet = -1.0;
  flp = &ip->p1.value;
  fep = &newevtp->p[0];
  ...
  if (O->sampleAccurate && !tie) {
    ...
    if (ip->p3.value > 0 && ip->xtratim == 0) {
      ...
      /* the ksmps_no_end field is initially 0, set to no_end in the last
         perf cycle */
      //  printf("*** duration_samps %d ip->ksmps_offset %d csound->ksmps %d ==>
      //  %d\n",
  ...
  if (ip->p3.value > FL(0.0) && ip->offtim > 0.0) { /* if still finite time, */
    ...
    if (O->sampleAccurate && !tie && ip->p3.value > 0 &&
    ...
    if (O->Beatmode) {
      p2 = ((p2 * csound->esr - csound->icurTime) / csound->ibeatTime) +
           csound->curBeat;
      ip->offbet =
          p2 + ((double)ip->p3.value * csound->esr / csound->ibeatTime);
    }
#ifdef BETA
    if (UNLIKELY(O->odebug))
      csound->Message(csound,
...
int MIDIinsert(CSOUND *csound, int insno, MCHNBLK *chn, MEVENT *mep) {
  ...
  if (csound->oparms->realtime) {
    ...
    return insert_midi(csound, insno, chn, mep);
}
...
int insert_midi(CSOUND *csound, int insno, MCHNBLK *chn, MEVENT *mep) {
  ...
  if (UNLIKELY((prvp = *ipp) != NULL)) { /*   if key currently activ */
    csoundWarning(csound, Str("MIDI note overlaps with key %d on same channel"),
                  (int)mep->dat1);
...
static void showallocs(CSOUND *csound) /* debugging aid */
{
  ...
  for (txtp = &(csound->engineState.instxtanchor); txtp != NULL;
       txtp = txtp->nxtinstxt)
    ...
    if ((p = txtp->instance) != NULL) {
      ...
      do {
        csound->Message(csound, "%d\t%p\t%p\t%p\t%p\t%p\t%p\t%d\t%3.1f\n",
                        (int)p->insno, (void *)p, (void *)p->nxtinstance,
                        (void *)p->prvinstance, (void *)p->nxtact,
                        (void *)p->prvact, (void *)p->nxtoff, p->actflg,
                        p->offtim);
      } while ((p = p->nxtinstance) != NULL);
    }
}

static void schedofftim(CSOUND *csound,
                        INSDS *ip) { /* put an active instr into offtime list */
  ...
  if ((nxtp = csound->frstoff) == NULL ||
    ...
    if (UNLIKELY(csound->oparms->odebug))
      csound->Message(csound, "schedofftim: %lf %lf %f\n", ip->offtim,
                      csound->icurTime / csound->esr, csound->curTime_inc);
    ...
    if (UNLIKELY(csound->oparms->odebug))
      csound->Message(
          csound, "schedofftim: %lf %lf %lf\n", ip->offtim,
          (csound->icurTime + (0.505 * csound->ksmps)) / csound->esr,
          csound->ekr *
              ((csound->icurTime + (0.505 * csound->ksmps)) / csound->esr));
#endif
  } else {
    while ((prvp = nxtp) && (nxtp = nxtp->nxtoff) != NULL &&
...
static void deact(CSOUND *csound,
                  INSDS *ip) { /* unlink single instr from activ chain */
  ...
  if (ip->opcod_deact) {
    ...
    for (k = 0; k < OPCODENUMOUTS_MAX; k++)
      if (p->cvt_in[k] != NULL) {
        ...
        break; // first null indicates end of cvt list
...
int kill_instance(CSOUND *csound, KILLOP *p) {
  if (LIKELY(*p->inst))
    xturnoff(csound, (INSDS *)((uintptr_t)*p->inst));
  else
    csound->Warning(csound, Str("instance not valid\n"));
  return OK;
}
...
{                                     /*      called by musmon              */
  ...
  csound->Message(csound, Str("could not find playing instr %f\n"), p1);
}
...
int subinstrset_(CSOUND *csound, SUBINST *p, int instno) {
  ...
  for (n = 1; (unsigned int)n < p->INOCOUNT; n++) {
    if (IS_STR_ARG(p->ar[inarg_ofs + n])) {
      ...
      if (str_cnt == 0)
        ...
        p->ip->strarg =
            csound->ReAlloc(csound, p->ip->strarg, len + strlen(argstr) + 1);
  ...
  if (!init_op && !(pip->reinitflag | pip->tieflag))
    csoundAuxAlloc(csound, (int32)csound->nspout * sizeof(MYFLT),
                   &p->saved_spout);
...
int subinstrset_S(CSOUND *csound, SUBINST *p) {
  ...
  if (UNLIKELY(instno == NOT_AN_INSTRUMENT))
    instno = -1;
    ...
    if (csoundGetTypeForArg(in) == &CS_VAR_TYPE_ARRAY) {
      if ((current->subType == &CS_VAR_TYPE_A ||
        ...
        return csound->InitError(
            csound, "audio/control arrays not allowed\n"
...
int32_t setksmpsset(CSOUND *csound, SETKSMPS *p) {
  ...
  if (CS_KSMPS != parent_ksmps)
    return OK; // no op if this has already changed

  l_ksmps = (unsigned int)*(p->i_ksmps);
  if (!l_ksmps)
    return OK; /* zero: do not change */
  if (UNLIKELY(l_ksmps < 1 || l_ksmps > CS_KSMPS ||
               ((CS_KSMPS / l_ksmps) * l_ksmps != CS_KSMPS))) {
    return csoundInitError(
        csound, Str("setksmps: invalid ksmps value: %d, original: %d"), l_ksmps,
        CS_KSMPS);
  }
  ...
  CS_VARIABLE *var = csoundFindVariableWithName(csound, ip->varPool, "ksmps");
...
int32_t oversampleset(CSOUND *csound, OVSMPLE *p) {
  ...
  if (os == 1 || CS_ESR != parent_sr)
    return OK; /* no op if changed already */
  ...
  */
  ...
  INSTRTXT *ip = p->h.insdshead->instr;
  CS_VARIABLE *var = csoundFindVariableWithName(csound, ip->varPool, "sr");
  MYFLT *varmem = p->h.insdshead->lclbas + var->memBlockIndex;
  *varmem = CS_ESR;
  var = csoundFindVariableWithName(csound, ip->varPool, "kr");
  varmem = p->h.insdshead->lclbas + var->memBlockIndex;
...
int32_t undersampleset(CSOUND *csound, OVSMPLE *p) {
  ...
  if (CS_KSMPS != parent_ksmps)
    return csoundInitError(
        csound, "can't undersample if local ksmps != parent ksmps\n");

  os = MYFLT2LRND(*p->os);
  onedos = FL(1.0) / os;
  if (os < 1)
    return csound->InitError(csound, "illegal undersampling ratio: %d\n", os);

  if (os == 1 || CS_ESR != parent_sr)
    return OK; /* no op if already changed */
...
int nstrstr(CSOUND *csound, NSTRSTR *p) {
  ...
  if (csound->engineState.instrumentNames) {
    ss = cs_inverse_hash_get(csound, csound->engineState.instrumentNames,
                             (int)*p->num);
  } else
    ss = "";
...
int subinstr(CSOUND *csound, SUBINST *p) {
  ...
  if (UNLIKELY(p->ip == NULL)) { /* IV - Oct 26 2002 */
    return csoundPerfError(csound, &(p->h), Str("subinstr: not initialised"));
  }
  ...
  for (chan = 0; chan < p->OUTOCOUNT; chan++) {
    for (pbuf = ip->spout + chan * nsmps, frame = 0; frame < nsmps; frame++) {
  ...
  if (this_instr->ksmps == 1) { /* special case for local kr == sr */
    do {
      ...
      if ((CS_PDS = (OPDS *)(this_instr->nxtp)) != NULL) {
        ...
        } while (error == 0 && p->ip != NULL && (CS_PDS = CS_PDS->nxtp));
      }
    ...
    */
    ...
    ofs = start;
    if (UNLIKELY(early))
      this_instr->ksmps_no_end = early % lksmps;
    do {
      this_instr->kcounter++;
...
static void instance(CSOUND *csound, int insno) {
  ...
  if (insno > csound->engineState.maxinsno) {
    ...
    size_t pcnt =
        sizeof(OPCOD_IOBUFS) + sizeof(MYFLT *) * (info->inchns + info->outchns);
  ...
  args:
    ...
    for (argStringCount = argsRequired(ep->outypes); n < argStringCount;
    ...
    for (; arg != NULL; n++, arg = arg->next) {
      ...
      if (arg->type == ARG_CONSTANT) {
        ...
        argpp[n] = (MYFLT *)(opMemStart + findLabelMemOffset(
                                              csound, tp, (char *)arg->argPtr));
      } else {
  ...
  /* VL 13-12-13: point the memory to the local ksmps & kr variables,
     and initialise them */
  CS_VARIABLE *var =
      csoundFindVariableWithName(csound, ip->instr->varPool, "ksmps");
  if (var) {
...
int delete_instr(CSOUND *csound, DELETEIN *p) {
  ...
  if (UNLIKELY(n == NOT_AN_INSTRUMENT || n > csound->engineState.maxinsno ||
  ...
  while (active != NULL) { /* Check there are no active instances */
    ...
    if (UNLIKELY(active->actflg)) { /* Can only remove non-active instruments */
      ...
      if (name)
        return csound->InitError(csound, Str("Instrument %s is still active"),
                                 name);
      else
        return csound->InitError(csound, Str("Instrument %d is still active"),
                                 n);
    }
#if 0
...
static SR_CONVERTER *src_linear_init(CSOUND *csound, int mode, float ratio,
                                     int size) {
  IGN(mode);
  SR_CONVERTER *pp =
      (SR_CONVERTER *)csound->Calloc(csound, sizeof(SR_CONVERTER));
  pp->data = csound->Calloc(csound, sizeof(MYFLT));
  pp->bufferin = csound->Calloc(csound, size * sizeof(MYFLT) *
...
static void src_linear_process(SR_CONVERTER *pp, MYFLT *in, MYFLT *out,
                               int outsamps) {
  int outcnt, incnt;
...
static int src_linear_convert(CSOUND *csound, SR_CONVERTER *pp, MYFLT *in,
                              MYFLT *out) {
  IGN(csound);
  ...
  if (ratio > 1) {
    ...
    if (!cnt)
      src_linear_process(pp, buff, out, size);
  }
  pp->cnt = cnt;
  return 0;
}
...
static SR_CONVERTER *src_init(CSOUND *csound, int mode, float ratio, int size) {
  return src_linear_init(csound, mode, ratio, size);
}
static int src_convert(CSOUND *csound, SR_CONVERTER *pp, MYFLT *in,
                       MYFLT *out) {
...
static SR_CONVERTER *src_init(CSOUND *csound, int mode, float ratio, int size) {
  if (mode < 4) {
    ...
    if (!err) {
      ...
      pp->bufferin =
          (float *)csound->Calloc(csound, sizeof(float) * p->cvt.input_frames);
      p->cvt.data_in = pp->bufferin;
      pp->bufferout =
          (float *)csound->Calloc(csound, sizeof(float) * p->cvt.output_frames);
...
/* this routine on upsampling feeds a buffer, converts, then outputs it in
   blocks; on downsampling, it feeds a buffer, when full converts and outputs
*/
static int src_convert(CSOUND *csound, SR_CONVERTER *pp, MYFLT *in,
                       MYFLT *out) {
  if (pp->mode < 4) {
...
static void src_deinit(CSOUND *csound, SR_CONVERTER *pp) {
  if (pp->mode < 4) {
    SRC *p = (SRC *)pp->data;
    src_delete(p->stat);
