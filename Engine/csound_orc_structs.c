...
int initStructVar(CSOUND* csound, void* p) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (mem->varType == ((CS_TYPE*) &CS_VAR_TYPE_S)) {
      ...
      dest = (ARRAYDAT*)csound->Calloc(csound, sizeof(ARRAYDAT));
      mem->varType->copyValue(csound, mem->varType, dest, src);
      printf("dest ptr: %p\n", dest);
      printf("dest->sizes ptr: %p\n", dest->sizes);
...
void initializeStructVar(CSOUND* csound, CS_VARIABLE* var, MYFLT* mem) {
  ...
  for (i = 0; i < len; i++) {
    ...
    csound->Message(csound, "Struct member ptr: %p\n", &memberMem->value);
...
void copyStructVar(CSOUND* csound, CS_TYPE* structType, void* dest, void* src) {
  ...
  for (i = 0; i < count; i++) {
    ...
    s->varType->copyValue(csound, s->varType, &d->value, &s->value);

    // if (dimensions > 0) {
    //     ARRAYDAT* arraySrc = (ARRAYDAT*) &s->value;
    //     ARRAYDAT* arrayDst = (ARRAYDAT*) &d->value;
    //     struct_array_member_assign(arraySrc, arrayDst, arraySrc->arrayType);
    // } else {
    // }
...
int add_struct_definition(CSOUND* csound, TREE* structDefTree) {
  ...
  while (current != NULL) {
    ...
    CS_TYPE* arrayType = ((CS_TYPE*) &CS_VAR_TYPE_ARRAY);
    ...
    CS_VARIABLE* var = (dimensions > 0 ? arrayType : memberType)->createVariable(
      csound, type
    );
