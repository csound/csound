...
static int gen02(FGDATA *ff, FUNC *ftp)
{
    ...
    if (ff->e.pcnt>=PMAX) csound->Warning(csound, "using extended arguments\n");
...
static int gen10(FGDATA *ff, FUNC *ftp)
{
    ...
    do {
      MYFLT *valp = (hcnt+4>=PMAX ? &ff->e.c.extra[hcnt+5-PMAX] : &ff->e.p[hcnt + 4]); 
...
static int gen17(FGDATA *ff, FUNC *ftp)
{
    ...
    while (--nsegs) {
      val = *valp++;
      if (UNLIKELY(nsw && valp>=&ff->e.p[PMAX-1])) nsw =0, valp = &(ff->e.c.extra[1]);
...
static int gen32(FGDATA *ff, FUNC *ftp)
{
    ...
    if (ff->e.pcnt>=PMAX) {
      csound->Warning(csound, "not using extended arguments\n");
      ff->e.pcnt = PMAX-1;
    }
    if (UNLIKELY(nargs < 4)) {
      return fterror(ff, Str("insufficient gen arguments"));
...
static int gen01raw(FGDATA *ff, FUNC *ftp)
{
    ...
    if (UNLIKELY((fd = sndgetset(csound, p))==NULL)) {  /* sndinset to open the file  */
      return fterror(ff, "Failed to open file");
    }
    if (ff->flen == 0) {                      /* deferred ftalloc requestd: */
      if (UNLIKELY((ff->flen = p->framesrem) <= 0)) {   /*   get minsize from soundin */
