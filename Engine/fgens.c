...
static const GEN or_sub[GENMAX + 1] = {
    GENUL, gen01, gen02, gen03, gen04, gen05, gen06, gen07, gen08, gen09, gen10,
    gen11, gen12, gen13, gen14, gen15, gen16, gen17, gen18, gen19, gen20, gen21,
    GENUL, gen23, gen24, gen25, GENUL, gen27, gen28, GENUL, gen30, gen31, gen32,
    gen33, gen34, GENUL, GENUL, GENUL, GENUL, GENUL, gen40, gen41, gen42, gen43,
    gen44, GENUL, GENUL, GENUL, GENUL, gen49, GENUL, gen51, gen52, gen53, GENUL,
    GENUL, GENUL, GENUL, GENUL, GENUL, GENUL};
  ...
  *ftpp = NULL;
  ...
  if (UNLIKELY(ff.fno > csound->maxfnum)) { /* extend list if necessary */
    ...
    for (size = csound->maxfnum; size < ff.fno; size += MAXFNUM)
      ;
    nn = (FUNC **)csound->ReAlloc(csound, csound->flist,
                                  (size + 1) * sizeof(FUNC *));
    csound->flist = nn;
  ...
  if (UNLIKELY(ff.e.pcnt > PMAX)) {
    // #ifdef BETA
    csound->DebugMsg(
        csound, "T%d/%d(%d): x=%p memcpy from %p to %p length %zu\n",
    ...
    ff.e.c.extra = (MYFLT *)csound->Malloc(
        csound, sizeof(MYFLT) * (evtblkp->c.extra[0] + 1));
  ...
  if (isstrcod(ff.e.p[4])) {
    ...
    if (genum < 0)
      genum = -genum;
    if (UNLIKELY(!genum ||
                 genum > csound->genmax)) { /*   & legal gen number x*/
  ...
  if (!ff.flen) {
    ...
    ff.guardreq = 1;
    if (UNLIKELY(genum != 1 && genum != 2 && genum != 23 && genum != 28 &&
                 genum != 44 && genum != 49 && genum <= GENMAX)) {
      return fterror(&ff,
                     Str("deferred size for GENs 1, 2, 23, 28 or 49 only"));
    }
    if (UNLIKELY(msg_enabled))
  ...
  powOfTwoLen:
    ...
    for (ltest = ff.flen, lobits = 0; (ltest & MAXLEN) == 0L;
         lobits++, ltest <<= 1)
      ;
  ...
  ftp->lenmask = ((ff.flen & (ff.flen - 1L))
                      ? 0L
                      : (ff.flen - 1L)); /*  init hdr w powof2 data  */
  ftp->lobits = lobits;
  i = (1 << lobits);
  ...
  if (ftp == NULL) {
    ...
    if (UNLIKELY(csound->actanchor.nxtact != NULL)) { /*   & chk for danger */
      ...
      csound->Warning(csound,
                      Str("ftable %d relocating due to size change"
...
static int gen05(FGDATA *ff, FUNC *ftp) {
  ...
  do {
    ...
    if (UNLIKELY(seglen < 0))
      goto gn5er1;
    if (UNLIKELY((mult = *valp / amp1) <= 0))
      goto gn5er2;
  ...
  if (ff->e.pcnt == 5 || (k == 1 && r == FL(1.0))) { /* simple "buzz" case */
    ...
    for (phs = 0; fp <= finp; phs++) {
      ...
      if (fabs(denom) < 1.0e-10) //(!(denom = (MYFLT) sin(x)))
        ...
        *fp++ = ((MYFLT)sin(tnp1 * x) / denom - FL(1.0)) * scale;
    }
    ...
    if ((absr = FABS(r)) > FL(0.999) && absr < FL(1.001))
      ...
      scale = (FL(1.0) - absr) / (FL(1.0) - FABS(rtn));
    for (phs = 0; fp <= finp; phs++) {
      x = (double)phs * tpdlen;
      numer = (MYFLT)cos(x * k) - r * (MYFLT)cos(x * km1) -
              rtn * (MYFLT)cos(x * kpn) + rtnp1 * (MYFLT)cos(x * kpnm1);
...
static int gen15(FGDATA *ff, FUNC *ftp) {
  ...
  hsin =
      (MYFLT *)csound->Malloc(csound, sizeof(MYFLT) * ((1 + ff->e.pcnt) / 2));
  nh = (nargs - 2) >> 1;
...
static int gen18(FGDATA *ff, FUNC *ftp) {
  ...
  while (cnt--) {
    fn = *pp++;
    if (UNLIKELY(nsw && pp >= &ff->e.p[PMAX - 1]))
      nsw = 0, pp = &(ff->e.c.extra[1]);
    amp = *pp++;
    if (UNLIKELY(nsw && pp >= &ff->e.p[PMAX - 1]))
      nsw = 0, pp = &(ff->e.c.extra[1]);
    start = (int)*pp++;
    ...
    if (LIKELY((fnp = csoundFTFind(csound, &fn)) !=
               NULL)) {                        /* make sure fn exists */
...
static int gen20(FGDATA *ff, FUNC *ftp) {
  ...
  case 6: /* Gaussian */
    ...
    for (i = 0, x = -6.0; i < ((int)ff->flen >> 1); i++, x += arg)
      ft[i] = (MYFLT)(xarg * (pow(2.718281828459,
                                  -(x * x) / (2.0 * varian * varian))));
  ...
  case 7: /* Kaiser */
  {
    ...
    for (i = 1; i < (int)ff->flen; i++, x += 1.0)
      ft[i] =
          (MYFLT)(xarg * besseli(beta * sqrt(1.0 - x * x * flen2)) * besbeta);
    return OK;
  }
  case 8: /* Rectangular */
...
top1:
  if (UNLIKELY(feof(f)))
    return NAN; /* Hope value is ignored */
...
{ /* And further hacked 2009 by JPff */
  ...
  fp = ftp->ftable;
  j = 0;
  while (!feof(infile) && j < ff->flen)
    fp[j++] = nextval(infile);
  tmp = nextval(infile); // overshot value
  if (UNLIKELY(!feof(infile)))
    csound->Warning(
        csound, Str("Number(s) after table full in GEN23, starting %f"), tmp);
...
static int gen25(FGDATA *ff, FUNC *ftp) {
  ...
  do {
    ...
    if (UNLIKELY(x2 < x1))
      goto gn25err;
    if (UNLIKELY(x1 > ff->flen || x2 > ff->flen))
      goto gn25err2;
    seglen = (int)(x2 - x1);
    if (UNLIKELY(y1 <= 0 || y2 <= 0))
      goto gn25err3;
    mult = y2 / y1;
    mult = POWER(mult, FL(1.0) / seglen);
    while (seglen--) {
...
static int gen33(FGDATA *ff, FUNC *ftp) {
  ...
  while (i--) {
    ...
    if (UNLIKELY(pnum < (-maxp) || pnum > maxp)) {
      srcft++;
      continue; /* skip partial with too high frequency */
    }
    /* initial phase */
    phs = TWOPI_F * *(srcft++);
    if (UNLIKELY(pnum < 0)) {
      phs = PI_F - phs;
      pnum = -pnum; /* negative frequency */
    }
    /* mix to FFT data */
...
static int gen34(FGDATA *ff, FUNC *ftp) {
  ...
  if (UNLIKELY((src = csoundFTnp2Findint(csound, &(ff->e.p[5]), 1)) == NULL))
    return NOTOK;
  srcft = src->ftable;
  srclen = (int32)src->flen;
  /* number of partials */
  nh = (int32)(ff->e.p[6] + FL(0.5));
  if (UNLIKELY(nh > (srclen / 3L)))
    nh = srclen / 3L;
  if (UNLIKELY(nh < 0L))
    nh = 0L;
  /* amplitude scale */
  ...
  csound->Free(csound, xn);
  csound->Free(csound, cn);
  csound->Free(csound, vn);
...
CS_NOINLINE int fterror(const FGDATA *ff, const char *s, ...) {
  ...
  va_end(args);
  csoundMessage(csound, "f%3.0f %8.2f %8.2f ", ff->e.p[1], ff->e.p2orig,
                ff->e.p3orig);
  if (isstrcod(ff->e.p[4]))
...
static CS_NOINLINE void ftresdisp(const FGDATA *ff, FUNC *ftp) {
  ...
  if (csound->csoundMakeGraphCallback_ == NULL)
    dispinit(csound);
  dispset(csound, &dwindow, ftp->ftable, (int32)(ff->flen), strmsg, 0,
          "ftable");
  display(csound, &dwindow);
}

static void generate_sine_tab(CSOUND *csound) { /* Assume power of 2 length */
  ...
  unsigned int i;
  int ltest, lobits;
  for (ltest = flen, lobits = 0; (ltest & MAXLEN) == 0L; lobits++, ltest <<= 1)
    ;
  ftp->lobits = lobits;
  i = (1 << lobits);
  ftp->lomask = (int32)(i - 1);
...
static CS_NOINLINE FUNC *ftalloc(const FGDATA *ff) {
  ...
  if (ftp == NULL) { /*   alloc space as reqd */
    ...
    ftp->ftable =
        (MYFLT *)csound->Calloc(csound, (1 + ff->flen) * sizeof(MYFLT));
  }
  ftp->fno = (int32)ff->fno;
  ftp->flen = ff->flen;
  return ftp;
...
FUNC *csoundFTFind(CSOUND *csound, MYFLT *argp) {
  ...
  fno = MYFLT2LONG(*argp);
  if (UNLIKELY(fno == -1)) {
    if (UNLIKELY(csound->sinetable == NULL))
      generate_sine_tab(csound);
    return csound->sinetable;
  }
  if (UNLIKELY(fno <= 0 || fno > csound->maxfnum ||
    ...
    csoundInitError(csound, Str("deferred-size ftable %f illegal here"), *argp);
    return NULL;
  }
  return ftp;
}
...
FUNC *csoundFTFindP(CSOUND *csound, MYFLT *argp) {
  ...
  if (UNLIKELY(fno <= 0 || fno > csound->maxfnum ||
    ...
    csound->ErrorMsg(csound,
                     Str("Deferred-size ftable %f load "
                         "not available at perf time."),
                     *argp);
    return NULL;
  }
  return ftp;
...
static int gen01raw(FGDATA *ff, FUNC *ftp) {
  ...
  memset(p, 0, sizeof(SOUNDIN));
  {
    ...
    } else if (filno >= 0 && filno <= csound->strsmax && csound->strsets &&
  ...
  if (p->channel == ALLCHNLS) {
    ...
    ftp->nchanls = 1;
  ftp->flenfrms =
      ff->flen / ftp->nchanls; /* VL fixed 8/10/19: using table nchnls */
  ftp->gen01args.sample_rate = (MYFLT)p->sr;
  ftp->cvtbas = LOFACT * p->sr * csound->onedsr;
  {
    SFLIB_INSTRUMENT lpd;
    int ans =
        sflib_command(fd, SFC_GET_INSTRUMENT, &lpd, sizeof(SFLIB_INSTRUMENT));
    if (ans) {
      double natcps;
#ifdef BETA
      if ((csound->oparms_.msglevel & 7) == 7) {
        csoundMessage(csound,
                      ...
                      "mode      : %d\n"
                      "start     : %d\tend       : %d\tcount  :%d\n\n",
                      lpd.basenote, 0U, lpd.key_lo, lpd.key_hi, lpd.velocity_lo,
                      lpd.velocity_hi, lpd.gain, lpd.loop_count,
      ...
      if (UNLIKELY(ftp->end1 > ff->flen || ftp->end2 > ff->flen)) {
        ...
        if ((maxend = ftp->end1) < ftp->end2)
          maxend = ftp->end2;
        csoundMessage(csound, Str("\tlooping endpoint %d exceeds ftsize %d\n"),
                      maxend, ff->flen);
  ...
  if (UNLIKELY((inlocs = getsndin(csound, fd, ftp->ftable, table_length, p)) <
               0)) {
...
static int gen44(FGDATA *ff, FUNC *ftp) {
  ...
  */
  ...
  if (ftp == NULL) {
    ...
    memset(fp = ftp->ftable, '\0', sizeof(MYFLT) * ff->e.p[3]);
  while (NULL != fgets(buff, 80, filp)) {
    ...
    if (n == 2)
      ...
      return fterror(ff, Str("GEN44: format error\n"));
...
static int gen51(FGDATA *ff, FUNC *ftp) /* Gab 1/3/2005 */
{
  ...
  for (j = 0; j < nvals; j++) {
    ...
    if (LIKELY(grade < PMAX - 10))
      x = pp[grade];
    else
      x = ff->e.c.extra[grade - PMAX + 11];
...
static int gen52(FGDATA *ff, FUNC *ftp) {
  ...
  for (n = 0; n < nchn; n++) {
    MYFLT *pp;
    if (LIKELY((n * 3) + 6 < PMAX - 1))
      pp = &(ff->e.p[(n * 3) + 6]);
    else
      pp = &(ff->e.c.extra[(n * 3) + 6 - PMAX]);
    ...
    src = f->ftable;
    i = n;
    if (LIKELY((n * 3) + 7 < PMAX - 1))
      j = MYFLT2LRND(ff->e.p[(n * 3) + 7]);
    else
      j = MYFLT2LRND(ff->e.c.extra[(n * 3) + 7 - PMAX]);
    if (LIKELY((n * 3) + 8 < PMAX - 1))
      k = MYFLT2LRND(ff->e.p[(n * 3) + 8]);
    else
      k = MYFLT2LRND(ff->e.c.extra[(n * 3) + 8 - PMAX]);
    while (i < len) {
      if (j >= 0 && j < len2)
        dst[i] = src[j];
      else
...
static void gen53_apply_window(MYFLT *buf, MYFLT *w, int npts, int wpts,
                               int minphase) {
  int64_t ph, ph_inc;
  int i, j;
  MYFLT pfrac;
...
static void gen53_freq_response_to_ir(CSOUND *csound, MYFLT *obuf, MYFLT *ibuf,
                                      MYFLT *wbuf, int npts, int wpts,
                                      int mode) {
  MYFLT *buf1, *buf2;
  double tmp;
  MYFLT scaleFac;
...
static int gen53(FGDATA *ff, FUNC *ftp) {
  ...
  if (mode & 2) { /* if input data is impulse response: */
    ...
    gen53_freq_response_to_ir(csound, dstftp, tmpft, winftp, dstflen, winflen,
                              mode);
    csound->Free(csound, tmpft);
  } else { /* input is frequency response: */
    csound->Message(csound, Str("GEN 53: frequency response input, "));
    gen53_freq_response_to_ir(csound, dstftp, srcftp, winftp, dstflen, winflen,
                              mode);
  }
  return OK;
}
...
int allocgen(CSOUND *csound, char *s, GEN fn) {
  ...
  if (LIKELY(csound->gensub == NULL)) {
    csound->gensub =
        (GEN *)csound->Malloc(csound, csound->genmax * sizeof(GEN));
