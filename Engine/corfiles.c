...
CORFIL *copy_to_corefile(CSOUND *csound, const char *fname, const char *env,
                         int fromScore) {
  CORFIL *mm;
  FILE *ff;
  void *fd;
...
void corfile_preputs(CSOUND *csound, const char *s, CORFIL *f) {
  char *body = f->body;
  f->body =
      (char *)csound->Malloc(csound, f->len = (strlen(body) + strlen(s) + 1));
  f->p = f->len - 1;
  strcpy(f->body, s);
  strcat(f->body, body);
...
#ifdef JPFF
/* Start of directory of corfiles currently unused except experimental in
 * CsFileC */
typedef struct dir {
  char *name;
  CORFIL *corfile;
