...
it takes all board control data through software bus channels (as well
as MIDI):
...
- leds 1 and 2 indicate the state of toggle1 and toggle2, seed led
  indicates the state of etoggle
- MIDI can be received via USB (seed USB port) and uart (mini-jack on
  the pod board) at the same time.
