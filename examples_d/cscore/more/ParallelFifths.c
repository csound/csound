                               ...
                               /* csound -C will run Csound scores as normal */
{
    ...
    while ( (a = cscoreListGetSection(cs)) != NULL && a->nevents > 0) {
      c = cscoreListSeparateTWF(cs, a);   /* separate non-i-statements to
                                             avoid copying them */
      b = cscoreListCopyEvents(cs, a);    /* copy event list a into b */
      a = cscoreListConcatenate(cs, a,c); /* recombine a & c */
      for ( i = 1; i <= b->nevents; i++ ) /* iterate over b: events have
                                             indices 1 to nevents */
          if ( b->e[i]->op == 'i' )
      ...
      /* re-append the s statement -- must assign to a in case list grows! */
      a = cscoreListAppendStringEvent(cs, a,"s");
    ...
    if (a)      cscoreListFreeEvents(cs, a);    /* must reclaim space from lget()
                                                   above if non-NULL */
    a = cscoreListCreate(cs, 1);                /* allocate space for 1, not 0! */
    a = cscoreListAppendStringEvent(cs, a,"e"); /* must reassign a in case it
                                                   grows */
