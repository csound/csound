...
Separate control is provided over each step of rendering a composition:
...
8. The rendering can be resumed at any time, and at any point in the score,
...
class CompositionBase(object):
    ...
    '''
    All filenames are based off the complete pathname of the composition script.
    '''
    ...
    def assembleOrchestra(self):
        csound.setCSD(defaultOrchestra)
    ...
    def getAudioCommand(self, dac):
        command = 'csound -R -W -f -r 44100 -k 441 -o %s %s %s' % (dac, self.getOrcFilename(), self.getScoFilename())
    ...
    def performMasterSoundfile(self):
        self.generate()
        if self.keepPerforming:
            self.renderMasterSoundfile()
    '''
    Stops rendering at any stage.
    '''
    def stop(self):
        self.keepPerforming = False
...
if __name__ == '__main__':
    ...
    print 'Began performing...'
    composition.performAudio()
    composition.performanceThread.Join()
    print 'Ended performing.'
