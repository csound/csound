...
int csPerform(ClientData clientData, Tcl_Interp* interp,
	...
	if (csoundGetOutputBufferSize(cs)
		...
		else while(!(result = csoundPerformBuffer(cs)));
		sprintf(res, "%d", result);
	    p->result = result; 
	} else sprintf(res, "%d", -1); 
	Tcl_SetResult(interp, res, TCL_VOLATILE);
	return(TCL_OK);
}
...
int csNoteList(ClientData clientData, Tcl_Interp* interp,
	...
	for(i=0; i < largc; i++) pFields[i] = (MYFLT) atof(largv[i]);
	if(p->status == CS_RUNNING || p->status == CS_COMPILED 
	  || p->status == CS_PAUSED) {
...
int csEventList(ClientData clientData, Tcl_Interp* interp,
	...
	Tcl_SplitList(interp, argv[1], &largc, (CONST84 char ***) &largv);
	type = *largv[1];
	for(i=1; i < largc; i++) pFields[i-1] = (MYFLT) atof(largv[i]);
	if(p->status == CS_RUNNING || p->status == CS_COMPILED 
	  || p->status == CS_PAUSED) {
...
int csInValue(ClientData clientData, Tcl_Interp* interp,
	...
	resp = Tcl_GetObjResult(interp);
	if(SetChannelValue(p->inchan, Tcl_GetStringFromObj(argv[1], NULL), (MYFLT) val) 
	  != CHAN_NOT_FOUND)
                                       ...
                                       const char *name, MYFLT val)
{
  ...
  if(GetChannelValue(p->outchan, (char*) name, &oldval) == CHAN_FOUND){
    ...
    if(oldval != val) Tcl_UpdateLinkedVar(p->interp, name);
   }
}
...
csSetControlChannel(ClientData clientData, Tcl_Interp* interp,
	...
	if(argc >= 3) {
	resp = Tcl_GetObjResult(interp);
	result = csoundGetChannelPtr(cs, &pvalue, Tcl_GetStringFromObj(argv[1], NULL), 
...
csSetStringChannel(ClientData clientData, Tcl_Interp* interp,
	...
	else Tcl_SetResult(interp, "argc not 3", TCL_VOLATILE);	
	return (TCL_OK);				
}
...
csGetStringChannel(ClientData clientData, Tcl_Interp* interp,
	...
	if(argc >= 2) {
	result = csoundGetChannelPtr(cs, &pvalue, argv[1], 
						CSOUND_OUTPUT_CHANNEL | CSOUND_STRING_CHANNEL);
	if(!result)
...
int tclcsound_initialise(Tcl_Interp* interp) { 
	...
	Tcl_CreateCommand(interp, "csCompileList", (Tcl_CmdProc *)csCompileList, (ClientData) pdata, NULL);
	Tcl_CreateCommand(interp, "csPlay", (Tcl_CmdProc *)csPlay, (ClientData) pdata, NULL);
	Tcl_CreateCommand(interp, "csPerform", (Tcl_CmdProc *)csPerform, (ClientData) pdata, NULL);
	Tcl_CreateCommand(interp, "csPerformKsmps", (Tcl_CmdProc *)csPerformKsmps, (ClientData) pdata, NULL);
	Tcl_CreateCommand(interp, "csPerformBuffer", (Tcl_CmdProc *)csPerformBuffer, (ClientData) pdata, NULL);
	...
	Tcl_CreateObjCommand(interp, "csNote", (Tcl_ObjCmdProc *)csNote, (ClientData) pdata, NULL);
	Tcl_CreateObjCommand(interp, "csEvent", (Tcl_ObjCmdProc *)csEvent, (ClientData) pdata, NULL);
	Tcl_CreateObjCommand(interp, "csTable", (Tcl_ObjCmdProc *)csTable, (ClientData) pdata, NULL);
	Tcl_CreateCommand(interp, "csNoteList", (Tcl_CmdProc *)csNoteList, (ClientData) pdata, NULL);
	Tcl_CreateCommand(interp, "csTableList", (Tcl_CmdProc *)csTableList, (ClientData) pdata, NULL);
	Tcl_CreateCommand(interp, "csEventList", (Tcl_CmdProc *)csEventList, (ClientData) pdata, NULL);		
	Tcl_CreateCommand(interp, "csRewind", (Tcl_CmdProc *)csRewind, (ClientData) pdata, NULL);
	Tcl_CreateObjCommand(interp, "csOffset", (Tcl_ObjCmdProc *)csOffset, (ClientData) pdata, NULL);
	Tcl_CreateObjCommand(interp, "csGetOffset", (Tcl_ObjCmdProc *)csGetOffset, (ClientData) pdata, NULL);
	Tcl_CreateObjCommand(interp, "csGetScoreTime", (Tcl_ObjCmdProc *)csGetScoreTime, (ClientData) pdata, NULL);
	...
	Tcl_CreateObjCommand(interp, "csOutValue", (Tcl_ObjCmdProc *)csOutValue, (ClientData) pdata, NULL);	
	Tcl_CreateObjCommand(interp, "csInValue", (Tcl_ObjCmdProc *)csInValue, (ClientData) pdata, NULL);	
	Tcl_CreateCommand(interp, "csOpcodedir", (Tcl_CmdProc *)csOpcodedir, (ClientData) pdata, NULL);
	Tcl_CreateCommand(interp, "csSetenv", (Tcl_CmdProc *)csSetenv, (ClientData) pdata, NULL);
	Tcl_CreateObjCommand(interp, "csSetTable", (Tcl_ObjCmdProc *)csSetTable, (ClientData) pdata, NULL);
	Tcl_CreateObjCommand(interp, "csGetTable", (Tcl_ObjCmdProc *)csGetTable, (ClientData) pdata, NULL);
	Tcl_CreateObjCommand(interp, "csGetTableSize", (Tcl_ObjCmdProc *)csGetTableSize, (ClientData) pdata, NULL);
    Tcl_CreateObjCommand(interp, "csGetControlChannel", (Tcl_ObjCmdProc *)csGetControlChannel, (ClientData) pdata, NULL);
    Tcl_CreateObjCommand(interp, "csSetControlChannel", (Tcl_ObjCmdProc *)csSetControlChannel, (ClientData) pdata, NULL);	
    Tcl_CreateCommand(interp, "csGetStringChannel", (Tcl_CmdProc *)csGetStringChannel, (ClientData) pdata, NULL);
    Tcl_CreateCommand(interp, "csSetStringChannel", (Tcl_CmdProc *)csSetStringChannel, (ClientData) pdata, NULL);
