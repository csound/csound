...
int isCsoundFile(char *in){
	...
	if(*in == '.'){
		tmp = *((int *)in);
		if(DEBUG) fprintf(stderr,"%d: %c%c%c%c\n", i, in[0],in[1], in[2], in[3]);
...
char *SetWorkingDirectory(globals *g){
	...
	if((res = GetControlByID(*(g->mainwindow), &textID,&textfield)) < 0) {
		if(DEBUG) fprintf(stderr, "code: %d\n", res);
		return;
	}
	GetControlData(textfield, 0, kControlEditTextTextTag, NULL, NULL, &actualsize);
	text = (char *) malloc(actualsize+2);
	GetControlData(textfield, 0, kControlEditTextTextTag, actualsize, text, &actualsize);
	if(text[actualsize-1] != '/') {
		text[actualsize] = '/';
		text[actualsize+1] = '\0';
	}
	else text[actualsize] = '\0';
	return text;
}
...
void Compile (globals *g){
	...
	if(DEBUG) fprintf(stderr, "text event: %s %d\n");
	tmp[0] = "csound";
...
OSStatus MainWindowHandler (EventHandlerCallRef inHandlerCallRef,
	...
	if (commandStruct.commandID == 'Manu') {
	 if(g->manopen) system("killall Safari");
	 system("/Applications/Safari.app/Contents/MacOS/Safari "
	"/Library/Frameworks/CsoundLib.Framework/Resources/Manual/index.html &");
	g->manopen = 1;
	}
}
pascal void Process(EventLoopTimerRef  theTimer,
					void* userData){	
	globals *g = (globals *)userData;
	if(!g->pause && g->on)csoundPerformBuffer(g->cs);
...
void EventHandlerInitialise(globals *g){
    ...
    InstallWindowEventHandler (*(g->mainwindow), handlerUPP,1, &eventType,g,NULL);
	...
	InstallWindowEventHandler (*(g->mainwindow), handlerUPP,1, &eventType,g->mainwindow,NULL);
	if(DEBUG) fprintf(stderr, "installed\n");
}
...
int main(int argc, char* argv[])
{
	...
	"-B<buffsize>                 set HW buffer to <buffsize> bytes\n"
	"-b<buffsize>                 set SW buffer to <buffsize> bytes\n"
	"\nif SFDIR (SADIR/SSDIR) is not set, output (and input) sound filenames need to be full-path\n"
	"TIP: you can set these variables in the ~/.MacOSX/environment.plist file",
	window);
