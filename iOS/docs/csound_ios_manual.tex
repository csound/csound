...
\subsection{CsoundDataBinder for Communicating with Csound}

The CsoundObj API has been created to ease communication with Csound by using objects that implement the CsoundDataBinder protocol.  The protocol definition is as follows:

\begin{lstlisting}[caption=CsoundDataBinder Protocol Definition]
...
CsoundDataBinders are used to both read values from Csound as well as write values to Csound.  The lifecycle of CsoundDataBinders is as follows:

\begin{itemize}
\item \textbf{setup} - this method is called after Csound's compile call but before the main performance loop. CsoundDataBinders should use this method to create any channel pointers and any other values they will need during performance.
...
\item \textbf{cleanup} - this method is called after Csound has completed its run and should be used by CsoundDataBinders to free up any allocated data and remove references to channel pointers.
\end{itemize}

By using CsoundDataBinders, CsoundObj functionality can be extended to communicate with as many items as you would like. Included with Csound for iOS is the CsoundUI which contains pre-made wrapper classes for common UI classes (UISlider, UIButton, UISwitch) and similarly, CsoundMotion provides wrapper for hardware sensors (Accelerometer, Attitude, Gyroscope).  CsoundObj has helper methods for the CsoundDataBinders that come with the CsoundObj API, as well as the generic \textbf{addDataBinder} and \textbf{removeDataBinder} methods for adding custom CsoundDataBinders. Please consult these classes as well as their use in context within the Csound for iOS Examples project.
...
-(id<CsoundDataBinder>)addSwitch:(UISwitch*)uiSwitch forChannelName:(NSString*)channelName;
-(id<CsoundDataBinder>)addSlider:(UISlider*)uiSlider forChannelName:(NSString*)channelName;
-(id<CsoundDataBinder>)addButton:(UIButton*)uiButton forChannelName:(NSString*)channelName;
\end{lstlisting}

These methods allow for easy binding of UISwitches, UISliders, and UIButtons, and return the CsoundDataBinder that was created to wrap the widget. If the design of your app requires that you remove a widget from being used with CsoundObj, you can use the returned CsoundDataBinder and pass it to the removeDataBinder method. To bind your own custom widgets, you will need to create your own CsoundDataBinder.  There are examples of both using the convenience widget binding methods as well as custom CsoundDataBinder in the Csound for iOS Examples project.
...
-(id<CsoundDataBinder>)enableAccelerometer;
-(id<CsoundDataBinder>)enableGyroscope;
-(id<CsoundDataBinder>)enableAttitude;
\end{lstlisting}

When these features are enabled, CsoundDataBinders that wrap the sensors will send values into Csound via hardcoded channels:
...
This example demonstrates using a CsoundDataBinder to read an f-table from Csound and displaying that table.  Note that the WaveView's code is doing some optimization to check if it has already loaded.  It is also checking that the table itself has completed loading before trying to grab any values for the table.
...
The ControlKnob widget is custom UIView that is also a CsoundDataBinder.  It uses a hardcoded Csound channel called "pitch".  After the CsoundObj object is setup and started, performance of the audio file is done in the callback to the Play button press:
