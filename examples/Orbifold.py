...
'''
print __doc__
# Comment all references to CsoundVST out if you don't want to hear anything
# (or you don't have CsoundVST).
...
import CsoundVST
from visual import *
scene.title = "VOICE-LEADING ORBIFOLD: click ball=play, drag right button=spin, drag middle button=zoom"
scene.fullscreen = 0
scene.width = 800
scene.height = 600
scene.autoscale = 1
...
trichords = {}
balls = {}
for layer in xrange(0, maximumLayer + 1):
    trichordCount = 0
            ...
            for z in xrange(y, layer/3+9):
                ...
                if sum == layer and z <= (x + tones):
                    ...
                    if trichord not in trichords:
                        ...
                        ball = sphere(pos = trichord, radius = 0.125, opacity=0.5)
                        balls[ball] = ball;
                        # Make a label for each ball, and hide it till it's needed.
                        ball.trichord = ((x + tones) % tones, (y + tones) % tones, (z + tones) % tones)
                        if ball.trichord == (10,1,6):
                            scene.center = ball.pos
                        ball.layer = layer
                        ball.sum = ball.trichord[0] + ball.trichord[1] + ball.trichord[2]
                        ball.name = "%2d (%2d,%2d,%2d)" % (ball.sum, ball.trichord[0], ball.trichord[1], ball.trichord[2])
                        ball.label = label(pos = trichord, space = 0, text = ball.name, height = 11, box = 0, opacity = 0.0, visible = 0)
                        hue = (float(ball.layer) + 1.0) / float(maximumLayer)
                        hue = float(z) / tones
                        saturation = 1.0
                        value = 1.0                            
                        ball.color = color.hsv_to_rgb((hue, saturation, value))
                        print "Layer", ball.name
    print
def connect(origin, neighbor):
    if neighbor in trichords:
        curve(pos = [origin, neighbor], color = (0.5, 0.5, 0.5))
...
i 220 	0  	10000   1       1 
    ...
    ''')
    csound.setCommand('csound -d -m0 -b400 -B400 -odac3 temp.orc temp.sco')
    ...
    if scene.mouse.clicked:
        ...
        if m.pick in balls:
            ...
            if pickedBall:
                ...
                note1 = "i  7 0 4 %d 70 0 -.75" % (60 + pickedBall.trichord[0])
                note2 = "i  8 0 4 %d 70 0  .0"  % (60 + pickedBall.trichord[1])
                note3 = "i 14 0 4 %d 70 0  .75" % (60 + pickedBall.trichord[2])
