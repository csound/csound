...
It is highly recommend that you update your MinGW installation from the SourceForge site to the ``current'' level for core gcc, g++, binutils, and utils.
...
CsoundVST has a built-in Python interpreter. With Python, the user can generate a score, import a MIDI file, process notes, load and run a Csound orchestra, and in general do anything that can be done either with Csound or in Python.

You must define the \texttt{PYTHONPATH} environment variable to include the directory containing the \texttt{\_CsoundVST} shared library and \texttt{CsounVST.py} script.
...
\begin{verbatim}
csound.setOrchestra('''sr = 44100kr = 441ksmps = 100   nchnls = 20dbfs = .1instr 1,2,3,4,5 ; FluidSynth General MIDI; INITIALIZATION; Channel, bank, and program determine the preset, that is, the actual sound.ichannel		=			p1iprogram		=			p6ikey	 		= 			p4ivelocity 		= 			p5 + 12ijunk6 			= 			p6ijunk7			=			p7; AUDIOistatus			=			144;			print			iprogram, istatus, ichannel, ikey, ivelocityaleft, aright		fluid			"c:/projects/csound5/samples/VintageDreamsWaves-v2.sf2", \
    iprogram, istatus, ichannel, ikey, ivelocity, 1			outs 			aleft, arightendin''')csound.setCommand("csound --opcode-lib=c:/projects/csound5/fluid.dll \
    -RWdfo ./koch.wav ./temp.orc ./temp.sco")csound.exportForPerformance()csound.perform()\end{verbatim}
...
\begin{enumerate}
\item Fix bug in -z option.
\item Get CsoundVST working in Cubase.
\item Complete this document sufficiently to get users started building running Csound and CsoundVST with MinGW and Linux.
\item Tutorial scores:
\begin{enumerate}
\item toot.csd
\item trapped.csd
\item trappedrt.csd
...
\item Singleton class for CsoundVST or CppSound so that VST scripts can be identical to Python interpreter scripts.
\item A \emph{Mathematica} interface - but what kind?
\end{enumerate}
\item Longer term:
\begin{enumerate}
\item Get the Loris plugin opcode into this build.
\item STK plugin opcodes into this build.
