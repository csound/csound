...
static void dct36(register MYFLT *in, register MYFLT *out1, register MYFLT *out2, register MYFLT *w, register MYFLT *ts)
{
  MYFLT tmp[18];
...
static void III_hybrid(mpadec_t mpadec, grinfo_t *gr_info, MYFLT fs_in[SBLIMIT][SSLIMIT], MYFLT ts_out[SSLIMIT][SBLIMIT], int channel)
{
  register struct mpadec_t *mpa = (struct mpadec_t *)mpadec;
  register grinfo_t *grinfo = gr_info;
...
void decode_layer3(mpadec_t mpadec, uint8_t *buffer)
{
  ...
  for (gr = 0; gr < granules; gr++) {
    ...
    if (mpa->frame.channels > 1) {
      ...
      if (ms_stereo) {
        MYFLT *in0 = (MYFLT *)(mpa->hybrid_in[0]), *in1 = (MYFLT *)(mpa->hybrid_in[1]);
