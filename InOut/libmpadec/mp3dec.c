...
int mp3dec_init_file(mp3dec_t mp3dec, int fd, int64_t length, int nogap)
{
  ...
  if (mp3->taginfo.flags & 1) {
    ...
    if (mp3->mpainfo.frames && mp3->mpainfo.frame_samples) {
      mp3->mpainfo.bitrate = (int32_t)((MYFLT
)(((MYFLT)mp3->stream_size*(MYFLT)mp3->mpainfo.frequency + 0.5)/((MYFLT)125.0*mp3->mpainfo.frame_samples*mp3->mpainfo.frames)));
    }
  } else if (mp3->mpainfo.bitrate && mp3->mpainfo.frame_samples) {
    mp3->mpainfo.frames = (int32_t)((MYFLT)(((MYFLT)mp3->stream_size*(MYFLT)mp3->mpainfo.frequency + 0.5)/((MYFLT)125.0*mp3->mpainfo.frame_samples*mp3->mpainfo.bitrate)));
  }
...
int mp3dec_seek(mp3dec_t mp3dec, int64_t pos, int units)
{
  ...
  if (units == MP3DEC_SEEK_BYTES) {
    ...
    MYFLT fsize = (MYFLT)(125.0*mp3->mpainfo.bitrate*mp3->mpainfo.decoded_frame_samples)/(MYFLT)mp3->mpainfo.decoded_frequency;
    newpos = (int64_t)((MYFLT)pos*fsize/(MYFLT)mp3->mpainfo.decoded_frame_samples);
