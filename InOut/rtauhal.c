	       ...
	       csdata *cdata, int isInput)
{
  ...
  psize = sizeof(AudioDeviceID);
  if(isInput) AudioUnitSetProperty(*aunit, kAudioOutputUnitProperty_CurrentDevice, 
  ...
  AudioUnitSetProperty(*aunit, kAudioUnitProperty_MaximumFramesPerSlice,
		       kAudioUnitScope_Global, isInput, &bufframes, sizeof(UInt32));

  psize = sizeof(AudioStreamBasicDescription);
  AudioUnitGetProperty(*aunit, kAudioUnitProperty_StreamFormat, 
		       (isInput ? kAudioUnitScope_Output : kAudioUnitScope_Input), isInput,
		       &format, &psize);
  ...
  AudioUnitSetProperty(*aunit, kAudioUnitProperty_StreamFormat, 
		       (isInput ? kAudioUnitScope_Output : kAudioUnitScope_Input), isInput, &format, 
  ...
  if(!isInput) { 
    ...
    AudioBufferList *CAInputData = (AudioBufferList*)malloc(sizeof(UInt32) 
    ...
    for (i = 0; i < cdata->inchnls; i++) {
      ...
      CAInputData->mBuffers[i].mDataByteSize = bufframes * sizeof(AudioUnitSampleType);
      CAInputData->mBuffers[i].mData = calloc(bufframes, sizeof(AudioUnitSampleType));
    }
