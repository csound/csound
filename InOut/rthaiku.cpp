...
static int playopen_(CSOUND *csound, const csRtAudioParams *parm)
{
	void** playdata = csound->GetRtPlayUserData(csound);
	 DPRINTF(("devName=%s devNum=%d frag size (smpls)=%d (=%d bytes) buf size=%d\n"
	 "chans=%d fmt=%d rate=%.2f\n",
		parm->devName, parm->devNum, parm->bufSamp_SW, parm->bufSamp_SW*sizeof(MYFLT),
		 parm->bufSamp_HW, parm->nChannels, parm->sampleFormat, parm->sampleRate);)
	// Note that buffer sample size is float, source is MYFLT (double!)
	Generator *gen = new Generator(parm->sampleRate, parm->nChannels,
	 parm->bufSamp_SW*sizeof(float)*parm->nChannels, sizeof(MYFLT));
	*playdata = gen;
	setSigAudio(csound);
	return gen->RunAudio();
}
