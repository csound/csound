...
def buildzip(env, target, source):
    ...
    specificFiles = "SConstruct _CsoundVST.* _loris.* pyrun.* CsoundCOM.dll msvcp70.dll libsndfile.dll portaudio.dll msvcr70.dll csound csound.exe CsoundVST CsoundVST.exe CsoundVST.* soundfonts.dll libpython23.a "
...
if getPlatform() == 'linux':
    ...
    zipDependencies.append(oscEnvironment.SharedLibrary('osc-sock', 
        Split('''
        Opcodes/osc-sock.c
