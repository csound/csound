/*  
    ...
    Copyright (C) 1989, 2001 Mark Dolson, John ffitch
...
static void chartran(MYFLT *, int), shortran(MYFLT *, int),
            longtran(MYFLT *, int), floatran(MYFLT *, int),
            bytetran(MYFLT *, int);
#ifdef never
static void alawtran(MYFLT *, int);
#endif
#ifdef ULAW
static void ulawtran(MYFLT *, int), 
#endif
...
#endif
    if (nfile==NULL) {
      die("Must have an example noise file (-I name)\n");
    }
...
/*     ofd = fopen(outfile, "wb"); */
    ...
    if (Chans > 2) {
      die("dnoise: input MUST be mono or stereo\n");
    }

    /* read noise reference file */

    if ((fp = SAsndgetset(nfile,&pn,&beg_ntime,&input_ndur,&srn,channel))<0) {
      die("dnoise: can't open noise reference file\n");
    }

    if (sr != srn) {
      die("Incompatible sample rates\n");
    }
    ...
    lj *= (long) Chans;
    if (lj > 32767) {
      die("dnoise: M too large\n");
    }
    lj = (long) L + 3 * (long) I;
    lj *= (long) Chans;
    if (lj > 32767) {
      die("dnoise: L too large\n");
    }
    ...
    if ((aWin = (MYFLT *) calloc((size_t)(M+Meven),sizeof(MYFLT))) == NULL) {
      die("dnoise: insufficient memory\n");
    }
...
/* noise reduction: calculate noise reference by taking as many
    ...
    if ((nref = (MYFLT *) calloc((N2+1),sizeof(MYFLT))) == NULL) {
      die("denoise: insufficient memory\n");
    }
    ...
    while (nMin > (long) ibuflen) {
      ...
      if (nread < ibuflen) {
        die("dnoise: begin time is greater than EOF of noise file!");
      }
      nMin -= (long) ibuflen;
    }
    i = (int) nMin;
    ...
    if (k == 0) {
      die("dnoise: not enough samples of noise reference\n");
    }
    fac = th * th / k;
    f = nref;
    for (i = 0; i <= N2; i++, f++)
