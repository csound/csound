...
static DELAYR *delayr_find(CSOUND *csound, MYFLT *ndx) {
  ...
  if (UNLIKELY(n < 1 || n > csound->delayr_stack_depth)) {
    csound->InitError(csound, Str("deltap: delayr index %.0f is out of range"),
                      (double)*ndx);
    return NULL;
  }
  /* find delay line */
...
int32_t delay(CSOUND *csound, DELAY *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("delay: not initialised"));
}
...
int32_t delayr(CSOUND *csound, DELAYR *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("delayr: not initialised"));
}
...
int32_t delayw(CSOUND *csound, DELAYW *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("delayw: not initialised"));
}
...
int32_t deltap(CSOUND *csound, DELTAP *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("deltap: not initialised"));
}
...
int32_t deltapi(CSOUND *csound, DELTAP *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("deltapi: not initialised"));
err2:
  return csound->PerfError(csound, &(p->h), Str("deltapi: INF delaytime"));
}
...
int32_t deltapn(CSOUND *csound, DELTAP *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("deltapn: not initialised"));
}
...
int32_t deltap3(CSOUND *csound, DELTAP *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("deltap3: not initialised"));
err2:
  return csound->PerfError(csound, &(p->h), Str("deltapi: INF delaytime"));
}
...
int32_t tapxset(CSOUND *csound, DELTAPX *p) {
  ...
  if (UNLIKELY(p->wsize < 4))
    p->wsize = 4;
  if (UNLIKELY(p->wsize > 1024))
    p->wsize = 1024;
  ...
  if (p->wsize != 4) { /* window size >= 8 */
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (x1 > 0.00000001 && x1 < 0.99999999) {
        ...
        do {
          w = 1.0 - d * d * d2x;
          if (UNLIKELY(++bufp >= bufend))
            bufp = buf1;
          n1 += w * w * (double)*bufp / d;
          d++;
          w = 1.0 - d * d * d2x;
          if (UNLIKELY(++bufp >= bufend))
            bufp = buf1;
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      a0 = am1 * am1;
      a2 = 0.16666667 * (am1 * a0 - am1); /* sample +2 */
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      if (x1 > 0.00000001 && x1 < 0.99999999) {
        ...
        do {
          ...
          if (UNLIKELY(++bufp >= bufend))
            bufp = buf1;
          *bufp = (MYFLT)((double)*bufp + w * w * n1 / d);
          d++;
          w = 1.0 - d * d * d2x;
          if (UNLIKELY(++bufp >= bufend))
            bufp = buf1;
...
int32_t comb(CSOUND *csound, COMB *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("comb: not initialised"));
}
...
int32_t invcomb(CSOUND *csound, COMB *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("combinv: not initialised"));
}
...
int32_t alpass(CSOUND *csound, COMB *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("alpass: not initialised"));
}
...
int32_t reverb(CSOUND *csound, REVERB *p) {
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    *p6++ = z = c6 * y2 + y1;
    ar[n] = y2 - c6 * z;
    if (UNLIKELY(p1 >= p->adr2))
      p1 = p->adr1;
    if (UNLIKELY(p2 >= p->adr3))
      p2 = p->adr2;
    if (UNLIKELY(p3 >= p->adr4))
      p3 = p->adr3;
    if (UNLIKELY(p4 >= p->adr5))
      p4 = p->adr4;
    if (UNLIKELY(p5 >= p->adr6))
      p5 = p->adr5;
    if (UNLIKELY(p6 >= endp))
      p6 = p->adr6;
  }
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("reverb: not initialised"));
}
...
int32_t pan(CSOUND *csound, PAN *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("pan: not initialised"));
}
