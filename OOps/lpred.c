...
static int32_t findzeros(int32_t M, MYFLT *a, MYCMPLX *zero, MYFLT *tmpbuf,
                         int32_t itmax) {
...
int32_t lpfil_perf(CSOUND *csound, LPCFIL *p) {
  ...
  if (*p->kflag) {
    ...
    if (p->win) {
      ...
      c = csound->LPred(csound, p->setup, p->ft->ftable + off);
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    if (--cp == 0) {
      ...
      for (i = 0; i < N + 2; i += 2) {
        ...
        int bin = i / 2;
        if (i > 0 && i < N)
          inv = sqrt(fftframe[i] * fftframe[i] +
                     fftframe[i + 1] * fftframe[i + 1]);
        else if (i == N)
          inv = fftframe[1];
        else
          inv = fftframe[0];
...
int32_t resonbnk_init(CSOUND *csound, RESONB *p) {
  ...
  siz = (p->ord + 1) / 2;
  if (!*p->istor &&
      (p->y1m.auxp == NULL || (uint32_t)(siz * sizeof(double)) > p->y1m.size))
    csound->AuxAlloc(csound, (int32_t)(siz * sizeof(double)), &p->y1m);
  if (!*p->istor &&
      (p->y2m.auxp == NULL || (uint32_t)(siz * sizeof(double)) > p->y2m.size))
    csound->AuxAlloc(csound, (int32_t)(siz * sizeof(double)), &p->y2m);

  if (!*p->istor &&
      (p->y1o.auxp == NULL || (uint32_t)(siz * sizeof(double)) > p->y1o.size))
    csound->AuxAlloc(csound, (int32_t)(siz * sizeof(double)), &p->y1o);
  if (!*p->istor &&
      (p->y2o.auxp == NULL || (uint32_t)(siz * sizeof(double)) > p->y2o.size))
    csound->AuxAlloc(csound, (int32_t)(siz * sizeof(double)), &p->y2o);

  if (!*p->istor &&
      (p->y1c.auxp == NULL || (uint32_t)(siz * sizeof(double)) > p->y1c.size))
    csound->AuxAlloc(csound, (int32_t)(siz * sizeof(double)), &p->y1c);
  if (!*p->istor &&
      (p->y2c.auxp == NULL || (uint32_t)(siz * sizeof(double)) > p->y2c.size))
...
int32_t resonbnk(CSOUND *csound, RESONB *p) {
  ...
  MYFLT bw, cf;
  MYFLT kcnt = p->kcnt, prd = *p->iprd, interp, fmin = *p->kmin,
        fmax = *p->kmax;
