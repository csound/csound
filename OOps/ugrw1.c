...
/* void sprints_local(char *outstring, char *fmt, MYFLT **kvals, int32_t
 * numVals) */
/* { */
/*     char strseg[8192]; */
/*     int32_t i = 0, j = 0; */
 ...
 */
static int32_t sprints(char *outstring, char *fmt, MYFLT **kvals,
                       int32_t numVals) {
  char tmp[8], cc;
  int32_t j = 0;
  int32_t len = 8192;
  ...
  if (p->printat <= CS_KCNT - 1) {
    ...
    if (sprints(string, p->txtstring, p->kvals, p->INOCOUNT - 2) != OK)
      return csound->PerfError(
          csound, &(p->h), Str("Insufficient arguments in formatted printing"));
...
int32_t printsset(CSOUND *csound, PRINTS *p) {
  ...
  if (sprints(string, pk.txtstring, p->kvals, p->INOCOUNT - 1) != OK)
    return csound->InitError(
        csound, Str("Insufficient arguments in formatted printing"));
  csound->MessageS(csound, CSOUNDMSG_ORCH, "%s", string);
  return OK;
}
...
int32_t printsset_S(CSOUND *csound, PRINTS *p) {
  ...
  if (strlen(pk.txtstring) < 8191) {
    ...
    csound->Warning(csound, Str("Formatting string too long: %s"),
                    pk.txtstring);
  }
  return OK;
}
...
int32_t inz(CSOUND *csound, IOZ *p) {
  ...
  if (UNLIKELY(indx + nchns >= zalast))
    ...
    for (i = 0; i < nchns; i++)
      for (n = 0; n < nsmps; n++)
        *writeloc++ =
            ((n >= offset && n < early) ? CS_SPIN[i * nsmps + n] : FL(0.0));
  }
  return OK;
err1:
...
err2:
  return csound->PerfError(csound, &(p->h), Str("inz index < 0. Not writing."));
}
...
err2:
  return csound->PerfError(csound, &(p->h), Str("outz index < 0. No output."));
}
