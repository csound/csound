...
int32_t iout_on_dur(CSOUND *csound, OUT_ON_DUR *p) {
  if (!(p->fl_expired)) {
    MYFLT actual_dur = (MYFLT)CS_KCNT * CS_ONEDKR - p->istart_time;
    MYFLT dur = *p->idur;
    if (dur < actual_dur) {
      p->fl_expired = TRUE;
...
int32_t moscil(CSOUND *csound, MOSCIL *p) {
  ...
  if (!(p->fl_note_expired)) {
    ...
    } else if (p->last_dur < (MYFLT)CS_KCNT * CS_ONEDKR - p->istart_time) {
      p->fl_note_expired = TRUE;
      note_off(csound, p->last_chn, p->last_num, p->last_vel);
    }
  } else {
    ...
    first_note : {
      int32_t temp;
      p->last_chn =
          (temp = abs((int32_t)*p->kchn - 1)) < NUMCHN ? temp : NUMCHN - 1;
  ...
  */
  ...
  if (p->fl_note_expired)
    ...
    if (p->h.insdshead->relesing) {
      ...
      int32_t curr_chn =
          (tt = abs((int32_t)*p->kchn - 1)) <= NUMCHN ? tt : NUMCHN;
      int32_t curr_num = (tt = abs((int32_t)*p->knum)) < 128 ? tt : 127;
      int32_t curr_vel = (tt = abs((int32_t)*p->kvel)) < 128 ? tt : 127;
