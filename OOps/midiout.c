...
void poly_after_touch(CSOUND *, int32_t chan, int32_t note_num, int32_t value,
                      int32_t port);
void csoundSendMidiMsg(CSOUND *, int32_t status, int32_t data1, int32_t data2,
                       int32_t port);
...
int32_t mclock(CSOUND *csound, MCLOCK *p)
{
  if (UNLIKELY(p->beginning_flag)) {    /* first time */
    csoundSendMidiMsg(csound, 0xF8, 0, 0, (int32_t)*p->port); /* clock message */
    p->beginning_flag=FALSE;
    return OK;
  }
  else if ((cs_float) CS_KCNT > p->clock_tics) {
    csoundSendMidiMsg(csound, 0xF8, 0, 0, (int32_t)*p->port);    /* clock message */
    p->clock_tics += p->period;
  }
  return OK;
...
int32_t mrtmsg(CSOUND *csound, MRT *p)
{
  ...
  case 0:
    csoundSendMidiMsg(csound, 0xFC, 0, 0, (int32_t)*p->port); /* stop */
    break;
  case 1:
    csoundSendMidiMsg(csound, 0xFA, 0, 0, (int32_t)*p->port); /* start */
    break;
  case 2:
    csoundSendMidiMsg(csound, 0xFB, 0, 0, (int32_t)*p->port); /* continue */
    break;
  case -1:
    csoundSendMidiMsg(csound, 0xFF, 0, 0, (int32_t)*p->port); /* system_reset */
    break;
  case -2:
...
int32_t iout_on_dur_set(CSOUND *csound, OUT_ON_DUR *p)
{
  ...
  note_on(csound, p->chn,p->num, p->vel,
          (int32_t)*p->port);
  p->istart_time = (cs_float)CS_KCNT * CS_ONEDKR;
  p->fl_expired = FALSE;
...
{
  if (!(p->fl_expired)) {
    cs_float actual_dur = (cs_float) CS_KCNT * CS_ONEDKR
      - p->istart_time;
    cs_float dur = *p->idur;
    if (dur < actual_dur) {
      p->fl_expired = TRUE;
...
int32_t moscil(CSOUND *csound, MOSCIL *p)
{
  ...
  if (!(p->fl_note_expired)) {
    ...
    else if (p->last_dur < (cs_float)CS_KCNT * CS_ONEDKR
             - p->istart_time) {
      p->fl_note_expired = TRUE;
      note_off(csound, p->last_chn, p->last_num, p->last_vel,
               (int32_t)*p->port);
    ...
    if (!p->fl_end_note
        ...
        (cs_float)CS_KCNT * CS_ONEDKR - p->istart_time
...
int32_t midiout(CSOUND *csound, MIDIOUT *p)         /*gab-A6 fixed*/
{
  ...
  csoundSendMidiMsg(csound, st | ch, d1, d2,
                    (int32_t)*p->port);
  return OK;
}
...
int32_t nrpn(CSOUND *csound, NRPN *p)
{
  ...
  if (chan != p->old_chan || parm != p->old_parm || value != p->old_value) {
    ...
    csoundSendMidiMsg(csound, status, 99, parm_msb,
                      (int32_t)*p->port);
    csoundSendMidiMsg(csound, status, 98, parm_lsb,
                      (int32_t)*p->port);
    csoundSendMidiMsg(csound, status,  6, value_msb,
                      (int32_t)*p->port);
    csoundSendMidiMsg(csound, status, 38, value_lsb,
                      (int32_t)*p->port);
    p->old_chan = chan;
    p->old_parm = parm;
    p->old_value = value;
...
int32_t mdelay(CSOUND *csound, MDELAY *p)                   /*gab-A6 fixed*/
{
  ...
  cs_float present_time =  CS_KCNT * CS_ONEDKR;
  ...
  if (p->status[read_index] &&
    ...
    csoundSendMidiMsg(csound,  p->status[read_index] | p->chan[read_index],
