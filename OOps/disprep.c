...
int32_t printv(CSOUND *csound, PRINTV *p) {
  ...
  while (nargs--) {
    csound->MessageS(csound, CSOUNDMSG_ORCH, "  %s = %5.3f", *txtp++, **valp++);
  }
  csound->MessageS(csound, CSOUNDMSG_ORCH, "\n");
  return OK;
}
...
int32_t kdsplay(CSOUND *csound, DSPLAY *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("display: not initialised"));
}
...
static void
ApplyHalfWin(MYFLT *buf, MYFLT *win,
             int32_t len) { /* Window only store 1st half, is symmetric */
  int32_t j;
...
int32_t fftset(CSOUND *csound,
               DSPFFT *p) /* fftset, dspfft -- calc Fast Fourier */
                          ...
                          /* displays coefficients (mag or db)   */
{
  ...
  if (UNLIKELY(window_size < WINDMIN)) {
    return csound->InitError(
        csound, Str("too few points requested (%d), minimum is %d"),
        window_size, WINDMIN);
  }
  ...
  if ((maxbin - minbin) != p->npts || minbin != p->start ||
    ...
    FillHalfWin(hWin, window_size, FL(1.0),
                hanning); /* fill with proper values */
    if (csound->disprep_fftcoefs == NULL) {
      /* room for WINDMAX*2 floats (fft size) */
      csound->disprep_fftcoefs =
          (MYFLT *)csound->Malloc(csound, WINDMAX * 2 * sizeof(MYFLT));
    }
    ...
    p->start = minbin;
    dispset(csound, &p->dwindow, csound->disprep_fftcoefs + p->start, p->npts,
            strmsg, (int32_t)*p->iwtflg, Str("dispfft"));
  }
...
d_fft(                            /* perform an FFT as reqd below */
      CSOUND *csound, MYFLT *sce, /* input array - pure packed real */
...
int32_t kdspfft(CSOUND *csound, DSPFFT *p) {
  ...
  if (bufp < p->sampbuf) /* skip any spare samples */
    ...
    *bufp++ = *p->signal;
    if (bufp >= endp) { /* when full, do fft:     */
      ...
      d_fft(csound, p->sampbuf, csound->disprep_fftcoefs, p->windsize, hWin,
            p->dbout, p->overN);
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("dispfft: not initialised"));
}
...
int32_t tempeset(CSOUND *csound, TEMPEST *p) {
  ...
  if (p->dtimcnt && !(p->dwindow.windid)) { /* init to display stmem & exp */
    snprintf(strmsg, 256,
             "instr %d tempest:", (int32_t)p->h.insdshead->p1.value);
  ...
  }
  {
    ...
    for (fltp = p->hbeg, nn = npts * 4; nn--;) /* clr 2 circ & 1st 2 lin bufs */
      *fltp++ = FL(0.0);
    for (fltp = p->ftable + npts, nn = npts;
         nn--;) {                          /* now sample the ftable  */
  ...
  p->fwdmask = FL(0.0);
#ifdef DEBUG
  csound->Message(csound, Str("kin lopass coef1 %6.4f, fwd mask coef1 %6.4f\n"),
                  p->coef1, p->fwdcoef);
#endif
...
static const MYFLT lenmults[NMULTS] = {FL(3.0), FL(2.0), FL(1.0), FL(0.5),
                                       FL(0.333)};

static const MYFLT lenfracs[NMULTS * 2] = {
    FL(0.30), FL(0.3667), FL(0.45), FL(0.55), FL(0.92),
    FL(1.08), FL(1.88),   FL(2.12), FL(2.85), FL(3.15)};
...
int32_t tempest(CSOUND *csound, TEMPEST *p) {
  ...
  if (!(--p->countdown)) { /* then on countdown:    */
    ...
    *xcur++ = FL(0.0);          /*    & clear the loc it occupied */
    if (xcur >= p->xend)
      xcur = p->xbeg; /* xcur now points to cur xarray  */
    p->xcur = xcur;
#ifdef DEBUG
    ...
    { /* ignore input below lopass */
      ...
      *hcur++ = kin + expect * p->xfdbak; /* join insample & expect val */
      if (hcur < hend)
        p->hcur = hcur; /* stor pntr for next insamp  */
      else
        p->hcur = p->hbeg;
      wrap = hcur - p->hbeg;
      memp = p->stmemp;
    ...
    if (weightot) { /* if accumed weights, */
      p->avglam =
          (p->avglam + lamtot / weightot) / FL(2.0); /* update the avglam */
      p->avglam /= p->tweek;
      p->tempo = p->tempscal / p->avglam; /*   & cvt to tempo    */
#ifdef DEBUG
      csound->Message(csound,
                      "lamtot %6.2f, weightot %6.2f, "
  ...
  *p->kout = p->tempo; /* put current tempo */
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("tempest: not initialised"));
}
