...
static CS_NOINLINE int32_t PVS_CreateWindow(CSOUND *csound, MYFLT *buf,
  ...
  case 2: /* Kaiser */
  {
    ...
    x = (even ? 0.5 : 0.05);
    for (i = 0; i < n; i++, x += 1.0)
      buf[i] = (MYFLT)(besseli(beta * sqrt(1.0 - x * x * flen2)) * besbeta);
    buf[i] = FL(0.0);
  }
    return OK;
  default:
...
int32_t pvssanalset(CSOUND *csound, PVSANAL *p) {
  ...
  if (p->fsig->frame.auxp == NULL ||
    ...
    csound->AuxAlloc(csound, CS_KSMPS * (N + 2) * sizeof(MYFLT),
                     &p->fsig->frame);
  else
    memset(p->fsig->frame.auxp, 0, CS_KSMPS * (N + 2) * sizeof(MYFLT));
  /* Space for remembering samples */
  if (p->input.auxp == NULL || N * sizeof(MYFLT) > (uint32_t)p->input.size)
    csound->AuxAlloc(csound, N * sizeof(MYFLT), &p->input);
  else
    memset(p->input.auxp, 0, N * sizeof(MYFLT));
  ...
  if (p->analwinbuf.auxp == NULL ||
    ...
    memset(p->analwinbuf.auxp, 0, NB * sizeof(CMPLX));
...
int32_t pvsanalset(CSOUND *csound, PVSANAL *p) {
  ...
  N = N + N % 2; /* Make N even */
  if (UNLIKELY(M < N)) {
    csound->Warning(csound, Str("pvsanal: window size too small for fftsize"));
    M = N;
  }
  ...
  if (M > N) {
    ...
    for (i = 1; i <= halfwinsize; i++)
      *(analwinhalf + i) *=
          (MYFLT)(dN * sin((double)(PI * (i + 0.5 * Mf) / dN)) /
                  (PI * (i + 0.5 * Mf)));
    for (i = 1; i <= halfwinsize; i++)
      *(analwinhalf - i) = *(analwinhalf + i - Mf);
  }
  ...
  p->nI = -((int64_t)(halfwinsize / overlap)) *
...
static void generate_frame(CSOUND *csound, PVSANAL *p) {
  ...
  if (format == PVS_AMP_PHASE) {
    ...
    for (i = 0, i0 = anal, i1 = anal + 1, oi = p->oldInPhase; i <= N2;
         i++, i0 += 2, i1 += 2, oi++) {
      real = *i0;
      imag = *i1;
      *i0 = HYPOT(real, imag);
  ...
  for (i = ii = 0 /*,i0=anal,i1=anal+1,oi=oldInPhase*/; i <= N2;
       i++, ii += 2 /*i0+=2,i1+=2, oi++*/) {
    real = anal[ii] /* *i0 */;
    imag = anal[ii + 1] /* *i1 */;
...
int32_t pvssanal(CSOUND *csound, PVSANAL *p) {
  ...
  for (i = offset; i < nsmps; i++) {
    ...
    case PVS_WIN_BLACKMAN_EXACT:
      ...
      for (j = 1; j < NB - 1; j++) {
        ff[j].re -= FL(0.49656061908856405847) * FL(0.5) *
                    (fw[j + 1].re + fw[j - 1].re);
        ff[j].im -= FL(0.49656061908856405847) * FL(0.5) *
                    (fw[j + 1].im + fw[j - 1].im);
      }
      for (j = 2; j < NB - 2; j++) {
        ff[j].re += FL(0.076848667239896818573) * FL(0.5) *
                    (fw[j + 2].re + fw[j - 2].re);
        ff[j].im += FL(0.076848667239896818573) * FL(0.5) *
                    (fw[j + 2].im + fw[j - 2].im);
      }
    ...
    case PVS_WIN_BHARRIS_3:
      ...
      ff[NB - 1].re +=
          -FL(0.49364) * fw[NB - 2].re + FL(0.05677) * fw[NB - 3].re;
      ff[1].re += -FL(0.49364) * fw[2].re + FL(0.05677) * fw[3].re;
      ff[NB - 2].re +=
          -FL(0.49364) * fw[NB - 3].re + FL(0.05677) * fw[NB - 4].re;
      ff[1].re = 0.5 * (fw[2].re + fw[0].re); /* HACK???? */
      ff[1].im = 0.5 * (fw[2].im + fw[0].im);
      break;
    case PVS_WIN_BHARRIS_MIN:
      ...
      ff[NB - 1].re +=
          -FL(0.4973406) * fw[NB - 2].re + FL(0.0782793) * fw[NB - 3].re;
      ff[1].re += -FL(0.4973406) * fw[2].re + FL(0.0782793) * fw[3].re;
      ff[NB - 2].re +=
          -FL(0.4973406) * fw[NB - 3].re + FL(0.0782793) * fw[NB - 4].re;
      ff[1].re = 0.5 * (fw[2].re + fw[0].re); /* HACK???? */
      ff[1].im = 0.5 * (fw[2].im + fw[0].im);
      break;
    }
...
int32_t pvsynthset(CSOUND *csound, PVSYNTH *p) {
  ...
  if (M <= N) {
    ...
    for (i = 1; i <= halfwinsize; i++)
      *(synwinhalf + i) *=
          (MYFLT)((double)IO * sin((double)(PI * (i + 0.5 * Lf) / IO)) /
                  (PI * (i + 0.5 * (double)Lf)));
    for (i = 1; i <= halfwinsize; i++)
      *(synwinhalf - i) = *(synwinhalf + i - Lf);
  }
...
#ifdef NOTDEF
  if (format == PVS_AMP_PHASE) {
    for (ii = 0 /*, i0=syn, i1=syn+1*/; ii <= NO2;
         ii += 2 /* i++, i0+=2,  i1+=2*/) {
      mag = syn[ii];       /* *i0; */
      phase = syn[ii + 1]; /* *i1; */
    ...
    for (i = ii = 0 /*, i0=syn, i1=syn+1*/; i <= NO2;
      ...
      angledif =
          p->TwoPioverR * (/* *i1 */ syn[ii + 1] - ((MYFLT)i * p->Fexact));
