...
int32_t phsor(CSOUND *csound, PHSOR *p) {
  ...
  if (IS_ASIG_ARG(p->xcps)) {
    ...
    for (n = offset; n < nsmps; n++) {
      ...
      rs[n] = (MYFLT)phase;
      phase += incr;
      if (UNLIKELY((MYFLT)phase >=
                   FL(1.0))) /* VL convert to MYFLT
...
int32_t itable(CSOUND *csound, TABLE *p) {
  if (LIKELY(itblchk(csound, p) == OK))
    return ktable(csound, p);
  return NOTOK;
}

int32_t itabli(CSOUND *csound, TABLE *p) {
  if (LIKELY(itblchk(csound, p) == OK))
    return ktabli(csound, p);
  return NOTOK;
}

int32_t itabl3(CSOUND *csound, TABLE *p) {
  if (LIKELY(itblchk(csound, p) == OK))
    return ktabl3(csound, p);
  return NOTOK;
}
 ...
 *
 * They are called directly via their entries in engineState.opcodlst, and also
 * by two functions which call them after they have coped with any change in the
 * k rate function table number.
 *
 * tablekt() and tablikt().
 *
 * */
...
int32_t tablefn(CSOUND *csound, TABLE *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("table: not initialised"));
}
...
int32_t tabli(CSOUND *csound, TABLE *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("tablei: not initialised"));
}
...
int32_t tabl3(CSOUND *csound, TABLE *p) /* Like tabli but cubic interpolation */
{
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("table3: not initialised"));
}
...
err1:
  return csound->PerfError(csound, &(p->h),
                           Str("k rate function table no. %f < 1"), *p->xfn);
err2:
  return csound->PerfError(csound, &(p->h),
                           Str("Offset %f < 0 or > tablelength"), p->offset);
}
...
static int32_t fill_func_from_array(ARRAYDAT *a, FUNC *f) {
  ...
  f->ftable = a->data;
  f->lenmask =
      ((flen & (flen - 1L)) ? 0L : (flen - 1L)); /*  init hdr w powof2 data  */
  f->lobits = lobits;
  i = (1 << lobits);
...
int32_t oscsetA(CSOUND *csound, OSC *p) {
  ...
  if (LIKELY((x != 0) && !(x & (x - 1)))) {
    ...
    return csound->InitError(csound, Str("array size not pow-of-two\n"));
}
...
int32_t osckk(CSOUND *csound, OSC *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("oscil: not initialised"));
}
...
int32_t osckki(CSOUND *csound, OSC *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("oscili: not initialised"));
}
...
int32_t koscl3(CSOUND *csound, OSC *p) {
  ...
  y0 = ftab[x0++];
  y1 = ftab[x0++];
  if (UNLIKELY(x0 > (int32_t)ftp->flen))
    y2 = ftab[1];
  else
    y2 = ftab[x0];
  {
...
int32_t osckk3(CSOUND *csound, OSC *p) {
  ...
  return OK;
err1:
  return csound->PerfError(csound, &(p->h), Str("oscil3: not initialised"));
}
