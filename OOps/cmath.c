...
int32_t seedrand(CSOUND *csound, PRAND *p) {
  ...
  int32_t *holdrand =
      (int32_t *)csound->QueryGlobalVariable(csound, "::HOLDRAND::");
...
static MYFLT exprand(CSOUND *csound, MYFLT lambda) {
  ...
  if (UNLIKELY(lambda < FL(0.0)))
    return (FL(0.0)); /* for safety */
...
static MYFLT pcauchrand(CSOUND *csound, MYFLT a) {
  ...
  x = TAN((MYFLT)r1 * HALFPI_F / FL(4294967295.0)) * (FL(1.0) / FL(318.3));
  return (x * a);
}
...
int kexprndi(CSOUND *csound, PRANDI *p) { /* rslt = (num1 + diff*phs) * amp */
  /* IV - Jul 11 2002 */
...
int32_t kgaussi(CSOUND *csound,
                PRANDI *p) { /* rslt = (num1 + diff*phs) * amp */
  /* IV - Jul 11 2002 */
  *p->ar = (p->num1 + (MYFLT)p->phs * p->dfdmax) * *p->xamp;
  p->phs += (int32_t)(*p->xcps * CS_KICVT); /* phs += inc           */
  if (UNLIKELY(p->phs >= MAXLEN)) {         /* when phs overflows,  */
...
int32_t kcauchyi(CSOUND *csound,
                 PRANDI *p) { /* rslt = (num1 + diff*phs) * amp */
  /* IV - Jul 11 2002 */
  *p->ar = (p->num1 + (MYFLT)p->phs * p->dfdmax) * *p->xamp;
  p->phs += (int32_t)(*p->xcps * CS_KICVT); /* phs += inc           */
