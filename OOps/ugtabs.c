...
int32_t tabler_init(CSOUND *csound, TABL *p) {
  ...
  if (UNLIKELY((p->ftp = csound->FTnp2Find(csound, p->ftable)) == NULL))
    return csound->InitError(csound, Str("table: could not find ftable %d"),
                             (int32_t)*p->ftable);
  len = p->ftp->flen;
  mask = p->ftp->lenmask;
    ...
    if (UNLIKELY(IS_ASIG_ARG(p->ndx) != IS_ASIG_ARG(p->sig))) {
      if (CS_KSMPS != 1)
        return csound->InitError(
            csound, Str("table: index type inconsistent with output"));
    }
  }
  if (UNLIKELY((p->ftp = csound->FTnp2Find(csound, p->ftable)) == NULL))
    return csound->InitError(csound, Str("table: could not find ftable %d"),
...
int32_t tabler_audio(CSOUND *csound, TABL *p) {
  ...
  if (UNLIKELY(koffset))
    memset(sig, '\0', koffset * sizeof(MYFLT));
  if (UNLIKELY(early)) {
    nsmps -= early;
...
int32_t table3r_init(CSOUND *csound, TABL *p) {
  ...
  if (UNLIKELY(ndx < 1 || ndx == len - 1 || len < 4)) {
    ...
    *p->sig = x1 + FL(0.5) * fracub +
              ...
              frac * fracsq * (temp1 / FL(6.0) - FL(0.5) * x2) +
              fracsq * (FL(0.5) * x2 - x1);
  }
  return OK;
}
  ...
  for (n = koffset; n < nsmps; n++) {
    ...
    if (UNLIKELY(ndx < 1 || ndx == len - 1 || len < 4)) {
      ...
      p->sig[n] =
          x1 + FL(0.5) * fracub +
          frac * (x2 - fracub / FL(6.0) - temp1 / FL(6.0) - x0 / FL(3.0)) +
          fracsq * frac * (temp1 / FL(6.0) - FL(0.5) * x2) +
          fracsq * (FL(0.5) * x2 - x1);
    }
  }
  return OK;
...
int32_t tablkt_setup(CSOUND *csound, TABL *p) {
  ...
  if (UNLIKELY(IS_ASIG_ARG(p->ndx) != IS_ASIG_ARG(p->sig))) {
    if (CS_KSMPS != 1)
      return csound->InitError(
          csound, Str("tablekt: index type inconsistent with output"));
  }
...
int32_t table_copy(CSOUND *csound, TGP *p) {
  ...
  if (UNLIKELY((dest = csound->FTnp2Find(csound, p->ftable)) == NULL ||
    ...
    csound->Warning(csound, Str("table: could not find ftables %d and/or %d"),
