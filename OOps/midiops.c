...
extern int32_t m_chinsno(CSOUND *csound, int32_t chan, int32_t insno,
                         int32_t reset_ctls);
...
int32_t octmidib(CSOUND *csound, MIDIKMB *p) {
  IGN(csound);
  INSDS *lcurip = p->h.insdshead;
  *p->r = (lcurip->m_pitch + (pitchbend_value(lcurip->m_chnbp) * p->scale)) /
              FL(12.0) +
          FL(3.0);
  return OK;
}
...
int32_t cpsmidi(CSOUND *csound, MIDIKMB *p) {
   ...
   *       pitchbend_value(lcurip->m_chnbp) * p->iscal)/ 12.0f + 3.0f) *
   * OCTRES);
   */
...
int32_t icpsmidib(CSOUND *csound, MIDIKMB *p) {
  ...
  p->prvbend = bend;
  loct = (int32_t)(((lcurip->m_pitch + bend * p->scale) / FL(12.0) + FL(3.0)) *
                   OCTRES);
  *p->r = CPSOCTL(loct);
  return OK;
}
...
int32_t kcpsmidib(CSOUND *csound, MIDIKMB *p) {
  ...
  if (bend == p->prvbend || lcurip->relesing)
    *p->r = p->prvout;
    ...
    p->prvbend = bend;
    loct =
        (int32_t)(((lcurip->m_pitch + bend * p->scale) / FL(12.0) + FL(3.0)) *
                  OCTRES);
...
int32_t ampmidi(CSOUND *csound,
                MIDIAMP *p) /* convert midi veloc to amplitude */
...
int32_t imidictl(CSOUND *csound, MIDICTL *p) {
  ...
  if (UNLIKELY((ctlno = (int32_t)*p->ictlno) < 0 || ctlno > 127))
    ...
    *p->r = MIDI_VALUE(csound->curip->m_chnbp, ctl_val[ctlno]) *
                (*p->ihi - *p->ilo) * dv127 +
            *p->ilo;
  return OK;
}
...
int32_t imidiaft(CSOUND *csound, MIDICTL *p) {
  ...
  if (UNLIKELY((ctlno = (int32_t)*p->ictlno) < 0 || ctlno > 127))
    ...
    *p->r = MIDI_VALUE(csound->curip->m_chnbp, polyaft[ctlno]) *
                (*p->ihi - *p->ilo) * dv127 +
            *p->ilo;
  return OK;
}
  ...
  if (*(p->ipgm) < FL(0.5)) { /* program <= 0: assign all pgms */
    ...
    if (UNLIKELY(pgm < 0 || pgm > 127)) {
      return csound->InitError(csound,
                               Str("pgmassign: invalid program number"));
    }
    if (!chn) { /* on all channels */
      for (chn = 0; chn < 16; chn++)
        csound->m_chnbp[chn]->pgm2ins[pgm] = ins;
...
int32_t ichanctl(CSOUND *csound, CHANCTL *p) {
  ...
  if (UNLIKELY((ctlno = (int32_t)*p->ictlno) < 0 || ctlno > 127))
    ...
    *p->r =
        csound->m_chnbp[chan]->ctl_val[ctlno] * (*p->ihi - *p->ilo) * dv127 +
        *p->ilo;
  return OK;
}
...
int32_t ipchbend(CSOUND *csound, MIDIMAP *p) {
  IGN(csound);
  *p->r =
      *p->ilo + (*p->ihi - *p->ilo) * pitchbend_value(p->h.insdshead->m_chnbp);
  return OK;
}
...
int32_t ctlin(CSOUND *csound, CTLIN *p) {
  ...
  if (p->local_buf_index != MGLOB(MIDIINbufIndex)) {
    ...
    d1 = *++temp;
    d2 = *++temp;
    if (st == 0xB0 && (p->watch1 == 0 || p->watch1 == ch) &&
...
int32_t midiarp(CSOUND *csound, MIDIARP *p) {
  ...
  if (p->local_buf_index != MGLOB(MIDIINbufIndex)) {
    ...
    if (p->status == 144 && p->data2 > 0) {
      ...
      p->noteCnt = (p->noteCnt > p->maxNumNotes - 1 ? p->maxNumNotes - 1
                                                    : p->noteCnt + 1);
    ...
    *p->counter = metroCounter(p);
    if (*p->counter == 1) {
      ...
      if (arpmode == 0) {
        ...
        if (p->direction > 0) {
          p->noteIndex =
              (p->noteIndex < p->maxNumNotes - 1 ? p->noteIndex + 1
                                                 : p->maxNumNotes - p->noteCnt);
        ...
        // down only pattern
        p->noteIndex =
            (p->noteIndex > p->maxNumNotes - p->noteCnt ? p->noteIndex - 1
                                                        : p->maxNumNotes - 1);
      } else if (arpmode == 3) {
        // random pattern
...
int presetctrl_perf(CSOUND *csound, PRESETCTRL *p) {
  ...
  if (tag >= q->max_num) {
    ...
    if (size < 10)
      size = 10;
    tt = (int **)csound->ReAlloc(csound, tt,
                                 (q->max_num + size) * sizeof(int *));
    if (tt == NULL)
      return csound->InitError(csound, "%s",
...
int presetctrl1_perf(CSOUND *csound, PRESETCTRL1 *p) {
  ...
  if (slot)
    csound->Free(csound, slot);
  q->presets[tag] =
      (int *)csound->Malloc(csound, sizeof(int) * (1 + p->arr->sizes[0]));
  slot = q->presets[tag];
