...
static int32_t getsndinfo(CSOUND *csound, SNDINFO *p, SFLIB_INFO *hdr,
                          int32_t strin) {
  ...
  if (strin)
    ...
    csound->strarg2name(csound, soundiname, p->ifilno, "soundin.", 0);
      ...
      if (f != NULL) {
        ...
        if (n == 1) {
          if (cvdata.magic == (int32_t)CVMAGIC &&
            ...
            hdr->channels =
                (cvdata.channel == (int32_t)ALLCHNLS ? (int32_t)cvdata.src_chnls
                                                     : 1);
      ...
      if (fd >= 0) {
        hdr->frames =
            (sf_count_t)(((int32_t)csound->PVOC_FrameCount(csound, fd) /
                          (int32_t)fmt.nChannels) *
                         (sf_count_t)pvdata.dwOverlap);
...
int32_t filepeak_(CSOUND *csound, SNDINFOPEAK *p, char *soundiname) {
  ...
  if (UNLIKELY(fd == NULL)) {
    /* RWD 5:2001 better to exit in this situation ! */
    return csound->InitError(csound, Str("diskinfo cannot open %s: %s"), sfname,
  ...
  if (channel <= 0) {
    if (sflib_command(sf, SFC_GET_SIGNAL_MAX, &peakVal, sizeof(double)) ==
      ...
      csound->Warning(csound,
                      Str("%s: no PEAK chunk was found, scanning "
                          "file for maximum amplitude"),
                      sfname);
      if (sflib_command(sf, SFC_CALC_NORM_SIGNAL_MAX, &peakVal,
                        sizeof(double)) != 0)
        peakVal = -1.0;
    }
  } else {
    ...
    if (sflib_command(sf, SFC_GET_MAX_ALL_CHANNELS, peaks, nBytes) ==
        SFLIB_FALSE) {
      csound->Warning(csound,
                      Str("%s: no PEAK chunk was found, scanning "
                          "file for maximum amplitude"),
