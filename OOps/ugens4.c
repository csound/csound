...
static inline MYFLT intpow1(MYFLT x,
                            int32_t n) /* Binary positive power function */
{
  MYFLT ans = FL(1.0);
  while (n != 0) {
    if (n & 1)
...
int32_t gbuzz(CSOUND *csound, GBUZZ *p) {
  ...
  if ((r = *p->kr) != p->prvr || nn != p->prvn) {
    ...
    if ((absr = FABS(r)) > FL(0.999) && absr < FL(1.001))
      ...
      p->rsumr = (FL(1.0) - absr) / (FL(1.0) - FABS(p->rtn));
    p->prvr = r;
    p->prvn = (int16)nn;
  }
  scal = *ampp * p->rsumr;
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    num = ftbl[phs * k & lenmask] - r * ftbl[phs * km1 & lenmask] -
  ...
  *ap = *(MYFLT *)auxp; /* last = copy of 1st */
  ...
  case 4: /* rough and stretch factor: 0 <= param1 <= 1, param2 >= 1 */
    ...
    if (UNLIKELY(p->param2 < FL(1.0)))
      ...
      p->thresh2 = (int16)(FL(32768.0) / p->param2);
    break;
  ...
  if (!p->new) {
    ...
    if (!(p->ampcod)) {
      ...
      for (n = offset; n < nsmps; n++) {
        rand = randint31(rand);
        ar[n] =
            base + (MYFLT)((int32_t)((uint32_t)rand << 1) - BIPOLAR) * ampscl;
      }
    } else {
      MYFLT *xamp = p->xamp;
      for (n = offset; n < nsmps; n++) {
        rand = randint31(rand);
        ar[n] = base + dv2_31 *
                           (MYFLT)((int32_t)((uint32_t)rand << 1) - BIPOLAR) *
                           xamp[n];
      }
    }
    p->rand = rand; /* save current rand */
  ...
  if (*p->iseed >= FL(0.0)) {  /* new seed:            */
    if (*p->iseed > FL(1.0)) { /* As manual suggest sseed in range [0,1] */
      ...
      if (ss > FL(1.0))
        ...
        p->rand = (int32_t)(*p->iseed * FL(2147483648.0));
      p->rand = randint31(p->rand);
      p->rand = randint31(p->rand);
      p->num1 = (MYFLT)((int32_t)((uint32_t)p->rand << 1) - BIPOLAR) * dv2_31;
    }
  ...
  if (*p->iseed >= FL(0.0)) {  /* new seed:            */
    if (*p->iseed > FL(1.0)) { /* As manual suggest seed in range [0,1] */
      ...
      if (ss > FL(1.0))
        ...
        p->rand = (int32_t)(FL(0.5) * FL(2147483648.0));
      else
        p->rand = (int32_t)(ss * FL(2147483648.0));
      p->rand = randint31(p->rand);
      p->rand = randint31(p->rand);
      p->num1 = (MYFLT)(p->rand < 1) * dv2_31; /* store num1,2 */
      p->rand = randint31(p->rand);
...
int32_t krandi(CSOUND *csound, RANDI *p) { /* rslt = (num1 + diff*phs) * amp */
  IGN(csound);
...
int32_t krandc(CSOUND *csound, RANDC *p) { /* rslt = (num1 + diff*phs) * amp */
  IGN(csound);
