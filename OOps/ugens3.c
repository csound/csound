...
int32_t losset_phs(CSOUND *csound, LOSCPHS *p) {
  ...
  if ((ftp = csound->FTnp2Finde(csound, p->ifn)) != NULL) {
    ...
    if (p->OUTOCOUNT == 2) {
      ...
      return csound->InitError(csound, Str("loscilphs: insufficient outputs"));
    }
    return OK;
  }
  return NOTOK;
...
static inline void loscil_linear_interp_mono(MYFLT *ar, MYFLT *ftbl, MYFLT phs,
                                             int32_t flen) {
  MYFLT fract, tmp;
  int32_t x;
...
static inline void loscil_cubic_interp_mono(MYFLT *ar, MYFLT *ftbl, MYFLT phs,
                                            int32_t flen) {
...
static CS_NOINLINE void loscil_cubic_interp_stereo(MYFLT *arL, MYFLT *arR,
                                                   MYFLT *ftbl, MYFLT phs,
                                                   int32_t flen) {
...
static int32_t adset_(CSOUND *csound, ADSYN *p, int32_t stringname) {
  ...
  if ((mfp = p->mfp) == NULL || strcmp(mfp->filename, filnam) != 0) {
    /* readfile if reqd */
    if (UNLIKELY((mfp = ldmemfile2withCB(csound, filnam, CSFTYPE_HETRO,
                                         NULL)) == NULL)) {
...
int32_t adsyn(CSOUND *csound, ADSYN *p) {
  ...
  if (UNLIKELY(csound->isintab == NULL)) { /* RWD fix */
    return csound->PerfError(csound, &(p->h), Str("adsyn: not initialised"));
  }
  /* IV - Jul 11 2002 */
