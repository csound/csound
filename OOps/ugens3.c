...
foscil (CSOUND *csound, FOSC *p)
{
  ...
  return OK;
err1:
  return csound->PerfError (csound, &(p->h), Str ("foscil: not initialised"));
}

int32_t
foscili (CSOUND *csound, FOSC *p)
{
  FUNC *ftp;
  ...
  return OK;
err1:
  return csound->PerfError (csound, &(p->h), Str ("foscili: not initialised"));
}

int32_t
losset (CSOUND *csound, LOSC *p)
{
  FUNC *ftp;
  if ((ftp = csound->FTnp2Finde (csound, p->ifn)) != NULL)
    {
      ...
      if ((p->mod1 = (int16)*p->imod1) < 0)
      ...
      else
        {
          ...
          else if (UNLIKELY (p->beg1 < 0 || p->end1 > maxphs
                             || p->beg1 >= p->end1))
            {
      ...
      if ((p->mod2 = (int16)*p->imod2) < 0)
      ...
      else
        {
          p->beg2 = *p->ibeg2;
          p->end2 = *p->iend2;
          if (UNLIKELY (p->mod2 < 0 || p->mod2 > 3 || p->beg2 < 0
                        || p->end2 > (int32_t)maxphs || p->beg2 >= p->end2))
            {
              goto lerr3;
            }
        }
      ...
      if (p->OUTOCOUNT == 1)
        {
          ...
          if (UNLIKELY (ftp->nchanls != 1))
            return csound->InitError (
                csound, Str ("mono loscil cannot read from stereo ftable"));
        }
      else
        {
          p->stereo = 1;
          if (UNLIKELY (ftp->nchanls != 2))
            return csound->InitError (
                csound, Str ("stereo loscil cannot read from mono ftable"));
        }
      return OK;
    }
  return NOTOK;
...
int32_t
losset_phs (CSOUND *csound, LOSCPHS *p)
{
  FUNC *ftp;
  if ((ftp = csound->FTnp2Finde (csound, p->ifn)) != NULL)
    {
      ...
      if (p->OUTOCOUNT == 2)
        {
          ...
          if (UNLIKELY (ftp->nchanls != 1))
            return csound->InitError (
                csound, Str ("mono loscilphs cannot read from stereo ftable"));
        }
      else if (p->OUTOCOUNT == 3)
        {
          ...
          if (UNLIKELY (ftp->nchanls != 2))
            return csound->InitError (
                csound, Str ("stereo loscilphs cannot read from mono ftable"));
        }
      else
        {
          return csound->InitError (csound,
                                    Str ("loscilphs: insufficient outputs"));
        }
      return OK;
    }
  return NOTOK;
...
static inline void
loscil_linear_interp_mono (MYFLT *ar, MYFLT *ftbl, MYFLT phs, int32_t flen)
{
  MYFLT fract, tmp;
  int32_t x;
...
static inline void
loscil_linear_interp_stereo (MYFLT *arL, MYFLT *arR, MYFLT *ftbl, MYFLT phs,
                             int32_t flen)
{
...
static inline void
loscil_cubic_interp_mono (MYFLT *ar, MYFLT *ftbl, MYFLT phs, int32_t flen)
{
...
loscil_cubic_interp_stereo (MYFLT *arL, MYFLT *arR, MYFLT *ftbl, MYFLT phs,
                            int32_t flen)
{
...
int32_t
loscil_phs (CSOUND *csound, LOSCPHS *p)
{
  IGN (csound);
  FUNC *ftp;
...
int32_t
loscil3_phs (CSOUND *csound, LOSCPHS *p)
{
  IGN (csound);
  FUNC *ftp;
  MYFLT *ar1, *ar2, *ftbl, *xamp, *sphs;
...
static int32_t
adset_ (CSOUND *csound, ADSYN *p, int32_t stringname)
{
  int32_t n;
  char filnam[MAXNAME];
  MEMFIL *mfp;
  ...
  if (csound->isintab == NULL)
    { /* if no sin table yet, make one */
      int16 *ip;
      csound->isintab = ip
  ...
  if (stringname)
    strNcpy (filnam, ((STRINGDAT *)p->ifilcod)->data, MAXNAME - 1);
  else if (csound->ISSTRCOD (*p->ifilcod))
    strNcpy (filnam, get_arg_string (csound, *p->ifilcod), MAXNAME - 1);
  else
    csound->strarg2name (csound, filnam, p->ifilcod, "adsyn.", 0);

  if ((mfp = p->mfp) == NULL || strcmp (mfp->filename, filnam) != 0)
    {
      /* readfile if reqd */
      if (UNLIKELY (
              (mfp = ldmemfile2withCB (csound, filnam, CSFTYPE_HETRO, NULL))
              == NULL))
        {
      ...
      if ((val = *adp++) < 0)
        { /* then for each brkpt set,   */
          switch (val)
            ...
            default:
              return csound->InitError (
                  csound, Str ("illegal code %d encountered"), val);
            }
        }
    }
  while (adp < endata);
...
int32_t
adset (CSOUND *csound, ADSYN *p)
{
  return adset_ (csound, p, 0);
}

int32_t
adset_S (CSOUND *csound, ADSYN *p)
{
...
adsyn (CSOUND *csound, ADSYN *p)
{
  ...
  if (UNLIKELY (csound->isintab == NULL))
    { /* RWD fix */
  ...
  while ((prvp = curp) && (curp = curp->nxtp) != NULL)
    {
      ...
      if ((amp = curp->amp))
        { /* for non-zero amp   */
          ...
          for (n = offset; n < nsmps; n++)
            {
              ar[n] += (ampscale * (MYFLT)csound->isintab[phs] * (MYFLT)amp)
                       / ADSYN_MAXLONG;
              phs += sinc;
              phs &= ADMASK;
            }
      ...
      if ((nxtim = (ap + 1)->tim) == 32767)
        {                          /* if last amp this partial */
          prvp->nxtp = curp->nxtp; /*   remov from activ chain */
          curp = prvp;
        }
      else
        { /* else interp towds nxt amp */
          if ((diff = (int16)((ap + 1)->val - amp)))
            {
              ktogo = (int16)(((nxtim << 10) - p->mksecs + timkincr - 1)
                              / timkincr);
              if (ktogo == 0)
                curp->amp += diff;
              else
