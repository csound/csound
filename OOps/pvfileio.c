...
static int pvsys_createFileHandle(CSOUND *csound) {
  ...
  if (i >= csound->pvNumFiles) {
    ...
    if (!csound->pvNumFiles) {
      csound->pvNumFiles = 8;
      tmp = (PVOCFILE **)csound->Malloc(csound, sizeof(PVOCFILE *) *
                                                    csound->pvNumFiles);
    } else {
      csound->pvNumFiles <<= 1;
      tmp = (PVOCFILE **)csound->ReAlloc(
          csound, csound->pvFileTable, sizeof(PVOCFILE *) * csound->pvNumFiles);
    }
    if (tmp == NULL)
      return -1;
...
static void prepare_pvfmt(WAVEFORMATEX *pfmt, uint32 chans, uint32 srate,
                          pv_stype stype) {
...
int32_t pvoc_createfile(CSOUND *csound, const char *filename, uint32 fftlen,
                        uint32 overlap, uint32 chans, uint32 format,
...
int32_t pvoc_openfile(CSOUND *csound, const char *filename, void *data_,
                      void *fmt_) {
  ...
  p->customWindow = NULL;
  p->fd = csound->FileOpen2(csound, &(p->fp), CSFILE_STD, filename, "rb",
                            "SADIR", CSFTYPE_PVCEX, 0);
  if (UNLIKELY(p->fd == NULL)) {
...
static int32_t pvoc_readfmt(CSOUND *csound, PVOCFILE *p,
                            WAVEFORMATPVOCEX *pWfpx) {
...
static int32_t pvoc_readheader(CSOUND *csound, PVOCFILE *p,
  ...
  if (UNLIKELY(pvfile_read_tag(p, &(tag[0])) != 0 || strcmp(tag, "RIFF") != 0 ||
  ...
  while (riffsize > (uint32_t)0) {
    ...
    if (strcmp(tag, "fmt ") == 0) {
      ...
      p->customWindow =
          csound->Malloc(csound, p->pvdata.dwWinlen * sizeof(float));
      if (UNLIKELY(pvoc_readWindow(p, p->customWindow, p->pvdata.dwWinlen) !=
                   0)) {
...
static int32_t pvoc_writeheader(CSOUND *csound, PVOCFILE *p) {
  ...
  size = SIZEOF_WFMTEX + sizeof(uint16_t) + sizeof(uint32_t) + sizeof(GUID) +
         2 * sizeof(uint32_t) + sizeof(PVOCDATA);
  ...
  if (p->customWindow) {
    ...
    if (UNLIKELY(pvoc_writeWindow(p, p->customWindow, p->pvdata.dwWinlen) !=
                 0)) {
...
static int32_t pvoc_updateheader(CSOUND *csound, int32_t ofd) {
  ...
  if (UNLIKELY(fseek(p->fp, (int32_t)(p->datachunkoffset - sizeof(uint32_t)),
                     SEEK_SET) != 0)) {
