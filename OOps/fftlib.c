...
static void fftrecurs(MYFLT *ioptr, int32_t M, MYFLT *Utbl, int32_t Ustride,
  ...
  if (M <= (int32_t)MCACHE)                              /* fits on chip ? */
    ...
    for (i1 = 0; i1 < 8; i1++) {
      fftrecurs(&ioptr[i1 * POW2(M - 3) * 2], M - 3, Utbl, 8 * Ustride, NDiffU,
                StageCnt - 1); /*  RADIX 8 Stages      */
    }
...
static void ifftrecurs(MYFLT *ioptr, int32_t M, MYFLT *Utbl, int32_t Ustride,
  ...
  if (M <= (int32_t)MCACHE)
    ...
    for (i1 = 0; i1 < 8; i1++) {
      ifftrecurs(&ioptr[i1 * POW2(M - 3) * 2], M - 3, Utbl, 8 * Ustride, NDiffU,
                 StageCnt - 1); /*  RADIX 8 Stages       */
    }
...
static void fftInit(CSOUND *csound, int32_t M) {
  ...
  /* init cos table */
  UtblArray[M] =
      (MYFLT *)csound->Malloc(csound, (POW2(M) / 4 + 1) * sizeof(MYFLT));
  ...
  if (M > 2) {
    ...
    if (BRLowArray[(M - 1) / 2] == 0) {
      BRLowArray[(M - 1) / 2] = (int16 *)csound->Malloc(
          csound, POW2((M - 1) / 2 - 1) * sizeof(int16));
...
void csoundRealFFTMult(CSOUND *csound, MYFLT *outbuf, MYFLT *buf1, MYFLT *buf2,
...
/*
  ...
  VL, 2016
*/
static void pffft_execute(CSOUND_FFT_SETUP *setup, MYFLT *sig) {
  int32_t i, N = setup->N;
  float s, *buf;
  buf = (float *)setup->buffer;
  for (i = 0; i < N; i++)
    buf[i] = sig[i];
  pffft_transform_ordered((PFFFT_Setup *)setup->setup, buf, buf, NULL,
                          setup->d);
  s = (setup->d == PFFFT_BACKWARD ? (MYFLT)setup->N : FL(1.0));
  for (i = 0; i < N; i++)
    sig[i] = buf[i] / s;
}
...
static void vDSP_execute(CSOUND_FFT_SETUP *setup, MYFLT *sig) {
#ifdef USE_DOUBLE
...
#ifdef USE_DOUBLE
  vDSP_fft_zripD((FFTSetupD)setup->setup, &tmp, 1, setup->M, setup->d);
#else
  vDSP_fft_zrip((FFTSetup)setup->setup, &tmp, 1, setup->M, setup->d);
#endif
...
static void *align_alloc(CSOUND *csound, size_t nb_bytes) {
  void *p, *p0 = csound->Malloc(csound, nb_bytes + ALIGN_BYTES);
  if (!p0)
    return (void *)0;
  p = (void *)(((size_t)p0 + ALIGN_BYTES) & (~((size_t)(ALIGN_BYTES - 1))));
  *((void **)p - 1) = p0;
  return p;
}
...
void *csoundRealFFT2Setup(CSOUND *csound, int32_t FFTsize, int32_t d) {
  ...
  setup = (CSOUND_FFT_SETUP *)csound->Calloc(csound, sizeof(CSOUND_FFT_SETUP));
  setup->N = FFTsize;
...
#endif
    setup->d = (d == FFT_FWD ? kFFTDirection_Forward : kFFTDirection_Inverse);
    setup->lib = lib;
    break;
#endif
  case PFFT_LIB:
    setup->setup = (void *)pffft_new_setup(FFTsize, PFFFT_REAL);
    setup->d = (d == FFT_FWD ? PFFFT_FORWARD : PFFFT_BACKWARD);
    setup->lib = lib;
    break;
  default:
    setup->lib = 0;
  ...
  csound->RegisterResetCallback(csound, (void *)setup,
                                (int32_t(*)(CSOUND *, void *))setupDispose);
  return (void *)setup;
}

void csoundRealFFT2(CSOUND *csound, void *p, MYFLT *sig) {
  CSOUND_FFT_SETUP *setup = (CSOUND_FFT_SETUP *)p;
  switch (setup->lib) {
#if defined(__MACH__)
  case VDSP_LIB:
  ...
  case PFFT_LIB:
    ...
    break;
  default:
    (setup->d == FFT_FWD ? csoundRealFFT(csound, sig, setup->N)
                         : csoundInverseRealFFT(csound, sig, setup->N));
    setup->lib = 0;
  }
}
...
void *csoundDCTSetup(CSOUND *csound, int32_t FFTsize, int32_t d) {
  CSOUND_FFT_SETUP *setup;
  setup = (CSOUND_FFT_SETUP *)csoundRealFFT2Setup(csound, FFTsize * 4, d);
  if (setup->lib == 0) {
    setup->buffer = (MYFLT *)csound->Calloc(csound, sizeof(MYFLT) * setup->N);
  }
  return setup;
}

void pffft_DCT_execute(CSOUND *csound, void *p, MYFLT *sig) {
  IGN(csound);
  CSOUND_FFT_SETUP *setup = (CSOUND_FFT_SETUP *)p;
  ...
  if (setup->d == FFT_FWD) {
    ...
    pffft_transform_ordered((PFFFT_Setup *)setup->setup, buffer, buffer, NULL,
                            PFFFT_FORWARD);
    for (i = j = 0; i < N / 2; i += 2, j++) {
      sig[j] = buffer[i];
    }
...
#if defined(__MACH__)
void vDSP_DCT_execute(CSOUND *csound, void *p, MYFLT *sig) {
  IGN(csound);
  CSOUND_FFT_SETUP *setup = (CSOUND_FFT_SETUP *)p;
  int32_t i, j, N = setup->N;
#ifdef USE_DOUBLE
...
void DCT_execute(CSOUND *csound, void *p, MYFLT *sig) {
  CSOUND_FFT_SETUP *setup = (CSOUND_FFT_SETUP *)p;
  int32_t i, j, N = setup->N;
