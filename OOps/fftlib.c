...
static
void
vDSP_RealFFT(CSOUND *csound,int
	     FFTsize,MYFLT *sig,FFTDirection d);
#else
static
...
#ifdef HAVE_VECLIB
    vDSP_RealFFT(csound,FFTsize,
		    buf,kFFTDirection_Forward);
#else
    pffft_RealFFT(csound,FFTsize,
		  buf, PFFFT_FORWARD);
#endif
}
