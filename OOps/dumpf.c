...
int32_t kdmpset_S(CSOUND *csound, KDUMP *p) {
  ...
  p->fdch.fd = csound->FileOpen2(csound, &(p->f), CSFILE_STD, soundoname, "wb",
                                 "", dumpf_format_table[p->format], 0);
...
int32_t kdmpset_p(CSOUND *csound, KDUMP *p) {
  ...
  if (csound->ISSTRCOD(*p->ifilcod))
    ...
    csound->strarg2name(csound, soundoname, p->ifilcod, "dumpk.", 0);
  if (p->fdch.fd != NULL)
    csound_fd_close(csound, &(p->fdch));
  p->fdch.fd = csound->FileOpen2(csound, &(p->f), CSFILE_STD, soundoname, "wb",
                                 "", dumpf_format_table[p->format], 0);
...
static void nkdump(CSOUND *csound, MYFLT *kp, FILE *ofd, int32_t format,
  ...
  case 8:
    *outbuf = '\0';
    ...
    break;
  default:
    csound->PerfError(csound, &(((KDUMP *)p)->h), Str("unknown kdump format"));
  }
...
int32_t krdset_p(CSOUND *csound, KREAD *p) {
  ...
  if (csound->ISSTRCOD(*p->ifilcod))
    ...
    csound->strarg2name(csound, soundiname, p->ifilcod, "readk.", 0);
  if (p->fdch.fd != NULL)
    csound_fd_close(csound, &(p->fdch));
  p->fdch.fd =
      csound->FileOpen2(csound, &(p->f), CSFILE_STD, soundiname, "rb",
...
static void nkread(CSOUND *csound, MYFLT *kp, FILE *ifd, int32_t format,
                   int32_t nk) {
  int32_t len;
  char inbuf[256];
  int in_comment = 0;
  ...
  case 1: {
    ...
    len = nk;
    if ((unsigned)len != fread(inbuf, 1, len, ifd))
      break; /* now read the buffer */
    while (nk--)
      *kp++ = (MYFLT)*bp++;
    break;
  }
  ...
  case 7:
    while (nk--) {
      ...
      do { /* Absorb digits */
        ...
        if (c == EOF)
          return;
        if ((unsigned)(bp - inbuf + 1) >= sizeof(inbuf))
          return;
        *(++bp) = (char)c;
      } while (isdigit(*bp) || *bp == '-' || *bp == '+' || *bp == '.' ||
               *bp == 'e' || *bp == 'E');
  ...
  case 8:
    ...
    break;
  default:
    csound->Warning(csound, Str("unknown kdump format"));
  }
}
