...
static void fft_(CSOUND *, MYFLT *, MYFLT *, int32_t, int32_t, int32_t,
                 int32_t);
...
static void fft_(CSOUND *csound, MYFLT *a, MYFLT *b, int32_t nseg, int32_t n,
                 int32_t nspn, int32_t isn)
...
/*    *b;       pointer to array 'banal' */
{
  ...
  int32_t m = 0, k, kt, jj, j, nf, ntot, maxf, maxp = -1;
  ...
  buf =
      csound->Calloc(csound, sizeof(MYFLT) * 4 * maxf + sizeof(int32_t) * maxp);
  at = (MYFLT *)buf;
...
static void fftmx(MYFLT *a, MYFLT *b, int32_t ntot, int32_t n, int32_t nspan,
                  int32_t isn, int32_t m, int32_t *kt, MYFLT *at, MYFLT *ck,
                  MYFLT *bt, MYFLT *sk, int32_t *np, int32_t nfac[]) {
  int32_t i, inc, j, jc, jf, jj, k, k1, k2, k3 = 0, k4, kk, klim, ks, kspan,
                                            kspnn, lim, maxf, mm, nn, nt;
  double aa, aj, ajm, ajp, ak, akm, akp, bb, bj, bjm, bjp, bk, bkm, bkp, c1,
      c2 = 0, c3 = 0, c72, cd, dr, rad, sd, s1, s2 = 0, s3 = 0, s72, s120;
...
{
  IGN(csound);
  int32_t inc, j, k, lim, mm, ml, nf, nk, nh;

  double aa, ab, ba, bb, cd, cn, dr, em, rad, re, sd, sn;
  double xx; /******* ADDED APRIL 1991 ******/
...
void csoundRealFFTnp2(CSOUND *csound, MYFLT *buf, int32_t FFTsize) {
  if (!(FFTsize & (FFTsize - 1))) {
    ...
    if (UNLIKELY(FFTsize < 2 || (FFTsize & 1))) {
      csound->Warning(csound, Str("csoundRealFFTnp2(): invalid FFT size, %d"),
                      FFTsize);
      return;
    }
