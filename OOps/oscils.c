...
static void init_sine_gen(double a, double f, double p, double *x, double *c,
                          double *v) {
...
int32_t tablexkt_set(CSOUND *csound, TABLEXKT *p) {
  ...
  if (UNLIKELY(p->wsize < 3)) {
    ...
    if (p->wsize > 1024)
      p->wsize = 1024; /* integer multiply of 4 */
  }
...
int32_t tablexkt(CSOUND *csound, TABLEXKT *p) {
  ...
  if (UNLIKELY((wsize < 2) || (wsize > 1024))) {
    return csound->PerfError(csound, &(p->h), Str("tablexkt: not initialised"));
  }
  wsized2 = wsize >> 1;
  ...
  if (UNLIKELY((ftable = ftp->ftable) == NULL))
    return NOTOK;
  ...
  for (n = offset; n < nsmps; n++) {
    ...
    case 2: /* ---- linear interpolation ---- */
      ar[n] = ftable[ndx_i];
      if (++ndx_i >= flen)
        ndx_i = (wrap_ndx ? ndx_i - flen : flen);
    ...
    default:           /* ---- sinc interpolation ---- */
      ...
      if (warp) { /* ---- warp enabled ---- */
        ...
        do {
          a1 = (MYFLT)d;
          a1 = FL(1.0) - a1 * a1 * win_fact;
          a1 = a1 * a1 / (MYFLT)d;
          ar[n] +=
              ftable[(ndx_i < 0L ? (wrap_ndx ? ndx_i + flen : 0L) : ndx_i)] *
              (MYFLT)x * a1;
          ndx_i++;
          d++;
          v += c * x;
        ...
        if (UNLIKELY(ndx < 0.00003))
          ar[n] += onedwarp * ftable[ndx_i];
        else {
          a1 = (MYFLT)d;
          a1 = FL(1.0) - a1 * a1 * win_fact;
          a1 = a1 * a1 / (MYFLT)d;
