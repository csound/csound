...
static int32_t callox(CSOUND *csound) {
  ...
  ST(socksout) =
      (SOCK *)csound->Calloc(csound, (size_t)MAXREMOTES * sizeof(SOCK));
...
void remote_Cleanup(CSOUND *csound) {
  ...
  if (ST(insrfd_list) != NULL) {
    csound->Free(csound, ST(insrfd_list));
    ST(insrfd_list) = NULL;
  }
  if (ST(chnrfd_list) != NULL) {
    csound->Free(csound, ST(chnrfd_list));
    ST(chnrfd_list) = NULL;
  }
  if (ST(insrfd) != NULL) {
    csound->Free(csound, ST(insrfd));
    ST(insrfd) = NULL;
  }
  if (ST(chnrfd) != NULL) {
    csound->Free(csound, ST(chnrfd));
    ST(chnrfd) = NULL;
  }
  if (ST(ipadrs) != NULL) {
    csound->Free(csound, ST(ipadrs));
    ST(ipadrs) = NULL;
  }
...
/* Server -- open to receive */
{
  ...
  if (UNLIKELY((socklisten = socket(PF_INET, SOCK_STREAM, 0)) < 0)) {
    ...
    csound->Message(csound, Str("created socket\n"));
    /* set the addresse to be reusable */
#if defined(WIN32) && !defined(__CYGWIN__)
  if (UNLIKELY(setsockopt(socklisten, SOL_SOCKET, SO_REUSEADDR,
                          (const char *)&opt, sizeof(opt)) < 0))
#else
  if (UNLIKELY(setsockopt(socklisten, SOL_SOCKET, SO_REUSEADDR, &opt,
                          sizeof(opt)) < 0))
#endif
    return csound->InitError(
        csound, Str("setting socket option to reuse the address\n"));
  ...
  ST(local_addr).sin_port = htons(
      (int32_t)ST(remote_port)); /* port we listen on, netwrk byte order */
  /* associate the socket with the address and port */
  if (UNLIKELY(bind(socklisten, (struct sockaddr *)&ST(local_addr),
...
int32_t remoteport(CSOUND *csound, REMOTEPORT *p) {
  if (csound->remoteGlobals == NULL) {
    if (UNLIKELY(callox(csound) < 0)) {
      return csound->InitError(csound,
                               Str("failed to initialise remote globals."));
    }
  }
  if (ST(socksin) == NULL) {
    if (*p->port <= FL(0.0))
...
{ /*       INSTR 0 opcode  */
  ...
  if (strcmp(ST(ipadrs), (char *)p->str1->data) == 0) {
    ...
    for (nargs -= 1; nargs--;) {
      ...
      if (UNLIKELY(ST(chnrfd)[chnum])) {
        return csound->InitError(csound,
                                 Str("channel already specific remote"));
      }
...
int32_t insGlobevt(CSOUND *csound,
                   EVTBLK *evt) { /* send an event to all remote fd's */
  int32_t nn;
...
int32_t MIDIGlobevt(CSOUND *csound,
                    MEVENT *evt) { /* send an Mevent to all remote fd's */
  int32_t nn;
