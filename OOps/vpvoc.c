...
int tblesegset(TABLESEG *p)
{
    ...
    argp = p->argums;
    if ((nxtfunc = ftfind(p->h.insdshead->csound, *argp++)) == NULL)
        return NOTOK;
    ...
    do {
        ...
        curfunc = nxtfunc;
        dur = **argp++;
        if ((nxtfunc = ftfind(p->h.insdshead->csound, *argp++)) == NULL) return OK;
        if (dur > FL(0.0)) {
                segp->d = dur * ekr;
...
int vpvset(VPVOC *p)
{
    ...
    if (*p->isegtab==FL(0.0)) p->tableseg = tbladr;
      ...
      if ((p->tableseg->outfunc = ftfind(p->h.insdshead->csound, p->isegtab)) == NULL) {
        sprintf(errmsg,
