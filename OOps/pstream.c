...
int32_t fsigs_equal(const PVSDAT *f1, const PVSDAT *f2) {
  if ((f1->overlap == f2->overlap) && (f1->winsize == f2->winsize) &&
      (f1->wintype == f2->wintype) /* harsh, maybe... */
      && (f1->N == f2->N) && (f1->format == f2->format) &&
...
int32_t pvscrosset(CSOUND *csound, PVSCROSS *p) {
  ...
  if (fsrc->sliding) {
    fout->NB = fsrc->NB;
    csound->AuxAlloc(csound, (N + 2) * sizeof(MYFLT) * CS_KSMPS, &fout->frame);
    return OK;
  }
  /* setup output signal */
  /* RWD MUST be 32bit */
...
static int32_t pvsfreadset_(CSOUND *csound, PVSFREAD *p, int32_t stringname) {
  ...
  if (stringname)
    strNcpy(pvfilnam, ((STRINGDAT *)p->ifilno)->data, MAXNAME - 1);
  else if (csound->ISSTRCOD(*p->ifilno))
    strNcpy(pvfilnam, get_arg_string(csound, *p->ifilno), MAXNAME - 1);
  else
    csound->strarg2name(csound, pvfilnam, p->ifilno, "pvoc.", 0);
...
int32_t pvsftw(CSOUND *csound, PVSFTW *p) {
  ...
  if (UNLIKELY(fsrc == NULL))
    return csound->PerfError(csound, &(p->h), Str("pvsftw: not initialised\n"));
  if (UNLIKELY(ftablea == NULL))
    return csound->PerfError(csound, &(p->h), Str("pvsftw: no amps ftable!\n"));
  if (p->outfnf) {
    ftablef = p->outfnf->ftable;
...
int32_t pvsftrset(CSOUND *csound, PVSFTR *p) {
  ...
  */
  ...
  if ((int32_t)*p->ifnf >= 1) {
    ...
    if (UNLIKELY(flenf < nbins))
      return csound->InitError(csound,
                               Str("pvsftr: freqs ftable too small.\n"));
    for (i = 0; i < nbins; i++)
...
int32_t pvsftr(CSOUND *csound, PVSFTR *p) {
  ...
  if (UNLIKELY(fdest == NULL))
    return csound->PerfError(csound, &(p->h), Str("pvsftr: not initialised\n"));
  nbins = p->fftsize / 2 + 1;
