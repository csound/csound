...
static CS_NOINLINE void diskin2_read_buffer(CSOUND *csound, DISKIN2 *p,
                                            int32_t bufReadPos) {
  MYFLT *tmp;
  int32_t nsmps;
  int32_t i;
...
static inline void diskin2_get_sample(CSOUND *csound, DISKIN2 *p, int32_t fPos,
                                      int32_t n, MYFLT scl) {
  int32_t bufPos, i;
...
static const int32_t diskin2_format_table[11] = {0,
...
static int32_t diskin2_init_(CSOUND *csound, DISKIN2 *p, int32_t stringname) {
  ...
  if (UNLIKELY(fd == NULL)) {
    return csound->InitError(csound,
                             Str("diskin2: %s: failed to open file (%s)"), name,
  ...
  if (csound->oparms->realtime == 1 && p->fforceSync == 0 &&
      (p->cb = csound->CreateCircularBuffer(
           csound, p->bufSize * p->nChannels * 2, sizeof(MYFLT))) != NULL) {
    ...
    if (UNLIKELY((csound->oparms_.msglevel & 7) == 7)) {
      csound->Message(csound,
                      ...
                      csound->GetFileName(fd), sfinfo.samplerate,
                      sfinfo.channels, Str("channel(s)"),
                      (int64_t)sfinfo.frames, Str("sample frames\n"));
    }
  } else {
    p->aOut_buf = NULL;
    p->aOut_bufsize = 0;
    ...
    if (UNLIKELY((csound->oparms_.msglevel & 7) == 7)) {
      csound->Message(csound,
                      "%s '%s':\n"
                      "         %d Hz, %d %s, %" PRId64 " %s\n",
                      Str("diskin2: opened"), csound->GetFileName(fd),
                      sfinfo.samplerate, sfinfo.channels, Str("channel(s)"),
                      (int64_t)sfinfo.frames, Str("sample frames\n"));
    }
  }
...
int32_t diskin2_async_deinit(CSOUND *csound, void *p) {
  ...
  if ((top = (DISKIN_INST **)csound->QueryGlobalVariable(
           csound, "DISKIN_INST")) == NULL)
    return NOTOK;
  current = *top;
  prv = NULL;
  ...
  if (*top == NULL) {
    ...
    start =
        (int32_t *)csound->QueryGlobalVariable(csound, "DISKIN_THREAD_START");
    *start = 0;
...
int32_t diskin2_perf_synchronous(CSOUND *csound, DISKIN2 *p) {
  ...
  if (!p->initDone && !p->SkipInit) {
    return csound->PerfError(csound, &(p->h), Str("diskin2: not initialised"));
  }
  ...
  default: /* ---- sinc interpolation ---- */
    ...
    if (p->pos_frac_inc > (int64_t)nn || p->pos_frac_inc < (int64_t)(-nn)) {
      warp = 1; /* enable warp */
...
int32_t diskin_file_read(CSOUND *csound, DISKIN2 *p) {
  ...
  int32_t nsmps = checkspace(
      p->cb, 1); // suggest in issue #1535 (no patch supplied though, but seems
                 // to work)
...
uintptr_t diskin_io_thread(void *p) {
  ...
  int32_t wakeup = 1000 * current->diskin->h.insdshead->ksmps /
                   current->diskin->h.insdshead->esr;
  int32_t *start = current->csound->QueryGlobalVariable(current->csound,
                                                        "DISKIN_THREAD_START");
...
int32_t diskin2_perf(CSOUND *csound, DISKIN2 *p) {
  if (!p->async)
    return diskin2_perf_synchronous(csound, p);
  else
    return diskin2_perf_asynchronous(csound, p);
}
...
static int32_t sndo1set_(CSOUND *csound, void *pp, int32_t stringname) {
  ...
  if (stringname == 0) {
    if (csound->ISSTRCOD(*ifilcod))
      ...
      csound->strarg2name(csound, name, ifilcod, "soundout.", 0);
  } else
    strNcpy(name, ((STRINGDAT *)ifilcod)->data, 1023);
...
int32_t soundout(CSOUND *csound, SNDOUT *p) {
  ...
  if (UNLIKELY(p->c.sf == NULL))
    return csound->PerfError(csound, &(p->h), Str("soundout: not initialised"));
  if (UNLIKELY(early))
    nsmps -= early;
  for (nn = offset; nn < nsmps; nn++) {
...
static int32_t diskin2_calc_buffer_size_array(DISKIN2_ARRAY *p,
                                              int32_t n_monoSamps) {
  int32_t i, nFrames;
...
static inline void diskin2_get_sample_array(CSOUND *csound, DISKIN2_ARRAY *p,
                                            int32_t fPos, int32_t n,
                                            MYFLT scl) {
  int32_t bufPos, i;
...
uintptr_t diskin_io_thread_array(void *p) {
  ...
  int32_t wakeup = 1000 * current->diskin->h.insdshead->ksmps /
                   current->diskin->h.insdshead->esr;
  int32_t *start = current->csound->QueryGlobalVariable(
      current->csound, "DISKIN_THREAD_START_ARRAY");
  _MM_SET_DENORMALS_ZERO_MODE(_MM_DENORMALS_ZERO_ON);
  while (*start) {
    current = (DISKIN_INST *)p;
    csoundSleep(wakeup > 0 ? wakeup : 1);
...
#ifndef __EMSCRIPTEN__
    if (top == NULL) {
      csound->CreateGlobalVariable(csound, "DISKIN_INST_ARRAY",
                                   sizeof(DISKIN_INST *));
      top = (DISKIN_INST **)csound->QueryGlobalVariable(csound,
                                                        "DISKIN_INST_ARRAY");
      *top = (DISKIN_INST *)csound->Calloc(csound, sizeof(DISKIN_INST));
      csound->CreateGlobalVariable(csound, "DISKIN_PTHREAD_ARRAY",
                                   sizeof(void **));
      csound->CreateGlobalVariable(csound, "DISKIN_THREAD_START_ARRAY",
                                   sizeof(int32_t));
      current = *top;
    } else
#endif
...
#ifndef __EMSCRIPTEN__
    if (*(start = csound->QueryGlobalVariable(
              csound, "DISKIN_THREAD_START_ARRAY")) == 0) {
      uintptr_t diskin_io_thread_array(void *p);
...
int32_t diskin2_perf_array(CSOUND *csound, DISKIN2_ARRAY *p) {
  if (!p->async)
    return diskin2_perf_synchronous_array(csound, p);
  else
    return diskin2_perf_asynchronous_array(csound, p);
}
