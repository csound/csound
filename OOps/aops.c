...
int32_t aassign(CSOUND *csound, ASSIGN *p, int32_t islocal) {
  ...
  if (LIKELY(nsmps != 1)) {
    ...
    if (UNLIKELY(islocal && offset))
      memset(p->r, '\0', offset * sizeof(MYFLT));
    if (UNLIKELY(early)) {
      nsmps -= early;
      if (islocal)
...
#define LOGCLX(OPNAME, OP)                                                     \
  int32_t OPNAME(CSOUND *csound, LOGCL *p) {                                   \
    IGN(csound);                                                               \
    *p->rbool = (*p->ibool OP * p->jbool) ? 1 : 0;                             \
    return OK;                                                                 \
  }
...
#define LIB1(OPNAME, LIBNAME)                                                  \
  int32_t OPNAME(CSOUND *csound, EVAL *p) {                                    \
    IGN(csound);                                                               \
    *p->r = LIBNAME(*p->a);                                                    \
    return OK;                                                                 \
  }
...
int32_t cpsmidinn(CSOUND *csound, EVAL *p) {
  ...
  *p->r = POWER(FL(2.0), (note - FL(69.0)) / FL(12.0)) * (MYFLT)(csound->A4);
  return OK;
}
...
int32_t cpsxpch(CSOUND *csound, XENH *p) { /* This may be too expensive */
  double fract;
  double loct;
  ...
  if (*p->et > 0) {
    ...
    *p->r = *p->ref * *(ftp->ftable + frt) * POWER(*p->cy, (MYFLT)loct);
  }
  return OK;
}
...
int32_t cps2pch(CSOUND *csound, XENH *p) {
  ...
  if (*p->et > 0) {
    ...
    *p->r = (MYFLT)(1.02197503906 * *(ftp->ftable + frt) * pow(2.0, loct));
  }
...
int32_t logbasetwo_set(CSOUND *csound, EVAL *p) {
  ...
  if (UNLIKELY(csound->logbase2 == NULL)) {
    ...
    int32_t i;
    csound->logbase2 =
        (MYFLT *)csound->Malloc(csound, (STEPS + 1) * sizeof(MYFLT));
    for (i = 0; i <= STEPS; i++) {
...
int32_t powoftwoa(CSOUND *csound,
                  EVAL *p) { /* by G.Maldonado, liberalised by JPff */
  MYFLT *a = p->a, *r = p->r;
  uint32_t offset = p->h.insdshead->ksmps_offset;
  uint32_t early = p->h.insdshead->ksmps_no_end;
  uint32_t n, nsmps = CS_KSMPS;
...
int32_t logbasetwo(CSOUND *csound, EVAL *p) {
  int32_t n = (int32_t)((*p->a - (FL(1.0) / INTERVAL)) /
                            (INTERVAL - FL(1.0) / INTERVAL) * STEPS +
                        FL(0.5));
  if (n < 0 || n > STEPS)
...
int32_t logbasetwoa(CSOUND *csound,
                    EVAL *p) { /* by G.Maldonado liberalised by JPff */
  MYFLT *r, *a;
  uint32_t offset = p->h.insdshead->ksmps_offset;
  uint32_t early = p->h.insdshead->ksmps_no_end;
  ...
  for (n = offset; n < nsmps; n++) {
    MYFLT aa = a[n];
    int32_t n = (int32_t)((aa - (FL(1.0) / INTERVAL)) /
                              (INTERVAL - FL(1.0) / INTERVAL) * STEPS +
                          FL(0.5));
    if (n < 0 || n > STEPS)
      r[n] = LOG(aa) * ONEdLOG2;
    else
      r[n] = csound->logbase2[n];
  }
  return OK;
}
...
int32_t ino(CSOUND *csound, INO *p) {
  MYFLT *sp = CS_SPIN, *ar1 = p->ar1, *ar2 = p->ar2, *ar3 = p->ar3,
        *ar4 = p->ar4, *ar5 = p->ar5, *ar6 = p->ar6, *ar7 = p->ar7,
        *ar8 = p->ar8;
  uint32_t offset = p->h.insdshead->ksmps_offset;
  uint32_t early = p->h.insdshead->ksmps_no_end;
  uint32_t n, nsmps = CS_KSMPS, k;
...
int32_t inch_opcode1(CSOUND *csound, INCH1 *p) {
  ...
  if (UNLIKELY(ch > (uint32_t)csound->inchnls)) {
    ...
    ain = p->ar;
    if (UNLIKELY(offset))
      memset(ain, '\0', offset * sizeof(MYFLT));
    if (UNLIKELY(early)) {
      nsmps -= early;
...
int32_t inch_opcode(CSOUND *csound,
                    INCH *p) { /* Rewritten to allow multiple args upto 40 */
...
inline static int32_t outn(CSOUND *csound, uint32_t k, uint32_t n, MYFLT **asig,
                           INSDS *p, MYFLT *arr) {
...
int32_t outs1(CSOUND *csound, OUTM *p) {
  int32_t ret;
  ret = outn(csound, 0, 1, &(p->asig), p->h.insdshead, NULL);
  return ret;
}
...
int32_t och2(CSOUND *csound, OUTM *p) {
  IGN(p);
  OUTCN(2)
}
int32_t och3(CSOUND *csound, OUTM *p) {
  IGN(p);
  OUTCN(3)
}
int32_t och4(CSOUND *csound, OUTM *p) {
  IGN(p);
  OUTCN(4)
}
...
int32_t outs2(CSOUND *csound, OUTM *p) {
  int32_t ret;
  ret = outn(csound, 1, 2, &(p->asig), p->h.insdshead, NULL);
  return ret;
}

int32_t outq3(CSOUND *csound, OUTM *p) {
  int32_t ret;
  ret = outn(csound, 2, 3, &(p->asig), p->h.insdshead, NULL);
  return ret;
}

int32_t outq4(CSOUND *csound, OUTM *p) {
  int32_t ret;
  ret = outn(csound, 3, 4, &(p->asig), p->h.insdshead, NULL);
  return ret;
}
...
int32_t outch(CSOUND *csound, OUTCH *p) {
  ...
  if (UNLIKELY((count & 1) != 0))
    return csound->PerfError(
        csound, &(p->h), Str("outch must have an even number of arguments"));
  for (n = 0; n < count; n += 2) {
    ch = (int)*p->args[n] - 1;
    if (ch < nchnls)
      ret = outn(csound, ch, ch + 1, &p->args[n + 1], p->h.insdshead, NULL);
  }
  return ret;
}
...
{
  uint32_t nch = p->INOCOUNT;
  int32_t ret = outn(csound, 0, (nch <= csound->nchnls ? nch : csound->nchnls),
  ...
  if (n > csound->nchnls) {
    if (p->nowarn == 0) {
      csound->Warning(csound,
                      Str("out: number of channels truncated from %d to %d"), n,
                      csound->nchnls);
    }
    n = csound->nchnls;
    p->nowarn = 1;
...
int32_t outrep(CSOUND *csound, OUTM *p) {
  ...
  int32_t ret;
  for (i = 0; i < n; i++)
    ret = outn(csound, i, i + 1, &p->asig, p->h.insdshead, NULL);
  return ret;
}
...
int32_t monitor_opcode_init(CSOUND *csound, MONITOR_OPCODE *p) {
  if (UNLIKELY(csound->GetOutputArgCnt(p) !=
...
int32_t pvalue(CSOUND *csound, PFIELD *p) {
  ...
  if (UNLIKELY(csound->init_event == NULL || n < 1 ||
               n > csound->init_event->pcnt)) {
...
int32_t pvaluestr(CSOUND *csound, PFIELDSTR *p) {
  ...
  if (p->ians->data != NULL)
    csound->Free(csound, p->ians->data);

  if (LIKELY(csound->ISSTRCOD(csound->init_event->p[n]))) {
    p->ians->data = csound->Strdup(
        csound, csound->GetString(csound, csound->init_event->p[n]));
...
int32_t pinit(CSOUND *csound, PINIT *p) {
  ...
  for (n = 0; (n < nargs) && (n <= pargs - start); n++) {
    ...
    if (csound->ISSTRCOD(csound->init_event->p[n + start])) {
      ((STRINGDAT *)p->inits[n])->data = csound->Strdup(
          csound, csound->GetString(csound, csound->init_event->p[n + start]));
