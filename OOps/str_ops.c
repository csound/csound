...
int32_t s_opcode(CSOUND *csound, STRGET_OP *p) {
  if (p->r->size < DEFAULT_STRING_SIZE) {
    p->r->data =
        (char *)csound->ReAlloc(csound, p->r->data, DEFAULT_STRING_SIZE);
...
static void str_set(CSOUND *csound, int32_t ndx, const char *s) {
  if (UNLIKELY(csound->strsets == NULL)) {
    ...
    csound->strsets =
        (char **)csound->Calloc(csound, (csound->strsmax + 1) * sizeof(char *));
  }
...
int32_t strget_init(CSOUND *csound, STRGET_OP *p) {
  ...
  if (indx < 0 || indx > (int32_t)csound->strsmax || csound->strsets == NULL ||
      csound->strsets[indx] == NULL)
    return OK;
...
static CS_NOINLINE int32_t StrOp_ErrMsg(void *p, const char *msg) {
  ...
  if (UNLIKELY(csound->ids != NULL && csound->ids->insdshead == csound->curip))
    ...
    return csound->PerfError(csound, (OPDS *)p, "%s: %s", opname, Str(msg));
  else
...
int32_t strcat_opcode(CSOUND *csound, STRCAT_OP *p) {
  ...
  if (size >= MAX_STRINGDAT_SIZE) {
    if (is_perf_thread(&p->h))
      return csound->PerfError(
          csound, &p->h, "strcatk: requested alloc size exceeds max (%u bytes)",
          MAX_STRINGDAT_SIZE);
    else
      return csound->InitError(
          csound, "strcat: requested alloc size exceeds max (%u bytes)",
          MAX_STRINGDAT_SIZE);
  }
  p->r->timestamp = kcnt;
  if (p->str1 != p->r && p->str2 != p->r) {
    ...
    if (size >= p->r->size) {
      csound->Free(csound, p->r->data);
      p->r->data = csound->Calloc(csound, 2 * size);
      p->r->size = 2 * size;
    }
...
sprintf_opcode_(CSOUND *csound, void *p, /* opcode data structure pointer */
    ...
    if (segwaiting != NULL) {
      ...
      case 'c':
        ...
        if (strlen(strseg) + 24 > (size_t)maxChars) {
          ...
          str->data = csound->ReAlloc(csound, str->data, str->size + 24);
          if (str->data == NULL) {
      ...
      case 's':
        if (LIKELY(IS_STR_ARG(parm))) {
          ...
          if (UNLIKELY(((STRINGDAT *)parm)->size + strlen(strseg) >=
                       (uint32_t)maxChars)) {
            int32_t offs = outstring - str->data;
          ...
          return StrOp_ErrMsg(p, Str("Not string type for %%s"));
        break;
      default:
        csound->Free(csound, strseg);
...
int32_t sprintf_opcode(CSOUND *csound, SPRINTF_OP *p) {
  ...
  if (UNLIKELY(sprintf_opcode_(csound, p, p->r, (char *)p->sfmt->data,
                               &(p->args[0]), (int32_t)p->INOCOUNT - 1,
                               0) == NOTOK)) {
    ((char *)p->r->data)[0] = '\0';
    return NOTOK;
  }
...
int32_t strchar_opcode(CSOUND *csound, STRCHAR_OP *p) {
  ...
  if (pos < 0 || (size_t)pos >= len)
    ...
    *(p->ichr) =
        (MYFLT)((int32_t)((unsigned char)((char *)p->Ssrc->data)[pos]));
...
int32_t str_from_url(CSOUND *csound, STRCPY_OP *p) {
  ...
  if (strstr(newVal, ":/") == NULL)
    return strcpy_opcode_S(csound, p);
  {
