...
int32_t schedule_SN(CSOUND *csound, SCHED *p) {
  ...
  int32_t argno = p->INOCOUNT + 1;
  char s[16384], sf[64];
  sprintf(s, "i \"%s\" %f %f", ((STRINGDAT *)p->which)->data, *p->when,
          *p->dur);
  for (i = 4; i < argno; i++) {
    MYFLT *arg = p->argums[i - 4];
    if (csoundGetTypeForArg(arg) == &CS_VAR_TYPE_S)
...
int32_t
ifschedule(CSOUND *csound,
           WSCHED *p) { /* All we need to do is ensure the trigger is set */
  IGN(csound);
  p->todo = 1;
  return OK;
}
...
int32_t lfok(CSOUND *csound, LFO *p) {
  ...
  default:
    return csound->PerfError(
        csound, &(p->h), Str("LFO: unknown oscilator type %d"), p->lasttype);
  case 0:
    iphs = phs >> 12;
...
int32_t triginset(CSOUND *csound, TRIGINSTR *p) {
  ...
  if (csound->global_kcounter > 0 && *p->trigger != FL(0.0) &&
      p->h.insdshead->p3.value == 0)
    ktriginstr_(csound, p, 0);
  return OK;
}
...
static int32_t ktriginstr_(CSOUND *csound, TRIGINSTR *p,
                           int32_t stringname) { /* k-rate event generator */
  int64_t starttime;
  int32_t i, argnum;
  EVTBLK evt;
  char name[512];
  ...
  if (*p->mintime > FL(0.0))
    ...
    p->timrem = 0;
  return (insert_score_event_at_sample(csound, &evt, starttime) == 0 ? OK
                                                                     : NOTOK);
}
