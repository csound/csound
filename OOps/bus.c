...
int32_t chano_opcode_perf_a(CSOUND *csound, CHNVAL *p) {
  ...
  if (UNLIKELY(offset))
    memset(&val, '\0', offset * sizeof(MYFLT));
...
int32_t pvsin_init(CSOUND *csound, FCHAN *p) {
  ...
  if (p->r->frame.auxp == NULL || p->r->frame.size < sizeof(float) * (N + 2))
...
int32_t pvsin_perf(CSOUND *csound, FCHAN *p) {
  ...
  if (fin->frame != NULL)
    ...
    memset(fout->frame.auxp, 0, sizeof(float) * (size + 2));
  csoundSpinUnLock(lock);
  return OK;
}
  ...
  if (head != NULL) {
    while (values != NULL) {
      ...
      /* SY - 2014.07.14 - Don't free entry->data and rely on Csound memory db
         ...
         maps to the CS_VAR_MEM's memblock, which is not what is allocated;
         other vars will be freed since they were Calloc'd */
...
static CS_NOINLINE CHNENTRY *alloc_channel(CSOUND *csound, const char *name,
                                           int32_t type) {
  CHNENTRY *pp;
  int32_t dsize = 0;
...
PUBLIC int csoundListChannels(CSOUND *csound, controlChannelInfo_t **lst) {
  ...
  *lst = (controlChannelInfo_t *)csound->Malloc(
      csound, n * sizeof(controlChannelInfo_t));
...
PUBLIC int csoundSetControlChannelHints(CSOUND *csound, const char *name,
  ...
  pp->hints = hints;
  if (hints.attributes) {
    pp->hints.attributes = (char *)csound->Malloc(
        csound, (strlen(hints.attributes) + 1) * sizeof(char));
...
PUBLIC int csoundGetControlChannelHints(CSOUND *csound, const char *name,
  ...
  *hints = pp->hints;
  if (pp->hints.attributes) {
    hints->attributes =
        (char *)csound->Malloc(csound, strlen(pp->hints.attributes) + 1);
...
static int32_t chnget_opcode_perf_k(CSOUND *csound, CHNGET *p) {
  if (strncmp(p->chname, p->iname->data, MAX_CHAN_NAME) ||
      !strcmp(p->iname->data, "")) {
    int32_t err =
        csoundGetChannelPtr(csound, &(p->fp), (char *)p->iname->data,
                            CSOUND_CONTROL_CHANNEL | CSOUND_INPUT_CHANNEL);
    if (err == 0) {
      p->lock =
          (spin_lock_t *)csoundGetChannelLock(csound, (char *)p->iname->data);
...
int32_t chnget_array_opcode_init(CSOUND *csound, CHNGETARRAY *p) {
  ...
  p->arraySize = arr->sizes[0];
  p->channels = (STRINGDAT *)arr->data;
  p->channelPtrs = (MYFLT **)csound->Malloc(
      csound,
      p->arraySize * sizeof(MYFLT *)); // VL: surely an array of pointers?
  tabinit(csound, p->arrayDat, p->arraySize);
  ...
  for (index = 0; index < p->arraySize; index++) {
    if (strcmp(p->channels[index].data, "")) {
      err = csoundGetChannelPtr(csound, &p->channelPtrs[index],
                                p->channels[index].data, channelType);
      ...
      if (LIKELY(!err)) {
        ...
        if (channelType == (CSOUND_STRING_CHANNEL | CSOUND_INPUT_CHANNEL)) {
          csoundSpinLock(p->lock);
          strings[index].data =
              cs_strdup(csound, ((STRINGDAT *)p->channelPtrs[index])->data);
          strings[index].size =
...
int32_t chnget_array_opcode_perf_a(CSOUND *csound, CHNGETARRAY *p) {
  ...
  for (index = 0; index < p->arraySize; index++) {
    ...
    if (CS_KSMPS == (uint32_t)csound->ksmps) {
      ...
      memcpy(&p->arrayDat->data[blockIndex + offset], p->channelPtrs[index],
             sizeof(MYFLT) * (CS_KSMPS - offset - early));
      if (UNLIKELY(early))
        memset(&p->arrayDat->data[blockIndex + CS_KSMPS - early], '\0',
               sizeof(MYFLT) * early);
      csoundSpinUnLock(p->lock);
    } else {
      csoundSpinLock(p->lock);
      if (UNLIKELY(offset))
        memset(&outPtr, '\0', offset);
...
int32_t chnget_array_opcode_perf_S(CSOUND *csound, CHNGETARRAY *p) {
  ...
  for (index = 0; index < p->arraySize; index++) {
    ...
    p->lock = (spin_lock_t *)csoundGetChannelLock(
        csound, (char *)p->channels[index].data);
    csoundSpinLock(p->lock);
    strings[index].data =
        cs_strdup(csound, ((STRINGDAT *)p->channelPtrs[index])->data);
    strings[index].size =
...
int32_t chnset_array_opcode_init_i(CSOUND *csound, CHNGETARRAY *p) {
  ...
  p->channelPtrs = (MYFLT **)csound->Malloc(
      csound, p->arraySize * sizeof(MYFLT *)); // VL surely array of pointers?
  ...
  for (index = 0; index < p->arraySize; index++) {
    ...
    x.d = valueArr->data[index];
    __atomic_store_n((MYFLT_INT_TYPE *)(p->channelPtrs[index]), x.i,
                     __ATOMIC_SEQ_CST);
#else
    {
...
int32_t chnset_array_opcode_init(CSOUND *csound, CHNGETARRAY *p) {
  ...
  p->arraySize = channelArr->sizes[0];
  p->channels = (STRINGDAT *)channelArr->data;
  p->channelPtrs = csound->Malloc(
      csound, p->arraySize * sizeof(MYFLT *)); // surely an array of pointers?
  ...
  for (index = 0; index < p->arraySize; index++) {
    err = csoundGetChannelPtr(csound, &(p->channelPtrs[index]),
                              (char *)p->channels[index].data, channelType);
    if (LIKELY(!err)) {
      p->lock = (spin_lock_t *)csoundGetChannelLock(
          csound, (char *)p->channels[index].data);
      strNcpy(p->chname, p->channels[index].data, MAX_CHAN_NAME);
...
int32_t chnset_array_opcode_perf_k(CSOUND *csound, CHNGETARRAY *p) {
  ...
  for (index = 0; index < p->arraySize; index++) {
    if (strncmp(p->chname, p->channels[index].data, MAX_CHAN_NAME)) {
      int32_t err = csoundGetChannelPtr(
          csound, &(p->channelPtrs[index]), (char *)p->channels[index].data,
          CSOUND_CONTROL_CHANNEL | CSOUND_INPUT_CHANNEL);
      if (err == 0) {
        p->lock = (spin_lock_t *)csoundGetChannelLock(
            csound, (char *)p->channels[index].data);
...
int32_t chnset_array_opcode_perf_a(CSOUND *csound, CHNGETARRAY *p) {
  ...
  for (index = 0; index < p->arraySize; index++) {
    if (CS_KSMPS == (uint32_t)csound->ksmps) {
      ...
      csoundSpinLock(p->lock);
      if (UNLIKELY(offset))
        memset(p->channelPtrs[index], '\0', sizeof(MYFLT) * offset);
      memcpy(&p->channelPtrs[index][offset],
             &valueArr->data[blockIndex + offset],
             sizeof(MYFLT) * (CS_KSMPS - offset - early));
      if (UNLIKELY(early))
        memset(&p->channelPtrs[index][early], '\0',
               sizeof(MYFLT) * (CS_KSMPS - early));
      csoundSpinUnLock(p->lock);
    } else {
      /* Need lock for the channel */
      csoundSpinLock(p->lock);
  ...
  if (LIKELY(!err)) {
    ...
    if (((STRINGDAT *)p->fp)->data != NULL) {
      if (((STRINGDAT *)p->fp)->size > ((STRINGDAT *)p->arg)->size) {
        ...
        strcpy(((STRINGDAT *)p->arg)->data, ((STRINGDAT *)p->fp)->data);
    }
    csoundSpinUnLock(p->lock);
  }
  return OK;
...
static int32_t chnset_opcode_perf_a(CSOUND *csound, CHNGET *p) {
  ...
  if (CS_KSMPS == (uint32_t)csound->ksmps) {
    ...
    csoundSpinLock(p->lock);
    if (UNLIKELY(offset))
      memset(p->fp, '\0', sizeof(MYFLT) * offset);
...
static int32_t chnclear_opcode_perf(CSOUND *csound, CHNCLEAR *p) {
  ...
  for (i = 0; i < n; i++) {
    ...
    memset(p->fp[i], 0,
           CS_KSMPS * sizeof(MYFLT)); /* Should this leave start? */
...
int32_t chnclear_opcode_init(CSOUND *csound, CHNCLEAR *p) {
  ...
  for (i = 0; i < n; i++) {
    ...
    err = csoundGetChannelPtr(csound, &(p->fp[i]), (char *)p->iname[i]->data,
                              CSOUND_AUDIO_CHANNEL | CSOUND_OUTPUT_CHANNEL);
    if (LIKELY(!err)) {
      p->lock[i] = (spin_lock_t *)csoundGetChannelLock(
          csound, (char *)p->iname[i]->data);
    } else
      return print_chn_err(p, err);
  }
...
int32_t chnset_opcode_perf_S(CSOUND *csound, CHNGET *p) {
  ...
  if (s == NULL)
    return NOTOK;
  if (((STRINGDAT *)p->fp)->data && strcmp(s, ((STRINGDAT *)p->fp)->data) == 0)
    return OK;
...
int32_t sensekey_perf(CSOUND *csound, KSENSE *p) {
  ...
  retval =
      csound->doCsoundCallback(csound, &keyCode,
                               (p->keyDown != NULL ? CSOUND_CALLBACK_KBD_EVENT
    ...
    if (csound->inChar_ < 0) {
      ...
      if (retval > 0) {
        ...
        if (UNLIKELY((n = read(0, &ch, 1)) < 0)) {
          csound->PerfError(csound, &(p->h), Str("read failure in sensekey\n"));
          return NOTOK;
        }
  ...
  *(p->ans) =
      (MYFLT)((keyCode & (int32_t)0xFFFF) ? (keyCode & (int32_t)0xFFFF) : -1);
...
int32_t kinval(CSOUND *csound, INVAL *p) {
  if (csound->InputChannelCallback_) {
    csound->InputChannelCallback_(csound, (char *)p->channelName.auxp, p->value,
                                  p->channelType);
  } else
    *(p->value) = FL(0.0);
...
int32_t kinvalS(CSOUND *csound, INVAL *p) {
  ...
  if (csound->InputChannelCallback_) {
    csound->InputChannelCallback_(csound, (char *)p->channelName.auxp,
...
int32_t koutvalS(CSOUND *csound, OUTVAL *p) {
  ...
  if (csound->OutputChannelCallback_) {
    csound->OutputChannelCallback_(csound, chan, ((STRINGDAT *)p->value)->data,
                                   p->channelType);
  }
  return OK;
}
