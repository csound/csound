...
int32_t
tonsetx(CSOUND *csound,
        TONEX *p) { /* From Gabriel Maldonado, modified for arbitrary order */
  {
    double b;
    p->prvhp = *p->khp;
  ...
  if (UNLIKELY((p->loop = (int32_t)(*p->ord + FL(0.5))) < 1))
    p->loop = 4;
...
int32_t kreson(CSOUND *csound, RESON *p) {
  ...
  *p->ar = (MYFLT)yt0;
  p->yt1 = yt0;
  p->yt2 = yt1; /* Write back for next cycle */
  return OK;
}
...
int32_t reson(CSOUND *csound, RESON *p) {
  ...
  p->yt1 = yt1;
  p->yt2 = yt2; /* Write back for next cycle */
  return OK;
}

int32_t rsnsetx(CSOUND *csound,
                RESONX *p) { /* Gabriel Maldonado, modifies for arb order */
  int32_t scale;
  p->scale = scale = (int32_t)*p->iscl;
  ...
  p->yt1 = (double *)p->aux.auxp;
  p->yt2 = (double *)p->aux.auxp + p->loop;
  if (UNLIKELY(scale && scale != 1 && scale != 2)) {
    return csound->InitError(csound, Str("illegal reson iscl value, %f"),
                             *p->iscl);
  }
...
int32_t lprdset_(CSOUND *csound, LPREAD *p, int32_t stringname) {
  ...
  /* Build file name */
  if (stringname)
    strNcpy(lpfilname, ((STRINGDAT *)p->ifilcod)->data, MAXNAME - 1);
  else if (csound->ISSTRCOD(*p->ifilcod))
    strNcpy(lpfilname, get_arg_string(csound, *p->ifilcod), MAXNAME - 1);
  else
    csound->strarg2name(csound, lpfilname, p->ifilcod, "lp.", 0);
  ...
  } else if (UNLIKELY(BYTREVL(lph->lpmagic) ==
                      LP_MAGIC)) { /* Header reversed: */
  ...
  if (UNLIKELY(csound->oparms->odebug))
    csound->Message(csound,
                    Str("npoles %" PRIi32 ", nvals %" PRIi32
                        ", totvals %" PRIi32 ", lastfram16 = %" PRIi32 "x\n"),
...
#ifdef TRACE_POLES
static void DumpPolesF(int32_t poleCount, MYFLT *part1, MYFLT *part2,
...
static int32_t DoPoleInterpolation(int poleCount, MYFLT *pm1, MYFLT *pp1,
                                   MYFLT *pm2, MYFLT *pp2, MYFLT factor,
                                   MYFLT *outMagn, MYFLT *outPhas) {
  int32_t i;
...
static inline void synthetize(int32_t poleCount, double *poleReal,
                              double *poleImag, double *polyReal,
                              double *polyImag) {
  int32_t j, k;
  double pr, pi, cr, ci;
...
int32_t lpread(CSOUND *csound, LPREAD *p) {
  ...
  if (UNLIKELY(p->mfp == NULL)) {
    return csound->PerfError(csound, &(p->h), Str("lpread: not initialised"));
  }
  /* Locate frame position range */
  if (UNLIKELY((framphase = (int32)(*p->ktimpt * p->framrat16)) < 0)) {
    /* for kfram reqd*/
    return csound->PerfError(csound, &(p->h), Str("lpread timpnt < 0"));
  }
  ...
  *p->krmr = *bp + (*np - *bp) * fract;
  bp++;
  np++; /* for 4 rslts */
  *p->krmo = *bp + (*np - *bp) * fract;
  bp++;
  np++;
  *p->kerr = *bp + (*np - *bp) * fract;
  bp++;
  np++;
  *p->kcps = *bp + (*np - *bp) * fract;
  bp++;
  np++;
  ...
  if (p->storePoles) {
    ...
    if (UNLIKELY(!status)) {
      return csound->PerfError(csound, &(p->h), Str("Interpolation failed"));
    }
    for (i = 0; i < p->npoles; i++) {
      *cp++ = interMagn[i];
      *cp++ = interPhas[i];
  ...
  /*  if (csound->oparms->odebug) {
        csound->Message(csound,
            "phase:%lx fract:%6.2f rmsr:%6.2f rmso:%6.2f kerr:%6.2f
     kcps:%6.2f\n", framphase,fract,*p->krmr,*p->krmo,*p->kerr,*p->kcps); cp =
     p->kcoefs; nn = p->npoles; do { csound->Message(csound, " %6.2f",*cp++); }
     while (--nn); csound->Message(csound, "\n");
      }  */
  return OK;
}
...
int32_t lpformant(CSOUND *csound, LPFORM *p) {
  ...
  j = (ndx < 1 ? 1
               : (ndx >= p->lpread->npoles / 2 ? p->lpread->npoles / 2 : ndx)) -
      1;
  if (bws[j] > sr / 2 || isnan(bws[j]))
    bws[j] = sr / 2;
  if (bws[j] < 1.0)
    bws[j] = 1.0;
  if (cfs[j] > sr / 2 || isnan(cfs[j]))
    cfs[j] = sr / 2;
  cfs[j] = FABS(cfs[j]);
  *p->kcf = cfs[j];
  *p->kbw = bws[j];
...
int32_t lpfreson(CSOUND *csound, LPFRESON *p) {
  ...
  if (*p->kfrqratio != p->prvratio) { /* for new freqratio */
    if (*p->kfrqratio <= FL(0.0)) {
      return csound->PerfError(csound, &(p->h), Str("illegal frqratio, %5.2f"),
                               *p->kfrqratio);
    } /*      calculate d  */
...
int32_t lpslotset(CSOUND *csound, LPSLOT *p) {
  ...
  if (UNLIKELY(n < 0))
    ...
    if (n >= csound->max_lpc_slot) {
      ...
      csound->lprdaddr = csound->ReAlloc(
          csound, csound->lprdaddr, csound->max_lpc_slot * sizeof(LPREAD **));
    }
...
int32_t limit(CSOUND *csound, LIMIT *p) {
  ...
  if (UNLIKELY(offset))
    memset(ans, '\0', offset * sizeof(MYFLT));
  if (UNLIKELY(early)) {
    nsmps -= early;
